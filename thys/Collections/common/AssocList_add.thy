...
lemma clearjunk_map_ran: "AList.clearjunk (map_ran f al) = map_ran f (AList.clearjunk al)"
...
lemma length_update: 
  "length (AList.update k v xs) = (if k \<in> fst ` set xs then length xs else Suc (length xs))"
by(induct xs) simp_all

lemma length_distinct: 
  "distinct (map fst xs) \<Longrightarrow> length (AList.delete k xs) = (if k \<in> fst ` set xs then length xs - 1 else length xs)"
