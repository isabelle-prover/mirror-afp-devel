...
definition lsi_empty :: "unit \<Rightarrow> 'a lsi" where "lsi_empty == (\<lambda>_::unit. [])"
...
by(unfold_locales)(simp, unfold lsi_invar_def lsi_\<alpha>_def lsi_iteratei_def, rule set_iterator_foldli_correct)

lemma lsi_isEmpty_impl: "set_isEmpty lsi_\<alpha> lsi_invar lsi_isEmpty"
by(unfold_locales)(auto simp add: lsi_defs)

lemmas lsi_union_impl = it_union_correct[OF lsi_iteratei_impl lsi_ins_impl, folded lsi_union_def] 

lemmas lsi_inter_impl = it_inter_correct[OF lsi_iteratei_impl lsi_memb_impl lsi_empty_impl lsi_ins_dj_impl, folded lsi_inter_def]

lemma lsi_union_dj_impl: "set_union_dj lsi_\<alpha> lsi_invar lsi_\<alpha> lsi_invar lsi_\<alpha> lsi_invar lsi_union_dj"
by(unfold_locales)(auto simp add: lsi_defs)

lemmas lsi_image_filter_impl = it_image_filter_correct[OF lsi_iteratei_impl lsi_empty_impl lsi_ins_impl, folded lsi_image_filter_def]
lemmas lsi_inj_image_filter_impl = it_inj_image_filter_correct[OF lsi_iteratei_impl lsi_empty_impl lsi_ins_dj_impl, folded lsi_inj_image_filter_def]
...
lemmas lsi_sel'_impl = iti_sel'_correct[OF lsi_iteratei_impl, folded lsi_sel'_def]
