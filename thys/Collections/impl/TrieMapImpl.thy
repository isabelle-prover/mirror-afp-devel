...
definition tm_empty :: "unit \<Rightarrow> ('k, 'v) tm"
where "tm_empty == (\<lambda>_::unit. Trie.empty)"
...
definition tm_iteratei :: "('k, 'v) tm \<Rightarrow> ('k list \<times> 'v, '\<sigma>) set_iterator"
where
