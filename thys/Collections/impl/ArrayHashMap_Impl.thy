...
proof 
  fix m :: "('a \<times> 'b) list array"
  assume invar_m: "ahm_invar_aux n m"
  obtain ms where m_eq[simp]: "m = Array ms" by (cases m)

  show "finite (dom (ahm_\<alpha>_aux m))" by (metis finite_dom_ahm_\<alpha>_aux invar_m)
...
proof
  fix hm :: "('a, 'b) hashmap"
  assume invar_hm: "ahm_invar hm"
  obtain A n where hm_eq [simp]: "hm = HashMap A n" by(cases hm)

  from map_iteratei.iteratei_rule [OF ahm_iteratei_aux_impl, of n A] invar_hm
  show map_it: "map_iterator (ahm_iteratei hm) (ahm_\<alpha> hm)" by simp 

  from map_iterator_finite [OF map_it] show "finite (dom (ahm_\<alpha> hm))" .
qed

lemma ahm_iteratei_aux_code [code]:
  "ahm_iteratei_aux a c f \<sigma> = idx_iteratei array_get array_length a c (\<lambda>x. foldli x c f) \<sigma>"
proof(cases a)
  case (Array xs)[simp]
  ...
  also have "\<dots> = idx_iteratei op ! length xs c (\<lambda>x. foldli x c f) \<sigma>" by (simp add: idx_iteratei_nth_length_conv_foldli)
...
proof -
  ...
  proof(rule ahm.iteratei_rule_P)
