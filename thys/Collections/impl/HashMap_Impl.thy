...
definition rm_map_entry 
  ...
  -- "Empty hashmap"
definition empty :: "unit \<Rightarrow> ('k :: hashable, 'v) hm_impl" where "empty == rm_empty"
...
definition sel :: "('k::hashable,'v) hm_impl \<Rightarrow> ('k \<times> 'v \<Rightarrow> 'r option) \<Rightarrow> 'r option" 
...
definition "iteratei m c f \<sigma>0 ==
  rm_iteratei m c (\<lambda>(hc, lm) \<sigma>. 
    lm_iteratei lm c f \<sigma>
  ) \<sigma>0"

lemma iteratei_alt_def :
  "iteratei m = set_iterator_image snd (
     set_iterator_product (rm_iteratei m) (\<lambda>hclm. lm_iteratei (snd hclm)))"
proof -
  have aux: "\<And>c f. (\<lambda>(hc, lm). lm_iteratei lm c f) = (\<lambda>m. lm_iteratei (snd m) c f)"
    by auto
  show ?thesis
...
proof -
  ...
  have it_rm: "map_iterator (rm_iteratei hm) (rm_\<alpha> hm)" by simp

  from map_iteratei.iteratei_rule[OF lm_iteratei_impl]
  have it_lm: "\<And>lm. map_iterator (lm_iteratei lm) (lm_\<alpha> lm)" by simp
 
  from set_iterator_product_correct [OF it_rm, of "\<lambda>hclm. lm_iteratei (snd hclm)"
     "\<lambda>hclm. map_to_set (lm_\<alpha> (snd hclm))", OF it_lm]
  have it_prod: "set_iterator
         (set_iterator_product (rm_iteratei hm) (\<lambda>hclm. lm_iteratei (snd hclm)))
         (SIGMA hclm:map_to_set (rm_\<alpha> hm). map_to_set (lm_\<alpha> (snd hclm)))" by simp
 
  show ?thesis unfolding iteratei_alt_def
  proof (rule set_iterator_image_correct[OF it_prod])
    from invar
    show "inj_on snd
       (SIGMA hclm:map_to_set (rm_\<alpha> hm). map_to_set (lm_\<alpha> (snd hclm)))"
  ...
  next
    ...
    show "map_to_set (ahm_\<alpha> (hm_\<alpha>' hm)) =
          snd ` (SIGMA hclm:map_to_set (rm_\<alpha> hm). map_to_set (lm_\<alpha> (snd hclm)))" 
      apply (simp add: inj_on_def invar_def ahm_invar_def hm_\<alpha>'_def Ball_def 
