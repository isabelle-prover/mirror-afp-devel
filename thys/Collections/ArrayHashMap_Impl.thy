...
lemma AL_iteratei_aux_conv_foldli: "Assoc_List.iteratei_aux c f = foldli c (\<lambda>(k, v). f k v)"
proof
  fix xs
  show "Assoc_List.iteratei_aux c f xs = foldli c (\<lambda>(k, v). f k v) xs"
    by(induct xs)(auto simp add: fun_eq_iff)
qed

lemma idx_iteratei_aux_array_get_Array_conv_nth:
  "idx_iteratei_aux array_get sz i c f (Array xs) \<sigma> = idx_iteratei_aux op ! sz i c f xs \<sigma>"
apply(induct get\<equiv>"op ! :: 'b list \<Rightarrow> nat \<Rightarrow> 'b" sz i c f xs \<sigma> rule: idx_iteratei_aux.induct)
apply(subst (1 2) idx_iteratei_aux.simps)
apply simp
done

lemma idx_iteratei_array_get_Array_conv_nth:
  "idx_iteratei array_get array_length c f (Array xs) = idx_iteratei nth length c f xs"
...
lemma idx_iteratei_aux_nth_conv_foldli_drop:
  ...
  shows "idx_iteratei_aux op ! (length xs) i c f xs \<sigma> = foldli c f (drop (length xs - i) xs) \<sigma>"
using assms
proof(induct get\<equiv>"op ! :: 'b list \<Rightarrow> nat \<Rightarrow> 'b" sz\<equiv>"length xs" i c f xs \<sigma> rule: idx_iteratei_aux.induct)
  case (1 i c f l \<sigma>)
  show ?case
  ...
  next
    ...
    hence "idx_iteratei_aux op ! (length l) i c f l \<sigma> = idx_iteratei_aux op ! (length l) (i - 1) c f l (f (l ! (length l - i)) \<sigma>)"
      by(subst idx_iteratei_aux.simps) simp
    also have "\<dots> = foldli c f (drop (length l - (i - 1)) l) (f (l ! (length l - i)) \<sigma>)"
    ...
    hence "foldli c f (drop (length l - (i - 1)) l) (f (l ! (length l - i)) \<sigma>) = foldli c f (drop (length l - i) l) \<sigma>"
      using c by simp
    finally show ?thesis .
  qed
qed
...
definition ahm_empty :: "('key :: hashable, 'val) hashmap"
where "ahm_empty \<equiv> new_hashmap_with (def_hashmap_size TYPE('key))"
...
primrec ahm_iteratei_aux :: "((('key :: hashable) \<times> 'val) list array, 'key, 'val, '\<sigma>) map_iteratori"
where "ahm_iteratei_aux c f (Array xs) = Assoc_List.iteratei_aux c f (concat xs)"

primrec ahm_iteratei :: "(('key :: hashable, 'val) hashmap, 'key, 'val, '\<sigma>) map_iteratori"
where
  "ahm_iteratei c f (HashMap a n) = ahm_iteratei_aux c f a"

definition ahm_rehash_aux' :: "nat \<Rightarrow> 'key \<Rightarrow> 'val \<Rightarrow> (('key :: hashable) \<times> 'val) list array \<Rightarrow> ('key \<times> 'val) list array"
where
  "ahm_rehash_aux' n k v a =
   (let h = bounded_hashcode n k
    in array_set a h ((k, v) # array_get a h))"

definition ahm_rehash_aux :: "(('key :: hashable) \<times> 'val) list array \<Rightarrow> nat \<Rightarrow> ('key \<times> 'val) list array"
where
  "ahm_rehash_aux a sz = ahm_iteratei_aux (\<lambda>x. True) (ahm_rehash_aux' sz) a (new_array [] sz)"
...
proof(cases a)
  ...
  also have "length (concat xs) = foldl op + 0 (map length xs)"
    by (simp add: length_concat listsum_def foldr_conv_foldl)
  also from inv
...
lemma ahm_iteratei_aux_impl:
  "map_iteratei ahm_\<alpha>_aux (ahm_invar_aux n) ahm_iteratei_aux"
proof -
  interpret ahm!: finite_map ahm_\<alpha>_aux "ahm_invar_aux n"
    by(rule finite_map_ahm_\<alpha>_aux)
  show ?thesis
  proof(unfold_locales)
    ...
    assume inv: "ahm_invar_aux n A"
      and I\<sigma>0: "I (dom (ahm_\<alpha>_aux A)) \<sigma>0"
      and Ipres: "\<And>k v it \<sigma>. \<lbrakk>c \<sigma>; k \<in> it; ahm_\<alpha>_aux A k = Some v; it \<subseteq> dom (ahm_\<alpha>_aux A); I it \<sigma>\<rbrakk> \<Longrightarrow> I (it - {k}) (f k v \<sigma>)"
    obtain xs where A [simp]: "A = Array xs" by(cases A)
    
    from inv have "distinct (map fst (concat xs))" 
      by(auto intro: ahm_invar_distinct_fst_concatD)
    moreover
    from I\<sigma>0 inv
    have "I (dom (map_of (concat xs))) \<sigma>0"
      by(simp add: ahm_\<alpha>_aux_conv_map_of_concat)
    moreover {
      fix k v it \<sigma>
      assume "c \<sigma>" "k \<in> it" "map_of (concat xs) k = Some v"
        and "it \<subseteq> dom (map_of (concat xs))" "I it \<sigma>"
      from `map_of (concat xs) k = Some v` `it \<subseteq> dom (map_of (concat xs))` inv
      have "ahm_\<alpha>_aux A k = Some v" "it \<subseteq> dom (ahm_\<alpha>_aux A)"
        by(simp_all add: ahm_\<alpha>_aux_conv_map_of_concat)
      with `c \<sigma>` `k \<in> it` have "I (it - {k}) (f k v \<sigma>)"
        using `I it \<sigma>` by(rule Ipres) }
    ultimately
    have "I {} (Assoc_List.iteratei_aux c f (concat xs) \<sigma>0) \<or>
      (\<exists>it\<subseteq>dom (map_of (concat xs)). it \<noteq> {} \<and> \<not> c (Assoc_List.iteratei_aux c f (concat xs) \<sigma>0) \<and> I it (Assoc_List.iteratei_aux c f (concat xs) \<sigma>0))"
      by(rule iteratei_aux_correct)
    thus "I {} (ahm_iteratei_aux c f A \<sigma>0) \<or>
      (\<exists>it\<subseteq>dom (ahm_\<alpha>_aux A). it \<noteq> {} \<and> \<not> c (ahm_iteratei_aux c f A \<sigma>0) \<and> I it (ahm_iteratei_aux c f A \<sigma>0))"
      using inv by(simp add: ahm_\<alpha>_aux_conv_map_of_concat)
  qed
qed
...
lemma ahm_iteratei_correct:
  ...
  and I\<sigma>0: "I (dom (ahm_\<alpha> hm)) \<sigma>0"
  and Ipres: "\<And>k v it \<sigma>. \<lbrakk>c \<sigma>; k \<in> it; ahm_\<alpha> hm k = Some v; it \<subseteq> dom (ahm_\<alpha> hm); I it \<sigma>\<rbrakk> \<Longrightarrow> I (it - {k}) (f k v \<sigma>)"
  shows "I {} (ahm_iteratei c f hm \<sigma>0) \<or>
        (\<exists>it\<subseteq>dom (ahm_\<alpha> hm). it \<noteq> {} \<and> \<not> c (ahm_iteratei c f hm \<sigma>0) \<and> I it (ahm_iteratei c f hm \<sigma>0))"
proof -
  obtain A n where hm [simp]: "hm = HashMap A n" by(cases hm)
  then obtain xs where A [simp]: "A = Array xs" by(cases A)
  ...
  moreover {
    ...
    assume "c \<sigma>" "k \<in> it" "map_of (concat xs) k = Some v"
      and "it \<subseteq> dom (map_of (concat xs))" "I it \<sigma>"
    from `map_of (concat xs) k = Some v` `it \<subseteq> dom (map_of (concat xs))` inv
    have "ahm_\<alpha> hm k = Some v" "it \<subseteq> dom (ahm_\<alpha> hm)"
      by(simp_all add: ahm_\<alpha>_aux_conv_map_of_concat)
    with `c \<sigma>` `k \<in> it` have "I (it - {k}) (f k v \<sigma>)"
      using `I it \<sigma>` by(rule Ipres) }
  ultimately
  have "I {} (Assoc_List.iteratei_aux c f (concat xs) \<sigma>0) \<or>
        (\<exists>it\<subseteq>dom (map_of (concat xs)). it \<noteq> {} \<and> \<not> c (Assoc_List.iteratei_aux c f (concat xs) \<sigma>0) \<and> I it (Assoc_List.iteratei_aux c f (concat xs) \<sigma>0))"
    by(rule iteratei_aux_correct)
  thus ?thesis using inv by(simp add: ahm_\<alpha>_aux_conv_map_of_concat)
qed

lemma ahm_iteratei_aux_code [code]:
  "ahm_iteratei_aux c f a \<sigma> = idx_iteratei array_get array_length c (Assoc_List.iteratei_aux c f) a \<sigma>"
proof(cases a)
  case (Array xs)[simp]
  have "ahm_iteratei_aux c f a \<sigma> = Assoc_List.iteratei_aux c f (concat xs) \<sigma>" by simp
  also have "\<dots> = foldli c (\<lambda>(x, y). f x y) (concat xs) \<sigma>" by(simp add: AL_iteratei_aux_conv_foldli)
  also have "\<dots> = foldli c (foldli c (\<lambda>(x, y). f x y)) xs \<sigma>" by(simp add: foldli_concat)
  also have "foldli c (\<lambda>(x, y). f x y) = Assoc_List.iteratei_aux c f" by(simp add: AL_iteratei_aux_conv_foldli)
  also have "foldli c (Assoc_List.iteratei_aux c f) xs \<sigma> = idx_iteratei op ! length c (Assoc_List.iteratei_aux c f) xs \<sigma>"
    by(simp add: idx_iteratei_nth_length_conv_foldli)
  also have "\<dots> = idx_iteratei array_get array_length c (Assoc_List.iteratei_aux c f) a \<sigma>"
...
lemma array_length_ahm_rehash_aux':
  "array_length (ahm_rehash_aux' n k v a) = array_length a"
...
proof -
  interpret ahm!: map_iteratei "ahm_\<alpha>_aux" "ahm_invar_aux n" "ahm_iteratei_aux"
  ...
  from inv have "?I {} ?a \<or> (\<exists>it\<subseteq>dom (ahm_\<alpha>_aux a). it \<noteq> {} \<and> \<not> True \<and> ?I it ?a)"
    unfolding ahm_rehash_aux_def
  proof(rule ahm.iteratei_rule)
