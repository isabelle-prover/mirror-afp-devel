...
theory ArrayHashMap imports 
  ArrayHashMap_Impl
begin

text {*
  Array based hash maps
  Abbreviations: ahm,a
*}
...
where "ahm_empty \<equiv> HashMap ArrayHashMap_Impl.ahm_empty"
...
definition ahm_iteratei :: "(('key :: hashable, 'val) hashmap, 'key, 'val, '\<sigma>) map_iteratori"
where "ahm_iteratei c f hm = ArrayHashMap_Impl.ahm_iteratei c f (impl_of hm)"
...
lemma impl_of_ahm_empty [code abstract]:
  "impl_of ahm_empty = ArrayHashMap_Impl.ahm_empty"
by(simp add: ahm_empty_def HashMap_inverse)
...
definition "ahm_iterate == iti_iterate ahm_iteratei"

definition "ahm_add == it_add ahm_update ahm_iterate"
...
by(simp add: ahm_\<alpha>_def ahm_empty_def HashMap_inverse)
...
proof(unfold_locales)
  ...
  assume "I (dom (ahm_\<alpha> hm)) \<sigma>0"
    and "\<And>k v it \<sigma>. \<lbrakk>c \<sigma>; k \<in> it; ahm_\<alpha> hm k = Some v; it \<subseteq> dom (ahm_\<alpha> hm); I it \<sigma>\<rbrakk> \<Longrightarrow> I (it - {k}) (f k v \<sigma>)"
  with ahm_invar_impl_of[of hm]
  show "I {} (ahm_iteratei c f hm \<sigma>0) \<or>
        (\<exists>it\<subseteq>dom (ahm_\<alpha> hm). it \<noteq> {} \<and> \<not> c (ahm_iteratei c f hm \<sigma>0) \<and> I it (ahm_iteratei c f hm \<sigma>0))"
    unfolding ahm_iteratei_def ahm_\<alpha>_def by(rule ahm_iteratei_correct)
qed
...
lemma ahm_iterate_impl: "map_iterate ahm_\<alpha> ahm_invar ahm_iterate"
unfolding ahm_iterate_def by(rule map_iteratei.iti_iterate_correct)(rule ahm_iteratei_impl)

lemma ahm_add_impl: "map_add ahm_\<alpha> ahm_invar ahm_add"
unfolding ahm_add_def by(rule it_add_correct)(rule ahm_iterate_impl ahm_update_impl)+
...
interpretation ahm!: map_iterate ahm_\<alpha> ahm_invar ahm_iterate 
  using ahm_iterate_impl .
