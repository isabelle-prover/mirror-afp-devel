...
*)
header {* \chapter{Implementations} \label{ch:Impl} \isaheader{Map Implementation by Associative Lists} *}
theory ListMapImpl
...
text {*
  In this theory, we implement maps by associative lists.
...
definition "lm_empty == Assoc_List.empty"
...
by(unfold_locales)(unfold lm_defs, rule iteratei_correct)
