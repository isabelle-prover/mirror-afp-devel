...
in that it is always required in order to act on any purpose (no matter which one). As such we can reasonably demand that
...
we add the following as an a priori characteristic of FWB: The state of affairs of some individual (e.g. "I") enjoying 
freedom and well-being is contingent.}\<close>
...
subsection \<open>Formal Proof of Gewirth's PGC\<close>
...
text\<open>\noindent{(5) I have (maybe nobody else does) a claim right to my FWB.}\<close>
text\<open>\noindent{(13) Every PPA has a claim right to their FWB.}\<close>

text\<open>\noindent{In the following we present a formalised proof for the Principle of Generic Consistency (PGC):
"Every PPA has a claim right to its freedom and well-being".}\<close>
theorem PGC: shows "\<forall>C. \<lfloor>PPA (Agent C) \<^bold>\<rightarrow> (RightTo (Agent C) FWB)\<rfloor>\<^sub>C"
proof - {
