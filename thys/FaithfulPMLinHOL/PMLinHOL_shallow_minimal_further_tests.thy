...
lemma T1b: "\<Turnstile>\<^sup>m \<box>\<^sup>mp\<^sup>m \<supset>\<^sup>m ((\<diamond>\<^sup>mq\<^sup>m) \<supset>\<^sup>m \<diamond>\<^sup>m(p\<^sup>m \<and>\<^sup>m q\<^sup>m))" \<comment>\<open>alternative interactive proof in modal object logic K\<close> 
  proof -   
