...
lemma HCderived1: "\<Turnstile>\<^sup>d (\<phi> \<supset>\<^sup>d \<phi>)" \<comment>\<open>sledgehammer(HC1 HC2 HC3 MP) returns: by (metis HC1 HC2 MP)\<close>  
proof -  
  have 1: "\<Turnstile>\<^sup>d \<phi> \<supset>\<^sup>d ((\<psi> \<supset>\<^sup>d \<phi>) \<supset>\<^sup>d \<phi>)" using H1 by auto 
...
lemma cM:  "reflexive R \<longleftrightarrow> (\<forall>\<phi> W V. \<forall>w:W. \<langle>W,R,V\<rangle>,w \<Turnstile>\<^sup>d \<box>\<^sup>d\<phi> \<supset>\<^sup>d \<phi>)" \<comment>\<open>sledgehammer: Proof found\<close> oops (**)
...
lemma c4a: "transitive R \<longrightarrow> (\<forall>\<phi> W V. \<forall>w:W. \<langle>W,R,V\<rangle>,w \<Turnstile>\<^sup>d \<box>\<^sup>d\<phi> \<supset>\<^sup>d \<box>\<^sup>d(\<box>\<^sup>d\<phi>))" by (metis RelativeTruthD.simps)  
