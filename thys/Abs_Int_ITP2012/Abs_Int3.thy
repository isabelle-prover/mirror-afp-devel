...
definition "widen_ivl ivl1 ivl2 =
  (\<^cancel>\<open>if is_empty ivl1 then ivl2 else
   if is_empty ivl2 then ivl1 else\<close>
...
definition "narrow_ivl ivl1 ivl2 =
  (\<^cancel>\<open>if is_empty ivl1 \<or> is_empty ivl2 then empty else\<close>
     case (ivl1,ivl2) of (I l1 h1, I l2 h2) \<Rightarrow>
...
abbreviation widen_acom :: "('a::WN)acom \<Rightarrow> 'a acom \<Rightarrow> 'a acom" (infix \<open>\<nabla>\<^sub>c\<close> 65)
where "widen_acom == map2_acom (\<nabla>)"

abbreviation narrow_acom :: "('a::WN)acom \<Rightarrow> 'a acom \<Rightarrow> 'a acom" (infix \<open>\<triangle>\<^sub>c\<close> 65)
