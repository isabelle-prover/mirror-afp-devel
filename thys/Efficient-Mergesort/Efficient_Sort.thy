...
\<close>
inductive linked for P :: "'a \<Rightarrow> 'a \<Rightarrow> bool"
  where
    Nil [iff]: "linked P []"
...
fun take_chain :: "'a \<Rightarrow> ('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a list \<Rightarrow> 'a list"
  where
    "take_chain a P [] = []"
...
fun drop_chain :: "'a \<Rightarrow> ('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a list \<Rightarrow> 'a list"
  where
    "drop_chain a P [] = []"
...
lemma linked_append:
  "linked P (xs @ ys) \<longleftrightarrow>
    linked P xs \<and> linked P ys \<and>
    (if xs \<noteq> [] \<and> ys \<noteq> [] then P (last xs) (hd ys) else True)"
  (is "?lhs = ?rhs")
proof
  assume "?lhs" then show "?rhs"
  proof (induct xs)
    case (Cons x xs) then show ?case by (cases xs, simp_all) (cases ys, auto)
  qed simp
next
  assume "?rhs" then show "?lhs"
  proof (induct xs)
    case (Cons x xs) then show ?case by (cases ys, auto) (cases xs, auto)
  qed simp
qed
...
proof
  assume "sorted xs" then show "linked (\<le>) xs"
  proof (induct xs)
...
abbreviation (in linorder) (input) lt :: "('b \<Rightarrow> 'a) \<Rightarrow> 'b \<Rightarrow> 'b \<Rightarrow> bool"
  where
    "lt key x y \<equiv> key x < key y"

abbreviation (in linorder) (input) le :: "('b \<Rightarrow> 'a) \<Rightarrow> 'b \<Rightarrow> 'b \<Rightarrow> bool"
  where
    "le key x y \<equiv> key x \<le> key y"

abbreviation (in linorder) (input) gt :: "('b \<Rightarrow> 'a) \<Rightarrow> 'b \<Rightarrow> 'b \<Rightarrow> bool"
  where
    "gt key x y \<equiv> key x > key y"

abbreviation (in linorder) (input) ge :: "('b \<Rightarrow> 'a) \<Rightarrow> 'b \<Rightarrow> 'b \<Rightarrow> bool"
  where
...
proof (induct xs arbitrary: a bs)
  case (Cons x xs) then show ?case by (cases "key a < key x") simp_all
qed simp
...
proof (induct key xs rule: sequences_induct)
  ...
  then show ?case using sorted_rev_take_chain_gt_append [of key b "[a]" xs]
...
next
  case (snoc x xs)
  then have IH: "[y\<leftarrow>dropWhile ?P xs. key z = key y] = []"
...
lemma filter_by_key_takeWhile [simp]:
  assumes "sorted (map key xs)"
  shows "[y\<leftarrow>takeWhile (\<lambda>x. key x \<le> key z) xs. key z = key y] = [y\<leftarrow>xs. key z = key y]"
...
next
  case (snoc x xs)
  then have IH: "[y\<leftarrow>takeWhile ?P xs. key z = key y] = [y\<leftarrow>xs. key z = key y]"
    by (auto simp: sorted_append)
  show ?case
  proof (cases "\<forall>z\<in>set xs. ?P z")
    case True
