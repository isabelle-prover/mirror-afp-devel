...
proof (eval_winding,simp_all)
  ...
  moreover have "cindex_pathE CR \<i> = -1"
    ...
    by (simp add: cindex_pathE_circlepath_upper(1)) 
...
proof (eval_winding)    
  ...
  show l1: "z \<notin> path_image ?l1"
    using closed_segment_same_Im order_asms(3) by fastforce 
  show l2:"z \<notin> path_image ?l2"
    using closed_segment_same_Re order_asms(2) by auto 
  show l3:"z \<notin> path_image ?l3"
    using closed_segment_same_Im order_asms(4) by auto 
  show l4:"z \<notin> path_image ?l4"
    using closed_segment_same_Re order_asms(1) by auto
  ...
  proof -
    ...
    then have "cindex_pathE ?l3 z = -1"
      using l3 order_asms by (simp add: cindex_pathE_linepath algebra_simps)
    ...
    then have "cindex_pathE ?l1 z = -1"    
      using l1 order_asms by (simp add: cindex_pathE_linepath algebra_simps)  
    moreover have "cindex_pathE ?l2 z = 0"    
      using l2 order_asms by (simp add: cindex_pathE_linepath algebra_simps)  
    moreover have "cindex_pathE ?l4 z = 0"
      using l4 order_asms by (simp add: cindex_pathE_linepath algebra_simps)
