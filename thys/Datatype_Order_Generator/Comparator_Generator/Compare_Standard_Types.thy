...
text \<open>For all of the following types, we define comparators and register them in the class 
  @{class compare}:
  @{type int}, @{type nat}, @{type bool}, @{type sum}, @{type option}, @{type list},
...
text \<open>We do not use the linear order to define the comparator for bools, but implement 
  a more efficient one.\<close>
...
text \<open>It is not directly possible to "derive (linorder) bool", since 
...
derive (linorder) compare_order bool
end