...
lemma coeff_mult_C[simp]: "coeff (a * C x) n = coeff a n * x"
  by (simp add: coeff_mult coeff_C_eq if_distrib[where f="\<lambda>x. a * x" for a] setsum.If_cases)

lemma coeff_C_mult[simp]: "coeff (C x * a) n = x * coeff a n"
  by (simp add: coeff_mult coeff_C_eq if_distrib[where f="\<lambda>x. x * a" for a] setsum.If_cases)
...
lemma degree_setsum_le_Max:
  "finite F \<Longrightarrow> degree (setsum f F) \<le> Max ((\<lambda>x. degree (f x))`F)"
  by (intro degree_setsum_le) (auto intro!: Max.coboundedI)
...
proof -
  ...
  show ?thesis
    using n
    by (simp add: poly_eq_iff coeff_setsum coeff_eq_0 setsum.If_cases eq coeff_pow_X
...
lemma degree_setsum_unique:
  ...
  using degree_setsum_unique'[of "I - {i}" i p] by (auto simp: insert_absorb)

lemma coeff_setsum_unique:
...
proof -
  have "(\<Sum>j\<in>I. coeff (p j) (degree (p i))) = (\<Sum>i\<in>{i}. coeff (p i) (degree (p i)))"
    using I by (intro setsum.mono_neutral_cong_right) (auto intro!: coeff_eq_0)
  then show ?thesis
...
lemma setsum_telescope_Ico: "a \<le> b \<Longrightarrow> (\<Sum>i=a ..< b. f i - f (Suc i) ::_::ab_group_add) = f a - f b"
  by (induction b rule: dec_induct) auto

lemmas map_sq_matrix = map_sq_matrix_diff map_sq_matrix_add map_sq_matrix_smult map_sq_matrix_setsum
...
proof -
  ...
  have degree_f_id: "degree (?f (\<lambda>i. i)) = CARD('n)"
    using coeff_mult_setprod_setsum[of "\<lambda>i. to_fun ?B i i" UNIV]
  ...
  have degree_less: "\<And>p. p \<noteq> id \<Longrightarrow> degree (?f p) < degree (?f (\<lambda>i. i))"
    ...
    by (rule le_less_trans[OF degree_setprod_le])
       (auto simp add: dB setsum.If_cases set_eq_iff intro!: psubset_card_mono)
  ...
  proof (subst coeff_setsum_unique)
...
lemma degree_le_max_perm_degree: "degree (det A) \<le> max_perm_degree A"
  unfolding det_eq
  by (rule order_trans[OF degree_setsum_le_Max])
    ...
    proof (safe intro!: Max.boundedI)
      ...
      have "max_perm_degree (?M j i) = card (UNIV - {i, j})"
        by (intro max_perm_degree_eqI)
           (auto simp: M setsum.If_cases if_distrib[of degree] simp del: card_Diff_insert
  ...
  next
    ...
    proof (subst degree_det)
      have "CARD('n) - 1 = (\<Sum>i\<in>UNIV. ?D x x i i)"
        by (simp add: M if_distrib[where f="degree"] setsum.If_cases Collect_neg_eq Compl_eq_Diff_UNIV)
      also have "\<dots> = degree (?P x x id)"
        by (auto intro!: antisym degree_setprod_le le_degree simp add: coeff_mult_setprod_setsum)
      ...
      have "degree (?P x x p) \<le> card (UNIV - {i, j})"
        ...
        by (rule order_trans)
           (auto simp: M if_distrib[where f="degree"] setsum.If_cases Collect_neg_eq Compl_eq_Diff_UNIV p intro!: card_mono)
      also have "\<dots> < CARD('n) - 1"
...
lemma map_sq_matrix_mult_eq_smult[simp]: "map_sq_matrix (op * a) M = a *\<^sub>S M"
  by transfer rule
...
text %visible \<open>\hrulefill ~~ Part 2 ~~ \hrulefill\<close>
  ...
  also have "(\<Sum>i\<le>n. X^i *\<^sub>S \<^bold>C (A * B i)) =
    ...
    by %invisible (simp add: Zero_notin_Suc monom_0 setsum.reindex one_poly_def[symmetric] diag_mult)
  finally have diag_charpoly:
    "charpoly A *\<^sub>S 1 = X^(n + 1) *\<^sub>S \<^bold>C (B n) +
      (\<Sum>i<n. X^(i + 1) *\<^sub>S \<^bold>C (B i - A * B (i + 1))) - \<^bold>C (A * B 0)"
    by %invisible (simp add: map_sq_matrix_diff C_linear setsum_subtractf smult_diff)
text %visible \<open>\hrulefill ~~ Part 3 ~~ \hrulefill\<close>
  ...
  have "(\<Sum>i\<le>n+1. ?p i) = ?p 0 + (\<Sum>i<n. ?p (i + 1)) + ?p (n + 1)"
    unfolding %invisible setsum_atMost_Suc_shift Suc_eq_plus1[symmetric]
    by %invisible (simp add: lessThan_Suc_atMost[symmetric])
  also have "?p 0 = - ?AB 0"
    by %invisible (simp add: coeff_smult_1 diag_charpoly map_sq_matrix)
  also have "(\<Sum>i<n. ?p (i + 1)) = (\<Sum>i=0..<n. ?AB i - ?AB (i + 1))"
      by %invisible (rule setsum.cong)
         (auto simp: coeff_smult_1 coeff_pow_X diag_charpoly map_sq_matrix setsum_subtractf
                     if_distrib[where f="\<lambda>x. x a" for a] if_distrib[where f="\<lambda>x. a * x" for a]
                     field_simps setsum.If_cases power_Suc2
               simp del: power_Suc)
  also have "\<dots> = ?AB 0 - ?AB n"
    unfolding %invisible Suc_eq_plus1[symmetric]
    by %invisible (subst setsum_telescope_Ico) auto
