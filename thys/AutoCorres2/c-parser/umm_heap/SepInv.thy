...
lemma sep_map_lift_wp:
  ...
  subgoal for v
    apply(subst sep_map'_lift [where g=g and d=d])
     ...
     apply simp
    apply(rule sep_conj_impl_same [where P="p \<mapsto>\<^sup>i\<^sub>g v" and Q="P v"])
    apply(unfold sep_map_inv_def)
...
lemma ptr_retyp_sep_cut'_wp:
  ...
  by (rule sep_conj_impl_same [where P="g \<turnstile>\<^sub>s\<^sup>i p" and Q=P]) (simp add: ptr_retyp_sep_cut')
