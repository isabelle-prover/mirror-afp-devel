...
lemma singleton_t_dom [simp]:
  ...
  subgoal for a b y x2 x2a
   apply(cases "a \<in> {ptr_val p..+size_of TYPE('a)}")
...
lemma singleton_d_restrict_eq:
  ...
  apply(rule ext)
  subgoal for x
    apply(cases "x \<in> s_footprint p"; simp)
  ...
  moreover from l merge have "s\<^sub>1 ++ singleton p v h (proj_d s\<^sub>0) = s\<^sub>1 ++ s\<^sub>0 ++ singleton p v h d"
    ...
    subgoal for  x
      ...
      subgoal for a b
        ...
        apply(cases "(a,b) \<in> s_footprint p")
        subgoal premises prems
        proof -
          have "(s\<^sub>1 ++ singleton p v h (proj_d s\<^sub>0)) (a, b) = singleton p v h d (a, b)"
            using prems 
...
lemma map_td_f_eq':
  "(f=g) \<longrightarrow> (map_td f h  t = map_td g h t)"
  "(f=g) \<longrightarrow> (map_td_struct f h st = map_td_struct g h st)"
  "(f=g) \<longrightarrow> (map_td_list f h ts = map_td_list g h ts)"
  "(f=g) \<longrightarrow> (map_td_tuple f h x = map_td_tuple g h x)"
...
lemma sep_map_lift_wp_exc:
  ...
  subgoal for v
    apply(rule sep_conj_impl_same [where P="p \<mapsto>\<^sub>g v" and Q="P v"])
...
lemma heap_list_proj_h_sub:
  ...
  proof -
    have  "{p + 1..+n} \<subseteq> {p..+Suc n}" using prems by (fastforce intro: intvl_plus_sub_Suc)
...
lemma heap_list_s_map_add_super_update_bs:
  ...
  subgoal premises prems
  proof -
    have  "heap_list (proj_h (s\<^sub>0 ++ s\<^sub>1)) (k + z + (n - (k+z))) p =
...
lemma field_access_take_drop:
  ...
  "\<forall>s m n f. field_lookup_list ts f m = Some (s,n) \<longrightarrow> wf_fd_list ts \<longrightarrow>
      take (size_td s) (drop (n - m) (access_ti_list ts v (replicate (size_td_list ts) 0))) =
        access_ti\<^sub>0 s v"
  "\<forall>s m n f. field_lookup_tuple x f m = Some (s,n) \<longrightarrow> wf_fd_tuple x \<longrightarrow>
...
lemma sep_map_mfs_sep_map2:
  ...
  subgoal premises prems
  proof -
    have "field_footprint p f = s_footprint ((Ptr &(p\<rightarrow>f))::'b ptr)"
      using prems
...
lemma sep_map_field_unfold:
  ...
  apply(rule iffI)
  subgoal for x
    apply(rule sep_conjI [where  s\<^sub>0="x |` (dom x - fs_footprint p {f})" and
          s\<^sub>1="x |` fs_footprint p {f}"] )
  ...
  subgoal for s\<^sub>0 s\<^sub>1
    apply(frule fi_fu_consistentD [where v="heap_list_s s\<^sub>1 (size_td (typ_info_t TYPE('b))) (ptr_val p + of_nat n)" and
    ...
    apply(drule arg_cong [where f="access_ti\<^sub>0 (typ_info_t TYPE('b))"])
    apply(drule arg_cong [where f="\<lambda>bs. update_ti_t t bs v"])
...
lemma field_access_update_nth_disj:
  ...
  "\<forall>m f s n x bs bs'. field_lookup_tuple y f m = Some (s,n) \<longrightarrow> x < size_td_tuple y \<longrightarrow>
      (x < n -  m \<or> x \<ge> (n - m) + size_td s) \<longrightarrow> wf_fd_tuple y \<longrightarrow>length bs = size_td s \<longrightarrow> length bs' = size_td_tuple y \<longrightarrow>
...
proof -
  ...
  have "lift_state (h, ptr_retyp p d) = s\<^sub>1 ++ lift_state (h, ptr_retyp p d) |` dom s\<^sub>0"
    ...
    apply(rule ext)
    subgoal for x
      apply (cases "x \<in> dom s\<^sub>0")
       apply(cases "x \<in> dom s\<^sub>1")
  ...
  with d have "(g \<turnstile>\<^sub>s p \<and>\<^sup>* sep_true) (lift_state (h, ptr_retyp p d) |` dom s\<^sub>0)"
    ...
    apply(rule sep_conjI [where s\<^sub>0="lift_state (h,d) |` ({(a, b). a \<in> {ptr_val p..+size_of TYPE('a)}} - s_footprint p)"])
       apply (simp add: sep_conj_ac)
  ...
  show ?thesis
    by (subst sep_conj_assoc [symmetric])
       (rule sep_conjI [where s\<^sub>0="(lift_state (h,ptr_retyp p d))|`dom s\<^sub>0" and s\<^sub>1=s\<^sub>1],
...
proof (rule sep_conjI [where s\<^sub>0="s|`{(a,b). a \<in> {p..+unat x}}" and
                s\<^sub>1="s|`({(a,b). a \<in> {p..+unat y}} - {(a,b). a \<in> {p..+unat x}})"])
