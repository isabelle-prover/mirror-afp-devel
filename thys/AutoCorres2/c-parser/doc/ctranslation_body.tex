...
\chapter{Translation of the \strictc{} Dialect (Outdated)}
\chapterauthor{Michael Norrish}
\label{chap:strictc}
...
The type \texttt{statement} is actually a \texttt{statement\_node}
...
to take a list of \texttt{block\_item} values as an argument, where a
\texttt{block\_item} is either a statement or a declaration.
...
are actually given in an auxiliary type (\texttt{statement\_node}),
but the recursive constructors take arguments of type
\texttt{statement}.  The type \texttt{statement} is then a type that
is mutually recursive with \texttt{statement\_node}, and which has
just one constructor, \texttt{Stmt}.\footnote{It would be nice if one
  ...
  \texttt{statement\_node~wrap}, but SML doesn't permit this.  It must
...
\texttt{get\_rettype} function from \MLfile{program\_analysis},
...
by the function \texttt{Absyn.constify\_abtype}.
...
these is the \texttt{var\_info} type.  This stores information about
...
The \texttt{var\_info} type stores information about declared
...
\texttt{var\_info} also includes information about where the variable
...
$\{\texttt{cse}|\texttt{vi}\}$\texttt{\_fupd\_fld} defined, with type
\[
  (\tau \rightarrow \tau) \rightarrow \textsf{rcd} \rightarrow
  \textsf{rcd}
\]
where \textsf{rcd} is either \texttt{var\_info} or \texttt{csenv}.
...
type \texttt{char} in another.\footnote{If this is attempted, the
  ...
  is stored in the variable's \texttt{var\_info} record.}
...
\texttt{global\_var\_type} that in turn contains all of the global
...
These two special variables are part of \texttt{global\_var\_type} and
so must have Isabelle types themselves.  The type of \globexnvar{} is
the enumerated type \texttt{c\_exntype}, defined in the Isabelle
...
\texttt{mem\_type}\footnote{See \texttt{umm\_heap/CTypes.thy}.}, it
supports functions
\begin{verbatim}
...
that these types are indeed in the \texttt{mem\_type} class.  In
...
types are proved to be in the class \texttt{mem\_type} once and for
...
is not shown to be in the class \texttt{mem\_type}.
...
If type $\tau$ is a \texttt{mem\_type}, then an array of $\tau$ values
will also be a \texttt{mem\_type}, as long as the size of the array is
...
\begin{verbatim}
   datatype expr_info =
     EI of {lval   : (term -> term -> term) option,
            ...
            right  : SourcePos.t }
\end{verbatim}
\caption{The \texttt{expr\_info} type, into which C expressions are
  translated internally.}
\label{fig:exprinfo}
\end{figure}

The SML types given to the \texttt{lval}, \texttt{addr} and
\texttt{rval} fields in the declaration of \texttt{expr\_info} are
...
\end{verbatim}
where \texttt{rval\_of} returns the \texttt{rval} field of an
\texttt{expr\_info} and the $\beta$-reduction happens within SML.

The fourth line of the record in \texttt{expr\_info} values stores the
...
the \texttt{expr\_info} record store the source-code position of the
original expression.  The two guard fields are explained below in
Section~\ref{sec:undefined-behaviour}.

The \texttt{ibool} field of the \texttt{expr\_info} records whether or
...
\end{verbatim}
The function \texttt{mk\_isabool} produces an \texttt{expr\_info} value
...
\texttt{strip\_kb} reverses this, turning an Isabelle boolean into an
...
to be ignored, and \texttt{s\_t} taken as the new \emph{typedef-name}.
...
\subsection{GCC \texttt{\_\_attribute\_\_} Declarations}
\label{sec:gcc-attributes}

The parser handles, but mostly ignores, various \texttt{gcc}-specific
extensions, such as \texttt{\_\_attribute\_\_}.  These are tricky to parse
(something admitted by the relevant \texttt{gcc} documentation): users
are given almost unlimited liberty to put their \texttt{\_\_attribute\_\_}
