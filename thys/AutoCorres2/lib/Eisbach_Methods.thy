...
section \<open>Attribute methods (for use with \<open>rule_by_method\<close> attributes)\<close>
...
section \<open>Shortcuts for @{method prove_prop}. \<close>

text \<open>Note these are less efficient than using the raw syntax because
...
text \<open>
  ...
  brutal method.
  E.g. \<open>apply (solves_emerging frule x=... in my_rule\<close>\<open>fastforce simp: ... intro!: ... \<close>
\<close>
