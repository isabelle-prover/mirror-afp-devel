...
definition
  pred_imp :: "('a \<Rightarrow> bool) \<Rightarrow> ('a \<Rightarrow> bool) \<Rightarrow>  bool" 
where
  "pred_imp P Q \<equiv> \<forall>x. P x \<longrightarrow> Q x"

lemma pred_impI: "(\<And>x. P x \<Longrightarrow> Q x) \<Longrightarrow> pred_imp P Q"
...
lemma fold_to_map_of:
  ...
  apply (rule ext)
  subgoal for x 
    apply (cases "fold (++) (map (\<lambda>x. [f x \<mapsto> g x]) xs) Map.empty x")
     apply clarsimp
...
lemma all_rv_choice_fn_eq_pred:
  ...
  apply (rule exI[where x="\<lambda>rv. SOME h. f rv = g h"])
...
section \<open> Take, drop, zip, \<open>list_all\<close> etc rules \<close>
...
lemma sorted_wrt_hd_min: "\<lbrakk> \<And>x. P x x; sorted_wrt P xs \<rbrakk> \<Longrightarrow> (\<forall>x \<in> set xs. P (hd xs) x)"
  by (induction xs) auto

lemma disjoint_no_subset: "A \<inter> B = {} \<Longrightarrow> A \<noteq> {} \<Longrightarrow> A \<subseteq> B \<Longrightarrow> False"
  by blast
