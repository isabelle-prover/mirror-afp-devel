...
type_synonym dens_ctxt = "vname set \<times> vname set \<times> (vname \<Rightarrow> pdf_type) \<times> (state \<Rightarrow> ereal)"
type_synonym expr_density = "state \<Rightarrow> val \<Rightarrow> ereal"
...
definition branch_prob :: "dens_ctxt \<Rightarrow> state \<Rightarrow> ereal" where
  "branch_prob \<Y> \<rho> = emeasure (dens_ctxt_measure \<Y> \<rho>) (space (dens_ctxt_measure \<Y> \<rho>))"

lemma nonneg_branch_prob[simp]: "branch_prob \<Y> \<rho> \<ge> 0"
  by (simp add: branch_prob_def emeasure_nonneg)
...
definition apply_dist_to_dens :: "pdf_dist \<Rightarrow> (state \<Rightarrow> val \<Rightarrow> ereal) \<Rightarrow> (state \<Rightarrow> val \<Rightarrow> ereal)" where
...
definition insert_dens
     :: "vname set \<Rightarrow> vname set \<Rightarrow> expr_density \<Rightarrow> (state \<Rightarrow> ereal) \<Rightarrow> state \<Rightarrow> ereal" where
  "insert_dens V V' f \<delta> \<equiv> \<lambda>\<sigma>. \<delta> (remove_var \<sigma>) * f (remove_var \<sigma>) (\<sigma> 0)"

definition if_dens :: "(state \<Rightarrow> ereal) \<Rightarrow> (state \<Rightarrow> val \<Rightarrow> ereal) \<Rightarrow> bool \<Rightarrow> (state \<Rightarrow> ereal)" where
  "if_dens \<delta> f b \<equiv> \<lambda>\<sigma>. \<delta> \<sigma> * f \<sigma> (BoolVal b)"

definition if_dens_det :: "(state \<Rightarrow> ereal) \<Rightarrow> expr \<Rightarrow> bool \<Rightarrow> (state \<Rightarrow> ereal)" where
...
proof (intro borel_measurable_ereal_times assms measurable_If)
...
locale density_context =
  ...
  assumes subprob_space_dens:
      ...
      and dens_nonneg[simp]:     "\<And>\<sigma>. \<sigma> \<in> space (state_measure (V \<union> V') \<Gamma>) \<Longrightarrow> \<delta> \<sigma> \<ge> 0"
...
proof-
  have "branch_prob \<Y> \<rho> =
      ...
      by (simp only: prod.case, subst emeasure_density, simp, simp,
          subst nn_integral_distr, simp add: state_measure_def, simp, simp)
...
proof-
  ...
  have "(\<lambda>\<rho>. \<integral>\<^sup>+x. \<delta> (merge V V' (x, \<rho>)) \<partial>state_measure V \<Gamma>) \<in> borel_measurable (state_measure V' \<Gamma>)"
    by (rule borel_measurable_nn_integral)
       (insert measurable_dens, unfold state_measure_def, simp)
  thus ?thesis by (simp add: branch_prob_altdef cong: measurable_cong)
qed
...
proof-
  ...
  show ?thesis unfolding marg_dens_def
    by (simp only: prod.case, subst measurable_split_conv, rule borel_measurable_nn_integral measurable_Pair_compose_split[OF measurable_add_dim])
       (auto simp: A state_measure_def measurable_split_conv simp del: insert_Diff_single
             intro!: measurable_compose[OF _ measurable_dens] 
                     measurable_compose[OF _ measurable_merge]
                     measurable_Pair_compose_split[OF measurable_add_dim])
qed
...
proof-
  interpret sigma_finite_measure "state_measure (V - {x, y}) \<Gamma>"
    unfolding state_measure_def
    by (rule product_sigma_finite.sigma_finite, simp_all add: product_sigma_finite_def)
  from assms have "V = insert y (insert x (V - {x,y}))" by blast
  hence A: "PiM V = PiM ..." by simp 
  show ?thesis unfolding marg_dens2_def
    by (simp only: prod.case, subst measurable_split_conv, rule borel_measurable_nn_integral)
       (auto simp: A state_measure_def measurable_split_conv 
             intro!: measurable_compose[OF _ measurable_dens] 
                     measurable_compose[OF _ measurable_merge]
                     measurable_Pair_compose_split[OF measurable_add_dim])
qed  
...
lemma marg_dens_nonneg[simp]: "marg_dens \<Y> x \<rho> v \<ge> 0"
  by (simp add: marg_dens_def nn_integral_nonneg)

lemma marg_dens2_nonneg[simp]: "marg_dens2 \<Y> x y \<rho> v \<ge> 0"
  by (simp add: marg_dens2_def nn_integral_nonneg)
...
proof-
  ...
  have [simp]: "indicator X' \<in> borel_measurable (Pi\<^sub>M V (\<lambda>y. stock_measure (\<Gamma> y)))"
    using assms unfolding state_measure_def by auto
  have "(\<integral>\<^sup>+\<sigma>. \<delta> (merge V V' (\<sigma>,\<rho>)) * indicator X' \<sigma> \<partial>state_measure V \<Gamma>) = 
    ...
    by (subst state_measure_integral_split[of x])
       (auto intro!: borel_measurable_ereal_times measurable_compose[OF _ measurable_dens]
                     measurable_compose[OF measurable_merge] simp: state_measure_def)
  ...
  also have "... = (\<integral>\<^sup>+ y. marg_dens \<Y> x \<rho> y * indicator X y \<partial>stock_measure (\<Gamma> x))"
    ...
    by (intro nn_integral_cong, subst nn_integral_multc)
       (auto intro!: measurable_compose[OF _ measurable_dens]  measurable_Pair
             measurable_compose[OF _ measurable_merge] simp: state_measure_def)
  finally show ?thesis ..
qed

lemma marg_dens2_integral:
  fixes X :: "val set" 
  assumes "x \<in> V" "y \<in> V" "x \<noteq> y" "X \<in> sets (stock_measure (PRODUCT (\<Gamma> x) (\<Gamma> y)))"
  assumes "\<rho> \<in> space (state_measure V' \<Gamma>)"
...
proof-
  ...
  with assms(1-5) have meas: "(\<lambda>\<sigma>. \<delta> (merge V V' (\<sigma>, \<rho>)) * indicator X' \<sigma>) 
                                   \<in> borel_measurable (state_measure V \<Gamma>)" 
    by (intro borel_measurable_ereal_times)
       (rule measurable_compose[OF _ measurable_dens], subst (1 2) state_measure_def,
        rule measurable_compose[OF measurable_Pair2' measurable_merge], fold state_measure_def, 
        assumption, rule borel_measurable_indicator)
  ...
  also have "... = \<integral>\<^sup>+z. marg_dens2 \<Y> x y \<rho> z * indicator X z \<partial>stock_measure (PRODUCT (\<Gamma> x) (\<Gamma> y))"
      ...
      apply (intro borel_measurable_ereal_times[OF measurable_marg_dens2 borel_measurable_indicator])
...
proof (rule measure_eqI)
  ...
  from assms have meas: "(\<lambda>\<sigma>. <|\<sigma> x,\<sigma> y|>) \<in> measurable (state_measure (V \<union> V') \<Gamma>) 
  ...
  also from assms(1-4) X meas 
    ...
    apply (subst emeasure_density, simp, subst sets_distr, rule measurable_sets, simp, simp)
    apply (subst nn_integral_distr, unfold state_measure_def, 
           rule measurable_compose[OF _ measurable_merge], rule measurable_Pair2', simp)
    apply (intro borel_measurable_ereal_times, rule measurable_dens[unfolded state_measure_def])
    apply (rule borel_measurable_indicator, erule measurable_sets, simp, simp)
    done
...
lemma nn_integral_dens_ctxt_measure:
  ...
  by (simp only: prod.case, subst nn_integral_density)
     (auto simp: nn_integral_distr state_measure_def intro!: borel_measurable_ereal_times)
...
lemma nonneg_insert_dens:
  assumes dens: "has_parametrized_subprob_density (state_measure (V\<union>V') \<Gamma>) F (stock_measure t) f"
  assumes \<sigma>: "\<sigma> \<in> space (state_measure (shift_var_set (V \<union> V')) (case_nat t \<Gamma>))"
  shows "insert_dens V V' f \<delta> \<sigma> \<ge> 0"
proof-
  note S = measurable_space[OF measurable_remove_var \<sigma>]
  from S have "\<delta> (remove_var \<sigma>) \<ge> 0" by (rule dens_nonneg)
  moreover from \<sigma> have "\<sigma> 0 \<in> space (stock_measure t)"
    by (force simp: state_measure_def space_PiM shift_var_set_def 
              simp del: space_stock_measure dest!: PiE_mem)
  from S and this have "f (remove_var \<sigma>) (\<sigma> 0) \<ge> 0"
    by (rule has_parametrized_subprob_densityD[OF dens])
  ultimately show "insert_dens V V' f \<delta> \<sigma> \<ge> 0" unfolding insert_dens_def
    by (rule ereal_0_le_mult)
qed
...
proof-
  note [measurable] = has_parametrized_subprob_densityD(4)[OF dens]
  have [measurable]:
  ...
  have "(\<integral>\<^sup>+x. indicator X x \<partial>M) = ?I"
    ...
    by (simp only: prod.case, subst nn_integral_density, simp, intro AE_I'[of "{}"])
       (simp_all add: nn_integral_distr MI nonneg_insert_dens[OF dens])
  finally show ?thesis .
qed
...
proof-
  ...
  have meas_integral: "(\<lambda>\<sigma>. \<integral>\<^sup>+ y. \<delta> (merge V V' (\<sigma>, \<rho> \<circ> Suc)) * f (merge V V' (\<sigma>, \<rho> \<circ> Suc)) y *
    ...
    apply (subst measurable_split_conv, intro borel_measurable_ereal_times)
  ...
  have "?I = \<integral>\<^sup>+\<sigma>. \<integral>\<^sup>+y. \<delta> (merge V V' (\<sigma>, \<rho> \<circ> Suc)) * f (merge V V' (\<sigma>, \<rho> \<circ> Suc)) y *
    ...
    apply (subst distr_state_measure_Suc[symmetric, of t], subst nn_integral_distr)
    apply (rule measurable_case_nat_Suc, rule meas_integral)
    apply (intro nn_integral_cong, simp add: m_eq)
    done
  also have "... = \<integral>\<^sup>+\<sigma>. \<delta> (merge V V' (\<sigma>, \<rho> \<circ> Suc)) * \<integral>\<^sup>+y. f (merge V V' (\<sigma>, \<rho> \<circ> Suc)) y *
                       indicator X (merge (shift_var_set V) (Suc`V') (case_nat y \<sigma>, \<rho>)) 
                  \<partial>stock_measure t \<partial>state_measure V \<Gamma>" using \<rho> X
    apply (intro nn_integral_cong, subst nn_integral_cmult[symmetric])
    apply (erule meas', rule dens_nonneg, erule measurable_space[OF merge_Suc_aux'[OF \<rho>]])
...
proof (intro allI conjI impI)
  fix \<sigma> assume \<sigma>': "\<sigma> \<in> space (state_measure (?V \<union> ?V') ?\<Gamma>')"
  from dens and this show "insert_dens V V' f \<delta> \<sigma> \<ge> 0"
    by (intro nonneg_insert_dens) simp_all
next
  note measurable_insert_dens[OF has_parametrized_subprob_densityD(4)[OF dens]] 
...
next
  ...
  proof (rule subprob_spaceI)
    ...
    also have "... = \<integral>\<^sup>+\<sigma>. \<delta> (merge V V' (\<sigma> \<circ> Suc, \<rho> \<circ> Suc)) * 
      ...
      apply (auto intro!: Suc_state_measure dens_nonneg)
      done
    also {
    ...
    hence "?I \<le> \<integral>\<^sup>+\<sigma>. \<delta> (merge V V' (\<sigma> \<circ> Suc, \<rho> \<circ> Suc)) * 1 \<partial>state_measure (Suc ` V) ?\<Gamma>'"
      by (intro nn_integral_mono ereal_mult_left_mono) (simp_all add: Suc_state_measure)
  ...
  also have "... = \<integral>\<^sup>+x. \<delta> (merge V V' (x, \<rho>)) *
    ...
    by (intro nn_integral_cong, subst nn_integral_density, simp)
       (auto simp: mult.assoc dens_ctxt_measure_def state_measure'_def 
             intro!: dens_nonneg merge_in_state_measure \<rho> AE_I'[of "{}"] 
...
proof (intro allI conjI impI subprob_spaceI)
  ...
  from D(4) show M: "if_dens \<delta> f b \<in> borel_measurable (state_measure (V \<union> V') \<Gamma>)"
    by (intro measurable_if_dens) simp_all
  show "\<And>\<rho>. \<rho> \<in> space (state_measure (V \<union> V') \<Gamma>) \<Longrightarrow> if_dens \<delta> f b \<rho> \<ge> 0" 
     by (simp add: if_dens_def D(2))
...

  {
    ...
    with \<sigma>\<rho> have "f (merge V V' (\<sigma>, \<rho>)) (BoolVal b) \<le> 1" "\<delta> (merge V V' (\<sigma>, \<rho>)) \<ge> 0"
      by (auto intro: subprob_count_space_density_le_1 dens_nonneg)
...
unfolding density_context_def
proof (intro allI conjI impI subprob_spaceI)
  show "\<And>\<rho>. \<rho> \<in> space (state_measure (V \<union> V') \<Gamma>) \<Longrightarrow> if_dens_det \<delta> e b \<rho> \<ge> 0" 
...
proof-
  ...
  also have "... = f \<rho> * branch_prob \<Y> \<rho>"
    by (subst nn_integral_const, simp add: assms) (simp_all add: assms branch_prob_def)
