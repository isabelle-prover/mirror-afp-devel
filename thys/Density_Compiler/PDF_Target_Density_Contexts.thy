...
definition is_density_expr :: "cdens_ctxt \<Rightarrow> pdf_type \<Rightarrow> cexpr \<Rightarrow> bool" where
  "is_density_expr \<equiv> \<lambda>(vs,vs',\<Gamma>,\<delta>) t e.
     case_nat t \<Gamma> \<turnstile>\<^sub>c e : REAL \<and> free_vars e \<subseteq> shift_var_set (set vs')"

lemma is_density_exprI:
  "case_nat t \<Gamma> \<turnstile>\<^sub>c e : REAL \<Longrightarrow> free_vars e \<subseteq> shift_var_set (set vs') \<Longrightarrow> 
...
proof (unfold density_context_def, intro allI ballI conjI impI subprob_spaceI)
  ...
  let ?M = "dens_ctxt_measure (set vs, set vs', \<Gamma>, \<lambda>x. ereal (extract_real (cexpr_sem x \<delta>))) \<rho>"
  ...
  hence "emeasure ?M (space ?M) = 
             \<integral>\<^sup>+x. ereal (extract_real (cexpr_sem (merge (set vs) (set vs') (x, \<rho>)) \<delta>))
...
lemma cexpr_sem_marg_dens:
  ...
  shows "AE v in stock_measure (\<Gamma> x). 
           ereal (extract_real (cexpr_sem (case_nat v \<rho>) (marg_dens_cexpr \<Gamma> vs x \<delta>))) = 
...
proof-
  ...
  proof (rule AE_mp[OF integrable], rule AE_I2, intro impI)
    ...
    finally show "ereal (extract_real (cexpr_sem (\<lambda>a. case a of 0 \<Rightarrow> v | Suc a \<Rightarrow> \<rho> a)
...
lemma cexpr_sem_marg_dens2:
  ...
  shows "AE z in stock_measure (PRODUCT (\<Gamma> x) (\<Gamma> y)). 
           ereal (extract_real (cexpr_sem (case_nat z \<rho>) (marg_dens2_cexpr \<Gamma> vs x y \<delta>))) = 
               marg_dens2 (dens_ctxt_\<alpha> (vs,vs',\<Gamma>,\<delta>)) x y \<rho> z"
proof-
  note invar = cdens_ctxt_invarD[OF assms(1)]
  let ?f = "\<lambda>x. ereal (extract_real (cexpr_sem x \<delta>))"
  ...
  have "(\<integral>\<^sup>+\<sigma>. ?f (merge (set vs) (set vs') (\<sigma>, \<rho>)) \<partial>state_measure (set vs) \<Gamma>) =
    ...
    by (subst A, subst product_nn_integral_fold) (simp_all add: borel_measurable_ereal)
  ...
  also have "... = \<integral>\<^sup>+z. \<integral>\<^sup>+\<sigma>. ?f (merge (set vs) (set vs') (\<sigma>(x := fst z, y := snd z), \<rho>)) 
    ...
    by (subst product_nn_integral_pair, subst measurable_split_conv, 
       ...
       (auto simp: measurable_split_conv state_measure_def intro!: borel_measurable_ereal
  ...
  proof (rule AE_mp[OF _ AE_I2[OF impI]])
    ...
    hence A: "(\<integral>\<^sup>+\<sigma>. ?f (merge (set vs) (set vs') (\<sigma>(x := fst z, y := snd z), \<rho>)) 
              ...
              (\<integral>\<^sup>+\<sigma>. ?f (merge (set ?vs) ({x,y} \<union> set vs') (\<sigma>, \<rho>(x := fst z, y := snd z))) 
                   \<partial>state_measure (set ?vs) \<Gamma>)" (is "_ = \<integral>\<^sup>+\<sigma>. ereal (?g \<sigma>) \<partial>?M")
    ...
    proof (intro integrableI_nonneg[OF _ AE_I2])
      ...
      show "(\<integral>\<^sup>+\<sigma>. ereal (?g \<sigma>) \<partial>?M) < \<infinity>" using fin A by simp
...
lemma cexpr_sem_branch_prob:
  assumes "cdens_ctxt_invar vs vs' \<Gamma> \<delta>"
  assumes \<rho>: "\<rho> \<in> space (state_measure (set vs') \<Gamma>)"
  shows "ereal (extract_real (cexpr_sem \<rho> (branch_prob_cexpr (vs, vs', \<Gamma>, \<delta>)))) = 
...
proof-
  ...
  have "ereal (extract_real (cexpr_sem \<rho> (branch_prob_cexpr (vs, vs', \<Gamma>, \<delta>)))) = 
...
proof (intro subprob_cexprI)
  ...
  from \<rho> have "(\<integral>\<^sup>+ x. ereal (extract_real (cexpr_sem (merge V V' (x, \<rho>)) \<delta>)) \<partial>state_measure V \<Gamma>) =
