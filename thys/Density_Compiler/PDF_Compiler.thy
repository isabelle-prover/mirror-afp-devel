...
lemma cdens_ctxt_invar_insert:
  ...
  assumes hd: "dens_ctxt_\<alpha> ([], vs @ vs', \<Gamma>, CReal 1) \<turnstile>\<^sub>d e \<Rightarrow> (\<lambda>x xa. ereal (eval_cexpr f x xa))" 
...
proof (intro cdens_ctxt_invarI)
  ...
  let ?vs = "shift_var_set (set vs)" and ?vs' = "Suc ` set vs'" and ?\<Gamma> = "case_nat t' \<Gamma>" and
      ?\<delta> = "insert_dens (set vs) (set vs') (\<lambda>\<sigma> x. ereal (eval_cexpr f \<sigma> x))
                        (\<lambda>x. ereal (extract_real (cexpr_sem x \<delta>)))"
  ...
  next
    ...
    from \<sigma> show "insert_dens (set vs) (set vs') (\<lambda>\<sigma> x. ereal (eval_cexpr f \<sigma> x))
                      (\<lambda>x. ereal (extract_real (cexpr_sem x \<delta>))) \<sigma> =
                   ereal (extract_real (cexpr_sem \<sigma> (map_vars Suc \<delta> *\<^sub>c f)))"
      unfolding insert_dens_def using invar is_density_exprD[OF wf1]
      by (subst cexpr_sem_Mult[of ?\<Gamma> _ _ _ "?vs \<union> ?vs'"])
         (auto intro!: cexpr_typing_map_vars simp: o_def shift_var_set_def eval_cexpr_def
  ...
  moreover from dens have "nonneg_cexpr (shift_var_set (set vs \<union> set vs')) (case_nat t' \<Gamma>) f"
    by (subst set_append[symmetric], intro has_parametrized_subprob_density_imp_nonneg_cexpr) simp
  ultimately show nonneg: "nonneg_cexpr (set (shift_vars vs) \<union> set (map Suc vs')) 
...
lemma cdens_ctxt_invar_insert_bool:
  assumes dens: "dens_ctxt_\<alpha> ([], vs @ vs', \<Gamma>, CReal 1) \<turnstile>\<^sub>d b \<Rightarrow> (\<lambda>\<rho> x. ereal (eval_cexpr f \<rho> x))"
...
proof (intro cdens_ctxt_invarI nonneg_cexpr_Mult nonneg_cexpr_subst_val)
  ...
  have dens': "has_parametrized_subprob_density (state_measure (set vs \<union> set vs') \<Gamma>) ?M 
    ...
    by (intro expr_has_density_sound_aux density_context.density_context_empty)
       (auto simp: extract_real_def one_ereal_def)
  ...
  proof (intro subprob_cexprI)
    ...
    let ?eval = "\<lambda>e \<sigma>. extract_real (cexpr_sem (merge (set vs) (set vs') (\<sigma>, \<rho>)) e)"
    {
      ...
      moreover have B: "ereal (?eval (cexpr_subst_val f (BoolVal v)) \<sigma>) =
                          ereal (eval_cexpr f (merge (set vs) (set vs') (\<sigma>, \<rho>)) (BoolVal v))"
    ...
    hence "(\<integral>\<^sup>+\<sigma>. ?eval (\<delta> *\<^sub>c cexpr_subst_val f (BoolVal v)) \<sigma> \<partial>state_measure (set vs) \<Gamma>) \<le>
              (\<integral>\<^sup>+\<sigma>. ?eval \<delta> \<sigma> \<partial>state_measure (set vs) \<Gamma>)" by (intro nn_integral_mono) simp
...
(*  case (edc_equiv f1 f2 \<Gamma> e t vs vs' \<delta> t')
  ...
  from edc_equiv have "(\<lambda>\<rho> x. ereal (eval_cexpr f1 \<rho> x)) = (\<lambda>\<rho> x. ereal (eval_cexpr f2 \<rho> x))"
...
next*)
...

  {
    ...
    hence "ereal (eval_cexpr (?e1 *\<^sub>c ?e2) \<rho> x) = 
             ereal (extract_real (cexpr_sem (case_nat x \<rho>) 
                 (map_vars Suc (branch_prob_cexpr (vs, vs', \<Gamma>, \<delta>))))) *
             ereal (extract_real (RealVal (bool_to_real (x = v))))" (is "_ = ?a * ?b")
      using invar unfolding eval_cexpr_def
      by (subst cexpr_sem_Mult[of "case_nat t \<Gamma>" _ _ _ "shift_var_set (set vs')"])
         (insert invar ctype1 ctype2, auto simp: shift_var_set_def)
    ...
    finally have "ereal (eval_cexpr (?e1 *\<^sub>c ?e2) \<rho> x) = 
                     branch_prob (dens_ctxt_\<alpha> (vs,vs',\<Gamma>,\<delta>)) \<rho> * indicator {v} x" .
  } note e = this

  have meas: "(\<lambda>(\<sigma>, x). ereal (eval_cexpr (?e1 *\<^sub>c ?e2) \<sigma> x))
                 \<in> borel_measurable (state_measure (set vs') \<Gamma> \<Otimes>\<^sub>M stock_measure (val_type v))"
    apply (subst measurable_split_conv, rule borel_measurable_ereal)
  ...
  show ?case unfolding dens_ctxt_\<alpha>_def
    ...
    apply (rule ctype, insert invar, auto simp: shift_var_set_def)
    done
next
  ...
  next
    have free_vars: "free_vars (marg_dens_cexpr \<Gamma> vs x \<delta>) \<subseteq> shift_var_set (set vs')"
      using edc_var.prems(2) by (rule free_vars_marg_dens_cexpr)
    show "(\<lambda>(\<rho>, y). ereal (eval_cexpr (marg_dens_cexpr \<Gamma> vs x \<delta>) \<rho> y))
  ...
  next
    ...
    hence "AE y in stock_measure t. 
             marg_dens (dens_ctxt_\<alpha> (vs, vs', \<Gamma>, \<delta>)) x \<rho> y =
                  ereal (eval_cexpr (marg_dens_cexpr \<Gamma> vs x \<delta>) \<rho> y)"
      using edc_var unfolding eval_cexpr_def by (subst t, subst eq_commute, intro cexpr_sem_marg_dens)
    thus "AE y in stock_measure t. 
            marg_dens (set vs, set vs', \<Gamma>, \<lambda>x. ereal (extract_real (cexpr_sem x \<delta>))) x \<rho> y =
                 ereal (eval_cexpr (marg_dens_cexpr \<Gamma> vs x \<delta>) \<rho> y)"
      by (simp add: dens_ctxt_\<alpha>_def)
  qed (insert edc_var.prems edc_var.hyps, auto simp: eval_cexpr_def intro!: nonneg_cexpr_sem_marg_dens)
next
  ...
  proof (simp only: prod.case, intro conjI is_density_exprI, rule hd_AE[OF hd_pair edc_pair.prems(1)])
    ...
    show "AE z in stock_measure t. 
              marg_dens2 (set vs, set vs', \<Gamma>, \<lambda>x. ereal (extract_real (cexpr_sem x \<delta>))) x y \<rho> z =
                ereal (eval_cexpr (marg_dens2_cexpr \<Gamma> vs x y \<delta>) \<rho> z)"
    ...
    fix z assume z: "z \<in> type_universe t"
    show "ereal (eval_cexpr (marg_dens2_cexpr \<Gamma> vs x y \<delta>) \<rho> z) \<ge> 0" 
      using edc_pair \<rho> z by (simp add: nonneg_cexpr_sem_marg_dens2 eval_cexpr_def)
...
next
  ...
  thus ?case by (auto simp: dens_ctxt_\<alpha>_def eval_cexpr_def extract_real_def 
                            zero_ereal_def[symmetric] hd_fail 
                      intro!: is_density_exprI)
...
next
  ...
  have dens1: "dens_ctxt_\<alpha> ([], vs @ vs', \<Gamma>, CReal 1) \<turnstile>\<^sub>d  e \<Rightarrow> 
                   (\<lambda>x xa. ereal (eval_cexpr f x xa))" and
  ...
  hence "dens_ctxt_\<alpha> (shift_vars vs, map Suc vs', case_nat t' \<Gamma>, map_vars Suc \<delta> *\<^sub>c f) \<turnstile>\<^sub>d
            e' \<Rightarrow> (\<lambda>x xa. ereal (eval_cexpr g x xa)) \<and>
  ...
  hence dens2: "dens_ctxt_\<alpha> ?\<Y>  \<turnstile>\<^sub>d e' \<Rightarrow> (\<lambda>x xa. ereal (eval_cexpr g x xa))" and
  ...
  hence "(shift_var_set (set vs), Suc ` set vs', case_nat t' \<Gamma>,
           insert_dens (set vs) (set vs') (\<lambda>x xa. ereal (eval_cexpr f x xa))
                (\<lambda>x. ereal (extract_real (cexpr_sem x \<delta>))))
          \<turnstile>\<^sub>d e' \<Rightarrow> (\<lambda>a aa. ereal (eval_cexpr g a aa))" using dens2
  ...
  hence "dens_ctxt_\<alpha> (vs, vs', \<Gamma>, \<delta>) \<turnstile>\<^sub>d LET e IN e' \<Rightarrow> 
            (\<lambda>\<rho> x. ereal (eval_cexpr g (case_nat undefined \<rho>) x))"
    unfolding dens_ctxt_\<alpha>_def
    by (simp only: prod.case, intro hd_let[where f = "\<lambda>x xa. ereal (eval_cexpr f x xa)"])
       (insert dens1 dens2, simp_all add: dens_ctxt_\<alpha>_def extract_real_def one_ereal_def t1')
  hence "dens_ctxt_\<alpha> (vs, vs', \<Gamma>, \<delta>) \<turnstile>\<^sub>d LET e IN e' \<Rightarrow> 
            (\<lambda>\<rho> x. ereal (eval_cexpr (map_vars (\<lambda>x. x - 1) g) \<rho> x))"
  proof (simp only: dens_ctxt_\<alpha>_def prod.case, erule_tac hd_cong[OF _ _ edc_let.prems(1,3)])
    ...
    have "eval_cexpr (map_vars (\<lambda>x. x - 1) g) \<rho> x = 
            extract_real (cexpr_sem (case_nat x \<rho> \<circ> (\<lambda>x. x - 1)) g)"
      unfolding eval_cexpr_def by (simp add: cexpr_sem_map_vars)
    also have "cexpr_sem (case_nat x \<rho> \<circ> (\<lambda>x. x - 1)) g = 
                   cexpr_sem (case_nat x (case_nat undefined \<rho>)) g"
      using is_density_exprD[OF wf2]
      by (intro cexpr_sem_eq_on_vars) (auto split: nat.split simp: shift_var_set_def)
    finally show "ereal (eval_cexpr g (case_nat undefined \<rho>) x) = 
                      ereal (eval_cexpr (map_vars (\<lambda>x. x - 1) g) \<rho> x)"
...
next
  ...
  have dens: "dens_ctxt_\<alpha> (vs, vs', \<Gamma>, \<delta>) \<turnstile>\<^sub>d e \<Rightarrow> (\<lambda>x xa. ereal (eval_cexpr f x xa))" and
  ...
  hence nonneg_e1: "nonneg_cexpr (shift_var_set (shift_var_set (set vs')))
                        (case_nat t  (case_nat t' \<Gamma>)) ?e1" 
    using has_parametrized_subprob_density_imp_nonneg_cexpr[OF dens'] 
    by (intro nonneg_cexpr_map_vars) simp_all
...

  {
    ...
    hence "eval_cexpr f \<rho> y * dist_dens dst y x = extract_real (cexpr_sem ?\<rho>'' (?e1 *\<^sub>c ?e2))"
      using \<rho>''
      by (subst A, subst times_ereal.simps[symmetric], subst cexpr_sem_dist_dens_cexpr[of ?\<Gamma>'' _ _ _ ?V''])
         (force simp: cexpr_sem_map_vars eval_cexpr_def t2 t_def intro!: cet_var')+
    note this B
  } note e1e2 = this

  {
    ...
    have "AE x in stock_measure t'. 
            apply_dist_to_dens dst (\<lambda>\<rho> x. ereal (eval_cexpr f \<rho> x)) \<rho> x = eval_cexpr ?e \<rho> x"
    proof (rule AE_mp[OF _ AE_I2[OF impI]])
      ...
      thus "AE x in stock_measure t'. ?f \<rho> x \<noteq> \<infinity>"
        using has_parametrized_subprob_densityD(4)[OF dens''] \<rho>
    ...
    next
      ...
      hence "(\<integral>\<^sup>+y. ereal (eval_cexpr f \<rho> y) * dist_dens dst y x \<partial>stock_measure t) =
      ...
      also from this and finite have finite': "?I < \<infinity>" by simp
      have "?I = ereal (eval_cexpr ?e \<rho> x)" using \<rho>' te te1 te2 vars_e1 vars_e2 nonneg_e1 nonneg_e2
      ...
      finally show "apply_dist_to_dens dst (\<lambda>\<rho> x. ereal (eval_cexpr f \<rho> x)) \<rho> x = 
                        ereal (eval_cexpr ?e \<rho> x)"
        unfolding apply_dist_to_dens_def by (simp add: t_def)
     qed
   } note AE_eq = this

  have meas: "(\<lambda>(\<rho>, x). ereal (eval_cexpr ?e \<rho> x))
                 \<in> borel_measurable (state_measure (set vs') \<Gamma> \<Otimes>\<^sub>M stock_measure t')"
    apply (subst measurable_split_conv, rule borel_measurable_ereal)
  ...
  show ?case
  proof (intro conjI, simp only: dens_ctxt_\<alpha>_def prod.case,
         rule hd_AE[OF hd_rand edc_rand.prems(1)])
    from dens show "(set vs, set vs', \<Gamma>, \<lambda>x. ereal (extract_real (cexpr_sem x \<delta>))) \<turnstile>\<^sub>d 
                        e \<Rightarrow> (\<lambda>x xa. ereal (eval_cexpr f x xa))"
      unfolding dens_ctxt_\<alpha>_def by simp
  next
    fix \<rho> x assume \<rho>: "\<rho> \<in> space (state_measure (set vs') \<Gamma>)" and x: "x \<in> type_universe t'"
    let ?e' = "map_vars (case_nat 0 (\<lambda>x. x + 2)) f *\<^sub>c dist_dens_cexpr dst (CVar 0) (CVar 1)"
    have "eval_cexpr (\<integral>\<^sub>c ?e' \<partial>dist_param_type dst) \<rho> x = 
            \<integral>y. extract_real (cexpr_sem (case_nat y (case_nat x \<rho>)) ?e') \<partial>stock_measure t"
      by (simp add: eval_cexpr_def extract_real_def t_def t2)
    also have "... \<ge> 0" using e1e2 \<rho> x by (intro integral_nonneg_AE AE_I2) simp
    finally show "ereal (eval_cexpr (\<integral>\<^sub>c ?e' \<partial>dist_param_type dst) \<rho> x) \<ge> 0" by simp
  qed (insert AE_eq meas te vars_e1 vars_e2, 
       auto simp: is_density_expr_def t_def t2 shift_var_set_def)
...
next
...

  {
    ...
    hence "eval_cexpr (?e1 *\<^sub>c ?e2) \<rho> x =    
             ereal (extract_real (cexpr_sem (case_nat x \<rho>) 
                 (map_vars Suc (branch_prob_cexpr (vs, vs', \<Gamma>, \<delta>))))) *
             ereal (extract_real (cexpr_sem (case_nat x \<rho>) ?e2))" (is "_ = ?a * ?b")
      using invar unfolding eval_cexpr_def
      by (subst cexpr_sem_Mult[of "case_nat t' \<Gamma>" _ _ _ "shift_var_set (set vs')"])
         (insert invar ctype1 vars2 ctype2 edc_rand_det.hyps(2), auto simp: shift_var_set_def)
  ...
  have meas: "(\<lambda>(\<rho>, x). ereal (eval_cexpr (?e1 *\<^sub>c ?e2) \<rho> x))
                \<in> borel_measurable (state_measure (set vs') \<Gamma> \<Otimes>\<^sub>M stock_measure t')"
    using ctype1 ctype2 vars2 invar edc_rand_det.hyps
    by (subst measurable_split_conv, intro borel_measurable_ereal,
  ...
  from ctype1 ctype2 vars2 invar edc_rand_det.hyps
    ...
    by (intro is_density_exprI) (auto intro!: cexpr_typing.intros simp: shift_var_set_def)
...
next
  ...
  proof (rule conjI, simp only: dens_ctxt_\<alpha>_def prod.case, rule hd_cong[OF hd_if_det])
    let ?\<Y> = "(set vs, set vs', \<Gamma>, if_dens_det (\<lambda>x. ereal (extract_real (cexpr_sem x \<delta>))) b True)"
    show "?\<Y> \<turnstile>\<^sub>d e1 \<Rightarrow> (\<lambda>\<rho> x. eval_cexpr f1 \<rho> x)"
    proof (rule hd_dens_ctxt_cong)
      let ?\<delta> = "\<lambda>\<sigma>. ereal (extract_real (cexpr_sem \<sigma> (\<delta> *\<^sub>c ?ind1)))"
      show "(set vs, set vs', \<Gamma>, ?\<delta>) \<turnstile>\<^sub>d e1 \<Rightarrow> (\<lambda>\<rho> x. ereal (eval_cexpr f1 \<rho> x))" 
      ...
      finally show "?\<delta> \<sigma> = if_dens_det (\<lambda>\<sigma>. ereal (extract_real (cexpr_sem \<sigma> \<delta>))) b True \<sigma>"
        by (simp add: if_dens_det_def)
    qed
  next
    let ?\<Y> = "(set vs, set vs', \<Gamma>, if_dens_det (\<lambda>x. ereal (extract_real (cexpr_sem x \<delta>))) b False)"
    show "?\<Y> \<turnstile>\<^sub>d e2 \<Rightarrow> (\<lambda>\<rho> x. eval_cexpr f2 \<rho> x)"
    proof (rule hd_dens_ctxt_cong)
      let ?\<delta> = "\<lambda>\<sigma>. ereal (extract_real (cexpr_sem \<sigma> (\<delta> *\<^sub>c ?ind2)))"
      show "(set vs, set vs', \<Gamma>, ?\<delta>) \<turnstile>\<^sub>d e2 \<Rightarrow> (\<lambda>\<rho> x. ereal (eval_cexpr f2 \<rho> x))"
      ...
      also have "extract_real (cexpr_sem \<sigma> ?ind2) = (if expr_sem_rf \<sigma> b = FALSE then 1 else 0)"
        using edc_if_det.hyps val_type_expr_sem_rf[OF tb b \<sigma>]
        by (auto simp: cexpr_sem_expr_rf_to_cexpr extract_real_def bool_to_real_def elim!: BOOL_E)
      finally show "?\<delta> \<sigma> = if_dens_det (\<lambda>\<sigma>. ereal (extract_real (cexpr_sem \<sigma> \<delta>))) b False \<sigma>"
        by (simp add: if_dens_det_def)
    qed
  next
    ...
    thus "ereal (eval_cexpr f1 \<rho> x) + ereal (eval_cexpr f2 \<rho> x) = ereal (eval_cexpr (f1 +\<^sub>c f2) \<rho> x)"
      by simp
  next
    show "is_density_expr (vs, vs', \<Gamma>, \<delta>) t (f1 +\<^sub>c f2)" using wf1 wf2
      by (auto simp: is_density_expr_def intro!: cet_op[where t = "PRODUCT REAL REAL"] cet_pair)
...
next
  ...
  have densb: "dens_ctxt_\<alpha> ([], vs @ vs', \<Gamma>, CReal 1) \<turnstile>\<^sub>d b \<Rightarrow> (\<lambda>\<rho> b. ereal (eval_cexpr f \<rho> b))" and
  ...
  have dens1: "dens_ctxt_\<alpha> (vs, vs', \<Gamma>, \<delta> *\<^sub>c ?\<delta>1) \<turnstile>\<^sub>d e1 \<Rightarrow> (\<lambda>x xa. ereal (eval_cexpr g1 x xa))" and
         wf1: "is_density_expr (vs, vs', \<Gamma>, \<delta> *\<^sub>c ?\<delta>1) t g1" and
       dens2: "dens_ctxt_\<alpha> (vs, vs', \<Gamma>, \<delta> *\<^sub>c ?\<delta>2) \<turnstile>\<^sub>d e2 \<Rightarrow> (\<lambda>x xa. ereal (eval_cexpr g2 x xa))" and
  ...
  let ?\<delta>' = "\<lambda>\<sigma>. ereal (extract_real (cexpr_sem \<sigma> \<delta>))" and ?f = "\<lambda>\<sigma> x. ereal (eval_cexpr f \<sigma> x)"
  show ?case
  proof (rule conjI, simp only: dens_ctxt_\<alpha>_def prod.case, rule hd_cong[OF hd_if])
    ...
    proof (rule hd_dens_ctxt_cong)
      let ?\<delta> = "\<lambda>\<sigma>. ereal (extract_real (cexpr_sem \<sigma> (\<delta> *\<^sub>c ?\<delta>1)))"
      show "(set vs, set vs', \<Gamma>, ?\<delta>) \<turnstile>\<^sub>d e1 \<Rightarrow> (\<lambda>\<rho> x. ereal (eval_cexpr g1 \<rho> x))" 
        using dens1 by (simp add: dens_ctxt_\<alpha>_def)
      fix \<sigma> assume \<sigma>: "\<sigma> \<in> space (state_measure (set vs \<union> set vs') \<Gamma>)"
      ...
      also have "... = if_dens ?\<delta>' ?f True \<sigma>" unfolding if_dens_def by (simp add: eval_cexpr_def)
  ...
  next
    ...
    proof (rule hd_dens_ctxt_cong)
      let ?\<delta> = "\<lambda>\<sigma>. ereal (extract_real (cexpr_sem \<sigma> (\<delta> *\<^sub>c ?\<delta>2)))"
      show "(set vs, set vs', \<Gamma>, ?\<delta>) \<turnstile>\<^sub>d e2 \<Rightarrow> (\<lambda>\<rho> x. ereal (eval_cexpr g2 \<rho> x))" 
        using dens2 by (simp add: dens_ctxt_\<alpha>_def)
      fix \<sigma> assume \<sigma>: "\<sigma> \<in> space (state_measure (set vs \<union> set vs') \<Gamma>)"
      have "extract_real (cexpr_sem \<sigma> (\<delta> *\<^sub>c ?\<delta>2)) = 
        ...
        using invar vars1 is_density_exprD[OF wfb] by (subst cexpr_sem_Mult[OF invar(3) _ \<sigma>]) auto
      also have "... = if_dens ?\<delta>' ?f False \<sigma>" unfolding if_dens_def by (simp add: eval_cexpr_def)
  ...
  next
    ...
    hence "eval_cexpr (g1 +\<^sub>c g2) \<rho> x = eval_cexpr g1 \<rho> x + eval_cexpr g2 \<rho> x"
      using wf1 wf2 unfolding eval_cexpr_def is_density_expr_def
      by (subst cexpr_sem_Add[where \<Gamma> = "case_nat t \<Gamma>" and V = "shift_var_set (set vs')"]) auto
    thus "ereal (eval_cexpr g1 \<rho> x) + ereal (eval_cexpr g2 \<rho> x) = ereal (eval_cexpr (g1 +\<^sub>c g2) \<rho> x)"
      by simp
  next
    show "is_density_expr (vs, vs', \<Gamma>, \<delta>) t (g1 +\<^sub>c g2)" using wf1 wf2
      by (auto simp: is_density_expr_def intro!: cet_op[where t = "PRODUCT REAL REAL"] cet_pair)
  next
    show "({}, set vs \<union> set vs', \<Gamma>, \<lambda>_. 1) \<turnstile>\<^sub>d b \<Rightarrow> (\<lambda>\<sigma> x. ereal (eval_cexpr f \<sigma> x))"
      using densb unfolding dens_ctxt_\<alpha>_def by (simp add: extract_real_def one_ereal_def)
...
next
  ...
  hence "nonneg_cexpr (shift_var_set (set vs')) (case_nat t \<Gamma>) f"
    by (rule has_parametrized_subprob_density_imp_nonneg_cexpr)
  ...
  proof (rule hd_cong[OF _ _ _ _  nn_integral_cong])
    ...
    thus "?f \<rho> x y = ereal (eval_cexpr ?expr' (case_nat x \<rho>) y)"
  ...
  proof (intro conjI is_density_exprI, simp only: dens_ctxt_\<alpha>_def prod.case, rule hd_AE[OF dens])
    ...
    show "AE x in stock_measure t'. ?dens x = ereal (eval_cexpr ?expr \<rho> x)"
    proof (rule AE_mp[OF _ AE_I2[OF impI]])
      from has_parametrized_subprob_density_integral[OF dens' \<rho>] and
           has_parametrized_subprob_densityD(4)[OF dens'] and \<rho>
    ...
    next
      ...
      thus "?dens x = ereal (eval_cexpr ?expr \<rho> x)" 
        using \<rho> vars' ctype' ctype' nonneg unfolding eval_cexpr_def
        by (subst cexpr_sem_integral_nonneg) (auto intro!: nonneg_cexpr_map_vars)
    qed
    fix x assume x: "x \<in> type_universe t'"
    have "nonneg_cexpr (shift_var_set (set vs')) (case_nat t' \<Gamma>) ?expr" using nonneg ctype''
      by (intro nonneg_cexpr_int nonneg_cexpr_comp nonneg_cexpr_map_vars) simp_all
    thus "ereal (eval_cexpr ?expr \<rho> x) \<ge> 0" unfolding eval_cexpr_def using \<rho> x
      by (subst ereal_less_eq, intro nonneg_cexprD) auto
...
next
  ...
  have dens': "has_parametrized_subprob_density (state_measure (set vs') \<Gamma>) ?M 
    ...
    by (intro expr_has_density_sound_aux density_context_\<alpha>) (simp_all add: dens_ctxt_\<alpha>_def)
  hence nonneg: "nonneg_cexpr (shift_var_set (set vs')) (case_nat (PRODUCT t t') \<Gamma>) f"
    by (intro has_parametrized_subprob_density_imp_nonneg_cexpr)
  ...
  show ?case
  proof (intro conjI, simp only: dens_ctxt_\<alpha>_def prod.case, rule hd_AE[OF dens'])
    fix \<rho> assume \<rho>: "\<rho> \<in> space (state_measure (set vs') \<Gamma>)"
    thus "AE x in stock_measure t. ?f \<rho> x = ereal (eval_cexpr (\<integral>\<^sub>c ?expr \<partial>t') \<rho> x)"
    ...
    with \<rho> show "ereal (eval_cexpr (\<integral>\<^sub>c ?expr \<partial>t') \<rho> x) \<ge> 0" using nonneg'
      by (simp only: ereal_less_eq eval_cexpr_def, intro nonneg_cexprD nonneg_cexpr_int) auto
  qed (insert edc_fst.prems ctype vars, auto simp: measurable_split_conv 
       intro!: is_density_exprI cet_int borel_measurable_ereal 
...
next
  ...
  have "dens_ctxt_\<alpha> (vs, vs', \<Gamma>, \<delta>) \<turnstile>\<^sub>d Minus $$ e \<Rightarrow> (\<lambda>\<sigma> x. ereal (eval_cexpr f \<sigma> (op_sem Minus x)))"
    using dens by (simp only: dens_ctxt_\<alpha>_def prod.case, intro hd_neg) simp_all
  also have "(\<lambda>\<sigma> x. ereal (eval_cexpr f \<sigma> (op_sem Minus x))) = 
                 (\<lambda>\<sigma> x. ereal (eval_cexpr (f \<circ>\<^sub>c -\<^sub>c CVar 0) \<sigma> x))"
...
next
  ...
  have "dens_ctxt_\<alpha> (vs, vs', \<Gamma>, \<delta>) \<turnstile>\<^sub>d Add $$ <e, e'> \<Rightarrow> 
            (\<lambda>\<sigma> x. ereal (eval_cexpr f \<sigma> (op_sem Add <|x, expr_sem_rf \<sigma> (Minus $$ e')|>)))"
  ...
  also have "?f = (\<lambda>\<sigma> x. ereal (eval_cexpr ?expr \<sigma> x))" using edc_addc.hyps
     by (intro ext) (auto simp: eval_cexpr_comp cexpr_sem_map_vars o_def cexpr_sem_expr_rf_to_cexpr)
                    ...
                    (\<lambda>\<sigma> x. ereal (eval_cexpr ?expr \<sigma> x))" .
  moreover have "is_density_expr (vs, vs', \<Gamma>, \<delta>) t ?expr" using ctype vars
    by (auto intro!: is_density_exprI)
  ultimately show ?case by (rule conjI)
...
next
  ...
  have "dens_ctxt_\<alpha> (vs, vs', \<Gamma>, \<delta>) \<turnstile>\<^sub>d Mult $$ <e, Val (RealVal c)> \<Rightarrow> 
            (\<lambda>\<sigma> x. ereal (eval_cexpr f \<sigma> (op_sem Mult <|x, op_sem Inverse (RealVal c)|>)) * 
                       ereal (inverse (abs (extract_real (RealVal c)))))"
  ...
  proof (simp only: dens_ctxt_\<alpha>_def prod.case, erule_tac hd_cong)
    ...
    finally show "ereal (eval_cexpr f \<rho> (op_sem Mult <|x, op_sem Inverse (RealVal c)|>)) *
                      ereal (inverse \<bar>extract_real (RealVal c)\<bar>) = ereal (eval_cexpr ?expr \<rho> x)"
...
next
  ...
  proof (intro conjI is_density_exprI, simp only: dens_ctxt_\<alpha>_def prod.case, rule hd_AE[OF dens])
    ...
    next
      ...
      thus "?dens x = ereal (eval_cexpr ?expr \<rho> x)" 
        using \<rho> vars' ctype' ctype'' nonneg unfolding eval_cexpr_def
        by (subst cexpr_sem_integral_nonneg) (auto intro!: nonneg_cexpr_comp nonneg_cexpr_map_vars)
    qed
    fix x assume x: "x \<in> type_universe t"
    have "nonneg_cexpr (shift_var_set (set vs')) (case_nat t \<Gamma>) ?expr" using nonneg ctype''
      by (intro nonneg_cexpr_int nonneg_cexpr_comp nonneg_cexpr_map_vars) simp_all
...
next
  ...
  from wf' have "free_vars (f \<circ>\<^sub>c (\<lambda>\<^sub>cx. inverse\<^sub>c x)) \<subseteq> shift_var_set (set vs')"
  ...
  proof (intro conjI, simp only: dens_ctxt_\<alpha>_def prod.case, rule hd_cong[OF hd_inv])
    ...
    thus "ereal (eval_cexpr f \<rho> (op_sem Inverse x)) * ereal ((inverse (extract_real x))\<^sup>2) =
            ereal (eval_cexpr ?expr \<rho> x)"
      by (auto simp: eval_cexpr_def lift_RealVal_def lift_RealIntVal2_def 
                     extract_real_def cexpr_sem_cexpr_comp elim!: REAL_E)
  qed (insert edc_inv.prems ctype vars dens, 
       auto intro!: density_context_\<alpha> is_density_exprI simp: dens_ctxt_\<alpha>_def)
...
next
  ...
  proof (intro conjI, simp only: dens_ctxt_\<alpha>_def prod.case, rule hd_cong[OF hd_exp])
    ...
    from \<rho> and wf' have "val_type (cexpr_sem (case_nat (RealVal (ln x')) \<rho>) f) = REAL"
      by (intro val_type_cexpr_sem[OF _ _ case_nat_in_state_measure ])
         (auto simp: type_universe_def simp del: type_universe_type)
    thus "(if 0 < extract_real x then ereal (eval_cexpr f \<rho> (lift_RealVal safe_ln x)) * 
              ereal (inverse (extract_real x)) else 0) = ereal (eval_cexpr ?expr \<rho> x)"
      by (auto simp: eval_cexpr_def lift_RealVal_def lift_RealIntVal2_def lift_Comp_def
                     extract_real_def cexpr_sem_cexpr_comp elim!: REAL_E)
...
lemma expr_has_density_cexpr_sound:
  ...
  shows "has_subprob_density (expr_sem \<sigma> e) (stock_measure t) (\<lambda>x. ereal (eval_cexpr f \<sigma> x))"
        "\<Gamma>' 0 = t \<Longrightarrow> \<Gamma>' \<turnstile>\<^sub>c f : REAL" 
        "free_vars f \<subseteq> {0}"
proof-
  have "dens_ctxt_\<alpha> ([], [], \<Gamma>, CReal 1) \<turnstile>\<^sub>d e \<Rightarrow> (\<lambda>\<rho> x. ereal (eval_cexpr f \<rho> x)) \<and>
  ...
  hence dens: "dens_ctxt_\<alpha> ([], [], \<Gamma>, CReal 1) \<turnstile>\<^sub>d e \<Rightarrow> (\<lambda>\<rho> x. ereal (eval_cexpr f \<rho> x))"
    and wf: "is_density_expr ([], [], \<Gamma>, CReal 1) t f" using assms by blast+
  
  have "has_subprob_density (expr_sem \<sigma> e) (stock_measure t) 
             (\<lambda>x. ereal (eval_cexpr f (\<lambda>_. undefined) x))" (is ?P) using dens assms
    by (intro expr_has_density_sound) (auto simp: dens_ctxt_\<alpha>_def extract_real_def one_ereal_def)
  ...
  hence "?P \<longleftrightarrow> has_subprob_density (expr_sem \<sigma> e) (stock_measure t) 
                      (\<lambda>x. ereal (eval_cexpr f \<sigma> x))"
...
lemma expr_compiles_to_sound:
  ...
  shows  "expr_sem \<sigma> e = density (stock_measure t) (\<lambda>x. ereal (eval_cexpr f \<sigma>' x))"
...
proof-
  ...
  with expr_has_density_cexpr_sound[OF A]
    show "expr_sem \<sigma> e = density (stock_measure t) (\<lambda>x. ereal (eval_cexpr f \<sigma>' x))"
         "\<forall>x\<in>type_universe t. eval_cexpr f \<sigma>' x \<ge> 0"
         "t \<cdot> \<Gamma>' \<turnstile>\<^sub>c f : REAL" 
         "free_vars f \<subseteq> {0}" unfolding has_subprob_density_def has_density_def by auto
