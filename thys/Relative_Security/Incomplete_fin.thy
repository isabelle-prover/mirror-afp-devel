...
lemma tr1_shape:"Opt.validFromS ss tr1 \<Longrightarrow> completedFromO ss tr1 \<Longrightarrow> tr1 = [ss, ss']"  
...
lemma noUnwind: shows init: "\<Delta> \<infinity> ss ss Eq ss ss Eq \<Longrightarrow> unwindCond \<Delta> \<Longrightarrow> False"
  subgoal premises unwind using unwind(2)[unfolded unwindCond_def]
  (*step1*)
  apply-apply(erule allE[of _ "\<infinity>"])
