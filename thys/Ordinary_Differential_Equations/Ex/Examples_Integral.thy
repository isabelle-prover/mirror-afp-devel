...
lemma ex2_3: "\<bar>integral {0 .. 1} (\<lambda>x::real. arctan (sqrt (x\<^sup>2 + 2)) / (sqrt (x\<^sup>2 + 2) * (x\<^sup>2 + 1))) - 5*pi\<^sup>2/96\<bar> \<le> 1 / 10^3"
  by (rule abs_minus_leI) (tactic \<open>integral_bnds_tac 1 30 10 6 12 "" @{context} 1\<close>)

lemma ex2_6: "\<bar>integral {0 .. 1} (\<lambda>x::real. arctan (sqrt (x\<^sup>2 + 2)) / (sqrt (x\<^sup>2 + 2) * (x\<^sup>2 + 1))) - 5*pi\<^sup>2/96\<bar> \<le> 1 / 10^6"
  by (rule abs_minus_leI) (tactic \<open>integral_bnds_tac 1 40 10 6 26 "" @{context} 1\<close>)
...
lemma
  ...
  by (rule abs_minus_leI) (tactic \<open>integral_bnds_tac 1 40 3 6 15 "" @{context} 1\<close>)
(*714 *)
lemma
