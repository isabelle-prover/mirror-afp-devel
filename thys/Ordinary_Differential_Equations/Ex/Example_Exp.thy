...
lemma exp_ivp_fderiv: "FDERIV (\<lambda>(x::real, y::real). (x, y))
   x : X
   :> (\<lambda>(a, b) (h\<^sub>1, h\<^sub>2). (h\<^sub>1, h\<^sub>2 + 0 *a*b)) x"
  by (auto intro!: FDERIV_eq_intros simp: split_beta id_def)
