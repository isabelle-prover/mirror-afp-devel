...
assumes wf:"wf_C_prog P"
shows "P,E \<turnstile> \<langle>e,s\<rangle> \<Rightarrow> \<langle>e\<^isub>1,s\<^isub>1\<rangle> \<Longrightarrow> 
       (\<And>e\<^isub>2 s\<^isub>2 T. \<lbrakk>P,E \<turnstile> \<langle>e,s\<rangle> \<Rightarrow> \<langle>e\<^isub>2,s\<^isub>2\<rangle>; P,E \<turnstile> e :: T; P,E \<turnstile> s \<surd>\<rbrakk> 
       \<Longrightarrow> e\<^isub>1 = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2)"
  and "P,E \<turnstile> \<langle>es,s\<rangle> [\<Rightarrow>] \<langle>es\<^isub>1,s\<^isub>1\<rangle> \<Longrightarrow> 
       (\<And>es\<^isub>2 s\<^isub>2 Ts. \<lbrakk>P,E \<turnstile> \<langle>es,s\<rangle> [\<Rightarrow>] \<langle>es\<^isub>2,s\<^isub>2\<rangle>; P,E \<turnstile> es [::] Ts; P,E \<turnstile> s \<surd>\<rbrakk> 
        \<Longrightarrow> es\<^isub>1 = es\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2)"
...
next
  ...
  proof(rule eval_cases)
    fix Xs Xs' a'
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref (a',Xs),s\<^isub>2\<rangle>" 
      and path_via':"P \<turnstile> Path last Xs to C via Xs'"
      and ref:"e\<^isub>2 = ref (a',Xs@\<^sub>pXs')"
    from IH[OF eval_ref wte sconf] have eq:"a = a' \<and> Cs = Xs \<and> s\<^isub>1 = s\<^isub>2" by simp
    with wf eval_ref sconf wte have last:"last Cs = D"
      by(auto dest:eval_preserves_type split:split_if_asm)
    from disj show "ref (a,Ds) = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2"
    proof (rule disjE)
  ...
  next
    ...
    from IH[OF eval_null wte sconf] show "ref (a,Ds) = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix Xs a'
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),s\<^isub>2\<rangle>" and notin:"C \<notin> set Xs"
      and notleq:"\<not> P \<turnstile> last Xs \<preceq>\<^sup>* C" and throw:"e\<^isub>2 = THROW ClassCast"
    from IH[OF eval_ref wte sconf] have eq:"a = a' \<and> Cs = Xs \<and> s\<^isub>1 = s\<^isub>2" by simp
...
next
  ...
  next
    assume eval_null:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<^isub>2\<rangle>"
    from IH[OF eval_null wte sconf] show "ref (a,Cs@[C]) = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix Xs a' 
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),s\<^isub>2\<rangle>" and notin:"C \<notin> set Xs"
...
next
  ...
  proof(rule eval_cases)
    fix Xs Xs' a'
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref (a',Xs),s\<^isub>2\<rangle>" 
    from IH[OF eval_ref wte sconf] show "null = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix Xs Xs' a'
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs@C#Xs'),s\<^isub>2\<rangle>"
    from IH[OF eval_ref wte sconf] show "null = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    assume eval_null:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<^isub>2\<rangle>" and "e\<^isub>2 = null"
    with IH[OF eval_null wte sconf] show "null = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix Xs a' 
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),s\<^isub>2\<rangle>"
    from IH[OF eval_ref wte sconf] show "null = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
...
next
  ...
  proof(rule eval_cases)
    ...
    with path_via wf have "P \<turnstile> last Cs \<preceq>\<^sup>* C" 
      by(auto dest:Subobjs_subclass simp:path_via_def)
    with notleq show "THROW ClassCast = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix Xs Xs' a'
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs@C#Xs'),s\<^isub>2\<rangle>"
    from IH[OF eval_ref wte sconf] have "a = a' \<and> Cs = Xs@C#Xs' \<and> s\<^isub>1 = s\<^isub>2" by simp
    with notin show "THROW ClassCast = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    assume eval_null:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<^isub>2\<rangle>"
    from IH[OF eval_null wte sconf] show "THROW ClassCast = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix Xs a' 
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),s\<^isub>2\<rangle>"
      and throw:"e\<^isub>2 = THROW ClassCast"
    from IH[OF eval_ref wte sconf] have "a = a' \<and> Cs = Xs \<and> s\<^isub>1 = s\<^isub>2"
      by simp
    with throw show "THROW ClassCast = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix e' assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2\<rangle>"
...
next
  ...
  next
    ...
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),(h,l)\<rangle>"
      ...
      and path_unique':"P \<turnstile> Path D' to C unique" and s2:"s\<^isub>2 = (h,l)"
  ...
  next
    fix Xs D' S a' h l
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),(h,l)\<rangle>"
      and not_unique:"\<not> P \<turnstile> Path last Xs to C unique" and s2:"s\<^isub>2 = (h,l)"
    from IH[OF eval_ref wte sconf] s2 have eq:"a = a' \<and> Cs = Xs \<and> s\<^isub>1 = s\<^isub>2" by simp
    with path_unique not_unique show "ref (a, Ds) = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix e' assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2\<rangle>"
    from IH[OF eval_throw wte sconf] show "ref (a, Ds) = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  qed
...
next
  ...
  next
    ...
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),(h',l')\<rangle>"
      and h':"h' a' = Some(D'',S')" and path_via':"P \<turnstile> Path D'' to C via Xs'"
      and s2:"s\<^isub>2 = (h',l')" and ref:"e\<^isub>2 = ref(a',Xs')"
  ...
  next
    ...
    from IH[OF eval_ref wte sconf] have eq:"a = a' \<and> Cs = Xs \<and> h = h' \<and> l = l'"
      by simp
    with h h' path_unique not_unique show "ref(a,Cs') = e\<^isub>2 \<and> (h,l) = s\<^isub>2" by simp
  next
    fix e' assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2\<rangle>"
...
next
  ...
  next
    fix Xs D' S a' h l
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),(h,l)\<rangle>" and s2:"s\<^isub>2 = (h,l)"
    from IH[OF eval_ref wte sconf] s2 show "null = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix e' assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2\<rangle>"
    from IH[OF eval_throw wte sconf] show "null = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  qed
next
  ...
  proof(rule eval_cases)
    ...
    from IH[OF eval_ref wte sconf] have eq:"a = a' \<and> Cs = Xs \<and> (h,l) = s\<^isub>2" by simp
    with path_unique not_unique2 show "null = e\<^isub>2 \<and> (h,l) = s\<^isub>2" by simp
  next
    fix Xs Xs' a'
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs@C#Xs'),s\<^isub>2\<rangle>"
    from IH[OF eval_ref wte sconf] have eq:"a = a' \<and> Cs = Xs@C#Xs' \<and> (h,l) = s\<^isub>2"
  ...
  next
    ...
    from IH[OF eval_ref wte sconf] have "a = a' \<and> Cs = Xs \<and> h = h' \<and> l = l'"
      by simp
    with h h' not_unique1 path_unique show "null = e\<^isub>2 \<and> (h,l) = s\<^isub>2" by simp
  next
    assume eval_null:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<^isub>2\<rangle>"
    from IH[OF eval_null wte sconf] show "null = e\<^isub>2 \<and> (h,l) = s\<^isub>2" by simp
  next
    fix Xs D'' S' a' h' l'
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),(h',l')\<rangle>"
      and null:"e\<^isub>2 = null" and s2:"s\<^isub>2 = (h',l')"
    from IH[OF eval_ref wte sconf] null s2 show "null = e\<^isub>2 \<and> (h,l) = s\<^isub>2" by simp
  next
    fix e' assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2\<rangle>"
...
next
  ...
  from eval show ?case
  proof(rule eval_cases)
    fix s w w\<^isub>1 w\<^isub>2
    assume eval_val1:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w\<^isub>1,s\<rangle>"
    ...
    with e2' s2 show "Val v = e\<^isub>2' \<and> s\<^isub>2 = s\<^isub>2'" by simp
  next
    fix e assume eval_throw:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e,s\<^isub>2'\<rangle>"
    from IH1[OF eval_throw wte1 sconf] show "Val v = e\<^isub>2' \<and> s\<^isub>2 = s\<^isub>2'" by simp
  next
    fix e s w 
    assume eval_val:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,s\<rangle>"
...
next
  ...
  proof(rule eval_cases)
    fix s w w\<^isub>1 w\<^isub>2
    assume eval_val:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w\<^isub>1,s\<rangle>"
    from IH[OF eval_val wte1 sconf] show "throw e = e\<^isub>2' \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix e' 
    assume eval_throw:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2\<rangle>" and throw:"e\<^isub>2' = throw e'"
    from IH[OF eval_throw wte1 sconf] throw show "throw e = e\<^isub>2' \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix e s w 
    assume eval_val:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,s\<rangle>"
    from IH[OF eval_val wte1 sconf] show "throw e = e\<^isub>2' \<and> s\<^isub>1 = s\<^isub>2" by simp
...
next
  ...
  next
    fix e' 
    assume eval_throw:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2'\<rangle>"
    from IH1[OF eval_throw wte1 sconf] show "throw e = e\<^isub>2' \<and> s\<^isub>2 = s\<^isub>2'" by simp
  next
    fix e' s w 
    assume eval_val:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,s\<rangle>"
      and eval_throw:"P,E \<turnstile> \<langle>e\<^isub>2,s\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2'\<rangle>"
...
next
  ...
  proof(rule eval_cases)
    fix U h' l'' w w'
    assume eval_val:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,(h',l'')\<rangle>" and env':"E V = Some U"
      and casts':"P \<turnstile> U casts w to w'" and e2:"e\<^isub>2 = Val w'" 
      and s2:"s\<^isub>2 = (h',l''(V \<mapsto> w'))"
    ...
    with e2 s2 l' eq show "Val v' = e\<^isub>2 \<and> (h, l') = s\<^isub>2" by simp
  next
    fix e' assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2\<rangle>"
...
next
  ...
  proof(rule eval_cases)
    fix U h' l'' w w'
    assume eval_val:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,(h',l'')\<rangle>"
...
next
  ...
  proof(rule eval_cases)
    ...
    and fs':"fs' F = Some v'" and e2:"e\<^isub>2 = Val v'" and s2:"s\<^isub>2 = (h',l')"
    ...
    with e2 s2 eq show "Val v = e\<^isub>2 \<and> (h,l) = s\<^isub>2" by simp
  next
    assume eval_null:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<^isub>2\<rangle>"
    from IH[OF eval_null wte sconf] show "Val v = e\<^isub>2 \<and> (h,l) = s\<^isub>2" by simp
  next
    fix e' assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2\<rangle>"
    from IH[OF eval_throw wte sconf] show "Val v = e\<^isub>2 \<and> (h,l) = s\<^isub>2" by simp
  qed
next
  ...
  proof(rule eval_cases)
    fix Xs' D' S' a' fs' h' l' v'
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs'),(h',l')\<rangle>"
    from IH[OF eval_ref wte sconf] show "THROW NullPointer = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    assume eval_null:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<^isub>2\<rangle>" and e2:"e\<^isub>2 = THROW NullPointer"
    from IH[OF eval_null wte sconf] e2 show "THROW NullPointer = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" 
      by simp
  next
    fix e' assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw e',s\<^isub>2\<rangle>"
...
next
  ...
  have eval:"P,E \<turnstile> \<langle>e\<^isub>1\<bullet>F{Cs} := e\<^isub>2,s\<^isub>0\<rangle> \<Rightarrow> \<langle>e\<^isub>2',s\<^isub>2\<rangle>"
    ...
    and h2':"h\<^isub>2' = h\<^isub>2(a \<mapsto> (D, S'))"
  ...
  proof(rule eval_cases)
    ...
    assume eval_ref:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),s\<rangle>"
      ...
      and S'':"(Xs@\<^sub>pCs,fs'') \<in> S''" and e2':"e\<^isub>2' = Val w'"
      and s2:"s\<^isub>2 = (h(a'\<mapsto>(D',insert (Xs@\<^sub>pCs,fs''(F \<mapsto> w')) 
                                     (S''-{(Xs@\<^sub>pCs,fs'')}))),l)"
    ...
    have oconf:"P,h\<^isub>2 \<turnstile> (D,S) \<surd>"
      by(simp add:sconf_def hconf_def)
    from eq eq' h2 h have "S = S''" by simp
    with oconf eq S S' S'' Ds have "fs = fs''" by (auto simp:oconf_def)
    with h2' h h2 eq eq' s2 S' Ds fs' v' e2' show "Val v' = e\<^isub>2' \<and> (h\<^isub>2',l\<^isub>2) = s\<^isub>2"
      by simp
  next
    fix s w assume eval_null:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<rangle>"
    from IH1[OF eval_null wte1 sconf] show "Val v' = e\<^isub>2' \<and> (h\<^isub>2',l\<^isub>2) = s\<^isub>2" by simp
  next
    fix ex assume eval_throw:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2\<rangle>"
    from IH1[OF eval_throw wte1 sconf] show "Val v' = e\<^isub>2' \<and> (h\<^isub>2',l\<^isub>2) = s\<^isub>2" by simp
  next
    fix ex s w
    assume eval_val:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,s\<rangle>" 
...
next
  ...
  proof(rule eval_cases)
    fix Xs D' S'' U a' fs'' h l s w w'
    assume eval_ref:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),s\<rangle>"
    from IH1[OF eval_ref wte1 sconf] show "THROW NullPointer = e\<^isub>2' \<and> s\<^isub>2 = s\<^isub>2'"
      by simp
  next
    fix s w 
    assume eval_null:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<rangle>"
  ...
  next
    fix ex assume eval_throw:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2'\<rangle>"
    from IH1[OF eval_throw wte1 sconf] show "THROW NullPointer = e\<^isub>2' \<and> s\<^isub>2 = s\<^isub>2'" 
      by simp
  next
    fix ex s w
    assume eval_val:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,s\<rangle>"
...
next
  ...
  proof(rule eval_cases)
    fix Xs D' S'' U a' fs'' h l s w w'
    assume eval_ref:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),s\<rangle>"
    from IH[OF eval_ref wte1 sconf] show "throw e' = e\<^isub>2' \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix s w 
    assume eval_null:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<rangle>"
    from IH[OF eval_null wte1 sconf] show "throw e' = e\<^isub>2' \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix ex
    assume eval_throw:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2\<rangle>" and e2':"e\<^isub>2' = throw ex"
    from IH[OF eval_throw wte1 sconf] e2' show "throw e' = e\<^isub>2' \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix ex s w assume eval_val:"P,E \<turnstile> \<langle>e\<^isub>1,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,s\<rangle>"
    from IH[OF eval_val wte1 sconf] show "throw e' = e\<^isub>2' \<and> s\<^isub>1 = s\<^isub>2" by simp
  qed
next
...
next
  ...
  have eval:"P,E \<turnstile> \<langle>e\<bullet>M(es),s\<^isub>0\<rangle> \<Rightarrow> \<langle>e\<^isub>2,s\<^isub>2\<rangle>"
    ...
    and l2':"l\<^isub>2' = [this \<mapsto> Ref (a, Cs'), pns [\<mapsto>] vs']"
  ...
  next
    ...
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref(a',Xs),s\<rangle>"
      ...
      \<langle>case U' of Class D \<Rightarrow> \<lparr>D\<rparr>body'' | _ \<Rightarrow> body'',
        (h,[this \<mapsto> Ref(a',Xs'), pns'' [\<mapsto>] ws'])\<rangle> \<Rightarrow> \<langle>e\<^isub>2,(h',l')\<rangle>"
      and s2:"s\<^isub>2 = (h',l)"
...
next
  ...
  from eval obtain h l where 
    eval':"P,E(V \<mapsto> T) \<turnstile> \<langle>e\<^isub>0,(h\<^isub>0,l\<^isub>0(V:=None))\<rangle> \<Rightarrow> \<langle>e\<^isub>2,(h,l)\<rangle>"
...
next
  ...
  proof(rule eval_cases)
    fix s w 
    assume eval_val:"P,E \<turnstile> \<langle>e\<^isub>0,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,s\<rangle>" 
...
next
  ...
  proof(rule eval_cases)
    fix s w 
    assume eval_val:"P,E \<turnstile> \<langle>e\<^isub>0,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,s\<rangle>"
    from IH[OF eval_val wte0 sconf] show "throw e = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix ex 
    assume eval_throw:"P,E \<turnstile> \<langle>e\<^isub>0,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2\<rangle>" and e2:"e\<^isub>2 = throw ex"
...
next
  ...
  next
    ...
    from IH1[OF eval_false wte sconf] show "e' = e\<^isub>2' \<and> s\<^isub>2 = s\<^isub>2'" by simp
  next
    fix ex assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2'\<rangle>"
...
next
  ...
  proof(rule eval_cases)
    fix s 
    assume eval_true:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>true,s\<rangle>"
    from IH1[OF eval_true wte sconf] show "e' = e\<^isub>2' \<and> s\<^isub>2 = s\<^isub>2'" by simp
  next
    fix s
    assume eval_false:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>false,s\<rangle>"
...
next
  ...
  next
    fix s assume eval_false:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>false,s\<rangle>"
    from IH[OF eval_false wte sconf] show "throw e' = e\<^isub>2' \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix ex
    assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2\<rangle>" and e2':"e\<^isub>2' = throw ex"
    from IH[OF eval_throw wte sconf] e2' show "throw e' = e\<^isub>2' \<and> s\<^isub>1 = s\<^isub>2" by simp
...
next
  ...
  from eval show ?case
  proof(rule eval_cases)
    assume eval_false:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>false,s\<^isub>2\<rangle>" and e2:"e\<^isub>2 = unit"
    from IH[OF eval_false wte sconf] e2 show "unit = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix s s' w
    assume eval_true:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>true,s\<rangle>"
  ...
  next
    fix ex assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2\<rangle>"
    from IH[OF eval_throw wte sconf] show "unit = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix ex s
    assume eval_true:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>true,s\<rangle>"
    from IH[OF eval_true wte sconf] show "unit = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
...
next
  ...
  from eval show ?case
  proof(rule eval_cases)
    assume eval_false:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>false,s\<^isub>2'\<rangle>"
    from IH1[OF eval_false wte sconf] show "e\<^isub>3 = e\<^isub>2 \<and> s\<^isub>3 = s\<^isub>2'" by simp
  next
    fix s s' w
    assume eval_true:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>true,s\<rangle>" 
 ...
 next
   fix ex assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2'\<rangle>"
   from IH1[OF eval_throw wte sconf] show "e\<^isub>3 = e\<^isub>2 \<and> s\<^isub>3 = s\<^isub>2'" by simp
 next
   fix ex s
   assume eval_true:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>true,s\<rangle>" 
...
next
  ...
  proof(rule eval_cases)
    assume eval_false:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>false,s\<^isub>2'\<rangle>"
    from IH1[OF eval_false wte sconf] show "throw e' = e\<^isub>2 \<and> s\<^isub>2 = s\<^isub>2'" by simp
  next
    fix s s' w
    assume eval_true:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>true,s\<rangle>" 
      and eval_val:"P,E \<turnstile> \<langle>c,s\<rangle> \<Rightarrow> \<langle>Val w,s'\<rangle>"
 ...
 next
   fix ex assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2'\<rangle>"
   from IH1[OF eval_throw wte sconf] show "throw e' = e\<^isub>2 \<and> s\<^isub>2 = s\<^isub>2'" by simp
 next
   fix ex s
   assume eval_true:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>true,s\<rangle>" 
...
next
  ...
  proof(rule eval_cases)
    fix r'
    assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref r',s\<^isub>2\<rangle>" and e2:"e\<^isub>2 = Throw r'"
    from IH[OF eval_ref wte sconf] e2 show "Throw r = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    assume eval_null:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<^isub>2\<rangle>"
    from IH[OF eval_null wte sconf] show "Throw r = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix ex assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2\<rangle>"
    from IH[OF eval_throw wte sconf] show "Throw r = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  qed
next
  case (ThrowNull E e s\<^isub>0 s\<^isub>1 e\<^isub>2 s\<^isub>2 T)
  have eval:"P,E \<turnstile> \<langle>throw e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>e\<^isub>2,s\<^isub>2\<rangle>"
    and wt:"P,E \<turnstile> throw e :: T" and sconf:"P,E \<turnstile> s\<^isub>0 \<surd>"
    and IH:"\<And>ei si T. \<lbrakk>P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ei,si\<rangle>; P,E \<turnstile> e :: T; P,E \<turnstile> s\<^isub>0 \<surd>\<rbrakk> 
                   \<Longrightarrow> null = ei \<and> s\<^isub>1 = si" .
  ...
  from eval show ?case
  proof(rule eval_cases)
    fix r' assume eval_ref:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>ref r',s\<^isub>2\<rangle>"
    from IH[OF eval_ref wte sconf] show "THROW NullPointer = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    assume eval_null:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>null,s\<^isub>2\<rangle>" and e2:"e\<^isub>2 = THROW NullPointer"
    from IH[OF eval_null wte sconf] e2 show "THROW NullPointer = e\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" 
...
next
  ...
  proof(rule evals_cases)
    fix es'' s w
    assume eval_val:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>Val w,s\<rangle>"
    from IH[OF eval_val wte sconf] show "throw e'#es = es\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  next
    fix ex 
    assume eval_throw:"P,E \<turnstile> \<langle>e,s\<^isub>0\<rangle> \<Rightarrow> \<langle>throw ex,s\<^isub>2\<rangle>" and es2:"es\<^isub>2 = throw ex#es"
    from IH[OF eval_throw wte sconf] es2 show "throw e'#es = es\<^isub>2 \<and> s\<^isub>1 = s\<^isub>2" by simp
  qed
qed
