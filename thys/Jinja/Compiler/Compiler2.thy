(*  Title:      Jinja/Compiler/Compiler2.thy
    ID:         $Id: Compiler2.thy,v 1.1 2005-05-31 23:21:04 lsf37 Exp $
...
consts
  compE\<^isub>2  :: "expr\<^isub>1      \<Rightarrow> instr list"
  compEs\<^isub>2 :: "expr\<^isub>1 list \<Rightarrow> instr list"
...
consts
  compxE\<^isub>2  :: "expr\<^isub>1      \<Rightarrow> pc \<Rightarrow> nat \<Rightarrow> ex_table"
  compxEs\<^isub>2 :: "expr\<^isub>1 list \<Rightarrow> pc \<Rightarrow> nat \<Rightarrow> ex_table"
...
consts
  max_stack :: "expr\<^isub>1 \<Rightarrow> nat"
  max_stacks :: "expr\<^isub>1 list \<Rightarrow> nat"
primrec
"max_stack (new C) = 1"
