(*
    ID:         $Id: LBVCorrect.thy,v 1.1 2005-05-31 23:21:04 lsf37 Exp $
    Author:     Gerwin Klein
...
locale (open) lbvs = lbv +
  ...
  fixes \<tau>s  :: "'a list"
  defines phi_def:
  "\<tau>s \<equiv> map (\<lambda>pc. if c!pc = \<bottom> then wtl (take pc ins) c 0 s\<^isub>0 else c!pc) 
       [0..<size ins]"
...
lemma (in lbvs) phi_None [intro?]:
  "\<lbrakk> pc < size ins; c!pc = \<bottom> \<rbrakk> \<Longrightarrow> \<tau>s!pc = wtl (take pc ins) c 0 s\<^isub>0"
(*<*) by (simp add: phi_def) (*>*)
...
lemma (in lbvs) wtl_suc_pc:
  assumes all: "wtl ins c 0 s\<^isub>0 \<noteq> \<top>" 
  assumes pc:  "pc+1 < size ins"
  shows "wtl (take (pc+1) ins) c 0 s\<^isub>0 \<sqsubseteq>\<^sub>r \<tau>s!(pc+1)"
(*<*)
proof -
  from all pc
  have "wtc c (pc+1) (wtl (take (pc+1) ins) c 0 s\<^isub>0) \<noteq> T" by (rule wtl_all)
...
lemma (in lbvs) wtl_stable:
  ...
  assumes s\<^isub>0:  "s\<^isub>0 \<in> A" and  pc:  "pc < size ins" 
...
proof (unfold stable_def, clarify)
  ...
  proof (cases "pc' = pc+1")
    ...
    with tkpc have "?s\<^isub>2 = wtc c pc ?s\<^isub>1" by - (rule wtl_Suc)
    with inst 
    ...
    also from wtl pc1 have "?s\<^isub>2 \<sqsubseteq>\<^sub>r \<tau>s!(pc+1)" by (rule wtl_suc_pc)
    also note True [symmetric]
    finally show ?thesis by simp    
  next
    case False
    from wt_s\<^isub>1 inst 
...
lemma (in lbvs) phi_not_top:
  assumes wtl: "wtl ins c 0 s\<^isub>0 \<noteq> \<top>" and pc:  "pc < size ins"
...
next
  case True with pc
  have "\<tau>s!pc = wtl (take pc ins) c 0 s\<^isub>0" ..
...
lemma (in lbvs) phi0:
  assumes wtl: "wtl ins c 0 s\<^isub>0 \<noteq> \<top>" and 0: "0 < size ins"
  shows "s\<^isub>0 \<sqsubseteq>\<^sub>r \<tau>s!0"
(*<*)
proof (cases "c!0 = \<bottom>")
  case True
  with 0 have "\<tau>s!0 = wtl (take 0 ins) c 0 s\<^isub>0" ..
  moreover have "wtl (take 0 ins) c 0 s\<^isub>0 = s\<^isub>0" by simp
  ultimately have "\<tau>s!0 = s\<^isub>0" by simp
  thus ?thesis by simp
next
  case False
