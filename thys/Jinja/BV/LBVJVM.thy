...
proof -  
  ...
  let ?A = "\<lambda>P C (mn,Ts,T\<^sub>r,(mxs,mxl\<^sub>0,ins,et)). wt_lbv P C Ts T\<^sub>r mxs mxl\<^sub>0 et (Cert C mn) ins"
  let ?B = "\<lambda>P C (M,Ts,T\<^sub>r,(mxs,mxl\<^sub>0,is,xt)). 
...
proof -
  ...
  let ?A = "\<lambda>P C (M,Ts,T\<^sub>r,(mxs,mxl\<^sub>0,is,xt)). 
                wt_method P C Ts T\<^sub>r mxs mxl\<^sub>0 is xt (\<Phi> C M)"
  let ?B = "\<lambda>P C (mn,Ts,T\<^sub>r,(mxs,mxl\<^sub>0,ins,et)).
                wt_lbv P C Ts T\<^sub>r mxs mxl\<^sub>0 et (?cert C mn) ins"
