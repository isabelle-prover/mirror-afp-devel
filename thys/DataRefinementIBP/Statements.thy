...
theorem demonic_bottom_top [simp]:
  "demonic \<bottom>  = \<top>"
  by (simp add: fun_eq_iff inf_fun_def sup_fun_def demonic_def inf_bool_def top_fun_def bot_fun_def)

theorem demonic_sup_inf:
  "demonic (Q \<squnion> Q') = demonic Q \<sqinter> demonic Q'"
  by (simp add: fun_eq_iff inf_fun_def sup_fun_def demonic_def inf_bool_def Collect_conj_eq)
...
by (simp add: fun_eq_iff inf_fun_def sup_fun_def angelic_def inf_bool_def sup_bool_def inf_sup_distrib1 Collect_imp_eq Collect_neg_eq)
