...
proof(induct rule: simCases)
  ...
  proof -
    ...
    thus ?thesis by simp
  qed
  ultimately obtain P' where PTrans: "P \<Longrightarrow>\<^isub>l\<^isup>^a<\<nu>x> \<prec> P'" and P'RelQ': "(P', Q') \<in> Rel" using PSimQ
    by(blast dest: Weak_Late_Sim.simE)
  from PTrans have "\<tau>.(P) \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P'"
  proof(induct rule: transitionCases)
    case Step
    have "P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P'" by fact
    with xFreshP obtain P''' P'' where PChain: "P \<Longrightarrow>\<^isub>\<tau> P'''"
                                   and P'''Trans:  "P''' \<longmapsto>a<\<nu>x> \<prec> P''"
                                   and P''Chain: "P'' \<Longrightarrow>\<^isub>\<tau> P'"
      by(blast dest: transitionE)
    have "\<tau>.(P) \<Longrightarrow>\<^isub>\<tau> P'''"
    proof -
...
next
  ...
  proof -
    ...
    thus ?thesis by simp
  qed
  ultimately obtain P'' where L1: "\<forall>u. \<exists>P'. P \<Longrightarrow>\<^isub>lu in P''\<rightarrow>a<x> \<prec> P' \<and> (P', Q'[x::=u]) \<in> Rel" using PSimQ
    by(blast dest: Weak_Late_Sim.simE)
  have "\<And>u. \<exists>P'. \<tau>.(P) \<Longrightarrow>\<^isub>lu in P''\<rightarrow>a<x> \<prec> P' \<and> (P', Q'[x::=u]) \<in> Rel"
  proof -
    fix u
    from L1 obtain P' where PTrans: "P \<Longrightarrow>\<^isub>lu in P''\<rightarrow>a<x> \<prec> P'" and P'RelQ': "(P', Q'[x::=u]) \<in> Rel" by blast    
    from PTrans xFreshP obtain P''' where PChain: "P \<Longrightarrow>\<^isub>\<tau> P'''"
                                     and P'''Trans:  "P''' \<longmapsto>a<x> \<prec> P''"
                                     and P''Chain: "P''[x::=u] \<Longrightarrow>\<^isub>\<tau> P'"
      by(blast dest: transitionE)
    have "\<tau>.(P) \<Longrightarrow>\<^isub>\<tau> P'''"
    proof -
      have "\<tau>.(P) \<longmapsto>\<tau> \<prec> P" by(rule Late_Semantics.Tau)
      thus ?thesis using PChain by auto
    qed
    hence "\<tau>.(P) \<Longrightarrow>\<^isub>lu in P''\<rightarrow>a<x> \<prec> P'" using P'''Trans P''Chain by(blast intro: Weak_Late_Step_Semantics.transitionI)
    with P'RelQ' show "\<exists>P'. \<tau>.(P) \<Longrightarrow>\<^isub>lu in P''\<rightarrow>a<x> \<prec> P' \<and> (P', Q'[x::=u]) \<in> Rel" by blast
  qed
  thus ?case by blast
next
  case(Free Q' \<alpha>)
  have Trans: "Q \<longmapsto>\<alpha> \<prec> Q'" by fact
  then obtain P' where PTrans: "P \<Longrightarrow>\<^isub>l\<^isup>^\<alpha> \<prec> P'" and P'RelQ': "(P', Q') \<in> Rel" using PSimQ
    by(blast dest: Weak_Late_Sim.simE)
  from PTrans have "\<tau>.(P) \<Longrightarrow>\<^isub>l\<alpha> \<prec> P'"
  proof(induct rule: transitionCases)
    case Step
    have "P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P'" by fact
  ...
  next
    ...
    moreover have "\<tau>.(P) \<Longrightarrow>\<^isub>l\<tau> \<prec> P"
    proof -
...
next
  ...
  moreover from PTrans have "P \<Longrightarrow>\<^isub>l\<tau> \<prec> P'" by(rule Weak_Late_Step_Semantics.singleActionChain)
...
proof(induct rule: simCases)
  ...
  with PSimQ obtain P' where "P \<Longrightarrow>\<^isub>l\<^isup>^a<\<nu>x> \<prec> P'" and "(P', Q') \<in> Rel" by(blast dest: Weak_Late_Sim.simE)
...
next
  ...
  with PSimQ obtain P' where PTrans: "P \<Longrightarrow>\<^isub>l\<^isup>^\<alpha> \<prec> P'" and P'RelQ': "(P', Q') \<in> Rel"
    by(blast dest: Weak_Late_Sim.simE)
...
proof(case_tac "\<exists>P'. P \<longmapsto>\<tau> \<prec> P' \<and> P' \<approx> Q")
  ...
  proof -
    from PBisimQ have "P \<leadsto>\<^isup>^<weakBisim> Q" by(blast dest: Weak_Late_Bisim.unfoldE)
    thus ?thesis using PTrans P'BisimP by(rule sim2)
  qed
  moreover have "\<tau>.(Q) \<leadsto><weakBisim> P"
  proof -
    from PBisimQ have "Q \<leadsto>\<^isup>^<weakBisim> P" by(blast dest: Weak_Late_Bisim.unfoldE)
...
lemma boundOutputRemoveTau:
  ...
  assumes "\<tau>.(P) \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P'"
  and     "x \<sharp> P"

  shows "P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P'"
proof -
  from assms obtain P''' P'''' where PChain:"\<tau>.(P) \<Longrightarrow>\<^isub>\<tau> P'''"
                                 and P'''Trans: "P''' \<longmapsto>a<\<nu>x> \<prec> P''''"
                                 and P''''Chain: "P'''' \<Longrightarrow>\<^isub>\<tau> P'"
  ...
  next
    ...
    with PChain obtain P'' where PTrans: "\<tau>.(P) \<longmapsto>\<tau> \<prec> P''" and P''Chain: "P'' \<Longrightarrow>\<^isub>\<tau> P'''"
...
lemma inputRemoveTau:
  ...
  assumes "\<tau>.(P) \<Longrightarrow>\<^isub>lu in P'''' \<rightarrow> a<x> \<prec> P'"

  shows "P \<Longrightarrow>\<^isub>lu in P'''' \<rightarrow> a<x> \<prec> P'"
proof -
  from assms obtain P''' where PChain:"\<tau>.(P) \<Longrightarrow>\<^isub>\<tau> P'''"
                            and P'''Trans: "P''' \<longmapsto>a<x> \<prec> P''''"
                            and P''''Chain: "P''''[x::=u] \<Longrightarrow>\<^isub>\<tau> P'"
    by(force dest: Weak_Late_Step_Semantics.transitionE)
  show ?thesis
...
lemma outputRemoveTau:
  ...
  assumes "\<tau>.(P) \<Longrightarrow>\<^isub>la[b] \<prec> P'"

  shows "P \<Longrightarrow>\<^isub>la[b] \<prec> P'"
proof -
  from assms obtain P''' P'''' where PChain:"\<tau>.(P) \<Longrightarrow>\<^isub>\<tau> P'''"
...
lemma tauRemoveTau:
  ...
  assumes PTrans: "\<tau>.(P) \<Longrightarrow>\<^isub>l\<tau> \<prec> P'"
  and     PineqP': "P \<noteq> P'"

  shows "P \<Longrightarrow>\<^isub>l\<tau> \<prec> P'"
proof -
  from PTrans obtain P''' P'''' where PChain:"\<tau>.(P) \<Longrightarrow>\<^isub>\<tau> P'''"
  ...
  proof(case_tac "P''' = \<tau>.(P)")
    ...
    with P'''Trans have "P = P''''"
      by(force elim: tauCases simp add: pi.inject residual.inject)
    with P''''Chain PineqP' obtain P'' where PTrans: "P \<longmapsto>\<tau> \<prec> P''" and P''Chain: "P'' \<Longrightarrow>\<^isub>\<tau> P'"
      by(blast dest: tauChainUnfold)
    moreover have "P \<Longrightarrow>\<^isub>\<tau> P" by simp
...
proof(induct rule: Weak_Late_Sim.simCases)
  ...
  with PSimQ obtain P' where PTrans: "\<tau>.(P) \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P'"
                         and P'RelQ': "(P', Q') \<in> Rel" by(auto dest: simE)
  from PTrans xFreshP have "P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P'" 
    by(blast intro: boundOutputRemoveTau)
  hence "P \<Longrightarrow>\<^isub>l\<^isup>^a<\<nu>x> \<prec> P'" by(simp add: weakTransition_def)
  with P'RelQ' show ?case by blast
next
  case(Input Q' a x)
  have QTrans: "Q \<longmapsto>a<x> \<prec> Q'" and xFreshP: "x \<sharp> P" by fact
  with PSimQ obtain P'' where PSim: "\<forall>u. \<exists>P'. \<tau>.(P) \<Longrightarrow>\<^isub>lu in P'' \<rightarrow> a<x> \<prec> P' \<and> (P', Q'[x::=u]) \<in> Rel"
    by(auto dest: simE)
  have "\<forall>u. \<exists>P'. P \<Longrightarrow>\<^isub>lu in P''\<rightarrow>a<x> \<prec> P' \<and> (P', Q'[x::=u]) \<in> Rel"
  proof(rule allI)
    fix u
    from PSim obtain P' where PTrans: "\<tau>.(P) \<Longrightarrow>\<^isub>lu in P'' \<rightarrow> a<x> \<prec> P'" and P'RelQ': "(P', Q'[x::=u]) \<in> Rel"
      by blast
    with PTrans have "P \<Longrightarrow>\<^isub>lu in P'' \<rightarrow> a<x> \<prec> P'" by(blast intro: inputRemoveTau)
    with P'RelQ' show "\<exists>P'. P \<Longrightarrow>\<^isub>lu in P'' \<rightarrow> a<x> \<prec> P' \<and> (P', Q'[x::=u]) \<in> Rel"  by blast
  qed
  thus ?case by blast
next
  case(Free Q' \<alpha>)
  with PSimQ obtain P' where PTrans: "\<tau>.(P) \<Longrightarrow>\<^isub>l\<alpha> \<prec> P'"
                         and P'RelQ': "(P', Q') \<in> Rel" by(auto dest: simE)
  from PTrans show ?case
  proof(cases \<alpha>, auto)
    fix a b
    assume "\<tau>.(P) \<Longrightarrow>\<^isub>l(a::name)[b] \<prec> P'"
    hence "P \<Longrightarrow>\<^isub>la[b] \<prec> P'" by(rule outputRemoveTau)
    hence "P \<Longrightarrow>\<^isub>l\<^isup>^a[b] \<prec> P'" by(simp add: weakTransition_def)
    with P'RelQ' show "\<exists>P'. P \<Longrightarrow>\<^isub>l\<^isup>^a[b] \<prec> P' \<and> (P', Q') \<in> Rel" by blast
  next
    assume PTrans: "\<tau>.(P) \<Longrightarrow>\<^isub>l\<tau> \<prec> P'"
    thus "\<exists>P'. P \<Longrightarrow>\<^isub>l\<^isup>^\<tau> \<prec> P' \<and> (P', Q') \<in> Rel"
    proof(case_tac "P = P'")
      assume "P = P'"
      hence "P \<Longrightarrow>\<^isub>l\<^isup>^\<tau> \<prec> P'" by(simp add: weakTransition_def)
      with P'RelQ' show ?thesis by blast
    next
      assume "P \<noteq> P'"
      hence "P \<Longrightarrow>\<^isub>l\<tau> \<prec> P'" using PTrans by(rule_tac tauRemoveTau)
      hence "P \<Longrightarrow>\<^isub>l\<^isup>^\<tau> \<prec> P'" by(simp add: weakTransition_def)
      with P'RelQ' show ?thesis by blast
    qed
  qed
...
lemma sim5:
  ...
  and     L1: "\<And>P Q. (P, Q) \<in> Rel \<Longrightarrow> P \<leadsto>\<^isup>^<Rel> Q"

  shows "P \<leadsto>\<^isup>^<Rel> Q"
proof -
  have "\<tau>.(Q) \<longmapsto>\<tau> \<prec> Q" by(rule Late_Semantics.Tau)
  with PSimQ obtain P' where PTrans: "P \<Longrightarrow>\<^isub>l\<tau> \<prec> P'" and P'RelQ: "(P', Q) \<in> Rel"
    by(auto dest: simE)
  from PTrans have PChain:"P \<Longrightarrow>\<^isub>\<tau> P'" by(rule Weak_Late_Step_Semantics.tauTransitionChain)
  from P'RelQ have P'SimQ: "P' \<leadsto>\<^isup>^<Rel> Q" by(rule L1)
  show ?thesis
  proof(induct rule: Weak_Late_Sim.simCases)
    case(Bound Q' a x)
    have QTrans: "Q \<longmapsto>a<\<nu>x> \<prec> Q'" and xFreshP: "x \<sharp> P" by fact
    ...
    with P'SimQ QTrans obtain P'' where P''Trans: "P' \<Longrightarrow>\<^isub>l\<^isup>^a<\<nu>x> \<prec> P''"
                                    and P''RelQ': "(P'', Q') \<in> Rel" by(auto dest: Weak_Late_Sim.simE)
    from PChain P''Trans have "P \<Longrightarrow>\<^isub>l\<^isup>^a<\<nu>x> \<prec> P''" 
  ...
  next
    ...
    have QTrans: "Q \<longmapsto>a<x> \<prec> Q'" and xFreshP: "x \<sharp> P" by fact
    from PChain xFreshP have xFreshP': "x \<sharp> P'" by(rule freshChain)
    with P'SimQ QTrans obtain P''' where PSim: "\<forall>u. \<exists>P''. P' \<Longrightarrow>\<^isub>lu in P''' \<rightarrow> a<x> \<prec> P'' \<and> (P'', Q'[x::=u]) \<in> Rel"
  ...
  next
    case(Free Q' \<alpha>)
    with P'SimQ obtain P'' where P'Trans: "P' \<Longrightarrow>\<^isub>l\<^isup>^\<alpha> \<prec> P''"
                             and P''RelQ': "(P'', Q') \<in> Rel" by(auto dest: Weak_Late_Sim.simE)
    from PChain P'Trans have "P \<Longrightarrow>\<^isub>l\<^isup>^\<alpha> \<prec> P''" by(rule chainTransitionAppend)
    with P''RelQ' show ?case by blast
  qed
qed
