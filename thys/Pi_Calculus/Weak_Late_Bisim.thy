...
lemma monoAux: "A \<subseteq> B \<Longrightarrow> P \<leadsto>\<^isup>^<A> Q \<longrightarrow> P \<leadsto>\<^isup>^<B> Q"
by(auto intro: Weak_Late_Sim.monotonic)

coinductive_set weakBisim :: "(pi \<times> pi) set"
where
  step: "\<lbrakk>P \<leadsto>\<^isup>^<weakBisim> Q; (Q, P) \<in> weakBisim\<rbrakk> \<Longrightarrow> (P, Q) \<in> weakBisim"
monos monoAux
...
lemma weakBisimCoinductAux[case_names weakBisim, case_conclusion weakBisim step, consumes 1]:
  assumes p: "(P, Q) \<in> X"
  and step:  "\<And>P Q. (P, Q) \<in> X \<Longrightarrow> P \<leadsto>\<^isup>^<(X \<union> weakBisim)> Q \<and> ((Q, P) \<in> X \<or> Q \<approx> P)"
...
lemma weakBisimCoinduct[consumes 1, case_names cSim cSym]:
  ...
  assumes "(P, Q) \<in> X"
  and     "\<And>P Q. (P, Q) \<in> X \<Longrightarrow> P \<leadsto>\<^isup>^<(X \<union> weakBisim)> Q"
...
lemma weak_coinduct[case_names weakBisim, case_conclusion weakBisim step, consumes 1]:
  assumes p: "(P, Q) \<in> X"
  and step:  "\<And>P Q. (P, Q) \<in> X \<Longrightarrow> P \<leadsto>\<^isup>^<X> Q \<and> (Q, P) \<in> X"
...
lemma weakBisimWeakCoinduct[consumes 1, case_names cSim cSym]:
  ...
  assumes "(P, Q) \<in> X"
  and     "\<And>P Q. (P, Q) \<in> X \<Longrightarrow> P \<leadsto>\<^isup>^<X> Q"
  and     "\<And>P Q. (P, Q) \<in> X \<Longrightarrow> (Q, P) \<in> X"

  shows "P \<approx> Q"
using assms
by(coinduct rule: weak_coinduct) auto
lemma monotonic: "mono(\<lambda>p x1 x2. \<exists>P Q. x1 = P \<and> x2 = Q \<and> P \<leadsto>\<^isup>^<{(xa, x). p xa x}> Q \<and> Q \<leadsto>\<^isup>^<{(xa, x). p xa x}> P)"
...
lemma unfoldE:
  ...
  shows "P \<leadsto>\<^isup>^<weakBisim> Q"
  and   "Q \<approx> P"
using assms
...
lemma unfoldI:
  ...
  assumes "P \<leadsto>\<^isup>^<weakBisim> Q"
  and     "Q \<approx> P"
...
proof(auto simp add: eqvt_def)
  ...
  moreover have "\<And>P Q perm::name prm. \<lbrakk>P \<leadsto>\<^isup>^<weakBisim> Q\<rbrakk> \<Longrightarrow> (perm \<bullet> P) \<leadsto>\<^isup>^<?X> (perm \<bullet> Q)"
  proof -
    fix P Q
    fix perm::"name prm"
    assume "P \<leadsto>\<^isup>^<weakBisim> Q"
...
lemma strongBisimWeakBisim:
  ...
  shows "P \<approx> Q"
proof -
  have "\<And>P Q. P \<leadsto>[bisim] Q \<Longrightarrow> P \<leadsto>\<^isup>^<(bisim \<union> weakBisim)> Q"
  proof -
    fix P Q
    assume "P \<leadsto>[bisim] Q"
    hence "P \<leadsto>\<^isup>^<bisim> Q" by(rule strongSimWeakSim)
    thus "P \<leadsto>\<^isup>^<(bisim \<union> weakBisim)> Q"
...
proof -
  ...
  moreover have "\<And>P Q R. \<lbrakk>Q \<leadsto>\<^isup>^<weakBisim> R; P \<approx> Q\<rbrakk> \<Longrightarrow>
                          P \<leadsto>\<^isup>^<(?X \<union> weakBisim)> R"
  proof -
    fix P Q R
    assume PBiSimQ: "P \<approx> Q"
    assume "Q \<leadsto>\<^isup>^<weakBisim> R"
    ...
    moreover have "\<And>P Q. P \<approx> Q \<Longrightarrow> P \<leadsto>\<^isup>^<weakBisim> Q" by(rule unfoldE)

    ultimately show "P \<leadsto>\<^isup>^<(?X \<union> weakBisim)> R" using PBiSimQ
...
lemma transitive_coinduct_weak[case_names WeakBisimEarly, case_conclusion WeakBisimEarly step, consumes 2]:
  assumes p: "(P, Q) \<in> X"
  and Eqvt: "eqvt X"
  and step: "\<And>P Q. (P, Q) \<in> X \<Longrightarrow> P \<leadsto>\<^isup>^<(bisim O X O bisim)> Q \<and> (Q, P) \<in> X"

  shows "P \<approx> Q"
proof -
  let ?X = "bisim O X O bisim"

  have Sim: "\<And>P P' Q' Q. \<lbrakk>P \<sim> P'; P'\<leadsto>\<^isup>^<?X> Q'; Q' \<leadsto>[bisim] Q\<rbrakk> \<Longrightarrow>
                          P \<leadsto>\<^isup>^<?X> Q"
  proof -
    fix P P' Q' Q
    assume PBisimP': "P \<sim> P'"
    assume P'SimQ': "P' \<leadsto>\<^isup>^<?X> Q'"
    assume Q'SimQ: "Q' \<leadsto>[bisim] Q"

    show "P \<leadsto>\<^isup>^<?X> Q"
    proof -
      ...
      moreover have "\<And>P Q. P \<sim> Q \<Longrightarrow> P \<leadsto>\<^isup>^<bisim> Q" by(blast dest: Strong_Late_Bisim.bisimE strongSimWeakSim)
  ...
  moreover from step Sim have "\<And>P Q. (P, Q) \<in> ?X \<Longrightarrow> P \<leadsto>\<^isup>^<?X> Q \<and> (Q, P) \<in> ?X"
...
lemma weakBisimTransitiveCoinduct[case_names cSim cSym, consumes 2]:
  assumes p: "(P, Q) \<in> X"
  and Eqvt: "eqvt X"
  and rSim: "\<And>P Q. (P, Q) \<in> X \<Longrightarrow> P \<leadsto>\<^isup>^<(bisim O X O bisim)> Q"
