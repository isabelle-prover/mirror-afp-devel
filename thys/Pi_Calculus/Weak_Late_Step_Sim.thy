...
definition weakStepSimAct :: "pi \<Rightarrow> residual \<Rightarrow> ('a::fs_name) \<Rightarrow> (pi \<times> pi) set \<Rightarrow> bool" where
  "weakStepSimAct P Rs C Rel \<equiv> (\<forall>Q' a x. Rs = a<\<nu>x> \<prec> Q' \<longrightarrow> x \<sharp> C \<longrightarrow> (\<exists>P' . P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel)) \<and>
                           ...
                           (\<forall>Q' \<alpha>. Rs = \<alpha> \<prec> Q' \<longrightarrow> (\<exists>P'. P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P' \<and> (P', Q') \<in> Rel))"

definition weakStepSimAux :: "pi \<Rightarrow> (pi \<times> pi) set \<Rightarrow> pi \<Rightarrow> bool" where
  "weakStepSimAux P Rel Q \<equiv> (\<forall>Q' a x. (Q \<longmapsto>a<\<nu>x> \<prec> Q' \<and> x \<sharp> P) \<longrightarrow> (\<exists>P' . P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel)) \<and>
                           ...
                           (\<forall>Q' \<alpha>. Q \<longmapsto>\<alpha> \<prec> Q' \<longrightarrow> (\<exists>P'. P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P' \<and> (P', Q') \<in> Rel))"
...
lemma simCasesCont[consumes 1, case_names Bound Input Free]:
  ...
  and     Bound: "\<And>Q' a x. \<lbrakk>x \<sharp> C; Q \<longmapsto>a<\<nu>x> \<prec> Q'\<rbrakk> \<Longrightarrow> \<exists>P'. P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel"
  ...
  and     Free:  "\<And>Q' \<alpha>. Q \<longmapsto> \<alpha> \<prec> Q' \<Longrightarrow> (\<exists>P'. P \<Longrightarrow>\<^isub>l \<alpha> \<prec> P' \<and> (P', Q') \<in> Rel)"
...
proof(auto simp add: weakStepSimDef)
  ...
  with cFreshC have "\<exists>P'. P \<Longrightarrow>\<^isub>l a<\<nu>c> \<prec> P' \<and> (P', [(x, c)] \<bullet> Q') \<in> Rel"
    by(rule Bound)
  then obtain P' where PTrans: "P \<Longrightarrow>\<^isub>l a<\<nu>c> \<prec> P'" and P'RelQ': "(P', [(x, c)] \<bullet> Q') \<in> Rel"
    by blast

  from PTrans xFreshP cineqx have xFreshP': "x \<sharp> P'" by(force dest: Weak_Late_Step_Semantics.freshTransition)
  with PTrans have "P \<Longrightarrow>\<^isub>l a<\<nu>x> \<prec> ([(x, c)] \<bullet> P')" by(simp add: alphaBoundResidual name_swap)
  ...
  qed
  ultimately show "\<exists>P'. P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel" by blast
next
  fix Q' a x u
...
lemma simCases[consumes 0, case_names Bound Input Free]:
  ...
  assumes Bound: "\<And>Q' a x. \<lbrakk>Q \<longmapsto>a<\<nu>x> \<prec> Q'; x \<sharp> P\<rbrakk> \<Longrightarrow> \<exists>P'. P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel"
...
lemma simActBoundCases[consumes 1, case_names Input BoundOutput]:
  ...
  and     DerBoundOutput: "\<And>b. a = BoundOutputS b \<Longrightarrow> (\<exists>P'. (P \<Longrightarrow>\<^isub>lb<\<nu>x> \<prec> P') \<and> (P', Q') \<in> Rel)"
...
proof(simp add: weakStepSimAct_def fresh_prod, auto)
  ...
  from yFreshP DerBoundOutput[OF this] Eq show "\<exists>P'. P \<Longrightarrow>\<^isub>lb<\<nu>y> \<prec> P' \<and> (P', Q'') \<in> Rel"
  proof(cases "x=y", auto simp add: residual.inject name_abs_eq)
    fix P'
    assume PTrans: "P \<Longrightarrow>\<^isub>lb<\<nu>x> \<prec> P'"
    ...
    ultimately show "\<exists>P'. P \<Longrightarrow>\<^isub>lb<\<nu>y> \<prec> P' \<and> (P', Q'') \<in> Rel" using PTrans by auto
  qed
next
  fix Q'' b y u
...
lemma simActFreeCases[consumes 0, case_names Free]:
  ...
  assumes Der: "\<exists>P'. (P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P') \<and> (P', Q') \<in> Rel"
...
lemma simE:
  ...
  shows "Q \<longmapsto>a<\<nu>x> \<prec> Q' \<Longrightarrow> x \<sharp> P \<Longrightarrow> \<exists>P'. P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel"
  ...
  and   "Q \<longmapsto>\<alpha> \<prec> Q' \<Longrightarrow> (\<exists>P'. P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P' \<and> (P', Q') \<in> Rel)"
...
proof -
  ...
  next
    ...
    ultimately have "\<exists>P''. P' \<Longrightarrow>\<^isub>l\<tau> \<prec> P'' \<and> (P'', Q'') \<in> Rel" by(rule simE)
    then obtain P'' where P'Trans: "P' \<Longrightarrow>\<^isub>l\<tau> \<prec> P''" and P''RelQ'': "(P'', Q'') \<in> Rel" by blast
    from P'Trans have "P' \<Longrightarrow>\<^isub>\<tau> P''" by(rule Weak_Late_Step_Semantics.tauTransitionChain)
...
proof(auto simp add: weakStepSimDef)
  ...
  from PTrans have "P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P'" by(rule Weak_Late_Step_Semantics.singleActionChain)
  thus "\<exists>P'. P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel" using P'RelQ' by blast
next
  fix Q' a x u
...
next
  ...
  from PTrans have "P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P'" by(rule Weak_Late_Step_Semantics.singleActionChain)
  thus "\<exists>P'. P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P' \<and> (P', Q') \<in> Rel" using P'RelQ' by blast
qed
...
proof(induct rule: simCasesCont[of _ "perm \<bullet> P"])
  ...
  ultimately obtain P' where PTrans: "P \<Longrightarrow>\<^isub>l (rev perm \<bullet> a)<\<nu>(rev perm \<bullet> x)> \<prec> P'"
                         and P'RelQ': "(P', rev perm \<bullet> Q') \<in> Rel" using Sim
    by(blast dest: simE)

  from PTrans have "(perm \<bullet> P) \<Longrightarrow>\<^isub>l perm \<bullet> ((rev perm \<bullet> a)<\<nu>(rev perm \<bullet> x)> \<prec> P')" 
    by(rule Weak_Late_Step_Semantics.eqvtI)
  hence "(perm \<bullet> P) \<Longrightarrow>\<^isub>l a<\<nu>x> \<prec> (perm \<bullet> P')" by(simp add: name_per_rev)
...
next
  ...
  ultimately obtain P'' 
    where L1:  "\<forall>u. \<exists>P'. P \<Longrightarrow>\<^isub>lu in P''\<rightarrow>(rev perm \<bullet> a)<(rev perm \<bullet> x)> \<prec> P' \<and> 
  ...
  proof(rule allI)
    ...
    hence "(perm \<bullet> P) \<Longrightarrow>\<^isub>lu in (perm \<bullet> P'')\<rightarrow>a<x> \<prec> (perm \<bullet> P')" by(simp add: name_per_rev)
...
next
  ...
  hence "Q \<longmapsto> (rev perm \<bullet> \<alpha>) \<prec> (rev perm \<bullet> Q')" 
    by(simp add: name_rev_per)
  with Sim obtain P' where PTrans: "P \<Longrightarrow>\<^isub>l (rev perm \<bullet> \<alpha>) \<prec> P'" and PRel: "(P', (rev perm \<bullet> Q')) \<in> Rel" 
    by(blast dest: simE)
  from PTrans have "(perm \<bullet> P) \<Longrightarrow>\<^isub>l perm \<bullet> ((rev perm \<bullet> \<alpha>)\<prec> P')"
    by(rule Weak_Late_Step_Semantics.eqvtI)
  hence "(perm \<bullet> P) \<Longrightarrow>\<^isub>l \<alpha> \<prec> (perm \<bullet> P')" by(simp add: name_per_rev)
  moreover have "((perm \<bullet> P'), Q') \<in> Rel'"
  proof -
...
lemma simE2:
  ...
  shows "Q \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> Q' \<Longrightarrow> x \<sharp> P \<Longrightarrow> \<exists>P'. P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel"
  and   "Q \<Longrightarrow>\<^isub>l\<alpha> \<prec> Q' \<Longrightarrow> \<exists>P'. P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P' \<and> (P', Q') \<in> Rel"
proof -
  assume QTrans: "Q \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> Q'"
  assume xFreshP: "x \<sharp> P"
  have Goal: "\<And>P Q a x Q'. \<lbrakk>P \<leadsto><Rel> Q; Q \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> Q'; x \<sharp> P; x \<sharp> Q; (P, Q) \<in> Rel\<rbrakk> \<Longrightarrow>
                            \<exists>P'. P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel"
  proof -
    fix P Q a x Q'
    ...
    then obtain P''' where P''Trans: "P'' \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P'''" and P'''RelQ''': "(P''', Q''') \<in> Rel"
    ...
    from PChain P''Trans P'''Chain xFreshP'' have "P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P'"
      by(blast dest: Weak_Late_Step_Semantics.chainTransitionAppend)
    with P'RelQ' show "\<exists>P'. P \<Longrightarrow>\<^isub>l a<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel" by blast
  qed
  ...
  from QTrans cFreshQ' have "Q \<Longrightarrow>\<^isub>la<\<nu>c> \<prec> ([(x, c)] \<bullet> Q')" by(simp add: alphaBoundResidual)
  with PSimQ have "\<exists>P'. P \<Longrightarrow>\<^isub>la<\<nu>c> \<prec> P' \<and> (P', [(x, c)] \<bullet> Q') \<in> Rel" using cFreshP cFreshQ PRelQ
    by(rule Goal)
  then obtain P' where PTrans: "P \<Longrightarrow>\<^isub>la<\<nu>c> \<prec> P'" and P'RelQ': "(P', [(x, c)] \<bullet> Q') \<in> Rel"
    by force
  have "P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> ([(x, c)] \<bullet> P')"
  proof -
  ...
  ultimately show "\<exists>P'. P \<Longrightarrow>\<^isub>l a<\<nu>x> \<prec> P' \<and> (P', Q') \<in> Rel" by blast
next
  assume QTrans: "Q \<Longrightarrow>\<^isub>l\<alpha> \<prec> Q'"
  ...
  thus "\<exists>P'. P \<Longrightarrow>\<^isub>l \<alpha> \<prec> P' \<and> (P', Q') \<in> Rel"
  proof(induct arbitrary: \<alpha> Q''' Q' rule: tauChainInduct)
    case(id \<alpha> Q''')
    from PSimQ `Q \<longmapsto>\<alpha> \<prec> Q'''` have "\<exists>P'. P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P' \<and> (P', Q''') \<in> Rel"
      by(blast dest: simE)
    then obtain P''' where PTrans: "P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P'''" and P'RelQ''': "(P''', Q''') \<in> Rel"
      by blast
    ...
    from P'''Chain PTrans have "P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P'"
      by(blast dest: Weak_Late_Step_Semantics.chainTransitionAppend)
  ...
  next
    ...
    with `Q'''' \<longmapsto>\<tau> \<prec> Q'''` obtain P''' where PTrans: "P \<Longrightarrow>\<^isub>l\<tau> \<prec> P'''" and P'''RelQ''': "(P''', Q''') \<in> Rel"
      by(drule_tac ih) auto
    from P'''RelQ''' `Q''' \<longmapsto>\<alpha> \<prec> Q''` obtain P'' where 
      P'''Trans: "P''' \<Longrightarrow>\<^isub>l\<^isup>^\<alpha> \<prec> P''" and P''RelQ'': "(P'', Q'') \<in> Rel"
      by(blast dest: Weak_Late_Sim.simE Sim)
    from P''RelQ'' `Q'' \<Longrightarrow>\<^isub>\<tau> Q'` Sim obtain P' where 
      P''Chain: "P'' \<Longrightarrow>\<^isub>\<tau> P'" and P'RelQ': "(P', Q')\<in> Rel"
      by(drule_tac Weak_Late_Sim.weakSimTauChain) auto
    
    from PTrans P'''Trans P''Chain have "P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P'"
...
proof(induct rule: simCasesCont[of _ "(P, Q)"])
  ...
  from QSimR RTrans xFreshQ obtain Q' where QTrans: "Q \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> Q'"
                                        and Q'RelR': "(Q', R') \<in> Rel'"
    by(blast dest: simE)
  from PSimQ Sim Eqvt PRelQ QTrans xFreshP obtain P' where PTrans: "P \<Longrightarrow>\<^isub>la<\<nu>x> \<prec> P'"
...
next
  ...
  from QSimR RTrans xFreshQ obtain Q''
    where "\<forall>u. \<exists>Q'. Q \<Longrightarrow>\<^isub>lu in Q''\<rightarrow>a<x> \<prec> Q' \<and> (Q', R'[x::=u]) \<in> Rel'" 
    by(blast dest: simE)
  ...
  from PChain xFreshP have xFreshP''': "x \<sharp> P'''" by(rule freshChain)
  from P'''RelQ''' have "P''' \<leadsto>\<^isup>^<Rel> Q'''" by(rule Sim)
  with xFreshP''' Q'''Trans obtain P'''' where L2: "\<forall>u. \<exists>P''. P''' \<Longrightarrow>\<^isub>lu in P''''\<rightarrow>a<x> \<prec> P'' \<and> (P'', Q''[x::=u]) \<in> Rel" 
...
next
  ...
  with QSimR obtain Q' where QTrans: "Q \<Longrightarrow>\<^isub>l\<alpha> \<prec> Q'" and Q'RelR': "(Q', R') \<in> Rel'"
    by(blast dest: simE)
  from PSimQ Sim Eqvt PRelQ QTrans obtain P' where PTrans: "P \<Longrightarrow>\<^isub>l\<alpha> \<prec> P'"
                                               and P'RelQ': "(P', Q') \<in> Rel"
    by(blast dest: simE2)
