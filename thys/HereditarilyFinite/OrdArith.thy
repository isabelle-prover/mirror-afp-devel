...
lemma hadd_hmem_cancel [simp]: "x @+ y ⋿ x @+ z \<longleftrightarrow> y ⋿ z"
...
lemma hmem_self_hadd [simp]: "k1 ⋿ k1 @+ k2 \<longleftrightarrow> 0 ⋿ k2"
...
lemma pred_mem [simp]: "Ord x \<Longrightarrow> x \<noteq> 0 \<Longrightarrow> pred x ⋿ x"
...
definition shift :: "hf \<Rightarrow> hf \<Rightarrow> hf"
  where "shift f delta = \<lbrace>v . u ⋿ f, \<exists>n y. u = \<langle>n, y\<rangle> \<and> v = \<langle>delta @+ n, y\<rangle>\<rbrace>"

lemma shiftD: "x ⋿ shift f delta \<Longrightarrow> \<exists>u. u ⋿ f \<and> x = \<langle>delta @+ hfst u, hsnd u\<rangle>"
  by (auto simp: shift_def hsplit_def) 

lemma hmem_shift_iff: "\<langle>m, y\<rangle> ⋿ shift f delta \<longleftrightarrow> (\<exists>n. m = delta @+ n \<and> \<langle>n, y\<rangle> ⋿ f)"
  by (auto simp: shift_def hrelation_def is_hpair_def)  

lemma hmem_shift_add_iff [simp]: "\<langle>delta @+ n, y\<rangle> ⋿ shift f delta \<longleftrightarrow> \<langle>n, y\<rangle> ⋿ f"
...
lemma hdomain_shift_add: "hdomain (shift f delta) = \<lbrace>delta @+ n . n ⋿ hdomain f\<rbrace>"
...
lemma app_hunion1: "~ x ⋿ hdomain g \<Longrightarrow> app (f \<squnion> g) x = app f x"
  by (auto simp: app_def) (metis hdomainI)

lemma app_hunion2: "~ x ⋿ hdomain f \<Longrightarrow> app (f \<squnion> g) x = app g x"
  by (auto simp: app_def) (metis hdomainI)

lemma Seq_append_app1: "Seq s k \<Longrightarrow> l ⋿ k \<Longrightarrow> app (seq_append k s s') l = app s l"
  apply (auto simp: Seq_def seq_append_def) 
...
definition OrdDom where
 "OrdDom r \<equiv> \<forall>x y. \<langle>x,y\<rangle> ⋿ r \<longrightarrow> Ord x"
...
definition LstSeq :: "hf \<Rightarrow> hf \<Rightarrow> hf \<Rightarrow> bool"
  where "LstSeq s k y \<equiv> Seq s (succ k) & Ord k & \<langle>k,y\<rangle> ⋿ s & OrdDom s"
...
lemma LstSeq_trunc: "LstSeq s k y \<Longrightarrow> l ⋿ k \<Longrightarrow> LstSeq s l (app s l)" 
...
lemma LstSeq_append_app1:
  "LstSeq s k y \<Longrightarrow> l ⋿ succ k \<Longrightarrow> app (seq_append (succ k) s s') l = app s l"
...
lemma Seq_append_pair:
  "\<lbrakk>Seq s1 k1; Seq s2 (succ n);  \<langle>n, y\<rangle> ⋿ s2; Ord n\<rbrakk> \<Longrightarrow> \<langle>k1 @+ n, y\<rangle> ⋿ (seq_append k1 s1 s2)"
...
definition Builds :: "(hf \<Rightarrow> bool) \<Rightarrow> (hf \<Rightarrow> hf \<Rightarrow> hf \<Rightarrow> bool) \<Rightarrow> hf \<Rightarrow> hf \<Rightarrow> bool"
  where "Builds B C s l \<equiv> B (app s l) \<or> (\<exists>m ⋿ l. \<exists>n ⋿ l. C (app s l) (app s m) (app s n))"
...
definition BuildSeq :: "(hf \<Rightarrow> bool) \<Rightarrow> (hf \<Rightarrow> hf \<Rightarrow> hf \<Rightarrow> bool) \<Rightarrow> hf \<Rightarrow> hf \<Rightarrow> hf \<Rightarrow> bool"
  where "BuildSeq B C s k y \<equiv> LstSeq s k y \<and> (\<forall>l ⋿ succ k. Builds B C s l)"

lemma BuildSeqI: "LstSeq s k y \<Longrightarrow> (\<And>l. l ⋿ succ k \<Longrightarrow> Builds B C s l) \<Longrightarrow> BuildSeq B C s k y"
...
lemma BuildSeq_conj_distrib:
 "BuildSeq (\<lambda>x. B x \<and> P x) (\<lambda>x y z. C x y z \<and> P x) s k y \<longleftrightarrow> 
  BuildSeq B C s k y \<and> (\<forall>l ⋿ succ k. P (app s l))"
...
lemma Builds_insf: "Builds B C s l \<Longrightarrow> LstSeq s k z \<Longrightarrow> l ⋿ succ k \<Longrightarrow> Builds B C (insf s (succ k) y) l"
...
next
  ...
  have s1L: "LstSeq s1 k1 y1"
   and s1BC: "\<And>l. l ⋿ succ k1 \<Longrightarrow> Builds B C s1 l"
   and s2L: "LstSeq s2 k2 y2"
   and s2BC: "\<And>l. l ⋿ succ k2 \<Longrightarrow> Builds B C s2 l"
  ...
  proof (rule hmem_hadd_E) 
    assume l1: "l ⋿ succ k1"
    hence "B (app s1 l) \<or> (\<exists>m⋿l. \<exists>n⋿l. C (app s1 l) (app s1 m) (app s1 n))" using s1BC
    ...
    next
      assume "\<exists>m⋿l. \<exists>n⋿l. C (app s1 l) (app s1 m) (app s1 n)"
      then obtain m n where mn: "m ⋿ l" "n ⋿ l" and C: "C (app s1 l) (app s1 m) (app s1 n)"
        by blast
      also have "m ⋿ succ k1" "n ⋿ succ k1"
  ...
  next
    ...
    hence "B (app s2 z) \<or> (\<exists>m⋿z. \<exists>n⋿z. C (app s2 z) (app s2 m) (app s2 n))" using s2BC
      by (simp add: Builds_def) 
    thus ?thesis
    proof
      assume "B (app s2 z)" 
    ...
    next
      assume "\<exists>m⋿z. \<exists>n⋿z. C (app s2 z) (app s2 m) (app s2 n)"
...
proof -
 ...
 hence "B y \<or> (\<exists>m ⋿ k. \<exists>n ⋿ k. C y (app s m) (app s n))"
...
lemma diff_hmem_imp_less: assumes "x ⋿ y" shows "Rep_hf (y - \<lbrace>x\<rbrace>) < Rep_hf y"
proof -
...
definition least :: "hf \<Rightarrow> hf"
  where "least a \<equiv> (THE x. x ⋿ a \<and> (\<forall>y. y ⋿ a \<longrightarrow> Rep_hf x \<le> Rep_hf y))"

lemma least_equality:
  assumes "x ⋿ a" and "\<And>y. y ⋿ a \<Longrightarrow> Rep_hf x \<le> Rep_hf y"
  shows "least a = x"
unfolding least_def
...
lemma leastI2_order:
  assumes "x ⋿ a"
    and "\<And>y. y ⋿ a \<Longrightarrow> Rep_hf x \<le> Rep_hf y"
    and "\<And>z. z ⋿ a \<Longrightarrow> \<forall>y. y ⋿ a \<longrightarrow> Rep_hf z \<le> Rep_hf y \<Longrightarrow> Q z"
  shows "Q (least a)"
by (metis assms least_equality)

lemma nonempty_imp_ex_least: "a \<noteq> 0 \<Longrightarrow> \<exists>x. x ⋿ a \<and> (\<forall>y. y ⋿ a \<longrightarrow> Rep_hf x \<le> Rep_hf y)"
...
lemma least_hmem: "a \<noteq> 0 \<Longrightarrow> least a ⋿ a"
