...
text{*Theorem 4.3*}
lemma Vset_universal: "\<exists>n. Ord n & x ⋿ Vset n"
...
next
  ...
  finally have "b \<triangleleft> a ⋿ Vset (succ (na \<squnion> nb))" using nab
...
lemma VsetI: "succ(rank a) \<le> k \<Longrightarrow> Ord k \<Longrightarrow> a ⋿ Vset k"
  by (metis Vset_mono hsubsetCE in_Vset_rank)

lemma Vset_succ_rank_le: "Ord k \<Longrightarrow> a ⋿ Vset (succ k) \<Longrightarrow> rank a \<le> k"
...
text{*Theorem 4.5*}
theorem rank_lt: "a ⋿ b \<Longrightarrow> rank(a) < rank(b)"
...
definition
  eclose    :: "hf \<Rightarrow> hf"  where
    "eclose X = \<Sqinter> \<lbrace>Y ⋿ HPow(Vset (rank X)). Transset Y & X\<le>Y\<rbrace>"

lemma eclose_facts: 
  shows Transset_eclose: "Transset (eclose X)"
   and  le_eclose: "X \<le> eclose X"
proof -
  have nz: "\<lbrace>Y ⋿ HPow(Vset (rank X)). Transset Y & X\<le>Y\<rbrace> \<noteq> 0"
...
proof -
  have "\<lbrace>Y ⋿ HPow(Vset (rank X)). Transset Y & X\<le>Y\<rbrace> \<noteq> 0"
    by (simp add: eclose_def hempty_iff) (metis Ord_rank Vset_Transset le_Vset_rank order_refl) 
...
lemma fst_in_eclose [simp]: "x ⋿ eclose \<langle>x, y\<rangle>"
  by (metis eclose_hinsert hmem_hinsert hpair_def hunion_iff)

lemma snd_in_eclose [simp]: "y ⋿ eclose \<langle>x, y\<rangle>"
  by (metis eclose_hinsert hmem_hinsert hpair_def hunion_iff)
...
lemma hmem_induct [case_names step]:
  assumes ih: "\<And>x. (\<And>y. y ⋿ x \<Longrightarrow> P y) \<Longrightarrow> P x" shows "P x"
proof -
  have "\<And>y. y ⋿ x \<Longrightarrow> P y"
  proof (induct x rule: hf_induct)
    case 0 thus ?case by simp
  next
    case (hinsert a b) thus ?case
...
lemma hmem_eclose_le: "y ⋿ x \<Longrightarrow> eclose y \<le> eclose x"
...
proof (unfold hmem_rel_def, rule iffI)
  ...
  thus "x ⋿ eclose y"
    ...
    qed
next
  have "Transset \<lbrace>x ⋿ eclose y. (x, y) \<in> hmem_rel\<rbrace>" using Transset_eclose
    by (auto simp: Transset_def hmem_rel_def intro: trancl_trans)
  hence "eclose y \<le> \<lbrace>x ⋿ eclose y. (x, y) \<in> hmem_rel\<rbrace>" 
    by (rule eclose_minimal) (auto simp: le_HCollect_iff le_eclose hmem_rel_def) 
  moreover assume "x ⋿ eclose y"
  ultimately show "(x, y) \<in> trancl {(x, y). x ⋿ y}"
...
lemma ecut_apply: "y ⋿ eclose x \<Longrightarrow> ecut f x y = f y"
  by (metis ecut_def)
