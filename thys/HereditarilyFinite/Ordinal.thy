...
definition hconverse :: "hf \<Rightarrow> hf"
  where "hconverse(r) = \<lbrace>z. w ⋿ r, \<exists>x y. w = \<langle>x,y\<rangle> & z = \<langle>y,x\<rangle>\<rbrace>"

definition hdomain :: "hf \<Rightarrow> hf"
  where "hdomain(r) = \<lbrace>x. w ⋿ r, \<exists>y. w = \<langle>x,y\<rangle>\<rbrace>"

definition hrange :: "hf \<Rightarrow> hf"
  where "hrange(r) = hdomain(hconverse(r))"

definition hrelation :: "hf \<Rightarrow> bool"
  where "hrelation(r) = (\<forall>z. z ⋿ r \<longrightarrow> is_hpair z)"

definition hrestrict :: "hf \<Rightarrow> hf \<Rightarrow> hf"
  --{* Restrict the relation r to the domain A *}
  where "hrestrict r A = \<lbrace>z ⋿ r. \<exists>x ⋿ A. \<exists>y. z = \<langle>x,y\<rangle>\<rbrace>"

definition nonrestrict :: "hf \<Rightarrow> hf \<Rightarrow> hf"
  where "nonrestrict r A = \<lbrace>z ⋿ r. \<forall>x ⋿ A. \<forall>y. z \<noteq> \<langle>x,y\<rangle>\<rbrace>"

definition hfunction :: "hf \<Rightarrow> bool"
  where "hfunction(r) = (\<forall>x y. \<langle>x,y\<rangle> ⋿ r \<longrightarrow> (\<forall>y'. \<langle>x,y'\<rangle> ⋿ r \<longrightarrow> y=y'))"

definition app :: "hf \<Rightarrow> hf \<Rightarrow> hf"
  where "app f x = (THE y. \<langle>x, y\<rangle> ⋿ f)"

lemma hrestrict_iff [iff]:
    "z ⋿ hrestrict r A \<longleftrightarrow> z ⋿ r & (\<exists> x y. z = \<langle>x, y\<rangle> & x ⋿ A)"
...
lemma hdomain_not_mem [iff]: "\<not> \<langle>hdomain r, a\<rangle> ⋿ r" 
...
lemma hdomainI: "\<langle>x, y\<rangle> ⋿ f \<Longrightarrow> x ⋿ hdomain f"
...
lemma app_hrestrict [simp]: "x ⋿ A \<Longrightarrow> app (hrestrict f A) x = app f x"
...
definition HSigma :: "hf \<Rightarrow> (hf \<Rightarrow> hf) \<Rightarrow> hf"
  where "HSigma A B = (\<Squnion>x⋿A. \<Squnion>y⋿B(x). \<lbrace>\<langle>x,y\<rangle>\<rbrace>)"

definition HPi :: "hf \<Rightarrow> (hf \<Rightarrow> hf) \<Rightarrow> hf"
  where "HPi A B = \<lbrace> f ⋿ HPow(HSigma A B). A \<le> hdomain(f) & hfunction(f)\<rbrace>"
...
syntax (xsymbols)
  "_PROD"     :: "[pttrn, hf, hf] \<Rightarrow> hf"        ("(3\<Pi>_⋿_./ _)" 10)
  "_SUM"      :: "[pttrn, hf, hf] \<Rightarrow> hf"        ("(3\<Sigma>_⋿_./ _)" 10)
  "_lam"      :: "[pttrn, hf, hf] \<Rightarrow> hf"        ("(3\<lambda>_⋿_./ _)" 10)
...
lemma HUN_iff [simp]: "b ⋿ (\<Squnion>x⋿A. B(x)) \<longleftrightarrow> (\<exists>x⋿A. b ⋿ B(x))"
  by auto

(*The order of the premises presupposes that A is rigid; b may be flexible*)
lemma HUN_I: "\<lbrakk> a ⋿ A;  b ⋿ B(a) \<rbrakk>  \<Longrightarrow> b ⋿ (\<Squnion>x⋿A. B(x))"
  by auto

lemma HUN_E [elim!]: assumes "b ⋿ (\<Squnion>x⋿A. B(x))" obtains x where "x ⋿ A"  "b ⋿ B(x)"
  using assms  by blast

lemma HINT_iff: "b ⋿ (\<Sqinter>x⋿A. B(x)) \<longleftrightarrow> (\<forall>x⋿A. b ⋿ B(x)) & A\<noteq>0"
  by (simp add: HInter_def HBall_def) (metis foundation hmem_hempty) 

lemma HINT_I: "\<lbrakk> !!x. x ⋿ A \<Longrightarrow> b ⋿ B(x);  A\<noteq>0 \<rbrakk> \<Longrightarrow> b ⋿ (\<Sqinter>x⋿A. B(x))"
  by (simp add: HINT_iff) 

lemma HINT_E: "\<lbrakk> b ⋿ (\<Sqinter>x⋿A. B(x));  a ⋿ A \<rbrakk> \<Longrightarrow> b ⋿ B(a)"
  by (auto simp: HINT_iff) 


subsection{*Generalized Cartesian product*}

lemma HSigma_iff [simp]: "\<langle>a,b\<rangle> ⋿ HSigma A B \<longleftrightarrow> a ⋿ A & b ⋿ B(a)"
  by (force simp add: HSigma_def)

lemma HSigmaI [intro!]: "\<lbrakk> a ⋿ A;  b ⋿ B(a) \<rbrakk>  \<Longrightarrow> \<langle>a,b\<rangle> ⋿ HSigma A B"
  by simp
...
lemma HSigmaE [elim!]: 
  ...
  obtains x y where "x ⋿ A" "y ⋿ B(x)" "c=\<langle>x,y\<rangle>"
  using assms  by (force simp add: HSigma_def) 

lemma HSigmaE2 [elim!]:
  assumes "\<langle>a,b\<rangle> ⋿ HSigma A B" obtains "a ⋿ A" and "b ⋿ B(a)"
  using assms  by auto 
...
lemma times_iff [simp]: "\<langle>a,b\<rangle> ⋿ A * B \<longleftrightarrow> a ⋿ A & b ⋿ B"
  by (simp add: times_hf_def) 

lemma timesI [intro!]: "\<lbrakk> a ⋿ A;  b ⋿ B \<rbrakk>  \<Longrightarrow> \<langle>a,b\<rangle> ⋿ A * B"
  by simp
...
lemma timesE [elim!]:
  assumes c: "c ⋿ A * B"
  obtains x y where "x ⋿ A" "y ⋿ B" "c=\<langle>x,y\<rangle>" using c
  by (auto simp: times_hf_def) 

text{*...and a specific one*}
lemma timesE2 [elim!]:
  assumes "\<langle>a,b\<rangle> ⋿ A * B" obtains "a ⋿ A" and "b ⋿ B"
using assms
  by auto 
...
lemma Inl_in_sum_iff [iff]: "Inl(a) ⋿ A+B \<longleftrightarrow> a ⋿ A"
  by (auto simp: sum_defs) 

lemma Inr_in_sum_iff [iff]: "Inr(b) ⋿ A+B \<longleftrightarrow> b ⋿ B"
  by (auto simp: sum_defs) 
...
definition
  Transset  :: "hf \<Rightarrow> bool"  where
    "Transset(x) \<equiv> \<forall>y. y ⋿ x \<longrightarrow> y \<le> x"
...
definition
  Ord  :: "hf \<Rightarrow> bool"  where
    "Ord(k)      \<equiv> Transset(k) & (\<forall>x ⋿ k. Transset(x))"
...
lemma succ_iff [simp]: "x ⋿ succ y \<longleftrightarrow> x=y \<or> x ⋿ y"
...
lemma hball_succ [simp]: "(\<forall>x ⋿ succ k. P x) \<longleftrightarrow> P k & (\<forall>x ⋿ k. P x)"
  by (auto simp: HBall_def) 

lemma hbex_succ [simp]: "(\<exists>x ⋿ succ k. P x) \<longleftrightarrow> P k | (\<exists>x ⋿ k. P x)"
  by (auto simp: HBex_def) 

lemma One_hf_eq_succ: "1 = succ 0"
  by (metis One_hf_def succ_def)

lemma zero_hmem_one [iff]: "x ⋿ 1 \<longleftrightarrow> x = 0"
  by (metis One_hf_eq_succ hmem_hempty succ_iff)
  
lemma hball_One [simp]: "(\<forall>x⋿1. P x) = P 0"
  by (simp add: One_hf_eq_succ) 

lemma hbex_One [simp]: "(\<exists>x⋿1. P x) = P 0"
  by (simp add: One_hf_eq_succ) 
...
lemma hmem_succ: "l ⋿ k \<Longrightarrow> l ⋿ succ k"
  by (metis succ_iff)
...
lemma OrdmemD: "Ord(k) \<Longrightarrow> j ⋿ k \<Longrightarrow> j \<le> k"
  by (simp add: Ord_def Transset_def HBall_def) 

lemma Ord_trans: "\<lbrakk> i⋿j;  j⋿k;  Ord(k) \<rbrakk>  \<Longrightarrow> i⋿k"
  by (blast dest: OrdmemD)

lemma hmem_0_Ord:
  assumes k: "Ord(k)" and knz: "k \<noteq> 0" shows "0 ⋿ k" 
  by (metis foundation [OF knz] Ord_trans hempty_iff hinter_iff k)

lemma Ord_in_Ord: "\<lbrakk> Ord(k);  m ⋿ k \<rbrakk>  \<Longrightarrow> Ord(m)"
...
lemma Ord_induct [consumes 1, case_names step]:
  assumes k: "Ord(k)"
      and step: "!!x.\<lbrakk> Ord(x);  \<And>y. y ⋿ x \<Longrightarrow> P(y) \<rbrakk>  \<Longrightarrow> P(x)"
  shows "P(k)"
proof -
  have "\<forall>m ⋿ k. Ord(m) \<longrightarrow> P(m)" 
...
lemma Ord_linear: "Ord(k) \<Longrightarrow> Ord(l) \<Longrightarrow> k⋿l | k=l | l⋿k"
...
text{*Theorem 2.5*}
lemma Ord_mem_iff_lt: "Ord(k) \<Longrightarrow> Ord(l) \<Longrightarrow> k⋿l \<longleftrightarrow> k < l"
...
lemma mem_succ_iff [simp]: "Ord j \<Longrightarrow> succ i ⋿ succ j \<longleftrightarrow> i ⋿ j"
  by (metis Ord_in_Ord Ord_mem_iff_lt Ord_succ succ_def less_eq_insert1_iff less_hf_def succ_iff)

lemma Ord_mem_succ_cases:
  assumes "Ord(k)" "l ⋿ k"
  shows "succ l = k \<or> succ l ⋿ k"
  by (metis assms mem_succ_iff succ_iff)

subsection{*Supremum and Infimum*}

lemma Ord_Union [intro,simp]: "\<lbrakk> !!i. i⋿A \<Longrightarrow> Ord(i) \<rbrakk>  \<Longrightarrow> Ord(\<Squnion> A)"
  by (auto simp: Ord_def Transset_def) blast

lemma Ord_Inter [intro,simp]: "\<lbrakk> !!i. i⋿A \<Longrightarrow> Ord(i) \<rbrakk>  \<Longrightarrow> Ord(\<Sqinter> A)"
...
lemma hmem_Sup_Ords: "\<lbrakk>A\<noteq>0; !!i. i⋿A \<Longrightarrow> Ord(i)\<rbrakk> \<Longrightarrow> \<Squnion>A ⋿ A"
...
lemma hmem_Inf_Ords: "\<lbrakk>A\<noteq>0; !!i. i⋿A \<Longrightarrow> Ord(i)\<rbrakk> \<Longrightarrow> \<Sqinter>A ⋿ A"
proof (induction A rule: hf_induct)
  case 0 thus ?case  by simp
next
  case (hinsert x A)
...
lemma zero_in_Ord: "Ord k \<Longrightarrow> k=0 \<or> 0 ⋿ k"
  by (induct k) auto
...
lemma zero_in_succ [simp,intro]: "Ord i \<Longrightarrow> 0 ⋿ succ i"
...
lemma Seq_Ord_D: "Seq s k \<Longrightarrow> l ⋿ k \<Longrightarrow> Ord k \<Longrightarrow> Seq s l"
...
lemma Seq_Ord_restr: "\<lbrakk>Seq s k; l ⋿ k; Ord k\<rbrakk> \<Longrightarrow> Seq (hrestrict s l) l"
...
lemma nonrestrictD: "a ⋿ nonrestrict s X \<Longrightarrow> a ⋿ s"
  by (auto simp: nonrestrict_def)

lemma hpair_in_nonrestrict_iff [simp]: "\<langle>a,b\<rangle> ⋿ nonrestrict s X \<longleftrightarrow> \<langle>a,b\<rangle> ⋿ s \<and> \<not> a ⋿ X"
  by (auto simp: nonrestrict_def)
...
lemma Seq_imp_eq_app: "\<lbrakk>Seq s d; \<langle>x,y\<rangle> ⋿ s\<rbrakk> \<Longrightarrow> app s x = y"
  by (metis Seq_def app_equality)

lemma Seq_iff_app: "\<lbrakk>Seq s d; x ⋿ d\<rbrakk> \<Longrightarrow> \<langle>x,y\<rangle> ⋿ s \<longleftrightarrow> app s x = y"
  by (auto simp: Seq_def hdomain_def app_equality)  

lemma Exists_iff_app: "Seq s d \<Longrightarrow> x ⋿ d \<Longrightarrow> (\<exists>y. \<langle>x, y\<rangle> ⋿ s & P y) = P (app s x)"
  by (metis Seq_iff_app)

lemma Ord_trans2: "\<lbrakk>i2 ⋿ i; i ⋿ j;  j ⋿ k;  Ord k\<rbrakk> \<Longrightarrow> i2⋿k"
  by (metis Ord_trans)
  ...
  where
   "ord_rec_Seq T G s k y \<longleftrightarrow>
        (Seq s k & y = G (app s (\<Squnion>k)) & app s 0 = T &
                   (\<forall>n. succ n ⋿ k \<longrightarrow> app s (succ n) = G (app s n)))"
