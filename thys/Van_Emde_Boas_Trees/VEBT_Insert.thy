...
proof -
  obtain deg' :: nat where "deg = Suc (Suc deg')"
    using \<open>2 \<le> deg\<close>
    by (metis add_2_eq_Suc less_eqE)
  then show ?thesis
...
next
  ...
  then obtain n' where "n = Suc n'"
    by (metis One_nat_def Suc_le_D set_n_deg_not_0)
  then have "deg = Suc (Suc (n' + n'))"
    using 2 by linarith
  then show ?case
    by (auto simp: both_member_options_def)
next
...
next
  ...
  have "length treeList =2^n" using 4 by blast
  ...
  proof(cases "x = mi \<or> x = ma")
    case True
    then show ?thesis
      by (auto simp add: \<open>deg = Suc (Suc _)\<close> both_member_options_def)
  next
    case False
    ...
    proof(cases "x < mi")
      ...
      then show ?thesis
        by (smt (verit, ccfv_threshold) "4.hyps"(1,3,4) VEBT_internal.membermima.simps(4) add_2_eq_Suc
            add_self_div_2 both_member_options_def deg_not_0 div_greater_zero_iff le_add_diff_inverse)
    next
      case False
      hence "vebt_insert ( Node (Some (mi, ma)) deg treeList summary) x = 
...
next
  ...
  have "length treeList =2^m" using 5 by blast
  hence "high x n < length treeList"
  ...
  next
    ...
    next
      ...
      Node (Some (mi, max x ma)) deg (treeList[(high x n):= vebt_insert (treeList ! (high x n)) (low x n)])
                               (if minNull (treeList ! high x n) then  vebt_insert summary (high x n) else summary)" 
        by (smt (verit) "5.IH"(1) "5.hyps"(3) "5.hyps"(4) \<open>\<not> (x = mi \<or> x = ma)\<close> \<open>high x n < length treeList\<close> add_Suc_right add_self_div_2 deg_not_0 div_greater_zero_iff even_Suc_div_two insert_simp_norm linorder_neqE_nat nth_mem odd_add)
...
next
  ...
  hence "x = mi \<or> x = ma \<or> both_member_options (treeList ! (high x n)) (low x n)"
    by (metis "00" "4.prems"(3) add_self_div_2 both_member_options_from_complete_tree_to_child
        trans_le_add1)
      ...
      next 
        ...
        next
          ...
          have 14:"vebt_insert (Node (Some (mi,ma)) deg  treeList summary) y =
            ...
            using "0" "00" "4.hyps"(7,8) False high_bound_aux by auto
...
next
  ...
  have "low x n <2^n \<and> low y n< 2^n" 
    by (simp add: low_def)
  hence "x = mi \<or> x = ma \<or> both_member_options (treeList ! (high x n)) (low x n)"
    by (metis "00" "5.prems"(3) add_leD1 both_member_options_from_complete_tree_to_child one_add_one)
...
next
  case (2 treeList n summary m deg)
  then obtain n' :: nat where "n = Suc n'"
    by (metis deg_not_0 less_iff_Suc_add)
  then have "deg = Suc (Suc (n' + n'))"
    using 2 by linarith
  then have "x = y"
    by (metis "2.prems"(3) member_inv vebt_insert.simps(4) vebt_member.simps(5))
  then show ?case ..
next
  case (3 treeList n summary m deg)
  then obtain n' :: nat where "n = Suc n'"
    by (metis One_nat_def lessI not_less set_n_deg_not_0 vebt_buildup.cases)
  then have "deg = Suc (Suc (Suc (n' + n')))"
    using 3 by linarith
  then have "x = y"
    by (metis "3.prems"(3) member_inv vebt_insert.simps(4) vebt_member.simps(5))
  then show ?case ..
next
  case (4 treeList n summary m deg mi ma)
  ...
  next
    ...
    proof(cases "mi < x")
      ...
      proof(cases "y = mi \<or> y = max x ma")
        ...
        proof(cases "y = mi")
          case True
          then show ?thesis
            using "00" le_Suc_ex by fastforce
        next
          case False
          hence "y = max x ma" 
            using True by blast
          ...
          proof(cases "x < ma")
            case True
            then show ?thesis
              using "00" \<open>y = max x ma\<close> le_Suc_ex by fastforce
          next
            case False
            then show ?thesis
      ...
      next
        ...
        proof(cases "high x n  = high y n")
          ...
          then show ?thesis 
            by (smt (verit) "00" "001" "002" "003" "4"(11) "4"(8) vebt_member.simps(5) True add_numeral_left add_self_div_2 bit_split_inv le_add_diff_inverse mimaxyprop not_less not_less_iff_gr_or_eq numerals(1) plus_1_eq_Suc semiring_norm(2))
        next
          case False
    ...
    next
      ...
      consider "y = x" | "y = max mi ma" | "y \<noteq> x" "y \<noteq> max mi ma"
        by satx
      then show ?thesis
      proof cases
        case 1
      ...
      next
        ...
        proof(cases "high mi n  = high y n")
          ...
          hence 005:"both_member_options (Node (Some (mi , ma)) deg treeList summary) y \<or> x = y" 
            by (metis "00" "001" "002" Suc_le_D True add_self_div_2 bit_split_inv both_member_options_def member_valid_both_member_options membermima.simps(4) mimaxyprop one_add_one plus_1_eq_Suc)
          then show ?thesis
            by (smt (verit) "00" "001" "002" "003" "4.hyps"(6) "4.hyps"(9) vebt_member.simps(5) True add_numeral_left add_self_div_2 bit_split_inv le_add_diff_inverse mimaxyprop not_less not_less_iff_gr_or_eq numerals(1) plus_1_eq_Suc semiring_norm(2))
        next
          case False
          hence 000:"vebt_member (treeList ! (high y n)) (low y n)" 
...
next
  ...
  next
    ...
    hence mimaxyprop: "\<not> (x = mi \<or> x = ma) \<and> high x n < 2^m \<and> high mi n < 2^m \<and> low x n <2^n \<and> low mi n < 2^n \<and> length treeList = 2^m" 
      ...
      by (smt (verit, best) dual_order.strict_trans1 less_numeral_extra(1))
    then show ?thesis 
    proof(cases "mi < x")
      case True
      hence "vebt_insert (Node (Some (mi,ma)) deg  treeList summary) x =
        ...
        by (smt (verit, del_insts) "00" Suc_mult_cancel1 add_Suc_right diff_Suc_1 even_Suc even_add mimaxyprop
            mult_2 odd_two_times_div_two_nat one_add_one plus_1_eq_Suc)
      then show ?thesis 
      proof(cases "y = mi \<or> y = max x ma")
        case True
        then show ?thesis 
      ...
      next
        ...
        proof(cases "high x n  = high y n")
          ...
          hence 005:"both_member_options (Node (Some (mi , ma)) deg treeList summary) y \<or> x = y"
            by (metis "00" "003" True add_Suc_right add_leD1 add_self_div_2 bit_split_inv
                both_member_options_from_chilf_to_complete_tree even_Suc_div_two even_add one_add_one
                xyprop)
          then show ?thesis 
            using both_member_options_equiv_member[of "(Node (Some (mi, ma)) deg treeList summary)" deg y]
    ...
    next
      ...
      hence "vebt_insert (Node (Some (mi,ma)) deg  treeList summary) x =
        ...
        by (smt (verit) add_Suc_right even_Suc_div_two odd_add)
      ...
      next
        case 2
        then show ?thesis
          using "00" \<open>y = max mi ma\<close> le_Suc_ex by fastforce
      next
        case 3
        hence vebtmem: "vebt_member ((treeList[(high mi n):=vebt_insert (treeList ! (high mi n)) (low mi n)]) ! (high y n)) (low y n)" 
        ...
        have invar: "invar_vebt (Node (Some (mi, ma)) deg treeList summary) deg"
          using "00" "5.IH"(1) "5.hyps"(1,5,6,7,8,9) invar_vebt.intros(5) by force

        moreover have "both_member_options (Node (Some (mi , ma)) deg treeList summary) y"
        proof(cases "high mi n  = high y n")
          case True
          hence 000:"vebt_member (vebt_insert (treeList ! (high mi n)) (low mi n)) (low y n)"
            by (metis vebtmem mimaxyprop nth_list_update_eq)
          have 001:"invar_vebt (treeList ! (high mi n)) n \<and> treeList ! (high mi n) \<in> set treeList "
            by (simp add: "5.IH"(1) mimaxyprop)
          moreover have "vebt_member (treeList ! (high mi n)) (low y n) \<or> low y n = low mi n"
            using "000" "5.IH"(1) \<open>low x n < 2 ^ n \<and> low y n < 2 ^ n\<close> mimaxyprop by fastforce
          moreover have "naive_member (treeList ! (high mi n)) (low y n) \<Longrightarrow>
              naive_member (Node (Some (mi , ma)) deg treeList summary) y" using naive_member.simps(3)[of "Some (mi, ma)" "deg-1" treeList summary y] 
            using "00" True mimaxyprop by fastforce
          ultimately show "both_member_options (Node (Some (mi , ma)) deg treeList summary) y" 
            by (smt (verit) "00" True add_Suc_right add_self_div_2 bit_split_inv both_member_options_def even_Suc_div_two member_valid_both_member_options membermima.simps(4) odd_add xyprop)
        next
          case False
          hence "vebt_member (treeList ! (high y n)) (low y n)" 
            using vebtmem by auto
          moreover have "invar_vebt (treeList ! (high y n)) n \<and> treeList ! (high y n) \<in> set treeList "
            using "5.IH"(1) "5.hyps"(2) "5.hyps"(3) xyprop by simp
          ultimately show "both_member_options (Node (Some (mi , ma)) deg treeList summary) y"
            using "00" both_member_options_def member_valid_both_member_options xyprop by fastforce
        qed

        ultimately show ?thesis
          using invar by (simp add: both_member_options_equiv_member)
      qed
    qed
  qed
qed
