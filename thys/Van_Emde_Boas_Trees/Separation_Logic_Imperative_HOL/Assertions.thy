...
lemma mod_starE: 
  ...
  obtains h\<^sub>1 h\<^sub>2 where "h\<^sub>1 \<Turnstile> a" "h\<^sub>2 \<Turnstile> b"
...
fun sngr_assn_raw :: "'a::heap ref \<Rightarrow> 'a \<Rightarrow> assn_raw" where
  "sngr_assn_raw r x (h,as) \<longleftrightarrow> Ref_Time.get h r = x \<and> as = {addr_of_ref r} \<and> 
  addr_of_ref r < lim h"
...
fun snga_assn_raw :: "'a::heap array \<Rightarrow> 'a list \<Rightarrow> assn_raw" 
  ...
  \<longleftrightarrow> Array_Time.get h r = x \<and> as = {addr_of_array r} 
