...
lemma unbounded_dbm_semantics:
  "[curry (unbounded_dbm n)]\<^bsub>v,n\<^esub> = UNIV"
...
lemma And_unbounded_dbm:
  "[And (curry (unbounded_dbm n)) M]\<^bsub>v,n\<^esub> = [M]\<^bsub>v,n\<^esub>"
  by (subst And_correct[symmetric]) (simp add: unbounded_dbm_semantics)

definition
  "start_inv_check \<equiv> dbm_subset n init_dbm (FW' (abstr_upd (inv_of A l\<^sub>0) (unbounded_dbm n)) n)"
...
proof -
  let ?M = "FW' (abstr_upd (inv_of A l\<^sub>0) (unbounded_dbm n)) n"
  ...
  moreover have "[curry (conv_M ?M)]\<^bsub>v,n\<^esub> = {u. u \<turnstile> inv_of (conv_A A) l\<^sub>0}"
    ...
    apply (subst RI_zone_equiv[symmetric,
           where M = "abstr_upd (inv_of (conv_A A) l\<^sub>0) (unbounded_dbm n)"])
     defer
