...
instance proof (rule wqo_class.intro)
  ...
  from wqo_on_lists [OF this]
    have "wqo_on (list_emb ?P) (lists UNIV)" .
  hence "wqo_on ?P' UNIV"
    using `wqo_on ?P UNIV`
  ...
  from reflp_on_list_emb [of ?P UNIV]
    have "reflp_on (list_emb ?P) (lists UNIV)" by (simp add: reflp_on_def)
  then have refl: "reflp_on ?P' UNIV"
    unfolding reflp_on_def less_eq_list_def by auto

  from transp_on_list_emb [OF wqo_on_imp_transp_on [OF wqo]]
    have "transp_on (list_emb ?P) (lists UNIV)" .
