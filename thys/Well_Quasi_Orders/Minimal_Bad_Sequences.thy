...
\emph{values} from @{term "vals A"}. Where @{term less} is the order that
is used for checking minimality. The required properties are:
\begin{itemize}
\item @{term "less"} has to be well-founded on @{term "vals A"}
\item @{term "less"} has to be transitive
\item @{term "less"} reflects the property of being in @{term "vals A"}
\end{itemize}*}
locale mbs =
  fixes less :: "'b \<Rightarrow> 'b \<Rightarrow> bool"
    and vals :: "'a set \<Rightarrow> 'b set"
  assumes wfp_on_less: "wfp_on less (vals A)"
    and less_trans: "less x y \<Longrightarrow> less y z \<Longrightarrow> less x z"
    and less_vals: "less x y \<Longrightarrow> y \<in> vals A \<Longrightarrow> x \<in> vals A"
begin
...
lemma lesseq_vals: "lesseq x y \<Longrightarrow> y \<in> vals A \<Longrightarrow> x \<in> vals A"
  using less_vals by auto
...
lemma bad_repl:
  ...
  shows "bad P (repl n f g)" (is "bad ?P ?f")
proof (rule ccontr)
  presume "good ?P ?f"
...
definition min_at :: "('b \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow> 'a set \<Rightarrow> 'b seq \<Rightarrow> nat \<Rightarrow> bool" where
  "min_at P A f n \<equiv>
    \<forall>g. (\<forall>i. g i \<in> vals A) \<and> (\<forall>i<n. g i = f i) \<and> less (g n) (f n) \<longrightarrow> good P g"

lemma less_induct [consumes 1, case_names IH]:
  assumes "x \<in> vals A"
    and "\<And>x. x \<in> vals A \<Longrightarrow> (\<And>y. y \<in> vals A \<Longrightarrow> less y x \<Longrightarrow> P y) \<Longrightarrow> P x"
  shows "P x"
  using wfp_on_induct [OF wfp_on_less, of x A P] and assms by blast
...
lemma
  assumes "\<forall>i. f i \<in> vals A"
    and "min_at P A f n"
    and "bad P f" (is "bad ?P f")
  shows "\<exists>g.
    (\<forall>i. g i \<in> vals A) \<and>
    (\<forall>i\<le>n. g i = f i) \<and>
    lesseq (g (Suc n)) (f (Suc n)) \<and>
    bad P (repl (Suc n) f g) \<and>
    min_at P A (repl (Suc n) f g) (Suc n)"
  (is "\<exists>g. ?C g f (f (Suc n))")
proof -
  let ?I = "\<lambda>x. \<forall>f.
    x = f (Suc n) \<and> (\<forall>i. f i \<in> vals A) \<and> min_at P A f n \<and> bad ?P f \<longrightarrow> (\<exists>g. ?C g f x)"
  {
    fix x
    assume "x = f (Suc n)"
    ...
    proof (induct x rule: less_induct)
      fix x
      assume "x \<in> vals A" and IH: "\<And>y. \<lbrakk>y \<in> vals A; less y x\<rbrakk> \<Longrightarrow> ?I y"
      show "?I x"
      proof (intro allI impI, elim conjE)
        fix f
        assume x: "x = f (Suc n)" and *: "\<forall>i. f i \<in> vals A"
          and "min_at P A f n" and "bad ?P f"
        show "\<exists>g. ?C g f x"
        proof (cases "min_at P A f (Suc n)")
          case True
          then have "min_at P A (repl (Suc n) f f) (Suc n)" by simp
          moreover have "\<forall>i. f i \<in> vals A" by fact
          moreover have "lesseq x x" by simp
        ...
        next
          ...
          have "?g (Suc n) \<in> vals A"
            using less_vals [OF less `x \<in> vals A` [unfolded x]] by simp
          moreover
          from less have less': "less (?g (Suc n)) x" by (simp add: x)
          ...
          qed
          moreover have min_at: "min_at P A ?g n"
            using `min_at P A f n` by (auto simp: min_at_def)
          moreover have bad: "bad ?P ?g"
            using bad_repl [OF * `bad ?P f` `bad ?P h`, of "Suc n", OF eq] .
          ultimately obtain M where C: "?C M ?g (?g (Suc n))" by blast
          moreover with less_imp_lesseq [OF less]
            have "lesseq (M (Suc n)) (f (Suc n))"
            using lesseq_trans [of "M (Suc n)" "?g (Suc n)"] by auto
...
proof (induct x\<equiv>"f (Suc n)" arbitrary: f rule: less_induct)
  ...
  next
    ...
    then obtain h
      ...
      and "bad P h" by (auto simp: min_at_def)
    let ?g = "repl (Suc n) f h"
    from less have less': "less (?g (Suc n)) (f (Suc n))" by simp
    have min_at: "min_at P A ?g n"
      using `min_at P A f n` by (auto simp: min_at_def)
    ...
    have bad: "bad P ?g"
      using bad_repl [OF * `bad P f` `bad P h`, of "Suc n", OF eq] .
    have "?g (Suc n) \<in> vals A" using less_vals [OF less IH(1)] by simp
    from IH(2) [of ?g, OF this less' vals min_at bad] obtain M
...
lemma minimal_bad_0:
  assumes "\<forall>i. f i \<in> vals A"
    and "bad P f"
  shows "\<exists>g. (\<forall>i. g i \<in> vals A) \<and> min_at P A g 0 \<and> bad P g"
using assms(1) [THEN spec, of "0"] and assms
proof (induct x\<equiv>"f 0" arbitrary: f rule: less_induct)
  case IH show ?case
  proof (cases "min_at P A f 0")
    case True with IH show ?thesis by blast
  next
    case False
    then obtain h
      where less: "less (h 0) (f 0)"
      and vals: "\<forall>i. h i \<in> vals A"
      and bad: "bad P h"
      unfolding min_at_def by auto
    have "h 0 \<in> vals A"
      using less_vals [OF less IH(1)] .
    from IH(2) [of h, OF this less vals bad] obtain e
...
lemma mbs:
  assumes "\<forall>i. f i \<in> vals A" and "bad P f"
  shows "\<exists>g. (\<forall>i. g i \<in> vals A) \<and> bad P g \<and> (\<forall>n. min_at P A g n)"
proof -
  ...
  from minimal_bad_Suc
    ...
    (\<exists>M.
      ...
      min_at P A (repl (Suc n) f M) (Suc n))"
  ...
  have "\<forall>n. (\<forall>i. ?g n i \<in> vals A)
    \<and> (\<forall>i\<le>n. min_at P A (?g n) i)
  ...
  proof
    ...
    proof (induction n)
      case 0
      with `\<And>i. g i \<in> vals A` and `min_at P A g 0` and `bad P g`
        show ?case by auto
    next
      case (Suc n)
      from Suc and * [of "?g n" n]
        ...
        and bad: "bad P (?g n)"
        and "bad P (?g (Suc n))"
        and min_at: "min_at P A (?g n) n"
        and min_at_Suc: "min_at P A (?g (Suc n)) (Suc n)"
      ...
      proof
        ...
        qed
      qed
      moreover have "\<forall>i\<le>Suc n. min_at P A (?g (Suc n)) i"
      proof (intro allI impI)
        fix i assume "i \<le> Suc n"
        show "min_at P A (?g (Suc n)) i"
        proof (cases "i = Suc n")
        ...
        next
          ...
          proof (unfold min_at_def, intro allI impI, elim conjE)
            ...
            assume "less (e i) (?g (Suc n) i)"
            hence 3: "less (e i) (?g n i)" using * and le by simp
  ...
  then have vals: "\<forall>i. ?A i \<in> vals A"
    and 1: "\<forall>n. \<forall>i\<le>n. min_at P A (?g n) i"
  ...
  moreover have "\<forall>n S.
    ...
    less (S n) (?A n) \<longrightarrow> good P S"
  proof (intro allI impI, elim conjE)
    fix S n
    ...
    moreover have "less (S n) (?A n)" by fact
