...
| "eupdate (PLUS ex1 ex2) =
    ...
    | UINT b1 v1 \<Rightarrow>
        (case (eupdate ex2) of
            UINT b2 v2 \<Rightarrow> UINT (max b1 b2) ((v1 + v2) mod (2^(max (bits.to_nat b1) (bits.to_nat b2))))
          | e.INT b2 v2 \<Rightarrow>
              if b1 < b2
                then let v=v1+v2 in
                  if v \<ge> 0
...
lemma update_bounds_int:
  ...
  shows "(v < 2^((bits.to_nat b)-1)) \<and> v \<ge> -(2^((bits.to_nat b)-1))"
proof (cases ex)
  case (INT b' v')
  then show ?thesis
  proof cases
    ...
    moreover have "?x \<ge> -(2^((bits.to_nat b')-1))" by simp
  ...
  next
    ...
    then have "eupdate (e.INT b' v') = e.INT b' ?x" by simp
    with assms have "b=b'" and "v=?x" using INT by (simp,simp)
    moreover have "(-v'+2^((bits.to_nat b')-1)-1) mod (2^(bits.to_nat b'))\<ge>0" by simp
    hence "?x < 2^((bits.to_nat b')-1)" by arith
    moreover have "?x \<ge> -(2^((bits.to_nat b')-1))" using lower_bound2[of b' v'] by simp
    ultimately show ?thesis by simp
  qed
next
  case (UINT b' v')
  ...
  proof (cases "eupdate e1")
    ...
    proof (cases "eupdate e2")
      ...
      proof cases
        ...
        moreover have "?x \<ge> -(2^(max (bits.to_nat b1) (bits.to_nat b2)-1))" by simp
        ultimately show ?thesis by simp
      next
        ...
        with i i2 have "eupdate (PLUS e1 e2) = e.INT (max b1 b2) ?x" by simp
        with assms have "b=max b1 b2" and "v=?x" using p by (simp,simp)
        moreover have "(-?v+2^(max (bits.to_nat b1) (bits.to_nat b2)-1)-1) mod (2^max (bits.to_nat b1) (bits.to_nat b2))\<ge>0" by simp
        hence "?x < 2 ^(max (bits.to_nat b1) (bits.to_nat b2)-1)" by arith
        moreover have "?x \<ge> -(2^(max (bits.to_nat b1) (bits.to_nat b2)-1))" using lower_bound2[of "max b1 b2" ?v] by simp
        ultimately show ?thesis by simp
      qed
    next
      case u: (UINT b2 v2)
      ...
      proof cases
        ...
        proof cases
          ...
          moreover have "?x \<ge> -(2^((bits.to_nat b1)-1))" by simp
          ultimately show ?thesis by simp
        next
          ...
          moreover have "(-?v+2^((bits.to_nat b1)-1)-1) mod 2^(bits.to_nat b1)\<ge>0" by simp
          hence "?x < 2 ^((bits.to_nat b1)-1)" by arith
          moreover have "?x \<ge> -(2^((bits.to_nat b1)-1))" using lower_bound2[of b1 ?v] by simp
          ultimately show ?thesis by simp
        qed
      next
        assume "\<not> b2<b1"
    ...
    proof (cases "eupdate e2")
      ...
      proof cases
        ...
        proof cases
          ...
          moreover have "?x \<ge> -(2^((bits.to_nat b2)-1))" by simp
          ultimately show ?thesis by simp
        next
          ...
          moreover have "(-?v+2^((bits.to_nat b2)-1)-1) mod 2^(bits.to_nat b2)\<ge>0" by simp
          hence "?x < 2 ^((bits.to_nat b2)-1)" by arith
          moreover have "?x \<ge> -(2^((bits.to_nat b2)-1))" using lower_bound2[of b2 ?v] by simp
          ultimately show ?thesis by simp
        qed
      next
        assume "\<not> b1<b2"
...
lemma update_bounds_uint:
  ...
  shows "v < 2^(bits.to_nat b) \<and> v \<ge> 0"
proof (cases ex)
  case (INT b' v')
        ...
        proof (cases a)
          ...
          proof (cases b)
            ...
            proof (cases t)
              case (TUInt x)
              then show ?thesis
              proof (cases "less (TUInt x) (TUInt b256) v (ShowL\<^sub>i\<^sub>n\<^sub>t (int al))")
                case None
              ...
              next
                ...
                proof (cases p)
                  case (Pair vb tb)
                  then show ?thesis
                  proof (cases "vb = ShowL\<^sub>b\<^sub>o\<^sub>o\<^sub>l True \<and> tb = TBool")
                    case True
                    ...
                    proof (cases "vb = ShowL\<^sub>b\<^sub>o\<^sub>o\<^sub>l True \<and> tb = TBool")
                      ...
                      proof (cases c)
                        ...
                        proof (cases "accessStore (hash loc v) (Memory st)")
                          case None
                          with Pair Some v1 TUInt k1 tp1 s1 c1.prems f1 True \<open>vb = ShowL\<^sub>b\<^sub>o\<^sub>o\<^sub>l True \<and> tb = TBool\<close> show ?thesis using c2 by (simp add:Statements.solidity.msel.simps)
                        next
                          case s3: (Some a)
                          then show ?thesis
                          proof (cases a)
                            case (MValue x1)
                            with Pair Some s3 v1 TUInt k1 tp1 s1 c1.prems f1 True \<open>vb = ShowL\<^sub>b\<^sub>o\<^sub>o\<^sub>l True \<and> tb = TBool\<close> show ?thesis using c2 by (simp add:Statements.solidity.msel.simps)
                          next
                            case mp: (MPointer l)
                            from c1.IH[OF c1.prems]
                              have "msel c tp l ix env cd st g' = msel c tp l (map f ix) env cd st g'" by simp
                            with mp Pair Some s3 v1 TUInt k1 tp1 s1 c1.prems f1 True \<open>vb = ShowL\<^sub>b\<^sub>o\<^sub>o\<^sub>l True \<and> tb = TBool\<close> show ?thesis using c2 by (simp add:Statements.solidity.msel.simps)
                          qed
                        qed
                      next
                        case False
                        ...
                        proof (cases "accessStore (hash loc v) cd")
                          case None
                          with Pair Some v1 TUInt k1 tp1 s1 c1.prems f1 False \<open>vb = ShowL\<^sub>b\<^sub>o\<^sub>o\<^sub>l True \<and> tb = TBool\<close> show ?thesis using c2 by (simp add:Statements.solidity.msel.simps)
                        next
                          case s3: (Some a)
                          then show ?thesis
                          proof (cases a)
                            case (MValue x1)
                            with Pair Some s3 v1 TUInt k1 tp1 s1 c1.prems f1 False \<open>vb = ShowL\<^sub>b\<^sub>o\<^sub>o\<^sub>l True \<and> tb = TBool\<close> show ?thesis using c2 by (simp add:Statements.solidity.msel.simps)
                          next
                            case mp: (MPointer l)
                            from c1.IH[OF c1.prems]
                              have "msel c tp l ix env cd st g' = msel c tp l (map f ix) env cd st g'" by simp
                            with mp Pair Some s3 v1 TUInt k1 tp1 s1 c1.prems f1 False \<open>vb = ShowL\<^sub>b\<^sub>o\<^sub>o\<^sub>l True \<and> tb = TBool\<close> show ?thesis using c2 by (simp add:Statements.solidity.msel.simps)
                          qed
                        qed
                      qed
                    next
...
next
  ...
  proof (cases "g > 0")
    ...
    then show ?thesis
      ...
      also have "createSInt b v = createSInt b ?m_def" using bits.to_nat[of b] `v \<ge> 0` unfolding createSInt_def by auto
      also from `v \<ge> 0` True have
    ...
    next
      ...
      also have "createSInt b v = createSInt b ?m_def" using bits.to_nat[of b] `\<not> v \<ge> 0` unfolding createSInt_def by (auto split:if_splits)
      also from `\<not> v \<ge> 0` True have
        "Normal ((KValue (createSInt b ?m_def), Value (TSInt b)),g) =expr (eupdate (e.INT b v)) env cd st g"
      by (simp add:Statements.solidity.expr.simps)
      finally show "expr (e.INT b v) env cd st g = expr (eupdate (e.INT b v)) env cd st g" by simp
    qed
...
next
  ...
  proof (cases "eupdate e1")
    ...
    proof (cases "g > 0")
      ...
      moreover from i have "v1 < 2^((bits.to_nat b1)-1)" and "v1 \<ge> -(2^((bits.to_nat b1)-1))" using update_bounds_int by auto
      ...
      proof (cases "eupdate e2")
        ...
        moreover from i2 have "v2 < 2^((bits.to_nat b2)-1)" and "v2 \<ge> -(2^((bits.to_nat b2)-1))" using update_bounds_int by auto
        ...
        proof (cases)
          ...
          proof -
            from `?v\<ge>0` have "eupdate (PLUS e1 e2) = e.INT (max b1 b2) ?x" using i i2 by simp
        ...
        next
          ...
          moreover have "expr (eupdate (PLUS e1 e2)) env cd st g
            = Normal ((KValue (ShowL\<^sub>i\<^sub>n\<^sub>t ?x), Value (TSInt (max b1 b2))),g)"
          proof -
            from `\<not>?v\<ge>0` have "eupdate (PLUS e1 e2) = e.INT (max b1 b2) ?x" using i i2 by simp
            moreover have "expr (e.INT (max b1 b2) ?x) env cd st g
              = Normal ((KValue (ShowL\<^sub>i\<^sub>n\<^sub>t ?x), Value (TSInt (max b1 b2))),g)"
            proof -
              from True have "expr (e.INT (max b1 b2) ?x) env cd st g
      ...
      next
        ...
        moreover from u2 have "v2 < 2^(bits.to_nat b2)" and "v2 \<ge> 0" using update_bounds_uint by auto
        ...
        proof (cases)
          ...
          proof (cases)
            ...
            proof -
              from `?v\<ge>0` `b2<b1` have "eupdate (PLUS e1 e2) = e.INT b1 ?x" using i u2 by simp
          ...
          next
            ...
            moreover have "expr (eupdate (PLUS e1 e2)) env cd st g
              = Normal ((KValue (ShowL\<^sub>i\<^sub>n\<^sub>t ?x), Value (TSInt b1)),g)"
            proof -
              from `\<not>?v\<ge>0` `b2<b1` have "eupdate (PLUS e1 e2) = e.INT b1 ?x" using i u2 by simp
              moreover have "expr (e.INT b1 ?x) env cd st g
                = Normal ((KValue (ShowL\<^sub>i\<^sub>n\<^sub>t ?x), Value (TSInt b1)),g)"
              proof -
                from True have "expr (e.INT b1 ?x) env cd st g
                  = Normal ((KValue (createSInt b1 ?x), Value (TSInt b1)),g)" by (simp add:Statements.solidity.expr.simps)
                moreover have "?x \<ge> - (2 ^ ((bits.to_nat b1) - 1))" using upper_bound2 by simp
  ...
  next
    ...
    proof (cases "g > 0")
      ...
      moreover from u have "v1 < 2^(bits.to_nat b1)" and "v1 \<ge> 0" using update_bounds_uint by auto
      ...
      next
        ...
        proof (cases)
          ...
          proof (cases)
            ...
            proof -
              from `?v\<ge>0` `b1<b2` have "eupdate (PLUS e1 e2) = e.INT b2 ?x" using u i2 by simp
          ...
          next
            ...
            moreover have "expr (eupdate (PLUS e1 e2)) env cd st g
              = Normal ((KValue (ShowL\<^sub>i\<^sub>n\<^sub>t ?x), Value (TSInt b2)),g)"
            proof -
              from `\<not>?v\<ge>0` `b1<b2` have "eupdate (PLUS e1 e2) = e.INT b2 ?x" using u i2 by simp
              moreover have "expr (e.INT b2 ?x) env cd st g
                = Normal ((KValue (ShowL\<^sub>i\<^sub>n\<^sub>t ?x), Value (TSInt b2)),g)"
              proof -
                from True have "expr (e.INT b2 ?x) env cd st g
                  = Normal ((KValue (createSInt b2 ?x), Value (TSInt b2)),g)" by (simp add:Statements.solidity.expr.simps)
                moreover have "?x \<ge> - (2 ^ ((bits.to_nat b2) - 1))" using upper_bound2 by simp
  ...
  proof (cases "eupdate e1")
    ...
    proof (cases "g > 0")
      ...
      next
        ...
        with True have "expr e2 env cd st g = Normal ((KValue (createUInt b2 v2), Value (TUInt b2)),g)" by (simp add:Statements.solidity.expr.simps)
        moreover from u have "v2 < 2^(bits.to_nat b2)" and "v2 \<ge> 0" using update_bounds_uint by auto
        ultimately have r2: "expr e2 env cd st g = Normal ((KValue (ShowL\<^sub>i\<^sub>n\<^sub>t v2), Value (TUInt b2)),g)"
          using createUInt_id[of v2 b2] by simp
        thus ?thesis
        proof (cases)
  ...
  next
    ...
    proof (cases "g > 0")
      ...
      next
        ...
        with True have "expr e2 env cd st g = Normal ((KValue (createSInt b2 v2), Value (TSInt b2)),g)" by (simp add:Statements.solidity.expr.simps)
        moreover from i have "v2 < 2^((bits.to_nat b2)-1)" and "v2 \<ge> -(2^((bits.to_nat b2)-1))" using update_bounds_int by auto
        ultimately have r2: "expr e2 env cd st g = Normal ((KValue (ShowL\<^sub>i\<^sub>n\<^sub>t v2), Value (TSInt b2)),g)"
          using createSInt_id[of v2 b2] by simp
        thus ?thesis
        proof (cases)
