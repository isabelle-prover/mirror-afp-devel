...
theory AbstractRewriting imports Main begin
...
lemma SN_imp_WN: assumes "SN r" shows "WN r"
proof -
  from `SN r` have "wf(r^-1)"
    by(simp add: SN_def SN_elt_def wf_iff_no_infinite_down_chain)
  show "WN r"
  proof (auto simp: WN_def WN_elt_def normalizability_def NF_def Image_def)
    fix a
    show "\<exists>b. (a, b) \<in> r\<^sup>* \<and> (\<forall>c. (b, c) \<notin> r)"
      by(rule wfE_min[OF `wf(r^-1)`, of a "r^* `` {a}", simplified])
        (metis rtrancl_into_rtrancl)
  qed
qed
...
proof (rule ccontr)
  assume "\<not> SN R"
  then obtain S::"nat \<Rightarrow> 'a" where "\<forall>i. (S i,S(Suc i)) \<in> R" unfolding SN_defs by auto
...
using assms proof (rule contrapos_pp)
  ...
  then obtain S :: "nat \<Rightarrow> 'a" where "S 0 = t" and S: "\<forall>i. (S i,S(Suc i)) \<in> R^+"
...
definition quasi_commute :: "'a ars \<Rightarrow> 'a ars \<Rightarrow> bool"
where "quasi_commute r s \<equiv> (s O r) \<subseteq> r O (r \<union> s)^*"
