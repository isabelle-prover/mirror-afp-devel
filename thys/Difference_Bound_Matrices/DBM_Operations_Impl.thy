...
\<comment> \<open>However, DBM entries are NOT a member of this typeclass.\<close>
...
proof -
  ...
  using assms
   ...
   proof -
     ...
     from prems(6) have "canonical (foldr (\<lambda>c M. reset_canonical M (v c) d) cs M) n"
       apply (induction cs)
       using \<open>canonical M n\<close> apply force
       apply simp
  ...
  also have "\<dots> = [reset'' M n cs v d]\<^bsub>v,n\<^esub>" unfolding reset''_def
...
definition
  "unbounded_dbm n = (\<lambda> (i, j). (if i = j \<or> i > n \<or> j > n then Le 0 else \<infinity>))"

definition And_upd :: "nat \<Rightarrow> ('t::{linorder,zero}) DBM' \<Rightarrow> 't DBM' \<Rightarrow> 't DBM'" where
  "And_upd n A B =
    fold (\<lambda>i M. 
...
lemma fold_loop_inv_rule:
  assumes "I 0 x"
  assumes "\<And>i x. I i x \<Longrightarrow> i \<le> n \<Longrightarrow> I (Suc i) (f i x)"
  assumes "\<And>x. I n x \<Longrightarrow> Q x"
...
lemma And_upd_min:
  ...
  apply (rule fold_loop_inv_rule[where I = "\<lambda>k M. \<forall>i<k. \<forall>j \<le> n. M(i,j) = min (A(i,j)) (B(i,j))"])
    apply (simp; fail)
  subgoal for k x
    apply (rule fold_loop_inv_rule[where I = "
        \<lambda>j' M. \<forall>i\<le>k.
          if i = k then
            (\<forall>j < j'. M(i,j) = min (A(i,j)) (B(i,j)))
          else
            (\<forall>j \<le> n. M(i,j) = min (A(i,j)) (B(i,j)))"])
    by (simp_all) (metis Suc_eq_plus1 less_Suc_eq_le)
  using assms by auto

lemma And_upd_And:
  assumes "i \<le> n" "j \<le> n"
    "\<forall> i \<le> n. \<forall> j \<le> n. A (i, j) = A' i j" "\<forall> i \<le> n. \<forall> j \<le> n. B (i, j) = B' i j"
...
\<comment> \<open>Copy from Regions Beta, original should be moved\<close>
