...
lemma Lp_tendsto_AE_subseq:
  ...
  shows "\<exists>r. subseq r \<and> (AE x in M. (\<lambda>n. f (r n) x) \<longlonglongrightarrow> g x)"
proof -
  ...
  have "\<exists>r. subseq r \<and> (AE x in M. (\<lambda>n. \<bar>f (r n) x - g x\<bar> powr p) \<longlonglongrightarrow> 0)"
    apply (rule tendsto_L1_AE_subseq) using int ** by auto
  then obtain r where "subseq r" "AE x in M. (\<lambda>n. \<bar>f (r n) x - g x\<bar> powr p) \<longlonglongrightarrow> 0"
    by blast
...
next
  ...
  obtain r0 where r0: "subseq r0" "(\<lambda>n. eNorm (\<LL> p M) (f (r0 n))) \<longlonglongrightarrow> le"
  ...
  have "subseq r" unfolding r_def using `subseq r0` by (simp add: subseq_Suc_iff)
  ...
  next
    ...
    have "liminf (\<lambda>n. ennreal(\<bar>f (r n) x\<bar> powr p2)) = \<bar>g x\<bar> powr p2" if "(\<lambda>n. f n x) \<longlonglongrightarrow> g x" for x
      ...
      using LIMSEQ_subseq_LIMSEQ[OF that `subseq r`] unfolding comp_def by auto
  ...
  next
    ...
    proof -
      have "(\<lambda>n. f (r n) x) \<longlonglongrightarrow> g x"
        using that LIMSEQ_subseq_LIMSEQ[OF _ `subseq r`] unfolding comp_def by auto
