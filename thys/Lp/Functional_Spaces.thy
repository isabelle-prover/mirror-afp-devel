  ...
  proof -
    ...
    proof -
      ...
      also have "... < defect N * f + defect N * f"
        ...
        using defect_ge_1[of N] `n \<ge> M` seq_suble[OF `subseq r`, of M] M1 M2 o_def unfolding M_def by auto
      finally show ?thesis
...
proof (rule complete\<^sub>N_I)
  ...
  then obtain r where r: "subseq r" "\<And>n. \<forall>m\<ge>r n. \<forall>p\<ge>r n. Norm N (v m - v p) < c n"
  ...
  then have "tendsto_in\<^sub>N N v x"
    using `\<forall>n. v n \<in> space\<^sub>N N` by (intro cauchy_tendsto_in_subseq[OF _ `cauchy_in\<^sub>N N v` `subseq r`], auto)
