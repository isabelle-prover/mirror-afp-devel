...
lemma Frel_left_Galois_le_left_Galois:
  ...
  apply (fold
    rel_restrict_right_eq_inv_restrict_left_inv[of "\<lambda>x y. x \<le>\<^bsub>L1\<^esub> r1 y" "in_codom (\<le>\<^bsub>R1\<^esub>)",
      unfolded rel_restrict_left_pred_def rel_inv_iff_rel]
    rel_restrict_right_eq_inv_restrict_left_inv[of "\<lambda>x y. x \<le>\<^bsub>L2\<^esub> r2 y" "in_codom (\<le>\<^bsub>R2\<^esub>)",
      unfolded rel_restrict_left_pred_def rel_inv_iff_rel]
    rel_restrict_right_eq_inv_restrict_left_inv[of "\<lambda>x y. x \<le>\<^bsub>L3\<^esub> r3 y" "in_codom (\<le>\<^bsub>R3\<^esub>)",
      unfolded rel_restrict_left_pred_def rel_inv_iff_rel])
