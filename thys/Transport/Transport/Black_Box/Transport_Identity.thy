...
locale transport_id = fixes L :: "'a \<Rightarrow> 'a \<Rightarrow> bool"
begin
...
lemma left_Galois_eq_left [simp]: "(\<^bsub>L\<^esub>\<lessapprox>) = (\<le>\<^bsub>L\<^esub>)"
...
locale transport_eq_restrict_id = fixes P :: "'a \<Rightarrow> bool"
begin
sublocale tbij ?: transport_eq_restrict_bijection P P id id
  using bijection_on_self_id by (unfold_locales) fastforce+
sublocale tper_id ?: transport_partial_equivalence_rel_id "(=\<^bsub>P\<^esub>)"
