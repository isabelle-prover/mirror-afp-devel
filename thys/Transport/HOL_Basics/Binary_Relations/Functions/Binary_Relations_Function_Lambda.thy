...
lemma cdep_fun_rel_lambda: "((x : A) \<rightarrow>\<^sub>c ((=) (f x))) (\<lambda>x : A. f x)"
  by (intro crel_dep_funI') auto

text \<open>Compare the following with @{thm mono_rel_dep_fun_mono_wrt_pred_eval}.\<close>

lemma mono_wrt_pred_mono_crel_dep_fun_rel_lambda:
  "(((x : A) \<Rrightarrow>\<^sub>m B x) \<Rrightarrow>\<^sub>m (x : A) \<rightarrow>\<^sub>c B x) (rel_lambda A)"
...
lemma rel_lambda_eval_eq_if_crel_dep_fun [simp]: "((x : A) \<rightarrow>\<^sub>c B x) R \<Longrightarrow> (\<lambda>x : A. R`x) = R"
...
proof -
  from assms mono_rel_dep_fun_ge_crel_dep_fun_rel_restrict_left have "((x : A') \<rightarrow>\<^sub>c B x) R\<restriction>\<^bsub>A'\<^esub>"
    by force
  then show ?thesis
