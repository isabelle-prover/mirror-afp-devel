...
begin
syntax
  "_rel_lambda"  :: "pttrns \<Rightarrow> 'a \<Rightarrow> 'b \<Rightarrow> 'c" (\<open>(2\<lambda>\<^sub>r_ : _./ _)\<close> [0, 0, 10] 10)
syntax_consts
  "_rel_lambda" \<rightleftharpoons> rel_lambda
translations
  "\<lambda>\<^sub>rx xs : A. f" \<rightharpoonup> "CONST rel_lambda A (\<lambda>x. _rel_lambda xs A f)"
  "\<lambda>\<^sub>rx : A. f" \<rightleftharpoons> "CONST rel_lambda A (\<lambda>x. f)"
end

lemma rel_lambdaI [intro]:
  assumes "A x"
  and "f x = y"
  shows "(\<lambda>\<^sub>rx : A. f x) x y"
  using assms unfolding rel_lambda_pred_def by auto

lemma rel_lambda_appI:
  assumes "A x"
  shows "(\<lambda>\<^sub>rx : A. f x) x (f x)"
  using assms by auto

lemma rel_lambdaE [elim!]:
  assumes "(\<lambda>\<^sub>rx : A. f x) x y"
  obtains "A x" "y = f x"
  using assms unfolding rel_lambda_pred_def by auto

lemma rel_lambda_cong [cong]:
  "\<lbrakk>\<And>x. A x \<longleftrightarrow> A' x; \<And>x. A' x \<Longrightarrow> f x = f' x\<rbrakk> \<Longrightarrow> (\<lambda>\<^sub>rx : A. f x) = (\<lambda>\<^sub>rx : A'. f' x)"
  by (intro ext) auto

lemma in_dom_rel_lambda_eq [simp]: "in_dom (\<lambda>\<^sub>rx : A. f x) = A"
  by auto

lemma in_codom_rel_lambda_eq_has_inverse_on [simp]: "in_codom (\<lambda>\<^sub>rx : A. f x) = has_inverse_on A f"
  by fastforce

lemma left_total_on_rel_lambda: "left_total_on A (\<lambda>\<^sub>rx : A. f x)"
  by auto

lemma right_unique_on_rel_lambda: "right_unique_on A (\<lambda>\<^sub>rx : A. f x)"
  by auto

lemma crel_dep_mono_wrt_pred_rel_lambda: "((x : A) \<rightarrow>\<^sub>c ((=) (f x))) (\<lambda>\<^sub>rx : A. f x)"
...
lemma app_eq_if_rel_lambda_eqI:
  assumes "(\<lambda>\<^sub>rx : A. f x) = (\<lambda>\<^sub>rx : A. g x)"
  and "A x"
  shows "f x = g x"
...
lemma crel_dep_mono_wrt_pred_inf_rel_lambda_inf_if_rel_dep_mono_wrt_pred:
  ...
  shows "((x : A \<sqinter> A') \<rightarrow>\<^sub>c B x) (\<lambda>\<^sub>rx : A \<sqinter> A'. R`x)"
  using assms by force

corollary crel_dep_mono_wrt_pred_rel_lambda_if_le_if_rel_dep_mono_wrt_pred:
  assumes "((x : A) \<rightarrow> B x) R"
  and [uhint]: "A' \<le> A"
  shows "((x : A') \<rightarrow>\<^sub>c B x) (\<lambda>\<^sub>rx : A'. R`x)"
...
lemma rel_lambda_ext:
  ...
  shows "(\<lambda>\<^sub>rx : A. f x) = R"
  using assms by (intro ext iffI) (auto intro!: rel_lambdaI intro: rel_eval_if_rel_dep_mono_wrt_predI)

lemma rel_lambda_eval_eq_if_crel_dep_mono_wrt_pred [simp]: "((x : A) \<rightarrow>\<^sub>c B x) R \<Longrightarrow> (\<lambda>\<^sub>rx : A. R`x) = R"
...
lemma eq_rel_lambda_if_crel_dep_mono_wrt_predE:
  assumes "((x : A) \<rightarrow>\<^sub>c B x) R"
  obtains f where "R = (\<lambda>\<^sub>rx : A. f x)"
proof
  let ?f="(\<lambda>x. R`x)"
  from assms show "R = (\<lambda>\<^sub>rx : A. (\<lambda>x. R`x) x)" by simp
qed

lemma rel_restrict_left_eq_rel_lambda_if_le_if_rel_dep_mono_wrt_pred:
  assumes "((x : A) \<rightarrow> B x) R"
  and "A' \<le> A"
  shows "R\<restriction>\<^bsub>A'\<^esub> = (\<lambda>\<^sub>rx : A'. R`x)"
proof -
...
lemma mono_rel_lambda: "mono (\<lambda>A. \<lambda>\<^sub>rx : A. f x)"
  by auto
