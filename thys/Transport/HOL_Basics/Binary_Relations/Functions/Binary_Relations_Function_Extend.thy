...
lemma rel_dep_fun_extend_if_rel_dep_funI:
  ...
  using assms by (intro rel_dep_funI left_total_on_sup_eq_extend_if_left_total_on
...
lemma rel_dep_fun_glue_if_right_unique_if_rel_dep_fun:
...
proof (intro rel_dep_funI dep_mono_wrt_predI left_total_onI)
  fix x assume "D x"
  with funs obtain R where hyps: "\<R> R" "A R x" "((x : A R) \<rightarrow> B x) R" by auto
  then have "B x (R`x)" by (auto elim: rel_dep_fun_relE)
  ...
  qed (use hyps in \<open>auto elim: rel_dep_fun_relE\<close>)
  ultimately show "B x (glue \<R>`x)" by simp
qed (use assms in \<open>fastforce+\<close>)

lemma crel_dep_fun_glue_if_right_unique_if_crel_dep_fun:
  ...
  using assms by (intro crel_dep_funI rel_dep_fun_glue_if_right_unique_if_rel_dep_fun) fastforce+
...
lemma crel_dep_fun_sup_if_eval_eq_if_crel_dep_fun:
...
proof -
  ...
  from dep_funs have "A = A'" if "R = R'" using that by (simp only: flip:
    in_dom_eq_if_crel_dep_fun[OF dep_funs(1)] in_dom_eq_if_crel_dep_fun[OF dep_funs(2)])
  then have [uhint]: "in_codom_on ((=) R \<squnion> (=) R') ?A = A \<squnion> A'"
    by (intro ext iffI) (fastforce split: if_splits)+
  show ?thesis by (urule (rr) crel_dep_fun_glue_if_right_unique_if_crel_dep_fun
    right_unique_on_sup_if_rel_agree_on_sup_if_right_unique_on
    rel_agree_on_if_eval_eq_if_rel_dep_fun)
    (use assms in \<open>auto 7 0 intro: rel_dep_fun_contravariant_dom dest: right_unique_onD\<close>)
qed
