...
lemma Scheduler_L2:
 assumes h1:"Scheduler C activation"
     and h2:"\<not> (t mod cycleLength C) mem (schedule C)"
 shows "activation t = []"
...
lemma fr_Send1:
 assumes h1:"FlexRayController (nReturn i) recv (nC i) (nStore i) (nSend i) (nGet i)"
     and h2:"\<not> (t mod cycleLength (nC i) mem schedule (nC i))"
 shows      "(nSend i) t = []"
proof - 
  from h1 obtain activation where 
    ...
    by (simp add: FlexRayController_def, auto)
  from a1 and h2 have sg1:"activation t = []" by (simp add: Scheduler_L2)
...
proof 
 from assms show "k < n \<longrightarrow> nSend k t \<noteq> []"  by (simp add: fr_Send3)
qed
...
proof (cases "j = k")
  assume a1: "j = k"
  from assms have sg1: "nSend k t = nReturn k t" by (simp add: fr_Send2)
  from sg1 and a1 and h6 show ?thesis by simp
next 
  assume a2:"j \<noteq> k"
  from assms and a2 show ?thesis by (simp add: disjointFrame_L3)
qed 
...
proof -
  from h1 and h3 have sg1:
    "FlexRayController (nReturn k) recv (nC k) (nStore k) (nSend k) (nGet k)"
    by auto
  from sg1 and h2  show ?thesis  by (rule fr_nGet1a)
qed
...
next
  assume a2:"\<not> (t mod cycleLength (nC k) mem schedule (nC k))"
  from h1 and a2 have sg2:"nGet k t = []" by (rule fr_nGet2a)
...
using assms by (simp add: msg_def, auto, rule length_nGet1)
...
proof -
  from h1 and h2 have sg1:
   "FlexRayController (nReturn k) recv (nC k) (nStore k) (nSend k) (nGet k)"
    by auto
  from sg1 show ?thesis by (rule msg_nGet1)
qed 
...
proof -
  ...
  proof (cases "\<exists>k<n. nSend k t \<noteq> []")
    assume a1:"\<exists>k<n. nSend k t \<noteq> []"
    from a1 obtain k where a2:"k<n" and a3:"nSend k t \<noteq> []"  by auto
    ...
    next
      assume aa2:"i \<noteq> k"
      from h7 and h4 and h1 and h2 and h3 and sg10 and a2 and h5 and aa2 have sg11:
    ...
    qed
  next
    assume a10:"\<not> (\<exists>k<n. nSend k t \<noteq> [])"
    from h7 and a10 have sg14:"recv t = []" by (simp add: Broadcast_nSend_empty1)  
