...
proof - 
  ...
  next
    ...
    proof (cases "Suc j \<le> d")
      assume "\<not> Suc j \<le> d" from this show ?thesis by simp
    next
      assume a0:"Suc j \<le> d"
      from a0  have sg2:"d + Suc j \<le> d + d"   by arith
      from sg2 have sg3:"Suc (d + j) \<le> d + d" by arith
...
next
  case (Suc m)
  from Suc have sg1:"msg (Suc 0) x1" by (simp add:  Loss_Delay_msg_a)
...
lemma Gateway_L6:
 assumes h1:"Gateway req dt a stop lose d ack i vc"
     and h2:"\<forall>m\<le>k. req (t + m) \<noteq> [send]"
     and h3:"\<forall>j\<le>k. lose (t + j) = [False]"
     and h4:"ack t = [connection_ok]"
...
lemma Gateway_L6a:
 assumes h1:"Gateway req dt a stop lose d ack i vc"
     and h2:"\<forall>m\<le>k. req (t + 2 + m) \<noteq> [send]"
     and h3:"\<forall>j\<le>k. lose (t + 2 + j) = [False]"
     and h4:"ack (t + 2) = [connection_ok]"
     and h5:"msg (Suc 0) req"
     and h6:"msg (Suc 0) stop"
     and h7:"msg (Suc 0) a"
     and h8:"ts lose"
...
proof -
  ...
  from h1 and h2 and h3 and h4 and h5 and h6 and h9 and sg1 and sg2 have sg3:
...
proof -
  ...
  from sg1 and sg2 and sg3 and sg4 and sg5 and sg6 and sg7 and h6 show ?thesis
    by (simp add: eval_nat_numeral)
qed
...
lemma GatewaySystem_L1aux:
  assumes h1:"msg (Suc 0) req"
      ...
      and h3:"msg (Suc 0) a"
      and h4:"ts lose"
      and h5:"msg (Suc 0) req \<and> msg (Suc 0) a \<and> msg (Suc 0) stop \<and> ts lose \<longrightarrow> 
...
proof -
  ...
  from h1 and h7 and h6 and h5 and h4 and h9 and sg12 and h12 and h8 and sg13 and h10
...
proof -
  ...
  from h14 have sg21:"\<forall>j\<le>2 * d. lose (t + 3 + k + j) = [False]"
    by (simp add: streamValue43)
  from h1 and h4 and h5 and h6 and h7 and sg21 and sg10 and sg20 have sg22:
    "\<forall>x \<le> d + d. ack (t + 3 + k + x) = [sending_data]"
    by (simp add: Gateway_L8)
  from h2 and h3 and h4 and h5 and h6 and h7 and h14 and sg16 and sg22 show ?thesis
