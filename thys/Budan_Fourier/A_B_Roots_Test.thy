(*
    Title:      A_B Roots Test
    Author:     Wenda Li <wl302@cam.ac.uk / liwenda1990@hotmail.com>
*)

section \<open>A\_B Roots Test\<close>

theory A_B_Roots_Test imports Budan_Fourier
begin

text \<open>The Alesina-Galuzzi "a\_b roots test" is a consequence of Descartes' rule of signs:
  through counting sign variations on coefficients of a Mobius-transformed polynomial, it can 
  over-approximate the number of real roots (counting multiplicity) within an interval. 
  Its ability is similar to the Budan-Fourier theorem, but is far more efficient in practice. 
  Therefore, this test is widely used in modern root isolation procedures. 
...
definition ab_roots_test::"real \<Rightarrow> real \<Rightarrow> real poly \<Rightarrow> nat" where
  "ab_roots_test a b p = nat (changes (coeffs (fcompose p [:a,b:] [:1,1:])))"
  
theorem ab_roots_test:
  fixes p::"real poly"
  assumes "p\<noteq>0" "a<b"
  shows "proots_count p {x. a < x \<and> x <b} \<le> ab_roots_test a b p  \<and> 
          even (ab_roots_test a b p - proots_count p {x. a < x \<and> x < b})"
proof -
  ...
  ultimately show ?thesis 
    unfolding ab_roots_test_def
    apply (fold q_def)
    by auto
qed

text \<open>The root test @{term ab_roots_test} is exact if its result is 0 or 1.\<close>

corollary ab_roots_test_zero:
  fixes p::"real poly"
  assumes "p\<noteq>0" "a<b" "ab_roots_test a b p = 0"
  shows "\<forall>x. a<x \<and> x<b \<longrightarrow> poly p x\<noteq>0"
proof -
  have "proots_count p {x. a < x \<and> x <b} = 0"
    using ab_roots_test[OF assms(1,2)] assms(3) by auto
  from proots_count_0_imp_empty[OF this \<open>p\<noteq>0\<close>] 
  show ?thesis by auto
qed

corollary ab_roots_test_one:
  fixes p::"real poly"
  assumes "p\<noteq>0" "a<b" "ab_roots_test a b p = 1"
  shows "proots_count p {x. a < x \<and> x <b} = 1"
  using ab_roots_test[OF \<open>p\<noteq>0\<close> \<open>a<b\<close>] \<open>ab_roots_test a b p = 1\<close> 
  by (metis dvd_diffD even_zero le_neq_implies_less less_one odd_one)


text \<open>Similar to the Budan-Fourier theorem, the a\_b test result is exact when all roots 
      are real.\<close>
corollary ab_roots_test_real:
  fixes p::"real poly"
  assumes "p\<noteq>0" "a<b" 
  assumes "all_roots_real p"
  shows "proots_count p {x. a < x \<and> x <b} = ab_roots_test a b p"
proof -
  define q where "q=fcompose p [:a,b:] [:1,1:]"
  have "q\<noteq>0"
    unfolding q_def
  ...
  then have "proots_count q {x. 0 < x} = nat (changes (coeffs q))"
    by simp
  ultimately show ?thesis unfolding ab_roots_test_def 
    apply (fold q_def)
    by auto
qed
