...
lemma linorder_of_list_permute:
  ...
  shows   "linorder_of_list (permute_list \<pi> xs) =
             map_relation (set xs) ((op !) xs \<circ> inv \<pi> \<circ> index xs) (linorder_of_list xs)"
proof -
...
proof -
  ...
  also have "linorder_of_list (permute_list \<pi> xs) = 
               map_relation A ((op !) xs \<circ> inv \<pi> \<circ> index xs) (linorder_of_list xs)"
  ...
  finally have "R' = map_relation A ((op !) xs \<circ> inv \<pi> \<circ> index xs) R" .
