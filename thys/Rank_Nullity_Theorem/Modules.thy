...
theory Modules
  imports Complex_Main "HOL-Library.FuncSet" "HOL-Library.Product_Plus" Hull
begin

lemma cond_application_beta:\<comment>\<open>TODO: duplicate!\<close>
  "(if b then f else g) x = (if b then f x else g x)"
  by auto

lemma isomorphism_expand:\<comment>\<open>TODO: duplicate!\<close>
...
lemma module_hom_iff:
  "module_hom scaleB scaleC  f \<longleftrightarrow>
    module scaleB \<and> module scaleC \<and>
    (\<forall>x y. f (x + y) = f x + f y) \<and> (\<forall>c x. f (scaleB c x) = scaleC c (f x))"
  by (simp add: module_hom_def module_hom_axioms_def)

lemma module_hom_zero: "module scale \<Longrightarrow> module scaleC \<Longrightarrow> module_hom scale scaleC (\<lambda>x. 0)"
  by (simp add: module_hom_iff module.scale_zero_right)

lemma module_hom_add: "module_hom scale scaleC f \<Longrightarrow> module_hom scale scaleC g \<Longrightarrow> module_hom scale scaleC (\<lambda>x. f x + g x)"
  by (simp add: module_hom_iff module.scale_right_distrib)

lemma module_hom_sub: "module_hom scale scaleC f \<Longrightarrow> module_hom scale scaleC g \<Longrightarrow> module_hom scale scaleC (\<lambda>x. f x - g x)"
  by (simp add: module_hom_iff module.scale_right_diff_distrib)

lemma module_hom_neg: "module_hom scale scaleC f \<Longrightarrow> module_hom scale scaleC (\<lambda>x. - f x)"
  by (simp add: module_hom_iff module.scale_minus_right)

lemma module_hom_scale: "module_hom scale scaleC f \<Longrightarrow> module_hom scale scaleC (\<lambda>x. scaleC c (f x))"
...
lemma module_hom_sum: "(\<And>i. i \<in> I \<Longrightarrow> module_hom scale scaleC (f i)) \<Longrightarrow> (I = {} \<Longrightarrow> module scale \<and> module scaleC) \<Longrightarrow> module_hom scale scaleC (\<lambda>x. \<Sum>i\<in>I. f i x)"
  apply (induction I rule: infinite_finite_induct)
  apply (auto intro!: module_hom_zero module_hom_add)
  apply (meson finite.intros(1) finite_subset module_hom_iff subset_iff)
  apply (meson finite.intros(1) finite_subset module_hom_iff subset_iff)
  by (simp add: module_hom_iff module.scale_right_distrib)

lemma module_hom_id: "module scale \<Longrightarrow> module_hom scale scale id"
  by (simp add: module_hom_iff)

lemma module_hom_eq_on_span:
  assumes lf: "module_hom scale scaleC f"
    and lg: "module_hom scale scaleC g"
    and fg: "\<And>x. x \<in> B \<Longrightarrow> f x = g x"
    and x: "x \<in> module.span scale B"
  shows "f x = g x"
proof -
  let ?h = "\<lambda>x. f x - g x"
  from lf lg interpret module_hom scale scaleC ?h by (rule module_hom_sub)
  have "f x - g x = 0"
    using _ x
    by (rule eq_0_on_span) (simp add: fg)
  then show ?thesis by simp
qed
