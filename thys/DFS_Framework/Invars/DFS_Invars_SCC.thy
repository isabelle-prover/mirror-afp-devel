...
context DFS_invar begin
  ...
  proof (rule ccontr)
    ...
    with scc_root_scc_tree_trancl[OF scc2] `v \<noteq> v'` have "(v',v) \<in> (tree_edges s)\<^sup>+" 
      by simp
    also from scc2 have "v' \<in> scc" "v' \<in> dom (discovered s)" 
      by (simp_all add: scc_root_def)
    with scc_root_scc_tree_trancl[OF scc1] `v\<noteq>v'` have "(v,v') \<in> (tree_edges s)\<^sup>+" 
      by simp
  ...
  proof
    ...
    hence "(v,x) \<in> (Restr E scc)\<^sup>*" using `x \<in> scc`
    ...
    next
      ...
      moreover from `(y,x) \<in> ?E` have "(y,x) \<in> E" "x \<in> scc" by auto
      ultimately have "x \<in> dom (discovered s)"
        using finished_imp_succ_discovered
        by blast
      with `x \<in> scc` show ?case
      ...
      proof (cases "v = x")
        ...
        next
...

          {
            ...
            with scc_disc `v \<noteq> y` * have "\<delta> s v < \<delta> s y" by simp
          ...
          with assms have "(v,x) \<in> ?E\<^sup>*"
            by (simp add: is_scc_connected')
          with `v\<noteq>x` have "(v,x) \<in> ?E\<^sup>+" by (metis rtrancl_eq_or_trancl)
          thus ?thesis using `v\<noteq>x`
          proof (induction)
          ...
          next
            ...
            next
              ...
              with * `v\<noteq>z` have \<delta>z: "\<delta> s v < \<delta> s z" by (simp add: scc_disc)
              

              from * edges_covered finished_no_pending[OF `y \<in> dom (finished s)`] 
                   y_disc have "(y,z) \<in> edges s" by auto
              thus ?thesis
              proof safe
                assume "(y,z) \<in> tree_edges s" with `(v,y) \<in> (tree_edges s)\<^sup>+` show ?thesis ..
              next
