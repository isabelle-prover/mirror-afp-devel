...
lemma top_add_annil[simp]: "\<top> + x = \<top>"
  by (metis add.commute less_eq_def top_ref)
...
lemma iteration_unfoldl': "z\<cdot>x\<^sup>\<infinity>\<cdot>y = z\<cdot>y + z\<cdot>x\<cdot>x\<^sup>\<infinity>\<cdot>y"
  by (metis distrib_left mult.assoc iteration_unfoldl_distr)

lemma iteration_idem[simp]: "x\<^sup>\<infinity>\<cdot>x\<^sup>\<infinity> = x\<^sup>\<infinity>"
  by (metis add_zerol annil isolation mult.assoc iteration_refl iteration_unfoldl_distr star_inductl_var_eq2 star_invol star_sum_unfold sup_id_star1)

lemma iteration_induct: "x\<cdot>x\<^sup>\<infinity> \<le> x\<^sup>\<infinity>\<cdot>x"
  by (metis eq_iff mult.assoc coinduction iteration_unfoldl_distl)
...
lemma iteration_star2[simp]: "x\<^sup>\<star>\<cdot>x\<^sup>\<infinity> = x\<^sup>\<infinity>"
  by (metis add.assoc boffa isolation mult_1_right iteration_idem iteration_unfoldl star_denest star_denest_var_2 star_invol star_slide star_zero)
...
lemma iteration_subdenest: "x\<^sup>\<infinity>\<cdot>y\<^sup>\<infinity> \<le> (x + y)\<^sup>\<infinity>"
  by (metis add.commute mult_isol_var iteration_idem iteration_subdist)
...
lemma iteration_slide_var: "x\<cdot>(y\<cdot>x)\<^sup>\<infinity> \<le> (x\<cdot>y)\<^sup>\<infinity>\<cdot>x"
  by (metis eq_refl iteration_sim mult.assoc)

lemma iteration_prod_unfold: "(y\<cdot>x)\<^sup>\<infinity> = 1 + y\<cdot>(x\<cdot>y)\<^sup>\<infinity>\<cdot>x"
  apply (rule antisym, metis iteration_unfoldl add_iso_var eq_refl iteration_slide_var mult.assoc mult_isol)
  by (metis add_iso_var iteration_refl iteration_slide_var iteration_unfoldr iteration_zero mult.assoc mult_isol mult_isol_var mult_oner)
...
proof -
  ...
  hence "y\<^sup>\<star>\<cdot>(x\<^sup>\<star>\<cdot>y)\<^sup>\<infinity> \<le> 1 + x\<^sup>\<star>\<cdot>y\<cdot>y\<^sup>\<star>\<cdot>(x\<^sup>\<star>\<cdot>y)\<^sup>\<infinity>"
    by (metis less_eq_def add.assoc distrib_left distrib_right mult_1_left mult.assoc star_ref)
  thus ?thesis
    by (metis mult_1_right mult.assoc coinduction star_ref mult_1_left mult_isor add.commute less_eq_def)
qed
...
lemma iteration_denest3: "(x + y)\<^sup>\<infinity> = (y\<^sup>\<star>\<cdot>x)\<^sup>\<infinity>\<cdot>y\<^sup>\<infinity>"
  apply (rule antisym, metis add.commute iteration_denest2 eq_refl coinduction)
  by (metis add.commute iteration_denest iteration_slide mult_isor iteration_iso iteration_ref_star)
...
proof -
  ...
  hence "y\<^sup>\<star>\<cdot>x\<cdot>(x + y)\<^sup>\<infinity> + y\<^sup>\<infinity> \<le> x\<cdot>(x + y)\<^sup>\<infinity> + y\<^sup>\<infinity>"
    by (metis mult_isor mult.assoc iteration_star2 add_iso_var eq_refl)
  thus ?thesis
...
proof -
  ...
  have "y\<^sup>\<star>\<cdot>z\<cdot>x \<le> x\<cdot>y\<^sup>\<star>\<cdot>z"
    by (metis zero_least assms(1) assms(2) independence1 mult.assoc)
  ...
  moreover have "... = (x + y\<^sup>\<star>\<cdot>z)\<^sup>\<infinity>\<cdot>y\<^sup>\<infinity>"
    by (metis add.assoc add.commute calculation iteration_sep2)
