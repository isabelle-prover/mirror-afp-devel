...
A simple example is
@{term "costp p \<le> (3::nat)"} in \secref{sec:specificationI}. *}
...
a function @{text "sort :: nat list \<Rightarrow> nat list"},
...
text {* In a pop-refinement step from @{term spec\<^sub>i} to @{text "spec\<^sub>i\<^sub>+\<^sub>1"},
the two predicates may be equivalent, i.e.\ @{text "spec\<^sub>i\<^sub>+\<^sub>1 = spec\<^sub>i"}.
But the formulation of @{text "spec\<^sub>i\<^sub>+\<^sub>1"} should be ``closer''
to the implementation than the formulation of @{term spec\<^sub>i}.
An example is in \secref{sec:refI:stepI}. *}

text {* When the implication @{text "spec\<^sub>i\<^sub>+\<^sub>1 p \<Longrightarrow> spec\<^sub>i p"} is strict,
...
where @{text "spec\<^sub>i\<^sub>+\<^sub>1 = \<lambda>p. False"},
...
text {* A predicate of the form @{term "spec\<^sub>i \<equiv> p = p\<^sub>0 \<and> \<Phi> p"}
...
text {* The final predicate @{term spec\<^sub>n} expresses
...
(e.g.\ lemma @{text f_rephrased} in \secref{sec:refI:stepIV}),
...
@{text "spec\<^sub>i\<^sub>+\<^sub>1 p \<Longrightarrow> spec\<^sub>i p"} holds at every step. *}
