...
next
  ...
  show \<open>N1 \<Turnstile>\<G> N2\<close> using ground.all_formulas_entailed N1_entails_C
    by (smt (z3) UN_E UN_I ground.entail_set_all_formulas singletonI)
next
  fix N1 N2 N3
  assume
...
lemma Prec_trans:
  ...
  using minimal_element.po assms unfolding po_on_def transp_on_def by (smt (z3) UNIV_I all_wf)
...
proof 
  show \<open>local.Red_F_\<G> N \<subseteq> {C. \<forall>Di\<in>\<G>_F C. Di \<in> Red_F_G (\<G>_Fset N) \<or>
    (\<exists>E\<in>N - local.Red_F_\<G> N. Prec_F_g Di E C \<and> Di \<in> \<G>_F E)}\<close>
  proof 
    fix C
    assume C_in: \<open>C \<in> Red_F_\<G> N\<close>
    have \<open>Di \<in> \<G>_F C \<Longrightarrow> \<forall>E\<in>N - local.Red_F_\<G> N. Prec_F_g Di E C \<longrightarrow> Di \<notin> \<G>_F E \<Longrightarrow> Di \<in> Red_F_G (\<G>_Fset N)\<close> for Di
    proof -
      fix D
      assume D_in: \<open>D \<in> \<G>_F C\<close> and
      ...
      proof (rule ccontr)
        ...
        then have D_in_F: \<open>D \<in> \<G>_F F\<close> unfolding B_def using non_empty
          by (smt (z3) Sup_UNIV Sup_upper UNIV_I contra_subsetD empty_iff empty_subsetI mem_Collect_eq
      ...
      qed
    qed
    then show \<open>C \<in> {C. \<forall>Di\<in>\<G>_F C. Di \<in> Red_F_G (\<G>_Fset N) \<or> (\<exists>E\<in>N - local.Red_F_\<G> N. Prec_F_g Di E C \<and> Di \<in> \<G>_F E)}\<close>
      by clarsimp
  qed
next
  show \<open>{C. \<forall>Di\<in>\<G>_F C. Di \<in> Red_F_G (\<G>_Fset N) \<or> 
    (\<exists>E\<in>N - local.Red_F_\<G> N. Prec_F_g Di E C \<and> Di \<in> \<G>_F E)} \<subseteq> local.Red_F_\<G> N\<close>
  proof
    fix C
    assume \<open>C \<in> {C. \<forall>Di\<in>\<G>_F C. Di \<in> Red_F_G (\<G>_Fset N) \<or> (\<exists>E\<in>N - local.Red_F_\<G> N. Prec_F_g Di E C \<and> Di \<in> \<G>_F E)}\<close>
    then have only_if: \<open>\<forall>D\<in>\<G>_F C. D \<in> Red_F_G (\<G>_Fset N) \<or> (\<exists>E\<in>N - Red_F_\<G> N. Prec_F_g D E C \<and> D \<in> \<G>_F E)\<close>
      by clarsimp
    then show \<open>C \<in> Red_F_\<G> N\<close>
      unfolding Red_F_\<G>_def by auto
  qed
qed
...
proof -
  ...
  then have to_bot: \<open>\<G>_Fset N - Red_F_G (\<G>_Fset N) \<Turnstile>G \<G>_F B\<close>
    using ground.Red_F_Bot Bot_map
      by (smt (z3) cSup_singleton ground.entail_set_all_formulas image_insert image_is_empty subsetCE)
...
lemma Red_F_of_subset_F: \<open>N \<subseteq> N' \<Longrightarrow> Red_F_\<G> N \<subseteq> Red_F_\<G> N'\<close>
  using ground.Red_F_of_subset unfolding Red_F_\<G>_def
  by (smt (verit, ccfv_threshold) Collect_mono \<G>_subset subset_iff)
...
lemma Red_I_of_subset_F: \<open>N \<subseteq> N' \<Longrightarrow> Red_I_\<G> N \<subseteq> Red_I_\<G> N'\<close>
  ...
  by (smt (z3) ground.Red_F_of_subset Un_iff)
  ...
  proof (rule,rule)
    ...
    proof
      ...
      then have \<open>D \<in> Red_F_G (\<G>_Fset (N - N'))\<close>
        ...
        by (smt (z3) DiffE DiffI ground.Red_F_of_subset subsetCE subsetI)
...
proof
  ...
  moreover {
    ...
    have "\<G>_F (concl_of \<iota>) \<subseteq> (\<G>_Fset (N - N') \<union> Red_F_G (\<G>_Fset (N - N')))"
      ...
      by (smt (z3) ground.Red_F_of_Red_F_subset ground.Red_F_of_subset UnCI UnE Un_Diff_cancel2
