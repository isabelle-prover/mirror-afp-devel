(*  Title:      Inductive definition of Hoare logic for total correctness
    ID:         $Id: PsHoareTotal.thy,v 1.3 2007-07-11 10:05:49 stefanberghofer Exp $
...
theorems tvalid_defs = tvalid_def ctvalid_def valids_def cvalids_def valid_defs
...
(*Semi*)
    ...
    apply(rule_tac Q = "\<lambda>z s. body q\<down>pre & (EX cs. ([body q], pre) \<rightarrow>\<^sup>* (c2#cs,s)) & z -c1\<rightarrow>s & c2\<down>s" in thoare.Semi)
     apply(erule thoare.Conseq)
     apply(rule conjI)
      apply clarsimp
