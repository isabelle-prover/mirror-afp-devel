...
proof -
  ...
  from this and `finite ?S` have "finite (\<Union> ?S)" using finite_Union by auto
  from this and `vars_of_cl C = \<Union> ?S` show ?thesis by auto
qed
...
proof -
  ...
  from this and `vars_of_lit L = vars_of_eq e` have "vars_of_lit L = vars_of t \<union> vars_of s" by auto
  ...
  from this and `vars_of_lit L = vars_of t \<union> vars_of s` show ?thesis by auto
qed
...
proof -
  ...
  from `u \<in> subterms_of_lit L` have "vars_of u \<subseteq> vars_of_lit L" using subterm_lit_vars by auto
  from  `L \<in> C` have "vars_of_lit L \<subseteq> vars_of_cl C" using vars_of_cl.simps by auto
  from this and `vars_of u \<subseteq> vars_of_lit L` show ?thesis by auto
qed
...
proof -
  ...
  proof
    assume "L = (Pos e)" 
    from this and `x \<in> subterms_of_lit L` have "x \<in> subterms_of_eq e" by auto
    from `L = (Pos e)` have "(subst_lit L \<sigma>) = (Pos (subst_equation e \<sigma>))"
      by auto
    ...
    from this have "(subst_equation e \<sigma>) = (Eq (subst t \<sigma>)  (subst s \<sigma>))"
      by auto
    from `x \<in> subterms_of_eq e` and `e = (Eq t s)` have "x \<in> subterms_of t \<or> x \<in> subterms_of s" by auto
    then show ?thesis
    proof
      ...
      from this and `(subst_lit L \<sigma>) \<in> (subst_cl C \<sigma>)` 
    ...
    assume "L = (Neg e)" 
    from this and `x \<in> subterms_of_lit L` have "x \<in> subterms_of_eq e" by auto
    from `L = (Neg e)` have "(subst_lit L \<sigma>) = (Neg (subst_equation e \<sigma>))"
      by auto
    obtain t s where "e = (Eq t s)" using equation.exhaust by auto
    from this have "(subst_equation e \<sigma>) = (Eq (subst t \<sigma>)  (subst s \<sigma>))"
      by auto
...
proof (rule ccontr)
    ...
    from `l = Pos e \<or> l = Neg e` have "?l' = Pos ?e' \<or> ?l' = Neg ?e'" by auto    
    from `l \<in> C` have "?l' \<in> ?D" by auto
    from `?l' = Pos ?e' \<or> ?l' = Neg ?e'` and `x \<in> (vars_of_lit ?l')` 
      have "x \<in> (vars_of_eq ?e')" by auto
    from this and `?e' = (Eq ?t' ?s')` have "x \<in> (vars_of ?t' \<union> vars_of ?s')" by auto 
    then have i:"\<not>(ground_term ?t') \<or> \<not>(ground_term ?s')" unfolding ground_term_def by auto
    from `vars_of_eq e = vars_of t \<union> vars_of s` and `vars_of_lit l = vars_of_eq e` and
      `vars_of_lit l \<subseteq> ?V` have "vars_of t \<subseteq> ?V" and "vars_of s \<subseteq> ?V" by auto
    from `vars_of t  \<subseteq> ?V` and `ground_on ?V \<sigma>` have  "ground_on (vars_of t) \<sigma>" 
      unfolding ground_on_def by auto
    then have ii:"ground_term ?t'" using ground_instance by auto
    from `vars_of s  \<subseteq> ?V` and `ground_on ?V \<sigma>` have  "ground_on (vars_of s) \<sigma>" 
      unfolding ground_on_def by auto
...
proof (rule ccontr)
  ...
  from this obtain x where "x \<in> vars_of_cl C" and "\<not> ground_term (subst (Var x) \<sigma>)"
    unfolding ground_on_def by auto
  from `\<not> ground_term (subst (Var x) \<sigma>)` obtain y where 
    "y \<in> vars_of (subst (Var x) \<sigma>)" unfolding ground_term_def by auto
  from `x \<in> vars_of_cl C` obtain L where "L \<in> C" and "x \<in> vars_of_lit L" by auto
  from `x \<in> vars_of_lit L` obtain e where "L = Pos e \<or> L = Neg e" and "x \<in> vars_of_eq e"
    by (metis vars_of_lit.elims) 
  from `x \<in> vars_of_eq e` obtain t s where "e = (Eq t s)" and "x \<in> vars_of t \<union> vars_of s"
  ...
  proof
    ...
    from `x \<in> vars_of t`  i 
      have "vars_of (subst (Var x) \<sigma>) \<subseteq> vars_of (subst t \<sigma>)" 
      by auto
    from this and `y \<in> vars_of (subst (Var x) \<sigma>)` `e = (Eq t s)` show ?thesis by auto
  next
    ...
    from `x \<in> vars_of s` i 
      have "vars_of (subst (Var x) \<sigma>) \<subseteq> vars_of (subst s \<sigma>)" 
      by auto
    from this and `y \<in> vars_of (subst (Var x) \<sigma>)` `e = (Eq t s)` show ?thesis by auto
  qed
  from this and `L = Pos e \<or> L = Neg e` have "y \<in> vars_of_lit (subst_lit L \<sigma>)"
    by auto
  from this and `L \<in> C` have "y \<in> vars_of_cl (subst_cl C \<sigma>)" by auto
...
proof -
  ...
  let ?D = "(subst_cl C \<sigma>)"
  from `ground_on ?V \<sigma>` have "(ground_clause ?D)" using ground_substs_yield_ground_clause [of C \<sigma>] by auto
...
proof - 
    ...
    from this and `y \<in> C` have "vars_of t \<subseteq> vars_of_cl C" by auto 
    ...
    from `y = Pos (Eq t s) \<or> y = Neg (Eq t s)` have "vars_of s \<subseteq> vars_of_lit y" by auto 
    from this and `y \<in> C` have "vars_of s \<subseteq> vars_of_cl C" by auto 
    ...
    from `subst s \<sigma> = s` and `subst t \<sigma> = t` and `y = Pos (Eq t s) \<or> y = Neg (Eq t s)`
...
proof 
  ...
  proof
    ...
    from assms(1) and `y \<in> C` and `x = subst_lit y \<sigma>` 
      have "x = y" using substs_preserve_ground_lit by auto
    from this and `y \<in> C` show "x \<in> C" by auto
  qed
next
  ...
  proof 
    ...
    from assms(1) and `x \<in> C` have "x = subst_lit x \<sigma>" 
      using substs_preserve_ground_lit [of C x] by auto
    from this and `x \<in> C` show "x \<in> subst_cl C \<sigma>" by auto
  qed
qed
...
proof -
  ...
  from this and `l = Pos e \<or> l = Neg e` show ?thesis by auto
qed
...
proof (rule ccontr)
  ...
  then obtain L where "L \<in> C" and "subst_lit L \<sigma> \<noteq> subst_lit L \<eta>"
    by force
  from assms(1) and `subst_lit L \<sigma> \<noteq> subst_lit L \<eta>` 
...
proof -
  ...
  from this and `coincide_on \<sigma> \<eta> (vars_of_eq e)` have "coincide_on \<sigma> \<eta> (vars_of t)" 
    unfolding coincide_on_def by auto
  from this have "subst t \<sigma> = subst t \<eta>" using coincide_on_term by auto
  from `e = Eq t s`  have "vars_of s \<subseteq> vars_of_eq e" by simp
  from this and `coincide_on \<sigma> \<eta> (vars_of_eq e)` have "coincide_on \<sigma> \<eta> (vars_of s)" 
    unfolding coincide_on_def by auto
  from this have "subst s \<sigma> = subst s \<eta>" using coincide_on_term by auto
  from `subst t \<sigma> = subst t \<eta>` 
    and `subst s \<sigma> = subst s \<eta>` 
    and `e = Eq t s` show ?thesis by auto
qed
...
proof -
  ...
  from this and `coincide_on \<sigma> \<eta> (vars_of_lit l)` have "coincide_on \<sigma> \<eta> (vars_of_eq e)" 
    unfolding coincide_on_def by auto
...
proof (rule ccontr)
  ...
  from this and `subst_lit L \<sigma> \<noteq> subst_lit L \<eta>` 
...
proof (rule ccontr)
  ...
  from `validate_clause_set I S` and assms(1) `\<not> validate_clause I C` show False  by auto
qed
...
proof (rule ccontr)
  ...
  from `ground_clause (subst_cl (subst_cl C \<sigma>) \<eta>)` 
    have ii: "ground_clause (subst_cl C (comp \<sigma> \<eta>))" 
    using composition_of_substs_cl by metis
  from i and ii have "\<not>validate_clause I C" by auto
  from `\<not>validate_clause I C` and `validate_clause I C` show False by blast
qed
...
proof -
  ...
  from `e = Eq t s`  have "vars_of s \<subseteq> vars_of_eq e" by simp
  from this and `equivalent_on \<sigma> \<eta> (vars_of_eq e) I` have "equivalent_on \<sigma> \<eta> (vars_of s) I" 
    unfolding equivalent_on_def by auto
  from this assms(2) have "I (subst s \<sigma>) (subst s \<eta>)" 
    using equivalent_on_term unfolding fo_interpretation_def by auto
  from assms(2) `I (subst t \<sigma>) (subst t \<eta>)` 
    and `I (subst s \<sigma>) (subst s \<eta>)` 
...
proof -
  obtain e where "l = Pos e \<or> l = Neg e" using literal.exhaust by auto 
  then have "vars_of_eq e \<subseteq> vars_of_lit l" by auto 
  from this and `equivalent_on \<sigma> \<eta> (vars_of_lit l) I` have "equivalent_on \<sigma> \<eta> (vars_of_eq e) I" 
    unfolding equivalent_on_def by auto
...
proof (rule ccontr)
  ...
  from this assms(2) and `(validate_ground_lit I (subst_lit L \<sigma>)) 
    \<noteq> (validate_ground_lit I (subst_lit L \<eta>))` 
