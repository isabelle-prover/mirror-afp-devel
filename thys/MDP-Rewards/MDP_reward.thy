...
lemma abs_r_le_r\<^sub>M: "\<bar>r sa\<bar> \<le> r\<^sub>M"
  using bounded_norm_le_SUP_norm r_bounded r\<^sub>M_def
  by fastforce
...
subsection \<open>Reward over a Trace\<close>
...
proof -
  have "(\<Sum>i. l ^ i * \<bar>r (t !! i)\<bar>) \<le> (\<Sum>i. l ^ i * r\<^sub>M)"
    by (auto simp: mult_left_mono abs_r_le_r\<^sub>M intro: suminf_le)
  thus ?thesis
    by (auto intro!: order_trans[OF summable_rabs])
qed

lemma abs_\<nu>_trace_fin_le: "\<bar>\<nu>_trace_fin t N\<bar> \<le> (\<Sum>i < N. l^i * r\<^sub>M)"
proof -
  have "\<bar>\<nu>_trace_fin t N\<bar> \<le> (\<Sum>i < N. \<bar>l ^ i * r (t !! i)\<bar>)"
    by blast
  also have "\<dots> \<le> (\<Sum>i < N. l ^ i * r\<^sub>M)"
    by (auto intro: sum_mono simp: mult_left_mono abs_r_le_r\<^sub>M)
  finally show ?thesis .
qed
...
subsection \<open>Integrals over Rewards\<close>
...
proof -
  have "integrable (measure_pmf P) (\<lambda>a. \<bar>r (s, a)\<bar>)" for P
    using abs_r_le_r\<^sub>M
    by (fastforce intro!: measure_pmf.integrable_const_bound)
  thus ?thesis
    by (auto simp: abs_r_le_r\<^sub>M intro: measure_pmf.integral_le_const)
qed
...
lemma \<nu>_fin_eq_Pn: "\<nu>_fin n s = (\<Sum>i<n. l^i * measure_pmf.expectation (Pn' p s i) r)"
  by (induction n; simp add: Pn'_eq_\<T> integral_distr)

definition "\<nu> s = lim (\<lambda>n. \<nu>_fin n s)"

lemma \<nu>_eq_lim: "\<nu> s = lim (\<lambda>n. \<nu>_fin n s)"
  using \<nu>_def.

lemma \<nu>_eq_\<nu>_trace: "\<nu> s = \<integral>t. \<nu>_trace t \<partial>\<T> p s"
proof -
  have "(\<lambda>n. \<nu>_fin n s) \<longlonglongrightarrow> \<integral>t. \<nu>_trace t \<partial>\<T> p s"
    unfolding \<nu>_fin_def
  ...
  next
    ...
    thus "AE x in \<T> p s. norm (\<nu>_trace_fin x N) \<le> (\<Sum>N. l ^ N * r\<^sub>M)" for N
      using abs_\<nu>_trace_fin_le order_trans
      by fastforce
  qed auto
  thus ?thesis
...
lemma abs_\<nu>_le: "\<bar>\<nu> s\<bar> \<le> (\<Sum>i. l^i * r\<^sub>M)"
  using abs_\<nu>_trace_le \<nu>_eq_\<nu>_trace integrable_\<nu>_trace
  by (auto intro!: prob_space.integral_le_const order_trans[OF integral_abs_bound])

lemma \<nu>_le: "\<nu> s \<le> (\<Sum>i. l^i * r\<^sub>M)"
  by (auto intro: abs_\<nu>_le abs_le_D1)

lemma \<nu>_eq_Pn: "\<nu> s = (\<Sum>i. l^i * measure_pmf.expectation (Pn' p s i) r)"
  by (simp add: \<nu>_fin_eq_Pn \<nu>_eq_lim suminf_eq_lim)

(* 6.1.2 in Puterman *)
lemma \<nu>_bfun: "\<nu> \<in> bfun"
  by (auto intro!: abs_\<nu>_le)

lemma \<nu>_fin_bfun: "(\<lambda>s. \<nu>_fin N s) \<in> bfun"
  by (auto intro!: abs_\<nu>_fin_le)

lift_definition \<nu>\<^sub>b :: "'s \<Rightarrow>\<^sub>b real" is \<nu>
  using \<nu>_bfun .
...
subsubsection \<open>Optimal Reward\<close>
...
lemma \<nu>_le_\<nu>_opt [intro]:
  ...
  using abs_\<nu>_le assms
  by (force intro: cSUP_upper intro!: bounded_imp_bdd_above boundedI)

lemma \<nu>_opt_eq_MR: "\<nu>_opt s = (\<Squnion>p \<in> \<Pi>\<^sub>M\<^sub>R. \<nu> (mk_markovian p) s)"
proof (rule antisym)
  ...
  proof (rule cSUP_mono)
    ...
    show "bdd_above ((\<lambda>p. \<nu> (mk_markovian p) s) ` \<Pi>\<^sub>M\<^sub>R)"
      by (auto intro!: boundedI bounded_imp_bdd_above abs_\<nu>_le) 
    show "n \<in> \<Pi>\<^sub>H\<^sub>R \<Longrightarrow> \<exists>m\<in>\<Pi>\<^sub>M\<^sub>R. \<nu> n s \<le> \<nu> (mk_markovian m) s" for n
      using is_\<Pi>\<^sub>M\<^sub>R_as_markovian order_refl mem_Collect_eq \<nu>_as_markovian
      by (subst \<nu>_as_markovian[symmetric]; blast)
  qed
...
lemma \<nu>_opt_bfun: "\<nu>_opt \<in> bfun"
  ...
  by (fastforce intro!: order_trans[OF cSup_abs_le] bfun_normI)
...
lemma abs_r_dec_le: "\<bar>r_dec s\<bar> \<le> r\<^sub>M"
  using expectation_abs_r_le integral_abs_bound order_trans 
  by fast
...
subsection \<open>Push-Forward of a Function Through the MDP\<close>
...
lemma norm_\<P>\<^sub>X_apply[simp]: "norm (\<P>\<^sub>X n x) \<le> norm x"
  using onorm[OF \<P>\<^sub>X_bounded_linear]
  by simp

lemma \<P>\<^sub>X_bound_r: "norm (\<P>\<^sub>X t (r_dec\<^sub>b (p t))) \<le> r\<^sub>M"
  using norm_\<P>\<^sub>X_apply norm_r_dec_le order.trans 
  by blast
...
lemma summable_norm_disc_reward'[simp]:
  shows "summable (\<lambda>t. l^t * norm (\<P>\<^sub>X t (r_dec\<^sub>b (p t))))"
  using \<P>\<^sub>X_bounded_r by auto

lemma summable_disc_reward_\<P>\<^sub>X [simp]:
  shows "summable (\<lambda>t. l^t *\<^sub>R \<P>\<^sub>X t (r_dec\<^sub>b (p t)))"
  using summable_disc_reward \<P>\<^sub>X_bounded_r
  by blast

lemma disc_reward_tendsto: 
  "(\<lambda>n. \<Sum>t<n. l^t *\<^sub>R \<P>\<^sub>X t (r_dec\<^sub>b (p t))) \<longlonglongrightarrow> (\<Sum>t. l^t *\<^sub>R \<P>\<^sub>X t (r_dec\<^sub>b (p t)))"
...
lemma \<P>\<^sub>X_Suc': "\<P>\<^sub>X p (Suc n) v = \<P>\<^sub>X p n (push_exp (K_st (p n)) v)"
...
lemma \<P>\<^sub>X_sconst: "\<P>\<^sub>X (\<lambda>_. p) n = (push_exp (K_st p))^^n"
  by (induction n) (auto simp: \<P>\<^sub>X_Suc' funpow_swap1)

lemma norm_P_n[simp]: "onorm (push_exp (K_st p) ^^ n) = 1"
  using norm_\<P>\<^sub>X[of "\<lambda>_. p"] by (auto simp: \<P>\<^sub>X_sconst)
...
lemma sum_disc_bound: "(\<Sum>x. l^x * apply_bfun f x) \<le> (norm f) /(1-l)"
  using norm_bfun_disc_le' sum_disc_lim 
  by auto
...
lemma \<nu>\<^sub>b_le_opt [intro]: "p \<in> \<Pi>\<^sub>H\<^sub>R \<Longrightarrow> \<nu>\<^sub>b p \<le> \<nu>\<^sub>b_opt"
  using \<nu>_le
  by (fastforce simp: \<nu>\<^sub>b.rep_eq \<nu>\<^sub>b_opt.rep_eq)
...
lemma \<nu>\<^sub>b_opt_eq_MR: "\<nu>\<^sub>b_opt s = (\<Squnion>p \<in> \<Pi>\<^sub>M\<^sub>R. \<nu>\<^sub>b (mk_markovian p) s)"
  by (auto simp: \<nu>_opt_eq_MR \<nu>\<^sub>b.rep_eq \<nu>\<^sub>b_opt.rep_eq)

lemma \<nu>\<^sub>b_as_markovian: "\<nu>\<^sub>b (mk_markovian (as_markovian p (return_pmf s))) s = \<nu>\<^sub>b p s"
  using \<nu>_as_markovian 
  by (auto simp: \<nu>\<^sub>b.rep_eq)

lemma \<nu>_elem: "\<nu> (mk_markovian p) s = (\<Sum>i. l^i * \<P>\<^sub>X p i (r_dec\<^sub>b (p i)) s)"
...
lemma \<nu>\<^sub>b_eq_\<P>\<^sub>X: "\<nu>\<^sub>b (mk_markovian p) = (\<Sum>i. l^i *\<^sub>R \<P>\<^sub>X p i (r_dec\<^sub>b (p i)))"
  by (auto simp: \<nu>_elem \<nu>\<^sub>b.rep_eq suminf_apply_bfun)
...
lemma \<P>\<^sub>X_const: "\<P>\<^sub>X (\<lambda>_. d) n = \<P>\<^sub>1 ^^ n"
  by (simp add: \<P>\<^sub>1.rep_eq \<P>\<^sub>1_pow \<P>\<^sub>X_sconst)

lemma norm_\<P>\<^sub>1 [simp]: "norm \<P>\<^sub>1 = 1"
  by (simp add: norm_blinfun.rep_eq \<P>\<^sub>1.rep_eq)

lemma norm_\<P>\<^sub>1_pow [simp]: "norm (\<P>\<^sub>1 ^^ t) = 1"
  by (simp add: \<P>\<^sub>1_pow norm_blinfun.rep_eq \<P>\<^sub>1.rep_eq)

lemma norm_\<P>\<^sub>1_l_less: "norm (l *\<^sub>R \<P>\<^sub>1) < 1"
  by auto

lemma \<P>\<^sub>1_pos: "0 \<le> u \<Longrightarrow> 0 \<le> \<P>\<^sub>1 u"
  by (auto simp: \<P>\<^sub>1.rep_eq less_eq_bfun_def)

lemma \<P>\<^sub>1_n_pos: "0 \<le> u \<Longrightarrow> 0 \<le> (\<P>\<^sub>1 ^^ n) u"
  by (induction n) (auto simp: \<P>\<^sub>1.rep_eq less_eq_bfun_def)

lemma \<P>\<^sub>1_n_disc_pos: "0 \<le> u \<Longrightarrow> 0 \<le> (l^n *\<^sub>R \<P>\<^sub>1 ^^n) u"
  by (auto simp: \<P>\<^sub>1_n_pos scaleR_nonneg_nonneg blinfun.scaleR_left)

lemma \<P>\<^sub>1_sum_pos: "0 \<le> u \<Longrightarrow> 0 \<le> (\<Sum>t\<le>n. l^t *\<^sub>R (\<P>\<^sub>1 ^^ t)) u"
...
lemma \<P>\<^sub>1_sum_ge: 
  assumes "0 \<le> u" 
  shows "u \<le> (\<Sum>t\<le>n. l^t *\<^sub>R \<P>\<^sub>1 ^^t) u"
  using \<P>\<^sub>1_n_disc_pos[OF assms, of "Suc _"]
  by (induction n) (auto intro: add_increasing2 simp add: blinfun.add_left)

lemma disc_\<P>\<^sub>1_tendsto: "(\<lambda>n. (\<Sum>t\<le>n. l^t *\<^sub>R \<P>\<^sub>1 ^^t)) \<longlonglongrightarrow> (\<Sum>t. l^t *\<^sub>R \<P>\<^sub>1 ^^t)"
  by (fastforce simp: bounded_iff intro: summable_LIMSEQ')

lemma disc_\<P>\<^sub>1_lim: "lim (\<lambda>n. (\<Sum>t\<le>n. l^t *\<^sub>R \<P>\<^sub>1 ^^ t)) = (\<Sum>t. l^t *\<^sub>R \<P>\<^sub>1 ^^t)"
  using limI disc_\<P>\<^sub>1_tendsto
  by blast

lemma convergent_disc_\<P>\<^sub>1: "convergent (\<lambda>n. (\<Sum>t\<le>n. l^t *\<^sub>R \<P>\<^sub>1 ^^t))"
  using convergentI disc_\<P>\<^sub>1_tendsto 
  by blast

lemma \<P>\<^sub>1_suminf_ge: 
  assumes "0 \<le> u" shows "u \<le> (\<Sum>t. l^t *\<^sub>R \<P>\<^sub>1 ^^t) u"
proof -
  have aux: "\<And>x. (\<lambda>n. (\<Sum>t\<le>n. l^t *\<^sub>R \<P>\<^sub>1 ^^t) u x) \<longlonglongrightarrow> (\<Sum>t. l^t *\<^sub>R \<P>\<^sub>1 ^^t) u x"
    using bfun_tendsto_apply_bfun disc_\<P>\<^sub>1_lim lim_blinfun_apply[OF convergent_disc_\<P>\<^sub>1] 
    by fastforce
  have "\<And>n. u \<le> (\<Sum>t\<le>n. l^t *\<^sub>R \<P>\<^sub>1 ^^t) u"
    using \<P>\<^sub>1_sum_ge[OF assms]
    by auto
  thus ?thesis
...
lemma \<P>\<^sub>1_suminf_pos: 
  assumes "0 \<le> u" 
  shows "0 \<le> (\<Sum>t. l^t *\<^sub>R \<P>\<^sub>1 ^^t) u"
  using \<P>\<^sub>1_suminf_ge[of u] assms
  by auto

lemma lemma_6_1_2_b:
  assumes "v \<le> u"
  shows "(\<Sum>t. l^t *\<^sub>R \<P>\<^sub>1 ^^t) v \<le> (\<Sum>t. l^t *\<^sub>R \<P>\<^sub>1 ^^t) u"
proof -
  have "0 \<le> (\<Sum>n. l ^ n *\<^sub>R \<P>\<^sub>1 ^^ n) (u - v)"
    using \<P>\<^sub>1_suminf_pos assms
    by simp
  thus ?thesis
    by (simp add: blinfun.diff_right)
qed

subsection \<open>The Bellman Operator @{term L}\<close>
definition "L v \<equiv> r_dec\<^sub>b d + l *\<^sub>R \<P>\<^sub>1 v"

lemma norm_L_le: "norm (L v) \<le> r\<^sub>M + l * norm v"
  using norm_blinfun[of "\<P>\<^sub>1"] norm_\<P>\<^sub>1 norm_r_dec_le
  by (auto intro!: norm_add_rule_thm mult_left_mono simp: L_def)

lemma abs_L_le: "\<bar>L v s\<bar> \<le> r\<^sub>M + l * norm v"
  using order.trans[OF norm_le_norm_bfun norm_L_le] 
  by auto



lemma \<nu>_stationary: "\<nu>\<^sub>b (mk_stationary d) = (\<Sum>t. l^t *\<^sub>R (\<P>\<^sub>1 ^^ t)) (r_dec\<^sub>b d)"
proof -
  have "\<nu>\<^sub>b (mk_stationary d) = (\<Sum>t. (l ^ t *\<^sub>R (\<P>\<^sub>1 ^^ t)) (r_dec\<^sub>b d))"
    by (simp add: \<nu>\<^sub>b_eq_\<P>\<^sub>X \<P>\<^sub>1_pow \<P>\<^sub>1.rep_eq scaleR_blinfun.rep_eq \<P>\<^sub>X_sconst)
  also have "...  = (\<Sum>t. (l ^ t *\<^sub>R (\<P>\<^sub>1 ^^ t))) (r_dec\<^sub>b d)"
...
proof -
  have s: "summable (\<lambda>t. l^t *\<^sub>R (\<P>\<^sub>X p (Suc t) (r_dec\<^sub>b (p (Suc t)))))"
    using \<P>\<^sub>X_bound_r 
    by (auto intro!: boundedI[of _ r\<^sub>M])
  have 
  ...
  also have 
    ...
    by (auto simp: \<P>\<^sub>X_Suc \<P>\<^sub>1.rep_eq linear_simps)
  also have 
    "\<dots> = L (p 0) (\<nu>\<^sub>b (mk_markovian (\<lambda>n. p (Suc n))))"
    by (simp add: \<nu>\<^sub>b_eq_\<P>\<^sub>X L_def \<P>\<^sub>1.rep_eq bounded_linear.suminf)
  finally show ?thesis .
qed
...
proof -
  have "r_dec\<^sub>b p = (id_blinfun - l *\<^sub>R \<P>\<^sub>1 p) v"
    using assms
    by (auto simp: eq_diff_eq L_def blinfun.diff_left blinfun.scaleR_left)
  hence "v = (\<Sum>t. (l *\<^sub>R \<P>\<^sub>1 p)^^t) (r_dec\<^sub>b p)"
    using inv_norm_le'(2)[OF norm_\<P>\<^sub>1_l_less]
    by auto
...
lemma L_\<nu>_fix_iff: "L d v = v \<longleftrightarrow> v = \<nu>\<^sub>b (mk_stationary d)"
  using L_fix_\<nu> L_\<nu>_fix 
  by auto


lemma apply_bfun_bounded_above [simp, intro]: 
...
lemma L_bounded'[simp, intro]: "bounded ((\<lambda>p. L p v s) ` X)"
  by (auto intro: bounded_subset[OF L_bounded])
...
lemma \<L>_bfun: "\<L> v \<in> bfun"
  unfolding \<L>_def 
  using abs_L_le ex_dec
  by (fastforce intro!: cSup_abs_le bfun_normI)
...
lemma L_mono: "u \<le> v \<Longrightarrow> L d u \<le> L d v"
  unfolding L_def
  by (auto intro: scaleR_left_mono)

lemma \<L>\<^sub>b_mono:
  assumes "u \<le> v" shows "\<L>\<^sub>b u \<le> \<L>\<^sub>b v"
  using L_mono[OF assms] ex_dec
  by (fastforce intro!: cSUP_mono simp: \<L>\<^sub>b.rep_eq \<L>_def)

lemma step_mono:
  assumes "\<L>\<^sub>b v \<le> v" "d \<in> D\<^sub>R"
  shows "L d v \<le> v"
  using assms L_le_\<L>\<^sub>b order.trans
  by blast
...
proof -
  have "v s \<le> (\<Squnion>p\<in>D\<^sub>R. L p v s)" for s
    using assms 
    by (auto simp: \<L>\<^sub>b.rep_eq \<L>_def)
  hence "\<exists>d \<in> D\<^sub>R. v s - e < L d v s" for s
    by (subst less_cSUP_iff[symmetric], auto simp: assms add_strict_increasing algebra_simps)
  hence aux: "\<exists>d. d \<in> D\<^sub>R \<and> v s < L d v s + e" for s
    by (simp add: diff_less_eq)
  have "\<exists>d\<in>D\<^sub>R. \<forall>s. v s < L d v s + e"
  proof -
    let ?d = "\<lambda>s. (SOME d. d \<in> D\<^sub>R \<and> v s < L d v s + e) s"
    have "?d s \<subseteq> A s" "v s < L ?d v s + e" for s
      using someI_ex[OF aux]
      by (auto simp: is_dec_def L_def \<P>\<^sub>1.rep_eq K_st_def)
    thus "\<exists>d\<in>D\<^sub>R. \<forall>s. v s < L d v s + e"
      using is_dec_def
      by blast
  qed
  thus ?thesis
    by (fastforce intro: less_imp_le)
qed
...
lemma \<P>\<^sub>X_Suc_n_elem: "\<P>\<^sub>X p n (\<P>\<^sub>1 (p n) v) = \<P>\<^sub>X p (Suc n) v"
  using \<P>\<^sub>X_Suc' \<P>\<^sub>1.rep_eq
  by auto

lift_definition \<nu>\<^sub>b_fin :: "('s, 'a) pol \<Rightarrow> nat \<Rightarrow> 's \<Rightarrow>\<^sub>b real" is \<nu>_fin
  using \<nu>_fin_bfun .
...
proof -
  ...
  thus "(\<lambda>n. (\<P>\<^sub>d p n v)) \<longlonglongrightarrow> 0" for p
    using tendsto_norm_zero_cancel
    by fast
qed

lemma \<P>\<^sub>1_bfun_one [simp]:"\<P>\<^sub>1 p 1 = 1"
  by (auto simp: \<P>\<^sub>1.rep_eq)

lemma \<P>\<^sub>1_pow_bfun_one [simp]: "((\<P>\<^sub>1 p)^^t) 1 = 1"
  by (induction t) auto
...
proof -
  ...
  proof -
    ...
    proof (induction n)
      case 0
      thus ?case
        by (auto simp: \<nu>\<^sub>b_fin_eq_\<P>\<^sub>X)
    next
      case (Suc n)
      have "\<P>\<^sub>X p n (r_dec\<^sub>b (p n)) + l *\<^sub>R (\<P>\<^sub>X p (Suc n) v) \<le> \<P>\<^sub>X p n v"
        using \<P>\<^sub>X_L_le assms that
        by (simp add: \<P>\<^sub>X_Suc_n_elem L_def linear_simps)
    ...
    have 1: "(\<lambda>n. (\<nu>\<^sub>b_fin ?p n + \<P>\<^sub>d p n v) s) \<longlonglongrightarrow> \<nu>\<^sub>b ?p s" for s
      using bfun_tendsto_apply_bfun Limits.tendsto_add[OF \<nu>\<^sub>b_fin_tendsto_\<nu>\<^sub>b \<P>\<^sub>d_lim] 
      by fastforce
    have "\<nu>\<^sub>b ?p s \<le> v s" for s
      using that aux assms
      by (fastforce intro!: lim_mono[OF _ 1, of  _ _ "\<lambda>n. v s"])
    thus ?thesis
      using that by blast
  qed
  thus ?thesis
    using policies_ne
    by (fastforce simp: is_policy_def \<nu>\<^sub>b_opt_eq_MR intro!: cSUP_least)
qed
...
proof -
  have aux: "v s \<le> \<nu>\<^sub>b_opt s + (e/(1-l))" if "e > 0" for s e
  proof -
    obtain d where "d \<in> D\<^sub>R" and hd: "v \<le> L d v + e *\<^sub>R 1"
      using assms step_mono_elem \<open>e > 0\<close> 
      by blast
    ...
    hence "v \<le> ?Pinf (r_dec\<^sub>b d + e *\<^sub>R 1)"
      using inv_norm_le'(2)[of "l *\<^sub>R \<P>\<^sub>1 d"]
      by (auto simp: blincomp_scaleR_right)
    ...
    also have "\<dots> = \<nu>\<^sub>b (mk_stationary d) + e *\<^sub>R (\<Sum>i. (l^i *\<^sub>R ((\<P>\<^sub>1 d^^i))) 1)"
      ...
      by (auto simp: summable_iff_convergent' 
          bounded_linear.suminf[where f = "\<lambda>x. blinfun_apply x 1"])
  ...
  qed
  hence "v s \<le> \<nu>\<^sub>b_opt s + e" if "e > 0" for s e
  proof -
    have "e * (1 - l) > 0"
      by (simp add: \<open>0 < e\<close>)
    thus "v s \<le> \<nu>\<^sub>b_opt s + e"
      using disc_lt_one that aux
...
lemma \<L>_fix_imp_opt:
  ...
  using assms dual_order.antisym[OF \<L>_dec_ge_opt \<L>_inc_le_opt]
  by auto
...
proof -
  ...
  proof -
    ...
    also have "\<dots> = l * (\<Squnion>d \<in> D\<^sub>R. \<P>\<^sub>1 d (v - u) s)"
      using D\<^sub>R_ne bounded_P
      by (fastforce intro: bounded_SUP_mul)
    ...
    proof -
      have "bounded ((\<lambda>x. norm ((\<P>\<^sub>1 x (v - u)) s)) ` D\<^sub>R)"
        using bounded_apply_bfun' bounded_P bounded_apply_blinfun bounded_norm_comp
        by metis
      thus ?thesis
        using D\<^sub>R_ne ex_dec bounded_norm_comp
  ...
  thus ?thesis
    using linear[of "\<L>\<^sub>b u _"]
    by (fastforce intro: dist_bound)
qed
...
proof -
  ...
  proof (cases "L p v s \<ge> L p u s")
    case True
    thus ?thesis
      using aux
      by (auto simp: dist_real_def dist_commute[of u])
  next
    case False
    thus ?thesis
      using aux[of v _ u]
...
lemma is_contraction_L: "is_contraction (L p)"
  unfolding is_contraction_def
  using contraction_L disc_lt_one zero_le_disc 
  by blast

subsubsection \<open>Existence of a Fixpoint of @{const \<L>\<^sub>b}\<close>
lemma \<L>\<^sub>b_conv:
  "\<exists>!v. \<L>\<^sub>b v = v" 
  "(\<lambda>n. (\<L>\<^sub>b ^^ n) v) \<longlonglongrightarrow> (THE v. \<L>\<^sub>b v = v)"
  using banach'[OF is_contraction_\<L>]
  by auto

lemma \<L>\<^sub>b_fix_iff_opt [simp]: "\<L>\<^sub>b v = v \<longleftrightarrow> v = \<nu>\<^sub>b_opt"
  using banach'(1) is_contraction_\<L> \<L>_fix_imp_opt 
  by metis
...
lemma \<L>\<^sub>b_lim: "(\<lambda>n. (\<L>\<^sub>b ^^ n) v) \<longlonglongrightarrow> \<nu>\<^sub>b_opt"
  using \<L>\<^sub>b_conv(2) \<nu>\<^sub>b_opt_fix 
  by presburger
...
lemma thm_6_2_6': "\<nu> p = \<nu>_opt \<longleftrightarrow> \<L>\<^sub>b (\<nu>\<^sub>b p) = \<nu>\<^sub>b p"
  using thm_6_2_6 \<nu>\<^sub>b.rep_eq \<nu>\<^sub>b_opt.rep_eq
  by fastforce
...
lemma \<L>\<^sub>b_imp_\<nu>_improving: 
  ...
  using assms L_le_\<L>\<^sub>b
  by (auto simp: \<nu>_improving_iff assms(2)[symmetric])

lemma \<nu>_improving_alt:
  assumes "d \<in> D\<^sub>R"
  shows "\<nu>_improving v d \<longleftrightarrow> \<L>\<^sub>b v = L d v"
  using \<L>\<^sub>b_imp_\<nu>_improving \<nu>_improving_imp_\<L>\<^sub>b assms 
  by blast
...
theorem conserving_imp_opt[simp]:
  ...
  using L_\<nu>_fix_iff \<nu>_conserving_imp_\<L>\<^sub>b[OF assms]
  by simp
...
proof -
  obtain d where d: "\<nu>_conserving (mk_dec_det d)"
    using assms ex_improving_imp_conserving 
    by auto
  hence "d \<in> D\<^sub>D"
    using \<nu>_conserving_iff is_dec_mk_dec_det_iff 
    by blast
  thus ?thesis
    using \<Pi>\<^sub>M\<^sub>R_imp_policies \<nu>\<^sub>b_le_opt
    by (fastforce intro!: less_eq_bfunD cSup_eq_maximum[where z = "\<nu>\<^sub>b_opt s", symmetric]
        simp: conserving_imp_opt[OF d] image_iff simp del: \<nu>\<^sub>b.rep_eq \<nu>\<^sub>b_opt.rep_eq)
qed
...
proof (subst Bochner_Integration.integral_add)
  show "integrable d (\<lambda>a. r (s, a))"
    using abs_r_le_r\<^sub>M 
    by (fastforce intro!: bounded_integrable simp: bounded_iff)
...
proof -
  have "\<exists>a\<in> A s. L d v s = L\<^sub>a a v s" for s
    ...
    by (intro lemma_4_3_1') 
      (auto intro: bounded_range_subset simp: assms(2)[symmetric] L_eq_L\<^sub>a[symmetric] SUP_step_MR_eq[symmetric])
...
proof (intro antisym cSUP_mono)
  ...
  show "\<exists>m\<in>A s. L (mk_dec_det n) v s \<le> L\<^sub>a m v s" if "n \<in> D\<^sub>D" for n
    using that is_dec_det_def
    by (auto simp: L_eq_L\<^sub>a_det intro: bexI[of _ "n s"])
...
proof (intro antisym)
  ...
  show "(\<Squnion>a\<in>A s. L\<^sub>a a v s) \<le> (\<Squnion>p\<in>{p. set_pmf p \<subseteq> A s}. \<integral>a. L\<^sub>a a v s \<partial>measure_pmf p)"
    unfolding SUP_step_MR_eq[symmetric] SUP_step_det_eq[symmetric]
    using ex_dec_det
...
lemma \<L>_eq_SUP_det: "\<L> v s = (\<Squnion>d \<in> D\<^sub>D. L (mk_dec_det d) v s)"
  unfolding \<L>_def
  using SUP_step_MR_eq SUP_step_det_eq SUP_L\<^sub>a_eq_det
  by auto

lemma \<L>\<^sub>b_eq_SUP_det: "\<L>\<^sub>b v s = (\<Squnion>d \<in> D\<^sub>D. L (mk_dec_det d) v s)"
  using \<L>_eq_SUP_det 
  unfolding \<L>\<^sub>b.rep_eq
  by auto
...
lemma opt_imp_opt_dec_det:
  ...
  shows "\<exists>d \<in> D\<^sub>D. \<nu>\<^sub>b (mk_stationary (mk_dec_det d)) = \<nu>\<^sub>b_opt"
proof -
  ...
  proof -
    ...
    have "\<nu>\<^sub>b_opt s = \<nu>\<^sub>b (mk_markovian ?ps) s"
      using assms \<nu>\<^sub>b_as_markovian
      by metis
    ...
    also have "\<dots> \<le> L (?ps 0) (\<nu>\<^sub>b_opt) s"
      unfolding L_def
      using markovian_suc_le \<P>\<^sub>1_mono
      by (auto intro!: mult_left_mono)
    ...
    have "L (?ps 0) \<nu>\<^sub>b_opt \<le> \<nu>\<^sub>b_opt"
      using \<open>?ps 0 \<in> D\<^sub>R\<close> L_le_\<L>\<^sub>b[of "?ps 0" "\<nu>\<^sub>b_opt"]
      by simp
    thus "L (?ps 0) \<nu>\<^sub>b_opt s = \<nu>\<^sub>b_opt s"
      using \<open>\<nu>\<^sub>b_opt s \<le> (L (?ps 0) \<nu>\<^sub>b_opt) s\<close> 
  ...
  hence "\<exists>d \<in> D\<^sub>D. L (mk_dec_det d) \<nu>\<^sub>b_opt = \<nu>\<^sub>b_opt"
    using \<L>\<^sub>b_sup_att_dec assms(1) \<L>\<^sub>b_opt is_policy_def mem_Collect_eq
    by metis
  thus ?thesis
    using conserving_imp_opt' \<nu>_conserving_alt'
    by blast
qed
...
proof -
  have "\<exists>d \<in> D\<^sub>D. L (mk_dec_det d) v = \<L>\<^sub>b v" for v
  proof -
    have "\<exists>a \<in> A s. L\<^sub>a a v s = \<L>\<^sub>b v s" for s
    ...
    hence "\<exists>d. \<forall>s. d s \<in> A s \<and> L\<^sub>a (d s) v s = \<L>\<^sub>b v s"
      by metis
    hence "\<exists>d \<in> D\<^sub>D. \<forall>s. L (mk_dec_det d) v s = \<L>\<^sub>b v s"
      unfolding  L_def is_dec_det_def mk_dec_det_def
      by (auto simp: K_st_def \<P>\<^sub>1.rep_eq bind_return_pmf)
    thus ?thesis
      using bfun_eqI by blast
  qed
  thus ?thesis
    using assms conserving_imp_opt' \<L>\<^sub>b_opt L_\<nu>_fix_iff
    by metis
qed
...
proof -
  ...
  proof -
    have "bdd_above ((\<lambda>a. L\<^sub>a a v s) ` A s)"
      using L\<^sub>a_le
      by (auto intro!: boundedI bounded_imp_bdd_above)
...
proof -
  ...
  then obtain d where "d \<in> D\<^sub>D" and d: "\<L>\<^sub>b \<nu>\<^sub>b_opt \<le> L (mk_dec_det d) \<nu>\<^sub>b_opt + ((1-l)*eps) *\<^sub>R 1"
    using ex_det_eps[of _ \<nu>\<^sub>b_opt] 
    by auto
  let ?d = "mk_dec_det d"
  ...
  hence "(\<Sum>i. ?lK ^^ i) (\<nu>\<^sub>b_opt - ?lK_opt - ((1-l)*eps) *\<^sub>R 1) \<le> \<nu>\<^sub>b (mk_stationary ?d)"
    using lemma_6_1_2_b suminf_cong
    by (simp add: blincomp_scaleR_right \<nu>_stationary)
  ...
  proof -
    have "(\<Sum>i. ?lK ^^ i) (((1-l)*eps) *\<^sub>R 1) = ((1-l)*eps) *\<^sub>R (\<Sum>i. ?lK^^i) 1"
      using blinfun.scaleR_right 
      by blast
    also have "\<dots> = ((1-l)*eps) *\<^sub>R (\<Sum>i. (?lK^^i) 1) "
      using s 
      by (auto simp: bounded_linear.suminf[of "\<lambda>x. blinfun_apply x 1"])
...
proof (subst le_cSUP_iff, safe)
  fix y
  assume "y < apply_bfun \<nu>\<^sub>b_opt s"
  then obtain eps where 1: " y \<le> apply_bfun \<nu>\<^sub>b_opt s - eps" and "eps > 0"
    by (smt (verit, ccfv_threshold) \<open>y < apply_bfun \<nu>\<^sub>b_opt s\<close>)
  hence "eps / 2 > 0" by auto
  obtain d where "d \<in> D\<^sub>D" and 2: "\<nu>\<^sub>b_opt s \<le> \<nu>\<^sub>b (mk_stationary_det d) s + eps / 2"
    using thm_6_2_11[OF \<open>eps / 2 > 0\<close>]
    by (auto simp: less_eq_bfun_def)
  hence "y < \<nu>\<^sub>b (mk_stationary_det d) s"
    using \<open>eps > 0\<close>
    by (auto simp: diff_less_eq intro: le_less_trans[OF 1] le_less_trans[OF 2]) 
  thus "\<exists>i\<in>D\<^sub>D. y < apply_bfun (\<nu>\<^sub>b (mk_stationary_det i)) s"
    using \<open>d \<in> D\<^sub>D\<close> by blast
next
  show "D\<^sub>D = {} \<Longrightarrow> False"
    using D_det_ne by blast
  show "bdd_above ((\<lambda>d. apply_bfun (\<nu>\<^sub>b (mk_stationary_det d)) s) ` D\<^sub>D)"
    by (auto intro!: bounded_imp_bdd_above boundedI abs_\<nu>_le simp: \<nu>\<^sub>b.rep_eq is_policy_def)
qed
...
lemma \<nu>\<^sub>b_opt_eq_det: "\<nu>\<^sub>b_opt s = (\<Squnion>d \<in> D\<^sub>D. \<nu>\<^sub>b (mk_stationary_det d) s)"
  ...
  by (auto intro!: antisym[OF \<nu>\<^sub>b_opt_le_det] cSUP_least simp add: less_eq_bfun_def)+
...
(* should be a general statement for convergent series? *)
lemma thm_6_3_1_b_aux: "(\<lambda>n. dist ((\<L>\<^sub>b^^n) v) ((\<L>\<^sub>b^^(Suc n)) v)) \<longlonglongrightarrow> 0"
  using \<L>\<^sub>b_lim tendsto_diff tendsto_norm LIMSEQ_Suc
...
locale MDP_act = MDP_att_\<L> A K r l
...
lemma is_opt_act_some: "is_opt_act v s (arb_act (opt_acts v s))"
  ...
  unfolding max_L_ex_def is_opt_act_def
  by auto

lemma some_opt_acts_in_A: "arb_act (opt_acts v s) \<in> A s"
  using is_opt_act_some unfolding is_opt_act_def is_arg_max_def
  by auto
...
lemma \<nu>_improving_opt_acts: "\<nu>_improving v0 (mk_dec_det (\<lambda>s. arb_act (opt_acts (apply_bfun v0) s)))"
  ...
  by (subst \<nu>_improving_alt) (fastforce simp: L_eq_L\<^sub>a_det thm_6_2_10_a_aux' is_dec_det_def)+
...
lemma \<nu>\<^sub>b_fin_Suc[simp]: "\<nu>\<^sub>b_fin (mk_stationary d) (Suc n) = \<nu>\<^sub>b_fin (mk_stationary d) n + ((l *\<^sub>R \<P>\<^sub>1 d)^^ n) (r_dec\<^sub>b d)"
  by (auto simp: \<P>\<^sub>X_sconst \<nu>\<^sub>b_fin.rep_eq \<nu>_fin_eq_\<P>\<^sub>X \<P>\<^sub>1.rep_eq \<P>\<^sub>1_pow blincomp_scaleR_right blinfun.scaleR_left)
...
proof (induction m arbitrary: v)
  ...
  also have "\<dots> = \<nu>\<^sub>b_fin (mk_stationary d) m + ((l *\<^sub>R \<P>\<^sub>1 d) ^^ m) (L d v)"
    using Suc by simp
  also have "\<dots> = \<nu>\<^sub>b_fin (mk_stationary d) (Suc m) + ((l *\<^sub>R \<P>\<^sub>1 d) ^^ m) ((l *\<^sub>R \<P>\<^sub>1 d) v)"
    unfolding L_def by (auto simp: blinfun.bilinear_simps)
  also have "\<dots> = \<nu>\<^sub>b_fin (mk_stationary d) (Suc m) + ((l *\<^sub>R \<P>\<^sub>1 d) ^^ Suc m) v"
    by (auto simp del: blinfunpow.simps simp add: blinfunpow_assoc)
  finally show ?case .
qed simp
...
lemma bounded_disc_\<P>\<^sub>1: "bounded ((\<lambda>x. (((l *\<^sub>R \<P>\<^sub>1 x) ^^ m) v) s) ` X)"
  by (auto simp: \<P>\<^sub>X_const[symmetric] blinfun.bilinear_simps blincomp_scaleR_right simp del: \<P>\<^sub>X_sconst 
...
lemma bounded_disc_\<P>\<^sub>1': "bounded ((\<lambda>x. ((\<P>\<^sub>1 x ^^ m) v) s) ` X)"
  by (auto simp: \<P>\<^sub>X_const[symmetric] simp del: \<P>\<^sub>X_sconst 
      intro!: boundedI[of _  "norm v"] order.trans[OF abs_le_norm_bfun])

lemma L_iter_le_\<L>\<^sub>b: "is_dec d \<Longrightarrow> (L d ^^ n) v \<le> (\<L>\<^sub>b ^^ n) v"
  using order_trans[OF L_mono L_le_\<L>\<^sub>b]
  by (induction n) auto
end
...
lemma arg_max_on_in: "has_arg_max f Q \<Longrightarrow> arg_max_on f Q \<in> Q"
  using has_arg_max_arg_max
  by (auto simp: arg_max_on_def)
...
proof -
  ...
  moreover have "d \<in> D\<^sub>D"
    using Sup_att arg_max_on_in  
    by (auto simp: d_def is_dec_det_def max_L_ex_def)
  ultimately show ?thesis
    by auto
qed

lemma ex_improving_det: "\<exists>d \<in> D\<^sub>D. \<nu>_improving v (mk_dec_det d)"
  using \<nu>_improving_alt ex_opt_det
  by auto
end
