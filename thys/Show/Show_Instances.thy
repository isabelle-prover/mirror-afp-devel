...
header {* Instances of the Show Class for Standard Types *}
...
text {* For several types, we just derive the show function *}

derive "show" bool
derive "show" option
derive "show" sum

text {* The derive-command is not used for @{type unit}, @{type prod}, and numbers:
  for @{type unit} and @{type prod}, we do not want to display ``Unity'' and ``Pair'';
  for @{type nat}, we do not want to display ``Suc (Suc ... Suc (Zero))''; and neither
  @{type int} nor @{type rat} are datatypes. *}
