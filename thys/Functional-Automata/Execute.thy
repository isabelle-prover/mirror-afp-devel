...
unfolding delta.simps Cset.bind_def delta'_def by auto
...
by (auto simp add: fun_eq_iff NA_Abs_def NA_Rep_def split: list.split)
...
unfolding plus'_def RegExp2NA.plus_def by (auto simp add: fun_eq_iff NA_Abs_def NA_Rep_def)
...
by (auto simp add: fun_eq_iff split: prod.split)
...
unfolding next_def DA_Abs_def by (cases A) simp
