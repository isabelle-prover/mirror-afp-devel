...
text \<open> In a production economy we need to specify profit maximisation. \<close>
...
lemma util_fun_def_holds: "i \<in> agents \<Longrightarrow> x \<succeq>[Pr[i]] y \<longleftrightarrow> U[i] x \<ge> U[i] y"
  by (meson agent_props all_larger_zero_in_csset eucl_ordinal_utility_def ordinal_utility_def)
...
proof -
  ...
  have "X \<in> budget_constraint (calculate_value P) consumption_set (poe_wealth P i Y)"
  ...
  proof
    ...
    obtain y where
      ...
      using \<open>0 < e\<close> all_larger_zero_in_csset lns by blast
    ...
    qed
    then show False
      using bgt_leq i_agts y_props(2) util_fun_def_holds by blast
  qed
  then show ?thesis
...
proof (rule ccontr)
  ...
  proof(rule ccontr)
    ...
    show False
      by (metis cpos all_larger_zero_in_csset all_prefered_are_more_expensive
          inner_commute obt_witness(1) obt_witness(2) assms(3))
  qed
  ...
  have xp_l_yp: "(\<Sum>i\<in>agents. X' i) \<le> (\<Sum>i\<in>agents. \<E>[i]) + (\<Sum>f\<in>firms. Y' f)"
    using is_feasible feasible_private_ownership_def by blast
  ...
  have yprime_in_prod_set: "\<forall>j \<in> firms. Y' j \<in> production_sets j"
    using xprime_pareto by (simp add: feasible_private_ownership_def)
...
proof -
  have "pareto_optimal \<X> \<Y>"
    by (meson assms equil first_welfare_theorem_priv_own)
...
corollary first_welfare_thm_monotone:
  ...
  assumes "competitive_equilibrium Price \<X> \<Y>"
  shows "pareto_optimal \<X> \<Y>"
  using all_larger_zero_in_csset assms(2) assms(3) assms(4) 
    first_welfare_theorem_priv_own unbounded_above_mono_imp_lns by blast
