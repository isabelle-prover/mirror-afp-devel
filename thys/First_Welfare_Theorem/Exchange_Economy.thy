...
definition budget_constraint :: "real \<Rightarrow> 'a set"
  where
    "budget_constraint W = {x \<in> consumption_set. Price \<bullet> x \<le> W}"
  ...
  where
    "comp_equilib_endow P X E \<equiv>
      ...
      (\<forall>i \<in> agents. X i \<in> arg_max_set U[i]
        (budget_constraint (Price \<bullet> \<E>[i])))"
...
lemma budget_constraint_is_feasible:
  assumes "i \<in> agents"
  assumes "X \<in> (budget_constraint (Price \<bullet> \<E>[i]))"
  shows "Price \<bullet> X \<le> Price \<bullet> \<E>[i]"
  using budget_constraint_def assms by blast

lemma arg_max_set_therefore_no_better :
  assumes "i \<in> agents"
  assumes "x \<in> arg_max_set U[i] (budget_constraint (Price \<bullet> \<E>[i]))"
  shows "U[i] y > U[i] x \<longrightarrow> y \<notin> budget_constraint (Price \<bullet> \<E>[i])"
...
lemma argmax_entire_budget :
  ...
  assumes "X \<in> arg_max_set U[i] (budget_constraint (Price \<bullet> \<E>[i]))"
  shows "Price \<bullet> X = Price \<bullet> \<E>[i]"
proof -
  have leq : "(Price \<bullet> X) \<le> (Price \<bullet> \<E>[i])"
  proof-
    have "X \<in> budget_constraint (Price \<bullet> \<E>[i])"
      using argmax_sol_in_s[of X "U[i]" "budget_constraint (Price \<bullet> \<E>[i])"]
        assms by auto
    thus ?thesis
      using assms(1) budget_constraint_is_feasible by blast
  qed
  have not_less: "\<not>( Price \<bullet> X < Price \<bullet> \<E>[i])"
  proof
    assume cpos: "(Price \<bullet> X) < (Price \<bullet> \<E>[i])"
    define lesS where "lesS = {x. Price \<bullet> x < Price \<bullet> \<E>[i]}"
    obtain e where
    ...
    hence "Y \<in> budget_constraint (Price \<bullet> \<E>[i])"
      using budget_constraint_def e lesS_def
        less_eq_real_def Y by blast
    thus False
...
lemma pref_more_expensive:
  ...
  assumes "x \<in> arg_max_set U[i] (budget_constraint (Price \<bullet> \<E>[i]))"
  assumes "U[i] y > U[i] x"
  shows "y \<bullet> Price > Price \<bullet> \<E>[i]"
proof (rule ccontr)
  assume cpos :  "\<not>(y \<bullet> Price > Price \<bullet> \<E>[i])"
  then have xp_leq : "y \<bullet> Price \<le> Price \<bullet>  \<E>[i]"
    by auto
  hence "x \<in> budget_constraint (Price \<bullet> \<E>[i])"
    using argmax_sol_in_s[of x " U[i]" "budget_constraint (Price \<bullet> \<E>[i])"]
      assms by auto
  hence xp_in: "y \<in> budget_constraint (Price \<bullet> \<E>[i])"
  proof -
    have "Price \<bullet> y \<le> Price \<bullet> \<E>[i]"
      by (metis xp_leq inner_commute)
    then show ?thesis
      using budget_constraint_def consumption_set_member by blast
  qed
  hence "y \<succ>[Pref i] x"
    using arg_max_set_therefore_no_better assms by blast
  hence "y \<succ>[Pref i] x \<and> y \<in> budget_constraint (Price \<bullet> \<E>[i])"
    using xp_in by blast
  hence "x \<notin> arg_max_set U[i] (budget_constraint (Price \<bullet> \<E>[i]))"
...
lemma same_util_is_equal_or_more_expensive:
  ...
  shows "y \<bullet> Price \<ge> Price \<bullet> \<E>[i]"
proof-
  have not_in: "y \<notin> arg_max_set U[i] (budget_constraint (Price \<bullet> \<E>[i]))
    \<Longrightarrow> y \<bullet> Price > Price \<bullet> \<E>[i]"
  proof-
    assume "y \<notin> arg_max_set U[i] (budget_constraint (Price \<bullet> \<E>[i]))"
    then have "y \<notin> budget_constraint (Price \<bullet> \<E>[i])"
...
lemma all_in_argmax_same_price:
  ...
  assumes "local_nonsatiation consumption_set Pr[i]"
  assumes "x \<in> arg_max_set U[i] (budget_constraint (Price \<bullet> \<E>[i]))"
    and  "y \<in> arg_max_set U[i] (budget_constraint (Price \<bullet> \<E>[i]))"
...
lemma walras_law_per_agent :
  ...
  assumes "comp_equilib_endow Price X \<E>"
  shows "\<forall>i \<in> agents. Price \<bullet> X i = Price \<bullet> \<E>[i]"
...
lemma walras_law:
  assumes "\<And>i. i \<in> agents \<Longrightarrow> local_nonsatiation consumption_set Pr[i]"
  assumes "comp_equilib_endow Price X \<E>"
  shows "(\<Sum>i\<in>agents. Price \<bullet> (X i)) - (\<Sum>i\<in>agents. Price \<bullet> \<E>[i]) = 0"
...
proof (rule ccontr)
  ...
  proof
    ...
    proof -
      have x_in_agmx : "(\<X> i) \<in> arg_max_set U[i] (budget_constraint (Price \<bullet> \<E>[i]))"
...
corollary first_welfare_exch_thm_monot:
  ...
  assumes "comp_equilib_endow Price \<X> \<E>"
  shows "pareto_optimal_endow \<X> \<E>"
  by (meson assms(2) assms(3) exchange_economy.consumption_set_member
