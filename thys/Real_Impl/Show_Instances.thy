...
header {* Instances of the Show Class *}
...
definition "shows_prec d (b::bool) = (if b then shows_string ''True'' else shows_string ''False'')"
lemma assoc_bool:
  "shows_prec d (b::bool) r @ s = shows_prec d b (r @ s)"
  by (unfold shows_prec_bool_def shows_string_def,simp)

standard_shows_list assoc_bool
end
...
instantiation nat :: "show" begin
  ...
  where
    ...
    (if n = 6 then ''6'' else
    (if n = 7 then ''7'' else
    (if n = 8 then ''8'' else ''9'')))))))))"

  fun shows_nat where "shows_nat (n::nat) = (
    if n < 10 then shows_string (digit2string n)
              else shows_nat (n div 10) \<circ> shows_string (digit2string (n mod 10))
  )"
  definition "shows_prec (d::nat) (n::nat) \<equiv> shows_nat n"
