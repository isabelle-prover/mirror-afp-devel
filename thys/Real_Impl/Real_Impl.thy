...
proof (transfer, unfold Let_def, clarsimp)
  ...
  next
    ...
    with sqrt_int_pos[OF Cons] have "sqrt (real_of_int (abs (a * b))) = of_int s" 
      by (metis abs_of_nonneg of_int_mult real_eq_of_int real_of_int_abs real_sqrt_abs2)
  ...
  proof (rule arg_cong[of _ _ "\<lambda> x. ((z1 * n2) + x) div (n1 * n2)"])
    ...
    proof (cases "z2 * n1 \<ge> 0")
      ...
      have radd: "r_add = sqrt (of_int (z2 * n1 * (z2 * n1) * int b))" 
        ...
        by (metis ab_semigroup_mult_class.mult_ac(1) of_int_mult real_eq_of_int real_sqrt_mult_distrib2)
      show ?thesis unfolding radd sqrt_int_floor_pos[OF ge_int] real_eq_of_int using True by simp
    next
      case False
      ...
      have "r_add = - sqrt (of_int (z2 * n1 * (z2 * n1) * int b))"
        unfolding r_add_def z2n1_def using sqrt_sqrt[OF ge] 
        by (metis minus_minus minus_mult_commute minus_mult_right of_int_minus of_int_mult real_of_int_def real_sqrt_minus real_sqrt_mult_distrib2 z2n1_def)
      hence radd: "floor r_add = - ceiling (sqrt (of_int (z2 * n1 * (z2 * n1) * int b)))"
        by (metis ceiling_def minus_minus)
      show ?thesis unfolding radd sqrt_int_ceiling_pos[OF ge_int] real_eq_of_int using False by simp
    qed
  qed
qed
