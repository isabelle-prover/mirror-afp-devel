...
lemma set_of_distrib_left:
  ...
  apply transfer
  apply (auto simp: Let_def add_mono distrib_left distrib_right)
  apply (metis add_mono min.cobounded1 min.left_commute)
  apply (metis add_mono min.cobounded1 min.left_commute)
  apply (metis add_mono min.assoc min.cobounded2)
  apply (meson add_mono_thms_linordered_semiring(1) dual_order.trans max.cobounded1 max.cobounded2)
  apply (meson add_mono_thms_linordered_semiring(1) dual_order.trans max.cobounded1 max.cobounded2)
  apply (meson add_mono_thms_linordered_semiring(1) dual_order.trans max.cobounded1 max.cobounded2)
  done
...
lemma mult_bounds_enclose_zero1:
  ...
  subgoal by (metis (no_types, hide_lams) that eq_iff min_le_iff_disj mult_zero_left
        mult_zero_right zero_le_mult_iff)
