...
if
  ...
  "welltyped_imgu_on (clause.vars D) \<V> t\<^sub>1 t\<^sub>2 \<mu>"
...
if 
  ...
  "\<not> is_Var t\<^sub>1"
  "welltyped_imgu_on (clause.vars (E \<cdot> \<rho>\<^sub>1) \<union> clause.vars (D \<cdot> \<rho>\<^sub>2)) \<V>\<^sub>3 (t\<^sub>1 \<cdot>t \<rho>\<^sub>1) (t\<^sub>2 \<cdot>t \<rho>\<^sub>2) \<mu>"
  ...
  "\<And>\<tau> \<tau>'. welltyped \<V>\<^sub>2 t\<^sub>2 \<tau> \<Longrightarrow> welltyped \<V>\<^sub>2 t\<^sub>2' \<tau>' \<Longrightarrow> \<tau> = \<tau>'"
...
where
  superposition'I:
   "infinite_variables_per_type \<V>\<^sub>1 \<Longrightarrow>
    ...
    (\<And>\<tau> \<tau>'. welltyped \<V>\<^sub>2 t\<^sub>2 \<tau> \<Longrightarrow> welltyped \<V>\<^sub>2 t\<^sub>2' \<tau>' \<Longrightarrow> \<tau> = \<tau>') \<Longrightarrow>
...
proof (cases E D C rule: neg_superposition.cases)
  ...
  then show ?thesis
    using
      superpositionI[of Neg \<V>\<^sub>1 \<V>\<^sub>2 \<rho>\<^sub>1 \<rho>\<^sub>2 E D t\<^sub>1 \<V>\<^sub>3 t\<^sub>2 \<mu> c\<^sub>1 t\<^sub>1' t\<^sub>2' l\<^sub>1 l\<^sub>2 E' D' C]
      literals_distinct(2)
    by blast
qed
