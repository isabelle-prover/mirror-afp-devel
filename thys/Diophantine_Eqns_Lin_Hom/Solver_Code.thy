    ...
    let
      ...
      val _ =
        File.write (Path.append build_dir \<^path>\<open>HLDE.hs\<close>)
          (#content (Generated_Files.get_file thy \<^path_binding>\<open>code/generated/HLDE.hs\<close>));

      (*compile*)
      val compile_rc =
        Isabelle_System.bash
          ("cd " ^ File.bash_path build_dir ^ " && " ^
            File.bash_path \<^path>\<open>$ISABELLE_GHC\<close> ^ " Main.hs");
      val () =
