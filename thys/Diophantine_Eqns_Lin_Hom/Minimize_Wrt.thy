...
lemma "sorted_wrt Q xs \<Longrightarrow> sorted_wrt Q (minimize_wrt P xs)"
  by (induct xs)
    (auto simp: sorted_wrt_Cons sorted_wrt_filter dest: subsetD [OF minimize_wrt_subset])
...
lemma in_minimize_wrt_False:
  ...
  shows False
  using assms
  by (induct xs) (auto dest: subsetD [OF minimize_wrt_subset] simp: sorted_wrt_Cons)
