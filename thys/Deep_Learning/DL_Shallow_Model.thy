...
shows "cprank_max1 (tensors_from_net (insert_weights (shallow_model' Z M N) w) $ y)"
  using assms proof (induction N arbitrary:w)
  case 0
  then have "input_sizes (insert_weights (shallow_model' Z M 0) w) = [M]"
...
next
  ...
  then show ?case unfolding shallow_model'.simps(2) insert_weights.simps tensors_from_net.simps
    using cprank_max1_IH cprank_max1_0 cprank_max1_prod index_component_mult y_le_0 y_le_IH by fastforce
qed


lemma cprank_shallow_model:
assumes "remove_weights m = shallow_model Y Z M N"
assumes "y < Y"
...
proof -
  ...
  proof -
    ...
    have 0:"tensors_from_net m $ y = Tensor_Plus.listsum (input_sizes (shallow_model' Z M N))
      (map (\<lambda>j. (extract_matrix w Y Z)  $$ (y, j) \<cdot> (tensors_from_net (insert_weights (shallow_model' Z M N) (\<lambda>i. w (i + Y * Z)))) $ j) [0..<Z])"
      unfolding `m = insert_weights (shallow_model Y Z M N) w` shallow_model_def insert_weights.simps tensors_from_net.simps
    ...
    proof -
      ...
      have 1:"cprank_max1 (tensors_from_net (insert_weights (shallow_model' Z M N) (\<lambda>i. w (i + Y * Z))) $ j)"
        using \<open>j < Z\<close> output_size_shallow_model' cprank_max1_shallow_model' by auto
      then have "cprank_max1 (extract_matrix w Y Z $$ (y, j) \<cdot> tensors_from_net (insert_weights (shallow_model' Z M N) (\<lambda>i. w (i + Y * Z))) $ j)"
