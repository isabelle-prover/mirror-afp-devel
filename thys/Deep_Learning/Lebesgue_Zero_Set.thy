    ...
    proof (induction m arbitrary:a rule:poly_mapping_induct)
      ...
      next
        ...
        then show ?thesis unfolding insertion_single apply measurable 
          using vars_monom_single_cases single False insert_subset lessThan_iff `a\<noteq>0` by fastforce
      qed
    next
      case (sum m1 m2 x i)
...
text \<open>Use the IH:\<close>
  ...
  proof - 
    ...
    proof (rule subset_antisym;rule subsetI)
      ...
      then show "x \<in> {f\<in>space (lborel_f n). insertion f (p_fix1 x1) = 0}" using `x \<in> space (lborel_f n)` by blast
    next
  ...
  proof - 
    ...
    then have "emeasure (lborel_f n) {f\<in>space (lborel_f n). insertion f (p_fix1 x1) = 0} = 0"
      ...
      using Suc.IH[OF `p_fix1 x1 \<noteq> 0`] `\<And>x1. vars (p_fix1 x1) \<subseteq> {..<n}` by auto
  ...
  have ae_zero: "AE x1 in lborel. emeasure (lborel_f n) {x \<in> space (lborel_f n). x(n := x1) \<in> A} \<le> 0"
    apply (rule AE_I'[OF `N \<in> null_sets lborel`])
    using `\<And>x1. x1\<notin>N \<Longrightarrow> emeasure (lborel_f n) {x\<in>space (lborel_f n). x(n := x1) \<in> A} = 0`
    by force
  ...
  have "\<And>x1. {x\<in>space (lborel_f n). x(n := x1) \<in> A} \<in> sets (lborel_f n)" 
    using `\<And>x1. x1\<in>N \<Longrightarrow> {x\<in>space (lborel_f n). x(n := x1) \<in> A} \<in> sets (lborel_f n)`
    `\<And>x1. x1\<notin>N \<Longrightarrow> {x\<in>space (lborel_f n). x(n := x1) \<in> A} \<in> sets (lborel_f n)` by auto
  have "emeasure (lborel_f (Suc n)) A = \<integral>\<^sup>+ y. emeasure (lborel_f n) {x \<in> space (lborel_f n). x(n := y) \<in> A} \<partial>lborel"
    using emeasure_lborel_f_Suc[OF `A \<in> sets (lborel_f (Suc n))`]
     `\<And>x1. {x\<in>space (lborel_f n). x(n := x1) \<in> A} \<in> sets (lborel_f n)` by blast
  also have "... = 0"
