...
proof -
  ...
  have "vectorspace.basis K (vs (span S)) S"
    using vectorspace.dim_gen_is_basis[OF `vectorspace K (span_vs S)` `finite S` `S \<subseteq> carrier (span_vs S)`
...
proof -
  ...
  then have lin_indpt: "LinearCombinations.module.lin_indpt K (span_vs S) U"
    using module.span_li_not_depend(2)[OF `U \<subseteq> span S`] \<open>lin_indpt U\<close> assms(1) span_is_submodule by blast
  have "span U = span S"
  proof (rule ccontr)
    assume "span U \<noteq> span S"
    have "span U \<subseteq> span S" using span_is_monotone `U\<subseteq>S` by metis
  ...
  then have span:"LinearCombinations.module.span K (vs (span S)) U = span S"
    using module.span_li_not_depend[OF `U \<subseteq> span S`]
...
proof -
  ...
  proof (induction "S-B" arbitrary:B rule: finite_psubset_induct)
    ...
    next
      ...
      then show ?thesis using psubset(2)[OF `S - B' \<subset> S - B` `P B'`] using `B \<subset> B'` by fast
    qed
  qed
qed
...
proof -
  have "finite (b1 \<union> (b2-b1))" "finite (b2 \<union> (b1-b2))"
    ...
    using `finite (b1 \<union> b2)` `b1 \<union> b2 \<subseteq> carrier M` `a2\<in> (b2\<rightarrow>carrier R)` by auto
  ...
  then have "x1 = lincomb (\<lambda>v. if v \<in> b1 then a1 v else \<zero>) (b1 \<union> b2)"
    ...
    using lincomb_union2[OF `finite (b1 \<union> (b2-b1))` `b1 \<union> (b2 - b1) \<subseteq> carrier M` `b1 \<inter> (b2 - b1) = {}` `a1\<in> (b1\<rightarrow>carrier R)` `(\<lambda>b. \<zero>\<^bsub>R\<^esub>) \<in> b2 - b1 \<rightarrow> carrier R`]
          lincomb_union2[OF `finite (b2 \<union> (b1-b2))` `b2 \<union> (b1-b2) \<subseteq> carrier M` `b2 \<inter> (b1 - b2) = {}` `a2\<in> (b2\<rightarrow>carrier R)` `(\<lambda>b. \<zero>\<^bsub>R\<^esub>) \<in> b1 - b2 \<rightarrow> carrier R`]
  ...
  show "lincomb (\<lambda>v. (\<lambda>v. if v \<in> b1 then a1 v else \<zero>) v \<oplus> (\<lambda>v. if v \<in> b2 then a2 v else \<zero>) v) (b1 \<union> b2) = x"
    using lincomb_sum[OF `finite (b1 \<union> b2)` `b1 \<union> b2 \<subseteq> carrier M`
    `(\<lambda>v. if v \<in> b1 then a1 v else \<zero>) \<in> (b1 \<union> b2) \<rightarrow> carrier R` `(\<lambda>v. if v \<in> b2 then a2 v else \<zero>) \<in> (b1 \<union> b2) \<rightarrow> carrier R`]
...
proof -
  ...
  have "W1 \<subseteq> carrier V" "W2 \<subseteq> carrier V" using `subspace K W1 V` `subspace K W2 V` subspace_def submodule_def by metis+
  have "b1 \<subseteq> carrier V"
    using `vectorspace.basis K (vs W1) b1` \<open>vectorspace K (vs W1)\<close> vectorspace.basis_def
    `W1 \<subseteq> carrier V` by fastforce
  have "b2 \<subseteq> carrier V"
    using `vectorspace.basis K (vs W2) b2` \<open>vectorspace K (vs W2)\<close> vectorspace.basis_def
    `W2 \<subseteq> carrier V` by fastforce
  have "finite (b1 \<union> b2)" "b1 \<union> b2 \<subseteq> carrier V"
    by (simp_all add: \<open>finite b1\<close> \<open>finite b2\<close> `b2 \<subseteq> carrier V` `b1 \<subseteq> carrier V`)
  ...
  proof (rule subsetI)
    ...
    obtain x1 x2 where  "x1 \<in> W1" "x2 \<in> W2" "x = x1 \<oplus>\<^bsub>V\<^esub> x2"
      using imageE[OF `x \<in> subspace_sum W1 W2`[unfolded submodule_sum_def]]
    ...
    obtain a1 where "x1 = lincomb a1 b1" "a1\<in> (b1\<rightarrow>carrier K)"
      using `span b1 = W1` finite_span[OF `finite b1` `b1 \<subseteq> carrier V`] \<open>x1 \<in> W1\<close> by auto
    obtain a2 where "x2 = lincomb a2 b2" "a2\<in> (b2\<rightarrow>carrier K)"
      using `span b2 = W2` finite_span[OF `finite b2` `b2 \<subseteq> carrier V`] \<open>x2 \<in> W2\<close> by auto
    obtain a where "x = lincomb a (b1 \<union> b2)" using lincomb_add[OF `finite (b1 \<union> b2)` `b1 \<union> b2 \<subseteq> carrier V`
      `x1 = lincomb a1 b1` `a1\<in> (b1\<rightarrow>carrier K)`  `x2 = lincomb a2 b2` `a2\<in> (b2\<rightarrow>carrier K)` `x = x1 \<oplus>\<^bsub>V\<^esub> x2`] by blast
    then show "x \<in> span (b1 \<union> b2)" using finite_span[OF `finite (b1 \<union> b2)` `(b1 \<union> b2) \<subseteq> carrier V`]
  ...
  proof (rule subset_antisym)
    ...
    show "subspace_sum W1 W2 \<subseteq> LinearCombinations.module.span K (vs (subspace_sum W1 W2)) (b1\<union>b2)"
      using module.span_li_not_depend(1)[OF `b1\<union>b2 \<subseteq> subspace_sum W1 W2` `submodule K (subspace_sum W1 W2) V`]
    ...
    show "subspace_sum W1 W2 \<supseteq> LinearCombinations.module.span K (vs (subspace_sum W1 W2)) (b1\<union>b2)"
      using `b1\<union>b2 \<subseteq> subspace_sum W1 W2` by (metis (full_types) LinearCombinations.module.span_is_subset2
  ...
  then have "vectorspace.dim K (vs (subspace_sum W1 W2)) \<le> card (b1 \<union> b2)"
    using vectorspace.gen_ge_dim[OF `vectorspace K (vs (subspace_sum W1 W2))` `finite (b1 \<union> b2)`]
...
shows "submodule R X (md W)"
  ...
  using `X \<subseteq> W` submodule_is_module[OF `submodule R W M`] using `submodule R X M`[unfolded submodule_def] by auto
...
proof -
  ...
  show ?thesis using li_le_dim(2)[OF `fin_dim` `b \<subseteq> carrier V` `lin_indpt b`]
...
proof -
  ...
  proof (rule subset_antisym)
    have 0:"LinearCombinations.module.span K (vs (subspace_sum W1 W2)) (b1 \<union> b2) = span (b1 \<union> b2)"
      using span_li_not_depend(1)[OF `b1 \<union> b2 \<subseteq> subspace_sum W1 W2`] sum_is_subspace[OF assms(1) assms(2)] by auto
    ...
    proof
      ...
      then show "x \<in> span (b1 \<union> b2)" unfolding `x = x1 \<oplus>\<^bsub>V\<^esub> x2`
