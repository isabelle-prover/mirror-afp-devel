...
lemma continuous_map_metric_limitin:
  "continuous_map (mtopology_of X) (mtopology_of Y) f \<longleftrightarrow> f \<in> mspace X \<rightarrow> mspace Y \<and> (\<forall>xn x. limitin (mtopology_of X) xn x sequentially \<longrightarrow> limitin (mtopology_of Y) (\<lambda>n. f (xn n)) (f x) sequentially)"
proof -
  interpret m1: Metric_space "mspace X" "mdist X"
    by simp
  interpret m2: Metric_space "mspace Y" "mdist Y"
    by simp
  have "continuous_map m1.mtopology m2.mtopology f \<longleftrightarrow> f \<in> mspace X \<rightarrow> mspace Y \<and> (\<forall>xn x. limitin (mtopology_of X) xn x sequentially \<longrightarrow> limitin (mtopology_of Y) (\<lambda>n. f (xn n)) (f x) sequentially)"
    unfolding mtopology_of_def
  proof safe
    fix xn x
    assume "continuous_map m1.mtopology m2.mtopology f"
    then have h: "f ` mspace X \<subseteq> mspace Y" "\<forall>a\<in>mspace X. \<forall>\<epsilon>>0. \<exists>\<delta>>0. \<forall>x. x \<in> mspace X \<and> mdist X a x < \<delta> \<longrightarrow> mdist Y (f a) (f x) < \<epsilon>"
      by(auto simp:  Metric_space.metric_continuous_map[OF m1.Metric_space_axioms m2.Metric_space_axioms])
    assume h': "limitin m1.mtopology xn x sequentially"
    then have x:"x \<in> mspace X"
      by (simp add: m1.limitin_mspace)
    show "limitin m2.mtopology (\<lambda>n. f (xn n)) (f x) sequentially"
      unfolding m2.limit_metric_sequentially
    proof safe
      fix e :: real
      assume e:"0 < e"
      with h(2) x obtain \<delta> where \<delta>: "\<delta> > 0" "\<And>y. y \<in> mspace X \<Longrightarrow> mdist X x y < \<delta> \<Longrightarrow> mdist Y (f x) (f y) < e"
        by fastforce
      then obtain N where N: "\<And>n. n \<ge> N \<Longrightarrow> xn n \<in> mspace X" "\<And>n. n \<ge> N \<Longrightarrow> mdist X (xn n) x < \<delta>"
        using h' by(fastforce simp add: m1.limit_metric_sequentially)
      show "\<exists>N. \<forall>n\<ge>N. f (xn n) \<in> mspace Y \<and> mdist Y (f (xn n)) (f x) < e"
        using N h(1) by(auto intro!: exI[where x=N] simp: mdist_commute \<delta>(2))
    qed(use x h in auto)
  next
    fix a and e :: real
    assume h:"f \<in> mspace X \<rightarrow> mspace Y" " \<forall>xn x. limitin m1.mtopology xn x sequentially \<longrightarrow> limitin m2.mtopology (\<lambda>n. f (xn n)) (f x) sequentially"
    show "continuous_map m1.mtopology m2.mtopology f"
      unfolding continuous_map_def
    proof safe
      fix U
      assume U:"openin m2.mtopology U"
      show "openin m1.mtopology {x \<in> topspace m1.mtopology. f x \<in> U}"
        unfolding m1.openin_mtopology2 m1.topspace_mtopology
      proof safe
        fix xn x
        assume xnx:"limitin m1.mtopology xn x sequentially" "x \<in> mspace X" "f x \<in> U"
        then have "limitin m2.mtopology (\<lambda>n. f (xn n)) (f x) sequentially"
          using h by auto
        with U xnx obtain N where N:"\<And>n. n\<ge>N \<Longrightarrow> f (xn n) \<in> U"
          by(fastforce simp: m2.openin_mtopology2)
        from xnx obtain N' where N': "\<And>n. n \<ge> N' \<Longrightarrow> xn n \<in> mspace X"
          by (meson lessI m1.limit_metric_sequentially of_nat_0_less_iff)
        show "\<exists>N. \<forall>n\<ge>N. xn n \<in> {x \<in> mspace X. f x \<in> U}"
          by(auto intro!: exI[where x="max N N'"] N N')
      qed
    qed(use h in auto)
...
lemma upper_semicontinuous_map_limsup_iff:
  ...
  shows "upper_semicontinuous_map (mtopology_of m) f \<longleftrightarrow> (\<forall>xn x. limitin (mtopology_of m) xn x sequentially \<longrightarrow> limsup (\<lambda>n. f (xn n)) \<le> f x)"
  unfolding upper_semicontinuous_map_def
proof safe
  fix xn x
  assume h:"\<forall>a. openin (mtopology_of m) {x \<in> topspace (mtopology_of m). f x < a}" "limitin (mtopology_of m) xn x sequentially"
  ...
  proof safe
    ...
    proof(rule ccontr)
      ...
      interpret Metric_space "mspace m" "mdist m"
        by simp
      have [simp]: "mtopology = mtopology_of m"
        by (simp add: mtopology_of_def)
      show False
        by (metis (mono_tags, lifting) limitin_subsequence[OF \<open>strict_mono a\<close> h(2)] openin_mtopology2[THEN iffD1,simplified,OF h(1)[rule_format,of y]] \<open>f x < y\<close> h(2) hc limitin_def linorder_not_less mem_Collect_eq) 
    qed
  qed
next
  fix a
  assume h:" \<forall>xn x. limitin (mtopology_of m) xn x sequentially \<longrightarrow> limsup (\<lambda>n. f (xn n)) \<le> f x"
  interpret Metric_space "mspace m" "mdist m"
    by simp
  have [simp]: "mtopology = mtopology_of m"
    by (simp add: mtopology_of_def)
  have "openin mtopology {x \<in> topspace (mtopology_of m). f x < a}"
    unfolding openin_mtopology2 topspace_mtopology_of
  proof safe
    fix x xn
    assume h':"limitin mtopology xn x sequentially" "x \<in> mspace m" "f x < a"
    ...
    obtain N' where N': "\<And>n. n\<ge>N' \<Longrightarrow> xn n \<in> mspace m"
      by (meson h' limitin_sequentially open_in_mspace)
    thus "\<exists>N. \<forall>n\<ge>N. xn n \<in> {x \<in> mspace m. f x < a}"
      using h'(3) N by(auto  intro!: exI[where x="max N N'"])
  qed
  thus "openin (mtopology_of m) {x \<in> topspace (mtopology_of m). f x < a}"
    by simp
qed
...
lemma upper_semicontinuous_map_limsup_real:
  fixes f :: "'a \<Rightarrow> real"
  shows "upper_semicontinuous_map (mtopology_of m) f \<longleftrightarrow> (\<forall>xn x. limitin (mtopology_of m) xn x sequentially \<longrightarrow> limsup (\<lambda>n. f (xn n)) \<le> f x)"
  unfolding upper_semicontinuous_map_real_iff upper_semicontinuous_map_limsup_iff by simp

lemma lower_semicontinuous_map_liminf_iff:
  fixes f :: "'a \<Rightarrow> ('b :: {complete_linorder,linorder_topology})"
  shows "lower_semicontinuous_map (mtopology_of m) f \<longleftrightarrow> (\<forall>xn x. limitin (mtopology_of m) xn x sequentially \<longrightarrow> f x \<le> liminf (\<lambda>n. f (xn n)))"
  unfolding lower_semicontinuous_map_def
proof safe
  fix xn x
  assume h:"\<forall>a. openin (mtopology_of m) {x \<in> topspace (mtopology_of m). a < f x}" "limitin (mtopology_of m) xn x sequentially"
...
next
  fix a
  assume h:" \<forall>xn x. limitin (mtopology_of m) xn x sequentially \<longrightarrow> f x \<le> liminf (\<lambda>n. f (xn n))"
  interpret Metric_space "mspace m" "mdist m"
    by simp
  have [simp]: "mtopology = mtopology_of m"
    by (simp add: mtopology_of_def)
  have "openin mtopology {x \<in> topspace (mtopology_of m). a < f x}"
    unfolding openin_mtopology2 topspace_mtopology_of
  proof safe
    fix x xn
    assume h':"limitin mtopology xn x sequentially" "x \<in> mspace m" "f x > a"
...
lemma lower_semicontinuous_map_liminf_real:
  fixes f :: "'a \<Rightarrow> real"
  shows "lower_semicontinuous_map (mtopology_of m) f \<longleftrightarrow> (\<forall>xn x. limitin (mtopology_of m) xn x sequentially \<longrightarrow> f x \<le> liminf (\<lambda>n. f (xn n)))"
  unfolding lower_semicontinuous_map_real_iff lower_semicontinuous_map_liminf_iff by simp
