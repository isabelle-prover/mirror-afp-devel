...
partial_function(tailrec) subresultant_prs_main where
  ...
  let
     g' = divide_poly pmod ((-1) ^ (d1 + 1) * lf * (c ^ d1))
...
proof -
  ...
  proof (rule arg_cong[of _ _ det])
    ...
    proof (rule mat_eqI, goal_cases)
      ...
      show ?case
        by (auto simp add: icoeff_def d mat_map_def mat_index_mat(1)[OF ij] hom_distribs)
    qed (auto simp: d)
  qed
qed
...
proof (intro ext)
  ...
  next
    ...
    show ?thesis unfolding resultant_impl_generic_def resultant_swap[of f g] using False by auto
  qed
qed
end
