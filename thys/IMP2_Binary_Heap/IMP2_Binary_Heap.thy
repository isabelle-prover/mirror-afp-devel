...
text \<open>In this submission imperative versions of the following array-based binary minimum heap
      ...
      \<^cite>\<open>"IMP2-AFP"\<close> is used. Here arrays are modeled by \<open>int \<Rightarrow> int\<close> functions. The imperative
      implementations are iterative versions of the partly recursive algorithms described in
      \<^cite>\<open>"MS"\<close> and \<^cite>\<open>"CLRS"\<close>.
...
text \<open>The next step is to show that the first element of the heap is always the ``smallest'' 
      ...
      numbers found in \<^cite>\<open>"Str_Ind"\<close>.\<close>
...
text \<open>The following imperative heap functions are based on \<^cite>\<open>"MS"\<close> and \<^cite>\<open>"CLRS"\<close>. All functions,
      that are recursive in these books, are iterative in the following implementations. The 
      function definitions are done with IMP2 \<^cite>\<open>"IMP2-AFP"\<close>. From now on the heaps only contain 
