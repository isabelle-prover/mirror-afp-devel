...
abbrevs  
 f_neg="\<^bold>\<not>"  f_or="\<^bold>\<or>" f_and="\<^bold>\<and>" f_impl="\<^bold>\<rightarrow>"  f_imp="\<^bold>\<leftarrow>"  mequ="\<^bold>\<leftrightarrow>" f_all="\<^bold>\<forall>" f_exi="\<^bold>\<exists>"
 f_all2="(\<^bold>\<forall>x.)" f_exi2="(\<^bold>\<exists>x.)" f_kleeneeq="\<cong>" f_primeq="\<simeq>" f_comp2="(\<cdot>)" f_comp="\<cdot>"
...
subsection\<open>AxiomsSet1\<close>
 ...
 locale AxiomsSet1 =
  ...
  begin
   ...
   lemma assumes "\<exists>x. \<^bold>\<not>(E x)" shows True nitpick [satisfy] oops  --{*Consistency*}
   lemma assumes "(\<exists>x. \<^bold>\<not>(E x)) \<and> (\<exists>x. (E x))" shows True nitpick [satisfy] oops  --{*Consistency*}
...
subsection\<open>AxiomsSet7 (technically flawed)\<close>
 ...
 locale AxiomsSet7 = 
   ...
   *)
   lemma InconsistencyAutomatic: "(\<exists>x. \<^bold>\<not>(E x)) \<^bold>\<rightarrow> False" by (metis A1 A2a A3a) --{*Inconsistency*}
...
subsection\<open>AxiomsSet8 (algebraic reading, still technically flawed)\<close>
 ...
 context AxiomsSet8
  begin
   lemma S1: "E(dom x) \<^bold>\<rightarrow> E x" nitpick oops --{*Nitpick finds a countermodel*} 
   lemma S2: "E(cod y) \<^bold>\<rightarrow> E y" nitpick oops --{*Nitpick finds a countermodel*} 
   lemma S3: "E(x\<cdot>y) \<^bold>\<leftrightarrow> dom x \<simeq> cod y" nitpick oops --{*Nitpick finds a countermodel*}  
   lemma S4: "x\<cdot>(y\<cdot>z) \<cong> (x\<cdot>y)\<cdot>z" nitpick oops --{*Nitpick finds a countermodel*} 
   lemma S5: "(cod y)\<cdot>y \<cong> y"  nitpick oops --{*Nitpick finds a countermodel*} 
   lemma S6: "x\<cdot>(dom x) \<cong> x"  nitpick oops --{*Nitpick finds a countermodel*} 
  end
