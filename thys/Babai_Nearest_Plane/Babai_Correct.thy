...
text
\<open>This theory contains the proof of correctness of the algorithm.
The main theorem is ``theorem babai-correct'', under the locale ``babai-with-assms''.
To use the theorem, one needs to show that lattice, the vectors in the lattice basis, and the target
vector all have the same dimension, that the lattice basis vectors are linearly independent, and that 
the lattice basis is LLL-weakly-reduced for some parameter $\alpha\geq 4/3$. \<close>
...
lemma babai_to_help:                          
  shows "s n = babai_of_LLL (uminus t) (RAT M)" 
  using babai_def babai.s_def babai_of_LLL.simps babai_axioms by force
...
proof(cases "closest_distance_sq = 0")
  ...
  proof(rule ccontr)
    ...
    proof-
      have "0<1/(4*\<alpha>^(n-1))" using non_trivial alpha by force
    ...
    ultimately show False
      using gram_schmidt_fs_lin_indpt.weakly_reduced_imp_short_vector[of n "(RAT M)" \<alpha> "w-v"]
            M_locale_2 reduced alpha
      unfolding gs.reduced_def L_def by force
  qed
next
  case False
...
sublocale vec_space "TYPE(rat)" n .

definition "mat_M_inv \<equiv> 
    (if (invertible_mat mat_M) then SOME B. (inverts_mat B mat_M) \<and> (inverts_mat mat_M B) else (0\<^sub>m n n))"

lemma carrier_M:
  shows "mat_M \<in> carrier_mat n n" 
  using basis unfolding gs.lin_indpt_list_def mat_M_def by auto

lemma mat_M_inv_is_inv:
  shows "invertible_mat mat_M" 
proof-
  have "vec_space.rank n mat_M = n"
    using vec_space.lin_indpt_full_rank[of mat_M n n] carrier_M basis mat_M_def
    unfolding cof_vec_space.lin_indpt_list_def
    by auto
  then have "det mat_M \<noteq> 0" using vec_space.det_rank_iff[of mat_M n] carrier_M  by fastforce
  then show "invertible_mat mat_M" using invertible_det[of mat_M n] carrier_M  by fastforce
qed
...
text \<open>This shifts the output, which is of the form $v-t$, with $v \in L$, back to $v$. \<close>
abbreviation vL where "vL \<equiv>  map_vec int_of_rat ( babai_out + t)"

lemma shifted_babai_correct: (*Shows that the shifted output is close to t*)
  shows "vL \<in> L 
  \<and> real_of_rat (sq_norm (map_vec rat_of_int (vL) - t)) \<le>
   real_of_rat ((rat_of_int n)*\<alpha>^n)*closest_distance_sq"
proof-
  let ?vC = "(babai_of_LLL (-t) (RAT M))"
  have t_carr: "t \<in> carrier_vec n" using babai_axioms unfolding babai_def by fastforce
...
text \<open>Here we prove correctness of the full algorithm, which starts with a non-reduced basis and outputs a vector in L close to t.\<close>
locale full_babai_with_assms =
  fixes target::"rat vec"
  fixes n::nat
  fixes fs_init::"int vec list"
  fixes \<alpha>::rat
  assumes non_triv:"0<n"
  assumes t_dim:"dim_vec target = n"
  assumes LLL_assms:"LLL.LLL_with_assms n n fs_init \<alpha>"
begin

sublocale vec_space "TYPE(rat)" n .

abbreviation B::real where "B \<equiv> (real_of_rat (\<alpha>^n) * (n))  * babai.closest_distance_sq fs_init target"
abbreviation out where "out \<equiv> (full_babai fs_init target \<alpha>)"

lemma LLL_output_babai_assms:
  shows "babai_with_assms (map of_int_hom.vec_hom (LLL_Impl.reduce_basis \<alpha> fs_init)) target \<alpha>"
proof-
  define fs where "fs = (LLL_Impl.reduce_basis \<alpha> fs_init)"
  have 1:"LLL.reduced n \<alpha> fs n \<and> LLL.lin_indep n fs \<and> length(fs) = n"
    using LLL_with_assms.reduce_basis[of n n fs_init "\<alpha>" fs] LLL_assms unfolding fs_def by blast
  then have 2:"LLL.weakly_reduced n \<alpha> fs n" unfolding gram_schmidt_fs.reduced_def by simp
  moreover have "babai (map of_int_hom.vec_hom (reduce_basis \<alpha> fs_init)) target"
    using 1 unfolding babai_def fs_def t_dim by fastforce
  moreover have "gram_schmidt_fs.weakly_reduced n
     (map of_int_hom.vec_hom (map of_int_hom.vec_hom fs)) \<alpha>
     n"
    using 2 by force
  moreover have "4/3 \<le> \<alpha>" using LLL_assms unfolding LLL_with_assms_def by simp
  ultimately show ?thesis 
    using LLL_assms 1 t_dim non_triv
    unfolding babai_with_assms_def babai_with_assms_axioms_def fs_def by auto
qed


lemma full_babai_correct:
  shows "real_of_rat (sq_norm ((map_vec rat_of_int out) - target)) 
      \<le> real_of_rat (\<alpha>) ^ n * real n * babai.closest_distance_sq fs_init target \<and> 
        out \<in> vec_module.lattice_of n fs_init"
proof-
  define fs where "fs = (LLL_Impl.reduce_basis \<alpha> fs_init)"
  have bab:"babai_with_assms fs target \<alpha>"
    using LLL_output_babai_assms unfolding fs_def by simp
  then have l:"length fs = n" 
    unfolding babai_with_assms_def babai_with_assms_axioms_def babai_def
    using t_dim by simp
  have "out = map_vec int_of_rat
          (babai_of_LLL
            (- target)
            (LLL.RAT fs)
          + target)" 
    unfolding fs_def using full_babai.simps[of fs_init target \<alpha>] by argo
  ...
  then have "out \<in> vec_module.lattice_of n fs \<and>
    ...
    \<le> real_of_rat (rat_of_int (int n) * \<alpha> ^ n) *
       babai.closest_distance_sq fs target"
    using babai_with_assms.shifted_babai_correct[of fs target \<alpha>] bab l 
    unfolding LLL.L_def
    by algebra
  moreover have lattices:"vec_module.lattice_of n fs = vec_module.lattice_of n fs_init"
    using LLL_with_assms.reduce_basis(1)[of n n fs_init "\<alpha>" fs] LLL_assms l unfolding fs_def LLL.L_def
    by argo
  moreover have "babai.closest_distance_sq fs target = babai.closest_distance_sq fs_init target"
  proof-
    have lf:"length fs_init = n"
      using LLL_assms unfolding LLL_with_assms_def by blast
    then have "babai fs target \<and> babai fs_init target"
      using t_dim l LLL_assms unfolding babai_def by argo
    then show ?thesis 
      using babai.closest_distance_sq_def[of fs target] 
            babai.closest_distance_sq_def[of fs_init target] 
            lattices l lf
      unfolding LLL.L_def
      by presburger
  qed
  moreover have "real_of_rat (rat_of_int (int n) * \<alpha> ^ n) = real_of_rat (\<alpha>) ^ n * real n"
    by (simp add: of_rat_divide of_rat_mult of_rat_power)
  ultimately show ?thesis by force
qed
end

text \<open>Here we prove correctness of the full algorithm, starting with a target vector of TYPE(int).\<close>
lemma full_babai_correct_int_target:
  assumes "full_babai_with_assms (map_vec rat_of_int target) n fs_init \<alpha>"
  shows "real_of_int (sq_norm ( (full_babai fs_init (map_vec rat_of_int target) \<alpha>) - target)) 
      \<le> (real_of_rat(\<alpha>)^n * (n)) * babai.closest_distance_sq fs_init (map_vec rat_of_int target) \<and> 
        (full_babai fs_init (map_vec rat_of_int target) \<alpha>) \<in> vec_module.lattice_of n fs_init"
proof-
  let ?t = "map_vec rat_of_int target"
  have "real_of_rat (sq_norm ( of_int_hom.vec_hom (full_babai fs_init ?t \<alpha>) - ?t)) 
      \<le> (real_of_rat (\<alpha>)^n * (n))* babai.closest_distance_sq fs_init ?t \<and> 
        (full_babai fs_init ?t \<alpha>) \<in> vec_module.lattice_of n fs_init"
    using full_babai_with_assms.full_babai_correct[of ?t n fs_init] assms by blast
  moreover have "real_of_rat (sq_norm ( of_int_hom.vec_hom (full_babai fs_init ?t \<alpha>) - ?t)) 
                = real_of_int (sq_norm ( (full_babai fs_init ?t \<alpha>) - target))"
  proof-
    have "of_int_hom.vec_hom (full_babai fs_init ?t \<alpha>) - ?t = map_vec rat_of_int ((full_babai fs_init ?t \<alpha>) - target)"
      by fastforce
    moreover have "sq_norm (map_vec rat_of_int ((full_babai fs_init ?t \<alpha>) - target)) = rat_of_int (sq_norm ((full_babai fs_init ?t \<alpha>) - target))"
      using sq_norm_of_int[of "((full_babai fs_init ?t \<alpha>) - target)"] by blast
    ultimately have "real_of_rat (sq_norm ( of_int_hom.vec_hom (full_babai fs_init ?t \<alpha>) - ?t)) = real_of_rat (rat_of_int (sq_norm ((full_babai fs_init ?t \<alpha>) - target)))" 
      by force
    also have "real_of_rat (rat_of_int (sq_norm ((full_babai fs_init ?t \<alpha>) - target))) = real_of_int (sq_norm ((full_babai fs_init ?t \<alpha>) - target))" by simp
    finally show ?thesis .
  qed
  ultimately show ?thesis by auto
qed

text \<open>The literature typically states the main result using $2^n$ as the approximation constant, rather than $\alpha^n * n$. Here we show that $\alpha^n * n$ is stronger for $\alpha = 4/3$.\<close>
lemma clean_bound:
  fixes n::nat
  shows "(4/3)^n * n \<le> 2 ^ n"
proof(induct n)
  case 0
  then show ?case by simp
next
  ...
  next
    { assume ind:"?SN>2" 
      ...
      ultimately have 3:"real_of_rat ((rat_of_int ?SN)*(4/3)^?SN) =  (n*(?SN/n)) * (real_of_rat ((4/3)^n*(4/3)))"
        by (metis of_int_of_nat_eq of_rat_mult of_rat_of_nat_eq)
      also have "(n*(?SN/n)) * real_of_rat ((4/3)^n*(4/3)) = real_of_rat ((rat_of_int n) * (4/3)^n) * ((?SN/n) *(real_of_rat (4/3)))"
        by (simp add: \<open>real (Suc n) = real n * (real (Suc n) / real n)\<close> mult_of_int_commute of_rat_divide of_rat_mult)
      finally have "real_of_rat ((rat_of_int ?SN)*(4/3)^?SN) = real_of_rat ((rat_of_int n) * (4/3)^n) * ((?SN/n) *(real_of_rat (4/3)))"
        by presburger
      then have "real_of_rat ((rat_of_int ?SN)*(4/3)^?SN) = real_of_rat ((rat_of_int n) * (4/3)^n) * ((?SN/n) *(real_of_rat (4/3)))"
        by argo
      moreover have "((?SN/n) *(real_of_rat (4/3)))\<le>2"
      proof-
        ...
        moreover have "4/3 = real_of_rat (4/3)" using of_rat_divide 
          by (metis of_rat_numeral_eq)
        ultimately have "(real_of_rat (4/3))*(?SN/n)\<le> 2" by algebra
        then show ?thesis by argo
      qed
      moreover have 
        "0\<le>real_of_rat (rat_of_int (int n) * (4/3) ^ n)" by force
      moreover have "0\<le>(?SN/n)*(real_of_rat (4/3))" by simp
      moreover have "(4 / 3) ^ n * real n * (real (Suc n) / real n * real_of_rat (4 / 3)) = real_of_rat (rat_of_int (int n) * (4 / 3) ^ n) * (real (Suc n) / real n * real_of_rat (4 / 3))"
        by (simp add: of_rat_divide of_rat_mult of_rat_power)
      ultimately have "real_of_rat ((rat_of_int ?SN)*(4/3)^?SN) \<le>2^n * 2"
        using Suc mult_mono[of 
                              "(4 / 3) ^ n * real n"
                              "2^n"
                              "((?SN/n) *(real_of_rat (4/3)))"
                              2] by force
      then show ?thesis
        by (metis "1" 3 mult_of_nat_commute of_rat_divide of_rat_numeral_eq of_rat_power power_Suc2)
    }
  qed
qed
