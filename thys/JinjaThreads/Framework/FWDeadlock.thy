...
inductive must_wait :: "('l,'t,'x,'m,'w) state \<Rightarrow> 't \<Rightarrow> ('l + 't) \<Rightarrow> 't \<Rightarrow> bool"
  ...
  "\<lbrakk> has_lock (locks s l) t'; t' \<noteq> t \<rbrakk> \<Longrightarrow> must_wait s t (Inl l) t'"
...
lemma must_wait_iff:
  "must_wait s t lt t' \<longleftrightarrow> (case lt of Inl l \<Rightarrow> t \<noteq> t' \<and> has_lock (locks s l) t'
...
where
  "deadlock s
   \<equiv> (\<exists>t. not_final_thread s t)
     ...
     \<and> (\<forall>t x ln. thr s t = \<lfloor>(x, ln)\<rfloor> \<and> (\<exists>l. ln l > 0) \<and> wset s t = None
...
lemma deadlockI:
  "\<lbrakk> not_final_thread s t;
    ...
    \<Longrightarrow> \<langle>x, shr s\<rangle> \<wrong> \<and> (\<forall>LT. \<langle>x, shr s\<rangle> LT \<wrong> \<longrightarrow> (\<exists>t'. thr s t' \<noteq> None \<and> (\<exists>lt \<in> LT. must_wait s t lt t')));
    \<And>t x ln l. \<lbrakk> thr s t = \<lfloor>(x, ln)\<rfloor>; ln l > 0; wset s t = None \<rbrakk>
    \<Longrightarrow> \<exists>l t'. ln l > 0 \<and> t \<noteq> t' \<and> thr s t' \<noteq> None \<and> has_lock (locks s l) t' \<rbrakk>
...
lemma deadlockE:
  ...
  and "\<forall>t x. thr s t = \<lfloor>(x, no_wait_locks)\<rfloor> \<and> \<not> final x \<and> wset s t = None
        \<longrightarrow> \<langle>x, shr s\<rangle> \<wrong> \<and> (\<forall>LT. \<langle>x, shr s\<rangle> LT \<wrong> \<longrightarrow> (\<exists>t'. thr s t' \<noteq> None \<and> (\<exists>lt \<in> LT. must_wait s t lt t')))"
  and "\<forall>t x ln. thr s t = \<lfloor>(x, ln)\<rfloor> \<and> (\<exists>l. ln l > 0) \<and> wset s t = None \<longrightarrow> (\<exists>l t'. ln l > 0 \<and> t \<noteq> t' \<and> thr s t' \<noteq> None \<and> has_lock (locks s l) t')"
...
lemma all_waiting_implies_deadlock:
  ...
  and normal: "\<And>t x. \<lbrakk> thr s t = \<lfloor>(x, no_wait_locks)\<rfloor>; \<not> final x; wset s t = None \<rbrakk> 
               \<Longrightarrow> \<langle>x, shr s\<rangle> \<wrong> \<and> (\<forall>LT. \<langle>x, shr s\<rangle> LT \<wrong> \<longrightarrow> (\<exists>t'. thr s t' \<noteq> None \<and> (\<exists>lt \<in> LT. must_wait s t lt t')))"
  and acquire: "\<And>t x ln l. \<lbrakk> thr s t = \<lfloor>(x, ln)\<rfloor>; wset s t = None; ln l > 0 \<rbrakk> \<Longrightarrow> \<exists>l'. ln l' > 0 \<and> \<not> may_lock (locks s l') t"
  shows "deadlock s"
...
next
  ...
  assume "thr s T = \<lfloor>(X, LN)\<rfloor>"
    and "0 < LN l'"
    and "wset s T = None"
  from acquire[OF `thr s T = \<lfloor>(X, LN)\<rfloor>` `wset s T = None`, OF `0 < LN l'`]
...
definition all_final_except :: "('l,'t,'x) thread_info \<Rightarrow> ('t \<Rightarrow> bool) \<Rightarrow> bool" where
  "all_final_except ts P \<equiv> \<forall>t. P t \<or> (case (ts t) of None \<Rightarrow> True | \<lfloor>(x, ln)\<rfloor> \<Rightarrow> final x)"
...
lemma all_final_exceptI:
  "(\<And>t x ln. \<lbrakk> ts t = \<lfloor>(x, ln)\<rfloor>; \<not> P t \<rbrakk> \<Longrightarrow> final x) \<Longrightarrow> all_final_except ts P"
by(auto simp add: all_final_except_def)

lemma all_final_exceptD:
  "\<lbrakk> all_final_except ts P; ts t = \<lfloor>(x, ln)\<rfloor>; \<not> P t \<rbrakk> \<Longrightarrow> final x"
by(auto simp add: all_final_except_def)
...
coinductive deadlocked :: "('l,'t,'x,'m,'w) state \<Rightarrow> 't \<Rightarrow> bool"
  ...
  where
  deadlockedLock:
    "\<lbrakk> thr s t = \<lfloor>(x, no_wait_locks)\<rfloor>; \<langle>x, shr s\<rangle> \<wrong>;
       \<forall>LT. \<langle>x, shr s\<rangle> LT \<wrong> \<longrightarrow> (\<exists>t'. (deadlocked s t' \<or> final_thread s t') \<and> (\<exists>lt \<in> LT. must_wait s t lt t')) \<rbrakk>
     \<Longrightarrow> deadlocked s t"

| deadlockedWait:
    "\<lbrakk> thr s t = \<lfloor>(x, ln)\<rfloor>; all_final_except (thr s) (deadlocked s); wset s t = \<lfloor>w\<rfloor> \<rbrakk> \<Longrightarrow> deadlocked s t"

| deadlockedAcquire:
    "\<lbrakk> thr s t = \<lfloor>(x, ln)\<rfloor>; ln l > 0; has_lock (locks s l) t'; t' \<noteq> t;
...
lemma deadlocked_elims [consumes 1, case_names lock wait acquire]:
  ...
  and lock: "\<And>x. \<lbrakk> thr s t = \<lfloor>(x, no_wait_locks)\<rfloor>; \<langle>x, shr s\<rangle> \<wrong>;
                     \<forall>LT. \<langle>x, shr s\<rangle> LT \<wrong> \<longrightarrow> (\<exists>t'. (deadlocked s t' \<or> final_thread s t') \<and> (\<exists>lt \<in> LT. must_wait s t lt t')) \<rbrakk>
             \<Longrightarrow> thesis"
  and wait: "\<And>x ln w. \<lbrakk> thr s t = \<lfloor>(x, ln)\<rfloor>; all_final_except (thr s) (deadlocked s); wset s t = \<lfloor>w\<rfloor>\<rbrakk> \<Longrightarrow> thesis"
  and acquire: "\<And>x ln l t'. \<lbrakk> thr s t = \<lfloor>(x, ln)\<rfloor>; 0 < ln l; has_lock (locks s l) t'; t \<noteq> t';
...
proof -
  ...
  proof(induct rule: redT_elims4)
	...
	by - (erule deadlocked.cases, fastsimp+)+
      ...
      from Pe ta have "\<langle>x, m\<rangle> ?LT \<wrong>" by(auto intro: can_syncI)
	...
	with lot have "may_lock (ls l) t"
	  by(auto elim!: collect_locksE lock_ok_las_may_lock)
	with `has_lock (locks s l) t'` s have "t' = t"
  ...
  next
    ...
    next
      case False
      with dead `ts t = \<lfloor>(x, ln)\<rfloor>` `0 < ln n` s
...
proof -
  ...
  proof(coinduct)
	...
	by(auto simp add: neq_no_wait_locks_conv)
      ...
      obtain l' where "0 < ln' l'" "\<not> may_lock (ls l') z"
	by(blast dest: acquire[simplified])
      then obtain t'' where "t'' \<noteq> z" "has_lock (ls l') t''"
	...
	by(clarsimp simp add: not_final_thread_iff final_thread_def)
      with `ws z = None` `0 < ln' l'` `ts z = \<lfloor>(x', ln')\<rfloor>` `t'' \<noteq> z` `has_lock (ls l') t''`
  ...
  proof(rule deadlocked'I)
	...
	{ fix LT
	  ...
	  with mw have "\<exists>t'.(not_final_thread s t' \<or> deadlocked s t' \<or> final_thread s t') \<and> (\<exists>lt\<in>LT. must_wait s t'' lt t')"
	    by fastsimp }
	with `\<langle>x'', shr s\<rangle> \<wrong>` tst'' have ?case by(simp) }
      note c3 = this
...
proof -
  ...
  next
	...
	by -(drule lock, auto elim!: deadlocked_thread_exists final_threadE)
      with `\<langle>x'', shr s\<rangle> \<wrong>` show ?case by(auto)
    next
      case (wait x'' ln'' w'')
  ...
  next
    ...
    assume "thr s t' = \<lfloor>(x', ln')\<rfloor>"
      and "0 < ln' l"
      and "wset s t' = None"
    hence "not_final_thread s t'" by(auto intro: not_final_thread.intros)
    hence "deadlocked s t'" by(rule deadlocked)
...
lemma deadlock_eq_deadlocked':
  "deadlock s \<longleftrightarrow> deadlocked' s"
by(auto intro: deadlock_implies_deadlocked' deadlocked'_implies_deadlock)
