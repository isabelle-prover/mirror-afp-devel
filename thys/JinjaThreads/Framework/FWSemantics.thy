...
lemma redT_elims [consumes 1, case_names normal acquire]:
  ...
  obtains (normal) x x' ta' m'
    ...
    and "s' = (redT_updLs (locks s) t \<lbrace>ta\<rbrace>\<^bsub>l\<^esub>, ((redT_updTs (thr s) \<lbrace>ta\<rbrace>\<^bsub>t\<^esub>)(t \<mapsto> (x', redT_updLns (locks s) t no_wait_locks \<lbrace>ta\<rbrace>\<^bsub>l\<^esub>)), m'), redT_updWs (wset s) t \<lbrace>ta\<rbrace>\<^bsub>w\<^esub>)"
  | (acquire) x ln n
    where "thr s t = \<lfloor>(x, ln)\<rfloor>" 
    and "ta = (\<lambda>\<^isup>f [], [], [], [], [ReacquireLocks ln])"
...
proof cases
  case redT_normal
  thus ?thesis by- (rule normal, fastsimp+)
next
  case redT_acquire
...
using assms
by(cases)(auto split: split_if_asm dest: wset_thread_okD redT_updTs_new_thread)
