...
by(simp add: lock_actions_append_def redT_updLs_def)
...
definition acquire_all :: "('l, 't) locks \<Rightarrow> 't \<Rightarrow> ('l \<Rightarrow> nat) \<Rightarrow> ('l, 't) locks"
where "acquire_all ls t ln \<equiv> \<lambda>l. acquire_locks (ls l) t (ln l)"
...
by(auto simp add: lock_actions_append_def lock_ok_las_def redT_updLs_def)
...
definition may_acquire_all :: "('l, 't) locks \<Rightarrow> 't \<Rightarrow> ('l \<Rightarrow> nat) \<Rightarrow> bool"
...
next
  ...
  proof(cases L)
    ...
    with laos' Lock mll have "lock_actions_ok (upd_lock l t L) t LS"
      by -(rule IH, auto simp add: lock_actions_ok'_def Cons_eq_append_conv)
    ultimately show ?thesis by simp
  next
    case Unlock
