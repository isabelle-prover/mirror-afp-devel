...
lemma assumes ha: "typeof_addr h a = \<lfloor>Class D\<rfloor>"
  ...
  \<Longrightarrow> \<tau>red1r P t h (e', xs) (Throw a, loc) \<and> P,e,n,h \<turnstile> (Throw a, loc) \<leftrightarrow> (stk, loc, pc, \<lfloor>a\<rfloor>)"
...
next
  ...
  hence "\<tau>red1r P t h (e', xs) (Throw a, loc) \<and> P,e,n,h \<turnstile> (Throw a, loc) \<leftrightarrow> (stk, loc, pc, \<lfloor>a\<rfloor>)"
    by(clarsimp simp add: match_ex_table_append_not_pcs compxE2_size_convs compxEs2_size_convs compxE2_stack_xlift_convs compxEs2_stack_xlift_convs match_ex_table_shift_pc_None)
  with `bsok c n` have "\<tau>red1r P t h (e';;while (c) e, xs) (Throw a, loc)"
...
next
  ...
  with ha have "P,t \<turnstile>1 \<langle>try Throw a catch(C V) e2, (h, loc)\<rangle> -\<epsilon>\<rightarrow> \<langle>Throw a, (h, loc)\<rangle>"
...
next
  ...
  hence "\<tau>red1r P t h (e', xs) (Throw a, loc) \<and> P,e2,Suc V,h \<turnstile> (Throw a, loc) \<leftrightarrow> (stk, loc, pc, \<lfloor>a\<rfloor>)"
...
next
  ...
  hence "xs = loc" by(auto dest: bisim1_ThrowD)
  with bisim `typeof_addr h a = \<lfloor>Class C''\<rfloor>` `\<not> P \<turnstile> C'' \<preceq>\<^sup>* C'` `bsok e2 (Suc V)`
...
| "conf_xcp' P h \<lfloor>a\<rfloor> = (\<exists>D. typeof_addr h a = \<lfloor>Class D\<rfloor> \<and> P \<turnstile> D \<preceq>\<^sup>* Throwable)"
...
by(simp add: ext_iff conf_xcp'_def option_case_def[symmetric])
...
lemma assumes wf: "wf_J1_prog P"
  ...
  \<Longrightarrow> \<exists>e'' xs''. P, E, n, h' \<turnstile> (e'', xs'') \<leftrightarrow> (stk', loc', pc', xcp') \<and>
     (if \<tau>move2 (compP2 P) h stk E pc xcp
      ...
      else \<exists>ta' e' xs'. \<tau>red1r P t h (e, xs) (e', xs') \<and> P,t \<turnstile>1 \<langle>e', (h, xs')\<rangle> -ta'\<rightarrow> \<langle>e'', (h', xs'')\<rangle> \<and> ta_bisim wbisim1 (extTA2J1 P ta') ta \<and> \<not> \<tau>move1 P h e' \<and> (call1 e = None \<or> no_call2 E pc \<or> e' = e \<and> xs' = xs))"
  ...
  \<Longrightarrow> \<exists>es'' xs''. P, Es, n, h' \<turnstile> (es'', xs'') [\<leftrightarrow>] (stk', loc', pc', xcp') \<and>
     (if \<tau>moves2 (compP2 P) h stk Es pc xcp
      ...
      else \<exists>ta' es' xs'. \<tau>reds1r P t h (es, xs) (es', xs') \<and> P,t \<turnstile>1 \<langle>es', (h, xs')\<rangle> [-ta'\<rightarrow>] \<langle>es'', (h', xs'')\<rangle> \<and> ta_bisim wbisim1 (extTA2J1 P ta') ta \<and> \<not> \<tau>moves1 P h es' \<and> (calls1 es = None \<or> no_calls2 Es pc \<or> es' = es \<and> xs' = xs))"
...
next
  ...
  from exec obtain ao where ao: "new_obj h C' = (h', ao)"
    by(cases "new_obj h C'")(auto elim!: exec_meth.cases simp add: exec_move_def)
  show ?case
  proof(cases "ao")
    case None
...
next
  ...
  next
    ...
    with bisim pc len have red: "\<tau>red1r P t h (e', xs) (Val v, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
    ...
    next
      ...
      with exec stk xcp obtain ao where ao: "new_arr h U (nat (sint I)) = (h', ao)"
        by(cases "new_arr h U (nat (sint I))")(auto elim!: exec_meth.cases simp add: exec_move_def)
      show ?thesis
      proof(cases "ao")
	case None
...
next
  ...
  note bisim = `P,e,n,h \<turnstile> (e', xs) \<leftrightarrow> (stk, loc, pc, xcp)`
  note len = `n + max_vars (e' instanceof U) \<le> length xs`
  note ST = `P,h \<turnstile> stk [:\<le>] ST`
  from bisim have pc: "pc \<le> length (compE2 e)" by(rule bisim1_pc_length_compE2)
  show ?case
  proof(cases "pc < length (compE2 e)")
  ...
  next
    ...
    from exec ST stk xcp have [simp]: "h' = h" "ta = \<epsilon>" "xcp' = None" "loc' = loc" "stk' = [?v]" "pc' = Suc pc"
      by(auto simp add: exec_move_def list_all2_Cons1 conf_def compP2_def elim!: exec_meth.cases split: split_if_asm)
    from `bsok e n` have bisim': "P,e instanceof U,n,h \<turnstile> (Val ?v, loc) \<leftrightarrow> ([?v], loc, length (compE2 (e instanceof U)), None)"
      by-(rule bisim1Val2, auto)
    from exec stk xcp `bsok e n` ST
    have red: "P,t \<turnstile>1 \<langle>(Val v) instanceof U,(h, loc)\<rangle> -\<epsilon>\<rightarrow> \<langle>Val ?v ,(h, loc)\<rangle>"
...
next
  ...
  note exec = `?exec (e instanceof U) stk loc pc \<lfloor>a\<rfloor> stk' loc' pc' xcp'`
  note bisim = `P,e,n,h \<turnstile> (Throw a, xs) \<leftrightarrow> (stk, loc, pc, \<lfloor>a\<rfloor>)`
  from bisim have pc: "pc < length (compE2 e)" by(auto dest: bisim1_ThrowD)
  from bisim have "match_ex_table (compP2 P) (cname_of h a) (0 + pc) (compxE2 e 0 0) = None"
    unfolding compP2_def by(rule bisim1_xcp_Some_not_caught)
...
next
  ...
  next
    ...
    with bisim1 obtain v where e1': "is_val e1' \<longrightarrow> e1' = Val v" 
      ...
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim1 pc len have rede1': "\<tau>red1r P t h (e1', xs) (Val v, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
...
next
  ...
  next
    ...
    with bisim2 obtain v2 where e2': "is_val e2' \<longrightarrow> e2' = Val v2" 
      ...
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim2 pc len have red: "\<tau>red1r P t h (e2', xs) (Val v2, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
    hence "\<tau>red1r P t h (Val v1 \<guillemotleft>bop\<guillemotright> e2', xs) (Val v1 \<guillemotleft>bop\<guillemotright> Val v2, loc)" by(rule BinOp_\<tau>red1r_xt2)
    also from exec xcp ST stk
    have "\<tau>red1r P t h (Val v1 \<guillemotleft>bop\<guillemotright> Val v2, loc) (Val (the (binop bop v1 v2)), loc)"
    ...
    ultimately show ?thesis using exec xcp stk call by(auto simp add: exec_move_def exec_meth_instr)
  qed
next
...
next
  ...
  next
    ...
    with bisim1 obtain v where a': "is_val a' \<longrightarrow> a' = Val v" 
      ...
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim1 pc len have rede1': "\<tau>red1r P t h (a', xs) (Val v, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
...
next
  ...
  next
    ...
    with bisim2 obtain v2 where i': "is_val i' \<longrightarrow> i' = Val v2" 
      ...
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim2 pc len have red: "\<tau>red1r P t h (i', xs) (Val v2, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
	...
	and [simp]: "v2 = Intg I" by(auto simp add: exec_move_def exec_meth_instr is_Ref_def conf_def)
      show ?thesis
      proof(cases "0 <=s I \<and> sint I < int (array_length h A)")
	case True
        hence "\<not> I <s 0" by auto
        moreover
        with exec xcp stk True obtain v3 where "stk' = [v3]" "heap_read h A (ACell (nat (sint I))) v3"
...
next
  ...
  next
    ...
    with bisim3 obtain v2 where stk: "stk = [v2]" and xcp: "xcp = None"
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim3 pc len have red: "\<tau>red1r P t h (e', xs) (Val v2, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
...
next
  ...
  next
    ...
    with bisim obtain v where stk: "stk = [v]" and xcp: "xcp = None"
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim pc len have "\<tau>red1r P t h (a', xs) (Val v, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
...
next
  ...
  next
    ...
    with bisim obtain v where stk: "stk = [v]" and xcp: "xcp = None"
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim pc len have "\<tau>red1r P t h (e', xs) (Val v, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
...
next
  ...
  next
    ...
    with bisim1 obtain v where stk: "stk = [v]" and xcp: "xcp = None"
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim1 pc len have rede1': "\<tau>red1r P t h (e', xs) (Val v, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
...
next
  ...
  next
    ...
    with bisim2 obtain v2 where stk: "stk = [v2]" and xcp: "xcp = None"
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim2 pc len have red: "\<tau>red1r P t h (e', xs) (Val v2, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
...
next
  ...
  next
    ...
    with bisim1 obtain v where stk: "stk = [v]" and xcp: "xcp = None" and call: "call1 obj' = None"
      ...
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim1 pc len have "\<tau>red1r P t h (obj', xs) (Val v, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
	...
	by(auto elim!: exec_meth.cases simp add: is_Ref_def exec_move_def compP2_def split: split_if_asm)
      thus ?thesis
      proof
	assume [simp]: "v = Null"
	...
	moreover have "P,t \<turnstile>1 \<langle>null\<bullet>M'(map Val []),(h, loc)\<rangle> -\<epsilon>\<rightarrow> \<langle>THROW NullPointer,(h, loc)\<rangle>"
	...
	with Ta iec have "P,t \<turnstile>1 \<langle>addr a\<bullet>M'(map Val []), (h, loc)\<rangle> -ta'\<rightarrow> \<langle>extRet2J (addr a\<bullet>M'(map Val [])) va, (h', loc)\<rangle>"
	  by -(rule Red1CallExternal, auto)
	moreover have "P,obj\<bullet>M'(ps),n,h' \<turnstile> (extRet2J (addr a\<bullet>M'(map Val [])) va, loc) \<leftrightarrow> (stk', loc', pc', xcp')"
	proof(cases va)
	  case (RetVal v)
	  ...
	  with ret RetExc show ?thesis by simp
        next
          ...
          have "P,obj\<bullet>M'([]),n,h' \<turnstile> (addr a\<bullet>M'([]), loc) \<leftrightarrow> ([Addr a], loc, length (compE2 obj), None)"
            by(rule bisim1_bisims1.bisim1Call1)(rule bisim1Val2, simp_all add: `bsok obj n`)
	...
	moreover from `preallocated h` red_external_hext[OF redex'] have "preallocated h'" by(rule preallocated_hext) 
        from wf' redex'' wtext'' `hconf h` have "hconf h'" by(rule external_call_hconf)
...
next
  ...
  note exec = `exec_move_d P t (obj\<bullet>M'(ps)) h (stk @ [v], loc, length (compE2 obj) + pc, xcp) ta h' (stk', loc', pc', xcp')`
  ...
  next
    ...
    with bisim2 len have reds: "\<tau>reds1r P t h (ps', xs) (map Val vs, loc)"
	...
	by(safe intro!: bisim1CallThrow elim!: bisim1_bsok bisims1_bsoks) simp
      moreover have "P,t \<turnstile>1 \<langle>null\<bullet>M'(map Val vs),(h, loc)\<rangle> -\<epsilon>\<rightarrow> \<langle>THROW NullPointer,(h, loc)\<rangle>"
	by(rule Red1CallNull)
	...
	and wtext: "P \<turnstile> Ta\<bullet>M'(Ts') :: U" "P \<turnstile> Ts [\<le>] Ts'"
	...
	by(fastsimp elim!: exec_meth.cases simp add: is_Ref_def exec_move_def compP2_def external_WT'_iff split: split_if_asm)
      ...
      from ret have [simp]: "h'' = h'" by simp
      from wtext' have wtext'': "compP2 P,h \<turnstile> a\<bullet>M'(vs) : U" by(simp add: compP2_def external_WT'_def)
      from wf have "wf_jvm_prog (compP2 P)" by(rule wt_compP2)
      then obtain wf_md where wf': "wf_prog wf_md (compP2 P)"
        unfolding wf_jvm_prog_def by(blast dest: wt_jvm_progD)
      from tconf have tconf': "compP2 P,h \<turnstile> t \<surd>t" by(simp add: compP2_def tconf_def)
      ...
      with Ta iec have "P,t \<turnstile>1 \<langle>addr a\<bullet>M'(map Val vs), (h, loc)\<rangle> -ta'\<rightarrow> \<langle>extRet2J (addr a\<bullet>M'(map Val vs)) va, (h', loc)\<rangle>"
	by -(rule Red1CallExternal, auto)
      moreover have "P,obj\<bullet>M'(ps),n,h' \<turnstile> (extRet2J (addr a\<bullet>M'(map Val vs)) va, loc) \<leftrightarrow> (stk', loc', pc', xcp')"
      proof(cases va)
	case (RetVal v)
	...
	from lenvs have "length ps = length vs" by simp
        from bisims1_map_Val_append[OF bisims1Nil `bsoks ps n` this, of P h' loc]
      ...
      moreover have "\<tau>move1 P h (Val v\<bullet>M'(map Val vs)) \<Longrightarrow> ta' = \<epsilon> \<and> h' = h"
        using Ta iec redex'
        by(auto simp add: map_eq_append_conv \<tau>external'_def dest: \<tau>external'_red_external_TA_empty \<tau>external'_red_external_heap_unchanged)
...
next
  ...
  note exec = `?exec (sync\<^bsub>V\<^esub> (e1) e2) [v, v] xs (Suc (length (compE2 e1))) None stk' loc' pc' xcp'`
...
next
  ...
  from exec have "(\<exists>a. v = Addr a \<and> stk' = [] \<and> loc' = xs[V := v] \<and> ta = \<epsilon>\<lbrace>\<^bsub>l\<^esub> Lock\<rightarrow>a \<rbrace>\<lbrace>\<^bsub>o\<^esub> SyncLock a\<rbrace> \<and> xcp' = None \<and> pc' = Suc (Suc (Suc (length (compE2 e1))))) \<or> (v = Null \<and> stk' = [v] \<and> loc' = xs[V := v] \<and> ta = \<epsilon> \<and> xcp' = \<lfloor>addr_of_sys_xcpt NullPointer\<rfloor> \<and> pc' = Suc (Suc (length (compE2 e1))))" (is "?c1 \<or> ?c2")
    by(fastsimp elim!: exec_meth.cases simp add: is_Ref_def expand_finfun_eq ext_iff finfun_upd_apply exec_move_def)
  thus ?case
  proof
    assume ?c1
    then obtain a where [simp]: "v = Addr a" "stk' = []" "loc' = xs[V := v]" "ta = \<epsilon>\<lbrace>\<^bsub>l\<^esub>Lock\<rightarrow>a\<rbrace>\<lbrace>\<^bsub>o\<^esub> SyncLock a\<rbrace>"
      "xcp' = None" "pc' = Suc (Suc (Suc (length (compE2 e1))))" by blast
    have "P,t \<turnstile>1 \<langle>sync\<^bsub>V\<^esub> (addr a) e2, (h, xs)\<rangle> -\<epsilon>\<lbrace>\<^bsub>l\<^esub>Lock\<rightarrow>a\<rbrace>\<lbrace>\<^bsub>o\<^esub> SyncLock a\<rbrace>\<rightarrow> \<langle>insync\<^bsub>V\<^esub> (a) e2,(h, xs[V := Addr a])\<rangle>"
  ...
  next
    ...
    from V have "P,t \<turnstile>1 \<langle>sync\<^bsub>V\<^esub> (null) e2, (h, xs)\<rangle> -\<epsilon>\<rightarrow> \<langle>THROW NullPointer,(h, xs[V := Null])\<rangle>"
...
next
  ...
  proof(cases "pc < length (compE2 e2)")
	...
	where ha': "typeof_addr h a' = \<lfloor>Class D\<rfloor>" and subcls: "P \<turnstile> D \<preceq>\<^sup>* Throwable" by(auto simp add: compP2_def)
      from ha' subcls bisim2 True have "\<tau>red1r P t h (e', xs) (Throw a', loc)"
  ...
  next
    ...
    moreover from bisim2 pc len have red: "\<tau>red1r P t h (e', xs) (Val v, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
...
next
  ...
  proof
    ...
    moreover from xsV V have "P,t \<turnstile>1 \<langle>insync\<^bsub>V\<^esub> (a) Val v, (h, xs)\<rangle> -\<epsilon>\<lbrace>\<^bsub>l\<^esub> Unlock\<rightarrow>a' \<rbrace>\<lbrace>\<^bsub>o\<^esub> SyncUnlock a'\<rbrace>\<rightarrow> \<langle>Val v,(h, xs)\<rangle>"
      by(rule Unlock1Synchronized)
    moreover from xsV V have "P,t \<turnstile>1 \<langle>insync\<^bsub>V\<^esub> (a) Val v, (h, xs)\<rangle> -\<epsilon>\<lbrace>\<^bsub>l\<^esub> UnlockFail\<rightarrow>a' \<rbrace>\<rightarrow> \<langle>THROW IllegalMonitorState,(h, xs)\<rangle>"
...
next
  ...
  moreover {
    fix A
    assume "xs ! V = Addr A"
    hence "P,t \<turnstile>1 \<langle>insync\<^bsub>V\<^esub> (a) Throw a',(h, xs)\<rangle> -\<epsilon>\<lbrace>\<^bsub>l\<^esub> Unlock\<rightarrow>A\<rbrace>\<lbrace>\<^bsub>o\<^esub> SyncUnlock A\<rbrace>\<rightarrow> \<langle>Throw a', (h, xs)\<rangle>"
      "P,t \<turnstile>1 \<langle>insync\<^bsub>V\<^esub> (a) Throw a',(h, xs)\<rangle> -\<epsilon>\<lbrace>\<^bsub>l\<^esub> UnlockFail\<rightarrow>A\<rbrace>\<rightarrow> \<langle>THROW IllegalMonitorState, (h, xs)\<rangle>"
...
next
  ...
  next
    ...
    with bisim1 obtain v where stk: "stk = [v]" and xcp: "xcp = None"
      by(auto dest: bisim1_pc_length_compE2D)
    with bisim1 pc len have red: "\<tau>red1r P t h (e', xs) (Val v, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
...
next
  ...
  moreover have "P,t \<turnstile>1 \<langle>while (c) e, (h, xs)\<rangle> -\<epsilon>\<rightarrow> \<langle>if (c) (e;;while (c) e) else unit, (h, xs)\<rangle>"
    by(rule Red1While)
...
next
  ...
  proof(cases "pc < length (compE2 e)")
	...
	by(auto simp add: compP2_def)
      then obtain C'' where ha': "typeof_addr h a' = \<lfloor>Class C''\<rfloor>" 
	...
	apply(simp only: compxE2_size_convs, simp)
      ...
      moreover from ha' subclsThrow have red: "\<tau>red1r P t h (e', xs) (Throw a', loc)"
...
next
  ...
  from bisim have "match_ex_table (compP2 P) (cname_of h a) (0 + pc) (compxE2 e 0 0) = None"
    unfolding compP2_def by(rule  bisim1_xcp_Some_not_caught)
  with `?exec (try e catch(C' V) e2) stk loc pc \<lfloor>a\<rfloor> stk' loc' pc' xcp'` pc `typeof_addr h a = \<lfloor>Class C''\<rfloor>` `\<not> P \<turnstile> C'' \<preceq>\<^sup>* C'`
...
next
  ...
  next
    ...
    ultimately show ?thesis using red' xcp stk stk' call v
      by(auto simp add: split_paired_Ex)(blast intro: rtranclp_trans rtranclp_tranclp_tranclp \<tau>reds1r_cons_\<tau>reds1r List1Red2 \<tau>reds1t_cons_\<tau>reds1t dest: \<tau>)+
  qed
next
  ...
  ultimately show ?case using red' stk'
    by(auto split: split_if_asm simp add: split_paired_Ex)(blast intro: rtranclp_trans rtranclp_tranclp_tranclp \<tau>reds1r_cons_\<tau>reds1r List1Red2 \<tau>reds1t_cons_\<tau>reds1t dest: \<tau>)+
qed
...
definition sim21_size :: "jvm_prog \<Rightarrow> jvm_thread_state \<Rightarrow> jvm_thread_state \<Rightarrow> bool"
where
...
lemma bisim1_Invoke_\<tau>Red:
  ...
  \<Longrightarrow> \<exists>e' xs'. \<tau>red1r P t h (e, xs) (e', xs') \<and> P,E,n,h \<turnstile> (e', xs') \<leftrightarrow> (rev vs @ Addr a # stk', loc, pc, None) \<and> call1 e' = \<lfloor>(a, M, vs)\<rfloor>"
  ...
  \<Longrightarrow> \<exists>es' xs'. \<tau>reds1r P t h (es, xs) (es', xs') \<and> P,Es,n,h \<turnstile> (es', xs') [\<leftrightarrow>] (rev vs @ Addr a # stk', loc, pc, None) \<and> calls1 es' = \<lfloor>(a, M, vs)\<rfloor>"
...
next
  ...
  moreover with `P,e2,n,h \<turnstile> (e', xs) \<leftrightarrow> (stk, loc, pc, None)` pc
...
next
  ...
  note bisim = `P,e,n,h \<turnstile> (e', xs) \<leftrightarrow> (rev vs @ Addr a # stk', loc, pc, None)`
...
next
  ...
  note bisim = `P,e2,n,h \<turnstile> (e', xs) \<leftrightarrow> (stk, loc, pc, None)`
...
next
  ...
  moreover with `P,e3,n,h \<turnstile> (e', xs) \<leftrightarrow> (stk, loc, pc, None)`  pc
  obtain vs'' v'' stk'' where "stk = vs'' @ v'' # stk''" and "length vs'' = length vs"
    by(auto dest!: bisim1_Invoke_stkD)
...
next
  ...
  note IH = `\<lbrakk>pc < length (compE2 obj); compE2 obj ! pc = Invoke M (length vs); n + max_vars obj' \<le> length xs
                     \<rbrakk> \<Longrightarrow> ?concl obj' xs obj n pc stk' loc`
  note bisim = `P,obj,n,h \<turnstile> (obj', xs) \<leftrightarrow> (rev vs @ Addr a # stk', loc, pc, None)`
  note len = `n + max_vars (obj'\<bullet>M'(ps)) \<le> length xs`
  hence len': "n + max_vars obj' \<le> length xs" by simp
  note inv = `compE2 (obj\<bullet>M'(ps)) ! pc = Invoke M (length vs)`
  with `pc < length (compE2 (obj\<bullet>M'(ps)))` bisim
  have pc: "pc < length (compE2 obj) \<or> ps = [] \<and> pc = length (compE2 obj)"
    by(cases ps)(auto split: split_if_asm dest: bisim1_pc_length_compE2)
  thus ?case
  proof
  ...
  next
    ...
    with bisim len' have "\<tau>red1r P t h (obj', xs) (addr a, loc)" by(auto intro: bisim1_Val_\<tau>red1r)
...
next
  ...
  note IH = `\<And>stk'. \<lbrakk>stk = rev vs @ Addr a # stk'; pc < length (compEs2 ps); compEs2 ps ! pc = Invoke M (length vs); n + max_varss ps' \<le> length xs
                     \<rbrakk> \<Longrightarrow> ?concls ps' xs ps n pc stk' loc`
  ...
  next
    ...
    from bisim len' have "\<tau>reds1r P t h (ps', xs) (map Val vs, loc)" by(auto intro: bisims1_Val_\<tau>Reds1r)
...
next
  case (bisim1TryCatch2 e V e' xs loc pc e1 C)
  note IH = `\<lbrakk>pc < length (compE2 e); compE2 e ! pc = Invoke M (length vs); Suc V + max_vars e' \<le> length xs
             \<rbrakk> \<Longrightarrow> ?concl e' xs e (Suc V) pc stk' loc`
...
next
  ...
  note IH = `\<And>stk'. \<lbrakk>stk = rev vs @ Addr a # stk'; pc < length (compEs2 es); compEs2 es ! pc = Invoke M (length vs); n + max_varss es' \<le> length xs
                     \<rbrakk> \<Longrightarrow> ?concls es' xs es n pc stk' loc`
...
proof(cases)
  ...
  proof(cases xcp)
	...
	from bisim have Bisim': "P,blocks1 0 (Class D#Ts) body,0,h \<turnstile> (e, xs) \<leftrightarrow> (rev (rev vs') @ Addr a' # stk', loc, pc, None)"
	  by simp
	from bisim1_Invoke_\<tau>Red[OF this, of MM t] True ins lenxs
	...
	from False execi True check ins sees' obtain C' Ts' T' meth D'
	  where ha': "typeof_addr h a' = \<lfloor>Class C'\<rfloor>"
	...
	have [simp]: "xcp' = None" "ta = \<epsilon>" "frs' = ?f' # ?f # FRS" "h' = h"
	  by(auto split: split_if_asm simp add: split_beta)(auto split: extCallRet.split_asm)
	...
	with call' ha'
	have "P,t \<turnstile>1 \<langle>(e', xs')/exs,h\<rangle> -\<epsilon>\<rightarrow> \<langle>(?e, ?xs)/ (e', xs') # exs, h\<rangle>"
	  using Sees vs'Ts' by(rule red1Call)
	hence "P,t \<turnstile>1 \<langle>(e', xs')/exs,h\<rangle> -\<epsilon>\<rightarrow> \<langle>(?e, ?xs)/ ?e'xs' # exs, h\<rangle>" by(simp)
	moreover from call' Sees ha' have "\<tau>Move1 P h ((e', xs'), exs)"
          by(auto simp add: synthesized_call_def dest: external_call_not_sees_method dest!: \<tau>move1_not_call1[where P=P and h=h])
	...
	ultimately show ?thesis using Red
      ...
      next
        ...
        with execi True have "\<exists>stk' loc' pc'. frs' = (stk', loc', C, M, pc') # FRS"
          by(cases "(compE2 body @ [Return]) ! pc")(auto split: split_if_asm simp: split_beta, auto split: extCallRet.splits)
	  ...
	  apply(cases "compE2 body ! pc", auto simp add: split_beta neq_Nil_conv split: split_if_asm)
        ...
        from red1_simulates_exec_instr[OF wf hconf tconf bisim this _ ST] lenxs \<tau>i obtain e'' xs''
	...
	by(cases "compE2 body ! pc")(auto split: split_if_asm simp add: compP2_def compMb2_def split_beta)
      ...
      with Bisim lenxs have red: "\<tau>red1r P t h (e, xs) (Val v, loc)"
	...
	have "P,t \<turnstile>1 \<langle>(Val v, loc)/(E'', xs'') # EXS',h\<rangle> -\<epsilon>\<rightarrow>
                      \<langle>(inline_call (Val v) E'', xs'')/EXS', h\<rangle>"
	  by(rule red1Return) simp
	hence "P,t \<turnstile>1 \<langle>(Val v, loc)/(E'', xs'') # EXS',h\<rangle> -\<epsilon>\<rightarrow> \<langle>(?e', ?xs')/EXS', h\<rangle>"
	  by simp
  ...
  next
    ...
    then obtain D' where ha': "typeof_addr h a' = \<lfloor>Class D'\<rfloor>" and subclsD': "P \<turnstile> D' \<preceq>\<^sup>* Throwable" by auto
	...
	by(auto simp add: exec_1_iff)
      ...
      from conf obtain CCC where ha: "typeof_addr h a' = \<lfloor>Class CCC\<rfloor>" and subcls: "P \<turnstile> CCC \<preceq>\<^sup>* Throwable"
        unfolding correct_state_def by(auto simp add: conf_f_def2 compP2_def)
      from bisim1_xcp_\<tau>Red[OF ha subcls bisim[unfolded Some], of "\<lambda>C M Ts T. compMb2"] match lenxs
	...
	by(auto simp add: exec_move_def exec_meth_xcpt)
      from conf obtain ST where "compP2 P,h \<turnstile> stk [:\<le>] ST" by(auto simp add: correct_state_def conf_f_def2)
      hence ST: "P,h \<turnstile> stk [:\<le>] ST" by(rule list_all2_mono)(simp add: compP2_def)
      from red1_simulates_exec_instr[OF wf hconf tconf bisim[unfolded `xcp = \<lfloor>a'\<rfloor>`] execm _ ST] lenxs ha' subclsD' \<tau>'
      obtain e'' xs''
...
lemma \<tau>Red1_simulates_exec_1_not_\<tau>:
  ...
  shows "\<exists>e' xs' exs' ta' e'' xs'' exs''. \<tau>Red1r P t h ((e, xs), exs) ((e', xs'), exs') \<and>
                                      P,t \<turnstile>1 \<langle>(e', xs')/exs', h\<rangle> -ta'\<rightarrow> \<langle>(e'', xs'')/exs'', h'\<rangle> \<and>
...
proof cases
  ...
  proof(cases xcp)
	...
	by(cases "(compE2 body @ [Return]) ! pc")(auto split: split_if_asm simp add: split_beta, auto split: extCallRet.splits)
      then obtain stk' loc' pc' where [simp]: "frs' = (stk', loc', C, M, pc') # FRS" by blast
      from conf obtain ST where "compP2 P,h \<turnstile> stk [:\<le>] ST" by(auto simp add: correct_state_def conf_f_def2)
      hence ST: "P,h \<turnstile> stk [:\<le>] ST" by(rule list_all2_mono)(simp add: compP2_def)
	...
	apply(cases "compE2 body ! pc", auto simp add: split_beta neq_Nil_conv split: split_if_asm)
      ...
      from red1_simulates_exec_instr[OF wf hconf tconf bisim this _ ST] lenxs \<tau>i obtain e'' xs'' ta' e' xs'
	where bisim': "P,blocks1 0 (Class D#Ts) body,0,h' \<turnstile> (e'', xs'') \<leftrightarrow> (stk', loc', pc', xcp')"
