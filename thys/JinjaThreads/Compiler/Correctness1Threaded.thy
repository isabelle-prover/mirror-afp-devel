...
lemma red0_Red1'_FWweak_bisim:
  ...
  shows "FWdelay_bisimulation_measure final_expr0 (mred0 P) final_expr1 (mred1' (compP1 P))
           bisim_red0_Red1 (\<tau>MOVE0 P) (\<tau>MOVE1 (compP1 P))
           (\<lambda>es es'. False)
           (\<lambda>(((e', xs'), exs'), h') (((e, xs), exs), h). countInitBlock e'< countInitBlock e)"
proof -
  interpret delay_bisimulation_measure "mred0 P" "mred1' (compP1 P)" bisim_red0_Red1 
                                    "ta_bisim bisim_red0_Red1" "\<tau>MOVE0 P" "\<tau>MOVE1 (compP1 P)"
                                    "\<lambda>es es'. False"
                                    "\<lambda>(((e', xs'), exs'), h') (((e, xs), exs), h). countInitBlock e' < countInitBlock e"
  ...
  proof
    ...
    ultimately show "\<exists>s2'. Red1'_\<tau>mthr.silent_moves (compP1 P) s2 s2' \<and> bisim_red0_Red1 s1 s2' \<and> (\<lambda>(x2, m). final_expr1 x2) s2'"
      by blast
  next
    fix s1 s2
    ...
    moreover hence "(\<lambda>(x1, m). final_expr0 x1) s1"
      by(cases s1)(cases s2,auto simp add: bisim_red0_Red1_def final_iff elim!: bisim_list1_elim elim: bisim_list.cases)
    ultimately show "\<exists>s1'. red0_\<tau>mthr.silent_moves P s1 s1' \<and> bisim_red0_Red1 s1' s2 \<and> (\<lambda>(x1, m). final_expr0 x1) s1'" by blast
    
  next
    fix x m1 x' m2 x1 x2 x1' ta1 x1'' m1' x2' ta2 x2'' m2'
    assume b: "bisim_red0_Red1 (x, m1) (x', m2)"
      and bo: "bisim_red0_Red1 (x1, m1) (x2, m2)"
      and \<tau>red1: "red0_\<tau>mthr.silent_moves P (x1, m1) (x1', m1)"
      and red1: "mred0 P (x1', m1) ta1 (x1'', m1')"
      and \<tau>1: "\<not> \<tau>MOVE0 P (x1', m1) ta1 (x1'', m1')"
      and \<tau>red2: "Red1'_\<tau>mthr.silent_moves (compP1 P) (x2, m2) (x2', m2)"
...
by(auto intro: el_loc_okI els_loc_okI syncvars_compE1 syncvarss_compEs1 \<B> \<B>s)

lemma
  shows el_loc_ok_not_contains_insync_local_change:
...
lemma el_loc_ok_extRet2J [simp]:
  "el_loc_ok (extRet2J va) xs"
by(cases va) auto

lemma red_IUF_expr_locks: 
  "\<lbrakk> P \<turnstile>1 \<langle>e, s\<rangle> -ta\<rightarrow> \<langle>e', s'\<rangle>; el_loc_ok e (lcl s); IUF e ta e'; \<lbrace>ta\<rbrace>\<^bsub>l\<^esub>\<^sub>f l = [UnlockFail] \<rbrakk> \<Longrightarrow> expr_locks e l > 0"
  and reds_IUFs_expr_lockss:
  "\<lbrakk> P \<turnstile>1 \<langle>es, s\<rangle> [-ta\<rightarrow>] \<langle>es', s'\<rangle>; els_loc_ok es (lcl s); IUFs es ta es'; \<lbrace>ta\<rbrace>\<^bsub>l\<^esub>\<^sub>f l = [UnlockFail] \<rbrakk> \<Longrightarrow> expr_lockss es l > 0"
...
lemma shows red1_preserves_el_loc_ok:
  "\<lbrakk> P \<turnstile>1 \<langle>e, s\<rangle> -ta\<rightarrow> \<langle>e', s'\<rangle>; sync_ok e; el_loc_ok e (lcl s) \<rbrakk> \<Longrightarrow> el_loc_ok e' (lcl s')"

  and reds1_preserves_els_loc_ok:
  "\<lbrakk> P \<turnstile>1 \<langle>es, s\<rangle> [-ta\<rightarrow>] \<langle>es', s'\<rangle>; sync_oks es; els_loc_ok es (lcl s) \<rbrakk> \<Longrightarrow> els_loc_ok es' (lcl s')"
proof(induct rule: red1_reds1.inducts)
  ...
  moreover from `P \<turnstile>1 \<langle>e,s\<rangle> -ta\<rightarrow> \<langle>e',s'\<rangle>` `unmod e V` have "unmod e' V"
    by(rule red1_unmod_preserved)
  moreover from red1_preserves_unmod[OF `P \<turnstile>1 \<langle>e,s\<rangle> -ta\<rightarrow> \<langle>e',s'\<rangle>` `unmod e V`] `lcl s ! V = Addr a`
...
lemma red1_preserves_sync_ok: "\<lbrakk> P \<turnstile>1 \<langle>e, s\<rangle> -ta\<rightarrow> \<langle>e', s'\<rangle>; sync_ok e \<rbrakk> \<Longrightarrow> sync_ok e'"
  and reds1_preserves_sync_oks: "\<lbrakk> P \<turnstile>1 \<langle>es, s\<rangle> [-ta\<rightarrow>] \<langle>es', s'\<rangle>; sync_oks es \<rbrakk> \<Longrightarrow> sync_oks es'"
by(induct rule: red1_reds1.inducts)(auto elim: not_contains_insync_sync_ok)

interpretation red0_Red1_FWbase: FWbisimulation_base final_expr0 "mred0 P" "final_expr1" "mred1' (compP1 P)" "bisim_red0_Red1" for P
.
...
lemma Red1_preserves_el_loc_ok1:
  ...
  shows "\<lbrakk> P \<turnstile>1 \<langle>ex/exs,m\<rangle> -ta\<rightarrow> \<langle>ex'/exs',m'\<rangle>; el_loc_ok1 (ex, exs) \<rbrakk>  \<Longrightarrow> el_loc_ok1 (ex', exs')"
...
lemma assumes wf: "wf_J1_prog P"
  ...
  "\<lbrakk> P \<turnstile>1 \<langle>e, s\<rangle> -ta\<rightarrow> \<langle>e', s'\<rangle>; NewThread t (C, M, a) h \<in> set \<lbrace>ta\<rbrace>\<^bsub>t\<^esub> \<rbrakk>
  \<Longrightarrow> el_loc_ok1 (({0:Class (fst (method P C M))=None; snd (snd (snd (method P C M)))}, xs), [(Val v\<bullet>M'([]), [])])"

  and reds1_el_loc_ok1_new_thread:
  "\<lbrakk> P \<turnstile>1 \<langle>es, s\<rangle> [-ta\<rightarrow>] \<langle>es', s'\<rangle>; NewThread t (C, M, a) h \<in> set \<lbrace>ta\<rbrace>\<^bsub>t\<^esub> \<rbrakk>
  \<Longrightarrow> el_loc_ok1 (({0:Class (fst (method P C M))=None; snd (snd (snd (method P C M)))}, xs), [(Val v\<bullet>M'([]), [])])"
proof(induct rule: red1_reds1.inducts)
...
lemma Red1_el_loc_ok1_new_thread:
  assumes wf: "wf_J1_prog P"
  shows "\<lbrakk> P \<turnstile>1 \<langle>ex/exs,m\<rangle> -ta\<rightarrow> \<langle>ex'/exs',m'\<rangle>; NewThread t exexs m' \<in> set \<lbrace>ta\<rbrace>\<^bsub>t\<^esub> \<rbrakk>
         \<Longrightarrow> el_loc_ok1 exexs"
by(erule Red1.cases)(auto intro!: red1_el_loc_ok1_new_thread[OF wf])

lemma Red1'_el_loc_ok: 
  assumes wf: "wf_J1_prog P"
  shows "lifting_wf (mred1' P) (\<lambda>exexs h. el_loc_ok1 exexs)"
...
lemma Red1_mthr_eq_Red1_mthr':
  ...
  and elo: "ts_ok (\<lambda>exexs h. el_loc_ok1 exexs) (thr s) (shr s)"
...
proof(intro ext)
  ...
  proof
    ...
    proof cases
      ...
      hence [simp]: "S = s" "S' = s'" "tta = (t, observable_ta_of ta)" by simp_all
      note thrS = `thr S t = \<lfloor>(x, no_wait_locks)\<rfloor>`
      note aoe = `Red1'_mthr.actions_ok S t ta` 
	...
	with red obtain ta'
	  where "P \<turnstile>1 \<langle>fst ex, (shr s, snd ex)\<rangle> -ta'\<rightarrow> \<langle>fst ex', (m', snd ex')\<rangle>" 
	...
	with ta' have "ta' = \<epsilon>\<lbrace>\<^bsub>l\<^esub>UnlockFail\<rightarrow>l\<rbrace>" by(cases ta') auto
	...
	ultimately show False by simp
      ...
      hence "Red1'_mthr.redT P s (t, observable_ta_of ta) s'"
	using thrS `wset S t = None` aoe `S' = redT_upd S t ta x' m'`
	unfolding `S = s` `S' = s'` by(rule Red1'_mthr.redT.redT_normal)
      thus ?thesis unfolding `tta = (t, observable_ta_of ta)` .
    next
	...
	`0 < ln\<^sub>f n` `S' = (acquire_all (locks S) t ln, (thr S(t \<mapsto> (x, no_wait_locks)), shr S), wset S)`
      show ?thesis unfolding `tta = (t, \<lambda>\<^isup>f [], [], [], [], ReacquireLocks ln)` `s' = S'` `s = S`
  ...
  next
	...
	by(auto simp add: split_beta)
      thus ?thesis using `thr S t = \<lfloor>(x, no_wait_locks)\<rfloor>` `wset S t = None`
       `Red1'_mthr.actions_ok S t ta` `S' = redT_upd S t ta x' m'`
	unfolding `s = S` `tta = (t, observable_ta_of ta)` `s' = S'`
...
lemma assumes wf: "wf_J1_prog P"
  ...
  "\<lbrakk> P \<turnstile>1 \<langle>e,s\<rangle> -TA\<rightarrow> \<langle>e',s'\<rangle>; NewThread t (ex, exs) h \<in> set (map (convert_new_thread_action (extNTA2J1 P)) \<lbrace>TA\<rbrace>\<^bsub>t\<^esub>) \<rbrakk>
  \<Longrightarrow> expr_lockss (map fst (ex # exs)) = (\<lambda>ad. 0)"
  and expr_lockss_new_thread1:
  "\<lbrakk> P \<turnstile>1 \<langle>es,s\<rangle> [-TA\<rightarrow>] \<langle>es',s'\<rangle>; NewThread t (ex, exs) h \<in> set (map (convert_new_thread_action (extNTA2J1 P)) \<lbrace>TA\<rbrace>\<^bsub>t\<^esub>) \<rbrakk>
  \<Longrightarrow> expr_lockss (map fst (ex # exs)) = (\<lambda>ad. 0)"
proof(induct rule: red1_reds1.inducts)
  case (Red1CallExternal s a T M vs ta va h' e' s')
  then obtain C fs where subThread: "P \<turnstile> C \<preceq>\<^sup>* Thread" and ext: "extNTA2J1 P (C, run, t) = (ex, exs)"
...
lemma assumes wf: "wf_J1_prog P"
  ...
  "\<lbrakk> P \<turnstile>1 \<langle>e, s\<rangle> -ta\<rightarrow> \<langle>e', s'\<rangle>; sync_ok e; el_loc_ok e (lcl s); \<not> IUF e ta e' \<rbrakk>
  ...
  "\<lbrakk> P \<turnstile>1 \<langle>es, s\<rangle> [-ta\<rightarrow>] \<langle>es', s'\<rangle>; sync_oks es; els_loc_ok es (lcl s); \<not> IUFs es ta es' \<rbrakk>
  \<Longrightarrow> upd_expr_locks (int o expr_lockss es) \<lbrace>ta\<rbrace>\<^bsub>l\<^esub> = int o expr_lockss es'"
proof -
  have "\<lbrakk> P \<turnstile>1 \<langle>e, s\<rangle> -ta\<rightarrow> \<langle>e', s'\<rangle>; sync_ok e; el_loc_ok e (lcl s); \<not> IUF e ta e' \<rbrakk> 
    ...
    and "\<lbrakk> P \<turnstile>1 \<langle>es, s\<rangle> [-ta\<rightarrow>] \<langle>es', s'\<rangle>; sync_oks es; els_loc_ok es (lcl s); \<not> IUFs es ta es' \<rbrakk>
  ...
  hence "\<lbrakk> P \<turnstile>1 \<langle>e, s\<rangle> -ta\<rightarrow> \<langle>e', s'\<rangle>; sync_ok e; el_loc_ok e (lcl s); \<not> IUF e ta e' \<rbrakk>
  ...
  thus "\<lbrakk> P \<turnstile>1 \<langle>e, s\<rangle> -ta\<rightarrow> \<langle>e', s'\<rangle>; sync_ok e; el_loc_ok e (lcl s); \<not> IUF e ta e'  \<rbrakk>
        \<Longrightarrow> upd_expr_locks (int o expr_locks e) \<lbrace>ta\<rbrace>\<^bsub>l\<^esub> = int o expr_locks e'"
    and "\<lbrakk> P \<turnstile>1 \<langle>es, s\<rangle> [-ta\<rightarrow>] \<langle>es', s'\<rangle>; sync_oks es; els_loc_ok es (lcl s); \<not> IUFs es ta es' \<rbrakk>
        \<Longrightarrow> upd_expr_locks (int o expr_lockss es) \<lbrace>ta\<rbrace>\<^bsub>l\<^esub> = int o expr_lockss es'"
...
lemma Red1'_preserves_lock_oks:
  ...
  and sync: "ts_ok (\<lambda>exexs h. el_loc_ok1 exexs) (thr s1) (shr s1)"
  shows "lock_oks1 (locks s1') (thr s1')"
using Red
proof(cases rule: Red1'_mthr.redT.cases)
  case (redT_normal x s ta x' m' t s')
  hence [simp]: "s1 = s" "ta1 = (t, observable_ta_of ta)" "s1' = s'" by simp_all
  obtain ex exs where x: "x = (ex, exs)" by (cases x)
  obtain ex' exs' where x': "x' = (ex', exs')" by (cases x')
  note thrst = `thr s t = \<lfloor>(x, no_wait_locks)\<rfloor>`
  note wst = `wset s t = None`
  note s' = `s' = redT_upd s t ta x' m'`
  note aoe = `Red1'_mthr.actions_ok s t ta`
  from `mred1' P (x, shr s) ta (x', m')`
  have red: "P \<turnstile>1 \<langle>ex/exs,shr s\<rangle> -ta\<rightarrow> \<langle>ex'/exs',m'\<rangle>"
    and IUF: "\<not> IUFL ex exs ta ex' exs'" unfolding x x' by simp_all
  thus ?thesis
  proof cases
    case (red1Red e h x TA e' h' x' EXS)
    hence [simp]: "ex = (e, x)" "exs = EXS" "shr s = h" "ta = extTA2J1 P TA"
      "ex' = (e', x')" "exs' = EXS" "m' = h'"
      and red: "P \<turnstile>1 \<langle>e,(h, x)\<rangle> -TA\<rightarrow> \<langle>e',(h', x')\<rangle>" by simp_all
    { fix t'
      assume None: "(redT_updTs (thr s) (map (convert_new_thread_action (extNTA2J1 P)) \<lbrace>TA\<rbrace>\<^bsub>t\<^esub>)(t \<mapsto> (((e', x'), EXS), redT_updLns (locks s) t (snd (the (thr s t))) \<lbrace>TA\<rbrace>\<^bsub>l\<^esub>))) t' = None"
      { fix l
	...
	also from loks None have "has_locks ((locks s)\<^sub>f l) t' = 0" unfolding lock_oks1_def
	  by(force split: split_if_asm dest!: redT_updTs_None)
	finally have "has_locks (upd_locks ((locks s)\<^sub>f l) t (\<lbrace>TA\<rbrace>\<^bsub>l\<^esub>\<^sub>f l)) t' = 0" by simp }
      hence "\<forall>l. has_locks (upd_locks ((locks s)\<^sub>f l) t (\<lbrace>TA\<rbrace>\<^bsub>l\<^esub>\<^sub>f l)) t' = 0" .. }
    moreover {
      fix t' eX eXS LN
      assume Some: "(redT_updTs (thr s) (map (convert_new_thread_action (extNTA2J1 P)) \<lbrace>TA\<rbrace>\<^bsub>t\<^esub>)(t \<mapsto> (((e', x'), EXS), redT_updLns (locks s) t (snd (the (thr s t))) \<lbrace>TA\<rbrace>\<^bsub>l\<^esub>))) t' = \<lfloor>((eX, eXS), LN)\<rfloor>"
      { fix l
 ...
 (upd_expr_lock_actions (int (expr_locks e l + expr_lockss (map fst exs) l)) (\<lbrace>ta\<rbrace>\<^bsub>l\<^esub>\<^sub>f l))"
    ...
    ultimately show ?thesis unfolding lock_oks1_def s' `s1' = s'` x' by(clarsimp simp del: fun_upd_apply)
  next
    ...
    thus ?thesis using red1Call thrst loks
      unfolding lock_oks1_def s' `s1' = s'` x' x
      by auto force+
  next
...
where "mbisim_Red1'_Red1 s1 s2 = (s1 = s2 \<and> lock_oks1 (locks s1) (thr s1) \<and> ts_ok (\<lambda>exexs h. el_loc_ok1 exexs) (thr s1) (shr s1))"
