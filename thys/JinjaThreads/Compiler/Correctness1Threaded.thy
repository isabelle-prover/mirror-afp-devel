...
proof(intro ext)
  ...
  proof
	...
	ultimately show False by simp
      ...
      moreover note thrS aoe `s' = redT_upd s t ta x' m'`
      ultimately have "Red1'_mthr.redT P s (t, observable_ta_of ta) s'"
  ...
  next
	...
	by(auto simp add: split_beta)
      moreover note `thr s t = \<lfloor>(x, no_wait_locks)\<rfloor>` `Red1_mthr.actions_ok s t ta` `s' = redT_upd s t ta x' m'`
      ultimately show ?thesis unfolding `tta = (t, observable_ta_of ta)`
...
proof(induct rule: red1_reds1.inducts)
  ...
  hence "expr_locks body = (\<lambda>ad. 0)" by(auto simp add: contains_insync_conv ext_iff)
...
proof -
  ...
  proof(induct rule: red1_reds1.inducts)
    case Red1CallExternal thus ?case
      by(auto simp add: ext_iff contains_insync_conv contains_insyncs_conv finfun_upd_apply elim!: red_external.cases)
  qed(fastsimp simp add: ext_iff contains_insync_conv contains_insyncs_conv finfun_upd_apply intro: ext)+
...
proof(cases rule: Red1'_mthr.redT.cases)
 ...
 (upd_expr_lock_actions (int (expr_locks e l + expr_lockss (map fst exs) l)) (\<lbrace>ta\<rbrace>\<^bsub>l\<^esub>\<^sub>f l))"
	  ...
	  hence "upd_expr_lock_actions (int (expr_locks e l)) (\<lbrace>TA\<rbrace>\<^bsub>l\<^esub>\<^sub>f l) = int (expr_locks e' l)"
	    by(simp add: upd_expr_locks_def ext_iff)
  ...
  next
    ...
    hence "expr_locks (blocks1 0 (Class D#Ts) body) = (\<lambda>l. 0)"
      by(auto simp add: expr_locks_blocks1 contains_insync_conv ext_iff dest!: WT1_not_contains_insync)
