...
where [code del]: "llength xs = inat_corec xs (\<lambda>xs. case xs of LNil \<Rightarrow> None | LCons x xs' \<Rightarrow> Some xs')"

definition ltake :: "inat \<Rightarrow> 'a llist \<Rightarrow> 'a llist"
where [code del]:
  "ltake n xs = 
   llist_corec (n, xs) (\<lambda>(n, xs). case n of 0 \<Rightarrow> None
                                    | iSuc n' \<Rightarrow> (case xs of LNil \<Rightarrow> None | LCons x xs' \<Rightarrow> Some (x, (n', xs'))))"
...
where [code del]:
  "lzip xs ys =
   llist_corec (xs, ys) (\<lambda>(xs, ys). case xs of LNil \<Rightarrow> None 
...
where [code del]: "llist_all2 P xs ys \<longleftrightarrow> llength xs = llength ys \<and> (\<forall>(x, y) \<in> lset (lzip xs ys). P x y)"
...
where [code del]:
  "lconcat xs =
   ...
   (\<lambda>xs. case xs of LNil \<Rightarrow> None | LCons x xs' \<Rightarrow> Some (lhd x, if ltl x = LNil then xs' else LCons (ltl x) xs'))"
...
lemma lappend_eq_LNil_conv [simp]: "lappend xs ys = LNil \<longleftrightarrow> xs = LNil \<and> ys = LNil"
...
proof -
  ...
  hence "(lappend xs ys, xs) \<in> {(lappend xs ys, xs)| xs ys. \<not> lfinite xs}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain xs ys where q: "q = (lappend xs ys, xs)" and xs: "\<not> lfinite xs" by blast
    from xs obtain x xs' where xs': "xs = LCons x xs'" "\<not> lfinite xs'" by(cases xs) auto
...
subsection {* The length of a lazy lists: @{term "llength"} *}
...
proof -
  have "(llength (lmap f xs), llength xs) \<in> {(llength (lmap f xs), llength xs)|xs. True}" by blast
  thus ?thesis
...
proof -
  ...
  proof(coinduct rule: inat_equalityI)
    case (Eqinat m n)
    then obtain xs where xs: "m = llength (lappend xs ys)" "n = llength xs + llength ys" by auto
    thus ?case
    proof(cases xs)
      case LNil
...
proof -
  ...
  hence "(llength (iterates f x), Infty) \<in> {(llength (iterates f x), Infty')|x. True}" by blast
  thus ?thesis
  proof(coinduct rule: inat_equalityI)
    case (Eqinat m n)
...
lemma ltake_iSuc_LCons [simp]: "ltake (iSuc n) (LCons x xs) = LCons x (ltake n xs)"
by(simp add: ltake_def llist_corec)

lemma ltake_iSuc: "ltake (iSuc n) xs = (case xs of LNil \<Rightarrow> LNil | LCons x xs' \<Rightarrow> LCons x (ltake n xs'))"
by(cases xs) simp_all

lemma ltake_LCons [code]: "ltake n (LCons x xs) = (case n of 0 \<Rightarrow> LNil | iSuc n' \<Rightarrow> LCons x (ltake n' xs))"
...
proof -
  have "(ltake n (lmap f xs), lmap f (ltake n xs)) \<in> {(ltake n (lmap f xs), lmap f (ltake n xs))|n xs. True}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain n xs where q: "q = (ltake n (lmap f xs), lmap f (ltake n xs))" by blast
    show ?case
    proof(cases xs)
...
proof -
  ...
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain m n xs where q: "q = (ltake n (ltake m xs), ltake (min n m) xs)" by blast
    show ?case
    proof(cases xs)
      case LNil with q have ?EqLNil by simp
      thus ?thesis ..
    next
      case (LCons x xs')
      with q show ?thesis
	by(cases n rule: inat_coexhaust)(simp, cases m rule: inat_coexhaust, auto simp add: min_def)
    qed
  qed
qed
...
proof -
  ...
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain m xs where q: "q = (ltake m xs, xs)" and len: "llength xs \<le> m" by blast
    show ?case
    proof(cases xs)
      case LNil with q have ?EqLNil by simp
      thus ?thesis ..
  ...
  next
    ...
    moreover have "lfinite (ltake (Fin n') xs)"
      by(induct n' arbitrary: xs)
        (auto simp add: zero_inat_def[symmetric] iSuc_Fin[symmetric] ltake_iSuc split: llist_split)
...
proof -
  ...
  proof(coinduct rule: llist_equalityI)
    ...
    next
      case (LCons x xs')
      with n show ?thesis unfolding q by(cases n rule: inat_coexhaust) auto
    qed
  qed
qed
...
lemma ldropn_LCons: "ldropn n (LCons x xs) = (case n of 0 \<Rightarrow> LCons x xs | Suc n' \<Rightarrow> ldropn n' xs)"
by(cases n)(simp_all add: ldropn.simps)
...
lemma ldrop_iSuc: "ldrop (iSuc n) xs = (case xs of LNil \<Rightarrow> LNil | LCons x xs' \<Rightarrow> ldrop n xs')"
by(cases xs) simp_all

lemma ldrop_LCons: "ldrop n (LCons x xs) = (case n of 0 \<Rightarrow> LCons x xs | iSuc n' \<Rightarrow> ldrop n' xs)"
by(cases n rule: inat_coexhaust) simp_all
...
proof -
  have "(lappend (ltake n xs) (ldrop n xs), xs) \<in> {(lappend (ltake n xs) (ldrop n xs), xs)|n xs. True}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain n xs where q: "q = (lappend (ltake n xs) (ldrop n xs), xs)" by blast
    show ?case
    proof(cases xs)
      case LNil with q have ?EqLNil by simp
      thus ?thesis ..
...
next
  ...
  next
    ...
    next
      case (Suc n')
      with IH[of n' xs'] `Fin n \<le> llength xs` LCons
      show ?thesis by(auto simp add: iSuc_Fin[symmetric] ldropn.simps)
    qed
  qed
qed
...
subsection {* The prefix ordering on lazy lists: @{term "lprefix"} and @{term "lstrict_prefix"} *}
...
lemma lprefix_LCons_conv: "lprefix xs (LCons y ys) \<longleftrightarrow> xs = LNil \<or> (\<exists>xs'. xs = LCons y xs' \<and> lprefix xs' ys)"
by(cases xs)(auto simp add: lprefix_def)

lemma LCons_lprefix_LCons [simp]: "lprefix (LCons x xs) (LCons y ys) \<longleftrightarrow> x = y \<and> lprefix xs ys"
...
lemma lprefix_antisym:
  ...
  shows "xs = ys"
proof -
  from assms have "(xs, ys) \<in> {(xs, ys)|xs ys. lprefix xs ys \<and> lprefix ys xs}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    ...
    next
      case (LCons x xs')
      with prefix obtain ys' where "ys = LCons x ys'" "lprefix ys' xs'" "lprefix xs' ys'"
...
lemma lprefix_llength_eq_imp_eq:
  ...
  shows "xs = ys"
proof -
  from assms have "(xs, ys) \<in> {(xs, ys)|xs ys. lprefix xs ys \<and> llength xs = llength ys}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain xs ys where q: "q = (xs, ys)"
...
proof -
  from assms have "(llength xs, llength ys) \<in> {(llength xs, llength ys)|xs ys :: 'a llist. lprefix xs ys}" by blast
  thus ?thesis
  proof(coinduct rule: inat_leI)
	...
	where "ys = LCons x ys'" "lprefix xs' ys'" by(auto simp add: LCons_lprefix_conv)
...
proof(rule ccontr)
  ...
  from `lprefix xs ys` have "llength xs \<le> llength ys" by(rule lprefix_llength_le)
  ultimately have "llength xs = llength ys" by auto
  with `lprefix xs ys` have "xs = ys" by(rule lprefix_llength_eq_imp_eq)
...
next
  ...
  next
    ...
    from `(xs', ys') \<in> lprefix_llist` have "lprefix (ltake (Fin n) xs') (ltake (Fin n) ys')" by(rule Suc)
    with Le_LCons show ?thesis by(simp add: iSuc_Fin[symmetric])
  qed
qed

lemma ltake_Fin_eq_imp_eq:
  assumes eq: "\<And>n. ltake (Fin n) xs = ltake (Fin n) ys"
  shows "xs = ys"
proof -
  from eq have "(xs, ys) \<in> {(xs, ys)|xs ys. \<forall>n. ltake (Fin n) xs = ltake (Fin n) ys}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain xs ys where q: "q = (xs, ys)"
...
next
  ...
  { fix n
    ...
    hence "llength (lappend (ltake (Fin n) xs) zs) = llength (ltake (Fin n) ys)" by simp
    hence n: "Fin n \<le> llength ys"
      by(cases "llength zs", cases "llength ys")(simp_all add: min_def split: split_if_asm)
...
lemma lprefixI [consumes 1, case_names lprefix, case_conclusion lprefix LeLNil LeLCons]:
  assumes major: "(xs, ys) \<in> X"
  and step [simplified mem_def]: "\<And>xs ys. (xs, ys) \<in> X 
            \<Longrightarrow> xs = LNil \<or> (\<exists>x xs' ys'. xs = LCons x xs' \<and> ys = LCons x ys' \<and> ((xs', ys') \<in> X \<or> lprefix xs' ys'))"
  shows "lprefix xs ys"
proof -
  from major have "curry X xs ys" by(auto simp add: mem_def)
  thus ?thesis by(rule lprefix_llist.coinduct[unfolded lprefix_llist_eq_lprefix])(auto dest: step)
qed
...
lemma lnth_ldropn [simp]: "Fin (n + m) < llength xs \<Longrightarrow> lnth (ldropn n xs) m = lnth xs (m + n)"
proof(induct n arbitrary: xs)
  case (Suc n)
...
lemma lnth_lappend1: "Fin n < llength xs \<Longrightarrow> lnth (lappend xs ys) n = lnth xs n"
proof(induct n arbitrary: xs)
...
lemma lnth_lappend_llist_of: 
  "lnth (lappend (llist_of xs) ys) n = (if n < length xs then xs ! n else lnth ys (n - length xs))"
proof(induct xs arbitrary: n)
...
next
  case (Suc m)
  from `Fin (Suc m) < n` obtain n' where "n = iSuc n'" by(cases n rule: inat_coexhaust) auto
...
next
  case (Suc n)
  from `Fin (Suc n) < llength xs` obtain x xs' where [simp]: "xs = LCons x xs'" by(cases xs) auto
  from `Fin (Suc n) < llength xs` have "Fin n < llength xs'" by(simp add: Suc_ile_eq)
...
proof -
  have "(lzip (lappend xs ys) (lappend us vs), lappend (lzip xs us) (lzip ys vs)) \<in> 
       {(lzip (lappend xs ys) (lappend us vs), lappend (lzip xs us) (lzip ys vs))|xs ys us vs. llength xs = llength us}"
    using len by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain xs ys us vs
      where q: "q = (lzip (lappend xs ys) (lappend us vs), lappend (lzip xs us) (lzip ys vs))"
...
proof -
  have "(llength (lzip xs ys), min (llength xs) (llength ys)) \<in> 
       {(llength (lzip xs ys), min (llength xs) (llength ys))|(xs :: 'a llist) (ys :: 'b llist). True}"
    by blast
  thus ?thesis
  proof(coinduct rule: inat_equalityI)
    case (Eqinat m n)
    then obtain xs :: "'a llist" and ys :: "'b llist"
      where m: "m = llength (lzip xs ys)" and n: "n = min (llength xs) (llength ys)" by blast
    show ?case
...
proof -
  have "(ltake n (lzip xs ys), lzip (ltake n xs) (ltake n ys)) \<in>
        {(ltake n (lzip xs ys), lzip (ltake n xs) (ltake n ys))|n xs ys. True}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain n xs ys where q: "q = (ltake n (lzip xs ys), lzip (ltake n xs) (ltake n ys))" by blast
    show ?case
...
lemma lzip_iterates: "lzip (iterates f x) (iterates g y) = iterates (\<lambda>(x, y). (f x, g y)) (x, y)"
proof -
  have "(lzip (iterates f x) (iterates g y), iterates (\<lambda>(x, y). (f x, g y)) (x, y)) \<in>
       {(lzip (iterates f x) (iterates g y), iterates (\<lambda>(x, y). (f x, g y)) (x, y))|x y. True}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain x y where q: "q = (lzip (iterates f x) (iterates g y), iterates (\<lambda>(x, y). (f x, g y)) (x, y))" by auto
...
lemma lappend_eq_lappend_conv:
  assumes len: "llength xs = llength us"
  shows "lappend xs ys = lappend us vs \<longleftrightarrow> xs = us \<and> (lfinite xs \<longrightarrow> ys = vs)" (is "?lhs \<longleftrightarrow> ?rhs")
proof
  assume ?rhs
...
next
  ...
  proof(intro conjI impI)
    from len eq have "(xs, us) \<in> {(xs, us). llength xs = llength us \<and> lappend xs ys = lappend us vs}" by blast
    thus "xs = us"
    proof(coinduct rule: llist_equalityI)
      case (Eqllist q)
	...
	with LNil q show ?thesis by simp
      next
	case (LCons x xs')
	with len obtain u us' where "us = LCons u us'" by(cases us)(auto)
	with LCons q len eq show ?thesis by simp
      qed
    qed
    assume "lfinite xs"
    then obtain xs' where "xs = llist_of xs'" by(auto simp add: lfinite_eq_range_llist_of)
    with eq `xs = us` have "(ys, vs) \<in> {(ys, vs). \<exists>xs'. lappend (llist_of xs') ys = lappend (llist_of xs') vs}" by blast
    thus "ys = vs"
    proof(coinduct rule: llist_equalityI)
      case (Eqllist q)
	...
	proof(cases vs)
	  ...
	  hence "llength (llist_of xs') = llength (lappend (llist_of (xs' @ [v])) vs')" by simp
	  hence "Fin (length xs') = Fin (Suc (length xs')) + llength vs'" by simp
	...
	thus ?thesis ..
      next
	case (LCons y ys')
	with eq obtain vs' where "vs = LCons y vs'" "lappend (llist_of (xs' @ [y])) ys' = lappend (llist_of (xs' @ [y])) vs'"
	proof(cases vs)
	  case LNil
	  with eq LCons have "llist_of xs' = lappend (llist_of (xs' @ [y])) ys'"
	    by(auto simp add: lappend_llist_of_LCons)
	  hence "llength (llist_of xs') = llength (lappend (llist_of (xs' @ [y])) ys')" by simp
	...
	next
	  ...
	  have "lappend (llist_of (xs' @ [y])) ys' = lappend (llist_of (xs' @ [v])) vs'"
	    by(auto simp add: lappend_llist_of_LCons)
	  also have "lnth \<dots> (length xs') = v" by(simp add: lnth_lappend1 lnth_llist_of)
...
lemma lzip_eq_lappend_conv:
  ...
  shows "\<exists>xs' xs'' ys' ys''. xs = lappend xs' xs'' \<and> ys = lappend ys' ys'' \<and> llength xs' = llength ys' \<and> us = lzip xs' ys' \<and> vs = lzip xs'' ys''"
proof -
  ...
  with eq have "lappend us vs = lzip (lappend ?xs' ?xs'') (lappend ?ys' ?ys'')" by simp
  with len have "lappend us vs = lappend (lzip ?xs' ?ys') (lzip ?xs'' ?ys'')"    
    by(simp add: lzip_lappend min_def)
  hence us: "us = lzip ?xs' ?ys'" and vs: "lfinite us \<longrightarrow> vs = lzip ?xs'' ?ys''" using len
  ...
  next
    ...
    moreover from False have "llength us = Infty" by(rule not_lfinite_llength)
    moreover with len have "llength xs = Infty" "llength ys = Infty" by auto
    moreover with `llength us = Infty` have "xs = ?xs'" "ys = ?ys'" by(simp_all add: ltake_all)
    from `llength us = Infty` len have "\<not> lfinite ?xs'" "\<not> lfinite ?ys'"
      by(auto simp del: llength_ltake lfinite_ltake simp add: ltake_all dest: lfinite_llength_Fin)
    with `xs = ?xs'` `ys = ?ys'` have "xs = lappend ?xs' ?xs''" "ys = lappend ?ys' ?ys''"
      by(simp_all add: lappend_inf)
    moreover have "vs = lzip ?xs'' ?ys''" by(coinduct vs rule: llist_fun_equalityI) simp_all
    ultimately show ?thesis using eq by(fastsimp  simp add: ltake_all)
  qed
qed

lemma lzip_lmap [simp]: "lzip (lmap f xs) (lmap g ys) = lmap (\<lambda>(x, y). (f x, g y)) (lzip xs ys)"
proof -
  have "(lzip (lmap f xs) (lmap g ys), lmap (\<lambda>(x, y). (f x, g y)) (lzip xs ys)) \<in>
       {(lzip (lmap f xs) (lmap g ys), lmap (\<lambda>(x, y). (f x, g y)) (lzip xs ys))|xs ys. True}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain xs ys where q: "q = (lzip (lmap f xs) (lmap g ys), lmap (\<lambda>(x, y). (f x, g y)) (lzip xs ys))" by blast
    show ?case unfolding q by(cases xs)(simp, cases ys, auto)
  qed
qed

lemma lzip_lmap1: "lzip (lmap f xs) ys = lmap (\<lambda>(x, y). (f x, y)) (lzip xs ys)"
by(subst (4) lmap_ident[symmetric])(simp only: lzip_lmap)

lemma lzip_lmap2: "lzip xs (lmap f ys) = lmap (\<lambda>(x, y). (x, f y)) (lzip xs ys)"
...
proof -
  have "(lmap fst (lzip xs ys), ltake (min (llength xs) (llength ys)) xs) \<in>
       {(lmap fst (lzip xs ys), ltake (min (llength xs) (llength ys)) xs)|xs (ys :: 'b llist). True}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain xs and ys :: "'b llist"
      where q: "q = (lmap fst (lzip xs ys), ltake (min (llength xs) (llength ys)) xs)" by blast
...
proof -
  have "(lmap snd (lzip xs ys), ltake (min (llength xs) (llength ys)) ys) \<in>
       {(lmap snd (lzip xs ys), ltake (min (llength xs) (llength ys)) ys)|(xs :: 'a llist) ys. True}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain xs :: "'a llist" and ys
      where q: "q = (lmap snd (lzip xs ys), ltake (min (llength xs) (llength ys)) ys)" by blast
    thus ?case by(cases xs)(simp, cases ys, auto)
  qed
qed

lemma lzip_conv_lzip_ltake_min_llength:
  "lzip xs ys = lzip (ltake (min (llength xs) (llength ys)) xs) (ltake (min (llength xs) (llength ys)) ys)"
proof -
  have "(lzip xs ys, lzip (ltake (min (llength xs) (llength ys)) xs) (ltake (min (llength xs) (llength ys)) ys)) \<in>
       {(lzip xs ys, lzip (ltake (min (llength xs) (llength ys)) xs) (ltake (min (llength xs) (llength ys)) ys))|xs ys. True}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain xs ys
      where q: "q = (lzip xs ys, lzip (ltake (min (llength xs) (llength ys)) xs) (ltake (min (llength xs) (llength ys)) ys))"
      by blast
    show ?case unfolding q by(cases xs)(simp, cases ys, auto)
  qed
qed
...
proof -
  ...
  thus ?thesis by(auto simp add: lset_def Suc_ile_eq lnth_LCons split: nat.split)
qed
...
lemma lset_lzip: 
  "lset (lzip xs ys) = {(lnth xs n, lnth ys n)|n. Fin n < min (llength xs) (llength ys)}"
...
proof -
  ...
  next
    ...
    with xs have "xs = lappend (LCons x ys) (LCons (lnth xs (Suc n)) zs)" "lfinite (LCons x ys)" by simp_all
    thus ?case by blast
  qed
qed
...
lemma llist_all2_conv_all_lnth:
  "llist_all2 P xs ys \<longleftrightarrow> llength xs = llength ys \<and> (\<forall>n. Fin n < llength ys \<longrightarrow> P (lnth xs n) (lnth ys n))"
...
lemma ltl_lappend:
  "ltl (lappend xs ys) = (case xs of LNil \<Rightarrow> ltl ys | LCons x xs' \<Rightarrow> lappend xs' ys)"
...
next
  case (Suc n)
  thus ?case by(cases xs)(simp_all del: ldropn.simps(2) add: ldropn_Suc)
qed
...
lemma llexord_LCons_left:
  "llexord r (LCons x xs) ys \<longleftrightarrow> (\<exists>y ys'. ys = LCons y ys' \<and> (x = y \<and> llexord r xs ys' \<or> r x y))"
...
using `lfinite xs` `llexord r (lappend xs ys) (lappend xs zs)` `!!x. x \<in> lset xs \<Longrightarrow> \<not> r x x`
by(induct) simp_all

lemma llexord_lappend_left:
  "\<lbrakk> lfinite xs; !!x. x \<in> lset xs \<Longrightarrow> \<not> r x x \<rbrakk> \<Longrightarrow> llexord r (lappend xs ys) (lappend xs zs) \<longleftrightarrow> llexord r ys zs"
...
proof(rule antisymI)
  fix xs ys
  assume "(xs, ys) \<in> {(xs, ys). llexord r xs ys}" and "(ys, xs) \<in> {(xs, ys). llexord r xs ys}"
...
lemma llexord_antisym:
  "\<lbrakk> llexord r xs ys; llexord r ys xs; !!a b. \<lbrakk> r a b; r b a \<rbrakk> \<Longrightarrow> False \<rbrakk> \<Longrightarrow> xs = ys"
...
proof
  ...
  proof(rule disjCI)
    assume "\<not> ?prefix"
    with `?lhs` 
    have "(xs, ys) \<in> {(xs, ys). llexord r xs ys \<and> (\<forall>zs xs' y ys'. lfinite zs \<longrightarrow> xs = lappend zs xs' \<longrightarrow>
    ...
    proof(coinduct rule: llist_equalityI)
      ...
      then obtain xs ys where q: "q = (xs, ys)"
	and "llexord r xs ys"
	and prefix: "\<And>zs xs' y ys'. \<lbrakk> lfinite zs; xs = lappend zs xs'; ys = lappend zs (LCons y ys') \<rbrakk>
	...
	case (llexord_LCons_eq xs' ys' x)
	{ fix zs xs'' y ys''
	  assume "lfinite zs" "xs' = lappend zs xs''" "ys' = lappend zs (LCons y ys'')"
	  hence "lfinite (LCons x zs)" "xs = lappend (LCons x zs) xs''" "ys = lappend (LCons x zs) (LCons y ys'')"
	...
	next
	  case (LCons y ys')
	  with prefix[of LNil xs y ys'] llexord_LNil show ?thesis by simp
	qed
	thus ?thesis ..
      qed
    qed
...
next
  ...
  proof(coinduct)
	...
	next
	  assume "\<exists>zs xs' y ys'. lfinite zs \<and> xs = lappend zs xs' \<and> ys = lappend zs (LCons y ys') \<and>
                                 (xs' = LNil \<or> r (lhd xs') y)"
	  then obtain zs xs' y' ys''
	    where "lfinite zs" "xs = lappend zs xs'" "ys = lappend zs (LCons y' ys'')"
                  "xs' = LNil \<or> r (lhd xs') y'" by blast
	  with True LCons `ys = LCons y ys'` show ?thesis by(cases zs) auto
	qed
      next
	case False
...
lemma llexord_conv_ltake_index:
  "llexord r xs ys \<longleftrightarrow>
   ...
   (\<exists>n. Fin n < min (llength xs) (llength ys) \<and> ltake (Fin n) xs = ltake (Fin n) ys \<and> r (lnth xs n) (lnth ys n))"
  (is "?lhs \<longleftrightarrow> ?rhs")
proof(rule iffI)
  assume ?lhs
  ...
  next
    ...
    then obtain zs xs' y ys'
      ...
      and [simp]: "xs = lappend zs xs'" "ys = lappend zs (LCons y ys')" by blast
    show ?thesis
    proof(cases xs')
      case LNil
	...
	using LCons `r (lhd xs') y` by(simp add: lappend_llist_of_LCons lnth_lappend1 lnth_llist_of)
...
next
  ...
  next
    assume "?B"
    then obtain n where "Fin n < min (llength xs) (llength ys)" "ltake (Fin n) xs = ltake (Fin n) ys"
      and "r (lnth xs n) (lnth ys n)" by blast
    have "xs = lappend (ltake (Fin n) xs) (ldrop (Fin n) xs)" by(simp only: lappend_ltake_ldrop)
    moreover from `ltake (Fin n) xs = ltake (Fin n) ys` `Fin n < min (llength xs) (llength ys)`
    have "ys = lappend (ltake (Fin n) xs) (LCons (lnth ys n) (ldrop (Fin (Suc n)) ys))"
      by(simp add: ldropn_Suc_conv_ldropn)(simp only: lappend_ltake_ldrop ldrop.simps[symmetric])
    moreover from `r (lnth xs n) (lnth ys n)` `Fin n < min (llength xs) (llength ys)`
    have "r (lhd (ldrop (Fin n) xs)) (lnth ys n)" by(simp add: lhd_ldropn)
...
lemma llexord_llist_of:
  "llexord r (llist_of xs) (llist_of ys) \<longleftrightarrow> xs = ys \<or> (xs, ys) \<in> lexord (\<lambda>(x, y). r x y)"
  (is "?lhs \<longleftrightarrow> ?rhs")
proof
  assume ?lhs
  { fix zs xs' y ys'
    ...
    next
      ...
      thus ?thesis ..
    qed
    hence "(xs, ys) \<in> {(x, y). \<exists>a v. y = x @ a # v \<or> (\<exists>u a b v w. (a, b) \<in> (\<lambda>(x, y). r x y) \<and> x = u @ a # v \<and> y = u @ b # w)}"
      by(simp add: split_def)(simp add: mem_def) }
  with `?lhs` show ?rhs unfolding lexord_def llexord_conv llist_of_inj by blast
next
  assume "?rhs"
  thus ?lhs
  proof
  ...
  next
    ...
    ultimately have "(xs', ys') \<in> {(llist_of xs, llist_of ys)|xs ys. (xs, ys) \<in> lexord (\<lambda>(x, y). r x y)}" by blast
    thus "llexord r xs' ys'"
    proof(coinduct)
      case (llexord xs' ys')
...
subsection {* More lemmas about filtering lazy lists with @{term "lfilter"} *}
...
proof -
  from lfilter_eq_LCons[OF assms] obtain ys' 
    where xs: "xs = lfilter P ys'" and ys': "(ys, LCons x ys') \<in> findRel P" by blast
...
lemma lfilter_eq_lappend_lfiniteD:
  ...
  shows "\<exists>us vs. xs = lappend us vs \<and> lfinite us \<and> ys = lfilter P us \<and> zs = lfilter P vs"
using `lfinite ys` `lfilter P xs = lappend ys zs`
proof(induct arbitrary: xs zs)
  case lfinite_LNil
  hence "xs = lappend LNil xs" "LNil = lfilter P LNil" "zs = lfilter P xs" by simp_all
  thus ?case by blast
next
  case (lfinite_LConsI ys y)
  note IH = `\<And>xs zs. lfilter P xs = lappend ys zs \<Longrightarrow>
            \<exists>us vs. xs = lappend us vs \<and> lfinite us \<and> ys = lfilter P us \<and> zs = lfilter P vs`
  from `lfilter P xs = lappend (LCons y ys) zs`
  have "lfilter P xs = LCons y (lappend ys zs)" by simp
  from lfilter_eq_LConsD[OF this] obtain us vs
    where xs: "xs = lappend us (LCons y vs)" "lfinite us" "P y" "\<forall>u\<in>lset us. \<not> P u"
    and vs: "lfilter P vs = lappend ys zs" by auto
  from IH[OF vs] obtain us' vs' where "vs = lappend us' vs'" "lfinite us'"
    "ys = lfilter P us'" "zs = lfilter P vs'" by blast
  with xs show ?case
    by(fastsimp simp add: lappend_snocL1_conv_LCons2[symmetric, where ys="lappend us' vs'"] lappend_assoc[symmetric] lfilter_False)
qed
...
proof -
  ...
  proof(coinduct rule: inat_leI)
    ...
    next
      ...
      moreover then obtain y ys' where "ys = LCons y ys'" by(auto dest!: findRel_imp_LCons)
...
lemma lfinite_lfilter [simp]:
  "lfinite (lfilter P xs) \<longleftrightarrow> lfinite xs \<or> finite {n. Fin n < llength xs \<and> P (lnth xs n)}"
proof
  assume "lfinite (lfilter P xs)"
  { assume "\<not> lfinite xs"
    with `lfinite (lfilter P xs)` have "finite {n. Fin n < llength xs \<and> P (lnth xs n)}"
    proof(induct ys\<equiv>"lfilter P xs" arbitrary: xs)
      ...
      hence eq: "{n. Fin n < llength xs \<and> P (lnth xs n)} = {}" by(auto simp add: lset_def)
	...
	"\<forall>u\<in>lset us. \<not> P u" "P x" "ys = lfilter P vs" by blast
      from `lfinite us` obtain us' where "us = llist_of us'" unfolding lfinite_eq_range_llist_of by blast 
	...
	ultimately have "finite {n. n \<le> k \<and> P (lnth xs n)}" by(rule finite_subset) }
      ultimately have "finite ((\<lambda>m. Suc (m + k)) ` {n. Fin n < llength vs \<and> P (lnth vs n)} \<union> {n. n \<le> k \<and> P (lnth xs n)})" by simp
      moreover have "(\<lambda>m. Suc (m + k)) ` {n. Fin n < llength vs \<and> P (lnth vs n)} \<union> {n. n \<le> k \<and> P (lnth xs n)} = {n. Fin n < llength xs \<and> P (lnth xs n)}"
	unfolding k_def using `xs = lappend us (LCons x vs)` `us = llist_of us'`
	by(auto simp add: lnth_lappend_llist_of iSuc_def lnth_LCons split: inat.split)(force split: nat.splits)+
      ultimately show ?case by(simp)
    qed }
...
next
  ...
  moreover {
    ...
    next
      ...
      proof(induct A\<equiv>"{n. Fin n < llength xs \<and> P (lnth xs n)}" arbitrary: xs rule: finite_induct)
	case empty
	hence "lfilter P xs = LNil" by(simp add: lfilter_empty_conv lset_def)
	thus ?case by simp
      next
	case (insert n A)
	...
	have xs: "xs = lappend (ltake (Fin n) xs) (ldrop (Fin n) xs)" by(simp only: lappend_ltake_ldrop)
	from `llength xs = \<infinity>` have "\<not> lfinite xs" by(auto dest: lfinite_llength_Fin)
	hence "\<not> lfinite (ldropn n xs)" by(subst xs) simp
	then obtain X XS where "ldropn n xs = LCons X XS" by(cases "ldropn n xs") auto
	moreover have "lnth (ldropn n xs) 0 = lnth xs n" using `llength xs = \<infinity>` by(simp del: lnth.simps(1))
	...
	with xs have xs: "xs = lappend (ltake (Fin n) xs) (LCons (lnth xs n) (ldropn (Suc n) xs))" by simp
	...
	hence [simp]: "llength ?xs = \<infinity>" using `llength xs = \<infinity>` xs by(simp)
	{
	  ...
	  ultimately have "P (lnth xs m) \<longleftrightarrow> P (lnth ?xs m) \<or> m = n" by(cases "m = n") simp_all }
...
lemma lconcat_LCons [simp, code]: "lconcat (LCons ys xss) = lappend ys (lconcat xss)"
proof(cases "ys = LNil")
  case True thus ?thesis by(simp add: lconcat_def)
next
  case False
  hence "(lconcat (LCons ys xss), lappend ys (lconcat xss)) \<in> 
        {(lconcat (LCons ys xss), lappend ys (lconcat xss))|ys. ys \<noteq> LNil}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain ys where q: "q = (lconcat (LCons ys xss), lappend ys (lconcat xss))"
      and "ys \<noteq> LNil" by blast
    then obtain y ys' where ys: "ys = LCons y ys'" by(auto simp add: neq_LNil_conv)
    have "lconcat (LCons (LCons y ys') xss) = LCons y (lconcat (LCons ys' xss))"
      by(auto simp add: lconcat_def llist_corec)
    moreover {
      assume "ys' = LNil"
      hence "lconcat (LCons ys' xss) = lappend ys' (lconcat xss)" by(simp add: lconcat_def) 
...
lemma lconcat_llist_of: "lconcat (llist_of (map llist_of xs)) = llist_of (concat xs)"
by(induct xs) simp_all
...
proof -
  { fix xss :: "'a llist llist"
    ...
    hence "(lmap f (lconcat xss), lconcat (lmap (lmap f) xss)) \<in>
          {(lmap f (lconcat xss), lconcat (lmap (lmap f) xss))|xss. LNil \<notin> lset xss}" by blast
  ...
  also have "lconcat (lmap (lmap f) (lfilter (\<lambda>xs. xs \<noteq> LNil) xss)) = lconcat (lmap (lmap f) xss)"
...
lemma lsublist_LCons:
  "lsublist (LCons x xs) A = (if 0 \<in> A then LCons x (lsublist xs {n. Suc n \<in> A}) else lsublist xs {n. Suc n \<in> A})"
proof -
  { fix n
    have "(lzip xs (iterates Suc (Suc n)), lmap (\<lambda>(x, y). (x, Suc y)) (lzip xs (iterates Suc n))) \<in>
         {(lzip xs (iterates Suc (Suc n)), lmap (\<lambda>(x, y). (x, Suc y)) (lzip xs (iterates Suc n)))|xs n. True}" by blast
    hence "lzip xs (iterates Suc (Suc n)) = lmap (\<lambda>(x, y). (x, Suc y)) (lzip xs (iterates Suc n))"
    proof(coinduct rule: llist_equalityI)
      case (Eqllist q)
      then obtain n xs
	where q: "q = (lzip xs (iterates Suc (Suc n)), lmap (\<lambda>(x, y). (x, Suc y)) (lzip xs (iterates Suc n)))" by blast
      show ?case
      proof(cases xs)
	...
	thus ?thesis ..
      qed
    qed
    hence "lmap fst (lfilter (\<lambda>(x, y). y \<in> A) (lzip xs (iterates Suc (Suc n)))) =
           lmap fst (lfilter (\<lambda>(x, y). Suc y \<in> A) (lzip xs (iterates Suc n)))"
      by(simp add: lfilter_lmap lmap_compose[symmetric] o_def split_def del: lmap_compose) }
  thus ?thesis
    by(auto simp add: lsublist_def)(subst iterates, simp)+
qed

lemma lset_lsublist: "lset (lsublist xs I) = {lnth xs i|i. Fin i<llength xs \<and> i \<in> I}"
...
lemma lsublist_singleton [simp]: "lsublist (LCons x LNil) A = (if 0 : A then LCons x LNil else LNil)"
by (simp add: lsublist_LCons)

lemma lsublist_upt_eq_ltake [simp]: "lsublist xs {..<n} = ltake (Fin n) xs"
apply(rule sym)
proof(induct n arbitrary: xs)
  case 0 thus ?case by(simp add: zero_inat_def[symmetric])
next
  case (Suc n)
  thus ?case by(cases xs)(simp_all add: iSuc_Fin[symmetric] lsublist_LCons lessThan_def)
qed

lemma lsublist_llist_of [simp]: "lsublist (llist_of xs) A = llist_of (sublist xs A)"
...
proof -
  have "llength (lfilter (\<lambda>(x, y). y \<in> A) (lzip xs (iterates Suc 0))) \<le> llength (lzip xs (iterates Suc 0))"
    by(rule llength_lfilter_ile)
  thus ?thesis by(simp add: lsublist_def llength_lzip)
qed

lemma lsublist_lmap [simp]: "lsublist (lmap f xs) A = lmap f (lsublist xs A)"
by(simp add: lsublist_def lzip_lmap1 lmap_compose[symmetric] lfilter_lmap o_def split_def del: lmap_compose)
...
proof -
  have "lsublist xs {n. Fin n < llength xs \<and> P (lnth xs n)} =
        lmap fst (lfilter (\<lambda>(x, y). Fin y < llength xs \<and> P (lnth xs y)) (lzip xs (iterates Suc 0)))"
    by(simp add: lsublist_def)
  ...
  also have "lmap fst (lfilter (P \<circ> fst) (lzip xs (iterates Suc 0))) = lfilter P (lmap fst (lzip xs (iterates Suc 0)))"
    unfolding lfilter_lmap ..
  also have "lmap fst (lzip xs (iterates Suc 0)) = xs" by(simp add: lmap_fst_lzip_conv_ltake ltake_all)
  finally show ?thesis ..
qed
...
next
  case (Suc n)
  have "ltake (Fin (Suc n)) (iterates Suc m) = LCons m (ltake (Fin n) (iterates Suc (Suc m)))"
    by(subst iterates)(simp add: iSuc_Fin[symmetric])
  also note Suc
  also have "LCons m (llist_of [Suc m..<n + Suc m]) = llist_of [m..<Suc n+m]"
    unfolding llist_of.simps[symmetric] by(auto simp del: llist_of.simps simp add: upt_conv_Cons)
  finally show ?case .
qed

lemma lsublist_lappend_lfinite: 
  assumes len: "llength xs = Fin k"
  shows "lsublist (lappend xs ys) A = lappend (lsublist xs A) (lsublist ys {n. n + k \<in> A})"
proof -
  from assms have fin: "lfinite xs" by(rule llength_eq_Fin_lfiniteD)
  have "lsublist (lappend xs ys) A = 
    lmap fst (lfilter (\<lambda>(x, y). y \<in> A) (lzip (lappend xs ys) (iterates Suc 0)))"
    by(simp add: lsublist_def)
  also have "iterates Suc 0 = lappend (ltake (Fin k) (iterates Suc 0)) (ldrop (Fin k) (iterates Suc 0))"
    by(simp only: lappend_ltake_ldrop)
  also note lzip_lappend
  also note lfilter_lappend_lfinite
  also note lmap_lappend_distrib
  also have "lzip xs (ltake (Fin k) (iterates Suc 0)) = lzip xs (iterates Suc 0)"
    using len by(subst (1 2) lzip_conv_lzip_ltake_min_llength) simp
  also note lsublist_def[symmetric]
  also have "ldrop (Fin k) (iterates Suc 0) = iterates Suc k" by(simp add: ldropn_iterates)
  also { fix n m
    have "(iterates Suc (n + m), lmap (\<lambda>n. n + m) (iterates Suc n)) \<in>
         {(iterates Suc (n + m), lmap (\<lambda>n. n + m) (iterates Suc n))|n. True}" by blast
    hence "iterates Suc (n + m) = lmap (\<lambda>n. n + m) (iterates Suc n)"
    proof(coinduct rule: llist_equalityI)
      case (Eqllist q)
      then obtain n where q: "q = (iterates Suc (n + m), lmap (\<lambda>n. n + m) (iterates Suc n))" by blast
      have ?EqLCons unfolding q by(subst (1 2) iterates) force
      thus ?case ..
    qed }
  from this[of 0 k] have "iterates Suc k = lmap (\<lambda>n. n + k) (iterates Suc 0)" by simp
  also note lzip_lmap2
  also note lfilter_lmap
  also note lmap_compose[symmetric]
  also have "fst \<circ> (\<lambda>(x, y). (x, y + k)) = fst" by(simp add: o_def split_def)
...
lemma lsublist_split:
  "lsublist xs A = lappend (lsublist (ltake (Fin n) xs) A) (lsublist (ldropn n xs) {m. n + m \<in> A})"
...
next
  ...
  hence "lsublist xs A = lsublist (lappend (ltake (Fin n) xs) (ldropn n xs)) A" by(simp)
...
proof -
  have "lfilter (\<lambda>(x, y). y \<in> A) (lzip ys (iterates Suc 0)) = lfilter (\<lambda>(x, y). y \<in> B) (lzip ys (iterates Suc 0))"
...
lemma lsublist_insert:
  ...
  shows "lsublist xs (insert n A) = lappend (lsublist (ltake (Fin n) xs) A) (LCons (lnth xs n) (lsublist (ldropn (Suc n) xs) {m. Suc (n + m) \<in> A}))"
proof -
  have "lsublist xs (insert n A) = lappend (lsublist (ltake (Fin n) xs) (insert n A)) (lsublist (ldropn n xs) {m. n + m \<in> (insert n A)})"
    by(rule lsublist_split)
  also have "lsublist (ltake (Fin n) xs) (insert n A) = lsublist (ltake (Fin n) xs) A" by(rule lsublist_cong[OF refl]) simp
  also { from n obtain X XS where "ldropn n xs = LCons X XS"
      by(cases "ldropn n xs")(auto simp add: ldropn_eq_LNil)
    moreover have "lnth (ldropn n xs) 0 = lnth xs n" using n by(simp del: lnth.simps(1))
    moreover have "ltl (ldropn n xs) = ldropn (Suc n) xs"
      by(cases xs)(simp_all add: ltl_ldropn del: ldropn_LCons)
    ultimately have "ldropn n xs = LCons (lnth xs n) (ldropn (Suc n) xs)" by simp
    hence "lsublist (ldropn n xs) {m. n + m \<in> insert n A} = LCons (lnth xs n) (lsublist (ldropn (Suc n) xs) {m. Suc (n + m) \<in> A})"
      by(simp add: lsublist_LCons) }
  finally show ?thesis .
qed
...
proof
  ...
  hence "lfinite xs \<or> finite {n. Fin n < llength xs \<and> (\<lambda>(x, y). y \<in> A) (lnth (lzip xs (iterates Suc 0)) n)}"
  ...
  finally show "lfinite xs \<or> finite A" by(auto simp add: not_lfinite_llength elim: contrapos_np)
next
  assume "lfinite xs \<or> finite A"
  moreover
  have "{n. Fin n < llength xs \<and> (\<lambda>(x, y). y \<in> A) (lnth (lzip xs (iterates Suc 0)) n)} =
        {n. Fin n < llength xs \<and> n \<in> A}" by(auto simp add: lnth_lzip)
  ultimately show "lfinite (lsublist xs A)" by(auto simp add: lsublist_def llength_lzip)
qed  


subsection {* 
  Alternative view on @{typ "'a llist"} as datatype with constructors @{term "llist_of"} and @{term "inf_llist"}
*}
...
proof -
  have "(lzip (inf_llist f) (inf_llist g), inf_llist (\<lambda>n. (f n, g n))) \<in> 
       {(lzip (inf_llist f) (inf_llist g), inf_llist (\<lambda>n. (f n, g n)))|f g. True}" by blast
  thus ?thesis
  proof(coinduct rule: llist_equalityI)
    case (Eqllist q)
    then obtain f g where q: "q = (lzip (inf_llist f) (inf_llist g), inf_llist (\<lambda>n. (f n, g n)))" by blast
