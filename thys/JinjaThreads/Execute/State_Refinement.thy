...
types
  ('l,'t,'m,'m_t,'m_w) state_refine = "('l,'t) locks \<times> ('m_t \<times> 'm) \<times> 'm_w"
...
inductive state_invar :: "('l,'t,'m,'m_t,'m_w) state_refine \<Rightarrow> bool"
where "\<lbrakk> thr_invar ts; ws_invar ws \<rbrakk> \<Longrightarrow> state_invar (ls, (ts, m), ws)"

inductive_simps state_invar_simps [simp]:
  "state_invar (ls, (ts, m), ws)"

lemma state_invarD [simp]:
  assumes "state_invar s"
  shows "thr_invar (thr s)" "ws_invar (wset s)"
using assms by(case_tac [!] s) auto

fun state_\<alpha> :: "('l,'t,'m,'m_t,'m_w) state_refine \<Rightarrow> ('l,'t,'x,'m,'w) state"
where "state_\<alpha> (ls, (ts, m), ws) = (ls, (thr_\<alpha> ts, m), ws_\<alpha> ws)"
...
definition (in heap_base) start_state_refine :: "'m_t \<Rightarrow> (thread_id \<Rightarrow> ('x \<times> addr released_locks) \<Rightarrow> 'm_t \<Rightarrow> 'm_t) \<Rightarrow> 'm_w \<Rightarrow> (cname \<Rightarrow> mname \<Rightarrow> ty list \<Rightarrow> ty \<Rightarrow> 'md \<Rightarrow> val list \<Rightarrow> 'x) \<Rightarrow> 'md prog \<Rightarrow> cname \<Rightarrow> mname \<Rightarrow> val list \<Rightarrow> (addr, thread_id, 'heap, 'm_t, 'm_w) state_refine"
where
  "start_state_refine thr_empty thr_update ws_empty f P C M vs =
  (let (D, Ts, T, m) = method P C M
   in (\<lambda>\<^isup>f None, (thr_update start_tid (f D M Ts T m vs, no_wait_locks) thr_empty, start_heap), ws_empty))"

definition Jinja_output :: "'s \<Rightarrow> thread_id \<Rightarrow> (addr, addr, 'x, 'heap, addr, obs_event list) thread_action \<Rightarrow> obs_event list option"
where "Jinja_output \<sigma> t ta = (if \<lbrace>ta\<rbrace>\<^bsub>o\<^esub> = [] then None else Some \<lbrace>ta\<rbrace>\<^bsub>o\<^esub>)"
