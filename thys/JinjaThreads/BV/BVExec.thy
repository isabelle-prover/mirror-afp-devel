...
definition kiljvm :: "'addr jvm_prog \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> ty \<Rightarrow> 
                    'addr instr list \<Rightarrow> ex_table \<Rightarrow> ty\<^isub>i' err list \<Rightarrow> ty\<^isub>i' err list"
where
...
definition wf_jvm_prog\<^isub>k :: "'addr jvm_prog \<Rightarrow> bool"
where
