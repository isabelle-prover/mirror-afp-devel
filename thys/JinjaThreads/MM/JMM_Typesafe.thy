...
proof -
  ...
  next
    ...
    have ns1: "non_speculative P (vs_type_all P) (llist_of (concat (map (\<lambda>(t, ta). \<lbrace>ta\<rbrace>\<^bsub>o\<^esub>) (list_of ?m_E''))))"
      ...
      by(simp_all add: lmap_lappend_distrib non_speculative_lappend split_beta lconcat_llist_of[symmetric] lmap_lconcat lmap_compose[symmetric] o_def split_def list_of_lmap[symmetric] del: lmap_compose list_of_lmap)
    ...
    have "concat (map (\<lambda>(t, ta). \<lbrace>ta\<rbrace>\<^bsub>o\<^esub>) (list_of ?m_E'')) = map snd (list_of (lconcat (lmap (\<lambda>(t, ta). llist_of (map (Pair t) \<lbrace>ta\<rbrace>\<^bsub>o\<^esub>)) ?m_E'')))"
      by(simp add: split_def lmap_lconcat lmap_compose[symmetric] o_def list_of_lconcat map_concat del: lmap_compose)
    ...
    hence "\<dots> \<in> lset ?EE''" using `enat w' < llength ?EE''` unfolding lset_def by(auto intro: rev_image_eqI)
...
proof(induction a arbitrary: ad al v rule: less_induct)
  ...
  next
    ...
    next
      ...
      have prefix: "lprefix ?EE'' (lmap snd E')" unfolding E''
        by(subst (2) E'_unfold)(simp add: lmap_lappend_distrib  lmap_lconcat lmap_compose[symmetric] o_def split_def lconcat_llist_of[symmetric] lmap_llist_of[symmetric] lprefix_lappend2I del: lmap_compose lmap_llist_of)
