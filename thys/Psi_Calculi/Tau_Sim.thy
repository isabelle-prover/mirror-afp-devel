...
using `\<Psi> \<rhd> \<alpha>\<cdot>P \<longmapsto>\<beta> \<prec> P'`
proof(nominal_induct rule: prefix.strong_induct)
  case(pInput M xvec N)
  from `\<Psi> \<rhd> (pInput M xvec N)\<cdot>P \<longmapsto>\<beta> \<prec> P'` have "\<Psi> \<rhd> M\<lparr>\<lambda>*xvec N\<rparr>.P \<longmapsto>\<beta> \<prec> P'" by simp
  thus ?case by(auto elim: inputCases intro: rInput)
next
  case(pOutput M N)
  from `\<Psi> \<rhd> (pOutput M N)\<cdot>P \<longmapsto>\<beta> \<prec> P'` have "\<Psi> \<rhd> M\<langle>N\<rangle>.P \<longmapsto>\<beta> \<prec> P'" by simp
  thus ?case by(auto elim: outputCases intro: rOutput)
next
  case pTau
  from `\<Psi> \<rhd> (pTau)\<cdot>P \<longmapsto>\<beta> \<prec> P'` have "\<Psi> \<rhd> \<tau>.(P) \<longmapsto>\<beta> \<prec> P'" by simp
  thus ?case
...
lemma prefixTauCases[consumes 1, case_names cTau]:
  ...
  shows "Prop \<alpha> P'"
proof -
  from `\<Psi> \<rhd> \<alpha>\<cdot>P \<longmapsto>\<tau> \<prec> P'` obtain \<beta> where "\<Psi> \<rhd> \<alpha>\<cdot>P \<longmapsto>\<beta> \<prec> P'" and "\<beta> = \<tau>" 
    by auto
  thus ?thesis
...
proof(induct rule: weakCongSimI)
  case(cTau Q')
  from `\<Psi> \<rhd> P \<leadsto><Rel> Q` `\<Psi> \<rhd> Q \<longmapsto>\<tau> \<prec> Q'`
...
proof(induct rule: weakCongSimI)
  case(cTau Q')
  from `\<Psi> \<rhd> \<tau>.(Q) \<longmapsto>\<tau> \<prec> Q'` have "\<Psi> \<rhd> Q \<sim> Q'" by(blast dest: tauActionE)
...
proof(induct rule: weakCongSimI)
  ...
  obtain P' where PChain: "\<Psi> \<rhd> P \<Longrightarrow>\<^sup>^\<^sub>\<tau> P'" and P'RelQ': "(\<Psi>, P', Q') \<in> Rel"
    by(blast dest: weakSimE)
  with C1 `\<Psi> \<rhd> Q \<longmapsto>\<tau> \<prec> Q'` show ?case 
...
proof(induct rule: weakSimI2)
  ...
  from `bn \<alpha> \<sharp>* (\<tau>.(P))` have "bn \<alpha> \<sharp>* P" by simp
  with `\<Psi> \<rhd> P \<leadsto><Rel> Q` `\<Psi> \<rhd> Q \<longmapsto>\<alpha> \<prec> Q'` `bn \<alpha> \<sharp>* \<Psi>` `\<alpha> \<noteq> \<tau>`
  ...
  from PTrans `bn \<alpha> \<sharp>* \<Psi>` `bn \<alpha> \<sharp>* P` obtain P''' where "\<Psi> : Q \<rhd> \<tau>.(P) \<Longrightarrow>\<alpha> \<prec> P'''" and "\<Psi> \<rhd> P'' \<sim> P'''"
    by(rule weakTransitionTau)
  moreover from `\<Psi> \<rhd> P'' \<sim> P'''` have "\<Psi> \<otimes> \<Psi>' \<rhd> P'' \<sim> P'''" by(rule bisimE)
  ...
  ultimately show ?case using `(\<Psi> \<otimes> \<Psi>', P', Q') \<in> Rel` by(metis bisimE C1)
next
  case(cTau Q')
  from `\<Psi> \<rhd> P \<leadsto><Rel> Q` `\<Psi> \<rhd> Q \<longmapsto>\<tau> \<prec> Q'`
  obtain P' where PChain: "\<Psi> \<rhd> P \<Longrightarrow>\<^sup>^\<^sub>\<tau> P'" and P'RelQ': "(\<Psi>, P', Q') \<in> Rel"
...
next
  ...
  moreover from `\<Psi> \<rhd> \<tau>.(Q) \<longmapsto>\<tau> \<prec> Q'` have "\<Psi> \<rhd> Q \<sim> Q'" by(rule tauActionE)
  with `(\<Psi>, P, Q) \<in> Rel` have "(\<Psi>, P, Q') \<in> Rel" by(rule C1)
...
proof(induct rule: weakSimI[where C=Q])
  ...
  from `\<Psi> \<rhd> \<alpha>\<cdot>Q \<longmapsto>\<beta> \<prec> Q'` `\<beta> \<noteq> \<tau>` show ?case
  proof(induct rule: prefixCases)
    case(cInput M xvec N K Tvec)
    note `\<Psi> \<turnstile> M \<leftrightarrow> K` `distinct xvec` `set xvec \<subseteq> supp N` `length xvec = length Tvec`
...
next
  case(cTau Q')
  from `\<Psi> \<rhd> \<alpha>\<cdot>Q \<longmapsto>\<tau> \<prec> Q'` show ?case
  proof(induct rule: prefixTauCases)
    ...
    obtain P'' where PTrans: "\<Psi> \<rhd> \<tau>.(P) \<longmapsto>\<tau> \<prec> P''" and "\<Psi> \<rhd> P \<sim> P''" using tauActionI
      by auto
    from PTrans `\<Psi> \<rhd> \<tau>.(P) \<sim> P'` obtain P''' where P'Trans: "\<Psi> \<rhd> P' \<longmapsto>\<tau> \<prec> P'''" and "\<Psi> \<rhd> P'' \<sim> P'''"
...
proof(induct rule: weakSimI[where C=Q])
  case(cAct \<Psi>' \<beta> Q')
  from `\<Psi> \<rhd> \<alpha>\<cdot>(\<tau>.(Q)) \<longmapsto>\<beta> \<prec> Q'` `\<beta> \<noteq> \<tau>`
  show ?case
  proof(induct rule: prefixCases)
    case(cInput M xvec N K Tvec)
    note `\<Psi> \<turnstile> M \<leftrightarrow> K` `distinct xvec` `set xvec \<subseteq> supp N` `length xvec=length Tvec`
...
next
  case(cTau Q')
  from `\<Psi> \<rhd> \<alpha>\<cdot>(\<tau>.(Q)) \<longmapsto>\<tau> \<prec> Q'` show ?case
  proof(induct rule: prefixTauCases)
    case cTau
    obtain P' where PTrans: "\<Psi> \<rhd> \<tau>.(P) \<longmapsto>\<tau> \<prec> P'" and "\<Psi> \<rhd> P \<sim> P'" using tauActionI
      by auto
...
next
  case(cTau P')
  from `\<Psi> \<rhd> \<tau>.(P) \<longmapsto>\<tau> \<prec> P'` have "\<Psi> \<rhd> P \<sim> P'" by(rule tauActionE)
  obtain P'' where PTrans: "\<Psi> \<rhd> \<tau>.(P) \<longmapsto>\<tau> \<prec> P''" and "\<Psi> \<rhd> P \<sim> P''" using tauActionI
    by auto
  ...
  moreover from `\<Psi> \<rhd> P \<sim> P''` `(\<Psi>, P, P) \<in> Rel` `\<Psi> \<rhd> P \<sim> P'` have "(\<Psi>, P'', P') \<in> Rel"
    by(metis C1 bisimE)
  ultimately show ?case by blast
qed  
...
proof(induct rule: weakSimI2)
  case(cAct \<Psi>' \<alpha> P')
  from `\<Psi> \<rhd> P \<oplus> \<tau>.(P) \<longmapsto>\<alpha> \<prec> P'`
  show ?case
  proof(induct rule: sumCases)
    ...
    obtain P'' where PTrans: "\<Psi> \<rhd> \<tau>.(P) \<longmapsto>\<tau> \<prec> P''" and "\<Psi> \<rhd> P \<sim> P''" using tauActionI
      by auto
    from PTrans have "\<Psi> \<rhd> \<tau>.(P) \<Longrightarrow>\<^sup>^\<^sub>\<tau> P''" by(rule tauActTauChain)
    moreover from `guarded P` have "insertAssertion (extractFrame P) \<Psi> \<simeq>\<^sub>F \<langle>\<epsilon>, \<Psi> \<otimes> \<one>\<rangle>"
      by(rule insertGuardedAssertion)
    with `\<Psi> \<rhd> P \<sim> P''` have "insertAssertion (extractFrame P'') \<Psi> \<simeq>\<^sub>F \<langle>\<epsilon>, \<Psi> \<otimes> \<one>\<rangle>"
    ...
    moreover from PTrans `bn \<alpha> \<sharp>* (\<tau>.(P))` have "bn \<alpha> \<sharp>* P''" by(rule tauFreshChainDerivative)
    with `\<Psi> \<rhd> P \<sim> P''` `\<Psi> \<rhd> P \<longmapsto>\<alpha> \<prec> P'` `bn \<alpha> \<sharp>* \<Psi>`
  ...
  next
    case cSum2
    thus ?case using `\<alpha> \<noteq> \<tau>`
      by(nominal_induct \<alpha> rule: action.strong_inducts) auto
  qed
next
  case(cTau P')
  from `\<Psi> \<rhd> P \<oplus> \<tau>.(P) \<longmapsto>\<tau> \<prec> P'`
  show ?case
  proof(induct rule: sumCases)
    case cSum1 
    obtain P'' where PTrans: "\<Psi> \<rhd> \<tau>.(P) \<longmapsto>\<tau> \<prec> P''" and "\<Psi> \<rhd> P \<sim> P''" using tauActionI
      by auto
    moreover from `\<Psi> \<rhd> P \<sim> P''` `\<Psi> \<rhd> P \<longmapsto>\<tau> \<prec> P'` 
    ...
    moreover from `\<Psi> \<rhd> P''' \<sim> P'` have "(\<Psi>, P''', P') \<in> Rel" by(rule C1)
    ultimately show ?case by blast
  next
    case cSum2
    from `\<Psi> \<rhd> \<tau>.(P) \<longmapsto>\<tau> \<prec> P'` have "\<Psi> \<rhd> P \<sim> P'" by(rule tauActionE)
    ...
    moreover from `\<Psi> \<rhd> P \<sim> P''` `\<Psi> \<rhd> P \<sim> P'` have "(\<Psi>, P'', P') \<in> Rel"
...
proof(induct rule: weakSimI2)
  ...
  from `\<Psi> \<rhd> \<alpha>\<cdot>(\<tau>.(P) \<oplus> Q) \<longmapsto>\<beta> \<prec> PQ` `\<beta> \<noteq> \<tau>`
  show ?case
  proof(induct rule: prefixCases)
    case(cInput M xvec N K Tvec)
    ...
    moreover from `\<Psi> \<turnstile> M \<leftrightarrow> K` `distinct xvec` `set xvec \<subseteq> supp N` `length xvec=length Tvec`
  ...
  next
    ...
    moreover from `\<Psi> \<turnstile> M \<leftrightarrow> K` have "\<Psi> \<rhd> M\<langle>N\<rangle>.(\<tau>.(P) \<oplus> Q) \<longmapsto>K\<langle>N\<rangle> \<prec> (\<tau>.(P) \<oplus> Q)"
      by(rule Output)
...
next
  case(cTau Q')
  from `\<Psi> \<rhd> \<alpha>\<cdot>(\<tau>.(P)) \<oplus>\<^sub>\<top> Q \<longmapsto> \<tau> \<prec> Q'` show ?case
  proof(induct rule: prefixTauCases)
    case cTau
...
proof(induct rule: weakSimI2)
  case(cAct \<Psi>' \<beta> PQ)
  from `\<Psi> \<rhd> \<alpha>\<cdot>P \<oplus> \<alpha>\<cdot>(\<tau>.(P) \<oplus> Q) \<longmapsto>\<beta> \<prec> PQ` show ?case
  proof(induct rule: sumCases)
    case cSum1
    from `\<Psi> \<rhd> \<alpha>\<cdot>P \<longmapsto>\<beta> \<prec> PQ` `\<beta> \<noteq> \<tau>` show ?case
    proof(induct rule: prefixCases)
      case(cInput M xvec N K Tvec)
    ...
    next
      ...
      moreover have "insertAssertion (extractFrame(M\<langle>N\<rangle>.P \<oplus> M\<langle>N\<rangle>.(\<tau>.(P) \<oplus> Q))) \<Psi> \<hookrightarrow>\<^sub>F insertAssertion (extractFrame(M\<langle>N\<rangle>.(\<tau>.(P) \<oplus> Q))) \<Psi>"
        by auto
      moreover from `\<Psi> \<turnstile> M \<leftrightarrow> K` have "\<Psi> \<rhd> M\<langle>N\<rangle>.(\<tau>.(P) \<oplus> Q) \<longmapsto>K\<langle>N\<rangle> \<prec> (\<tau>.(P) \<oplus> Q)" by(rule Output)
  ...
  next
    case cSum2
    from `\<Psi> \<rhd> \<alpha>\<cdot>(\<tau>.(P) \<oplus> Q) \<longmapsto>\<beta> \<prec> PQ` `\<beta> \<noteq> \<tau>` show ?case
    proof(induct rule: prefixCases)
      case(cInput M xvec N K Tvec)
      have "\<Psi> \<rhd> M\<lparr>\<lambda>*xvec N\<rparr>.(\<tau>.(P) \<oplus> Q) \<Longrightarrow>\<^sup>^\<^sub>\<tau> M\<lparr>\<lambda>*xvec N\<rparr>.(\<tau>.(P) \<oplus> Q)" by auto
      moreover have "insertAssertion (extractFrame((M\<lparr>\<lambda>*xvec N\<rparr>.P) \<oplus> M\<lparr>\<lambda>*xvec N\<rparr>.(\<tau>.(P) \<oplus> Q))) \<Psi> \<hookrightarrow>\<^sub>F insertAssertion (extractFrame(M\<lparr>\<lambda>*xvec N\<rparr>.(\<tau>.(P) \<oplus> Q))) \<Psi>"
...
next
  case(cTau PQ)
  from `\<Psi> \<rhd> \<alpha>\<cdot>P \<oplus> \<alpha>\<cdot>(\<tau>.(P) \<oplus> Q) \<longmapsto>\<tau> \<prec> PQ` show ?case
  proof(induct rule: sumCases)
    case cSum1
    from `\<Psi> \<rhd> \<alpha>\<cdot>P \<longmapsto> \<tau> \<prec> PQ` show ?case
    proof(induct rule: prefixTauCases)
      case cTau
      obtain P' where PTrans: "\<Psi> \<rhd> \<tau>.((\<tau>.(P)) \<oplus> Q) \<longmapsto>\<tau> \<prec> P'" and "\<Psi> \<rhd> (\<tau>.(P)) \<oplus> Q \<sim> P'" using tauActionI
        by auto
      ...
      with `\<Psi> \<rhd> (\<tau>.(P)) \<oplus> Q \<sim> P'` obtain P''' where P''Trans: "\<Psi> \<rhd> P' \<longmapsto>\<tau> \<prec> P'''" and "\<Psi> \<rhd> P'' \<sim> P'''"
        apply(drule_tac bisimE(4))
        apply(drule_tac bisimE(2))
  ...
  next
    case cSum2
    from `\<Psi> \<rhd> \<alpha>\<cdot>((\<tau>.(P)) \<oplus> Q) \<longmapsto>\<tau> \<prec> PQ` show ?case
    proof(induct rule: prefixTauCases)
      case cTau
      obtain P' where PTrans: "\<Psi> \<rhd> \<tau>.((\<tau>.(P)) \<oplus> Q) \<longmapsto>\<tau> \<prec> P'" and "\<Psi> \<rhd> (\<tau>.(P)) \<oplus> Q \<sim> P'" using tauActionI
        by auto
