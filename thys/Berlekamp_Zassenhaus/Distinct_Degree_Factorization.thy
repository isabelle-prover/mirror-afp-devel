...
lemma dist_degree_factorize_main: assumes 
  ...
  deg: "\<And> f. irreducible f \<Longrightarrow> f dvd v \<Longrightarrow> degree f > d" 
shows "u = prod_list (map snd facts)" 
  using dist w prod
proof (induct v w d res rule: dist_degree_factorize_main.induct)
  ...
  proof (cases "v = 1") 
    case True
    thus ?thesis using res u by auto
  next
    ...
    proof (cases "degree v < d + d")
      case True
      with v res u show ?thesis by auto
    next
      case False
      ...
      let ?v = "v div ?g" 
      let ?d = "Suc d" 
      from res[simplified] v False
      have res: "(if ?g = 1 then dist_degree_factorize_main v ?w ?d res
      ...
      proof (cases "?g = 1")
        case True
        with res have "dist_degree_factorize_main v ?w ?d res = facts" by auto
        from IH(1)[OF True this ww u] show ?thesis .
      next
        case False
        with res have res: "dist_degree_factorize_main ?v (?w mod ?v) ?d ((?d, ?g) # res) = facts" 
          by auto 
        have ww: "?w mod ?v = monom 1 1 ^ p ^ ?d mod ?v" unfolding ww
          by (rule mod_mod_cancel[OF gv']) 
        have u: "u = ?v * prod_list (map snd ((?d, ?g) # res))" 
          unfolding u by simp
        from IH(2)[OF False refl res ww u] show ?thesis .
      qed
    qed
  qed
qed
