...
theorem T3: "\<lfloor>\<^bold>\<box>(\<^bold>\<exists>x. G x)\<rfloor>" 
  ...
  proof -
    have 1: "\<lfloor>(G x \<^bold>\<supset> NE x) \<^bold>\<and> (G Ess. x \<^bold>\<supset> \<^bold>\<box>(\<^bold>\<exists>x. G x))\<rfloor>" using A5 Ess_def God_def NecExist_def by smt
    hence 2: "\<lfloor>(\<^bold>\<exists>x. G x) \<^bold>\<supset> \<^bold>\<box>(\<^bold>\<exists>x. G x)\<rfloor>" using A5 God_def NecExist_def T2 by smt
    hence 3: "\<lfloor>\<^bold>\<diamond>(\<^bold>\<exists>x. G x) \<^bold>\<supset> (\<^bold>\<diamond>(\<^bold>\<box>(\<^bold>\<exists>x. G x)) \<^bold>\<supset> \<^bold>\<box>(\<^bold>\<exists>x. G x))\<rfloor>" using Rsymm by blast
...
lemma Filter: "\<lfloor>FilterP P\<rfloor>" using A1 God_def Rsymm T1 T3 by (smt (verit, best))
lemma UltraFilter: "\<lfloor>UFilterP P\<rfloor>" using Filter A1 by blast
lemma True nitpick[satisfy,card=1,eval="\<lfloor>P (\<lambda>x.\<^bold>\<bottom>)\<rfloor>"] oops \<comment>\<open>One model found of cardinality one\<close>
