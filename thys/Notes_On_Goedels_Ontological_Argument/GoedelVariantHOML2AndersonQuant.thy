subsection\<open>GoedelVariantHOML2AndersonQuan.thy\<close>
text\<open>The same as GoedelVariantHOML2, but now for a mixed use of actualist and possibilist quantifiers 
...
theorem Th4: "\<lfloor>\<^bold>\<diamond>(\<^bold>\<exists>\<^sup>Ex. G x)\<rfloor>" using Ax2a Ax4 L Rsymm Th2 by metis
