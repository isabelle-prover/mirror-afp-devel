subsection\<open>HOMLinHOLonlyK.thy (slight variation of Figure 3 of \cite{J75})\<close>
text\<open>Shallow embedding of higher-order modal logic (HOML) in the classical higher-order logic (HOL)
of Isabelle/HOL utilizing the LogiKEy methodology. Here logic K is introduced.\<close>
...
consts R::"i\<Rightarrow>i\<Rightarrow>bool" ("_\<^bold>r_") \<comment>\<open>Accessibility relation between worlds\<close>
 (* axiomatization where Rrefl: "\<forall>x. x\<^bold>rx" and Rsymm: "\<forall>x y. x\<^bold>ry \<longrightarrow> y\<^bold>rx" and Rtrans: "\<forall>x y z. x\<^bold>ry \<and> y\<^bold>rz \<longrightarrow> x\<^bold>rz" *)
