(*  Title:       Countable Ordinals
    ID:          $Id: OrdinalVeblen.thy,v 1.2 2005-11-16 15:19:47 lsf37 Exp $
...
"0 < z \<Longrightarrow> range (oVeblen F z) = {x. \<forall>y<z. oVeblen F y x = x}"
 ...
 apply (rule_tac A="critical_set (range F) (f (Suc n))" in subsetD)
  apply (rule critical_set_mono)
  apply (simp add: strict_mono_monoD)
 apply (drule_tac x="Suc n" in spec, drule mp)
  apply (rule_tac y="f n" in order_le_less_trans, simp)
