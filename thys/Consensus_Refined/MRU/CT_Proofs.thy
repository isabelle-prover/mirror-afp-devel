...
proof(clarsimp simp add: PO_rhoare_defs CT_trans_step_def all_conj_distrib)
  ...
  def cand \<equiv> "commt (sc' (coord (Suc r)))"
  def C \<equiv> "{cand}"
...
proof(clarsimp  simp add: PO_rhoare_defs CT_trans_step_def all_conj_distrib)
  ...
  def v \<equiv> "commt (sc (coord r))"
  def S \<equiv> "{p. coord r \<in> HOs r p}"
...
proof -
  ...
  from   
    ...
    apply (metis One_nat_def Suc_eq_plus1 add_Suc_right comm_semiring_1_class.normalizing_semiring_rules(24) mod_Suc 
      mod_Suc_eq_Suc_mod mod_mult_self2 n_not_Suc_n)
    done
