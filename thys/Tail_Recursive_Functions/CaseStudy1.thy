...
@{text l_sort} performing insertion sort on lists of elements of a linear order,
...
properties of function @{text l_sort_naive}:
...
@{text "l_sorted (l_sort_naive xs [] [])"}

\null

@{text "l_count x (l_sort_naive xs [] []) = l_count x xs"}
...
which is hopeless as the formula is false for any unsorted list @{text zs}.
...
Since the significant inputs of function @{text l_sort_naive} match pattern
@{text _}, @{text "[]"}, @{text "[]"}, those of function @{text l_sort_aux}
match pattern @{text "(_, [], [])"}, thus being in a one-to-one correspondence
...
@{text "l_sorted (l_sort xs)"}

\null

@{text "l_count x (l_sort xs) = l_count x xs"}
*}
...
@{text l_sort_aux} will equivalently be associated to two distinct introduction
rules in the definition of the inductive set @{text l_sort_set}, one for either
...
next
  ...
  proof (cases "x \<le> z", simp_all)
    ...
    moreover have "l_sort_aux (xs, [], ys @ x # z # zs) \<in>
     l_sort_set (xs, [], ys @ x # z # zs)" using case1 and `x \<le> z` by simp
  ...
  next
    ...
    moreover have "l_sort_aux (x # xs, ys @ [z], zs) \<in>
     l_sort_set (x # xs, ys @ [z], zs)" using case2 and `\<not> x \<le> z` by simp
...
second equation in the definition of function @{text l_sort_aux}, i.e. the one
...
@{text "'a l_type \<Rightarrow> bool"} according to the method specification, shall be
comprised of function @{text "l_sort_inv_2 x xs"}, where @{text x}, @{text xs}
are the free variables appearing in the latter target theorem.

Both of the above definitions are non-recursive; command @{text fun} is used in
