...
(*
locale remove_alt_card_stratproof_weak_prop_rep_anon_papp =
  card_stratproof_weak_prop_rep_anon_papp n_voters parties committee_size r
  for n_voters and parties :: "'a set" and committee_size r +
  fixes x y :: 'a
  assumes xy: "x \<in> parties" "y \<in> parties" "x \<noteq> y"
begin

sublocale remove_alt_card_stratproof_weak_rep_anon_papp
  by standard (use xy in auto)

sublocale lowered: weak_prop_rep_anon_papp n_voters "parties - {y}" committee_size lowered
proof
  fix A z l
  assume A: "lowered.is_pref_profile A" and z: "l * n_voters \<le> committee_size * count A {z}"
  interpret A: anon_papp_profile n_voters "parties - {y}" committee_size A
    by fact
  ...
  next
    ...
    have "committee_size > 0"
      using z n_voters_pos l by (intro Nat.gr0I) auto
    ...
    proof (cases "z = x")
      case False
      have "l * n_voters \<le> committee_size * count A {z}"
        by fact
      also have "count A {z} \<le> count (lift_profile A) {z}"
        using A.A_subset z' False by (subst count_lift_profile) auto
      hence "committee_size * count A {z} \<le> committee_size * count (lift_profile A) {z}"
        ...
        by (intro weak_proportional_representation A')
      thus ?thesis
        using False z' by (simp add: lowered_def count_lower_result)
    next
      case [simp]: True
      show ?thesis
        sorry
