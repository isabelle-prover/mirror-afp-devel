...
where "lexord_fusion g1 g2 s1 s2 = ord_class.lexordp (unstream g1 s1) (unstream g2 s2)"

definition lexord_eq_fusion :: "('a, 's1) generator \<Rightarrow> ('a, 's2) generator \<Rightarrow> 's1 \<Rightarrow> 's2 \<Rightarrow> bool"
where "lexord_eq_fusion g1 g2 s1 s2 = lexordp_eq (unstream g1 s1) (unstream g2 s2)"
...
text \<open>Stream version based on Coutts \<^cite>\<open>"Coutts2010PhD"\<close>.\<close>
...
text \<open>
  ...
  \<^cite>\<open>"FarmerHoenerGill2014PEPM"\<close>, this fusion is too general for further optimisations because the
  ...
  (More precisely, Coutts already mentions this approach in his PhD thesis \<^cite>\<open>"Coutts2010PhD"\<close>,
