...
definition sumlist
  where "sumlist xs \<equiv> foldr (op \<oplus>) xs \<zero>"
...
definition summset where "summset M \<equiv> fold_mset (op \<oplus>) \<zero> M"

lemma summset_empty [simp]: "summset {#} = \<zero>" by (simp add: summset_def)

lemma fold_mset_add_carrier: "a \<in> carrier G \<Longrightarrow> set_mset M \<subseteq> carrier G \<Longrightarrow> fold_mset op \<oplus> a M \<in> carrier G" 
...
lemma sumlist_as_fold:
  assumes "set xs \<subseteq> carrier G"
  shows "sumlist xs = fold (op \<oplus>) xs \<zero>"
...
lemma sqrt_int_ceiling_bound: "0 \<le> x \<Longrightarrow> x \<le> (sqrt_int_ceiling x)^2"
  unfolding sqrt_int_ceiling using le_of_int_ceiling of_int_le_iff sqrt_le_D by fastforce
...
proof -
  ...
  thus ?thesis  
    using assms unfolding cong_int_def 
...
abbreviation cscalar_prod :: "'a vec \<Rightarrow> 'a vec \<Rightarrow> 'a :: conjugatable_ring" (infix "\<bullet>c" 70)
  where "op \<bullet>c \<equiv> \<lambda>v w. v \<bullet> conjugate w"
...
lemma sum_list_map_dropWhile0:
  assumes f0: "f 0 = 0"
  shows "sum_list (map f (dropWhile (op = 0) xs)) = sum_list (map f xs)"
  by (induct xs, auto simp add: f0)

lemma coeffs_poly_of_vec:
  "coeffs (poly_of_vec v) = rev (dropWhile (op = 0) (list_of_vec v))"
proof-
