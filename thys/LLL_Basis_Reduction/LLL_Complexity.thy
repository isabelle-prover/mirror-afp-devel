...
lemma basis_reduction_add_rows_cost: assumes "LLL_invariant state F G" 
...
lemma basis_reduction_step_cost: assumes "LLL_invariant state F G" 
...
function basis_reduction_main_cost :: "state \<Rightarrow> state cost" where
  "basis_reduction_main_cost state = (
     case state of (i,F,G) \<Rightarrow>
     if i < m \<and> (\<exists> FF GG. LLL_invariant state FF GG) 
...
proof (standard, rule wf_measure[of LLL_measure], goal_cases)
  ...
  from * obtain FF GG where i: "i < m" and inv: "LLL_invariant (i, F, G) FF GG" by auto
...
lemma basis_reduction_main_cost: fixes F G assumes "LLL_invariant state F G"
  shows "result (basis_reduction_main_cost state) = basis_reduction_main \<alpha> m state" (is ?g1) 
   "cost (basis_reduction_main_cost state) \<le> body_cost * num_loops" (is ?g2)
proof -
  have inv: "LLL_partial_invariant state F G" using assms unfolding LLL_invariant_def by auto
  ...
  proof (atomize (full), induct state arbitrary: F G rule: wf_induct[OF wf_measure[of LLL_measure]])
    case (1 state F G)
    note inv = 1(2)
    have ex: "(\<exists>FF. Ex (LLL_invariant state FF)) = True" using inv by auto
    ...
    proof (cases "i < m")
      ...
      note bsr = basis_reduction_step[OF inv i bb]
      from bsr(1) obtain F' G' where inv: "LLL_invariant state1 F' G'" by auto
  ...
  proof (rule mult_left_mono; linarith?)
    ...
    have "LLL_measure state \<le> nat (ceiling (m + k * l))" unfolding l_def k_def
      using LLL_measure_approx[OF inv[unfolded state] \<alpha>_gt m0, folded state] by linarith
    ...
    finally show "LLL_measure state \<le> num_loops" .
  qed
  finally show ?g2 . 
qed
...
lemma basis_reduction_state_cost: 
   "result (basis_reduction_state_cost fs_init) = basis_reduction_state n \<alpha> fs_init"  (is ?g1)
   "cost (basis_reduction_state_cost fs_init) \<le> initial_gso_cost + body_cost * num_loops" (is ?g2)
proof -
  ...
  have res: "basis_reduction_state_cost fs_init = (state2, c1 + c2)" 
  ...
  from initial_state[OF init]
  obtain F' G' where inv: "LLL_invariant state1 F' G'" by auto
  from basis_reduction_main_cost[OF inv, unfolded main cost_simps]
  have main: "basis_reduction_main \<alpha> m state1 = state2" and c2: "c2 \<le> body_cost * num_loops" 
    by auto
  have res': "basis_reduction_state n \<alpha> fs_init = state2" unfolding basis_reduction_state_def len init main ..
...
lemma reduce_basis_cost: 
   "result (reduce_basis_cost fs_init) = fst (reduce_basis n \<alpha> fs_init)"  (is ?g1)
   "cost (reduce_basis_cost fs_init) \<le> initial_gso_cost + body_cost * num_loops" (is ?g2)
proof -
  obtain state c where b: "basis_reduction_state_cost fs_init = (state,c)" (is "?b = _") by (cases ?b, auto)
  from basis_reduction_state_cost[unfolded b cost_simps]
  have bb: "basis_reduction_state n \<alpha> fs_init = state" and c: "c \<le> initial_gso_cost + body_cost * num_loops" 
    by auto
