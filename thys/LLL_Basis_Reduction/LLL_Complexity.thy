...
lemma basis_reduction_main_cost: fixes F G assumes "LLL_invariant True state F G"
...
proof -
  have inv: "LLL_invariant True state F G" using assms unfolding LLL_invariant_def by auto
  ...
  proof (atomize (full), induct state arbitrary: F G rule: wf_induct[OF wf_measure[of LLL_measure]])
    ...
    proof (cases "i < m")
      ...
      from bsr(1) obtain F' G' where inv: "LLL_invariant True state1 F' G'" by auto
...
proof -
  ...
  obtain F' G' where inv: "LLL_invariant True state1 F' G'" by auto
