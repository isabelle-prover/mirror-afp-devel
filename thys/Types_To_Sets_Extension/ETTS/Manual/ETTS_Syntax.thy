...
new (unnamed) tts context.
    \<^descr> @{element "tts"}~\<open>:\<close>~\<open>(?a\<^sub>1\<close> \<^theory_text>\<open>to\<close> \<open>U\<^sub>1)\<close> \<^theory_text>\<open>and\<close> \<open>\<dots>\<close> \<^theory_text>\<open>and\<close> 
...
For each \<open>i\<close> (\<open>1\<le>i\<le>n\<close>), \<open>?a\<^sub>i\<close> must be a schematic type variable that
...
is a fixed type variable.
    \<^descr> @{element "sbterms"}~\<open>:\<close>~\<open>(tbcv\<^sub>1\<close> \<^theory_text>\<open>to\<close> \<open>sbt\<^sub>1)\<close> \<^theory_text>\<open>and\<close> \<open>\<dots>\<close> \<^theory_text>\<open>and\<close>
...
For each individual entry \<open>i\<close>, such that \<open>1\<le>i\<le>n\<close>, \<open>tbcv\<^sub>i\<close> has to be either an
...
every theorem that is provided as an input to the command, and \<open>sbt\<^sub>i\<close> has to be
a term registered in the sbt-database.
    \<^descr> @{element "rewriting"} \<open>thm\<close> provides means for the declaration
of the rewrite rules for the set-based theorem.
    \<^descr> @{element "substituting"} \<open>thm\<^sub>1\<close> \<^theory_text>\<open>and\<close> \<open>\<dots>\<close> \<^theory_text>\<open>and\<close> \<open>thm\<^sub>n\<close> provides
means for the declaration of the known premises for the set-based theorem.
    \<^descr> @{element "eliminating"} \<open>term\<^sub>1\<close> \<^theory_text>\<open>and\<close> \<open>\<dots>\<close> \<^theory_text>\<open>and\<close> \<open>term\<^sub>n\<close> 
@{element "through"} \<open>method\<close> provides means for the declaration of
the specification of the elimination of premises in the set-based theorem.
    \<^descr> @{element "applying"} \<open>[attr\<^sub>1, \<dots>, attr\<^sub>n]\<close> provides means for 
...
description of the keyword @{element "in"} below. 
    ...
    \<^descr> @{element "in"} \<open>sbf\<^sub>1 = tbf\<^sub>1\<close> \<^theory_text>\<open>and\<close> \<open>\<dots>\<close> \<^theory_text>\<open>and\<close> \<open>sbf\<^sub>n = tbf\<^sub>n\<close> is used for
the specification of the type-based theorems and the output of the command.
For each individual entry \<open>i\<close>, such that \<open>1\<le>i\<le>n\<close>, \<open>tbf\<^sub>i\<close> is used for
