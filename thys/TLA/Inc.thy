...
locale Firstprogram =
  ...
  and phi :: "temporal"
  and Live :: "temporal"
  defines init: "init \<equiv> TEMP $x = # 0 \<and> $y = # 0"
  and m1 : "m1 \<equiv> TEMP x` = Suc<$x> \<and> y` = $y"
  and m2: "m2 \<equiv> TEMP y` = Suc<$y> \<and> x` = $x"
  and Live: "Live \<equiv> TEMP WF(m1)_(x,y) \<and> WF(m2)_(x,y)"
  and phi:  "phi \<equiv> TEMP (init \<and> \<box>[m1 \<or> m2]_(x,y) \<and> Live)"
  assumes bvar: "basevars (x,y)"

lemma (in Firstprogram) "STUTINV phi"
  by (auto simp: phi init m1 m2 Live stutinvs nstutinvs livestutinv)
...
locale Secondprogram = Firstprogram +
  ...
  and psi :: "temporal"
  and I :: "temporal"
  defines vars: "vars \<equiv> LIFT (x,y,sem,pc1,pc2)" 
  and initPsi: "initPsi \<equiv> TEMP $pc1 = # a \<and> $pc2 = # a \<and> $x = # 0 \<and> $y = # 0 \<and> $sem = # 1"
  and alpha1: "alpha1 \<equiv> TEMP $pc1 =#a \<and> # 0 < $sem \<and> pc1$ = #b \<and> sem$ = $sem - # 1 \<and> Unchanged (x,y,pc2)" 
  and alpha2: "alpha2 \<equiv> TEMP $pc2 =#a \<and> # 0 < $sem \<and> pc2` = #b \<and> sem$ = $sem - # 1 \<and> Unchanged (x,y,pc1)" 
  and beta1: "beta1 \<equiv> TEMP $pc1 =#b \<and> pc1` = #g \<and> x` = Suc<$x> \<and> Unchanged (y,sem,pc2)" 
  and beta2: "beta2 \<equiv> TEMP $pc2 =#b \<and> pc2` = #g \<and> y` = Suc<$y> \<and> Unchanged (x,sem,pc1)" 
  and gamma1: "gamma1 \<equiv> TEMP $pc1 =#g \<and> pc1` = #a \<and> sem` = Suc<$sem> \<and> Unchanged (x,y,pc2)"
  and gamma2: "gamma2 \<equiv> TEMP $pc2 =#g \<and> pc2` = #a \<and> sem` = Suc<$sem> \<and> Unchanged (x,y,pc1)"
  and n1: "n1 \<equiv> TEMP (alpha1 \<or> beta1 \<or> gamma1)"
  and n2: "n2 \<equiv> TEMP (alpha2 \<or> beta2 \<or> gamma2)"
  and Live2: "Live2 \<equiv> TEMP SF(n1)_vars \<and> SF(n2)_vars"
  and psi: "psi \<equiv> TEMP (initPsi \<and> \<box>[n1 \<or> n2]_vars \<and> Live2)"
  and I: "I \<equiv> TEMP ($sem = # 1 \<and> $pc1 = #a \<and> $pc2 = # a)
...
proof -
  have init: "\<turnstile> initPsi \<longrightarrow> I" by (auto simp: initPsi I)
  have "|~ I \<and> Unchanged vars \<longrightarrow> \<circ>I" by (auto simp: I vars_def tla_defs)
  moreover
  have "|~ I \<and> n1 \<longrightarrow> \<circ>I" by (auto simp: I Sact2_defs tla_defs)
  moreover
  have "|~ I \<and> n2 \<longrightarrow> \<circ>I" by (auto simp: I Sact2_defs tla_defs)
  ...
  have "\<turnstile> initPsi \<and> \<box>[n1 \<or> n2]_vars \<and> Live2 ==> \<turnstile> initPsi \<and> \<box>[n1 \<or> n2]_vars"
   by auto
  with goal show ?thesis unfolding psi by auto
qed
...
proof -
  ...
  proof (rule refinement1)
    show "\<turnstile> initPsi \<longrightarrow> init" by (auto simp: initPsi init)
  next
    show "|~ I \<and> \<circ>I \<and> [n1 \<or> n2]_vars \<longrightarrow> [m1 \<or> m2]_(x,y)"
      by (auto simp: I m1 m2 vars Sact2_defs tla_defs)
  qed
  with psiI show ?thesis unfolding psi by force
qed
...
proof -
  have "|~ n1 \<longrightarrow> \<langle>n1\<rangle>_vars" by (auto simp: Sact2_defs tla_defs vars)
...
proof (clarsimp simp: tla_defs)
  ...
  thus "s \<Turnstile> Enabled alpha1"
    by (intro base_enabled[OF bvar2]) (auto simp: Sact2_defs tla_defs vars)
qed
...
next
  ...
  proof (clarsimp simp: tla_defs)
    ...
    thus "s \<Turnstile> Enabled n1"
      ...
      by (cases "pc1 (s 0)") (force simp: n1 Enabled_disj[int_rewrite] tla_defs)+
  qed
qed
...
proof -
  have "|~ n2 \<longrightarrow> \<langle>n2\<rangle>_vars" by (auto simp: Sact2_defs tla_defs vars)
  thus ?thesis by (auto simp: angle_actrans_sem[int_rewrite])
qed
...
next
  ...
  proof (clarsimp simp: tla_defs)
    ...
    thus "s \<Turnstile> Enabled n2"
      ...
      by (cases "pc2 (s 0)") (force simp: n2 Enabled_disj[int_rewrite] tla_defs)+
  qed
qed
...
proof -
  ...
  next
    ...
    *}
    ...
    proof -
      ...
      proof (rule STL4)
        have "|~ $pc1 = #b \<and> [(n1 \<or> n2) \<and> \<not> beta1]_vars \<longrightarrow> \<circ>($pc1 = #b)"
          by (auto simp: Sact2_defs vars tla_defs)
        from this[THEN INV1]
        ...
        *}
        ...
        next
          ...
          proof -
            ...
            next
              show "|~ $pc2 = #g \<and> \<langle>((n1 \<or> n2) \<and> \<not>beta1) \<and> n2\<rangle>_vars \<longrightarrow> \<circ>($pc2 = #a)"
                by (auto simp: n2 alpha2 beta2 gamma2 vars tla_defs)
            next
          ...
          have "\<turnstile> \<diamond>($pc2 = #a \<and> $pc1 = #a \<and> I) \<longrightarrow> \<diamond>Enabled \<langle>n1\<rangle>_vars"
            unfolding enab_n1[int_rewrite] by (rule STL4_eve) (auto simp: I tla_defs)
          ultimately
    ...
    qed
  qed
  with psiI show ?thesis unfolding psi Live2 STL5[int_rewrite] by force
qed
        ...
        *}
        ...
        next
          ...
          proof -
            ...
            next
              show "|~ $pc1 = #b \<and> \<langle>((n1 \<or> n2) \<and> \<not>beta2) \<and> n1\<rangle>_vars \<longrightarrow> \<circ>($pc1 = #g)"
                by (auto simp: n1 alpha1 beta1 gamma1 vars tla_defs)
            next
          ...
          have "\<turnstile> \<diamond>($pc1 = #a \<and> $pc2 = #a \<and> I) \<longrightarrow> \<diamond>Enabled \<langle>n2\<rangle>_vars"
            unfolding enab_n2[int_rewrite] by (rule STL4_eve) (auto simp: I tla_defs)
          ultimately
