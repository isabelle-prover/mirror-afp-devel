...
``cannot be proven from the given assumptions'' \<^cite>\<open>Fitch\<close>. To formulate that, we need propositions that
reference their own provability, so just as Fitch builds on Gödel's work, we build on Paulson's
formalisation of Gödel's incompleteness theorems in Isabelle \<^cite>\<open>Incompleteness\<close>.
\<close>
