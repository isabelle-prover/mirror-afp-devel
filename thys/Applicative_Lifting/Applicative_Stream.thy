...
lemma ap_stream_id: "pure (\<lambda>x. x) \<diamond> x = x"
by (coinduction arbitrary: x) simp

lemma ap_stream_homo: "pure f \<diamond> pure x = pure (f x)"
by coinduction simp

lemma ap_stream_interchange: "f \<diamond> pure x = pure (\<lambda>f. f x) \<diamond> f"
by (coinduction arbitrary: f) auto

lemma ap_stream_composition: "pure (\<lambda>g f x. g (f x)) \<diamond> g \<diamond> f \<diamond> x = g \<diamond> (f \<diamond> x)"
