...
definition "size_mset M = fold_mset (op + \<circ> pair_size) 0 M"

interpretation size_mset_fun:
  comp_fun_commute "op + \<circ> pair_size"
  by standard auto

lemma fold_pair_size_plus:
  "fold_mset (op + \<circ> pair_size) 0 M + n = fold_mset (op + \<circ> pair_size) n M"
