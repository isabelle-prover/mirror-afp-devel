(* Title:     HOL/MiniML/Type.thy
   ID:        $Id: Type.thy,v 1.4 2004-04-19 22:23:43 lsf37 Exp $
...
lemma free_tv_nth_nat_A [rule_format (no_asm)]: 
...
lemma typ_substitutions_only_on_free_variables [rule_format (no_asm)]: 
...
lemma scheme_substitutions_only_on_free_variables [rule_format (no_asm)]: 
...
lemma free_tv_of_substitutions_extend_to_types [rule_format (no_asm)]: 
...
lemma free_tv_of_substitutions_extend_to_schemes [rule_format (no_asm)]: 
...
lemma free_tv_of_substitutions_extend_to_scheme_lists [rule_format (no_asm)]: 
...
lemma bound_tv_subst_scheme: 
  "!!sch::type_scheme. bound_tv ($ S sch) = bound_tv sch"
apply (induct_tac "sch")
apply (simp_all (no_asm_simp))
done
...
lemma bound_tv_subst_scheme_list: 
  "!!A::type_scheme list. bound_tv ($ S A) = bound_tv A"
...
lemma new_tv_subst_type_scheme [rule_format (no_asm)]: 
...
lemma new_tv_only_depends_on_free_tv_type_scheme [rule_format (no_asm)]: 
  "!!sch::type_scheme. (free_tv sch) = (free_tv sch') --> (new_tv n sch --> new_tv n sch')"
apply (unfold new_tv_def)
apply (simp (no_asm_simp))
done

lemma new_tv_only_depends_on_free_tv_scheme_list [rule_format (no_asm)]: 
  "!!A::type_scheme list. (free_tv A) = (free_tv A') --> (new_tv n A --> new_tv n A')"
apply (unfold new_tv_def)
apply (simp (no_asm_simp))
done
...
lemma subst_id_on_type_scheme_list': 
  "!!A::type_scheme list. !x : free_tv A. S x = (TVar x) ==> $ S A = $ id_subst A"
apply (rule scheme_list_substitutions_only_on_free_variables)
apply (simp add: id_subst_def)
done

lemma subst_id_on_type_scheme_list: 
  "!!A::type_scheme list. !x : free_tv A. S x = (TVar x) ==> $ S A = A"
apply (subst subst_id_on_type_scheme_list')
apply assumption
apply (simp (no_asm))
done

lemma nth_subst [rule_format (no_asm)]: 
