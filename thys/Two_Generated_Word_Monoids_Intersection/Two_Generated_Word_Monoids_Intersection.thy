...
theory Two_Generated_Word_Monoids_Intersection
  imports 
    ...
    Combinatorics_Words.Binary_Code_Morphisms 
...
proof-
  from criticalE 
  obtain pa qa where "\<alpha>\<^sub>g \<cdot> g\<^sub>m pa = \<alpha>\<^sub>h \<cdot> h\<^sub>m qa" and "\<And>p' q'. \<alpha>\<^sub>g \<cdot> g\<^sub>m p' = \<alpha>\<^sub>h \<cdot> h\<^sub>m q' \<Longrightarrow> pa \<le>p p' \<and> qa \<le>p q'" by metis
  ...
  have pq: "\<alpha>\<^sub>g \<cdot> g\<^sub>m p = \<alpha>\<^sub>h \<cdot> h\<^sub>m q" "\<alpha>\<^sub>g \<cdot> g\<^sub>m p' = \<alpha>\<^sub>h \<cdot> h\<^sub>m q' \<Longrightarrow> p \<le>p p' \<and> q \<le>p q'"
    unfolding fst_beginning_block_def snd_beginning_block_def beginning_block_def by fast+
...
proof-
  ...
  also have "... \<longleftrightarrow> \<alpha> \<cdot> g\<^sub>m p \<cdot> g\<^sub>m r' = h\<^sub>m q \<cdot> h\<^sub>m s'"
    unfolding begin_block marked.g.morph[symmetric] marked.h.morph[symmetric] assms..
...
lemma solution_ext_conv: "g r = h s \<longleftrightarrow> \<alpha> \<cdot> g\<^sub>m (r \<cdot> p) = h\<^sub>m (s \<cdot> q)"
  unfolding marked.g.morph marked.h.morph lassoc begin_block[symmetric]  cancel_right solution_marked_version_conv..
...
proof-
  ...
  from neq_induct[OF \<open>hd ?ua \<noteq> hd ?ub\<close>[folded \<open>hd ea = hd ?ua\<close> \<open>hd eb = hd ?ub\<close>] marked.block_ex[OF \<open>g\<^sub>m ea =\<^sub>m h\<^sub>m fa\<close>] marked.block_ex[OF \<open>g\<^sub>m eb =\<^sub>m h\<^sub>m fb\<close>]]
...
sublocale marked: two_binary_marked_blocks g\<^sub>m h\<^sub>m
  by (standard) (simp add: both_blocks)
...
next
  ...
  hence gp: "g\<^sub>m p \<cdot> g\<^sub>m (\<ee> \<tau>) = g\<^sub>m ((p\<cdot>(\<ee> \<tau>))\<^sup><\<inverse>p) \<cdot> g\<^sub>m p"
    unfolding marked.g.morph[symmetric] rq_suf[OF \<open>p \<le>s p \<cdot> (\<ee> \<tau>)\<close>] by blast 
  have hq: "h\<^sub>m q \<cdot> h\<^sub>m (\<ff> \<tau>) = h\<^sub>m ((q\<cdot>(\<ff> \<tau>))\<^sup><\<inverse>q) \<cdot> h\<^sub>m q"
    unfolding  marked.h.morph[symmetric] rq_suf[OF \<open>q \<le>s q \<cdot> (\<ff> \<tau>)\<close>] by blast
  from this
  show "g r = h s"
...
lemma  
  minblock0:  "g\<^sub>m (\<ee> \<zero>) =\<^sub>m h\<^sub>m (\<ff> \<zero>)" and
  minblock1:  "g\<^sub>m (\<ee> \<one>) =\<^sub>m h\<^sub>m (\<ff> \<one>)" and
  hdblock0:   "hd (\<ee> \<zero>)  = bin0" and
  hdblock1:   "hd (\<ee> \<one>) = bin1"
...
theorem simple_blocks_basis: assumes "\<And>a. coin_block [a]" 
  shows "\<BB> \<T> = {\<zero>, \<one>}"
  using  basis_of_hull[of "{\<zero>,\<one>}"] code.code_is_basis[OF bin_basis_code]
...
proof-
  ...
  hence "p \<bowtie>\<^sub>s \<ee> [c]" and "q \<bowtie>\<^sub>s \<ff> [c]"
    unfolding marked.sucs.g.morph marked.sucs.h.morph lassoc using ruler_pref''[reversed] by blast+
  have "\<not> \<ee> [c] \<le>s p"
  proof
    assume "\<ee> [c] \<le>s p"
    hence "g\<^sub>m (\<ee> [c]) \<le>s \<alpha> \<cdot> g\<^sub>m p"
      using marked.g.suf_mono suf_ext by blast
    hence "h\<^sub>m (\<ff> [c]) \<le>s h\<^sub>m q"
      unfolding begin_block sucs_eq.
    hence "\<ff> [c] \<le>s q"
      using marked.h.suf_mono
        \<open>q \<bowtie>\<^sub>s \<ff> [c]\<close>[unfolded suf_comp_or] marked.h.code_morph_code  suffix_order.antisym by metis 
    ...
    hence "\<alpha> \<cdot> g\<^sub>m (p\<^sup><\<inverse>\<ee> [c]) = h\<^sub>m (q\<^sup><\<inverse>\<ff>[c])"
      unfolding marked.g.morph marked.h.morph marked.block_eq[OF both_blocks] lassoc cancel_right.
...
lemma rich_block_suf_fst':
  ...
  shows "marked.g.bin_code_lcs \<cdot> g\<^sub>m p \<le>s g\<^sub>m (\<ee> ([1-c]\<cdot>[c]\<^sup>@Suc i))"
proof-
  ...
  then obtain p' where "\<ee> [c] = [c] \<cdot> p' \<cdot> p" 
    using ssufE[OF \<open>p <s \<ee> [c]\<close>] ssuf_tl_suf suf_def by metis 
  ...
  show "marked.g.bin_code_lcs \<cdot> g\<^sub>m p \<le>s g\<^sub>m (\<ee> ([1-c]\<cdot>[c]\<^sup>@Suc i))" 
    unfolding * marked.g.morph lassoc suf_cancel_conv lcp_diff[symmetric] by simp  
qed
...
proof(rule ccontr)
  ...
  have "\<ff> ([1 - c] \<cdot> [c] \<^sup>@ Suc i) <s q"
   ...
   from marked.h.ssuf_mono[OF this]  rich_block_suf_fst[OF assms, unfolded marked.suc_eq'[OF both_blocks] begin_block]
  show False by force
qed
...
proof (cases)
  ...
  show "coin_block [c]"
    unfolding coin_block_def using per_drop_exp_rev[OF Suc_not_Zero] by metis
next
  assume "z \<notin> [c]*"
  ...
  from ssufD[OF this, unfolded suf_def]
  ...
  proof(cases)
    ...
    thus "coin_block [c]"
      unfolding coin_block_def using ssufD1[OF ssuf_ext[OF \<open>p <s \<ee> [c]\<close>]] by fast
  next \<comment> \<open>the other option leads to a contradiction\<close>
    write
      ...
      marked.h.bin_code_lcs ("\<beta>\<^sub>H") and
      marked.g.bin_code_lcs ("\<beta>\<^sub>G") and
...
\<comment> \<open>analysis of q\<close>
    ...
    have "[\<dd> b] \<cdot> \<beta>\<^sub>\<hh> \<le>s \<ff> ([c]\<^sup>@Suc t \<cdot> z') \<cdot> \<ff> [b]"
      by (rule marked.sucs.revs.h.bin_lcp_mismatch_pref_all_set[reversed])
        (simp add: \<open>b \<noteq> c\<close>[unfolded bin_neq_iff])
    from this[folded marked.sucs.h.morph lassoc, unfolded suf_def] 
...
\<comment> \<open>1. inequality\<close>
    ...
    from begin_block[unfolded q_factors, unfolded pow_Suc2 marked.sucs.h.morph marked.h.morph, folded sucs_eq[of "[c]"], unfolded \<open>\<ee>[c] = [c] \<cdot> tl p' \<cdot> p\<close> marked.g.morph lassoc cancel_right, unfolded rassoc]
...
proof-
  ...
  have "\<tau> \<noteq> \<epsilon>"
    using rq_self[of p] marked.sucs.g.emp_to_emp clean_emp(1) by metis 
...
proof (rule notI)
  assume "q = \<epsilon>"
  with coin_block_def
    marked.ne_g[OF suf_of_emp[OF begin_block[unfolded marked.h.emp_to_emp'[OF this]]]]  non_block  
  show False by blast
qed

lemma p_suf: "p <s \<ee> [1-a1]"
  using last_letter_fst_suf[of \<epsilon>, unfolded clean_emp, OF swap_coin_block].
...
proof-
  have "\<^bold>|q\<^bold>| \<le> \<^bold>|\<ff> ([1-a1]\<^sup>@\<^bold>|q\<^bold>|)\<^bold>|"
    using long_pow_exp marked.sucs.h.pow_morph marked.sucs.h.sing_to_nemp by metis  
...
next
  ...
  have "q \<le>s \<ff> [a1] \<cdot> \<ff> ([1-a1]\<^sup>@Suc t)"
    using coin_exp rich_block_suf_snd[of \<epsilon> "1 - a1" t, unfolded clean_emp binsimp] unfolding marked.sucs.h.morph by blast
...
proof-
  ...
  note \<open>b \<noteq> 1-a1\<close>[symmetric, unfolded bin_neq_iff binsimp]
  from \<open>[b] \<cdot> [1-a1]\<^sup>@i \<le>s w\<close>[unfolded this, unfolded suf_def] 
  ...
  proof(cases)
    ...
    then show "coin_block w \<longleftrightarrow> [1-a1]\<^sup>@Suc t \<le>s w"
      unfolding w \<open>i = 0\<close> clean_pows using last_letter_block non_block by meson
  next
    assume "i \<noteq> 0"
    ...
    proof 
      show "coin_block (w' \<cdot> [a1] \<cdot> [1 - a1] \<^sup>@ Suc k) \<Longrightarrow> q \<le>s \<ff> ([a1] \<cdot> [1 - a1] \<^sup>@ Suc k)"
        using rich_block_suf_snd[of w' "1 - a1" k] suffix_append unfolding binsimp marked.sucs.h.morph by blast   
    ...
    have "t \<le> k \<longleftrightarrow> [1-a1]\<^sup>@Suc t \<le>s w'\<cdot>[a1]\<cdot>[1-a1]\<^sup>@Suc k" for k
      using sing_exp_pref_iff[reversed, OF bin_swap_neq', symmetric, of "Suc t" "Suc k" "1-a1", unfolded Suc_le_mono binsimp rassoc]. 
...
proof (induction x rule: \<W>.induct) 
  ...
  proof
    ...
    have "pp \<noteq> \<epsilon>"
      using fac clean_emp(2) bin_swap_neq[of a1] unfolding pow_Suc rassoc by force
...
proof
  show nemp: "u \<in> {[1 - a1]} \<union> \<W> \<Longrightarrow> u \<noteq> \<epsilon>" for u
  ...
  next
    assume "hd u = a1"
    note nemp[OF u_in] and nemp[OF v_in]
    ...
    hence "\<not> u \<le>p butlast v"
      using \<W>_fac[OF \<open>v \<in> \<W>\<close>] unfolding fac_def suf_def pref_def by fastforce
...
proof-      
  ...
  next
    assume "x \<in> \<W>"
    show "x \<in> \<T>"
      unfolding \<T>_def' coin_block_with_bad_letter[OF \<W>_set[OF \<open>x \<in> \<W>\<close>]] using suffix_appendD[OF \<W>_suf[OF \<open>x \<in> \<W>\<close>]].
  qed
qed
  ...
  proof
    ...
    proof (induction "\<^bold>|x\<^bold>|" arbitrary: x rule: less_induct)
      ...
      proof (cases "\<exists> px. px \<noteq> \<epsilon> \<and> px <p x \<and> coin_block px")
        ...
        from sprefE[OF \<open>px <p x\<close>]
      ...
      next
        ...
        next
          ...
          proof (rule ccontr)
            ...
            from \<open>[1 - a1] \<cdot> tl x = x\<close>[unfolded this clean_emp]
            show False
              using neq_in_set_not_pow[OF bin_swap_neq[of a1] \<open>a1 \<in> set x\<close>, of 1, unfolded clean_pows] by simp
          qed
          ...
          hence "\<not> [a1]\<cdot>[1-a1]\<^sup>@Suc(Suc t) \<le>p x"
            unfolding pow_Suc2[of _ "Suc t"] lassoc
          ...
          next
            ...
            have "hd x' = a1"  
              ...
              using bin_neq_iff'[of "hd x'" "1-a1", unfolded binsimp] by fastforce
            ...
            hence "\<not> [a1] \<cdot> tl x' \<le>s [1-a1]\<^sup>@Suc t"
              unfolding suf_def using Cons_eq_appendI in_set_conv_decomp by metis
            with ruler[reversed, of x', OF _ \<open>[1-a1]\<^sup>@Suc t \<le>s x\<close>]
            have "[1-a1]\<^sup>@Suc t \<le>s x'" 
              unfolding \<open>x = [a1] \<cdot> [1-a1]\<^sup>@j \<cdot> x'\<close> \<open>[a1] \<cdot> tl x' = x'\<close> suf_def by fastforce
            have "a1 \<in> set x'"
...
proof
  ...
  proof
    ...
    proof (induction x rule: \<W>.induct, force)
      case (2 \<tau> i)
...
theorem bin_inter_basis: assumes "binary_code x y" and "binary_code u v" 
  ...
  using  two_code_morphisms.range_inter_basis_snd(1)[OF two_code_morphisms.intro, OF bin_code_code_morph bin_code_code_morph, OF assms, folded coin_set_inter_snd] unfolding image_comp.
...
theorem binary_intersection_code:
  ...
  using two_code_morphisms.range_inter_code[OF two_code_morphisms.intro[OF bin_code_code_morph[OF assms(1)] bin_code_code_morph[OF assms(2)]]]
...
proof-
  ...
  interpret two_nonerasing_morphisms g h 
    by (simp add: assms'(1) assms'(2) bin_code_morph_iff binary_code.non_comm first_morphism_def second_morphism_def two_bin_code_morphs_nonerasing_morphs)

  interpret two_binary_code_morphisms g h
    by (metis assms'(1) assms'(2) bin_code_morph_iff binary_code.bin_not_comp_suf first_morphism_def second_morphism_def suf_comp_refl two_binary_code_morphisms_def two_binary_morphisms_def two_morphisms_axioms)
  ...
  proof (cases)
    ...
    have "\<langle>{x',y'}\<rangle> \<inter> \<langle>{u',v'}\<rangle> = {\<epsilon>}"  
      unfolding bin_inter_coin_set_snd image_comp[symmetric] mdefs[symmetric]
        min_coin_gen_snd[symmetric, unfolded \<open>\<CC>\<^sub>m g h = {}\<close>] 
  ...
  next 
    ...
    next
      ...
      proof(cases)
        ...
        define \<beta> where "\<beta> = (h \<circ> (\<lambda>x. (q \<cdot> x)\<^sup><\<inverse>q ) \<circ> \<ff>) \<zero>"
        define \<gamma> where "\<gamma> = (h \<circ> (\<lambda>x. (q \<cdot> x)\<^sup><\<inverse>q ) \<circ> \<ff>) \<one>"
      ...
      next
        ...
        have "(h q1 \<cdot> h q2) \<cdot> (h q1 \<cdot> \<alpha>\<^sub>h) = (h q1 \<cdot> \<alpha>\<^sub>h) \<cdot> (h\<^sub>m q2 \<cdot> h\<^sub>m q1)"
          unfolding rassoc h.marked_version_conjugates[of "q2 \<cdot> q1", unfolded marked.h.morph h.morph].. 
        ...
        obtain \<beta> \<gamma> k' where bg: "\<beta> \<cdot> \<gamma> = h (q1 \<cdot> q2)" and "\<gamma> \<cdot> \<beta> = h\<^sub>m (q2 \<cdot> q1)" and
          ...
          unfolding marked.h.morph h.morph shift_pow by blast 
        have bgb_q: "\<beta> \<cdot> (\<gamma> \<cdot> \<beta>)\<^sup>@(k' + k) = \<alpha>\<^sub>h \<cdot> h\<^sub>m q"
          ...
          unfolding \<open>\<gamma> \<cdot> \<beta> = h\<^sub>m (q2 \<cdot> q1)\<close> h.marked_version_conjugates[symmetric]
            rassoc cancel q_def shift_pow unfolding marked.h.morph marked.h.pow_morph.. 
        define \<delta> where "\<delta> = h\<^sub>m (\<ff> [a1])"
        have bg_def: "\<beta> \<cdot> \<gamma> = h ((q \<cdot> \<ff> [1 - a1])\<^sup><\<inverse>q )"
          unfolding bg q_def q21 rassoc shift_pow pow_comm using rq_triv
            marked.h.morph by force 
        ...
        have "\<delta> \<noteq> \<epsilon>"
          unfolding \<delta>_def using marked.h.nonerasing  marked.sucs.h.sing_to_nemp by blast  
        have "hd \<delta> \<noteq> hd (\<gamma> \<cdot> \<beta>)"
          unfolding \<delta>_def gb_def
          using marked.h.hd_im_eq_hd_eq marked.sucs.h.bin_marked_sing marked.sucs.h.sing_to_nemp by blast 
        ...
        have w_decode: "w \<in> \<langle>{[a1] \<cdot> [1 - a1] \<^sup>@ i |i. i \<le> t}\<rangle> \<Longrightarrow>  h\<^sub>m (\<ff> w) \<in> \<langle>{\<delta> \<cdot> (\<gamma> \<cdot> \<beta>) \<^sup>@ i |i. i \<le> t}\<rangle>"
          for w
        proof (induct w rule: hull.induct, unfold marked.sucs.h.emp_to_emp marked.h.emp_to_emp, fast)
          case (prod_cl w1 w2)
          then obtain i where "w1 = [a1] \<cdot> [1-a1]\<^sup>@i" and "i \<le> t" by blast
          with prod_cl.hyps
          show ?case 
            unfolding marked.sucs.h.morph marked.sucs.h.pow_morph marked.h.morph 
                      marked.h.pow_morph \<open>w1 = [a1] \<cdot> [1-a1]\<^sup>@i\<close> \<delta>_def[symmetric] gb_def[symmetric] 
            by fast
        qed
        ...
        proof (induct w rule: hull.induct, use marked.sucs.h.emp_to_emp marked.h.emp_to_emp in force)
          case (prod_cl w1 w2)
          ...
          have "([a1] \<cdot> [1 - a1] \<^sup>@ j) \<cdot> w' \<in>  \<langle>{[a1] \<cdot> [1 - a1] \<^sup>@ i |i. i \<le> t}\<rangle>"
            using \<open>w' \<in> \<langle>{[a1] \<cdot> [1 - a1] \<^sup>@ i |i. i \<le> t}\<rangle>\<close> \<open>j \<le> t\<close> by fast
          moreover have "h\<^sub>m (\<ff> (([a1] \<cdot> [1 - a1] \<^sup>@ j) \<cdot> w')) = w1 \<cdot> w2"
            unfolding marked.sucs.h.morph marked.sucs.h.pow_morph marked.h.morph marked.h.pow_morph \<open>h\<^sub>m (\<ff> w') = w2\<close>
        ...
        proof (cases)
          ...
          from ssuf_extD[OF this[unfolded bgb_q[symmetric] marked.sucs.h.pow_morph q21 gb_def marked.h.pow_morph]]
          have "k' + k < Suc t" 
          ...
          have "\<beta> = h q1 \<cdot> \<alpha>\<^sub>h"
            using \<open>\<beta> \<cdot> (\<gamma> \<cdot> \<beta>)\<^sup>@k'  = h q1 \<cdot> \<alpha>\<^sub>h\<close>[unfolded \<open>k' = 0\<close> clean_pows]. 
          from gb_def[unfolded q21 marked.h.morph this h.marked_version_conjugates[symmetric]]
          ...
          proof-
            have "\<beta> \<cdot> (\<gamma> \<cdot> \<beta>)\<^sup>@t \<cdot> h\<^sub>m (\<ff> w) \<cdot> \<delta> \<cdot> \<gamma> = \<alpha>\<^sub>h \<cdot> h\<^sub>m (q \<cdot> \<ff> (w \<cdot> [a1])) \<cdot> \<gamma>"
              unfolding marked.h.morph marked.sucs.h.morph \<delta>_def lassoc cancel_right \<open>\<beta> \<cdot> (\<gamma> \<cdot> \<beta>) \<^sup>@ t = \<alpha>\<^sub>h \<cdot> h\<^sub>m q\<close>..
        ...
        next
          ...
          have "\<alpha>\<^sub>h \<cdot> h\<^sub>m q \<le>s (\<alpha>\<^sub>h \<cdot> h\<^sub>m q) \<cdot> h\<^sub>m ((q2 \<cdot> q1) \<^sup>@ Suc t)"
            unfolding q_def shift_pow rassoc marked.h.morph[symmetric] pows_comm[of _ k]
            unfolding marked.h.morph lassoc suf_cancel_conv
            unfolding rassoc marked.h.morph[symmetric] shift_pow[symmetric]
            unfolding marked.h.morph lassoc suf_cancel_conv 
          ...
          from this[unfolded q_def shift_pow marked.h.morph, unfolded marked.h.pow_morph, folded gb_def[unfolded q21], unfolded lassoc,
          ...
          have "\<alpha>\<^sub>h \<cdot> h\<^sub>m q = \<beta> \<cdot> (\<gamma> \<cdot> \<beta>) \<^sup>@ (k' + k)"
            unfolding q_def shift_pow marked.h.morph 
            unfolding marked.h.pow_morph gb_def[symmetric, unfolded q21] lassoc k'[folded h.marked_version_conjugates, symmetric]
          ...
          note long = rich_block_suf_snd'[of \<epsilon> "1-a1", unfolded clean_emp binsimp, OF coin_exp]  

          have delta_suf: "(\<beta> \<cdot> (\<gamma> \<cdot> \<beta>) \<^sup>@ (k' + k - Suc t)) \<le>s \<delta>" 
            using long unfolding bgb_q[symmetric] \<delta>_def marked.sucs.h.morph marked.sucs.h.pow_morph q21 marked.h.morph
            unfolding marked.h.pow_morph gb_def[unfolded q21,symmetric] split
          ...
          have apply_h0:  "h ((q \<cdot> \<ff> (w \<cdot> [a1] \<cdot> [1 - a1] \<^sup>@ Suc t))\<^sup><\<inverse>q) 
            ...
            unfolding h.marked_version_conjugates[symmetric] marked.h.morph marked.sucs.h.morph
            unfolding lassoc bgb_q unfolding rassoc \<delta>_def gb_def marked.h.pow_morph marked.sucs.h.pow_morph..
