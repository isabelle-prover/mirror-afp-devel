...
section \<open>Correctness of the KZG\<close>

locale KZG_PCS_correct = KZG 
begin 

subsubsection \<open>Helping lemmas for the computation of \<open>\<psi>\<close>\<close>

text \<open>Helping lemmas for the computation of \<open>\<psi>\<close> (function \<open>\<psi>_of\<close>) in \<open>\<phi>(x)-\<phi>(c)=(x-c)*\<psi>(x)\<close>,
which is used to compute \<open>\<psi>\<close> in CreateWitness.\<close>
...
text \<open>state that the computed polynomial \<open>\<psi>\<close>, is of degree less equal to \<open>\<phi>\<close>.\<close>
lemma degree_q_le_\<phi>: "degree (\<psi>_of \<phi> u) \<le> degree \<phi>"
  unfolding \<psi>_of_def
  by (metis degree_Poly \<psi>_coeffs_length)

text \<open>This lemma is essentially resorting the summation according to the idea given in KZG\_def 
...
text \<open>We now show that the inner sum from the last lemma, which calculates the i-th coefficient for \<open>\<psi>\<close>,
is equal to the i-th coefficient calculated from the \<open>\<psi>_of\<close> function.\<close>
...
text \<open>We now take together the last few lemmas and definitions and show that \<open>\<psi>_of_poly\<close>
calculates the correct \<open>\<psi>\<close>.
With the sum\_horiz\_to\_vert lemma, we restructure the left-hand side to calculate the 
coefficients of \<open>\<psi>\<close> before multiplying with x\textasciicircum{}i.
With the \<open>\<psi>_of_ith_coeff_eq_sum_ith_coeff\<close> lemma, show the coefficients of the result of 
sum\_horiz\_to\_vert equal to the coefficients calculated by \<open>\<psi>_of_poly\<close> and thus showing 
\<open>poly (\<psi>_of_poly \<phi> u) x\<close> equal to the result sum of sum\_horiz\_to\_vert.\<close>
...
We know \<open>\<phi>(x)-\<phi>(u)=(x-u)\<psi>(x)\<close> from the previous corollary, now we show the equality is also valid with 
...
Showing that the ith public parameter is actually the ith public parameter (\<open>g^(\<alpha>^i)\<close>)\<close>
...
Which is the first prestep to showing ($\prod$PK. $\phi$) = g\textasciicircum{}$\phi$($\alpha$).\<close>
...
text \<open>show 
\<open>g^(\<Sum>i\<le>n. coeff \<phi> i * \<alpha>^i)\<close>
...
g * g\textasciicircum{}($\alpha$ \textasciicircum{} 1st coefficient of $\phi$) * g\textasciicircum{}(($\alpha$\textasciicircum{}2) \textasciicircum{} 2nd coefficient of $\phi$) * ... * g\textasciicircum{}(($\alpha$\textasciicircum{}t) \textasciicircum{} t-th coefficient of $\phi$).
Which we further form into \<open>g^(\<Sum>i\<le>n. coeff \<phi> i * \<alpha>^i)\<close>, which is nothing else then g\textasciicircum{}$\phi$($\alpha$) (poly\_altdef),
with the poly\_altdef\_to\_fold lemma\<close>
...
text \<open>Finally put everything together and show perfect correctness of Eval and verify\_eval\<close>
