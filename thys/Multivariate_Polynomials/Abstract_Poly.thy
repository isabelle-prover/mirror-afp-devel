...
lift_definition monom_mult_right::"('a, 'b) mpoly \<Rightarrow> 'b::semiring_0 \<Rightarrow> 'a \<Rightarrow> ('a, 'b) mpoly" is
  "\<lambda>p. \<lambda>c::'b. \<lambda>t. (\<lambda>s. (if t dvd s then (p (s divide t)) * c else 0))"
proof -
  fix c::'b and t::'a and p::"'a \<Rightarrow> 'b"
  have "{s. (if t dvd s then p (s divide t) * c else 0) \<noteq> 0} \<subseteq> (\<lambda>x. t * x) ` {s. p s \<noteq> 0}"
    (is "?L \<subseteq> ?R")
  proof
    fix x::"'a"
    assume "x \<in> ?L"
    hence "(if t dvd x then p (x divide t) * c else 0) \<noteq> 0" by simp
    hence "t dvd x" and cp_not_zero: "p (x divide t) * c \<noteq> 0" by (simp_all split: if_split_asm)
    show "x \<in> ?R"
    proof
      from dvd_divide[OF \<open>t dvd x\<close>] show "x = t * (x divide t)" by (simp add: ac_simps)
    next
  ...
  hence "finite ?R" by (intro finite_imageI)
  from finite_subset[OF \<open>?L \<subseteq> ?R\<close> this]
    show "finite {s. (if t dvd s then p (s divide t) * c else 0) \<noteq> 0}" .
qed
...
lemma monom_mult_right_coeff:
  fixes c::"'b::semiring_0" and t s::"'a" and p::"('a, 'b) mpoly"
  shows "coeff (monom_mult_right p c t) (s * t) = coeff p s * c"
proof transfer
  fix c::"'b" and t s::"'a" and p::"'a \<Rightarrow> 'b"
  from dvd_triv_left[of t s] times_divide[of s t]
    show "(if t dvd s * t then p (s * t divide t) * c else 0) = p s * c" by simp
qed
...
proof transfer
  fix c d::"'b::semiring_0" and s t::"'a" and p::"'a \<Rightarrow> 'b"
  show "(\<lambda>x. if t dvd x then (if s dvd x divide t then p (x divide t divide s) * c else 0) * d else 0) =
...
lemma uminus_monom_mult_right:
  fixes p::"('a, 'b::comm_ring_1) mpoly"
  shows "-p = monom_mult_right p (-1) 1"
proof transfer
  fix p::"'a \<Rightarrow> 'b"
  show "(\<lambda>t. - p t) = (\<lambda>s. if 1 dvd s then p (s divide 1) * (- 1) else 0)"
  proof
    fix t::"'a"
    show "- p t = (if 1 dvd t then p (t divide 1) * (- 1) else 0)" by (simp add: divide_one)
  qed
qed
...
lemma monom_mult_right_right1:
  fixes p::"('a, 'b::semiring_1) mpoly"
  shows "(monom_mult_right p 1 1) = p"
proof (transfer, rule)
  fix p::"'a \<Rightarrow> 'b" and s::"'a"
  from divide_one[of s] show "(if 1 dvd s then p (s divide 1) * 1 else 0) = p s" by simp
qed

lemma monom_mult_monom:
  fixes c d::"'b::semiring_0" and s t::"'a"
  shows "monom_mult c s (monom d t) = monom (c * d) (s * t)"
proof (transfer, rule)
  fix c::'b and s::"'a" and d t u
  show "(if s dvd u then c * (if u divide s = t then d else 0) else 0) =
        (if u = s * t then c * d else 0)"
  proof (simp, intro conjI, intro impI)
    assume ust: "u divide s = t"
    show "(s * t divide s = t \<longrightarrow> s dvd u \<longrightarrow> u \<noteq> s * t \<longrightarrow> c * d = 0) \<and>
          (s * t divide s \<noteq> t \<longrightarrow> (s dvd u \<longrightarrow> c * d = 0) \<and>
          (\<not> s dvd u \<longrightarrow> u = s * t \<longrightarrow> c * d = 0))"
    proof (intro conjI, intro impI)
      assume "s dvd u" and "u \<noteq> s * t"
      from dvd_divide[OF \<open>s dvd u\<close>] ust have "u = t * s" by simp
      from \<open>u \<noteq> s * t\<close> this show "c * d = 0" by (simp add: ac_simps)
    next
      show "s * t divide s \<noteq> t \<longrightarrow> (s dvd u \<longrightarrow> c * d = 0) \<and> (\<not> s dvd u \<longrightarrow> u = s * t \<longrightarrow> c * d = 0)"
      proof (intro impI)
        assume "s * t divide s \<noteq> t"
        thus "(s dvd u \<longrightarrow> c * d = 0) \<and> (\<not> s dvd u \<longrightarrow> u = s * t \<longrightarrow> c * d = 0)"
          using times_divide_2 by simp
      qed
    qed
  next
    show "u divide s \<noteq> t \<longrightarrow> s * t divide s \<noteq> t \<longrightarrow> u = s * t \<longrightarrow> c * d = 0"
    proof (intro impI)
      assume "s * t divide s \<noteq> t"
      thus "c * d = 0" using times_divide_2 by simp
    qed
  qed
qed
...
lemma monom_mult_right_monom:
  fixes c d::"'b::semiring_0" and s t::"'a"
  shows "monom_mult_right (monom c s) d t = monom (c * d) (s * t)"
proof (transfer, rule)
  fix c::'b and s::"'a" and d t u
  show "(if t dvd u then (if u divide t = s then c else 0) * d else 0) =
        (if u = s * t then c * d else 0)"
  proof (simp, intro conjI, intro impI)
    assume uts: "u divide t = s"
    show "(s * t divide t = s \<longrightarrow> t dvd u \<longrightarrow> u \<noteq> s * t \<longrightarrow> c * d = 0) \<and>
          (s * t divide t \<noteq> s \<longrightarrow> (t dvd u \<longrightarrow> c * d = 0) \<and>
          (\<not> t dvd u \<longrightarrow> u = s * t \<longrightarrow> c * d = 0))"
    proof (intro conjI, intro impI)
      assume "t dvd u" and "u \<noteq> s * t"
      from dvd_divide[OF \<open>t dvd u\<close>] uts have "u = s * t" by simp
      from \<open>u \<noteq> s * t\<close> this show "c * d = 0" by simp
    next
      show "s * t divide t \<noteq> s \<longrightarrow> (t dvd u \<longrightarrow> c * d = 0) \<and> (\<not> t dvd u \<longrightarrow> u = s * t \<longrightarrow> c * d = 0)"
      proof (intro impI)
        assume "s * t divide t \<noteq> s"
        thus "(t dvd u \<longrightarrow> c * d = 0) \<and> (\<not> t dvd u \<longrightarrow> u = s * t \<longrightarrow> c * d = 0)"
          using times_divide by simp
      qed
    qed
  next
    show "u divide t \<noteq> s \<longrightarrow> s * t divide t \<noteq> s \<longrightarrow> u = s * t \<longrightarrow> c * d = 0"
    proof (intro impI)
      assume "s * t divide t \<noteq> s"
      thus "c * d = 0" using times_divide by simp
    qed
  qed
qed
...
lemma monom_mult_left_monom_monom_mult_right:
  fixes c d::"'b::semiring_0" and s t::"'a"
  shows "monom_mult c s (monom d t) = monom_mult_right (monom c s) d t"
using monom_mult_monom[of c s] monom_mult_right_monom[of c s] by simp

lemma monom_mult_left_monom_mult_right:
  fixes c::"'b::comm_semiring_0" and t::"'a" and p::"('a, 'b) mpoly"
  shows "monom_mult c t p = monom_mult_right p c t"
proof (transfer, rule)
  fix c::'b and t s::"'a" and p
  show "(if t dvd s then c * p (s divide t) else 0) = (if t dvd s then p (s divide t) * c else 0)"
...
lemma monom_mult_right_monom:
  fixes c d::"'b::comm_semiring_0" and s t::"'a"
  shows "monom_mult_right (monom c s) d t = monom_mult_right (monom d t) c s"
...
lemma times_mpoly_right_monom:
  shows "p * (monom c t) = monom_mult_right p c t"
proof (induct rule: times_mpoly_induct[of _ "monom c t" p])
  from monom_mult_right_left0 show "0 = monom_mult_right 0 c t" by simp
next
  fix p
  assume "p \<noteq> 0" and IH: "rest_mpoly p * monom c t = monom_mult_right (rest_mpoly p) c t"
  from IH some_monomial_rest[OF \<open>p \<noteq> 0\<close>]
       monom_mult_left_monom_monom_mult_right[of "coeff p (some_term p)" "some_term p" c t]
       monom_mult_right_dist_left[of "monom (coeff p (some_term p)) (some_term p)" "rest_mpoly p" c t]
...
next
  ...
  also have "\<dots> = (p + q) * (monom ?c ?t) + (p * ?r + q * ?r)"
    using IH by simp
  also have "\<dots> = monom_mult_right (p + q) ?c ?t + (p * ?r + q * ?r)"
    using times_mpoly_right_monom by simp
  also have "\<dots> = (monom_mult_right p ?c ?t + monom_mult_right q ?c ?t) + (p * ?r + q * ?r)"
...
proof (induct rule: times_mpoly_induct[of "\<lambda>x y _. (monom_mult c t x) * y = monom_mult c t (x * y)"])
  from times_mpoly_left0 monom_mult_right0[of c t] show "monom_mult c t 0 * q = monom_mult c t (0 * q)"
    by simp
next
  fix p::"('a, 'b) mpoly"
  let ?c = "coeff p (some_term p)"
  let ?t = "some_term p"
  let ?r = "rest_mpoly p"
  assume "p \<noteq> 0" and IH: "monom_mult c t ?r * q = monom_mult c t (?r * q)"
  
  have "monom_mult c t p = monom_mult c t ((monom ?c ?t) + ?r)"
    using some_monomial_rest[OF \<open>p \<noteq> 0\<close>] by simp
  also have "\<dots> = monom_mult c t (monom ?c ?t) + monom_mult c t ?r"
    using monom_mult_dist_right[of c t] by simp
  finally have eq: "monom_mult c t p = monom_mult c t (monom ?c ?t) + monom_mult c t ?r" .
  
  have "monom_mult c t p * q = (monom_mult c t (monom ?c ?t) + monom_mult c t ?r) * q"
    by (simp only: eq)
  also have "\<dots> = (monom_mult c t (monom ?c ?t)) * q + (monom_mult c t ?r) * q"
    by (simp only: times_mpoly_dist_right)
  also have "\<dots> = (monom_mult c t (monom ?c ?t)) * q + monom_mult c t (?r * q)"
    by (simp only: IH)
  also have "\<dots> = (monom (c * ?c) (t * ?t)) * q + monom_mult c t (?r * q)"
    by (simp only: monom_mult_monom)
  also have "\<dots> = monom_mult (c * ?c) (t * ?t) q + monom_mult c t (?r * q)"
    by (simp only: times_mpoly_left_monom)
  also have "\<dots> = monom_mult c t (monom_mult ?c ?t q) + monom_mult c t (?r * q)"
    by (simp only: monom_mult_assoc)
  also have "\<dots> = monom_mult c t ((monom_mult ?c ?t q) + ?r * q)"
    by (simp only: monom_mult_dist_right)
  also have "\<dots> = monom_mult c t ((monom ?c ?t) * q + ?r * q)"
    by (simp only: times_mpoly_left_monom)
  also have "\<dots> = monom_mult c t (((monom ?c ?t) + ?r) * q)"
    by (simp only: times_mpoly_dist_right)
...
next
  fix p q r::"('a, 'b) mpoly"
  from times_mpoly_dist_left show "p * (q + r) = p * q + p * r" .
qed
...
next
  fix p::"('a, 'b) mpoly"
  from times_mpoly_right_monom[of p 1 1] monom_mult_right_right1[of p] show "p * 1 = p"
    unfolding one_mpoly_def by simp
next
...
next
  ...
  also have "\<dots> = p * (monom ?c ?t) + p * ?r"
    by (simp only: times_mpoly_dist_left)
  also have "\<dots> = p * (monom ?c ?t) + ?r * p"
    using IH by simp
  also have "\<dots> = monom_mult_right p ?c ?t + ?r * p"
    by (simp only: times_mpoly_right_monom)
  also have "\<dots> = monom_mult ?c ?t p + ?r * p"
    by (simp only: monom_mult_left_monom_mult_right)
