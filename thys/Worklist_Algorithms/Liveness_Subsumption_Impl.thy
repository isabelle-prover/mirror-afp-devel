...
\<comment> \<open>The lemma has this form to avoid unwanted eta-expansions.
  The eta-expansions arise from the type of \<^term>\<open>insert_map_set v S\<close>.\<close>
...
\<comment> \<open>The check for emptiness may be superfluous if we thread through the pre-condition\<close>
