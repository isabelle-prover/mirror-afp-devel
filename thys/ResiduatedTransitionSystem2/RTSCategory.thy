...
begin
  ...
  locale rts_category =
    ...
    H: category hcomp +
    VV: fiber_product_rts resid resid resid H.dom H.cod +
    HC: simulation VV.resid resid
  ...
  begin
    ...
    abbreviation in_trn  ("\<guillemotleft>_ : _ \<Rightarrow> _\<guillemotright>")
    where "in_trn t f g \<equiv> V.arr t \<and> V.src t = f \<and> V.trg t = g"

    abbreviation trn
    where "trn f g \<equiv> {t. \<guillemotleft>t : f \<Rightarrow> g\<guillemotright>}"
    ...
    interpretation VV: fiber_product_of_extensional_rts resid resid resid H.dom H.cod
      ..
    ...
    shows "sta (t \<star> u)"
      using assms VV.ide_char\<^sub>F\<^sub>P HC.preserves_ide
      by (elim H.seqE) auto

    lemma src_hcomp [simp]:
    assumes "H.seq t u"
    shows "V.src (t \<star> u) = V.src t \<star> V.src u"
      using assms VV.arr_char H.preserves_src [of "(t, u)"] VV.src_char VV.arr_src_iff_arr
      by (elim H.seqE) auto

    lemma trg_hcomp [simp]:
    assumes "H.seq t u"
    shows "V.trg (t \<star> u) = V.trg t \<star> V.trg u"
    ...
    assumes "t \<frown> u"
    shows "H.par t u"
      using assms V.con_implies_arr arr_coincidence cod_src V.con_imp_eq_src dom_src
      by metis
    ...
    proof -
      have r: "V.arr r" and s: "V.arr s" and rs: "H.dom r = H.cod s"
        using assms H.seqE
          apply auto[3]
    ...
    sublocale src: "functor" hcomp hcomp V.src
      apply unfold_locales
      subgoal using V.src_def by auto
      by auto

    sublocale trg: "functor" hcomp hcomp V.trg
      apply unfold_locales
    ...
    proof -
      ...
      have f: "V.arr f" and g: "V.arr g" and fg: "H.dom f = H.cod g"
        using inv_fg arr_coincidence
        by auto fastforce+
      (* TODO: There has to be a shorter proof than this. *)
      have "sta (H.cod f \<star> f)"
      proof -
        have "sta ((V.trg f \<star> V.trg g) \<star> f)"
        proof -
          have 1: "sta (V.trg g \<star> f)"
          proof -
            have 2: "sta ((g \<star> V.src f) \<cdot> (V.trg g \<star> f))"
            proof -
              have "sta (g \<star> f)"
                using inv_fg obj_implies_sta by blast
              moreover have "V.composable g (V.trg g)"
                using V.composable_iff_comp_not_null g by auto
              moreover have "V.composable (V.src f) f"
                using V.composable_iff_comp_not_null f by auto
              moreover have "H.seq g (V.src f)"
            ...
            qed
            moreover have "V.composable (g \<star> V.src f) (V.trg g \<star> f)"
              using 2 V.composable_iff_arr_comp by blast
            ultimately show "sta (V.trg g \<star> f)"
              using g V.comp_is_composite_of
                    V.divisors_of_ide
                      [of "g \<star> V.src f" "V.trg g \<star> f" "(g \<star> V.src f) \<cdot> (V.trg g \<star> f)"]
              by blast
          qed
          moreover have "H.seq (V.trg f) (V.trg g \<star> f)"
            using f g inv_fg 1
            by (intro H.seqI) auto
          moreover have "sta (V.trg f)"
            using assms arr_coincidence by fastforce
          ultimately show ?thesis
            using g H.comp_assoc by auto
        qed
        moreover have "H.inverse_arrows (V.trg f) (V.trg g)"
  ...
  subsection "Hom-RTS's"
    ...
    proof
      show "\<And>t. \<guillemotleft>t : a \<rightarrow> b\<guillemotright> \<Longrightarrow> V.arr t"
        by auto
    ...
    where "HOM\<^sup>\<rightarrow> a t \<equiv>
           (\<lambda>x. if residuation.arr (HOM a (H.dom t)) x then t \<star> x else V.null)"

    lemma cov_HOM_in_hom [intro]:
    assumes "\<guillemotleft>t : a \<rightarrow> b\<guillemotright>" and "\<guillemotleft>x : c \<rightarrow> a\<guillemotright>"
    shows "\<guillemotleft>HOM\<^sup>\<rightarrow> c t x : c \<rightarrow> b\<guillemotright>"
    and "\<guillemotleft>HOM\<^sup>\<rightarrow> c t x : HOM\<^sup>\<rightarrow> c (V.src t) (V.src x) \<Rightarrow> HOM\<^sup>\<rightarrow> c (V.trg t) (V.trg x)\<guillemotright>"
    proof -
      show "\<guillemotleft>HOM\<^sup>\<rightarrow> c t x : c \<rightarrow> b\<guillemotright>"
        using assms
        apply auto[1]
        by (metis H.in_homE sub_rts.arr_char sub_rts_HOM)
      thus "\<guillemotleft>HOM\<^sup>\<rightarrow> c t x : HOM\<^sup>\<rightarrow> c (V.src t) (V.src x) \<Rightarrow> HOM\<^sup>\<rightarrow> c (V.trg t) (V.trg x)\<guillemotright>"
        using assms
        apply (elim H.in_homE)
        apply auto[1]
        by (metis src.preserves_hom sub_rts.arr_char sub_rts_HOM trg.preserves_hom
            src_dom trg_dom)+
    qed

    lemma cov_HOM_simps [simp]:
    assumes "\<guillemotleft>t : a \<rightarrow> b\<guillemotright>" and "\<guillemotleft>x : c \<rightarrow> a\<guillemotright>"
    shows "H.dom (HOM\<^sup>\<rightarrow> c t x) = c" and "H.cod (HOM\<^sup>\<rightarrow> c t x) = b"
    and "V.src (HOM\<^sup>\<rightarrow> c t x) = HOM\<^sup>\<rightarrow> c (V.src t) (V.src x)"
    and "V.trg (HOM\<^sup>\<rightarrow> c t x) = HOM\<^sup>\<rightarrow> c (V.trg t) (V.trg x)"
      using assms(1,2) cov_HOM_in_hom(1)
         apply blast
      using assms cov_HOM_in_hom
        apply blast
      using assms(1,2) cov_HOM_in_hom(2)
      by blast+

    lemma simulation_cov_HOM_sta:
    assumes "obj a" and "sta f"
    shows "simulation (HOM a (H.dom f)) (HOM a (H.cod f)) (HOM\<^sup>\<rightarrow> a f)"
    proof -
      interpret HOM_a: sub_rts resid \<open>\<lambda>t. \<guillemotleft>t : a \<rightarrow> H.dom f\<guillemotright>\<close>
        using sub_rts_HOM by blast
      interpret HOM_b: sub_rts resid \<open>\<lambda>t. \<guillemotleft>t : a \<rightarrow> H.cod f\<guillemotright>\<close>
        using sub_rts_HOM by blast
      ...
      proof
        ...
        have 1: "\<guillemotleft>f \<star> t : a \<rightarrow> H.cod f\<guillemotright> \<and> \<guillemotleft>f \<star> u : a \<rightarrow> H.cod f\<guillemotright>"
        proof -
          have "f \<frown> f"
          ...
          thus ?thesis
            ...
            by auto
        qed
        have 2: "\<guillemotleft>f \<star> t : a \<rightarrow> H.cod f\<guillemotright> \<and> \<guillemotleft>f \<star> u : a \<rightarrow> H.cod f\<guillemotright> \<and> f \<star> t \<frown> f \<star> u"
    ...
    assumes "obj a" and "V.arr t"
    shows "transformation (HOM a (H.dom t)) (HOM a (H.cod t))
             (HOM\<^sup>\<rightarrow> a (V.src t)) (HOM\<^sup>\<rightarrow> a (V.trg t)) (HOM\<^sup>\<rightarrow> a t)"
    proof -
      interpret Dom': sub_rts resid \<open>\<lambda>x. x \<in> H.hom a (H.dom t)\<close>
        using assms sub_rts_HOM by auto
      interpret Dom': sub_rts_of_extensional_rts resid \<open>\<lambda>x. x \<in> H.hom a (H.dom t)\<close> ..
      interpret Cod': sub_rts resid \<open>\<lambda>x. x \<in> H.hom a (H.cod t)\<close>
        using assms  sub_rts_HOM by auto
      interpret Cod': sub_rts_of_extensional_rts resid \<open>\<lambda>x. x \<in> H.hom a (H.cod t)\<close> ..
      have Dom'_eq: "Dom'.resid = HOM a (H.dom t)"
        using assms Cod'.null_char by auto
      have Cod'_eq: "Cod'.resid = HOM a (H.cod t)"
        using assms Cod'.null_char by auto
      interpret Dom: extensional_rts \<open>HOM a (H.dom t)\<close>
        using Dom'_eq Dom'.extensional_rts_axioms by simp
      interpret Cod: extensional_rts \<open>HOM a (H.cod t)\<close>
        using Cod'_eq Cod'.extensional_rts_axioms by simp
      interpret Src: simulation
                       \<open>HOM a (H.dom t)\<close> \<open>HOM a (H.cod t)\<close> \<open>HOM\<^sup>\<rightarrow> a (V.src t)\<close>
        using assms simulation_cov_HOM_sta [of a "V.src t"] by auto
      interpret Trg: simulation
                       \<open>HOM a (H.dom t)\<close> \<open>HOM a (H.cod t)\<close> \<open>HOM\<^sup>\<rightarrow> a (V.trg t)\<close>
        using assms simulation_cov_HOM_sta [of a "V.trg t"] by auto
      show ?thesis
      proof
        ...
        have 1: "H.seq t f"
          ...
          by (intro H.seqI) auto
        have 2: "\<guillemotleft>t \<star> f : a \<rightarrow> H.cod t\<guillemotright>"
            using f 1 Dom'.ide_char Dom'.arr_char H.cod_comp
            by (intro H.in_homI) auto
        show "Cod.src (HOM\<^sup>\<rightarrow> a t f) = HOM\<^sup>\<rightarrow> a (V.src t) f"
          using f 1 2 Cod'.src_char Dom'.ide_char Cod'.arr_char by simp
        show "Cod.trg (HOM\<^sup>\<rightarrow> a t f) = HOM\<^sup>\<rightarrow> a (V.trg t) f"
        ...
        have 4: "Dom.src f = V.src f"
          using f Dom'_eq Dom'.src_char
          by (simp add: Dom.sources_char\<^sub>W\<^sub>E)
        have 5: "VV.con (t, Dom.src f) (V.src t, f)"
          unfolding VV.con_char
          using assms f 3 4 Dom'_eq Dom'.arr_char
          by (simp add: arr_f)
        have 6: "VV.con (V.src t, f) (t, Dom.src f)"
          using assms arr_f 3 4 VV.con_char by auto
        have 7: "VV.resid (t, Dom.src f) (V.src t, f) = (t, V.trg f)"
          using f 4 5 VV.resid_def VV.con_char Dom'.src_char V.con_implies_arr
          by auto
        show "HOM a (H.cod t) (HOM\<^sup>\<rightarrow> a t x) (HOM\<^sup>\<rightarrow> a (V.src t) f) =
              HOM\<^sup>\<rightarrow> a t (HOM a (H.dom t) x f)"
        proof -
          have "HOM a (H.cod t) (HOM\<^sup>\<rightarrow> a t x) (HOM\<^sup>\<rightarrow> a (V.src t) f) =
                Cod'.resid (t \<star> V.src f) (V.src t \<star> f)"
            using f 4 Dom.arr_src_iff_arr [of f]
            by (simp add: Dom.sources_char\<^sub>W\<^sub>E)
          also have "... = (t \<star> V.src f) \\ (V.src t \<star> f)"
          proof -
            have "\<guillemotleft>V.src t \<star> f : a \<rightarrow> H.cod t\<guillemotright>"
              using assms f Dom'.con_char by fastforce
            moreover have "\<guillemotleft>t \<star> V.src f : a \<rightarrow> H.cod t\<guillemotright>"
              using assms f "5" VV.con_char calculation by fastforce
            ultimately show ?thesis
            unfolding Cod'.resid_def
            using assms f by auto
          qed
          also have "... = t \\ V.src t \<star> V.src f \\ f"
            using assms f 4 5 7 VV.arr_char Dom'.arr_char
                  HC.preserves_resid [of "(t, V.src f)" "(V.src t, f)"]
                  VV.con_implies_arr(1) VV.con_implies_arr(2)
            by force
          also have "... = HOM\<^sup>\<rightarrow> a t (HOM a (H.dom t) x f)"
            using Dom'.trg_char Dom.source_is_prfx Dom.sources_char\<^sub>W\<^sub>E
                  arr_f assms(2) f
            by fastforce
          finally show ?thesis by blast
        qed
        show "HOM a (H.cod t) (HOM\<^sup>\<rightarrow> a (V.src t) f) (HOM\<^sup>\<rightarrow> a t x) =
              HOM\<^sup>\<rightarrow> a (V.trg t) f"
        proof -
          have "HOM a (H.cod t) (HOM\<^sup>\<rightarrow> a (V.src t) f) (HOM\<^sup>\<rightarrow> a t x) =
                Cod'.resid (V.src t \<star> f) (t \<star> x)"
            using f Dom.sources_char\<^sub>W\<^sub>E by force
          also have "... = (V.src t \<star> f) \\ (t \<star> V.src f)"
          proof -
            have "\<guillemotleft>V.src t \<star> f : a \<rightarrow> H.cod t\<guillemotright>"
              using assms f Dom'.con_char by fastforce
            moreover have "\<guillemotleft>t \<star> x : a \<rightarrow> H.cod t\<guillemotright>"
              using assms f Dom'.con_char by fastforce
            ultimately show ?thesis
              unfolding Cod'.resid_def
              using assms f
          ...
          qed
          also have "... = V.src t \\ t \<star> f \\ V.src f"
            using assms f arr_f 4 6 VV.arr_char Dom'.arr_char VV.resid_def
                  HC.preserves_resid [of "(V.src t, f)" "(t, V.src f)"]
            by (simp add: VV.con_char)
          also have "... = HOM\<^sup>\<rightarrow> a (V.trg t) f"
            using assms f arr_f
        ...
        qed
        show "Cod.join_of (HOM\<^sup>\<rightarrow> a t x) (HOM\<^sup>\<rightarrow> a (V.src t) f)
                (HOM\<^sup>\<rightarrow> a t f)"
        proof -
          have "Dom.arr f"
            using f Dom.sources_char\<^sub>W\<^sub>E by blast
          moreover have "V.join_of (t \<star> x) (V.src t \<star> f) (t \<star> f)"
            using assms f V.join_is_join_of join_hcomp [of t "V.src t" x f]
    ...
    proof
      interpret au: transformation \<open>HOM a (H.dom u)\<close> \<open>HOM a (H.cod u)\<close>
                      \<open>HOM\<^sup>\<rightarrow> a (V.src u)\<close> \<open>HOM\<^sup>\<rightarrow> a (V.trg u)\<close> \<open>HOM\<^sup>\<rightarrow> a u\<close>
        using assms transformation_cov_HOM_arr [of a u] by fastforce
      interpret at: transformation \<open>HOM a (H.dom t)\<close> \<open>HOM a (H.cod t)\<close>
                      \<open>HOM\<^sup>\<rightarrow> a (V.src t)\<close> \<open>HOM\<^sup>\<rightarrow> a (V.trg t)\<close> \<open>HOM\<^sup>\<rightarrow> a t\<close>
        using assms transformation_cov_HOM_arr [of a t] by fastforce
      fix x
      have "(HOM\<^sup>\<rightarrow> a t \<circ> HOM\<^sup>\<rightarrow> a u) x =
            (if au.A.arr x then (t \<star> u) \<star> x else V.null)"
    ...
    shows "consistent_transformations
             (HOM a (H.dom t)) (HOM a (H.cod t))
             (HOM\<^sup>\<rightarrow> a (V.src t)) (HOM\<^sup>\<rightarrow> a (V.trg t)) (HOM\<^sup>\<rightarrow> a (V.trg u))
             (HOM\<^sup>\<rightarrow> a t) (HOM\<^sup>\<rightarrow> a u)"
    and "cov_HOM a (t \\ u) =
         consistent_transformations.resid
           (HOM a (H.dom t)) (HOM a (H.cod t)) (HOM\<^sup>\<rightarrow> a (V.trg u))
           (HOM\<^sup>\<rightarrow> a t) (HOM\<^sup>\<rightarrow> a u)"
    proof -
      have 1: "HOM a (H.dom u) = HOM a (H.dom t)"
        using assms V.con_imp_eq_src dom_src by metis
      have 2: "HOM a (H.cod u) = HOM a (H.cod t)"
        using assms V.con_imp_eq_src cod_src by metis
      interpret A: sub_rts resid \<open>\<lambda>x. \<guillemotleft>x : a \<rightarrow> H.dom t\<guillemotright>\<close>
        using sub_rts_HOM by blast
      interpret A: extensional_rts A.resid
        using assms V.con_implies_arr extensional_rts_HOM by simp
      interpret B: sub_rts resid \<open>\<lambda>x. \<guillemotleft>x : a \<rightarrow> H.cod t\<guillemotright>\<close>
        using sub_rts_HOM by blast
      ...
      interpret au: transformation A.resid B.resid
                      \<open>HOM\<^sup>\<rightarrow> a (V.src t)\<close> \<open>HOM\<^sup>\<rightarrow> a (V.trg u)\<close> \<open>HOM\<^sup>\<rightarrow> a u\<close>
      ...
      interpret at_au: consistent_transformations A.resid B.resid
                         \<open>HOM\<^sup>\<rightarrow> a (V.src t)\<close> \<open>HOM\<^sup>\<rightarrow> a (V.trg t)\<close> \<open>HOM\<^sup>\<rightarrow> a (V.trg u)\<close>
      ...
      proof (intro transformation_eqI)
        show "extensional_rts (HOM a (H.cod t))" ..
        show "transformation (HOM a (H.dom t)) B.resid
                (cov_HOM a (V.trg u)) at_au.apex at_au.resid"
          using at_au.transformation_resid by blast
        show "transformation (HOM a (H.dom t)) B.resid
                (HOM\<^sup>\<rightarrow> a (V.trg u)) at_au.apex (HOM\<^sup>\<rightarrow> a (t \\ u))"
        proof -
          have "H.dom (t \\ u) = H.dom t"
            using assms dom_resid by blast
          moreover have "H.cod (t \\ u) = H.cod t"
            using assms cod_resid by blast
          moreover have "(\<lambda>x. if residuation.arr (HOM a (H.dom u)) x
                              then V.src (t \\ u) \<star> x else V.null) =
                         (\<lambda>x. if residuation.arr (HOM a (H.dom u)) x
                              then V.trg u \<star> x else V.null)"
            using assms by auto
          moreover have "(\<lambda>x. if A.arr x then V.trg (t \\ u) \<star> x else V.null) = at_au.apex"
          proof
            fix x
            show "(if A.arr x then V.trg (t \\ u) \<star> x else V.null) = at_au.apex x"
            proof (cases "A.arr x")
              case False
              show ?thesis
              ...
              show ?thesis
              proof -
                have 3: "residuation.arr (HOM a (H.dom u)) (A.src x)"
                  using True 1 by auto
                have "\<guillemotleft>t \<star> A.src x : a \<rightarrow> H.cod t\<guillemotright>"
                  using True B.arr_char at.preserves_arr by force
                moreover have "\<guillemotleft>u \<star> A.src x : a \<rightarrow> H.cod t\<guillemotright>"
                  using True 1 A.arr_char A.arr_src_if_arr B.arr_char au.preserves_arr
                  by force
                moreover have "\<guillemotleft>V.trg u \<star> x : a \<rightarrow> H.cod t\<guillemotright>"
                  using assms True A.arr_char con_implies_hpar by fastforce
                moreover have 4: " \<guillemotleft>(t \<star> A.src x) \\ (u \<star> A.src x) : a \<rightarrow> H.cod t\<guillemotright>"
                ...
                moreover have "V.trg (t \\ u) \<star> x =
                               (V.trg u \<star> x) \\ ((t \<star> A.src x) \\ (u \<star> A.src x))"
                proof -
                  have "(V.trg u \<star> x) \\ ((t \<star> A.src x) \\ (u \<star> A.src x)) =
                        (V.trg u \<star> x) \\ ((t \\ u) \<star> A.src x)"
                  ...
                  also have "... = V.trg (t \\ u) \<star> x"
                  proof -
                    have "\<guillemotleft>V.trg u \<star> x : a \<rightarrow> H.cod t\<guillemotright>"
                      using assms True A.arr_char con_implies_hpar by fastforce
                    thus ?thesis
                      using assms True resid_hcomp [of "V.trg u" "t \\ u" x "A.src x"]
    ...
    where "HOM\<^sup>\<leftarrow> c t \<equiv>
           (\<lambda>x. if residuation.arr (HOM (H.cod t) c) x then x \<star> t else V.null)"
    ...
    proof -
      ...
      interpret HOM_b: sub_rts resid \<open>\<lambda>t. \<guillemotleft>t : b \<rightarrow> c\<guillemotright>\<close>
        using sub_rts_HOM by blast
      obtain f g where fg: "H.inverse_arrows f g \<and> H.dom f = a \<and> H.cod f = b"
      ...
      have g: "\<guillemotleft>g : b \<rightarrow> a\<guillemotright>"
        using fg H.inverse_arrows_def
        by (intro H.in_homI) auto
      let ?F = "\<lambda>t. if HOM_b.arr t then t \<star> f else V.null"
      let ?G = "\<lambda>t. if HOM_a.arr t then t \<star> g else V.null"
      ...
      proof
        show "?F \<circ> ?G = Id HOM_a.resid"
        proof
          fix x
          show "(?F \<circ> ?G) x = Id HOM_a.resid x"
            using G.preserves_reflects_arr H.comp_arr_dom H.comp_assoc
                  H.comp_inv_arr HOM_a.arr_char HOM_a.null_char fg
            by auto
        qed
        show "?G \<circ> ?F = Id HOM_b.resid"
        proof
          fix x
          show "(?G \<circ> ?F) x = Id HOM_b.resid x"
          proof (cases "HOM_b.arr x")
            ...
            proof -
              have "obj (f \<star> g)"
                using fg by blast
              moreover have "V.arr (x \<star> f \<star> g)"
                using f g x HOM_b.arr_char by blast
              moreover have "\<guillemotleft>x \<star> f : H.dom f \<rightarrow> c\<guillemotright>"
  ...
  text\<open>
    ...
    invertible as a functor.  An \emph{RTS-equivalence functor} is an equivalence
    functor that is locally an invertible simulation.
  \<close>
  ...
  begin
    ...
    proof -
      ...
      proof
        show "\<And>t. \<not> B.V.arr t \<Longrightarrow> G t = A.V.null"
          using FG.F.extensionality by simp
        show inv: "F \<circ> G = Id resid\<^sub>B" and inv': "G \<circ> F = Id resid\<^sub>A"
  ...
  locale rts_equivalence_functor =
    rts_functor +
    equivalence_functor comp\<^sub>A comp\<^sub>B F +
  assumes locally_isomorphic:
            "\<lbrakk>A.obj a; A.obj b\<rbrakk> \<Longrightarrow>
                invertible_simulation (A.HOM a b) (B.HOM (F a) (F b))
                  (\<lambda>t. if A.H.in_hom t a b then F t else B.V.null)"

  definition equivalent_rts_categories
  where "equivalent_rts_categories resid comp\<^sub>A resid\<^sub>B comp\<^sub>B \<equiv>
         \<exists>F. rts_equivalence_functor resid comp\<^sub>A resid\<^sub>B comp\<^sub>B F"
