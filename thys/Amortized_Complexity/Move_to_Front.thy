...
lemma Chebyshev_sum_upper:
  ...
  shows "real n * (\<Sum>k=0..<n. a k * b k) \<le> (\<Sum>k=0..<n. a k) * (\<Sum>k=0..<n. b k)"
proof -
  let ?S = "(\<Sum>j=0..<n. (\<Sum>k=0..<n. (a j - a k) * (b j - b k)))"
  have "2 * n * (\<Sum>j=0..<n. (a j * b j)) - 2 * (\<Sum>j=0..<n. b j) * (\<Sum>k=0..<n. a k) = ?S"
    by (simp add: algebra_simps setsum_subtractf setsum.distrib real_of_nat_def[symmetric]
      setsum.commute[of "\<lambda>i j. a i * b j"] setsum_right_distrib)
  also
  { fix i j::nat assume "i<n" "j<n"
    ...
    by (force intro!: setsum_nonpos simp: mult_le_0_iff)
...
using Chebyshev_sum_upper[of n "real o a" "real o b"]
by (simp del: real_of_nat_mult real_of_nat_setsum
  add: real_of_nat_mult[symmetric] real_of_nat_setsum[symmetric])
