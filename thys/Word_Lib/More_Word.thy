...
lemma push_bit_mask_eq: \<comment> \<open>TODO move\<close>
...
text \<open>\<close> \<comment> \<open>TODO move\<close>
...
proof -
  from that have \<open>take_bit m (numeral n :: nat) = numeral q\<close>
    by (auto simp add: take_bit_num_def More_Word.numeral_num_of_nat_unfold split: if_splits)
...
lemma take_bit_numeral_minus_numeral_word:
...
next
  case False
  then show ?thesis
    by (transfer fixing: m n) (simp add: take_bit_numeral_minus_numeral_int)
qed

declare take_bit_num_simps [simp] take_bit_num_numeral_simps [simp] take_bit_numeral_numeral [simp]
  take_bit_numeral_minus_numeral_int [simp] take_bit_numeral_minus_numeral_word [simp]

context semiring_bit_operations \<comment> \<open>TODO move, overwrite\<close>
begin
...
lemma nat_mask_eq: \<comment> \<open>TODO move\<close>
  \<open>nat (mask n) = mask n\<close>
  by (simp add: nat_eq_iff of_nat_mask_eq)

lemma (in semiring_bit_operations) unsigned_minus_1_eq_mask: \<comment> \<open>TODO move\<close>
  \<open>unsigned (- 1 :: 'b::len word) = mask LENGTH('b)\<close>
  by (transfer fixing: mask) (simp add: take_bit_minus_one_eq_mask nat_mask_eq of_nat_mask_eq)

declare (in semiring_bit_operations) push_bit_of_1 [simp] \<comment> \<open>TODO move\<close>
...
lemma mask_eq_0_iff [simp]: \<comment> \<open>TODO move\<close>
  \<open>mask n = 0 \<longleftrightarrow> n = 0\<close>
  by (cases n) (simp_all add: mask_Suc_double or_eq_0_iff)

end

lemma mask_nat_less_exp [simp]: \<comment> \<open>TODO move\<close>
  \<open>(mask n :: nat) < 2 ^ n\<close>
  by (simp add: mask_eq_exp_minus_1)

lemma mask_nat_positive_iff [simp]: \<comment> \<open>TODO move\<close>
...
lemma div_word_self: \<comment> \<open>TODO move\<close>
  \<open>w div w = 1\<close> if \<open>w \<noteq> 0\<close> for w :: \<open>'a::len word\<close>
  using that by transfer simp

lemma mod_word_self [simp]:  \<comment> \<open>TODO move\<close>
  \<open>w mod w = 0\<close> for w :: \<open>'a::len word\<close>
  apply (cases \<open>w = 0\<close>)
  apply auto
  using word_mod_div_equality [of w w] by (simp add: div_word_self)

lemma div_word_less: \<comment> \<open>TODO move\<close>
  \<open>w div v = 0\<close> if \<open>w < v\<close> for w v :: \<open>'a::len word\<close>
  using that by transfer simp

lemma mod_word_less: \<comment> \<open>TODO move\<close>
  \<open>w mod v = w\<close> if \<open>w < v\<close> for w v :: \<open>'a::len word\<close>
  using word_mod_div_equality [of w v] using that by (simp add: div_word_less)

lemma div_word_one [simp]: \<comment> \<open>TODO move\<close>
...
lemma mod_word_one [simp]: \<comment> \<open>TODO move\<close>
  \<open>1 mod w = 1 - w * of_bool (w = 1)\<close> for w :: \<open>'a::len word\<close>
  using word_mod_div_equality [of 1 w] by simp

lemma div_word_by_minus_1_eq [simp]: \<comment> \<open>TODO move\<close>
...
lemma mod_word_by_minus_1_eq [simp]: \<comment> \<open>TODO move\<close>
  ...
  using word_mod_div_equality [of w \<open>- 1\<close>] by simp
...
lemmas take_bit_Suc_minus_1 [simp] = take_bit_Suc_1 \<comment> \<open>TODO move\<close>

lemmas take_bit_numeral_minus_1 = take_bit_numeral_1 \<comment> \<open>TODO move\<close>

declare take_bit_minus_one_eq_mask [simp] \<comment> \<open>TODO move, preferred variant\<close>

declare push_bit_minus_one_eq_not_mask [simp] \<comment> \<open>TODO move\<close>

end

lemma bit_minus_numeral_Bit0_Suc_iff [simp]: \<comment> \<open>TODO move\<close>
  \<open>bit (- numeral (num.Bit0 w) :: int) (Suc n) \<longleftrightarrow> bit (- numeral w :: int) n\<close>
  by (simp add: bit_Suc)

lemma bit_minus_numeral_Bit1_Suc_iff [simp]: \<comment> \<open>TODO move\<close>
...
lemma (in ring_bit_operations) push_bit_numeral_minus_1 [simp]: \<comment> \<open>TODO move\<close>
  \<open>push_bit (numeral n) (- 1) = - (2 ^ numeral n)\<close>
  by (simp add: push_bit_eq_mult)

lemma take_bit_tightened_less_eq_nat: \<comment> \<open>TODO move\<close>
...
lemma take_bit_tightened_less_eq_int: \<comment> \<open>TODO move\<close>
...
lemma signed_drop_bit_of_1 [simp]: \<comment> \<open>TODO move\<close>
...
lemma take_bit_word_beyond_length_eq: \<comment> \<open>TODO move\<close>
