...
header{*QR Decomposition over iarrays*}
...
proof (unfold iarray_exhaust2 list_eq_iff_nth_eq, rule conjI, auto, unfold sub_def[symmetric] length_def[symmetric])
  ...
  proof -
    have f1: "i < card (UNIV\<Colon>'rows set)" by (metis i length_eq_card_rows)
    have "\<And>x\<^sub>4. vec_to_iarray x\<^sub>4 = IArray (map (\<lambda>uua. x\<^sub>4 $ (from_nat uua\<Colon>'cols)\<Colon>real) [0..<card (UNIV\<Colon>'cols set)])" by (simp add: vec_to_iarray_def)
    hence "0 < card (UNIV\<Colon>'rows set) \<and> length (IArray.list_of (IArray (map (\<lambda>R. IArray.of_fun (\<lambda>Ra. ((1 / norm_iarray (column_iarray Ra (IArray (map (\<lambda>R. vec_to_iarray (A $ from_nat R)) [0..<card (UNIV\<Colon>'rows set)])))) *\<^sub>R column_iarray Ra (IArray (map (\<lambda>R. vec_to_iarray (A $ from_nat R)) [0..<card (UNIV\<Colon>'rows set)]))) !! R) (card (UNIV\<Colon>'cols set))) [0..<card (UNIV\<Colon>'rows set)]) !! i)) = length (IArray.list_of (IArray (map (\<lambda>R. vec_to_iarray (divide_by_norm A $ from_nat R)) [0..<card (UNIV\<Colon>'rows set)]) !! i))" using f1 by auto
    thus "IArray.length (IArray (map (\<lambda>x. vec_to_iarray (divide_by_norm A $ from_nat x)) [0..<card (UNIV\<Colon>'rows set)]) !! i) = IArray.length (IArray.of_fun (\<lambda>i. IArray.of_fun (\<lambda>b. ((1 / norm_iarray (column_iarray b (IArray (map (\<lambda>x. vec_to_iarray (A $ from_nat x)) [0..<card (UNIV\<Colon>'rows set)])))) *\<^sub>R column_iarray b (IArray (map (\<lambda>x. vec_to_iarray (A $ from_nat x)) [0..<card (UNIV\<Colon>'rows set)]))) !! i) (IArray.length (IArray (map (\<lambda>x. vec_to_iarray (A $ from_nat x)) [0..<card (UNIV\<Colon>'rows set)]) !! 0))) (IArray.length (IArray (map (\<lambda>x. vec_to_iarray (A $ from_nat x)) [0..<card (UNIV\<Colon>'rows set)]))) !! i)" by (simp add: vec_to_iarray_def)
  qed
