(*  Title:       BDD
    ID:          $Id: LevellistProof.thy,v 1.6 2008-06-11 14:22:50 lsf37 Exp $
...
lemma list_ext: 
  "\<And>ys. \<lbrakk>length xs = length ys; \<forall>i < length xs. xs!i=ys!i\<rbrakk> \<Longrightarrow> xs=ys"
apply (induct xs)
apply  simp
apply (case_tac ys)
apply  simp
...
proof -
  ...
  show ?thesis
  proof (rule list_ext [OF leq, rule_format])
    fix i
    assume "i < length ll"
    with ll ll'
    show "ll!i = ll'!i"
