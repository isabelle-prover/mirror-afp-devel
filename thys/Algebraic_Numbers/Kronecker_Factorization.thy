...
definition kronecker_factorization_main :: "int poly \<Rightarrow> int poly option" where
  "kronecker_factorization_main p \<equiv> if degree p \<le> 1 then None else let 
   ...
   | None \<Rightarrow> let djs = map (\<lambda> (v,j). map (Pair j) (if j = 0 then dp v else df v)) cjs in 
     map_option the (find_map_filter interpolation_poly_int 
...
proof -
  ...
  from res have res: "(case map_of ?zero 0 of
     None \<Rightarrow> map_option the (find_map_filter interpolation_poly_int filt tests) | Some j \<Rightarrow> Some [:- j, 1:]) =
     Some q" by auto
  ...
  next
    ...
    have res: "map_option the (find_map_filter interpolation_poly_int filt tests) = Some q" by auto
    then obtain qq where 
      res: "find_map_filter interpolation_poly_int filt tests = Some qq" and q: "q = the qq" 
      by (auto split: option.splits)
    from find_map_filter_Some[OF res] 
    have filt: "filt qq" and tests: "qq \<in> interpolation_poly_int ` set tests" by auto
    ...
    from tests obtain t where t: "t \<in> set tests" and l: "interpolation_poly_int t = Some g" unfolding qq 
      by auto
...
proof -
  ...
  with res have res: "find_map_filter interpolation_poly_int filt tests = None" by auto
  {
    fix qq
    ...
    have nfilt: "\<not> filt (interpolation_poly_int t)" by auto
    ...
    from interpolation_poly_int_None[OF dist _ qt degq] have
      "interpolation_poly_int t \<noteq> None" by auto
    then obtain g where lt: "interpolation_poly_int t = Some g" by auto
