...
definition "divisors_nat_copy n \<equiv> if n = 0 then [] else remdups_adj (sort (map listprod (sublists (prime_factorization_nat n))))"
...
proof -
  ...
  from res have res: "(case map_of ?zero 0 of
     None \<Rightarrow> map_option the (find_map_filter lagrange_interpolation_poly_int filt tests) | Some j \<Rightarrow> Some [:- j, 1:]) =
     Some q" by auto
  ...
  next
    ...
    have res: "map_option the (find_map_filter lagrange_interpolation_poly_int filt tests) = Some q" by auto
    then obtain qq where 
      res: "find_map_filter lagrange_interpolation_poly_int filt tests = Some qq" and q: "q = the qq" 
      by (auto split: option.splits)
    from find_map_filter_Some[OF res] 
    have filt: "filt qq" and tests: "qq \<in> lagrange_interpolation_poly_int ` set tests" by auto
    ...
    from tests obtain t where t: "t \<in> set tests" and l: "lagrange_interpolation_poly_int t = Some g" unfolding qq 
      by auto
    ...
    from lagrange_interpolation_int_Some[OF dist[folded id] l, unfolded lent lenj]
...
proof -
  ...
  with res have res: "find_map_filter lagrange_interpolation_poly_int filt tests = None" by auto
  {
    fix qq
    ...
    have nfilt: "\<not> filt (lagrange_interpolation_poly_int t)" by auto
    have qt: "\<And>x y. (x, y) \<in> set t \<Longrightarrow> poly q x = y" unfolding t_def by auto
    from lagrange_interpolation_int_None[OF dist _ qt degq] have
      "lagrange_interpolation_poly_int t \<noteq> None" by auto
    then obtain g where lt: "lagrange_interpolation_poly_int t = Some g" by auto
    from lagrange_interpolation_int_Some[OF dist lt] 
