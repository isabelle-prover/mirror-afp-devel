...
instantiation rat :: ring_div_gcd_field
begin
definition "div_rat = (op / :: rat \<Rightarrow> rat \<Rightarrow> rat)"
definition "mod_rat (x :: rat) (y :: rat) = (if y = 0 then x else 0)"
definition "gcd_rat (x :: rat) (y :: rat) = (if x = 0 \<and> y = 0 then (0 :: rat) else 1)"
definition "lcm_rat (x :: rat) (y :: rat) = x * y"
instance
  by (intro_classes, auto simp: div_rat_def mod_rat_def gcd_rat_def)
end

instantiation real :: ring_div_gcd_field
begin
definition "div_real = (op / :: real \<Rightarrow> real \<Rightarrow> real)"
definition "mod_real (x :: real) (y :: real) = (if y = 0 then x else 0)"
definition "gcd_real (x :: real) (y :: real) = (if x = 0 \<and> y = 0 then (0 :: real) else 1)"
definition "lcm_real (x :: real) (y :: real) = x * y"
instance
  by (intro_classes, auto simp: div_real_def mod_real_def gcd_real_def)
end

instantiation complex :: ring_div_gcd_field
begin
definition "div_complex = (op / :: complex \<Rightarrow> complex \<Rightarrow> complex)"
definition "mod_complex (x :: complex) (y :: complex) = (if y = 0 then x else 0)"
definition "gcd_complex (x :: complex) (y :: complex) = (if x = 0 \<and> y = 0 then (0 :: complex) else 1)"
definition "lcm_complex (x :: complex) (y :: complex) = x * y"
instance
  by (intro_classes, auto simp: div_complex_def mod_complex_def gcd_complex_def)
end
...
(* TODO: is there nothing similar to semiring_gcd? 
   semiring_gcd defines lcm as purely syntactic! *)
definition list_lcm :: "int list \<Rightarrow> int" where
...
lemma list_lcm_int: "x \<in> set xs \<Longrightarrow> x dvd list_lcm xs" 
proof (induct xs)
  case (Cons y ys)
  ...
  proof (cases "x = y")
    ...
    thus ?thesis unfolding list_lcm_simps by simp
  qed (simp add: list_lcm_simps)
qed simp

lemma list_lcm_least_int: "(\<And> x. x \<in> set xs \<Longrightarrow> x dvd y) \<Longrightarrow> list_lcm xs dvd y"
proof (induct xs)
  case (Cons x xs)
  from Cons have "x dvd y" "list_lcm xs dvd y" by auto
  thus ?case unfolding list_lcm_simps by (rule lcm_least_int)
qed (simp add: list_lcm_simps)
...
lemma lcm_mult_distrib_int: "abs (k::int) * lcm m n = lcm (k * m) (k * n)"
  ...
  unfolding lcm_mult_distrib_nat[symmetric] by (simp add: of_nat_mult)

lemma list_lcm_mult_int[simp]: 
  "list_lcm (map (op * a) xs) = (if xs = [] then 1 else \<bar>a\<bar> * list_lcm xs)"
  by (induct xs, auto simp: list_lcm_simps lcm_mult_distrib_int abs_mult)

lemma list_lcm_pos: "list_lcm xs \<ge> 0" "0 \<notin> set xs \<Longrightarrow> list_lcm xs \<noteq> 0"
