...
proof -
  ...
  with r have deg: "degree r' \<noteq> 0" unfolding r' poly_dvd_1 dvd_field_iff by (cases r', auto)
...
lemma coprime_idom_prime_divisorI:
  assumes no_prime_divisor: "\<And> r :: 'a :: factorial_ring_gcd. r dvd p \<Longrightarrow> r dvd q 
    \<Longrightarrow> (\<And> s t. r dvd s * t \<Longrightarrow> r dvd s \<or> r dvd t) \<Longrightarrow> r dvd 1"
  shows "coprime p q" 
proof (rule ccontr)
  let ?gcd = "gcd p q" 
  assume "?gcd \<noteq> 1" 
  hence no_unit: "\<not> is_unit ?gcd" by simp
  {
    assume "?gcd \<noteq> 0" 
  ...
  {
    assume "?gcd = 0" 
    hence "p = 0" "q = 0" by auto
    from no_prime_divisor[of 0, unfolded this] have False by auto
  }
...
lemma poly_x_minus_y_poly_lift_coprime_complex:
  ...
  shows "coprime (poly_x_minus_y p) (poly_lift q)" (is "coprime ?p ?q")
...
lemma poly_x_div_y_poly_lift_coprime_complex:
  ...
  shows "coprime (poly_x_div_y p) (poly_lift q)" (is "coprime ?p ?q")
proof (rule coprime_idom_prime_divisorI, rule ccontr)
  fix r assume "r dvd ?q" and r: "\<not> r dvd 1"
