...
begin
(* TODO: inform Isabelle developers *)
text \<open>Create a local copy of @{const pderiv}, as the current version
  is only defined for @{class real_normed_field}. Once this is fixed
  in the distribution, the copy should be removed. The lemmas for 
  the copy are literally copied from the distribution.\<close>

qualified function pderiv :: "'a poly \<Rightarrow> 'a poly"
where
...
private lemma pderiv_0 [simp]:
  "pderiv 0 = 0"
  using pderiv.simps [of 0 0] by simp

private lemma pderiv_pCons:
  "pderiv (pCons a p) = p + pCons 0 (pderiv p)"
  by (simp add: pderiv.simps)

private lemma coeff_pderiv: "coeff (pderiv p) n = of_nat (Suc n) * coeff p (Suc n)"
  by (induct p arbitrary: n) 
     (auto simp add: pderiv_pCons coeff_pCons algebra_simps split: nat.split)

(* faster implementation than in distribution *)
fun pderiv_coeffs_code :: "'a \<Rightarrow> 'a list \<Rightarrow> 'a list" where
  "pderiv_coeffs_code f (x # xs) = cCons (f * x) (pderiv_coeffs_code (f+1) xs)"
| "pderiv_coeffs_code f [] = []"

definition pderiv_coeffs :: "'a list \<Rightarrow> 'a list" where
...
proof (induct xs arbitrary: f n)
  ...
  next
    ...
    proof (cases "pderiv_coeffs_code (f + 1) xs = [] \<and> f * x = 0")
      case False
      hence "nth_default 0 (pderiv_coeffs_code f (x # xs)) n = nth_default 0 (pderiv_coeffs_code (f + 1) xs) m" 
        by (auto simp: cCons_def n)
      also have "\<dots> = (f + of_nat n) * (nth_default 0 xs m)" unfolding Cons by (simp add: n)
...
private lemma coeffs_pderiv [code abstract]:
  "coeffs (pderiv p) = pderiv_coeffs (coeffs p)" unfolding pderiv_coeffs_def
proof (rule coeffs_eqI, unfold pderiv_coeffs_code coeff_pderiv)
  fix n
  ...
  show "of_nat (Suc n) * coeff p (Suc n) =
         (1 + of_nat n) * nth_default 0 (tl (coeffs p)) n"
    unfolding coeffs_def map_upt_Suc
    by (cases "p = 0", auto simp: id)
next
  obtain n xs where id: "tl (coeffs p) = xs" "(1 :: 'a) = n" by auto
  show "pderiv_coeffs_code 1 (tl (coeffs p)) \<noteq> [] \<Longrightarrow> last (pderiv_coeffs_code 1 (tl (coeffs p))) \<noteq> 0"
    unfolding id
    by (induct xs arbitrary: n, auto simp: cCons_def)
qed    


private lemma pderiv_eq_0_iff: "pderiv p = 0 \<longleftrightarrow> degree p = 0"
  apply (rule iffI)
  apply (cases p, simp)
...
private lemma degree_pderiv: "degree (pderiv p) = degree p - 1"
  ...
  apply (insert leading_coeff_0_iff,  auto)
  apply (metis degree_0 leading_coeff_0_iff nat.distinct(1))
  done

private lemma pderiv_singleton [simp]: "pderiv [:a:] = 0"
by (simp add: pderiv_pCons)

private lemma pderiv_add: "pderiv (p + q) = pderiv p + pderiv q"
by (rule poly_eqI, simp add: coeff_pderiv algebra_simps)

private lemma pderiv_minus: "pderiv (- p) = - pderiv p"
by (rule poly_eqI, simp add: coeff_pderiv)

private lemma pderiv_diff: "pderiv (p - q) = pderiv p - pderiv q"
by (rule poly_eqI, simp add: coeff_pderiv algebra_simps)

private lemma pderiv_smult: "pderiv (smult a p) = smult a (pderiv p)"
by (rule poly_eqI, simp add: coeff_pderiv algebra_simps)

private lemma pderiv_mult: "pderiv (p * q) = p * pderiv q + q * pderiv p"
by (induct p) (auto simp: pderiv_add pderiv_smult pderiv_pCons algebra_simps)

private lemma pderiv_power_Suc:
...
text \<open>The following lemmas are not present in the distribution.\<close>  

lemma pderiv_1[simp]: "pderiv (1 :: 'a poly) = 0" unfolding one_poly_def by simp

lemma pderiv_setprod: "pderiv (setprod f (as)) = 
  (\<Sum> a \<in> as. setprod f (as - {a}) * pderiv (f a :: 'a poly))"
proof (induct as rule: infinite_finite_induct)
  case (insert a as)
...
lemma not_dvd_pderiv: assumes "degree p \<noteq> 0"
