...
proof -
  ...
  proof (cases "degree p = 1")
    ...
    from True have rr: "?rr = {Rational (Rat.Fract (- c) (d))}" unfolding d d_def c_def by auto
  ...
  next
    ...
    have empty: "Ball (set empty) invariant_2" unfolding empty_def by auto
    ...
    have "real_of_2 ` set (?main bnds empty) =
      ...
      Ball (set (?main bnds empty)) invariant_2" (is "?one' \<and> ?two'")
      using empty bnds
        ...
        next
          ...
          next
            ...
            have "real_of_2 ` set (?main ?lrs rais) =
              ...
              Ball (set (?main ?lrs rais)) invariant_2"
              (is "?one \<and> ?two")
              by (rule IH)
            hence ?one ?two by blast+
...
lemma roots_of_3: "p \<noteq> 0 \<Longrightarrow> real_of_3 ` set (roots_of_3 p) = {x. ipoly p x = 0}"
...
lemma real_roots_of_int_poly: assumes "p \<noteq> 0" 
  shows "set (real_roots_of_int_poly p) = {x. ipoly p x = 0}" 
  unfolding real_roots_of_int_poly_def using roots_of_real_alg[OF assms] by auto

lemma real_roots_of_rat_poly: assumes "p \<noteq> 0" 
  shows "set (real_roots_of_rat_poly p) = {x. rpoly p x = 0}" 
proof -
    ...
    next
      ...
      next
        ...
        also have "\<dots> = real_of_rat_poly ?q"
          ...
          using p by auto
        from real_roots_of_rat_poly[OF q, folded id l] show ?thesis by simp
      qed
    qed
  qed
qed
