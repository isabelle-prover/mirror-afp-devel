...
lemma admissible_tailrec:
  "ccpo.admissible
     (fun_lub (flat_lub undefined)) (fun_ord tailrec_ord)
     (\<lambda>a. \<forall>x. a x \<noteq> undefined \<longrightarrow> P x (a x))"
proof(rule ccpo.admissibleI[OF tailrec.ccpo], intro strip)
  fix A x
...
proof -
  have "\<forall>x y. U f x = y \<longrightarrow> y \<noteq> undefined \<longrightarrow> P x y"
    by(rule tailrec.fixp_induct_uc[of U F C, OF mono eq inverse2])(auto intro: step admissible_tailrec)
...
declaration {* Partial_Function.init "tailrec" @{term tailrec.fixp_fun}
  ...
  (SOME @{thm fixp_induct_tailrec_undefined}) *}
