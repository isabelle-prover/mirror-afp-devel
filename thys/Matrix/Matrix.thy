(*  Title:       Executable Matrix Operations on Matrices of Arbitrary Dimensions
    ...
    Maintainer:  Christian Sternagel and RenÃ\<copyright> Thiemann
*)
...
definition vec_geI :: "('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a vec \<Rightarrow> 'a vec \<Rightarrow> bool"
  where "vec_geI ge v w \<equiv> list_all (\<lambda> (a,b). ge a b) (zip v w)"

(* comparison of matrices using >= componentwise *)
definition mat_geI :: "('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a mat \<Rightarrow> 'a mat \<Rightarrow> bool"
  where "mat_geI ge m1 m2 \<equiv> list_all (\<lambda> (v,w). vec_geI ge v w) (zip m1 m2)"
...
definition mat_gtI :: "('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> ('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> nat \<Rightarrow> 'a mat \<Rightarrow> 'a mat \<Rightarrow> bool"
  where "mat_gtI ge gt sd m1 m2 \<equiv> mat_geI ge m1 m2 \<and> mat_pre_gtI gt (sub_mat sd sd m1) (sub_mat sd sd m2)"
...
(* max0 on matrices *)
definition mat_max0I :: "('a \<Rightarrow> 'a) \<Rightarrow> 'a mat \<Rightarrow> 'a mat"
  where "mat_max0I \<equiv> mat_map"
...
by(simp add: mat_def mat0I_def set_replicate_conv_if)
...
proof (simp, induct nc arbitrary: m1 m2, simp)
  ...
  next
    ...
    next
      ...
      show ?thesis by  (simp add: Cons oCons, rule conjI, rule vec_plus, (simp add: Cons oCons Suc)+, rule Suc, auto simp: Cons oCons Suc l1 l2)
    qed
  qed
qed
...
using assms 
proof (induct m arbitrary: nc, simp add: mat_def vec_def set_replicate_conv_if)
  case (Cons v m)
  ...
  from `mat nr nc (v # m)` have wfV: "length v = nr" unfolding mat_def by (simp add:  vec_def)
  show ?case unfolding mat_def
  proof (rule conjI, simp add: wfV wfRec mat_def vec_def)
    show "min nr (length (transpose nr m)) = nr" using wfRec unfolding mat_def by auto
  next
    show "list_all (vec nc) (transpose nr (v # m))"
    proof (simp add: vec_def nc, rule ballI, simp only: split_paired_all)
      fix a b
      ...
      thus "length (split op # (a,b)) = Suc ncc" by auto
    qed
  qed
qed
...
lemma mat_max0: assumes "mat nr nc m" shows "mat nr nc (mat_max0I mz m)"
using assms unfolding mat_max0I_def by (rule mat_map)
...
using wf 
proof (induct m arbitrary: nc , simp add: col_def row_def mat_def i)
  case (Cons v m)
  ...
  show ?case proof (rule nth_equalityI, simp add: col_def row_def lengths lengths2 i, intro allI impI)
    fix j
    have simpme: "transpose nr m ! i = row m i" using colRec unfolding col_def by auto
    assume "j < length (col (transpose nr (v # m)) i)"
    hence "j < Suc ncc" by (simp add: col_def lengths2 lengths i) 
    thus "col (transpose nr (v # m)) i ! j = row (v # m) i ! j"
...
lemma mat_max0_index: assumes wf: "mat nr nc m" and i: "i < nc" and j: "j < nr" 
  shows "mat_max0I mz m ! i ! j = mz (m ! i ! j)"
unfolding mat_max0I_def using assms by (rule mat_map_index)
...
lemma vec_ge_index : assumes "vec nr v1" 
  and "vec nr v2"
  shows "vec_geI ge v1 v2 = (\<forall> i < nr. ge (v1 ! i) (v2 ! i))"
using assms
unfolding vec_def vec_geI_def 
proof (simp, induct nr arbitrary: v1 v2, simp)
  case (Suc nrr)
  ...
  have rec: "(\<forall> a \<in> set (zip w1 w2). split ge a) = (\<forall> i < nrr. ge (w1 ! i) (w2 ! i))"
...
lemma mat_ge_index : assumes "mat nr nc m1" 
  and "mat nr nc m2"
  shows "mat_geI ge m1 m2 = (\<forall> i < nc. \<forall> j < nr. ge (m1 ! i ! j) (m2 ! i ! j))"
using assms
unfolding mat_def mat_geI_def
proof (simp, induct nc arbitrary: m1 m2, simp)
  case (Suc ncc)
  ...
  have rec: "(\<forall> a \<in> set (zip mm1 mm2). split (vec_geI ge) a) = (\<forall> i < ncc. \<forall> j < nr. ge (mm1 ! i ! j) (mm2 ! i ! j))"
    by (rule Suc, auto simp: Suc lm1 lm2)
  show ?case (is "?l = ?r")
  proof (rule iffI)
    assume ?r
    thus ?l using m1 m2 lm1 lm2 rec vec_ge_index[OF wf1 wf2] by auto
  next
    assume ?l
    hence ge: "vec_geI ge v1 v2" and "\<forall> a \<in> set (zip mm1 mm2). split (vec_geI ge) a" using m1 m2 by auto
    with rec have ge2: " (\<forall>i<ncc. \<forall>j<nr. ge (mm1 ! i ! j) (mm2 ! i ! j))" by simp
    show ?r
    proof (rule allI, rule impI)
      fix i 
      assume i: "i < Suc ncc" 
      show "\<forall> j < nr. ge (m1 ! i ! j) (m2 ! i ! j)"
      proof (cases i, simp add: m1 m2, simp only: vec_ge_index[OF wf1 wf2, symmetric], rule ge)
	case (Suc ii)	
...
proof -
  ...
  show ?thesis
  proof (simp only: Not_eq_iff[symmetric, of "mat_pre_gtI gt m1 m2"], unfold mat_pre_gtI_def, simp, simp only: set_zip l2 min_max.inf_idem)
    have "(\<forall> x \<in> {(m1 ! i, m2 ! i) | i. i < length m1}. \<not> split (vec_pre_gtI gt) x) = (\<forall> i < nc. \<not> vec_pre_gtI gt (m1 ! i) (m2 ! i))" (is "?l = _")
      by (auto simp: l1)
    also have "\<dots> = (\<forall> i < nc. \<forall> j < nr. \<not> gt (m1 ! i ! j) (m2 ! i ! j))" (is "_ = ?r")
    proof -
      let ?l = "\<lambda> i. \<not> vec_pre_gtI gt (m1 ! i) (m2 ! i)"
      let ?r = "\<lambda> i. \<forall> j < nr. \<not> gt (m1 ! i ! j) (m2 ! i ! j)"
      have "\<forall> i < nc. ?l i = ?r i"
      proof (intro allI impI)
	fix i
	assume i: "i < nc"
...
lemma mat_gt_index : assumes wf1: "mat n n m1"
  and wf2: "mat n n m2"
  and sd: "sd \<le> n"
  shows "mat_gtI ge gt sd m1 m2 = (mat_geI ge m1 m2 \<and> (\<exists> i < sd. \<exists> j < sd. gt (m1 ! i ! j) (m2 ! i ! j)))"
proof -
...
proof -
  ...
  show ?thesis unfolding mat_monoI_def sub_mat_def sub_vec_def 
  proof (simp)
    show "(\<forall> x \<in> set (take sd m). \<exists> x \<in> set (take sd x). geq1 x) = (\<forall> i < sd. \<exists> j < sd. geq1 (m ! i ! j))"
      (is "?l = ?r")
    proof
      assume ?l
      show ?r
...
proof (induct x arbitrary: y, simp)
  case (Cons a x)
  thus ?case 
    by (cases y, simp, simp add: add_commute) 
qed
...
proof (induct nr arbitrary: u, simp)
...
using assms 
unfolding mat_def 
proof (induct nc arbitrary: m, simp add: mat_plusI_def mat0I_def)
  case (Suc nn) 
  thus ?case proof (cases m, simp)
    case (Cons v mm)
...
using assms 
proof (induct v1 arbitrary: v2, simp add: scalar_prodI_def)
  case (Cons y1 w1)
...
proof (induct m arbitrary: nc c)
  case Nil
  hence "nc = 0" unfolding mat_def by (auto)
  with Nil show ?case 
  proof (simp add: scalar_prodI_def row_def)
    ...
    qed
    thus "zero = foldr ?fun (zip r (map (\<lambda> k. zero) [0..<nr])) zero" by (simp add: nr)
  qed
next
  case (Cons v m)
  ...
  qed
  show ?case 
    by (simp add: c scalar_prod_cons, simp only: nc, simp only: id, simp add: scalar_prod_cons, simp only: rec, simp only: nr, simp only: idtm1 idtm2, simp only: main, simp only: idtm2[symmetric], simp add: row_def scalar_prod_cons)
qed
...
proof (induct nn arbitrary: v, simp)
  case (Suc m)
...
lemma scalar_right_zero : "scalar_prod v (vec0 nn) = zero"
  unfolding vec0I_def scalar_prodI_def
proof (induct v arbitrary: nn, simp)
  case (Cons a vv)
...
proof (induct nr arbitrary: u v w, simp)
  case (Suc n)
...
proof (induct nn arbitrary: v i, blast)
...
(* preorders are not used, since the condition 
   (x > y = x \<ge> y \<and> \<not> y \<ge> x)
   is sometimes not applicable. E.g., for polynomials over the naturals we have 2x \<ge> x, \<not> x \<ge> 2x, but \<not> 2x > x for x = 0
*)
...
begin
abbreviation vec_ge :: "'a vec \<Rightarrow> 'a vec \<Rightarrow> bool"
where "vec_ge \<equiv> vec_geI ge"

abbreviation mat_ge :: "'a mat \<Rightarrow> 'a mat \<Rightarrow> bool"
where "mat_ge \<equiv> mat_geI ge"

      
lemma vec_ge_refl: "vec_ge v v"
  unfolding vec_geI_def 
  by (induct v, auto simp: ge_refl)

lemma mat_ge_refl: "mat_ge m m"
  unfolding mat_geI_def
  by (induct m, auto simp: vec_ge_refl)
...
lemma plus_right_mono: "ge y z \<Longrightarrow> ge (x + y) (x + z)"
  by (simp add: add_commute[of x], rule plus_left_mono, auto)

lemma vec_plus_left_mono: assumes ge: "vec_ge v1 v2" and wf_1: "vec nr v1" and wf_2: "vec nr v2" and wf_3: "vec nr v3"
...
lemma mat_plus_left_mono: assumes ge: "mat_ge m1 m2" and wf_1: "mat nr nc m1" and wf_2: "mat nr nc m2" and wf_3: "mat nr nc m3" 
...
lemma scalar_prod_mono_left: assumes wf1: "vec nr v1"
  and wf2: "vec nr v2"
  and wf3: "vec nr v3"
  and ge1: "vec_ge v1 v2"
  and ge2: "vec_ge v3 (vec0 nr)"
  shows "ge (scalar_prod v1 v3) (scalar_prod v2 v3)"
using assms unfolding vec_def vec_geI_def vec0I_def
proof (simp)
  assume "length v1 = nr" and "length v2 = nr" and " length v3 = nr" and " \<forall>x\<in>set (zip v1 v2). split ge x" and " \<forall>x\<in>set (zip v3 (replicate nr 0)). split ge x"
  thus "ge (scalar_prod v1 v3) (scalar_prod v2 v3)"
  proof (induct nr arbitrary: v1 v2 v3, simp add: scalar_prodI_def ge_refl)
    case (Suc nrr)
...
lemma scalar_prod_mono_right: assumes wf1: "vec nr v1"
  and wf2: "vec nr v2"
  and wf3: "vec nr v3"
...
using assms unfolding vec_def vec_geI_def vec0I_def
proof (simp)
  assume "length v1 = nr" and "length v2 = nr" and " length v3 = nr" and " \<forall>x\<in>set (zip v2 v3). split ge x" and " \<forall>x\<in>set (zip v1 (replicate nr 0)). split ge x"
...
proof -
  ...
  proof (simp only: mat_ge_index[OF wf13 wf23], intro allI impI)
	...
	simp add: row_col[OF wf1 j] row_col[OF wf2 j] col_def, (auto simp: ge1a j)[1],
...
lemma mat1_ge_mat0: "mat_ge (mat1 n) (mat0 n n)"
unfolding mat_ge_index[OF mat1 mat0]
proof (intro allI impI)
  fix i j
  assume i: "i < n" and j: "j < n"
  have zero_ij: "mat0 n n ! i ! j = 0" by (rule mat0_index[OF i j])
  have one_ij: "mat1 n ! i ! j = (if i = j then 1 else 0)" by (rule mat1_index[OF i j])
  show "mat1 n ! i ! j \<ge> mat0 n n ! i ! j"
...
abbreviation mat_max0 :: "'a mat \<Rightarrow> 'a mat"
where "mat_max0 \<equiv> mat_max0I max0"
...
proof (unfold mat_eq_index[OF mat_max0[OF wf] wf], intro allI impI)
  ...
  hence "m ! i ! j \<ge> 0" using mat0_index[OF i j, of 0] by simp
...
(*
   we do not use a class here, but only a locale since we often 
   have parametric orders, e.g. on rational numbers there are several orders 
   > where x > y = x \<ge> y + \<delta> for some parameter \<delta>
*)
locale order_pair = 
...
where "mat_gt \<equiv> mat_gtI ge gt"
...
proof -
  ...
  show ?thesis 
    by (simp add: mat_gt_index[OF mat1 mat0 sd_n] mat_default_ge_mat0,
      (rule exI[of _ 0], simp add: sd_pos)+, simp add: mat1_index[OF n n] mat0_index[OF n n] default_gt_zero)
qed
end
...
locale SN_one_mono_ordered_semiring_1 = one_mono_ordered_semiring_1 + order_pair + 
  assumes SN: "SN {(x,y) . ((y :: 'a) \<ge> 0) \<and> (gt x y)}"
begin
...
proof -
  ...
  show ?thesis 
  proof (simp only: mat_gt_index[OF wf12 wf13 sd_n], simp add: gem, 
      rule exI[of _ i], simp add: i, 
      rule exI[of _ k], simp add: k)
...
where "mat_gt_arc \<equiv> mat_geI gt"

lemma scalar_prod_left_mono: assumes wf1: "vec nr v1"
  and wf2: "vec nr v2"
  and wf3: "vec nr v3"
  and gt1: "vec_geI gt v1 v2"
  shows "gt (scalar_prod v1 v3)  (scalar_prod v2 v3)"
proof -
  from vec_ge_index[OF wf1 wf2] gt1 have g1: "\<forall> i < nr. gt (v1 ! i) (v2 ! i)" by auto
  from g1 wf1 wf2 wf3 show ?thesis unfolding vec_def
  proof (induct nr arbitrary: v1 v2 v3, simp add: scalar_prodI_def zero_leastI)
    case (Suc nrr)
    from Suc obtain a1 w1 where v1: "v1 = a1 # w1" and w1: "length w1 = nrr" by (cases v1, auto)
    from Suc obtain a2 w2 where v2: "v2 = a2 # w2" and w2: "length w2 = nrr" by (cases v2, auto)
    from Suc obtain a3 w3 where v3: "v3 = a3 # w3" and w3: "length w3 = nrr" by (cases v3, auto)
...
proof (simp only: mat_ge_index[OF wf1 wf3], intro allI impI)
  fix i j
  assume i: "i < nc" and j: "j < nr"
  from ge i j have one: "m1 ! i ! j \<ge> m2 ! i ! j" using mat_ge_index[OF wf1 wf2] by auto
  from gt i j have two: "gt (m2 ! i ! j) (m3 ! i ! j)" using mat_ge_index[OF wf2 wf3] by auto
...
proof (simp only: mat_ge_index[OF wf1 wf3], intro allI impI)
  fix i j
  assume i: "i < nc" and j: "j < nr"
  from gt i j have one: "gt (m1 ! i ! j) (m2 ! i ! j)" using mat_ge_index[OF wf1 wf2] by auto
...
proof -
  ...
  show ?thesis 
  proof (simp only: mat_ge_index[OF wfxz wfyu], intro allI impI)
    fix i j
    assume i: "i < nc" and j: "j < nr"
	...
	simp only: mat_plus_index[OF wfy wfu i j],
      ...
      show "gt (x ! i ! j) (y ! i ! j)" using gt1 i j mat_ge_index[OF wfx wfy] by auto
    next
      show "gt (z ! i ! j) (u ! i ! j)" using gt2 i j mat_ge_index[OF wfz wfu] by auto
    qed
  qed
qed
...
proof -
  ...
  show ?thesis 
  proof (simp only: mat_ge_index[OF wfxz wfyz], intro allI impI)
    fix i j
    assume i: "i < ncc" and j: "j < nr"
	...
	simp only: vec_ge_index[OF wfxj wfyj],
	intro allI impI
      )
      fix k
      assume k: "k < nc"
      from gt1 mat_ge_index[OF wfx wfy, of gt] j k
...
proof (simp only: mat_eq_index[OF wf mat0], intro allI impI)
  ...
  proof (simp only: mat0_index[OF i j], rule zero_leastII)
    show "gt 0 (m ! i ! j)" using i j gt mat_ge_index[OF mat0 wf, of gt 0] mat0_index[OF i j, of "0 :: 'a"] by force
  qed
qed
...
proof (simp only: mat_ge_index[OF mat1 mat0], intro allI impI)
  fix i j
  assume i: "i < n" and j: "j < n"
...
proof (rule ccontr)
  ...
  proof
    ...
    show "gt (f i ! 0 ! 0)  (f (Suc i) ! 0 ! 0)" using mat_ge_index[OF wf1 wf2, of gt] mat0_index[OF n_pos n_pos, of 0] gt n_pos by force
  qed
...
proof -
  ...
  show ?thesis unfolding mat_arc_posI_def 
    ...
    simp add: m1 v1 m2 v2 col_def row_def scalar_prod_cons,
    rule arc_pos_plus[OF prod])
qed
...
where "weak_mat_gt \<equiv> mat_gtI ge weak_gt"
...
lemma weak_mat_gt_mono: assumes sd_n: "sd \<le> n" and
   ...
   shows "\<exists> gt. SN_strict_mono_ordered_semiring_1 default gt mono \<and> (\<forall> m1 m2. mat n n m1 \<and> mat n n m2 \<and> (m1, m2) \<in> set m12s \<longrightarrow> mat_gtI ge gt sd m1 m2)"
proof -
  ...
  proof (intro exI allI conjI impI, rule order)
    ...
    show "mat_gtI op \<ge> gt sd m1 m2" unfolding mat_gt_index[OF wf1 wf2 sd_n]
...
where "weak_mat_gt_arc \<equiv> mat_geI weak_gt"
...
lemma weak_mat_gt_both_mono: assumes orient: "\<And> m1 m2. mat n n m1 \<and> mat n n m2 \<and> (m1,m2) \<in> set m12s \<Longrightarrow> weak_mat_gt_arc m1 m2"
   shows "\<exists> gt. SN_both_mono_ordered_semiring_1 default gt arc_pos \<and> (\<forall> m1 m2. mat n n m1 \<and> mat n n m2 \<and> (m1, m2) \<in> set m12s \<longrightarrow> mat_geI gt m1 m2)"
proof -
  let ?m1x = "concat (concat (map fst m12s))"
  let ?m2y = "concat (concat (map snd m12s))"
  let ?pairs = "concat (map (\<lambda> x. map (\<lambda> y. (x,y)) ?m2y) ?m1x)"
  ...
  proof (intro exI allI conjI impI, rule order)
    ...
    assume ass: "mat n n m1 \<and> mat n n m2 \<and> (m1,m2) \<in> set m12s"
    hence wf1: "mat n n m1" and wf2: "mat n n m2" and m1m2: "(m1,m2) \<in> set m12s" by auto
    show "mat_geI gt  m1 m2" unfolding mat_ge_index[OF wf1 wf2]
    proof (intro allI impI)
      fix i j
      assume i: "i < n" and j: "j < n"
      with orient[OF ass, unfolded mat_ge_index[OF wf1 wf2]] have weak_gt: "weak_gt (m1 ! i ! j) (m2 ! i ! j)" (is "weak_gt ?x ?y") by auto  
