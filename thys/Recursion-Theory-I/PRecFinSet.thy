...
proof -
  from mod_is_pr power_is_pr div_is_pr have "(\<lambda> x u. (u div (2 ^ x)) mod 2) \<in> PrimRec2" by (tactic "pr_ex_tac facts")
...
proof -
  ...
  from g_is_pr have h_is_pr: "h \<in> PrimRec2" by (unfold h_def, tactic "pr_ex_tac facts")
...
proof -
  ...
  from p_is_pr have h_is_pr: "h \<in> PrimRec3" by (unfold h_def, tactic "pr_ex_tac facts")
...
proof -
  ...
  next
    from g_is_pr show "(\<lambda>x y. g x) \<in> PrimRec2" by (tactic "pr_ex_tac facts")
  next
  ...
  from f1_is_pr have "(\<lambda> u. f1 u u) \<in> PrimRec1" by (tactic "pr_ex_tac facts")
...
next
  from power_is_pr show "(\<lambda>x y. y + 2 ^ x) \<in> PrimRec2" by (tactic "pr_ex_tac facts")
qed
...
proof -
  ...
  proof -
    ...
    from c_in_is_pr c_insert_is_pr have h_is_pr: "h \<in> PrimRec3" by (unfold h_def, tactic "pr_tac ()")
    have f_at_0: "\<forall> x. f 0 x = g x"
    proof
  ...
  from f_is_pr have union_is_pr: "union \<in> PrimRec2" by (unfold union_def, tactic "pr_ex_tac facts")
...
proof -
  ...
  proof -
    ...
    from c_in_is_pr c_remove_is_pr have h_is_pr: "h \<in> PrimRec3" by (unfold h_def, tactic "pr_tac ()")
    have f_at_0: "\<forall> x. f 0 x = g x"
    proof
      fix x show "f 0 x = g x" by (unfold f_def, unfold g_def, simp)
    qed
  ...
  from f_is_pr have diff_is_pr: "diff \<in> PrimRec2" by (unfold diff_def, tactic "pr_ex_tac facts")
...
proof -
  ...
  from c_diff_is_pr c_union_is_pr have f_is_pr: "f \<in> PrimRec2" by (unfold f_def, tactic "pr_ex_tac facts")
