...
proof -
  ...
  from pA_is_pr A2 have q_is_pr: "q \<in> PrimRec2" by (unfold q_def, tactic "pr_tac ()")
...
proof -
  ...
  from pA_is_pr have q_is_pr: "q \<in> PrimRec1" by (unfold q_def, tactic "pr_ex_tac facts")
...
proof -
  ...
  from pA_is_pr A2 have q_is_pr: "q \<in> PrimRec2" by (unfold q_def, tactic "pr_ex_tac facts")
...
proof -
  ...
  have S1: "?f \<in> PrimRec2" by (insert const_is_pr_2 [where ?n="a"], tactic "pr_tac()")
...
proof -
  ...
  from S2 S5 have S7: "?p \<in> PrimRec2" by (tactic "pr_ex_tac facts")
...
proof -
  ...
  from p_is_pr have q_is_pr: "q \<in> PrimRec2" by (unfold q_def, tactic "pr_ex_tac facts")
  have S1: "A = {x. p(x) = 0}"
  proof -
...
proof -
  ...
  proof
    ...
    qed
  qed
  moreover from c_in_is_pr have "(\<lambda> x. sgn2 (c_in x u)) \<in> PrimRec1" by (tactic "pr_ex_tac facts")
  ultimately show ?thesis by auto
qed
...
proof -
  ...
  from q_is_pr have p_is_pr: "p \<in> PrimRec3" by (unfold p_def, tactic "pr_ex_tac facts")
...
proof -
  ...
  from p_is_pr have q_is_pr: "q \<in> PrimRec2" by (unfold q_def, tactic "pr_tac()")
...
proof -
  ...
  from p_r_is_pr p_s_is_pr have p_is_pr: "p \<in> PrimRec3" by (unfold p_def, tactic "pr_tac()")
...
proof -
  ...
  from p_is_pr have q_is_pr: "q \<in> PrimRec3" by (unfold q_def, tactic "pr_tac()")
...
proof -
  ...
  proof -
    ...
    have p_is_pr: "p \<in> PrimRec1" by (unfold p_def, tactic "pr_tac()")
...
proof -
  ...
  from A have p_is_pr: "p \<in> PrimRec1" by (unfold p_def, tactic "pr_tac()")
...
proof -
  ...
  from c_assoc_have_key_is_pr c_assoc_value_is_pr pr_gr_is_pr have p_is_pr: "p \<in> PrimRec2" by (unfold p_def, tactic "pr_ex_tac facts")
...
proof -
  ...
  from p_def have p_is_pr: "p \<in> PrimRec1" by (unfold p_def, tactic "pr_tac()")
  from q_def have q_is_pr: "q \<in> PrimRec1" by (unfold p_def, tactic "pr_tac()")
...
proof -
  ...
  from pA_is_pr have p_is_pr: "p \<in> PrimRec2" by (unfold p_def, tactic "pr_tac ()")
...
lemma s_ce_is_pr: "s_ce \<in> PrimRec2" by (unfold s_ce_def, insert comp_by_index_is_pr s1_1_is_pr, tactic "pr_tac()")
...
proof -
  ...
  have f_is_pr: "f \<in> PrimRec1" by (unfold f_def, tactic "pr_tac()")
...
proof -
  ...
  from s_ce_is_pr have f_is_pr: "f \<in> PrimRec1" by (unfold f_def, tactic "pr_tac()")
  then have f_tr: "total_recursive f" by (rule pr_is_total_rec)
  have f_inj: "inj f"
  proof (rule datatype_injI)
    fix x show "\<forall>y. f x = f y \<longrightarrow> x = y"
