...
lemma  shift_interpret:
  assumes  "ws \<in> lists \<C>" and "ws' \<in> lists \<C>"  and "z \<notin> \<langle>\<C>\<rangle>" and
    conjug: "z \<cdot> concat ws = concat ws' \<cdot> z" and "\<^bold>|z\<^bold>| < \<^bold>|concat ws'\<^bold>|"
    and "us \<le>p ws" and "us \<noteq> \<epsilon>"
  obtains p s vs ps ss where  "p (concat us) s \<sim>\<^sub>\<I> vs" and "ps \<cdot> vs \<le>p ws' \<cdot> ws'" 
    and "us \<cdot> ss = ws \<cdot> ws" and
    "concat ps \<cdot> p = z" and "s \<le>p concat ss" and "p \<le>s concat ws" and "vs \<in> lists \<C>"
    "\<And> us' vs'. us' \<le>p us \<Longrightarrow> vs' \<le>p vs \<Longrightarrow> p \<cdot> concat us' \<noteq> concat vs'" and "p \<noteq> \<epsilon>" and "s \<noteq> \<epsilon>"
proof-
  ...
  have "z \<cdot> concat ws \<le>p concat (ws' \<cdot> ws')"
    unfolding concat_morph conjug pref_cancel_conv using eq_le_pref[OF conjug less_imp_le[OF \<open>\<^bold>|z\<^bold>| < \<^bold>|concat ws'\<^bold>|\<close>]].
  ...
  from obtain_fac_interpret[OF fac_u \<open>concat us \<noteq> \<epsilon>\<close>]
  ...
  note fac_interpretE[OF \<open>p (concat us) s \<sim>\<^sub>\<I> vs\<close>]

  from prefixE[OF pref_ext[OF \<open>us \<le>p ws\<close>]]
  obtain ss where [symmetric]:"ws \<cdot> ws = us \<cdot> ss".
  ...
  from \<open>concat (ws' \<cdot> ws') \<le>p z \<cdot> concat (ws \<cdot> ws)\<close>[folded arg_cong[OF \<open>ps \<cdot> vs \<cdot> ss' = ws' \<cdot> ws'\<close>, of concat] \<open>us \<cdot> ss = ws \<cdot> ws\<close>,
      unfolded concat_morph, folded \<open>p \<cdot> concat us \<cdot> s = concat vs\<close> \<open>concat ps \<cdot> p = z\<close>,
      unfolded rassoc pref_cancel_conv]
  have "s \<le>p concat ss"
    using append_prefixD by blast

  have "\<^bold>|p\<^bold>| \<le> \<^bold>|concat ws\<^bold>|"
    using  \<open>\<^bold>|z\<^bold>| < \<^bold>|concat ws'\<^bold>|\<close>[folded lenarg[OF \<open>concat ps \<cdot> p = z\<close>], unfolded \<open>\<^bold>|concat ws'\<^bold>| = \<^bold>|concat ws\<^bold>|\<close>]
    by simp
  ...
  from that[OF \<open>p (concat us) s \<sim>\<^sub>\<I> vs\<close> \<open>ps \<cdot> vs \<le>p ws' \<cdot> ws'\<close> \<open>us \<cdot> ss = ws \<cdot> ws\<close> \<open>concat ps \<cdot> p = z\<close> \<open>s \<le>p concat ss\<close> \<open>p \<le>s concat ws\<close> \<open>vs \<in> lists \<C>\<close>
      disjoint \<open>p \<noteq> \<epsilon>\<close> \<open> s \<noteq> \<epsilon>\<close>]
  show thesis.
qed
...
lemma imprim_witness_shift:
  ...
  obtains z n  where "concat ws = z\<^sup>@Suc(Suc n)" "z \<notin> \<langle>\<C>\<rangle>" and "z \<cdot> concat ws = concat ws \<cdot> z" and "\<^bold>|z\<^bold>| < \<^bold>|concat ws\<^bold>|"
proof-
  ...
  obtain z n where [symmetric]: "z\<^sup>@Suc(Suc n) = concat ws"
    using not_prim_primroot_expE[OF \<open>\<not> primitive (concat ws)\<close> \<open>concat ws \<noteq> \<epsilon>\<close>, of thesis] by force
  ...
  proof
    ...
    from is_code[OF \<open>ws \<in> lists \<C>\<close> pow_in_lists[OF \<open>zs \<in> lists \<C>\<close>],
        unfolded concat_pow \<open>concat ws = z\<^sup>@Suc(Suc n)\<close> \<open>concat zs = z\<close>]
    show False
      using \<open>primitive ws\<close> pow_not_prim by blast 
  qed   

  have "\<^bold>|z\<^bold>| < \<^bold>|concat ws\<^bold>|"
    unfolding lenarg[OF \<open>concat ws = z\<^sup>@Suc(Suc n)\<close>, unfolded lenmorph pow_len]
    using nemp_len[OF \<open>z \<noteq> \<epsilon>\<close>] by simp

  from that[OF \<open>concat ws = z\<^sup>@Suc(Suc n)\<close> \<open>z \<notin> \<langle>\<C>\<rangle>\<close> _  this]
  show thesis
    unfolding \<open>concat ws = z\<^sup>@Suc(Suc n)\<close>  pow_comm by blast
...
proof-
  ...
  show "x = y"
    using same_len_nemp_root_eq[OF \<open>x \<le>p t \<cdot> x\<close> \<open>y \<le>p t \<cdot> y\<close> \<open>t \<noteq> \<epsilon>\<close> \<open>\<^bold>|x\<^bold>| = \<^bold>|y\<^bold>|\<close>].
qed
...
proof-
  ...
  then obtain t where y: "y = p \<cdot> t"
    using eqd[OF eq] by force   
  from eqd_eq[OF _  \<open>\<^bold>|x\<^bold>| = \<^bold>|y\<^bold>|\<close>[unfolded y swap_len[of p]], unfolded rassoc,
      OF eq[unfolded this rassoc cancel]] 
...
proof-
  note fac_interpretE[OF \<open>p  (x \<cdot> y)  s \<sim>\<^sub>\<I> vs\<close>]
...
proof (standard, rule ccontr)
  ...
  obtain z n where "concat ws' = z \<^sup>@ Suc (Suc n)" "z \<notin> \<langle>{x, y}\<rangle>" "z \<cdot> concat ws' = concat ws' \<cdot> z" "\<^bold>|z\<^bold>| < \<^bold>|concat ws'\<^bold>|".
  from shift_interpret[OF \<open>ws' \<in> lists {x,y}\<close> \<open>ws' \<in> lists {x,y}\<close> this(2-4) \<open>[x,y] \<le>p ws'\<close> \<open>[x,y] \<noteq> \<epsilon>\<close>]
  obtain vs p s where "vs \<in> lists {x,y}"  "p (concat [x, y]) s \<sim>\<^sub>\<I> vs" "p \<noteq> \<epsilon>" by metis   
  from uniform_square_interp(1)[OF \<open>x \<cdot> y \<noteq> y \<cdot> x\<close> \<open>\<^bold>|x\<^bold>| = \<^bold>|y\<^bold>|\<close> \<open>vs \<in> lists {x,y}\<close> _ \<open>p \<noteq> \<epsilon>\<close>]
       \<open>primitive (x \<cdot> y)\<close> \<open>p (concat [x, y]) s \<sim>\<^sub>\<I> vs\<close>
  show False by force
qed (simp add: prim_concat_prim)

\<comment> \<open>A stronger version is implied by the following lemma.\<close>

lemma bin_uniform_imprim: assumes "x \<cdot> y \<noteq> y \<cdot> x" and "\<^bold>|x\<^bold>| = \<^bold>|y\<^bold>|" and "\<not> primitive x"
  shows "primitive (x \<cdot> y)"
proof (rule ccontr)
  obtain s n where "s\<^sup>@n = x" and "2 \<le> n"
    using not_prim_pow[OF \<open>\<not> primitive x\<close>].
  assume "\<not> primitive (x \<cdot> y)"
  have "x \<cdot> y \<noteq> \<epsilon>"
    using \<open>x \<cdot> y \<noteq> y \<cdot> x\<close> by blast
  from not_prim_expE[OF \<open>\<not> primitive (x \<cdot> y)\<close> this]
  obtain z k where "primitive z" and  "2 \<le> k" and "z\<^sup>@k = x \<cdot> y".
  from split_pow[OF \<open>x \<cdot> y \<noteq> y \<cdot> x\<close> \<open>z\<^sup>@k = x \<cdot> y\<close>[symmetric]]
  obtain l m u v where "z \<^sup>@ l \<cdot> u = x" "v \<cdot> z \<^sup>@ m = y" "u \<cdot> v = z" "u \<cdot> v \<noteq> v \<cdot> u" "k = Suc (l + m)".
  have "u \<noteq> \<epsilon>" and "v \<noteq> \<epsilon>"
    using \<open>u \<cdot> v \<noteq> v \<cdot> u\<close> by blast+
  have "m = l" and "\<^bold>|u\<^bold>| = \<^bold>|v\<^bold>|" 
    using almost_equal_equal[OF nemp_len[OF \<open>u \<noteq> \<epsilon>\<close>] nemp_len[OF \<open>v \<noteq> \<epsilon>\<close>], of l m] lenarg[OF \<open>z\<^sup>@l \<cdot> u = x\<close>[symmetric], unfolded \<open>\<^bold>|x\<^bold>| =\<^bold>|y\<^bold>|\<close>, folded lenarg[OF \<open>v \<cdot> z\<^sup>@m = y\<close>]] unfolding lenmorph pow_len
           lenarg[OF \<open>u \<cdot> v = z\<close>, symmetric] by presburger+ 
  have "x \<le>p z \<cdot> x"
    using pref_prod_root[OF pref_ext[of x, OF self_pref, of y, folded \<open>z\<^sup>@k = x \<cdot> y\<close>]]. 
  have "x \<le>p s \<cdot> x"
    unfolding \<open>s\<^sup>@n = x\<close>[symmetric] using pref_prod_root by blast 
  have "z \<cdot> s \<noteq> s \<cdot> z"
    using \<open>x \<cdot> y \<noteq> y \<cdot> x\<close> comm_add_exps[of s z n k, unfolded \<open>s \<^sup>@ n = x\<close> \<open>z \<^sup>@ k = x \<cdot> y\<close> rassoc cancel]
    by force
  with two_pers[OF \<open>x \<le>p z \<cdot> x\<close> \<open>x \<le>p s \<cdot> x\<close>] 
  have "\<^bold>|x\<^bold>| < \<^bold>|z\<^bold>| + \<^bold>|s\<^bold>|"  by linarith
  have "l = 1"
  proof (rule ccontr)
    assume "l \<noteq> 1" hence "2 \<le> l"
      using \<open>k = Suc (l + m)\<close> \<open>2 \<le> k\<close> unfolding \<open>m = l\<close> by force
    from  trans_le_add1[OF mult_le_mono1[OF this]]
    have "2*\<^bold>|z\<^bold>| \<le> \<^bold>|x\<^bold>|"
      unfolding lenarg[OF \<open>z\<^sup>@l \<cdot> u = x\<close>, unfolded pow_len lenmorph, symmetric].
    from  mult_le_mono1[OF \<open>2 \<le> n\<close>]
    have "2*\<^bold>|s\<^bold>| \<le> \<^bold>|x\<^bold>|"
      unfolding lenarg[OF \<open>s\<^sup>@n = x\<close>, unfolded pow_len lenmorph, symmetric].
    hence "2*\<^bold>|z\<^bold>| + 2*\<^bold>|s\<^bold>| \<le> 2*\<^bold>|x\<^bold>|"
      using \<open>2*\<^bold>|z\<^bold>| \<le> \<^bold>|x\<^bold>|\<close> by force
    thus False
      using mult_less_mono1[OF \<open>\<^bold>|x\<^bold>| < \<^bold>|z\<^bold>| + \<^bold>|s\<^bold>|\<close>, of 2] by force
  qed
  hence "x = u \<cdot> v \<cdot> u"
    using \<open>u \<cdot> v = z\<close> \<open>z \<^sup>@ l \<cdot> u = x\<close> by fastforce
  have "u \<noteq> v"
     using \<open>u \<cdot> v \<noteq> v \<cdot> u\<close> by blast
  hence "primitive [u,v,u]"
    by primitivity_inspection
  hence "primitive [u,v,u] \<noteq> primitive (concat [u, v, u])"
    using \<open>x = u \<cdot> v \<cdot> u\<close> \<open>\<not> primitive x\<close> by auto 
  with bin_uniform_prim_morph[OF \<open>u \<cdot> v \<noteq> v \<cdot> u\<close> \<open>\<^bold>|u\<^bold>| = \<^bold>|v\<^bold>|\<close> \<open>primitive z\<close>[folded \<open>u \<cdot> v = z\<close>], of "[u,v,u]"]
  show False by simp
qed

theorem bin_uniform_prim_morph':
  assumes "x \<cdot> y \<noteq> y \<cdot> x" and "\<^bold>|x\<^bold>| = \<^bold>|y\<^bold>|" and "primitive (x \<cdot> y) \<or> \<not> primitive x"
         and "ws \<in> lists {x,y}" and "2 \<le> \<^bold>|ws\<^bold>|"
       shows "primitive ws \<longleftrightarrow>  primitive (concat ws)"
  using bin_uniform_prim_morph[OF assms(1-2) _ assms(4-5)] bin_uniform_imprim[OF assms(1-2)] assms(3) by fast
...
proof-
  ...
  have "\<not> primitive (x\<^sup>@(count_list ws x) \<cdot> y)"
    using \<open>\<not> primitive (concat ws)\<close> by (auto simp add: concat_pow)

  from not_prim_pow[OF this]
  ...
  interpret LS_len_le x y "count_list ws x" 1 l z
    ...
    using \<open>2 \<le> count_list ws x\<close> by auto
...
proof
  ...
  (* have "[x,y] \<noteq> \<epsilon>" and "\<^bold>|[x,y]\<^bold>| \<le> \<^bold>|ws\<^bold>|" and "concat[x,y] = x \<cdot> y" *)
    (* using \<open>2 \<le> \<^bold>|ws\<^bold>|\<close> by auto *)

  (* interpret binary_code x y *)
    (* using \<open>x \<cdot> y \<noteq> y \<cdot> x\<close> by unfold_locales *)
  (* from switch_fac[OF bin_code_neq bin_prim_long_set[OF \<open>ws \<in> lists {x,y}\<close> \<open>primitive ws\<close> \<open>2 \<le> \<^bold>|ws\<^bold>|\<close>]] *)
  (* have "[x,y] \<le>f ws \<cdot> ws". *)
  (* from rotate_into_pref_sq[OF this] *)
  (* obtain ws' where "ws \<sim> ws'" and "[x,y] \<le>p ws'" *) 
    (* using \<open>2 \<le> \<^bold>|ws\<^bold>|\<close> by auto *)  

  (* have "ws' \<in> lists {x,y}" and "primitive ws'" and "\<not> primitive (concat ws')" *)
    (* using \<open>ws \<in> lists {x,y}\<close>[unfolded conjug_in_lists_iff [OF \<open>ws \<sim> ws'\<close>]] *) 
  ...
  (* from imprim_witness_shift[OF \<open>ws' \<in> lists {x,y}\<close> \<open>primitive ws'\<close> \<open>\<not> primitive (concat ws')\<close>] *)
  (* obtain z where "z \<notin> \<langle>{x, y}\<rangle>" and "z \<cdot> concat ws' = concat ws' \<cdot> z" and "\<^bold>|z\<^bold>| < \<^bold>|concat ws'\<^bold>|". *)

  (* from shift_interpret[OF \<open>ws' \<in> lists {x,y}\<close> \<open>ws' \<in> lists {x,y}\<close> this \<open>[x,y] \<le>p ws'\<close> \<open>[x,y] \<noteq> \<epsilon>\<close>] *)
  (* obtain p s vs where "p (x \<cdot> y) s \<sim>\<^sub>\<I> vs" and \<open>vs \<in> lists {x, y}\<close> and *)
    (* disjoint: "(\<And>us' vs'. us' \<le>p [x, y] \<Longrightarrow> vs' \<le>p vs \<Longrightarrow> p \<cdot> concat us' \<noteq> concat vs')" *)
    (* unfolding \<open>concat [x,y] = x \<cdot> y\<close> by metis *) 
  (* from disjoint[of \<epsilon> \<epsilon>] have "p \<noteq> \<epsilon>" by simp *)

  (* from uniform_square_interp(1)[OF \<open>x\<cdot>y \<noteq> y\<cdot>x\<close> conjug_len[OF \<open>x \<sim> y\<close>] \<open>vs \<in> lists {x, y}\<close> *)
      (* \<open>p (x \<cdot> y) s \<sim>\<^sub>\<I> vs\<close> \<open>p \<noteq> \<epsilon>\<close>] *)
  (* have "\<not> primitive (x \<cdot> y)". *)
...
subsection \<open>Square prefix of the longer word and both words primitive (was all\_assms\<close>
...
proof-
  have "x \<noteq> y"
    using assms(1) by blast
  hence "2 \<le> \<^bold>|ws\<^bold>|"
    by (metis Cons_in_lists_iff Suc_1 Suc_leI assms(2) assms(3) assms(5) assms(6) assms(7) nemp_le_len order_le_less prim_nemp sing_word_concat two_elem_cases)
  have "\<^bold>|[x, x]\<^bold>| \<le> \<^bold>|ws\<^bold>|"
    using \<open>2 \<le> \<^bold>|ws\<^bold>|\<close> by force
  obtain ws' where "ws \<sim> ws'" "[x,x] \<le>p ws'"
    using rotate_into_pos_sq[of \<epsilon> "[x,x]", unfolded clean_emp, OF \<open>[x, x] \<le>f ws \<cdot> ws\<close>
        _ \<open>\<^bold>|[x, x]\<^bold>| \<le> \<^bold>|ws\<^bold>|\<close>]
    by auto

  have "ws' \<in> lists {x,y}"
    by (meson \<open>ws \<sim> ws'\<close> assms(5) conjug_in_lists')
  have "primitive ws'"
    by (meson \<open>ws \<sim> ws'\<close> assms(6) prim_conjug)
  have "\<not> primitive (concat ws')"
    using \<open>ws \<sim> ws'\<close> assms(7) conjug_concat_prim_iff by blast

  have "ws' = [x,x,y]"
  proof(rule ccontr)
    assume "ws' \<noteq> [x,x,y]"

    have "2 \<le> \<^bold>|ws'\<^bold>|" and "[x,x] \<noteq> \<epsilon>"
      using \<open>[x,x] \<le>p ws'\<close> unfolding prefix_def by auto
    have "x \<noteq> y" 
      using  prim_exp_eq[OF \<open>primitive ws'\<close> sing_lists_exp_len] \<open>ws' \<in> lists {x,y}\<close> \<open>[x,x] \<le>p ws'\<close> by force    
    hence "x \<cdot> y \<noteq> y \<cdot> x"
      using comm_prim[OF \<open>primitive x\<close> \<open>primitive y\<close>] by blast 
    from bin_imprim_not_conjug[OF \<open>ws' \<in> lists {x,y}\<close> this \<open>2 \<le> \<^bold>|ws'\<^bold>|\<close> \<open>primitive ws'\<close> \<open>\<not> primitive (concat ws')\<close>]
    have "\<not> x \<sim> y". 
    have "[x,x] \<le>np ws'"
      using npI[OF _ \<open>[x,x] \<le>p ws'\<close>] by blast
    have "concat ws' \<noteq> \<epsilon>"
      using \<open>primitive x\<close> \<open>[x,x] \<le>p ws'\<close> by (fastforce simp add: prefix_def)
    have "ws' \<noteq> \<epsilon>"
      using \<open>concat ws' \<noteq> \<epsilon>\<close> by auto
    have "ws' \<cdot> ws' \<cdot> ws' \<in> lists {x, y}" and "ws' \<cdot> ws' \<in> lists {x, y}" 
      using \<open>ws' \<in> lists {x,y}\<close> by inlists
    have "concat [x,x] = x \<cdot> x" by simp
    ...
    obtain z n where con_ws: "concat ws' = z \<^sup>@ Suc (Suc n)" and "z \<notin> \<langle>{x, y}\<rangle>" and "z \<cdot> concat ws' = concat ws' \<cdot> z" and "\<^bold>|z\<^bold>| < \<^bold>|concat ws'\<^bold>|".

    obtain vs p s ps ss where "p (x \<cdot> x) s \<sim>\<^sub>\<I> vs" and "ps \<cdot> vs \<le>p ws' \<cdot> ws'" and 
      "[x, x] \<cdot> ss = ws' \<cdot> ws'" and \<open>concat ps \<cdot> p = z\<close> and "s \<le>p concat ss" and "p \<le>s concat ws'" and \<open>vs \<in> lists {x, y}\<close> and
      disjoint: "(\<And>us' vs'. us' \<le>p [x, x] \<Longrightarrow> vs' \<le>p vs \<Longrightarrow> p \<cdot> concat us' \<noteq> concat vs')" and "p \<noteq> \<epsilon>" and "s \<noteq> \<epsilon>"   
      using xy.shift_interpret[OF \<open>ws' \<in> lists {x,y}\<close> \<open>ws' \<in> lists {x,y}\<close> \<open>z \<notin> \<langle>{x, y}\<rangle>\<close> \<open>z \<cdot> concat ws' = concat ws' \<cdot> z\<close>
             \<open>\<^bold>|z\<^bold>| < \<^bold>|concat ws'\<^bold>|\<close> \<open>[x,x] \<le>p ws'\<close> \<open>[x,x] \<noteq> \<epsilon>\<close>, of thesis]
      unfolding \<open>concat [x,x] = x \<cdot> x\<close> by fast  
    from disjoint[of \<epsilon> \<epsilon>] have "p \<noteq> \<epsilon>" by simp

    have triv: "2 \<le> Suc(Suc n)" "Suc 0 < Suc (Suc n)" by simp_all
    ...
    have "ss \<in> lists {x,y}"
      using \<open>ws' \<in> lists {x,y}\<close> \<open>[x,x] \<cdot> ss = ws' \<cdot> ws'\<close> append_in_lists_conv by metis 

    interpret square_interp_ext x y p s vs
      by (rule square_interp_ext.intro[OF square_interp.intro, unfolded square_interp_ext_axioms_def,
            OF \<open>primitive x\<close> \<open>primitive y\<close> \<open>\<^bold>|y\<^bold>| \<le> \<^bold>|x\<^bold>|\<close> \<open>vs \<in> lists {x,y}\<close> \<open>\<not> x \<sim> y\<close> disjoint], simp_all only: \<open>p x \<cdot> x s \<sim>\<^sub>\<I> vs\<close>)  
        (meson \<open>p \<le>s concat ws'\<close> \<open>s \<le>p concat ss\<close> \<open>ss \<in> lists {x, y}\<close> \<open>ws' \<in> lists {x, y}\<close> concat_in_hull')
...
\<comment> \<open>Establishing the connection between ws' = [x,x,y] and z = xp.\<close>
    ...
    proof
      ...
      from power_commuting_commutes[symmetric, OF this[symmetric], of 2,
          THEN power_commuting_commutes, of "Suc (Suc n)", unfolded pow_two]
      have "z\<^sup>@Suc(Suc n) \<cdot> xp \<cdot> xp = xp \<cdot> xp \<cdot> z\<^sup>@Suc(Suc n)"
        unfolding  rassoc.
    ...
    proof (rule notI)
      ...
      have "z \<^sup>@ Suc (Suc n) \<cdot> xp\<^sup>@Suc(Suc 0) = xp\<^sup>@Suc(Suc 0) \<cdot> z \<^sup>@ Suc (Suc n)"
        unfolding pow_Suc pow_zero clean_emp rassoc.
      from comm_drop_exps[OF this] 
      show False
        using \<open>z \<cdot> xp \<noteq> xp \<cdot> z\<close> by blast 
    qed
...
\<comment> \<open>Looking at the first occurrence of z:\<close>
    ...
    have lcp_xp_z: "concat (ws' \<cdot> [x,x,y]) \<and>\<^sub>p concat ([x,x,y] \<cdot> ws') = bin_lcp z (x \<cdot> p)"
      ...
      using bin_lcp_pows[of  "Suc n" "(x\<cdot>p)\<^sup>@Suc 0" "Suc 0" "z\<^sup>@(Suc n)"]  
      unfolding xp_def pow_Suc pow_zero clean_emp rassoc.
...
\<comment> \<open>Looking at the second occurrence of z:\<close>
    ...
    proof 
      ...
      show False 
        using \<open>z \<cdot> xp \<noteq> xp \<cdot> z\<close> comm_drop_exp' by blast
    qed
    ...
    have 
      ...
      unfolding lassoc cancel 
      unfolding rassoc lcp_ext_left cancel  
      using bin_lcp_pows[of "(Suc n)" \<epsilon> 0 "z \<cdot> z\<^sup>@n", unfolded pow_one rassoc clean_emp]
      unfolding lassoc pow_Suc[symmetric] pow_Suc2[symmetric]
      using xp_def by auto
    ...
    proof (rule xy.bin_code_lcp_concat')
      ...
      show "\<not> concat (ws' \<cdot> ws'') \<bowtie> concat (ws'' \<cdot> ws')"
        unfolding concat_morph con_ws \<open>concat ws'' = z \<cdot> xp\<close> pow_Suc rassoc comp_cancel 
        unfolding lassoc pow_Suc[symmetric] pow_Suc2[symmetric] comm_comp_eq_conv
          comm_drop_exps_conv[of _ _ _ 0, unfolded pow_one]
        using non_comm. 
    qed
...
proof (rule bin_imprim_both_squares_prim) 
  ...
  show "\<rho> x \<cdot> \<rho> y \<noteq> \<rho> y \<cdot> \<rho> x"
    using  \<open>x \<cdot> y \<noteq> y \<cdot> x\<close>[unfolded comp_primroot_conv'[OF \<open>x \<noteq> \<epsilon>\<close> \<open>y \<noteq> \<epsilon>\<close>]]. 
...
\<comment> \<open>The primitivity of ws' is obtained from the fact that the decompositions into
  ...
  have "concat (map ?R [x,x]) = [\<rho> x] \<^sup>@ (Suc (e\<^sub>\<rho> x -1) + Suc (e\<^sub>\<rho> x - 1))"
    unfolding  Suc_minus[OF primroot_exp_nemp[OF \<open>x \<noteq> \<epsilon>\<close>]] by (simp add: add_exps)
  ...
  have "concat (map ?R [y,y]) = [\<rho> y] \<^sup>@ (Suc (e\<^sub>\<rho> y -1) + Suc (e\<^sub>\<rho> y - 1))"
    unfolding  Suc_minus[OF primroot_exp_nemp[OF \<open>y \<noteq> \<epsilon>\<close>]] by (simp add: add_exps)
...
\<comment> \<open>rotating last if necessary for successful gluing\<close>
      ...
      hence rot: "last ws = x' \<Longrightarrow> hd ws = x' \<Longrightarrow> butlast ws \<cdot> [x',x'] \<cdot> tl ws = ws \<cdot> ws"
        using append_butlast_last_id hd_tl hd_word mult_assoc by metis 
      from this[THEN facI']
...
lemma assumes "x = [(0::nat),1,0,1,0]" and "y = [1,0,0,1]" 
  ...
  unfolding factor_interpretation_def assms by (simp add: suf_def)
...
proof-
  ...
  have "ws \<sim> [x]\<^sup>@(j2+j1)\<cdot>[y]\<^sup>@1"
    ...
    unfolding \<open>[x] \<^sup>@ j1 \<cdot> [y] \<cdot> [x] \<^sup>@ j2 = ws\<close>[symmetric] power_add rassoc pow_one'.
