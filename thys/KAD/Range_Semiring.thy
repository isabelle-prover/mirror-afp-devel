...
sublocale range_semiring \<subseteq> rdual: domain_semiring "op +" "\<lambda>x y. y \<cdot> x" 1 0 range_op "op \<le>" "op <"
  rewrites "rdual.fd x y = \<langle>x| y"
proof -
  show "class.domain_semiring op + (\<lambda>x y. y \<cdot> x) 1 0 range_op op \<le> op <"
    by (standard, auto simp: mult_assoc distrib_left)
  then interpret rdual: domain_semiring "op +" "\<lambda>x y. y \<cdot> x" 1 0 range_op "op \<le>" "op <" .
  show "rdual.fd x y = \<langle>x| y"
    unfolding rdual.fd_def bd_def by auto
qed

sublocale domain_semiring \<subseteq> ddual: range_semiring "op +" "\<lambda>x y. y \<cdot> x" 1 0 domain_op "op \<le>" "op <"
  rewrites "ddual.bd x y = domain_semiringl_class.fd x y"
proof -
  show "class.range_semiring op + (\<lambda>x y. y \<cdot> x) 1 0 domain_op op \<le> op <"
    by (standard, auto simp: mult_assoc distrib_left)
  then interpret ddual: range_semiring "op +" "\<lambda>x y. y \<cdot> x" 1 0 domain_op "op \<le>" "op <" .
...
sublocale antirange_semiring \<subseteq> ardual: antidomain_semiring antirange_op "op +" "\<lambda>x y. y \<cdot> x" 1 0 "op \<le>" "op <"
  rewrites "ardual.ads_d x = r x"
  and "ardual.fdia x y = \<langle>x| y"
  and "ardual.fbox x y = [x| y"
proof -
  show "class.antidomain_semiring antirange_op op + (\<lambda>x y. y \<cdot> x) 1 0 op \<le> op <"
    by (standard, auto simp: mult_assoc distrib_left)
  then interpret ardual: antidomain_semiring antirange_op "op +" "\<lambda>x y. y \<cdot> x" 1 0 "op \<le>" "op <" .
...
sublocale rs: range_semiring "op +" "op \<cdot>" 1 0 "\<lambda>x. ar ar x" "op \<le>" "op <"
  by unfold_locales

end

sublocale antidomain_semiring \<subseteq> addual: antirange_semiring "op +" "\<lambda>x y. y \<cdot> x" 1 0 antidomain_op "op \<le>" "op <"
  rewrites "addual.ars_r x = d x"
  and "addual.bdia x y = |x\<rangle> y"
  and "addual.bbox x y = |x] y"
proof -
  show "class.antirange_semiring op + (\<lambda>x y. y \<cdot> x) 1 0 antidomain_op op \<le> op <"
    by (standard, auto simp: mult_assoc distrib_left)
  then interpret addual: antirange_semiring "op +" "\<lambda>x y. y \<cdot> x" 1 0 antidomain_op "op \<le>" "op <" .
...
sublocale antirange_kleene_algebra \<subseteq> dual: antidomain_kleene_algebra antirange_op "op +" "\<lambda>x y. y \<cdot> x" 1 0 "op \<le>" "op <" "star"
  by (standard, auto simp: local.star_inductr' local.star_inductl)


sublocale antidomain_kleene_algebra \<subseteq> dual: antirange_kleene_algebra "op +" "\<lambda>x y. y \<cdot> x" 1 0 "op \<le>" "op <" "star" antidomain_op
