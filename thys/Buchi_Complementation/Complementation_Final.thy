...
*)
  ...
  schematic_goal complement_impl:
    ...
    assumes [autoref_rules]: "(Ai, A) \<in> \<langle>Id, nat_rel, unit_rel\<rangle> nbai_nba_rel"
  ...
  theorem
    assumes "finite (nodes A)"
    assumes "(Ai, A) \<in> \<langle>Id, nat_rel, unit_rel\<rangle> nbai_nba_rel"
    shows "language (nbae_nba (nbae (complement_impl Ai))) = streams (alphabet A) - language A"
  proof -
    have "(language ((nbae_nba \<circ> nbae) (complement_impl Ai)), language (id (complement_3 A))) \<in>
