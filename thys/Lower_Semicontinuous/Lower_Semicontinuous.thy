...
lemma rel_interior_extreal_semiline:
  fixes a :: extreal
  shows "rel_interior {y. a \<le> extreal y} = {y. a < extreal y}"
proof (cases a)
...
lemma closed_extreal_semiline:
  fixes a :: extreal
  shows "closed {y. a \<le> extreal y}"
proof (cases a)
  case (real r) then show ?thesis
    using closed_real_atLeast unfolding atLeast_def by simp
qed auto

lemma extreal_semiline_unique:
  fixes a b :: extreal
  shows "{y. a \<le> extreal y} = {y. b \<le> extreal y} \<longleftrightarrow> a = b"
by (metis Collect_def extreal_le_real order_antisym)
...
{ assume "~?rhs"
  ...
  from compact_extreal[of "f o x"] obtain l r where r_def: "subseq r & ((f o x) o r) ----> l" by auto
  ...
  } hence l_prop: "l ~: S & l<=f x0" using r_def x_def X_def Lim_bounded_extreal by auto
...
{ assume "?rhs"
 { fix x A assume x_def: "x ----> x0" "(f o x) ----> A"
   { assume "A ~= f x0"
     ...
     hence "f x0 <= A" using Lim_bounded2_extreal[of "f o x" A "max N1 N2" "f x0"] x_def by auto
...
(is "?lhs <-> ?rhs")
proof-
obtain m where m_def: "f x0 = extreal m" using assms by (cases "f x0") auto
{ assume lsc: "lsc_at x0 f"
  { fix e assume e_def: "(e :: extreal)>0"
    hence *: "f x0 : {f x0 - e <..< f x0 + e}" using assms extreal_between by auto
...
{ assume "?rhs"
  { fix S assume S_def: "open S & f x0 : S"
    from this obtain e where e_def: "e>0 & {f x0 - e<..<f x0 + e} <= S"
      apply (subst extreal_open_cont_interval[of S "f x0"]) using assms by auto
    ...
    { fix y assume "y:T" "f y <= f x0" hence "f y < f x0 + e"
         using assms e_def extreal_between[of "f x0" e] by auto
...
{ assume fin: "f x0 ~= -\<infinity>" "f x0 ~= PInfty"
  { assume lsc: "lsc_at x0 f"
    { fix C assume "C<f x0"
      hence "f x0-C>0" using fin extreal_less_minus_iff by auto
...
proof-
{ assume lsc: "lsc_at x0 f"
  { fix C :: extreal assume "C<f x0"
    from this obtain T where "open T & x0 : T & (!y : T. C < f y)"
      using lsc lsc_at_extreal[of x0 f] by auto
...
{ assume "?rhs"
  ...
  } hence "lsc_at x0 f" using assms lsc_at_extreal[of x0 f] by auto
}
ultimately show ?thesis by auto
qed
...
by (metis inf_extreal_def le_iff_inf lsc_liminf_at)
...
next
  ...
  { fix e assume e_def: "(e :: extreal)>0"
    ...
    hence "f x0 <= liminf (f o x) + e" apply (cases e) unfolding extreal_minus_le_iff by auto
  }
  then show ?thesis by (intro extreal_le_epsilon) auto
qed auto
...
{ assume "?rhs"
  { fix x l assume "x ----> x0" "(f o x) ----> l"
    ...
    moreover
    { assume "l = -\<infinity>"
      { fix B :: real obtain N where N_def: "ALL n>=N. f(x n) <= extreal B"
        ...
        moreover have "ALL n. f(g n) <= extreal B" using g_def N_def by auto
        ultimately have "f x0 <= extreal B" using `?rhs` by auto
...
{ assume lsc: "lsc_at x0 f"
  { fix x c assume xc_def: "x ----> x0 & (ALL n. f(x n)<=c)"
    hence "liminf (f o x) <= c" using limsup_bounded[of 0 "f o x" c] extreal_Liminf_le_Limsup[of sequentially "f o x"] by auto
...
{ assume lsc: "lsc_at x0 (%x. -f x)"
  { fix x A assume x_def: "x ----> x0" "(f o x) ----> A"
    hence "(%i. - f (x i)) ----> -A" using extreal_lim_uminus[of "f o x" A] by auto
...
{ assume usc: "usc_at x0 f"
  { fix x A assume x_def: "x ----> x0" "((%x. - f x) o x) ----> A"
    ...
    hence "(%i. f (x i)) ----> - A" using extreal_lim_uminus[of "(%i. - f (x i))" A] by auto
    hence "(f o x) ----> -A" unfolding o_def by auto
    hence "f x0 >= - A" apply (subst usc_at_mem[of x0 "f" x]) using usc x_def by auto
    hence "-f x0 <= A" unfolding extreal_uminus_le_reorder by auto
...
{ fix x assume "x ----> x0"
  have "(-f x0 <= -limsup (f o x)) <-> (-f x0 <= liminf ((%x. - f x) o x))"
     using extreal_Liminf_uminus[of _ "f o x"] unfolding o_def by auto
...
proof-
  ...
  also have "... <-> -(f x0) <= -(Limsup (at x0) f)" by (metis extreal_Liminf_uminus)
...
{ assume a: "(lsc_at x0 f) & (usc_at x0 f)"
  { fix x assume "x ----> x0"
    ...
    ultimately have "limsup (f o x) = f x0 & liminf (f o x) = f x0"
       using extreal_Liminf_le_Limsup[of sequentially "f o x"] by auto
    hence "(f o x) ----> f x0" using extreal_Liminf_eq_Limsup[of sequentially] by auto
...
definition "Epigraph S (f::_ => extreal) = {xy. fst xy : S & f (fst xy) <= extreal(snd xy)}"


lemma mem_Epigraph: "(x, y) : Epigraph S f <-> x : S & f x <= extreal y" unfolding Epigraph_def by auto


lemma extreal_closed_levels:
fixes f :: "'a::metric_space => extreal"
shows "(ALL y. closed {x. f(x)<=y}) <-> (ALL r. closed {x. f(x)<=extreal r})"
(is "?lhs <-> ?rhs")
proof-
{ assume "?rhs"
  { fix y :: extreal
    ...
    moreover
    { assume "y=-\<infinity>"
      hence "{x. f(x)<=y} = Inter {{x. f(x)<=extreal r} |r. r : UNIV}" using extreal_bot by auto
...
{ assume "lsc f"
  { fix z z0 assume a: "ALL n. z n : (Epigraph UNIV f) & z ----> z0"
    { fix n have "z n : (Epigraph UNIV f)" using a by auto
      hence "f (fst (z n)) <= extreal(snd (z n))" using a unfolding Epigraph_def by auto
      hence "EX xn cn. z n = (xn, cn) & f(xn)<=extreal cn"
         apply (rule_tac x="fst (z n)" in exI) apply (rule_tac x="snd (z n)" in exI) by auto
    } from this obtain x c where xc_def: "ALL n. z n = (x n, c n) & f(x n)<=extreal (c n)" by metis
    ...
    hence "f(x0)<=extreal c0" apply (subst lsc_sequentially_mem[of x0 f x "extreal o c" "extreal c0"])
...
{ assume "closed (Epigraph UNIV f)"
     hence *: "ALL x l. (ALL n. f (fst (x n)) <= extreal(snd (x n))) & x ----> l -->
     f (fst l) <= extreal(snd l)" unfolding Epigraph_def closed_sequential_limits by auto
  { fix r :: real
    { fix z z0 assume a: "ALL n. f(z n)<=extreal r & z ----> z0"
      hence "f(z0)<=extreal r" using *[rule_format, of "(%n. (z n,r))" "(z0,r)"]
         tendsto_Pair[of z z0] by auto
    } hence "closed {x. f(x)<=extreal r}" by (simp add: closed_sequential_limits)
  } hence "ALL y. closed {x. f(x)<=y}" using extreal_closed_levels by auto
}
moreover
  ...
  { fix x0
    { fix x l assume "x ----> x0" "(f o x) ----> l"
      ...
      { assume mi: "l = -\<infinity>"
        { fix B :: real
          obtain N where N_def: "ALL n>=N. f(x n)<=extreal B"
          ...
          { fix d assume "(d::real)>0"
            ...
            hence "EX y. dist y x0 < d & y : {x. f(x)<=extreal B}"
              apply (rule_tac x="x (max N N1)" in exI) using N_def by auto
          }
          hence "x0 : closure {x. f(x)<=extreal B}" apply (subst closure_approachable) by auto
          hence "f x0 <= extreal B" using a by auto
...
definition lsc_hull :: "('a::metric_space => extreal) => ('a::metric_space => extreal)" where
...
using extreal_less_eq(3)[of y z] order_trans_rules(23) by blast
...
{ fix x
  ...
  hence "EX a. {z. (x, z) : S} = {z. a <= extreal z}" apply (subst mono_closed_extreal) using mono by auto
} from this obtain g where g_def: "ALL x. {z. (x, z) : S} = {z. g x <= extreal z}" by metis
{ fix s
  have "s:S <-> (fst s, snd s) : S" by auto
  also have "... <-> g(fst s) <= extreal (snd s)" using g_def[rule_format, of "fst s"] by blast
  finally have "s:S <-> g(fst s) <= extreal (snd s)" by auto
}
...
proof-
{ fix x
  { fix C have "f x <= extreal C <-> (x,C) : Epigraph UNIV f" unfolding Epigraph_def by auto
    also have "... <-> (x,C) : Epigraph UNIV g" using assms by auto
    also have "... <-> g x <= extreal C" unfolding Epigraph_def by auto
    finally have "f x <= extreal C <-> g x <= extreal C" by auto
  } hence "g x = f x" using extreal_le_real[of "g x" "f x"] extreal_le_real[of "f x" "g x"] by auto
...
{ assume l: "f x <= Liminf (at x) f"
  ...
  } hence "f x <= (SUP e:{0<..}. INF y:ball x e. f y)" using extreal_le_extreal[of "f x"] by auto
...
{ fix x z assume "(x,z):Epigraph UNIV (%x. min (f x) (Liminf (at x) f))"
  hence xz_def: "extreal z >= (SUP e:{0<..}. INF y:ball x e. f y)"
...
{ fix x z assume xz_def: "(x,z):closure (Epigraph UNIV f)"
  { fix e::real assume "e>0"
    ...
    also have "...<=extreal(snd y)" using y_def unfolding Epigraph_def by auto
    also have "... < extreal(z+e)" using h2 unfolding dist_norm by auto
    finally have "(INF y:ball x e. f y) < extreal(z+e)" by auto
  } hence *: "ALL e>0. (INF y:ball x e. f y) < extreal(z+e)" by auto

  { fix e assume "(e::real)>0"
    { fix d assume "(d::real)>0"
      { assume "d<e"
        have "(INF y:ball x d. f y) < extreal(z+d)" using * `d>0` by auto
        also have "...< extreal(z+e)" using `d<e` by auto
        finally have "(INF y:ball x d. f y) < extreal(z+e)" by auto
      }
      moreover
      { assume "~(d<e)"
        hence "ball x e <= ball x d" by auto
        hence "(INF y:ball x d. f y) <= (INF y:ball x e. f y)" apply (subst INF_mono) by auto
        also have "...< extreal(z+e)" using * `e>0` by auto
        finally have "(INF y:ball x d. f y) < extreal(z+e)" by auto
      } ultimately have "(INF y:ball x d. f y) < extreal(z+e)" by blast
      hence "(INF y:ball x d. f y) <= extreal(z+e)" by auto
    } hence "min (f x) (Liminf (at x) f) <= extreal (z+e)" unfolding min_Liminf_at by (auto intro: SUP_leI)
  } hence "min (f x) (Liminf (at x) f) <= extreal z" using extreal_le_epsilon2 by auto
...
definition
  convex_on :: "'a::real_vector set => ('a => extreal) => bool" where
  "convex_on s f <->
  (ALL x:s. ALL y:s. ALL u>=0. ALL v>=0. u + v = 1
  --> f (u *\<^sub>R x + v *\<^sub>R y) <= extreal u * f x + extreal v * f y)"


lemma convex_on_extreal_mem:
  assumes "convex_on s f"
  assumes "x:s" "y:s"
  assumes "u>=0" "v>=0" "u+v=1"
  shows "f (u *\<^sub>R x + v *\<^sub>R y) <= extreal u * f x + extreal v * f y"
using assms unfolding convex_on_def by auto


lemma convex_on_extreal_subset: "convex_on t f ==> s <= t ==> convex_on s f"
  unfolding convex_on_def by auto


lemma convex_on_extreal_univ: "convex_on UNIV f <-> (ALL S. convex_on S f)"
  using convex_on_extreal_subset by auto

lemma extreal_pos_setsum_right_distrib:
  fixes f :: "'a => extreal"
...
proof (cases "finite A")
  ...
  next
    case (insert x A) thus ?case using assms by (simp add: extreal_pos_distrib)
  qed
next
...
lemma convex_extreal_add:
  fixes f g :: "'a::real_vector => extreal"
...
proof-
  { fix x y assume "x:s" "y:s" moreover
    ...
    ultimately have "f (u *\<^sub>R x + v *\<^sub>R y) + g (u *\<^sub>R x + v *\<^sub>R y)
      <= (extreal u * f x + extreal v * f y) + (extreal u * g x + extreal v * g y)"
      using assms unfolding convex_on_def by (auto simp add:extreal_add_mono)
    also have "...=(extreal u * f x + extreal u * g x) + (extreal v * f y + extreal v * g y)"
      by (simp add: algebra_simps)
    also have "...= extreal u * (f x + g x) + extreal v * (f y + g y)"
      using uv by (simp add: extreal_pos_distrib)
    finally have "f (u *\<^sub>R x + v *\<^sub>R y) + g (u *\<^sub>R x + v *\<^sub>R y)
      <= extreal u * (f x + g x) + extreal v * (f y + g y)" by auto }
  thus ?thesis unfolding convex_on_def by auto
qed

lemma convex_extreal_cmul:
  assumes "0 <= (c::extreal)" "convex_on s f"
  shows "convex_on s (%x. c * f x)"
proof-
{ fix x y assume "x:s" "y:s" moreover
  fix u v ::real assume uv: "0 <= u" "0 <= v" "u + v = 1"
  ultimately have "f (u *\<^sub>R x + v *\<^sub>R y) <= (extreal u * f x + extreal v * f y)"
     using assms unfolding convex_on_def by auto
  hence "c * f (u *\<^sub>R x + v *\<^sub>R y) <= c * (extreal u * f x + extreal v * f y)"
     using assms by (intro extreal_mult_left_mono) auto
  also have "... <=  c * (extreal u * f x) + c * (extreal v * f y)"
     using assms by (simp add: extreal_pos_le_distrib)
  also have "... = extreal u *(c* f x) + extreal v *(c* f y)" by (simp add: algebra_simps)
  finally have "c * f (u *\<^sub>R x + v *\<^sub>R y)
      <= extreal u * (c * f x) + extreal v * (c * f y)" by auto }
  thus ?thesis unfolding convex_on_def by auto
qed



lemma convex_extreal_max:
  fixes f g :: "'a::real_vector => extreal"
  assumes "convex_on s f" "convex_on s g"
...
proof-
  { fix x y assume "x:s" "y:s" moreover
    ...
    ultimately have "max (f (u *\<^sub>R x + v *\<^sub>R y)) (g (u *\<^sub>R x + v *\<^sub>R y))
      <= max (extreal u * f x + extreal v * f y) (extreal u * g x + extreal v * g y)"
      apply (subst extreal_max_mono) using assms unfolding convex_on_def by auto
    also have "...<=extreal u * max (f x) (g x) + extreal v * max (f y) (g y)"
      apply (subst extreal_max_least)
      apply (subst extreal_add_mono) prefer 4 apply (subst extreal_add_mono)
      by (subst extreal_mult_left_mono, auto simp add: uv)+
    finally have "max (f (u *\<^sub>R x + v *\<^sub>R y)) (g (u *\<^sub>R x + v *\<^sub>R y))
      <= extreal u * max (f x) (g x) + extreal v * max (f y) (g y)" by auto }
  thus ?thesis unfolding convex_on_def by auto
qed


lemma convex_on_extreal_alt:
  fixes C :: "'a::real_vector set"
  assumes "convex C"
  shows "convex_on C f =
  (ALL x : C. ALL y : C. ALL m :: real. m >= 0 & m <= 1
      --> f (m *\<^sub>R x + (1 - m) *\<^sub>R y) <= (extreal m) * f x + (1 - (extreal m)) * f y)"
proof safe
  fix x y fix m :: real
  have[simp]: "extreal (1 - m) = (1 - extreal m)"
    using extreal_minus(1)[of 1 m] by (auto simp: one_extreal_def)
  assume asms: "convex_on C f" "x : C" "y : C" "0 <= m" "m <= 1"
  from this[unfolded convex_on_def, rule_format]
  have "ALL u v. ((0 <= u & 0 <= v & u + v = 1) -->
       f (u *\<^sub>R x + v *\<^sub>R y) <= (extreal u) * f x + (extreal v) * f y)" by auto
  from this[rule_format, of "m" "1 - m", simplified] asms
  show "f (m *\<^sub>R x + (1 - m) *\<^sub>R y)
          <= (extreal m) * f x + (1 - extreal m) * f y" by auto
next
  assume asm: "ALL x:C. ALL y:C. ALL m. 0 <= m & m <= 1
      --> f (m *\<^sub>R x + (1 - m) *\<^sub>R y) <= (extreal m) * f x + (1 - extreal m) * f y"
  { fix x y fix u v :: real
    assume lasm: "x : C" "y : C" "u >= 0" "v >= 0" "u + v = 1"
    hence[simp]: "1-u=v" "1 - extreal u = extreal v"
    using extreal_minus(1)[of 1 m] by (auto simp: one_extreal_def)
    from asm[rule_format, of x y u]
    have "f (u *\<^sub>R x + v *\<^sub>R y) <= (extreal u) * f x + (extreal v) * f y"
      using lasm by auto }
  thus "convex_on C f" unfolding convex_on_def by auto
qed


lemma convex_on_extreal_alt_mem:
  fixes C :: "'a::real_vector set"
  assumes "convex C"
  assumes "convex_on C f"
  assumes "x : C" "y : C"
  assumes "(m::real) >= 0" "m <= 1"
  shows "f (m *\<^sub>R x + (1 - m) *\<^sub>R y) <= (extreal m) * f x + (1 - (extreal m)) * f y"
by (metis assms convex_on_extreal_alt)


lemma extreal_add_right_mono: "(a::extreal) <= b ==> a + c <= b + c"
by (metis extreal_add_mono order_refl)

lemma convex_on_extreal_setsum_aux:
  assumes "1-a>0"
  shows "(1 - extreal a) * (extreal (c / (1 - a)) * b) = (extreal c) * b"
  by (metis mult_assoc mult_commute eq_divide_eq extreal_minus(1) assms
            one_extreal_def real_less_def times_extreal.simps(1))


lemma convex_on_extreal_setsum:
  fixes a :: "'a => real"
  ...
  shows "f (SUM i : s. a i *\<^sub>R y i) <= (SUM i : s. extreal (a i) * f (y i))"
using assms(1,2,5-)
proof (induct s arbitrary:a rule:finite_ne_induct)
  case (singleton i)
  hence ai: "a i = 1" by auto
  thus ?case by (auto simp: one_extreal_def[symmetric])
next
  case (insert i s)
  from `convex_on C f`
  have conv: "ALL x y m. ((x : C & y : C & 0 <= m & m <= 1)
      --> f (m *\<^sub>R x + (1 - m) *\<^sub>R y) <= (extreal m) * f x + (1 - extreal m) * f y)"
      using convex_on_extreal_alt[of C f] `convex C` by auto
  { assume "a i = 1"
    hence "(SUM j : s. a j) = 0"
    ...
    hence ?case using insert.hyps(1-3) `a i = 1`
      by (simp add: zero_extreal_def[symmetric] one_extreal_def[symmetric]) }
  moreover
  { assume asm: "a i ~= 1"
    ...
    hence i1: "1 - extreal (a i) > 0" using extreal_minus(1)[of 1 "a i"]
      by (simp add: zero_extreal_def[symmetric] one_extreal_def[symmetric])
    have i2: "1 - extreal (a i) ~= PInfty" using extreal_minus(1)[of 1]
      by (simp add: zero_extreal_def[symmetric] one_extreal_def[symmetric])
    let "?a j" = "a j / (1 - a i)"
    { fix j assume "j : s"
      hence "?a j >= 0"
    ...
    have asum_le: "f (SUM j : s. ?a j *\<^sub>R y j) <= (SUM j : s. extreal (?a j) * f (y j))"
    ...
    also have "... <= (1 - extreal (a i)) * f ((SUM j : s. ?a j *\<^sub>R y j)) + (extreal (a i)) * f (y i)"
      using conv[rule_format, of "y i" "(SUM j : s. ?a j *\<^sub>R y j)" "a i"]
      using yai(1) asum yai(2) ai1 by (auto simp add:add_commute)
    also have "... <= (1 - extreal (a i)) * (SUM j : s. extreal (?a j) * f (y j)) + (extreal (a i)) * f (y i)"
      using extreal_add_right_mono[OF extreal_mult_left_mono[of _ _ "1 - extreal (a i)",
        OF asum_le less_imp_le[OF i1]], of "(extreal (a i)) * f (y i)"] by simp
    also have "... = (SUM j : s. (1 - extreal (a i)) * (extreal (?a j) * f (y j))) + (extreal (a i)) * f (y i)"
      unfolding extreal_pos_setsum_right_distrib[of "1 - extreal (a i)" "% j. (extreal (?a j)) * f (y j)", OF less_imp_le[OF i1] i2] by auto
    also have "... = (SUM j : s. (extreal (a j)) * f (y j)) + (extreal (a i)) * f (y i)" using i0 convex_on_extreal_setsum_aux by auto
    also have "... = (extreal (a i)) * f (y i) + (SUM j : s. (extreal (a j)) * f (y j))" by (simp add: add_commute)
    also have "... = (SUM j : insert i s. (extreal (a j)) * f (y j))" using insert by auto
    finally have "f (SUM j : insert i s. a j *\<^sub>R y j) <= (SUM j : insert i s. (extreal (a j)) * f (y j))" by simp }
...
lemma convex_on_extreal_iff:
  assumes "convex s"
  shows "convex_on s f <-> (ALL k u x. (ALL i:{1..k::nat}. 0 <= u i & x i : s) & setsum u {1..k} = 1 -->
   f (setsum (%i. u i *\<^sub>R x i) {1..k} ) <= setsum (%i. (extreal (u i)) * f(x i)) {1..k} )"
  (is "?rhs <-> ?lhs")
proof-
{ assume "?rhs"
  { fix k u x
    ...
    ultimately have "f (setsum (%i. u i *\<^sub>R x i) {1..k} )
      <= setsum (%i. (extreal (u i)) * f(x i)) {1..k}"
      using convex_on_extreal_setsum[of "{1..k}" s f u x] using assms `?rhs` by auto
  } hence "?lhs" by auto
}
moreover
{ assume "?lhs"
  { fix x y u v
    ...
    moreover have "(SUM i = 1..2. extreal (uv i) * f (xy i)) = extreal u * f x + extreal v * f y"
        using setsum_2[of "(%i. extreal (uv i) * f (xy i))"] unfolding xy_def uv_def using xys uv1 by auto
    ultimately have "f (u *\<^sub>R x + v *\<^sub>R y) <= extreal u * f x + extreal v * f y"
...
{ assume rhs: "convex(Epigraph S f)"
  { fix x y assume xy: "x:S" "y:S"
    ...
    have "f (u *\<^sub>R x + v *\<^sub>R y) <= extreal u * f x + extreal v * f y"
    proof-
      { assume "u=0 | v=0" hence ?thesis using uv by (auto simp: zero_extreal_def[symmetric] one_extreal_def[symmetric]) }
      moreover
      { assume "f x = PInfty | f y = PInfty" hence ?thesis using uv by (auto simp: zero_extreal_def[symmetric] one_extreal_def[symmetric]) }
      moreover
      { assume a: "f x = -\<infinity> & (f y ~= PInfty) & ~(u=0)"
        from this obtain z where "f y <= extreal z" apply (cases "f y") by auto
        ...
        { fix C::real
          ...
          hence "(f (u *\<^sub>R x + v *\<^sub>R y) <= extreal C)" unfolding Epigraph_def by auto
        } hence "f (u *\<^sub>R x + v *\<^sub>R y) = -\<infinity>" using extreal_bot by auto
        hence ?thesis by auto }
      moreover
      { assume a: "(f x ~= PInfty) & f y = -\<infinity> & ~(v=0)"
        from this obtain z where "f x <= extreal z" apply (cases "f x") by auto
      ...
      { assume a: "f x ~= PInfty & f x ~= -\<infinity> & f y ~= PInfty & f y ~= -\<infinity>"
        from this obtain fx fy where fxy: "f x = extreal fx & f y = extreal fy"
...
{ assume lhs: "convex_on S f"
  { fix x y fx fy assume xy: "(x,fx):Epigraph S f" "(y,fy):Epigraph S f"
    fix u v ::real assume uv: "0 <= u" "0 <= v" "u + v = 1"
    hence le: "f x <= extreal fx & f y <= extreal fy" using xy unfolding Epigraph_def by auto
    ...
    ultimately have "f(u *\<^sub>R x + v *\<^sub>R y) <= (extreal u) * f x + (extreal v) * f y"
      using lhs convex_on_extreal_mem[of S f x y u v] uv by auto
    also have "... <= (extreal u) * (extreal fx) + (extreal v) * (extreal fy)"
      apply (subst extreal_add_mono) apply (subst extreal_mult_left_mono)
      prefer 4 apply (subst extreal_mult_left_mono) using le uv by auto
    also have "... = extreal (u * fx + v * fy)" by auto
...
definition
  concave_on :: "'a::real_vector set => ('a => extreal) => bool" where
  "concave_on S f <-> convex_on S (%x. - f x)"

definition
  finite_on :: "'a::real_vector set => ('a => extreal) => bool" where
  "finite_on S f <-> (ALL x:S. (f x ~= PInfty & f x ~= -\<infinity>))"

definition
  affine_on :: "'a::real_vector set => ('a => extreal) => bool" where
  "affine_on S f <-> (convex_on S f & concave_on S f & finite_on S f)"

definition
  "domain (f::_ => extreal) = {x. f x < PInfty}"


lemma domain_Epigraph_aux:
  assumes "x ~= PInfty"
  shows "EX r. x<=extreal r"
...
} from this show ?thesis using convex_on_extreal_subset by auto
qed


lemma convex_on_domain2:
  "convex_on (domain f) f <-> (ALL S. convex_on S f)"
by (metis convex_on_domain convex_on_extreal_univ)


lemma convex_domain:
  fixes f :: "'a::euclidean_space => extreal"
...
proof-
  ...
  moreover have "(%x. if x:S then (f x) else PInfty) = (%x. max (f x) (g x))"
    ...
    apply (metis extreal_less_eq(2) min_max.sup_absorb1)
    by (metis extreal_less_eq(1) min_max.sup_absorb2)
  ultimately show ?thesis using convex_extreal_max assms by auto
qed



definition
  proper_on :: "'a::real_vector set => ('a => extreal) => bool" where
  "proper_on S f <-> ((ALL x:S. f x ~= -\<infinity>) & (EX x:S. f x ~= PInfty))"

definition
  proper :: "('a::real_vector => extreal) => bool" where
...
lemma extreal_MInf_plus[simp]: "-\<infinity> + x = (if x = \<infinity> then \<infinity> else -\<infinity>)"
  by (cases x) auto
...
{ assume nemp: "domain f ~= {}"
  ...
  { fix x assume "x:rel_interior(domain f)"
    ...
    hence "f x <= (1-extreal v) * f u+extreal v * f y"
      using convex_on_extreal_alt_mem[of "UNIV" f y u v] assms convex_UNIV v01
      by (simp add: convex_UNIV add_commute)
    moreover have "f y < \<infinity>" using m_def y_def unfolding domain_def by auto
    moreover have *: "0 < 1 - extreal v" using v01
      by (metis diff_less_iff(1) extreal_less(2) extreal_minus(1) one_extreal_def)
    moreover hence "(1-extreal v) * f u = -\<infinity>" using u_def by auto
...
{ assume "ALL x. f x = PInfty"
  ...
  ultimately have "ALL x. (lsc_hull f) x = PInfty" by (metis lsc_hull_lsc)
     ...
     by (metis extreal_infty_less_eq(1) lsc_hull_le)
moreover have "(EX x. f x = -\<infinity>) --> (EX x. (lsc_hull f) x = -\<infinity>)"
  by (metis extreal_infty_less_eq2(2) lsc_hull_le)
...
by (metis assms convex_improper extreal_infty_less_eq(2) lsc_hull_le)
...
by (metis assms convex_improper extreal_MInfty_lessI mem_def proper_iff rel_interior_univ2 top1I)
...
lemma rel_interior_Epigraph:
  ...
  shows "(x,z) : rel_interior (Epigraph UNIV f) <->
    (x : rel_interior (domain f) & f x < extreal z)"
apply (subst rel_interior_projection[of _ "(%y. {z. (y, z) : Epigraph UNIV f})"])
apply (metis assms convex_EpigraphI convex_UNIV convex_on_extreal_univ)
unfolding domain_Epigraph Epigraph_def using rel_interior_extreal_semiline by auto
...
lemma rel_interior_EpigraphI:
  ...
  shows "rel_interior (Epigraph UNIV f) =
    {(x,z) |x z. x : rel_interior (domain f) & f x < extreal z}"
proof-
{ fix x z
  have "(x,z):rel_interior (Epigraph UNIV f) <-> (x : rel_interior (domain f) & f x < extreal z)"
...
proof-
  def A == "{(x::'a::euclidean_space,m)|x m. extreal m<a}"
  obtain x where "f x < a" using assms by auto
  then obtain z where z_def: "f x < extreal z & extreal z < a" using extreal_dense2 by auto
  ...
  then obtain x0 z0 where "(x0,z0):A & x0 : rel_interior (domain f) & f x0 < extreal z0"
...
proof-
  ...
  thus ?thesis
    by (metis "*" g_def less_extreal.simps(2))
qed
...
lemma lsc_hull_of_convex_determined:
  fixes f g :: "'a::euclidean_space => extreal"
...
{ fix x assume "x:domain (lsc_hull f)"
  ...
  then obtain z where z_def: "min (f x) (Liminf (at x) f) < z & z < PInfty" by (metis extreal_dense)
  { fix e::real assume "e>0"
...
{ fix x assume x_def: "x : rel_interior (domain f)"
  ...
  then obtain r where r_def: "(x,r):rel_interior (Epigraph UNIV f)"
    using assms x_def rel_interior_Epigraph[of f x]  by (metis extreal_dense2)
  ...
  have "Epigraph UNIV f Int M = {x} <*> {m. f x <= extreal m}"
    unfolding Epigraph_def M_def by auto
  moreover have "closed({x} <*> {m. f x<= extreal m})" apply (subst closed_Times)
    using closed_extreal_semiline by auto
  ultimately have "{x} <*> {m. f x <= extreal m} = closure (Epigraph UNIV f) Int M"
    by (metis "*" Int_commute closure_closed)
  also have "...=Epigraph UNIV (lsc_hull f) Int M" by (metis Int_commute epigraph_lsc_hull)
  also have "...={x} <*> {m. ((lsc_hull f) x) <= extreal m}"
    unfolding Epigraph_def M_def by auto
  finally have "{m. f x <= extreal m} = {m. lsc_hull f x <= extreal m}" by auto
  hence "f x = (lsc_hull f) x" using extreal_semiline_unique by auto
} thus ?thesis by auto
qed
...
proof-
  obtain x where x_def: "x:rel_interior (domain f) & f x < PInfty"
     by (metis assms convex_less_ri_domain extreal_less_PInfty proper_iff)
...
lemma extreal_divide_pos:
  assumes "(a::extreal)>0" "b>0"
  shows "a/(extreal b)>0"
  by (metis assms extreal.simps(2) extreal_less(2) extreal_less_divide_pos extreal_mult_zero)

lemma lsc_hull_of_convex_aux:
  "Limsup (at 1 within {0..<1}) (%m. extreal ((1-m)*a+m*b)) <= extreal b"
proof-
{ fix y assume "extreal b<y"
  hence y_def:"0 <(y - extreal b)" using extreal_less_minus_iff by auto
  have "EX d>0. ALL z:{0..<1}. 0 < dist z 1 & dist z 1 < d --> extreal ((1 - z) * a + z * b) < y"
  proof(cases "y=PInfty")
    case True thus ?thesis apply(rule_tac x="1" in exI) by auto
    next
    case False
    then obtain r where r_def: "y=extreal r" using y_def by (cases y) auto
    { assume "a>b"
      hence "a-b>0" by (simp add: algebra_simps)
      hence "(extreal r-extreal b)/(extreal(a-b))>0" using y_def r_def by (metis extreal_divide_pos)
      then obtain d where d_def: "0<d & extreal d<(extreal r-extreal b)/(extreal(a-b))"
        using extreal_dense2[of 0] by auto
      { fix z::real assume "dist z 1 < d"
        hence "extreal(1 - z)<extreal d" unfolding dist_norm by auto
        hence "extreal(1 - z)<(extreal r-extreal b)/(extreal(a-b))" using d_def by (metis order_less_trans)
        hence "(a - b) * (1 - z) < r - b"
          using extreal_less_divide_pos[of "extreal(a-b)" "extreal (1 - z)" "(extreal r-extreal b)"] `a-b>0` by auto
        hence "(1 - z) * a + z * b < r" by (simp add: algebra_simps)
        hence "extreal ((1 - z) * a + z * b) < y" using r_def by auto
    ...
    { assume "b>=a"
      { fix z::real assume "z:{0..<1}"
        ...
        hence "extreal ((1 - z) * a + z * b) < y" using r_def `extreal b<y` by auto
...
{ assume "y~=x"
  ...
  { fix b assume "extreal b>=(lsc_hull f) y"
    ...
    then obtain a where"extreal a>f x" by (metis extreal_dense2)
      ...
      { fix m :: real assume "0 <= m & m < 1"
        hence "(y, b) - (1-m) *\<^sub>R ((y, b) - (x, a)) : rel_interior (Epigraph UNIV f)"
          ...
          apply (metis assms(1) convex_Epigraph convex_UNIV convex_on_extreal_univ)
          using yb xa by auto
        hence "f (y - (1-m) *\<^sub>R (y-x) ) < extreal (b-(1-m)*(b-a))"
          using assms(1) rel_interior_Epigraph by auto
        hence "?g m <= extreal ((1-m)*a+m*b)" by (simp add: algebra_simps)
      }
      hence "eventually (%m. ?g m <= extreal ((1-m)*a+m*b))
         (at 1 within {0..<1})" apply (subst always_eventually_within) by auto
      hence "Limsup (at 1 within {0..<1}) ?g <= Limsup (at 1 within {0..<1}) (%m. extreal ((1-m)*a+m*b))"
         apply (subst Limsup_mono) by auto
      also have "... <= extreal b" using lsc_hull_of_convex_aux by auto
      finally have "Limsup (at 1 within {0..<1}) ?g <=extreal b" by auto
    }
    hence "Limsup (at 1 within {0..<1}) ?g <= (lsc_hull f) y"
      using extreal_le_real[of "(lsc_hull f) y"] by auto
    ...
    moreover hence "Liminf (at 1 within {0..<1}) ?g <= Limsup (at 1 within {0..<1}) ?g"
      apply (subst extreal_Liminf_le_Limsup) by auto
    ultimately have "Limsup (at 1 within {0..<1}) ?g = (lsc_hull f) y
                   & Liminf (at 1 within {0..<1}) ?g = (lsc_hull f) y"
      using * by auto
  hence ?thesis apply (subst extreal_Liminf_eq_Limsup) using nontr by auto
} ultimately show ?thesis by auto
qed
