...
lemma iso_finfun_mem_mem [code_unfold]: "x \<in> A\<^sub>f \<longleftrightarrow> x \<in>\<^isub>f A"
by(auto simp add: mem_def finfun_mem_def)

declare iso_finfun_mem_mem [simp]

lemma iso_finfun_subset_subset [code_unfold]:
...
lemma iso_finfun_Un_Un [code_unfold]:
  "A\<^sub>f \<union> B\<^sub>f = (A \<union>\<^isub>f B)\<^sub>f"
by(auto)

lemma iso_finfun_Int_Int [code_unfold]:
...
lemma iso_finfun_insert_insert [code_unfold]:
  "insert a A\<^sub>f = (insert\<^isub>f a A)\<^sub>f"
by(auto)

lemma iso_finfun_Compl_Compl [code_unfold]:
  fixes A :: "'a set\<^isub>f"
  shows "- A\<^sub>f = (- A)\<^sub>f"
by(auto)

lemma iso_finfun_diff_diff [code_unfold]:
  fixes A :: "'a set\<^isub>f"
