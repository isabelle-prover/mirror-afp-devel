...
lemma scalar_matrix_mult_code [code abstract]: "vec_nth (scalar_matrix_mult c A) 
  = scalar_matrix_mult_row c A"
  unfolding scalar_matrix_mult_def scalar_matrix_mult_row_def[abs_def]
...
lemma minorM_code [code abstract]: "vec_nth (minorM A i j) = minorM_row A i j"
  unfolding minorM_def minorM_row_def[abs_def]
  using vec_lambda_beta by auto

lemmas cofactor_def[code_unfold]

definition "cofactorM_row A i = vec_lambda (%j. cofactor A i j)"

lemma cofactorM_row_code [code abstract]:
  "vec_nth (cofactorM_row A i) = (%j. cofactor A i j)"
  by(simp add: cofactorM_row_def cofactor_def fun_eq_iff)

lemma cofactorM_code [code abstract]: "vec_nth (cofactorM A) = cofactorM_row A"
  unfolding cofactorM_def cofactorM_row_def[abs_def]
  using vec_lambda_beta by auto


definition mat2matofpoly_row
  where "mat2matofpoly_row A i = vec_lambda (%j. [: A $ i $ j :])"
...
lemma evalmat_code[code]:
  "evalmat P A = listsum (map (\<lambda>i. (coeff P (nat i)) *ss (matpow A (nat i)))  [0..(degree P)])"
proof -
  ...
  have "evalmat P A = (\<Sum>i\<in>{n::nat. n \<le> degree P}. coeff P i *ss matpow A i)"
    unfolding evalmat_def ..
  also have "... = (\<Sum>i\<in>set[0..(degree P)]. coeff P (nat i) *ss matpow A (nat i))"
    unfolding transfer_nat_int_sum_prod unfolding set_rw ..
  also have "... = listsum (map (\<lambda>i. (coeff P (nat i)) *ss (matpow A (nat i)))  [0..(degree P)])"  
...
lemma coeffM_zero_row_code [code abstract]:
  ...
  by(simp add: coeffM_zero_row_def coeffM_zero_def fun_eq_iff)
