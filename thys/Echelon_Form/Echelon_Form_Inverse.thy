...
lemma mult_adjugate_det: "A ** adjugate A = mat (matrix_det A)"
...
lemma invertible_imp_matrix_inv:
  ...
  shows "matrix_inv A = (1 div (matrix_det A)) *k adjugate A"
proof - 
  let ?A = "adjugate A"
  have "A ** ?A = matrix_det A *k mat 1" 
    unfolding mult_adjugate_det by (simp add: scalar_mult_mat)
  hence "matrix_inv A ** (A ** ?A) = matrix_inv A ** (matrix_det A *k mat 1)"
    by auto
  hence "?A = matrix_det A *k matrix_inv A"
...
lemma inverse_matrix_code_rings[code_unfold]: 
  ...
  shows "inverse_matrix A = (let d=matrix_det A in if is_unit d then Some ((1 div d) *k adjugate A) else None)"
