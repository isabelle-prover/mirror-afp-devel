...
lemma fst_snd_echelon_form_of_column_k_le_nrows: 
  ...
  shows "fst (snd (echelon_form_of_column_k (A, i, bezout) k)) \<le> nrows A"
  using assms 
...
lemma fst_snd_snd_echelon_form_of_column_k_det_le_nrows:
  assumes "i\<le>nrows A"
  shows "fst (snd (snd (echelon_form_of_column_k_det (n, A, i, bezout) k))) \<le> nrows A"
...
lemma matrix_to_iarray_echelon_form_of_column_k_det:
  ...
  shows "matrix_to_iarray (fst (snd (echelon_form_of_column_k_det (n, A, i, bezout) k))) 
...
lemma fst_snd_snd_echelon_form_of_column_k_det:
  assumes "k < ncols A"
  and "i \<le> nrows A"
  shows "fst (snd (snd (echelon_form_of_column_k_det (n,A,i,bezout) k))) 
...
lemma 
  ...
  "fst (echelon_form_of_upt_k_det (1::'a,A) k bezout) 
  = fst (echelon_form_of_upt_k_det_iarrays (1::'a,matrix_to_iarray A) k bezout)"
  and matrix_to_iarray_snd_echelon_form_of_upt_k_det:
  "matrix_to_iarray ((snd (echelon_form_of_upt_k_det (1::'a,A) k bezout))) 
  = (snd (echelon_form_of_upt_k_det_iarrays (1::'a, matrix_to_iarray A) k bezout))"
  and "fst (snd (snd (foldl echelon_form_of_column_k_det (1::'a,A,0,bezout) [0..<Suc k]))) \<le> nrows A"
  and "fst (snd (snd (foldl echelon_form_of_column_k_det_iarrays 
  (1::'a,matrix_to_iarray A,0,bezout) [0..<Suc k]))) = fst (snd (snd 
  (foldl echelon_form_of_column_k_det (1::'a,A,0,bezout) [0..<Suc k])))"
  using assms
proof (induct k)
  show "fst (echelon_form_of_upt_k_det (1, A) 0 bezout) 
  ...
  show "matrix_to_iarray (snd (echelon_form_of_upt_k_det (1, A) 0 bezout)) =
  ...
  show "fst (snd (snd (foldl echelon_form_of_column_k_det (1, A, 0, bezout) [0..<Suc 0]))) \<le> nrows A"
  ...
  show "fst (snd (snd (foldl echelon_form_of_column_k_det_iarrays (1, matrix_to_iarray A, 0, bezout) [0..<Suc 0]))) =
    fst (snd (snd (foldl echelon_form_of_column_k_det (1, A, 0, bezout) [0..<Suc 0])))"
    unfolding echelon_form_of_column_k_det_iarrays_def echelon_form_of_column_k_det_def
    by (auto, metis fst_snd_matrix_to_iarray_echelon_form_of_column_k le0 ncols_not_0 neq0_conv 
      snd_echelon_form_of_column_k_det snd_echelon_form_of_column_k_det_eq)
next
  fix k
  assume "(k < ncols A \<Longrightarrow> fst (echelon_form_of_upt_k_det (1::'a, A) k bezout) 
    = fst (echelon_form_of_upt_k_det_iarrays (1::'a, matrix_to_iarray A) k bezout))"
    and "(k < ncols A \<Longrightarrow>
    matrix_to_iarray (snd (echelon_form_of_upt_k_det (1::'a, A) k bezout)) 
    = snd (echelon_form_of_upt_k_det_iarrays (1::'a, matrix_to_iarray A) k bezout))"
    and "(k < ncols A \<Longrightarrow> 
    fst (snd (snd (foldl echelon_form_of_column_k_det (1::'a, A, 0, bezout) [0..<Suc k]))) \<le> nrows A)"
    and "(k < ncols A \<Longrightarrow>
    fst (snd (snd (foldl echelon_form_of_column_k_det_iarrays (1::'a, matrix_to_iarray A, 0, bezout) [0..<Suc k]))) =
    fst (snd (snd (foldl echelon_form_of_column_k_det (1::'a, A, 0, bezout) [0..<Suc k]))))" 
    and S: "Suc k < ncols A"
  hence hyp1: "fst (echelon_form_of_upt_k_det (1::'a, A) k bezout) 
    = fst (echelon_form_of_upt_k_det_iarrays (1::'a, matrix_to_iarray A) k bezout)"
    and hyp2: "matrix_to_iarray (snd (echelon_form_of_upt_k_det (1::'a, A) k bezout)) 
    = snd (echelon_form_of_upt_k_det_iarrays (1::'a, matrix_to_iarray A) k bezout)" 
    and hyp3: "fst (snd (snd (foldl echelon_form_of_column_k_det (1::'a, A, 0, bezout) [0..<Suc k]))) 
    \<le> nrows A"
  ...
  have f_rw: "?f= (fst ?f, fst (snd ?f), fst (snd (snd ?f)), snd (snd (snd ?f)))" by simp
  ...
  have rw3: "fst (snd (snd ?g)) = fst (snd (snd ?f))" 
    using hyp4 .
  ...
  show "fst (echelon_form_of_upt_k_det (1, A) (Suc k) bezout) 
    ...
    apply (subst f_rw)
    apply (subst g_rw)
    unfolding rw1[symmetric] rw2 rw3 rw4
  proof (rule fst_matrix_to_iarray_echelon_form_of_column_k_det)
    show "Suc k < ncols (fst (snd ?f))" using S unfolding ncols_def .
    show " fst (snd (snd ?f)) \<le> nrows (fst (snd ?f))" by (metis hyp3 nrows_def)
  qed
  show "matrix_to_iarray (snd (echelon_form_of_upt_k_det (1, A) (Suc k) bezout)) =
  ...
  proof (rule matrix_to_iarray_echelon_form_of_column_k_det)
    ...
    show " fst (snd (snd ?f)) \<le> nrows (fst (snd ?f))" by (metis hyp3 nrows_def)
  qed
  show "fst (snd (snd (foldl echelon_form_of_column_k_det (1, A, 0, bezout) [0..<Suc (Suc k)]))) \<le> nrows A"
  ...
  show "fst (snd (snd (foldl echelon_form_of_column_k_det_iarrays 
    ...
    = fst (snd (snd (foldl echelon_form_of_column_k_det (1, A, 0, bezout) [0..<Suc (Suc k)])))"
    unfolding echelon_form_of_upt_k_det_iarrays_def echelon_form_of_upt_k_det_def Let_def fst_conv snd_conv
    unfolding list_rw foldl_append
    unfolding List.foldl.simps
    apply (subst f_rw)
...
proof -
  ...
  have "listprod (map ?f [0..<nrows_iarray (matrix_to_iarray A)]) 
