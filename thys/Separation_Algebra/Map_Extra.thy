...
section \<open>More properties of maps plus map disjuction.\<close>
...
text \<open>
  ...
  directly into Map.thy, thus uses \<open>m\<close> for map variable names.
  Anything involving heap disjunction is not really mergeable with Map, is
  destined for use in separation logic, and hence uses \<open>h\<close>
\<close>
