...
where "poly_vars_list p = remdups (concat_map (map fst o fst) p)"

(* check whether a variable occurs in p *)
definition poly_var :: "('v,'a)poly \<Rightarrow> 'v \<Rightarrow> bool"
where "poly_var p v = v mem (concat_map (map fst o fst) p)"
...
proof (rule poly_vars_list)
  ...
  thus "f w = g w" using pv diff unfolding poly_vars_list_def poly_var_def by (cases "v = w", auto simp: mem_iff)
qed
