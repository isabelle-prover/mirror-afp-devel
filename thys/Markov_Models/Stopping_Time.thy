...
proof (intro allI, subst measurable_cong)
  ...
  then show "(\<Sqinter>{i. P i \<omega>} \<le> t) = (\<exists>i\<in>{..t}. P i \<omega>)"
...
locale countable_cover_filtration = filtration \<Omega> F
  for \<Omega> and F :: "'t::{linorder_topology,second_countable_topology} \<Rightarrow> 'a measure" +
  fixes D :: "'t set"
  assumes countable_D[intro]: "countable D" and semidense_D: "\<And>a b. a < b \<Longrightarrow> \<exists>d\<in>D. a \<le> d \<and> d < b"
begin
