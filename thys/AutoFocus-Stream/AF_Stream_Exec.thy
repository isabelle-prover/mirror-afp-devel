...
definition
  f_Exec_Comp_N :: "('comp, 'input) Comp_Trans_Fun \<Rightarrow> nat \<Rightarrow> 'input list \<Rightarrow> 'comp \<Rightarrow> 'comp"
where
  "f_Exec_Comp_N trans_fun n xs c \<equiv> f_Exec_Comp trans_fun (xs \<down> n) c"
...
definition
  i_Exec_Comp_Stream :: "('comp, 'input) Comp_Trans_Fun \<Rightarrow> 'input ilist \<Rightarrow> 'comp \<Rightarrow> 'comp ilist"
where
  "i_Exec_Comp_Stream \<equiv> \<lambda>trans_fun input c n. 
    f_Exec_Comp trans_fun (input \<Down> Suc n) c"
definition
  i_Exec_Comp_Stream_Init :: "('comp, 'input) Comp_Trans_Fun \<Rightarrow> 'input ilist \<Rightarrow> 'comp \<Rightarrow> 'comp ilist"
where
  "i_Exec_Comp_Stream_Init \<equiv> \<lambda>trans_fun input c n. 
...
thm f_Exec_N_append[of trans_fun n "Suc 0" xs c]
...
thm i_Exec_Stream_append[of trans_fun "[input 0]" "input \<Up> Suc 0" c]
...
thm f_Exec_Stream_nth_eq_i_Exec_Stream_nth
...
thm f_Exec_Stream_Init_eq_f_Exec_Stream_Cons
...
definition
  list_Previous :: "'value list \<Rightarrow> 'value \<Rightarrow> nat \<Rightarrow> 'value"
where
  "list_Previous xs init n \<equiv> 
    case n of 
      0 \<Rightarrow> init | 
      Suc n' \<Rightarrow> xs ! n'"
definition
  ilist_Previous :: "'value ilist \<Rightarrow> 'value \<Rightarrow> nat \<Rightarrow> 'value"
where
  "ilist_Previous f init n \<equiv> 
    case n of 
      0 \<Rightarrow> init | 
...
thm list_Previous_def ilist_Previous_def
...
thm f_Exec_Stream_Pre_Post1
thm f_Exec_Stream_Pre_Post1[OF _ refl refl]
...
lemma f_Exec_Stream_Init_Pre_Post1: "
  ...
  (P1 x_n \<and> P2 c_n \<longrightarrow> Q (trans_fun x_n c_n))"
thm f_Exec_Stream_Init_nth_Suc_calc
by (simp add: f_Exec_Stream_Init_nth_Suc_calc)
...
thm i_Exec_Stream_Pre_Post1[OF refl refl]
text {* Direct relation between input and result after transition *}
...
thm f_Exec_Stream_prefix_causal
...
by (simp add: prefix_def iprefix_eq_iprefix_take i_Exec_Stream_Init_take)
...
definition
  f_Streams_Connected :: "'a fstream_af \<Rightarrow> 'a fstream_af \<Rightarrow> bool"
where
  "f_Streams_Connected outS inS \<equiv> inS = \<NoMsg> # outS"
definition
  i_Streams_Connected :: "'a istream_af \<Rightarrow> 'a istream_af \<Rightarrow> bool"
where
  "i_Streams_Connected outS inS \<equiv> inS = [\<NoMsg>] \<frown> outS"
lemmas Streams_Connected_defs = 
  f_Streams_Connected_def
  i_Streams_Connected_def
thm nth_Cons

lemma f_Streams_Connected_imp_not_empty: "
  f_Streams_Connected outS inS \<Longrightarrow> inS \<noteq> []"
...
thm f_Exec_Stream_output_causal
thm f_Exec_Stream_output_causal[OF _ refl refl]
...
definition
  f_Streams_Connected_Init :: "'a fstream_af \<Rightarrow> 'a fstream_af \<Rightarrow> bool"
where
  "f_Streams_Connected_Init outS inS \<equiv> inS = outS"
definition
  i_Streams_Connected_Init :: "'a istream_af \<Rightarrow> 'a istream_af \<Rightarrow> bool"
where
  "i_Streams_Connected_Init outS inS \<equiv> inS = outS"
...
typ "('comp1, 'input) Comp_Trans_Fun"
typ "('input, 'input1) Port_Input_Value"
typ "('comp1, 'output) Port_Output_Value"
definition
  Equiv_Exec :: "
    'input \<Rightarrow>
...
term "Equiv_Exec m equiv_states equiv_input"
...
by (simp add: Equiv_Exec_def)
thm Equiv_Exec_le1[THEN conjunct1]
thm Equiv_Exec_le1[THEN conjunct2]
...
definition Exec_Equal_State :: "
    ('comp, 'state) Comp_Local_State \<Rightarrow> 
    ('comp, 'input message_af) Comp_Trans_Fun \<Rightarrow> bool"
where
  "Exec_Equal_State localState trans_fun \<equiv>
...
corollary f_Exec_State_Idle_replicate_NoMsg_gr_output: "
  \<lbrakk> State_Idle localState output_fun trans_fun (
    ...
    m < n \<rbrakk> \<Longrightarrow>
  output_fun (f_Exec_Comp trans_fun \<NoMsg>\<^bsup>n\<^esup> c) = \<NoMsg>"
thm f_Exec_State_Idle_append_replicate_NoMsg_gr_output
...
corollary State_Idle_output_Msg_eq_last_message: "
  ...
  last_message (map output_fun (f_Exec_Comp_Stream trans_fun xs c)) =
  output_fun (f_Exec_Comp trans_fun xs c)"
thm subst[OF State_Idle_append_replicate_NoMsg_output_Msg_eq_last_message, rule_format]
...
definition
  f_Exec_Comp_Stream_Acc_Output :: "
    ...
    'output fstream_af"
where
  "f_Exec_Comp_Stream_Acc_Output k output_fun trans_fun xs c \<equiv> 
...
definition
  f_Exec_Comp_Stream_Acc_LocalState :: "
    ...
    'state list"
where
  "f_Exec_Comp_Stream_Acc_LocalState k localState trans_fun xs c \<equiv> 
    (map localState (f_Exec_Comp_Stream trans_fun (xs \<odot>\<^sub>f k) c)) \<div>\<^bsub>fl\<^esub> k"

definition
  i_Exec_Comp_Stream_Acc_Output :: "
    nat \<Rightarrow> (* Acceleration factor *)
    ('comp \<Rightarrow> 'output message_af) \<Rightarrow> (* Output extraction function *)
    ('comp, 'input message_af) Comp_Trans_Fun \<Rightarrow> 
    ...
    'output istream_af"
where
  "i_Exec_Comp_Stream_Acc_Output k output_fun trans_fun input c \<equiv> 
    (output_fun \<circ> (i_Exec_Comp_Stream trans_fun (input \<odot>\<^sub>i k) c)) \<div>\<^sub>i k"
definition
  i_Exec_Comp_Stream_Acc_LocalState :: "
    nat \<Rightarrow> (* Acceleration factor *)
    ('comp \<Rightarrow> 'state) \<Rightarrow> (* Local state extraction function *)
    ('comp, 'input message_af) Comp_Trans_Fun \<Rightarrow> 
    'input istream_af \<Rightarrow> 'comp \<Rightarrow>
    'state ilist"
where
  "i_Exec_Comp_Stream_Acc_LocalState k localState trans_fun input c \<equiv> 
    (localState \<circ> (i_Exec_Comp_Stream trans_fun (input \<odot>\<^sub>i k) c)) \<div>\<^bsub>il\<^esub> k"

definition
  f_Exec_Comp_Stream_Acc_Output_Init :: "
    nat \<Rightarrow> (* Acceleration factor *)
    ('comp \<Rightarrow> 'output message_af) \<Rightarrow> (* Output extraction function *)
    ('comp, 'input message_af) Comp_Trans_Fun \<Rightarrow> 
    'input fstream_af \<Rightarrow> 'comp \<Rightarrow>
    'output fstream_af"
where
  "f_Exec_Comp_Stream_Acc_Output_Init k output_fun trans_fun xs c \<equiv> 
    (output_fun c) # f_Exec_Comp_Stream_Acc_Output k output_fun trans_fun xs c"
definition
  f_Exec_Comp_Stream_Acc_LocalState_Init :: "
    nat \<Rightarrow> (* Acceleration factor *)
    ('comp \<Rightarrow> 'state) \<Rightarrow> (* Local state extraction function *)
    ('comp, 'input message_af) Comp_Trans_Fun \<Rightarrow> 'input fstream_af \<Rightarrow> 'comp \<Rightarrow>
    'state list"
where
  "f_Exec_Comp_Stream_Acc_LocalState_Init k localState trans_fun xs c \<equiv> 
...
definition 
  i_Exec_Comp_Stream_Acc_Output_Init :: "
    ...
    'output istream_af"
where
  "i_Exec_Comp_Stream_Acc_Output_Init k output_fun trans_fun input c \<equiv> 
    [output_fun c] \<frown> (i_Exec_Comp_Stream_Acc_Output k output_fun trans_fun input c)"
definition
  i_Exec_Comp_Stream_Acc_LocalState_Init :: "
    nat \<Rightarrow> (* Acceleration factor *)
    ('comp \<Rightarrow> 'state) \<Rightarrow> (* Local state extraction function *)
    ('comp, 'input message_af) Comp_Trans_Fun \<Rightarrow> 
    'input istream_af \<Rightarrow> 'comp \<Rightarrow>
    'state ilist"
where
  "i_Exec_Comp_Stream_Acc_LocalState_Init k localState trans_fun input c \<equiv> 
...
thm f_Exec_Stream_expand_map_aggregate_append
...
thm f_Exec_Stream_expand_aggregate_map_nth
...
definition 
  Acc_Trans_Fun_Step :: "
    ...
    'comp"
where
  "Acc_Trans_Fun_Step k trans_fun pointwise_shrink x c \<equiv> 
...
definition is_Pointwise_Output_Shrink :: "
  ...
  bool"
where
  "is_Pointwise_Output_Shrink pointwise_shrink output_fun \<equiv> 
...
definition is_correct_localState_Pointwise_Output_Shrink :: "
  ...
  bool"
where
  "is_correct_localState_Pointwise_Output_Shrink pointwise_shrink localState \<equiv>
...
apply clarify
thm Deterministic_f_Exec_Stream
...
thm f_Exec_Stream_Acc_LocalState_eq_Acc_Trans_Fun_Step_LocalState
thm f_Exec_Stream_Acc_Output_eq_Acc_Trans_Fun_Step_Output
...
thm f_Exec_Stream_Init_strictly_causal
...
thm i_Exec_Stream_Init_strictly_causal
...
thm f_Exec_Stream_Init_eq_f_Exec_Stream_Cons_output
...
thm f_Exec_Stream_Init_tl_eq_f_Exec_Stream
lemma f_Exec_Stream__Acc_OutputInit_tl_eq_f_Exec_Stream_Acc_Output: "
  ...
  f_Exec_Comp_Stream_Acc_Output k output_fun trans_fun xs c"
by (simp add: f_Exec_Stream_Acc_Output_Init_eq_f_Exec_Stream_Acc_Output_Cons)
thm f_Exec_Stream_Previous_f_Exec_Stream_Init
...
thm f_Exec_Stream_Init_eq_output_channel
...
thm i_Exec_Stream_Init_eq_i_Exec_Stream_Cons
...
thm i_Exec_Stream_Init_eq_i_Exec_Stream_Cons_output
...
thm i_Exec_Stream_Previous_i_Exec_Stream_Init
...
thm i_Exec_Stream_Init_eq_output_channel
...
apply (rename_tac i')
thm f_Exec_eq_f_Exec_Stream_last2[symmetric]
apply (subst f_Exec_eq_f_Exec_Stream_last2[symmetric], simp)+
...
thm f_Equiv_Exec_Stream_expand_shrink_equiv_state_set[no_vars]
thm f_Equiv_Exec_Stream_expand_shrink_equiv_state[no_vars]

thm f_Equiv_Exec_Stream_expand_shrink_equiv_state_set[rule_format, no_vars]
...
thm f_shrink_last_nth[of "length input - Suc 0" "f_Exec_Comp_Stream trans_fun1 (map input_fun1 input \<odot>\<^sub>f k1) c1" k1, symmetric]
...
thm f_Equiv_Exec_Stream_expand_shrink_equiv_state_set[rule_format, no_vars]
thm f_Equiv_Exec_Stream_expand_shrink_equiv_state_set[of k1 k2 equiv_states localState1 _ localState2 _ A input_fun1 input_fun2 output_fun1 output_fun2]
...
lemma f_Equiv_Exec_expand_shrink_equiv_state:"
  ...
  equiv_states
    (localState1 (f_Exec_Comp trans_fun1 (map input_fun1 input \<odot>\<^sub>f k1) c1))
    (localState2 (f_Exec_Comp trans_fun2 (map input_fun2 input \<odot>\<^sub>f k2) c2))"
thm f_Equiv_Exec_expand_shrink_equiv_state_set
...
apply (rule Equiv_Exec_output_eqI[of equiv_states localState1 _ localState2 _ _ input_fun1 input_fun2])
 ...
 thm f_Exec_Stream_expand_shrink_last_nth_eq_f_Exec_Comp[symmetric]
...
thm f_Equiv_Exec_Stream_expand_shrink_output_set_eq[no_vars]
...
thm f_Equiv_Exec_Stream_expand_shrink_output_eq[no_vars]
...
thm f_Equiv_Exec_Stream_expand_shrink_equiv_state_set[of 
  k1 k2 equiv_states localState1 c1 localState2 c2 A 
  input_fun1 input_fun2 output_fun1 output_fun2 trans_fun1 trans_fun2 input]
...
thm f_Equiv_Exec_Stream_Acc_LocalState[no_vars]
...
thm f_Equiv_Exec_Stream_Acc_Output_eq[no_vars]
...
apply (induct xs, simp)
apply (case_tac i)
 thm f_Exec_State_Idle_append_replicate_NoMsg_state
...
apply (drule meta_mp)
 thm f_Exec_Stream_Acc_LocalState_nth
...
thm f_Exec_Stream_Acc_LocalState__State_Idle_nth[no_vars]
...
apply (subgoal_tac "
 ...
 prefer 2
 thm f_Exec_Stream_Acc_LocalState_nth[of k _ xs, symmetric]
...
thm f_Exec_Stream_Acc_LocalState__State_Idle_all[no_vars]
