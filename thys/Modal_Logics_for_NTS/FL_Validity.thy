...
begin
  ...
  function (sequential) valid_Tree :: "'state \<Rightarrow> ('idx,'pred,'act,'effect) Tree \<Rightarrow> bool" where
  ...
  | "valid_Tree P (tPred f \<phi>) \<longleftrightarrow> \<langle>f\<rangle>P \<turnstile> \<phi>"
  | "valid_Tree P (tAct f \<alpha> t) \<longleftrightarrow> (\<exists>\<alpha>' t' P'. tAct f \<alpha> t =\<^sub>\<alpha> tAct f \<alpha>' t' \<and> \<langle>f\<rangle>P \<rightarrow> \<langle>\<alpha>',P'\<rangle> \<and> valid_Tree P' t')"
  ...
  next
    case (4 P f \<alpha> t) show ?case
      proof
        assume "valid_Tree P (tAct f \<alpha> t)"
        then obtain \<alpha>' t' P' where *: "tAct f \<alpha> t =\<^sub>\<alpha> tAct f \<alpha>' t' \<and> \<langle>f\<rangle>P \<rightarrow> \<langle>\<alpha>',P'\<rangle> \<and> valid_Tree P' t'"
          by auto
        ...
        ultimately show "valid_Tree (p \<bullet> P) (p \<bullet> tAct f \<alpha> t)"
          by auto
      next
        assume "valid_Tree (p \<bullet> P) (p \<bullet> tAct f \<alpha> t)"
        then obtain \<alpha>' t' P' where *: "p \<bullet> tAct f \<alpha> t =\<^sub>\<alpha> tAct (p \<bullet> f) \<alpha>' t' \<and> (p \<bullet> \<langle>f\<rangle>P) \<rightarrow> \<langle>\<alpha>',P'\<rangle> \<and> valid_Tree P' t'"
          by auto
        ...
        ultimately show "valid_Tree P (tAct f \<alpha> t)"
          by auto
      qed
  qed
  ...
  next
    ...
    proof
      assume "valid_Tree P (tAct f1 \<alpha>1 t1)"
      then obtain \<alpha>' t' P' where "tAct f1 \<alpha>1 t1 =\<^sub>\<alpha> tAct f1 \<alpha>' t' \<and> \<langle>f1\<rangle>P \<rightarrow> \<langle>\<alpha>',P'\<rangle> \<and> valid_Tree P' t'"
        by auto
      moreover from tAct.hyps have "tAct f1 \<alpha>1 t1 =\<^sub>\<alpha> tAct f2 \<alpha>2 t2"
        using alpha_tAct by blast
      ultimately show "valid_Tree P (tAct f2 \<alpha>2 t2)"
        using tAct.hyps by (metis Tree\<^sub>\<alpha>.abs_eq_iff valid_Tree.simps(4))
    next
      assume "valid_Tree P (tAct f2 \<alpha>2 t2)"
      then obtain \<alpha>' t' P' where "tAct f2 \<alpha>2 t2 =\<^sub>\<alpha> tAct f2 \<alpha>' t' \<and> \<langle>f2\<rangle>P \<rightarrow> \<langle>\<alpha>',P'\<rangle> \<and> valid_Tree P' t'"
        by auto
      moreover from tAct.hyps have "tAct f1 \<alpha>1 t1 =\<^sub>\<alpha> tAct f2 \<alpha>2 t2"
        using alpha_tAct by blast
      ultimately show "valid_Tree P (tAct f1 \<alpha>1 t1)"
        using tAct.hyps by (metis Tree\<^sub>\<alpha>.abs_eq_iff valid_Tree.simps(4))
    qed
  qed simp_all
  ...
  lift_definition valid_Tree\<^sub>\<alpha> :: "'state \<Rightarrow> ('idx,'pred,'act,'effect) Tree\<^sub>\<alpha> \<Rightarrow> bool" is
    valid_Tree
  ...
  lemma valid_Tree\<^sub>\<alpha>_Pred\<^sub>\<alpha> [simp]: "valid_Tree\<^sub>\<alpha> P (Pred\<^sub>\<alpha> f \<phi>) \<longleftrightarrow> \<langle>f\<rangle>P \<turnstile> \<phi>"
  by transfer simp

  lemma valid_Tree\<^sub>\<alpha>_Act\<^sub>\<alpha> [simp]: "valid_Tree\<^sub>\<alpha> P (Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha>) \<longleftrightarrow> (\<exists>\<alpha>' t\<^sub>\<alpha>' P'. Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha> = Act\<^sub>\<alpha> f \<alpha>' t\<^sub>\<alpha>' \<and> \<langle>f\<rangle>P \<rightarrow> \<langle>\<alpha>',P'\<rangle> \<and> valid_Tree\<^sub>\<alpha> P' t\<^sub>\<alpha>')"
  proof
    assume "valid_Tree\<^sub>\<alpha> P (Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha>)"
    ...
    ultimately show "\<exists>\<alpha>' t\<^sub>\<alpha>' P'. Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha> = Act\<^sub>\<alpha> f \<alpha>' t\<^sub>\<alpha>' \<and> \<langle>f\<rangle>P \<rightarrow> \<langle>\<alpha>',P'\<rangle> \<and> valid_Tree\<^sub>\<alpha> P' t\<^sub>\<alpha>'"
      by (metis Act\<^sub>\<alpha>.abs_eq Tree\<^sub>\<alpha>.abs_eq_iff valid_Tree.simps(4) valid_Tree\<^sub>\<alpha>.abs_eq)
  next
    assume "\<exists>\<alpha>' t\<^sub>\<alpha>' P'. Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha> = Act\<^sub>\<alpha> f \<alpha>' t\<^sub>\<alpha>' \<and> \<langle>f\<rangle>P \<rightarrow> \<langle>\<alpha>',P'\<rangle> \<and> valid_Tree\<^sub>\<alpha> P' t\<^sub>\<alpha>'"
    moreover have "\<And>\<alpha>' t\<^sub>\<alpha>'. Act\<^sub>\<alpha> f \<alpha>' t\<^sub>\<alpha>' = abs_Tree\<^sub>\<alpha> (tAct f \<alpha>' (rep_Tree\<^sub>\<alpha> t\<^sub>\<alpha>'))"
      by (metis Act\<^sub>\<alpha>.abs_eq Tree\<^sub>\<alpha>_abs_rep)
    ultimately show "valid_Tree\<^sub>\<alpha> P (Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha>)"
  ...
  lift_definition valid :: "'state \<Rightarrow> ('idx,'pred,'act,'effect) formula \<Rightarrow> bool" (infix "\<Turnstile>" 70) is
  ...
  lemma valid_Pred [simp]: "P \<Turnstile> Pred f \<phi> \<longleftrightarrow> \<langle>f\<rangle>P \<turnstile> \<phi>"
  by transfer simp

  lemma valid_Act: "P \<Turnstile> Act f \<alpha> x \<longleftrightarrow> (\<exists>\<alpha>' x' P'. Act f \<alpha> x = Act f \<alpha>' x' \<and> \<langle>f\<rangle>P \<rightarrow> \<langle>\<alpha>',P'\<rangle> \<and> P' \<Turnstile> x')"
  proof
