...
definition rotation :: "(real^3 \<Rightarrow> real^3) \<Rightarrow> bool" where
  "rotation f \<longleftrightarrow> orthogonal_transformation f \<and> matrix_det (matrix f) = 1"
...
proof -
  ...
  moreover have "matrix_det (matrix (id::real^3 \<Rightarrow> real^3)) = 1"
    by simp
...
proof -
  from assms have f_orth: "orthogonal_transformation f"
    ...
    and f_det: "matrix_det (matrix f) = 1"
    and g_det: "matrix_det (matrix g) = 1"
  ...
  hence "matrix_det (matrix (f \<circ> g)) = matrix_det (matrix f) * matrix_det (matrix g)"
    by (simp add: det_mul)
  also have "\<dots> = 1" using f_det g_det by simp
  finally have "matrix_det (matrix (f \<circ> g)) = 1" .
...
lemma det_Rx_mat [simp]: "matrix_det Rx_mat = 1"
  unfolding Rx_mat_def
  by (simp add: det_3 vector_def rot_c_sq_plus_rot_s_sq power2_eq_square algebra_simps)

lemma det_Rz_mat [simp]: "matrix_det Rz_mat = 1"
  unfolding Rz_mat_def
  by (simp add: det_3 vector_def rot_c_sq_plus_rot_s_sq power2_eq_square algebra_simps)

lemma det_Rx_inv_mat [simp]: "matrix_det Rx_inv_mat = 1"
  unfolding Rx_inv_mat_def
  by (simp add: det_3 vector_def rot_c_sq_plus_rot_s_sq power2_eq_square algebra_simps)

lemma det_Rz_inv_mat [simp]: "matrix_det Rz_inv_mat = 1"
