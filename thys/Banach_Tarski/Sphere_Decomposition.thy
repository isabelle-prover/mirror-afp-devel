...
lemma det_Rz_angle_mat [simp]: "matrix_det (Rz_angle_mat t) = 1"
...
proof -
  ...
  have "matrix_det (matrix (f \<circ> Hilbert_Choice.inv f)) =
      matrix_det (matrix f) * matrix_det (matrix (Hilbert_Choice.inv f))"
    using matrix_compose[OF inv_lin f_lin] by (simp add: det_mul)
  also have "\<dots> = matrix_det (matrix (Hilbert_Choice.inv f))"
    using f_det by simp
  finally have "matrix_det (matrix (Hilbert_Choice.inv f)) = 1"
...
proof -
  ...
  obtain S where S_orth: "orthogonal_transformation S"
    and S_det: "matrix_det (matrix S) = 1"
    and S_e3: "S e3_3 = a"
