...
proof -
  have "\<L>\<^sub>b v x = L (mk_dec_det (policy_improvement d v)) v x" for x
    ...
    by (fastforce simp: thm_6_2_10_a_aux' L_eq_L\<^sub>a_det is_opt_act_def policy_improvement_def
        arg_max_SUP[symmetric, of _ _ "(policy_improvement d v x)"] )
  thus ?thesis
    using policy_improvement_is_dec_det assms
    by (auto simp: \<nu>_improving_alt)
qed
...
lemma eval_policy_step_L:
  ...
  using assms
  by (auto simp: \<nu>_improving_imp_\<L>\<^sub>b[OF policy_improvement_improving])
...
proof -
  ...
  have "L (mk_dec_det d) (policy_eval d) \<le> L ?d' (policy_eval d)"
    using assms
    by (auto simp: L_le_\<L>\<^sub>b eval_policy_step_L)
  hence "policy_eval d \<le> L ?d' (policy_eval d)"
    using L_\<nu>_fix policy_eval_def
    by auto
  ...
  hence "\<nu>\<^sub>b ?dp \<le> ?P (r_dec\<^sub>b ?d')"
    using inv_norm_le'(2)[OF norm_\<P>\<^sub>1_l_less] blincomp_scaleR_right suminf_cong
    by (metis (mono_tags, lifting))
  thus ?thesis
    unfolding policy_eval_def
    by (auto simp: \<nu>_stationary)
qed
