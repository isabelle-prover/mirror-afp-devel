...
locale MDP_MPI = MDP_finite_type A K r l + MDP_act A K r l arb_act
  for A and K :: "'s :: finite \<times> 'a :: finite \<Rightarrow> 's pmf" and r l arb_act
begin
...
proof -
  ...
  show ?thesis
    unfolding *
  proof (rule antisym)
    show "(\<Squnion>d\<in>D\<^sub>R. L d v s - v s) \<le> \<L> v s - v s"
      unfolding \<L>_def
      using ex_dec
  ...
  next
    ...
    thus "\<L> v s - v s \<le> (\<Squnion>d \<in> D\<^sub>R. L d v s - v s)"
      unfolding \<L>_def diff_le_eq
...
lift_definition B\<^sub>b :: "('s \<Rightarrow>\<^sub>b real) \<Rightarrow> 's \<Rightarrow>\<^sub>b real" is "B"
  using \<L>\<^sub>b.rep_eq[symmetric] B_eq_\<L>
...
lemma U_zero [simp]: "U 0 v = v"
  unfolding U_def \<L>_def
  by (auto simp: \<nu>\<^sub>b_fin.rep_eq)

lemma U_one_eq_\<L>: "U 1 v s = \<L> v s"
  unfolding U_def \<L>_def 
  by (auto simp: \<nu>\<^sub>b_fin_eq_\<P>\<^sub>X L_def blinfun.bilinear_simps)
...
proof -
  ...
  have "norm (\<nu>\<^sub>b_fin (mk_stationary d) m) \<le> (\<Sum>i<m. l ^ i * r\<^sub>M)" for d m
    using abs_\<nu>_fin_le \<nu>\<^sub>b_fin.rep_eq
...
proof -
  ...
  proof -
    ...
    also have "\<dots> = l^m * (\<Squnion>d \<in> D\<^sub>R. ((\<P>\<^sub>1 d ^^ m) (v - u) s))" 
      using D\<^sub>R_ne bounded_P bounded_disc_\<P>\<^sub>1'
    ...
    also have "\<dots> \<le> l^m * (\<Squnion>d \<in> D\<^sub>R. norm ((v - u)))"
      using norm_\<P>\<^sub>X_apply
      by (auto simp: \<P>\<^sub>X_const[symmetric] cSUP_least mult_left_mono)
...
proof -
  ...
  thus ?thesis
    using L_iter_le_\<L>\<^sub>b ex_dec
...
proof -
  ...
  proof -
    ...
    thus ?thesis
      ...
      by (auto intro!: order.trans[OF _ L_iter_le_U\<^sub>b])
  qed
  have "U\<^sub>b m \<nu>\<^sub>b_opt \<le> \<nu>\<^sub>b_opt" for m
    using \<L>_inc_le_opt
    by (auto intro!: order.trans[OF U\<^sub>b_le_\<L>\<^sub>b] simp: funpow_swap1)
  ...
  moreover have "(lim (\<lambda>n. (U\<^sub>b (Suc m) ^^n) v)) = U\<^sub>b (Suc m) (lim (\<lambda>n. (U\<^sub>b (Suc m) ^^n) v))"
    using limI[OF U\<^sub>b_conv(2)] theI'[OF U\<^sub>b_conv(1)]
    by auto
  ultimately show ?thesis
    using U\<^sub>b_conv(1)
    by metis
qed

lemma U\<^sub>b_tendsto: "(\<lambda>n. (U\<^sub>b (Suc m) ^^ n) v) \<longlonglongrightarrow> \<nu>\<^sub>b_opt"
  using lim_U\<^sub>b U\<^sub>b_convergent convergent_LIMSEQ_iff
  by metis
...
proof -
  ...
  hence "W d m v =  v + ((\<Sum>i<m. (l *\<^sub>R \<P>\<^sub>1 d)^^i) (L d v)) - (\<Sum>i<m. (l *\<^sub>R \<P>\<^sub>1 d)^^i) v"
    by (auto simp: W_def B\<^sub>b_eq_\<L>\<^sub>b blinfun.bilinear_simps algebra_simps )
  also have "\<dots> = v + \<nu>\<^sub>b_fin (mk_stationary d) m + (\<Sum>i<m. ((l *\<^sub>R \<P>\<^sub>1 d)^^i) ((l *\<^sub>R \<P>\<^sub>1 d) v)) - (\<Sum>i<m. (l *\<^sub>R \<P>\<^sub>1 d)^^i) v"
    unfolding L_def
    by (auto simp: \<nu>\<^sub>b_fin_eq blinfun.bilinear_simps blinfun.sum_left scaleR_right.sum)
...
proof -
  have "U\<^sub>b m u - W d m v \<ge> \<nu>\<^sub>b_fin (mk_stationary d) m + ((l *\<^sub>R \<P>\<^sub>1 d) ^^ m) u - (\<nu>\<^sub>b_fin (mk_stationary d) m + ((l *\<^sub>R \<P>\<^sub>1 d)^^m) v)"
    using \<nu>_improving_D_MR assms(2) bounded_stationary_\<nu>\<^sub>b_fin bounded_disc_\<P>\<^sub>1
    by (fastforce intro!: diff_mono bounded_imp_bdd_above cSUP_upper bounded_plus_comp simp: U\<^sub>b.rep_eq U_def L_iter W_eq_L_iter)
  hence *: "U\<^sub>b m u - W d m v \<ge> ((l *\<^sub>R \<P>\<^sub>1 d) ^^ m) (u - v)"
    by (auto simp: blinfun.diff_right)
  show "W d m v \<le> U\<^sub>b m u"
    using order.trans[OF \<P>\<^sub>1_n_disc_pos[unfolded blincomp_scaleR_right[symmetric]] *] assms
    by auto
qed
...
proof -
  have "\<L>\<^sub>b v \<le> u + B\<^sub>b u"
    using assms(1) \<L>\<^sub>b_mono B\<^sub>b_eq_\<L>\<^sub>b
    by auto
...
proof -
  have "r_dec\<^sub>b d + (l *\<^sub>R \<P>\<^sub>1 d - id_blinfun) u \<le> B\<^sub>b u"
    using L_def L_le_\<L>\<^sub>b assms     
    by (auto simp: B\<^sub>b_eq_\<L>\<^sub>b \<L>\<^sub>b.rep_eq \<L>_def blinfun.bilinear_simps)
  moreover have "B\<^sub>b v = r_dec\<^sub>b d + (l *\<^sub>R \<P>\<^sub>1 d - id_blinfun) v"
    using assms
    by (auto simp: B\<^sub>b_eq_\<L>\<^sub>b \<nu>_improving_imp_\<L>\<^sub>b[of _ d] L_def blinfun.bilinear_simps)
  ultimately show ?thesis
...
proof -
  ...
  thus ?thesis using B\<^sub>b_eq_\<L>\<^sub>b
    by auto
qed

subsection \<open>Computing the Bellman Operator over Multiple Steps\<close>
definition L_pow :: "('s \<Rightarrow>\<^sub>b real) \<Rightarrow> ('s \<Rightarrow> 'a) \<Rightarrow> nat \<Rightarrow> ('s \<Rightarrow>\<^sub>b real)" where
...
(* eq 6.5.7 *)
lemma L_pow_eq:
  assumes "\<nu>_improving v (mk_dec_det d)"
  shows "L_pow v d m = v + (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 (mk_dec_det d))^^i)) (B\<^sub>b v)"
proof -
  let ?d = "(mk_dec_det d)"
  have "(\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) (B\<^sub>b v) = (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) (L ?d v) - (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) v"
    using assms
    by (auto simp: B\<^sub>b_eq_\<L>\<^sub>b blinfun.bilinear_simps \<nu>_improving_imp_\<L>\<^sub>b)
  also have "\<dots> = (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) (r_dec\<^sub>b ?d) + (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) ((l *\<^sub>R \<P>\<^sub>1 ?d) v) - (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) v"
    by (simp add: L_def blinfun.bilinear_simps)
  also have "\<dots> = (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) (r_dec\<^sub>b ?d) + (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^Suc i)) v - (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) v"
    by (auto simp: blinfun.sum_left blinfunpow_assoc simp del: blinfunpow.simps)
  also have "\<dots> = (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) (r_dec\<^sub>b ?d) + (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^Suc i) - (l *\<^sub>R \<P>\<^sub>1 ?d)^^i) v"
    by (simp add: blinfun.diff_left sum_subtractf)
  also have "\<dots> = (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) (r_dec\<^sub>b ?d) + ((l *\<^sub>R \<P>\<^sub>1 ?d)^^Suc m) v - v"
    by (subst sum_telescope') (auto simp: blinfun.bilinear_simps)
  finally have "(\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) (B\<^sub>b v) = (\<Sum>i \<le> m. ((l *\<^sub>R \<P>\<^sub>1 ?d)^^i)) (r_dec\<^sub>b ?d) + ((l *\<^sub>R \<P>\<^sub>1 ?d)^^Suc m) v - v" .
  moreover have "L_pow v d m = \<nu>\<^sub>b_fin (mk_stationary_det d) (Suc m) + ((l *\<^sub>R \<P>\<^sub>1 ?d)^^Suc m) v"
    by (simp only: L_pow_def L_iter lessThan_Suc_atMost[symmetric])
  ultimately show ?thesis
    by (auto simp: \<nu>\<^sub>b_fin_eq lessThan_Suc_atMost)
qed
...
lemma L_pow_\<L>\<^sub>b_mono_inv:
  ...
  using assms L_pow_eq_W \<L>\<^sub>b_W_ge policy_improvement_improving 
  by auto
...
lemma mpi_val_eq: "mpi_val (Suc n) = 
  ...
  using L_pow_eq[OF \<nu>_improving_mpi_pol] mpi_val_Suc
  by auto
...
lemma mpi_includes_value_it: 
  ...
  using assms B\<^sub>b_eq_\<L>\<^sub>b mpi_val_eq
  by auto
...
proof -
  ...
  next
    ...
    have val_eq_W: "mpi_val (Suc n) = W (mk_dec_det (mpi_pol n)) (Suc (m n (mpi_val n))) (mpi_val n)"
      using \<nu>_improving_mpi_pol mpi_val_Suc W_eq_L_iter L_pow_def
      by auto
    ...
    moreover have "mpi_val (Suc n) \<le> w (Suc n)"
      using Suc.IH \<nu>_improving_mpi_pol 
      by (auto simp: val_eq_W intro: order.trans[OF _ W_le_U\<^sub>b])
    moreover have "y (Suc n) \<le> mpi_val (Suc n)"
      using Suc.IH \<nu>_improving_mpi_pol W_ge_\<L>\<^sub>b
      by (auto simp: y_def B\<^sub>b_eq_\<L>\<^sub>b val_eq_W)
...
proof -
  note dist_\<L>\<^sub>b_lt_dist_opt
...
function (domintros) mpi_algo where "mpi_algo d v m = (
  ...
  else mpi_algo (policy_improvement d v) (L_pow v (policy_improvement d v) (m 0 v)) (\<lambda>n. m (Suc n)))"
  by auto
...
fun mpi' where
  ...
  "mpi' d v (Suc n) m = (
  let d' = policy_improvement d v; v' = L_pow v d' (m 0 v) in mpi' d' v' n (\<lambda>n. m (Suc n)))"

lemma mpi_Suc':
  assumes "d \<in> D\<^sub>D"
  shows "mpi d v m (Suc n) = mpi (policy_improvement d v) (L_pow v (policy_improvement d v) (m 0 v)) (\<lambda>a. m (Suc a)) n"
...
proof -
  define n where "n = (LEAST n. 2 * l * dist (mpi_val d v m n) (\<L>\<^sub>b (mpi_val d v m n)) < eps * (1 - l))" (is "n = (LEAST n. ?P d v m n)")
  ...
  proof (induction n arbitrary: v d m)
    case 0
    have "2 * l * dist (mpi_val d v m 0) (\<L>\<^sub>b (mpi_val d v m 0)) < eps * (1 - l)"
  ...
  next
    ...
    have "Suc n = Suc (LEAST n. 2 * l * dist (mpi_val d v m (Suc n)) (\<L>\<^sub>b (mpi_val d v m (Suc n))) < eps * (1 - l))"
    ...
    hence "n = (LEAST n. 2 * l * dist (mpi_val d v m (Suc n)) (\<L>\<^sub>b (mpi_val d v m (Suc n))) < eps * (1 - l))"
      by auto
    hence n_eq: "n =
    (LEAST n. 2 * l * dist (mpi_val ?d (L_pow v ?d (m 0 v)) (\<lambda>a. m (Suc a)) n) (\<L>\<^sub>b (mpi_val ?d (L_pow v ?d (m 0 v)) (\<lambda>a. m (Suc a)) n))
        < eps * (1 - l))"
    ...
    moreover have "mpi_algo_dom (?d, L_pow v ?d (m 0 v), \<lambda>a. m (Suc a))"
      using Suc.IH[OF n_eq \<open>0 < eps\<close>] Suc.prems is_dec_det_pi L_pow_\<L>\<^sub>b_mono_inv by auto
    ultimately show ?case
...
abbreviation "mpi_alg_rec d v m \<equiv> 
    (if 2 * l * dist v (\<L>\<^sub>b v) < eps * (1 - l) then (policy_improvement d v, v)
...
lemma mpi_algo_eq_mpi:
  assumes "d \<in> D\<^sub>D" "v \<le> \<L>\<^sub>b v" "eps > 0"
  shows "mpi_algo d v m = mpi d v m (LEAST n. 2 * l * dist (mpi_val d v m n) (\<L>\<^sub>b (mpi_val d v m n)) < eps * (1 - l))"
proof -
  define n where "n = (LEAST n. 2 * l * dist (mpi_val d v m n) (\<L>\<^sub>b (mpi_val d v m n)) < eps * (1 - l))" (is "n = (LEAST n. ?P d v m n)")
  from n_def assms show ?thesis
  ...
  next
    ...
    hence not0: "\<not> (2 * l * dist v (\<L>\<^sub>b v) < eps * (1 - l))"
      ...
      by auto
    obtain n' where "2 * l * dist (mpi_val d v m n') (\<L>\<^sub>b (mpi_val d v m n')) < eps * (1 - l)"
      using mpi_val_term_ex[OF Suc(3) Suc(4), of _ m] assms by blast
    hence "n = (LEAST n. ?P d v m (Suc n))"
      using Suc(2) Suc
      by (subst (asm) Least_Suc) auto
    hence "n = (LEAST n. ?P (policy_improvement d v) (L_pow v (policy_improvement d v) (m 0 v)) (\<lambda>n. m (Suc n)) n)"
    ...
    hence "mpi_algo d v m = mpi d v m (Suc n)"
      ...
      using Suc(1) Suc.prems(2-4) is_dec_det_pi mpi_Suc' not0 L_pow_\<L>\<^sub>b_mono_inv by force
    thus ?case
...
proof -
  let ?P = "\<lambda>n. 2 * l * dist (mpi_val d v0 m n) (\<L>\<^sub>b (mpi_val d v0 m n)) <  eps * (1 - l)"
  let ?n = "Least ?P"
...
abbreviation "r_min \<equiv> (\<Sqinter>s' a. r (s', a))"
...
proof (rule less_eq_bfunI)
  fix x
  have "r_min \<le> r (s, a)" for s a
    by (fastforce intro: cInf_lower2)
  hence "r_min \<le> (1-l) * r (s, a) + l * r_min" for s a
    using disc_lt_one zero_le_disc
    by (meson order_less_imp_le order_refl segment_bound_lemma)
  hence "r_min / (1 - l) \<le> ((1-l) * r (s, a) + l * r_min) / (1 - l)" for s a
    using order_less_imp_le[OF disc_lt_one]
    by (auto intro!: divide_right_mono)
  hence "r_min / (1 - l) \<le> r (s, a) + (l * r_min) / (1 - l)" for s a
    using disc_lt_one
    by (auto simp: add_divide_distrib)
  thus "v0_mpi\<^sub>b x \<le> \<L>\<^sub>b v0_mpi\<^sub>b x"
    unfolding \<L>\<^sub>b_eq_SUP_L\<^sub>a v0_mpi\<^sub>b.rep_eq v0_mpi_def
