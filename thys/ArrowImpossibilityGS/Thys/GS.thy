...
proof-
  ...
  { fix n have "n<=N \<Longrightarrow> f(?M n) = f P"
	...
	by(simp (no_asm_simp) add:ext_iff f_inv_into_f)
...
proof (auto simp:dict_def)
  ...
  ultimately show "(a,f P) : P i" using `a\<noteq>f P` by simp
qed
