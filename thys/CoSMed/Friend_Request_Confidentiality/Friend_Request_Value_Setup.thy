...
text \<open>The confidential information is the friendship requests between two arbitrary but fixed users:\<close>
...
datatype "value" =
  isFRVal: FRVal fUser req \<comment> \<open>friendship requests from \<open>UID1\<close> to \<open>UID2\<close> (or vice versa)\<close>
| isFVal: FVal bool \<comment> \<open>updates to the status of friendship between them\<close>
...
next
  ...
  moreover have "friends12 s1' \<longleftrightarrow> friends12 s1"
    ...
    by (cases "(uid,uid') \<in> {(UID1,UID2), (UID2,UID1)}") (auto simp add: c_defs friends12_def)
  moreover have "UID1 \<in>\<in> pendingFReqs s1' UID2 \<longleftrightarrow> UID1 \<in>\<in> pendingFReqs s1 UID2"
    using step trn uids UID1_UID2 UID1_UID2_UIDs
    by (cases "(uid,uid') \<in> {(UID1,UID2), (UID2,UID1)}") (auto simp add: c_defs)
  moreover have "UID2 \<in>\<in> pendingFReqs s1' UID1 \<longleftrightarrow> UID2 \<in>\<in> pendingFReqs s1 UID1"
    using step trn uids UID1_UID2 UID1_UID2_UIDs
    by (cases "(uid,uid') \<in> {(UID1,UID2), (UID2,UID1)}") (auto simp add: c_defs)
...
next
  ...
  moreover have "friends12 s1' \<longleftrightarrow> friends12 s1"
    using step trn uids UID1_UID2 UID1_UID2_UIDs
    by (cases "(uid,uid') \<in> {(UID1,UID2), (UID2,UID1)}") (auto simp add: d_defs friends12_def)
