...
record
  m1_state = "m1r_state" +
    leak :: "(key \<times> agent \<times> agent \<times> nonce \<times> time) set"   -- {* key leaked plus context *}
    clk :: "time" 
...
text {* The initiator injectively agrees with the responder on @{text "Kab"},
@{text "Ts"}, and @{text "Ta"}. *}
