...
section {* Needham-Schroeder Shared Key (L3) *}
...
where
  "m3_step4 Ra A B Na Kab X \<equiv> {(s, s1).
    ...
    Crypt (shrK A) {| Nonce Na, Agent B, Key Kab, X |} \<in> IK s \<and> (* recv msg 2 *)
...
where
  "m3_inv2_ticket \<equiv> {s. \<forall>A B N K X.
     ...
     Crypt (shrK A) {| Nonce N, Agent B, Key K, X |} \<in> parts (IK s) \<longrightarrow>
       X = Crypt (shrK B) {| Key K, Agent A |} \<and> K \<in> range sesK 
  }"
...
definition
  R23_keys :: "(m2_state \<times> m3_state) set" where     -- {* only an implication! *}
