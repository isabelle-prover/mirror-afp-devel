...
text {* Programs in pGCL are probabilistic automata.  They can do anything a traditional program
can, plus, they may make truly probabilistic choices. *}
...
text {* The primitive state operation is @{term Apply}, which takes a state transformer as an
...
text {* We can treat the record's fields as the names of \emph{variables}. Note that the right-hand
...
text {* Assertions in pGCL are real-valued functions of the state, which are often interpreted as a
...
embedded as @{term_type "\<guillemotleft>P::'s \<Rightarrow> bool\<guillemotright>"}, such that @{term "P s \<longrightarrow> \<guillemotleft>P\<guillemotright> s = 1 \<and> \<not> P s \<longrightarrow> \<guillemotleft>P\<guillemotright> s = 0"}.
  ...
  entails_implies}. Thus a valid specification of @{term "a := b"} is: *}
...
text {* Any ordinary computation and its associated annotation can be expressed in this form. *}

subsection {* Probability *}

text {* Next, we introduce the syntax @{term "x ;; y"} for the sequential composition of @{term x}
...
lemma
  ...
  txt {* Sufficiently small problems can be handled by the simplifier, by symbolic evaluation. *}
  by(simp add:wp_eval o_def)

subsection {* Nondeterminism *}

text {* We can also under-specify a program, using the \emph{nondeterministic choice} operator,
...
is \emph{at least} 1/3, but we can make no stronger statement: *}
...
text {* The probabilities of independent events combine as usual, by multiplying: The chance
of getting heads on two separate coins is @{term "1/4"}. *}
...
text {* If, rather than two coins, we use two dice, we can make some slightly more involved
...
text {* The expected value of a roll of a six-sided die is @{term "7/2"}: *}

lemma
  "(\<lambda>s. 7/2) = wp (bind v at (\<lambda>s. Puniform {1..6} v) in red := (\<lambda>_. v)) red"
  by(simp add:wp_eval supp_Puniform sum_head_Suc Puniform_in)
