...
sublocale Ker: vectorspace class_ring VK 
  rewrites "carrier VK = mat_kernel A"
    ...
    and [simp]: "module.smult VK = op \<cdot>\<^sub>v"
...
lemma mat_kernel_mult_right_gen_set: assumes A: "A \<in> carrier_mat nr nc"
  ...
  shows "kernel.gen_set nc A ((op *\<^sub>v B) ` gen)" "op *\<^sub>v B ` gen \<subseteq> mat_kernel A" "card ((op *\<^sub>v B) ` gen) = card gen"
proof -
  let ?AB = "A * B"
  let ?gen = "(op *\<^sub>v B) ` gen"
  ...
  hence inj_gen: "inj_on (op *\<^sub>v B) gen" unfolding inj_on_def by auto
  show "card ?gen = card gen" using inj_gen by (rule card_image)
  {
    ...
    proof (rule eq_vecI; unfold dim1 dim1b dim2)
      ...
      from sub inj have inj: "inj_on (op *\<^sub>v B) gen'" unfolding inj_on_def by auto
      {
        fix v
        assume "v \<in> gen'"
      ...
      have "A.Ker.lincomb a' ?gen' $ i = (\<Sum>v\<in>op *\<^sub>v B ` gen'. a' v * v $ i)"
  ...
  from one_dir other_dir show "kernel.gen_set nc A ((op *\<^sub>v B) ` gen)" by auto
qed
...
lemma mat_kernel_mult_right_basis: assumes A: "A \<in> carrier_mat nr nc"
  ...
  shows "kernel.basis nc A ((op *\<^sub>v B) ` gen)" 
  "card ((op *\<^sub>v B) ` gen) = card gen"
proof -
  let ?AB = "A * B"
  let ?gen = "(op *\<^sub>v B) ` gen"
