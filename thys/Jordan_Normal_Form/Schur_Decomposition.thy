...
definition vec_inv :: "'a::conjugatable_field vec \<Rightarrow> 'a vec"
  where "vec_inv v = 1 / (v \<bullet>c v) \<odot>\<^sub>v conjugate\<^sub>v v"

lemma vec_inv_closed[simp]: "v \<in> carrier\<^sub>v n \<Longrightarrow> vec_inv v \<in> carrier\<^sub>v n"
  unfolding vec_inv_def by auto

lemma vec_inv_dim[simp]: "dim\<^sub>v (vec_inv v) = dim\<^sub>v v"
  unfolding vec_inv_def by auto

lemma vec_inv[simp]:
  assumes v: "v : carrier\<^sub>v n"
      and v0: "(v::'a::conjugatable_ordered_field vec) \<noteq> \<zero>\<^sub>v n"
  shows "vec_inv v \<bullet> v = 1"
proof -
  { assume "v \<bullet>c v = 0"
    hence "v = \<zero>\<^sub>v n" using vec_conjugate_square_zero[OF v] by auto
    hence False using v0 by auto
  }
  moreover have "conjugate\<^sub>v v \<bullet> v = v \<bullet>c v"
    apply (rule scalar_prod_comm) using v by auto
  ultimately show ?thesis
    unfolding vec_inv_def
    apply (subst scalar_scalar_prod_assoc)
    using assms by auto
qed

lemma corthogonal_inv:
  assumes orth: "corthogonal (vs ::'a::conjugatable_field vec list)"
      and V: "set vs \<subseteq> carrier\<^sub>v n"
...
proof -
  ...
  show ?thesis
    unfolding mat_inverts_def
    apply rule
    unfolding mat_of_rows_dim length_map l_def[symmetric]
    unfolding mat_index_one
  proof -
    show "dim\<^sub>r (?W \<otimes>\<^sub>m ?V) = l" "dim\<^sub>c (?W \<otimes>\<^sub>m ?V) = l"
      unfolding mat_mult_mat_def rW cV by auto
    fix i j assume i:"i<l" and j: "j<l"
    hence i2: "i<length vs"
      and i3: "i<length (map vec_inv vs)"
      and j2: "j<length vs" using l_def by auto
    hence id2: "vs ! i \<in> carrier\<^sub>v n"
      and id3: "map vec_inv vs ! i \<in> carrier\<^sub>v n"
      and id4: "conjugate\<^sub>v (vs ! i) \<in> carrier\<^sub>v n"
      and jd2: "vs ! j \<in> carrier\<^sub>v n" using dim by auto
    show "(?W \<otimes>\<^sub>m ?V) $$ (i,j) = (if i = j then 1 else 0)"
      unfolding mat_mult_mat_def rW cV
      unfolding mat_index_mat[OF i j] split
      unfolding mat_of_rows_row[OF i3 id3]
      unfolding mat_of_cols_col[OF j2 jd2]
      unfolding nth_map[OF i2]
      unfolding vec_inv_def
      unfolding scalar_scalar_prod_assoc[OF id4 jd2]
      unfolding scalar_prod_comm[OF id4 jd2]
      using corthogonalD[OF orth j2 i2] by auto
  qed
qed

definition corthogonal_inv :: "'a::conjugatable_field mat \<Rightarrow> 'a mat"
  where "corthogonal_inv A = mat_of_rows (dim\<^sub>r A) (map vec_inv (cols A))"

definition mat_adjoint :: "'a :: conjugatable_field mat \<Rightarrow> 'a mat"
  where "mat_adjoint A \<equiv> mat_of_rows (dim\<^sub>r A) (map conjugate\<^sub>v (cols A))"

definition corthogonal_mat :: "'a::conjugatable_field mat \<Rightarrow> bool"
  where "corthogonal_mat A \<equiv>
    let B = mat_adjoint A \<otimes>\<^sub>m A in
...
proof
  have ci: "col A i : carrier\<^sub>v (dim\<^sub>r A)"
   and cj: "col A j : carrier\<^sub>v (dim\<^sub>r A)" by auto
  note [simp] = vec_conjugate_conjugate_sprod[OF ci cj]

  let ?B = "mat_adjoint A \<otimes>\<^sub>m A"
  have diag: "diagonal_mat ?B" and zero: "\<And>i. i<dim\<^sub>c A \<Longrightarrow> ?B $$ (i,i) \<noteq> 0"
...
lemma corthogonal_matI[intro]:
  assumes "(\<And>i j. i < dim\<^sub>c A \<Longrightarrow> j < dim\<^sub>c A \<Longrightarrow> (col A i \<bullet>c col A j = 0) = (i \<noteq> j))"
  shows "corthogonal_mat A"
proof -
  { fix i j assume i: "i < dim\<^sub>c A" and j: "j < dim\<^sub>c A" and ij: "i \<noteq> j"
    have "conjugate\<^sub>v (col A i) \<bullet> col A j = 0"
      by (metis assms col_dim i j ij vec_conjugate_sprod_comm)
  }
  moreover
  { fix i assume "i < dim\<^sub>c A"
    hence "conjugate\<^sub>v (col A i) \<bullet> col A i \<noteq> 0"
      by (metis assms scalar_prod_comm vec_conjugate_closed vec_elemsI)
  }
  ultimately show ?thesis
...
lemma corthogonal_inv_result:
  ...
  shows "corthogonal_inv A inverts\<^sub>m A"
proof -
...
definition basis_completion :: "'a::ring_1 vec \<Rightarrow> 'a vec list" where
  "basis_completion v \<equiv> let 
     ...
     vs = [vec_unit n i. i <- [0..<n], i \<noteq> drop_index] 
   in v # vs"

lemma (in vec_space) basis_completion: fixes v :: "'a :: field vec"
  assumes v: "v \<in> carrier\<^sub>v n"
      and v0: "v \<noteq> \<zero>\<^sub>v n"
  shows 
    "basis (set (basis_completion v))"
    "set (basis_completion v) \<subseteq> carrier\<^sub>v n"
    "span (set (basis_completion v)) = carrier\<^sub>v n" 
...
proof -
  ...
  from v have dim: "dim\<^sub>v v = n" by auto
  let ?is = "[ i . i <- [0..<n], v $ i \<noteq> 0]"
  {
    assume empty: "set ?is = {}"
    have "v = \<zero>\<^sub>v n"
      by (rule vec_eqI, insert empty v, auto)
  }
    ...
    by (intro nth_equalityI, insert k, auto simp: nth_append) 
  {
    ...
    hence "[unit\<^sub>v n i. i <- as, i \<noteq> k] = [unit\<^sub>v n i. i <- as]"
      by (induct as, auto)
  } note conv = this
  have b_all: "?b = v # [unit\<^sub>v n i. i <- [0..<n], i \<noteq> k]"
    unfolding d dim id by simp 
  also have "[unit\<^sub>v n i. i <- [0..<n], i \<noteq> k] = [unit\<^sub>v n i. i <- [0..<k]] @ [unit\<^sub>v n i. i <- [Suc k..<n]]"
    unfolding split by (auto simp: conv)
  finally have b: "?b = v # [unit\<^sub>v n i. i <- [0..<k]] @ [unit\<^sub>v n i. i <- [Suc k..<n]]" by simp
  show carr: "set ?b \<subseteq> carrier\<^sub>v n" (is "?S \<subseteq> _")
  ...
  have I: "\<And> i. I i \<noteq> k" "\<And> i. Suc i < n \<Longrightarrow> I i < n" unfolding I_def by auto
  {
    fix i
    assume i: "i < n"
    have "?b ! i = (if i = 0 then v else unit\<^sub>v n (I (i - 1)))"
  ...
  proof (intro allI impI)
    ...
    proof 
      ...
      proof (cases "i = 0")
        ...
        from True ij have bj: "?b ! j = unit\<^sub>v n (I (j - 1))" "Suc (j - 1) = j" unfolding bi[OF j] by auto
        with id2[of k, unfolded biv bj] vk I[of "j - 1"] k j
        have False by simp
        thus ?thesis ..
      next
        case False note i0 = this
        hence bi': "?b ! i = unit\<^sub>v n (I (i - 1))" "Suc (i - 1) = i" unfolding bi[OF i] by auto
        show ?thesis
        proof (cases "j = 0")
          case True
        ...
        next
          ...
          hence bj: "?b ! j = unit\<^sub>v n (I (j - 1))" "Suc (j - 1) = j" unfolding bi[OF j] by auto
          have "1 = ?b ! i $ I (i - 1)" unfolding bi' using I[of "i - 1"] i i0 by auto
          also have "\<dots> = unit\<^sub>v n (I (j - 1)) $ I (i - 1)" unfolding id1 bj by simp
    ...
    qed
  qed
  have "span (set ?b) \<subseteq> carrier\<^sub>v n" using carr by auto
  moreover
  {
    fix w :: "'a vec"
    ...
    have "?w \<in> carrier\<^sub>v n" using carr by auto
    with w have dim: "dim\<^sub>v w = n" "dim\<^sub>v ?w = n" by auto
    have "w = ?w" 
    proof (rule vec_eqI; unfold dim)
      fix i
      assume i: "i < n"
      show "w $ i = ?w $ i" unfolding lincomb_def 
      proof (subst finsum_index[OF i _ carr]) 
        show "(\<lambda>v. a v \<odot>\<^sub>v v) \<in> set ?b \<rightarrow> carrier\<^sub>v n" using carr by auto
        {
          fix x :: "'a vec" and j
          assume "x = unit\<^sub>v n j" "j \<noteq> k" "j < n"
        ...
        proof (cases "i = k")
          ...
          proof (rule setsum.neutral, intro ballI)
            ...
            show "(a x \<odot>\<^sub>v x) $ k = 0" unfolding a[OF x] unfolding x using x k by auto
          qed
          ultimately show ?thesis unfolding True by simp
        next
          case False
          let ?ui = "unit\<^sub>v n i :: 'a vec"
          {
            assume "?ui = v"
            from arg_cong[OF this, of "\<lambda> v. v $ k"] vk i k False have False by auto
          }
          hence diff: "?ui \<noteq> v" by auto
          from a[OF refl False] have ai: "(a ?ui \<odot>\<^sub>v ?ui) $ i = w $ i - w $ k / v $ k * v $ i" 
            using i by auto          
  ...
  ultimately show span: "span (set ?b) = carrier\<^sub>v n" by blast
  show "basis (set ?b)"
...
lemma orthogonal_mat_of_cols:
  assumes W: "set ws \<subseteq> carrier\<^sub>v n"
    and orth: "corthogonal ws"
    and len: "length ws = n"
  shows "corthogonal_mat (mat_of_cols n ws)" (is "corthogonal_mat ?W")
proof
    fix i j assume i: "i < dim\<^sub>c ?W" and j: "j < dim\<^sub>c ?W"
    hence [simp]: "ws ! i : carrier\<^sub>v n" "ws ! j : carrier\<^sub>v n"
      using W len by auto
...
lemma corthogonal_col_ev_0: fixes A :: "'a :: conjugatable_ordered_field mat"
  ...
  and v0: "v \<noteq> \<zero>\<^sub>v n"
  and eigen[simp]: "A \<otimes>\<^sub>m\<^sub>v v = e \<odot>\<^sub>v v"
  and n: "n \<noteq> 0"
  and hdws: "hd ws = v"
  and ws: "set ws \<subseteq> carrier\<^sub>v n" "corthogonal ws" "length ws = n"
  defines "W == mat_of_cols n ws"
  defines "W' == corthogonal_inv W"
  defines "A' == W' \<otimes>\<^sub>m A \<otimes>\<^sub>m W"
  shows "col A' 0 = vec n (\<lambda> i. if i = 0 then e else 0)"
proof -
  let ?f = "(\<lambda> i. if i = 0 then e else 0)"
  from ws have W: "W \<in> carrier\<^sub>m n n" unfolding W_def by auto
  from W have W': "W' \<in> carrier\<^sub>m n n" unfolding W'_def 
    corthogonal_inv_def mat_of_rows_def by auto
  from A W W' have A': "A' \<in> carrier\<^sub>m n n" unfolding A'_def by auto
  show "col A' 0 = vec n ?f"
  proof (rule,unfold vec_dim_vec)
    show dim: "dim\<^sub>v (col A' 0) = n" using A' by simp
    have row0: "vec_inv v \<bullet> (A \<otimes>\<^sub>m\<^sub>v v) = e"
      using scalar_prod_scalar_assoc[OF vec_inv_closed[OF v] v]
      using vec_inv[OF v v0] by auto
    fix i assume i: "i < n"
    hence i2: "i < length ws" using ws by auto
    let ?wsi = "ws ! i"
    have z: "0 < dim\<^sub>c A'" using A' n by auto
    hence z2: "0 < length ws" using A' ws by auto
    have wsi[simp]: "ws!i : carrier\<^sub>v n" using ws i by auto
    hence ws0[simp]: "ws!0 = v" using hd_conv_nth[symmetric] hdws z2 by auto
    have "col A' 0 $ i = A' $$ (i, 0)" using A' i by auto
    also have "... = (W' \<otimes>\<^sub>m (A \<otimes>\<^sub>m W)) $$ (i, 0)" unfolding A'_def using W' A W by auto
    also have "... = row W' i \<bullet> col (A \<otimes>\<^sub>m W) 0"
      apply (subst mat_index_mat_mult_mat) using W W' A i by auto
    also have "row W' i = vec_inv ?wsi"
      unfolding W'_def W_def unfolding corthogonal_inv_def using i ws by auto
    also have "col (A \<otimes>\<^sub>m W) 0 = A \<otimes>\<^sub>m\<^sub>v col W 0" using A W z A' by auto
    also have "col W 0 = v" unfolding W_def using z2 ws0 n mat_of_cols_col v by blast
    also have "A \<otimes>\<^sub>m\<^sub>v v = e \<odot>\<^sub>v v" using eigen.
    also have "vec_inv ?wsi \<bullet> (e \<odot>\<^sub>v v) = e * (vec_inv ?wsi \<bullet> v)"
      using scalar_prod_scalar_assoc[OF vec_inv_closed[OF wsi] v].
    also have "... = ?f i"
    proof(cases "i = 0")
    ...
    next 
      ...
      have "vec_inv ?wsi \<bullet> v = 1 / (?wsi \<bullet>c ?wsi) * (conjugate\<^sub>v ?wsi \<bullet> v)"
        unfolding vec_inv_def unfolding scalar_scalar_prod_assoc[OF cwsi v].. 
      also have "conjugate\<^sub>v ?wsi \<bullet> v = v \<bullet>c ?wsi"
        using scalar_prod_comm[OF cwsi v].
      also have "... = 0"
...
fun schur_decomposition :: "'a::conjugatable_field mat \<Rightarrow> 'a list \<Rightarrow> 'a mat \<times> 'a mat \<times> 'a mat" where 
  "schur_decomposition A [] = (A, \<one>\<^sub>m (dim\<^sub>r A), \<one>\<^sub>m (dim\<^sub>r A))"
| "schur_decomposition A (e # es) = (let
       ...
       W = mat_of_cols n ws;
       W' = corthogonal_inv W;
       A' = W' \<otimes>\<^sub>m A \<otimes>\<^sub>m W;
       (A1,A2,A0,A3) = split_block A' 1 1;
       (B,P,Q) = schur_decomposition A3 es;
       z_row = (\<zero>\<^sub>m 1 n1);
       z_col = (\<zero>\<^sub>m n1 1);
       one_1 = \<one>\<^sub>m 1
     in (four_block_mat A1 (A2 \<otimes>\<^sub>m P) A0 B, 
     W \<otimes>\<^sub>m four_block_mat one_1 z_row z_col P, 
     four_block_mat one_1 z_row z_col Q \<otimes>\<^sub>m W'))"


theorem schur_decomposition:
  assumes A: "(A::'a::conjugatable_ordered_field mat) \<in> carrier\<^sub>m n n"
      and c: "char_poly A = (\<Prod> (e :: 'a) \<leftarrow> es. [:- e, 1:])"
      and B: "schur_decomposition A es = (B,P,Q)"
  shows "mat_similar_wit A B P Q \<and> upper_triangular B \<and> mat_diag B = es"
  using assms
proof (induct es arbitrary: n A B P Q)
  case Nil
  with degree_monic_char_poly[of A n]
  show ?case by (auto intro: mat_similar_wit_refl simp: mat_diag_def)
next
  case (Cons e es n A C P Q)
  let ?n1 = "n - 1"
  from Cons have A: "A \<in> carrier\<^sub>m n n" and dim: "dim\<^sub>r A = n" by auto
  let ?cp = "char_poly A"
  from Cons(3)
  ...
  let ?P' = "four_block_mat (\<one>\<^sub>m 1) (\<zero>\<^sub>m 1 ?n1) (\<zero>\<^sub>m ?n1 1) P'"
  let ?Q' = "four_block_mat (\<one>\<^sub>m 1) (\<zero>\<^sub>m 1 ?n1) (\<zero>\<^sub>m ?n1 1) Q'"
  have C: "C = four_block_mat A1 (A2 \<otimes>\<^sub>m P') A0 B" and P: "P = W \<otimes>\<^sub>m ?P'" and Q: "Q = ?Q' \<otimes>\<^sub>m W'"
  ...
  have v[simp]: "v \<in> carrier\<^sub>v n" and v0: "v \<noteq> \<zero>\<^sub>v n" using A by auto
  interpret cof_vec_space n "TYPE('a)" .
  from basis_completion[OF v v0, folded b_def]
  have span_b: "span (set b) = carrier\<^sub>v n" and dist_b: "distinct b" 
    and indep: "\<not> lin_dep (set b)" and b: "set b \<subseteq> carrier\<^sub>v n" and hdb: "hd b = v" 
  ...
  have ws: "set ws \<subseteq> carrier\<^sub>v n" "corthogonal ws" "length ws = n" 
    by (auto simp: len_b)
  ...
  have W: "W \<in> carrier\<^sub>m n n"
    using ws unfolding W_def using mat_of_cols_dim(1)[of n ws] by auto
  have W': "W' \<in> carrier\<^sub>m n n" unfolding W'_def corthogonal_inv_def using W 
    by (auto simp: mat_of_rows_def)  
  from corthogonal_inv_result[OF orth_W] 
  have W'W: "W' inverts\<^sub>m W" unfolding W'_def .
  hence WW': "W inverts\<^sub>m W'" using mat_mult_left_right_inverse[OF W' W] W' W
    unfolding mat_inverts_def by auto
  have A': "A' \<in> carrier\<^sub>m n n" using W W' A unfolding A'_def by auto
  have A'A_wit: "mat_similar_wit A' A W' W"
    by (rule mat_similar_witI[of _ _ n], insert W W' A A' W'W WW', auto simp: A'_def
    mat_inverts_def)
  hence A'A: "mat_similar A' A" unfolding mat_similar_def by blast
  from mat_similar_wit_sym[OF A'A_wit] have simAA': "mat_similar_wit A A' W W'" by auto
  have eigen[simp]: "A \<otimes>\<^sub>m\<^sub>v v = e \<odot>\<^sub>v v" and v0: "v \<noteq> \<zero>\<^sub>v n"
  ...
  from A' n have "dim\<^sub>r A' = 1 + ?n1" "dim\<^sub>c A' = 1 + ?n1" by auto
  from split_block[OF splitA' this] have A2: "A2 \<in> carrier\<^sub>m 1 ?n1"
    and A3: "A3 \<in> carrier\<^sub>m ?n1 ?n1" 
  ...
  have A1: "A1 \<in> carrier\<^sub>m 1 1" unfolding A1id by auto
  {
    fix i
    assume "i < ?n1"
  ...
  have A0id: "A0 = \<zero>\<^sub>m ?n1 1"
    using splitA'[unfolded split_block_def Let_def] A'0 A' by auto
  have A0: "A0 \<in> carrier\<^sub>m ?n1 1" unfolding A0id by auto
  ...
  also have "char_poly A1 = [: -e,1 :]" unfolding A1id char_poly_defs det_def
    by (simp add: signof_def sign_def one_poly_def)
  ...
  have simIH: "mat_similar_wit A3 B P' Q'" and ut: "upper_triangular B" and diag: "mat_diag B = es" by auto
  from mat_similar_witD2[OF A3 simIH] 
  have B: "B \<in> carrier\<^sub>m ?n1 ?n1" and P': "P' \<in> carrier\<^sub>m ?n1 ?n1" and Q': "Q' \<in> carrier\<^sub>m ?n1 ?n1" 
    and PQ': "P' \<otimes>\<^sub>m Q' = \<one>\<^sub>m ?n1" by auto
  have A0_eq: "A0 = P' \<otimes>\<^sub>m A0 \<otimes>\<^sub>m \<one>\<^sub>m 1" unfolding A0id using P' by auto
  have simA'C: "mat_similar_wit A' C ?P' ?Q'" unfolding A'block C
    by (rule mat_similar_wit_four_block[OF mat_similar_wit_refl[OF A1] simIH _ A0_eq A1 A3 A0],
  ...
  from A1id have diagA1: "mat_diag A1 = [e]" unfolding mat_diag_def by auto
  from four_block_diag[OF A1 B] have diag: "mat_diag C = e # es" unfolding diag diagA1 C by simp
  from ut mat_similar_wit_trans[OF simAA' simA'C, folded P Q] diag
  show ?case by blast
qed
...
lemma schur_upper_triangular:
  ...
  shows "B \<in> carrier\<^sub>m n n" "upper_triangular B" "mat_similar A B" 
proof -
  ...
  have sim: "mat_similar_wit A B P Q" and B: "upper_triangular B" unfolding B by auto
  from sim show "mat_similar A B" unfolding mat_similar_def by auto
  from mat_similar_witD2[OF A sim] show "B \<in> carrier\<^sub>m n n" by auto
  show "upper_triangular B" by fact
qed

lemma schur_decomposition_exists: assumes A: "A \<in> carrier\<^sub>m n n"
  and linear: "char_poly A = (\<Prod> (a :: 'a :: conjugatable_ordered_field) \<leftarrow> es. [:- a, 1:])"
  shows "\<exists> B \<in> carrier\<^sub>m n n. upper_triangular B \<and> mat_similar A B" 
  using schur_upper_triangular[OF A linear] by blast

lemma char_poly_0_block: fixes A :: "'a :: conjugatable_ordered_field mat"
  assumes A: "A = four_block_mat B C (\<zero>\<^sub>m m n) D"
...
proof -
  ...
  from schur_decomposition[OF B cB sB] mat_similar_witD2[OF B, of B'] have 
    simB: "mat_similar B B'" and utB: "upper_triangular B'" and diagB: "mat_diag B' = bs"
    and B': "B' \<in> carrier\<^sub>m n n"
    by (auto simp: mat_similar_def)
  from schur_decomposition[OF D cD sD] mat_similar_witD2[OF D, of D'] have 
    simD: "mat_similar D D'" and utD: "upper_triangular D'" and diagD: "mat_diag D' = ds"
    and D': "D' \<in> carrier\<^sub>m m m"
    by (auto simp: mat_similar_def)
  let ?z = "\<zero>\<^sub>m m n"
  from mat_similar_four_block_0_ex[OF simB simD C B D, folded A]
    obtain B0 where B0: "B0 \<in> carrier\<^sub>m n m" and sim: "mat_similar A (four_block_mat B' B0 ?z D')" 
    by auto
  let ?block = "four_block_mat B' B0 ?z D'"
  let ?cp = char_poly
  let ?prod = "QB \<otimes>\<^sub>m C \<otimes>\<^sub>m PD"
  let ?diag = "\<lambda> A. (\<Prod>a\<leftarrow>mat_diag A. [:- a, 1:])"
  from char_poly_similar[OF sim] have "?cp A = ?cp ?block" by simp
  also have "\<dots> = ?diag ?block"
    by (rule char_poly_upper_triangular[OF four_block_mat_carrier[OF B' D'] upper_triangular_four_block[OF B' D' utB utD]])      
  also have "\<dots> = ?diag B' * ?diag D'" unfolding four_block_diag[OF B' D']
    by auto
  also have "?diag B' = ?cp B'"
    by (subst char_poly_upper_triangular[OF B' utB], simp)
  also have "\<dots> = ?cp B"
    by (rule char_poly_similar[OF mat_similar_sym[OF simB]])
  also have "?diag D' = ?cp D'"
    by (subst char_poly_upper_triangular[OF D' utD], simp)
  also have "\<dots> = ?cp D"
    by (rule char_poly_similar[OF mat_similar_sym[OF simD]])
  finally show ?thesis .
qed


lemma char_poly_0_block': fixes A :: "'a :: conjugatable_ordered_field mat"
  assumes A: "A = four_block_mat B (\<zero>\<^sub>m n m) C D"
  and linearB: "\<exists> es. char_poly B = (\<Prod> a \<leftarrow> es. [:- a, 1:])"
  and linearD: "\<exists> es. char_poly D = (\<Prod> a \<leftarrow> es. [:- a, 1:])"
  and B: "B \<in> carrier\<^sub>m n n"
  and C: "C \<in> carrier\<^sub>m m n"
  and D: "D \<in> carrier\<^sub>m m m"
  shows "char_poly A = char_poly B * char_poly D"
proof -
  let ?A = "four_block_mat B (\<zero>\<^sub>m n m) C D"
  let ?B = "transpose\<^sub>m B"
  let ?D = "transpose\<^sub>m D"
  have AC: "?A \<in> carrier\<^sub>m (n + m) (n + m)" using B D by auto
  from arg_cong[OF four_block_mat_transpose[OF B mat_zero_closed C D], of char_poly,
    unfolded char_poly_mat_transpose[OF AC], folded A]
  have "char_poly A =
    char_poly (four_block_mat ?B (transpose\<^sub>m C) (\<zero>\<^sub>m m n) ?D)" by auto
