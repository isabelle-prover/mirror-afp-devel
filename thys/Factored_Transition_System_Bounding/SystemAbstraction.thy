...
proof (induction as arbitrary: s vs)
  ...
  next
    ...
    then have "(fmdom' (snd a) \<inter> vs = {})" 
      using False drestrict_empty_iff_disj[of vs "snd a"]
      by argo
...
proof -
  {
    fix x
    have "
      (x \<in> fmdom' (fmrestrict_set vs (fst a)) \<or> x \<in> fmdom' (fmrestrict_set vs (snd a)))
      \<longleftrightarrow> (x \<in> action_dom (fst a) (snd a) \<and> x \<in> vs)
    "
      unfolding action_dom_def 
      using Un_iff contra_subsetD fmdom'_restrict_set x_in_fdom_eq_in_fdom_drestrict
      by metis 
  }
  then show ?thesis
    by (simp add: 
        Int_commute action_dom_def action_proj_def fmdom'.rep_eq fmlookup_fmrestrict_set_dom 
        inf_sup_distrib1) 
qed
...
proof -
  {
    have 1: "fmrestrict_set vs (fst h) \<subseteq>\<^sub>f s" 
      using assms(1) FmapUtils.fmrestrict_subset submap_imp_state_succ_submap_a 
      by blast
    then have "
...
proof -
  ...
  then have "fmlookup (fmrestrict_set vs (snd a)) \<subseteq>\<^sub>m fmlookup (snd a)"
    using FmapUtils.fmrestrict_subset fmsubset.rep_eq 
    by blast 
...
proof (induction as arbitrary: PROB vs)
  ...
  then have "fmdom' (fmrestrict_set vs (snd a)) \<noteq> {}"
    by (metis 
        FDOM_eff_subset_prob_dom_pair dual_order.trans drestrict_empty_iff_disj inf.orderE 
  ...
  next
    ...
    proof (cases "fmdom' (fmrestrict_set vs (snd a)) \<noteq> {}")
      ...
      then have "fmdom' (fmrestrict_set vs (snd a)) = {}" 
        using drestrict_empty_iff_disj
        by metis 
      then show ?thesis
        using true2 
        by blast 
...
lemma PROJ_DOM_PRE_EFF_SUBSET_DOM: 
  ...
  "
  unfolding action_proj_def less_eq_set_def
  by (metis (no_types, lifting) 
      fmdom'_restrict_set fst_conv x_in_fdom_eq_in_fdom_drestrict predicate1I set_mp snd_conv) 
...
lemma dep_proj_dep:
  ...
  apply auto
  by (metis FmapUtils.fmrestrict_subset as_needed_asses_submap_exec_v)+
...
lemma agree_imp_submap: 
  ...
  using assms 
  unfolding agree_def
  by (simp add: fmdom'.rep_eq fmsubset.rep_eq map_le_def)
...
lemma agree_DRESTRICT: 
  ...
  unfolding agree_def fmrestrict_set_def fmdom'_fmrestrict_subset
  using assms
  by (metis agree_def fmlookup_restrict_set fmrestrict_set_def x_in_fdom_eq_in_fdom_drestrict) 
...
lemma agree_DRESTRICT_2:
  ...
  unfolding agree_def
  using assms 
  by (metis agree_def fmdom'_notI fmlookup_restrict_set x_in_fdom_eq_in_fdom_drestrict)
    ...
    moreover {
      ...
      next 
        ...
        next
          ...
          thm submap_drest_submap
          then show ?thesis
            unfolding state_succ_def fmap_add_ltr_def
            using a False submap_drest_submap 
