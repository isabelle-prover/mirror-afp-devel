...
\<comment> \<open>NOTE added lemma.\<close>
lemma fmdom'_fmrestrict_subset: 
...
proof -
    ...
    by simp
  {
    ...
    then have "fmdom' (fmrestrict_set X s) = X" 
      using 1 fmdom'_fmrestrict_subset
      by metis
...
lemma valid_states_nempty: 
  ...
  unfolding valid_states_def 
  using assms finite_dom_always_has_mapping FINITE_prob_dom
  by auto
...
proof -
  ...
  then have "finite domain"
    using 1 assms(1, 2) finite_fmdom'
    by fast 
...
next 
  ...
  have 1: "fmdom' ?f = ?vs1" "fmdom' ?g = ?vs2" 
    ...
    by (metis (mono_tags, lifting) fmdom'_restrict_set x_in_fdom_eq_in_fdom_drestrict mem_Collect_eq 
...
proof -
  ...
  then have "fmlookup f \<subseteq>\<^sub>m fmlookup g"
    by (simp add: fmdom'.rep_eq map_le_def) 
  then show ?thesis
...
proof -
  ...
  let ?B="{v \<in> fmdom' s. v \<in> fmdom' (fst a) \<and> fmlookup (fst a) v = fmlookup s v}"
  {
    ...
    then have "v \<in> fmdom' s"
      by (meson fmdom'_notI fmlookup_restrict_set x_in_fdom_eq_in_fdom_drestrict)
    moreover have "fmlookup (fst a) v = fmlookup s v"
    ...
    by blast
  {
    ...
    moreover {
      ...
      ultimately have "v \<in> fmdom' (fmrestrict_set (system_needed_vars PROB s) s)"
        by (meson action_needed_vars_subset_sys_needed_vars_subset contra_subsetD 
            x_in_fdom_eq_in_fdom_drestrict ii(1))
    }
    note iii = this
