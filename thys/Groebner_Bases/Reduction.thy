...
proof -
  ...
  from assms(2) have "lookup (p + r) (t \<oplus> lt f) = 0" by (simp add: in_keys_iff)
...
proof -
  ...
  with assms(2) have cr_0: "lookup r (t \<oplus> lt f) = 0"
    by (meson in_keys_plusI2 not_in_keys_iff_lookup_eq_zero)
  from \<open>f \<noteq> 0\<close> show ?thesis unfolding red_single_def
  proof (intro conjI)
...
proof -
  ...
  from assms have "f \<noteq> 0" and "lookup p ?t \<noteq> 0"
    ...
    by (simp_all add: red_single_def)
  from assms(2) have cpr: "lookup (p + r) ?t \<noteq> 0" by blast
  from assms(3) have cqr: "lookup (q + r) ?t \<noteq> 0" by blast
...
proof -
  ...
  proof (cases "lookup p ?s = 0")
    case True
    with diff have "?s \<in> keys q"
      by (simp add: lookup_not_eq_zero_eq_in_keys)
    moreover have "lookup (p - q) ?s = - lookup q ?s" by (simp add: lookup_minus True)
    ultimately have ?B using \<open>f \<noteq> 0\<close> 
      by (auto simp add: red_single_def r monom_mult_uminus_left)
    thus ?thesis by simp
  next
    case False
      hence "?s \<in> keys p"
        by (meson not_in_keys_iff_lookup_eq_zero)
    show ?thesis
    proof (cases "lookup q ?s = 0")
      case True
      hence "lookup (p - q) ?s = lookup p ?s" by (simp add: lookup_minus)
      hence ?A by (simp add: False \<open>f \<noteq> 0\<close> r red_single_def)
      thus ?thesis ..
    next
      case False
      hence "?s \<in> keys q"
      ...
      proof (intro exI conjI)
        from \<open>f \<noteq> 0\<close> \<open>?s \<in> keys p\<close> show "red_single p ?p f t" 
          using red_single_def by blast
      next
        from \<open>f \<noteq> 0\<close> \<open>?s \<in> keys q\<close> show "red_single q ?q f t"
          using red_single_def by blast
      next
...
proof -
  ...
  proof (rule red_single_plus_2)
    ...
    then have "t \<oplus> lt f \<notin> keys q \<union> keys r"
      by (metis "*" IntI UnE assms(2) empty_iff lookup_not_eq_zero_eq_in_keys red_single_def)
...
next
  ...
  proof (rule red_plus_keys_disjoint)
    ...
    proof (rule ccontr)
      ...
      then have "v \<preceq>\<^sub>t lt y" and "y \<noteq> 0" by (auto simp add: lt_max_keys)
...
next
  ...
  next
    ...
    proof (rule red_rtrancl_plus_higher)
      ...
      from this(1) have "punit.tail p \<noteq> 0" and "s \<preceq> punit.lt (punit.tail p)"
        using \<open>s \<in> keys (punit.tail p)\<close> punit.lt_max_keys by auto
...
proof -
  from assms obtain f u where "f \<in> F" and "u \<in> keys (p + q)" and "f \<noteq> 0"
    and a: "lt f adds\<^sub>t u" by (rule is_red_addsE)
  then have "u \<in> keys p \<union> keys q"
    by (meson Poly_Mapping.keys_add subsetD) 
  thus ?thesis
  proof
    assume "u \<in> keys p"
...
proof (rule dgrad_set_leI, simp)
  ...
  moreover from assms(2) have "t \<oplus> lt f \<in> keys p"
    by (simp add: lookup_not_eq_zero_eq_in_keys red_single_def)
...
proof -
  ...
  from assms(2) have "t \<oplus> lt f \<in> keys p" and q: "q = p - ?f" by (simp_all add: lookup_not_eq_zero_eq_in_keys red_single_def)
  ...
  proof (induct p)
    ...
    next
      ...
      have "red_single p q b (lp p)"
        by (simp add: red_single_def \<open>b \<noteq> 0\<close> q_def eq \<open>lt p \<in> keys p\<close> lookup_not_eq_zero_eq_in_keys)
...
proof -
  ...
  have "red_single p (p - ?f) f (lp p - lp f)"
    by (simp add: red_single_def eq lc_def \<open>f \<noteq> 0\<close> lt_in_keys lookup_not_eq_zero_eq_in_keys assms(1))
    ...
    proof (rule 1(1))
      ...
      proof
        ...
        also have "keys (r + monomial (lc p) (lt p)) \<subseteq> keys r \<union> keys (monomial (lc p) (lt p))"
          by (meson Poly_Mapping.keys_add)
        also have "... \<subseteq> insert (lt p) (keys r)" by auto
        finally show False
        proof
          assume "u = lt p"
          from * \<open>f \<in> set fs\<close> show ?thesis
            using \<open>f \<noteq> 0\<close> \<open>lt f adds\<^sub>t u\<close> \<open>u = lt p\<close> find_adds_NoneE by blast
        next
          assume "u \<in> keys r" then show ?thesis
            using "1.prems" \<open>f \<in> set fs\<close> \<open>f \<noteq> 0\<close> \<open>lt f adds\<^sub>t u\<close> is_red_addsI by blast
        qed
      qed
    qed
  next
