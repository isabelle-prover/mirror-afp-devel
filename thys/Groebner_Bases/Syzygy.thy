...
definition rep :: "('m \<Rightarrow>\<^sub>0 'r) \<Rightarrow> 'm"
  where "rep r = (\<Sum>v\<in>keys r. lookup r v \<odot> v)"

lemma rep_monomial [simp]: "rep (monomial c x) = c \<odot> x"
proof -
  have sub: "keys (monomial c x) \<subseteq> {x}" by simp
  have "rep (monomial c x) = (\<Sum>v\<in>{x}. lookup (monomial c x) v \<odot> v)" unfolding rep_def
    by (rule sum.mono_neutral_left, simp, fact sub, simp)
  also have "... = c \<odot> x" by simp
  finally show ?thesis .
qed
...
proof -
  ...
  from fin have eq1: "(\<Sum>v\<in>keys r \<union> keys s. lookup r v \<odot> v) = (\<Sum>v\<in>keys r. lookup r v \<odot> v)"
  proof (rule sum.mono_neutral_right)
    show "\<forall>v\<in>keys r \<union> keys s - keys r. lookup r v \<odot> v = 0" by simp
  qed simp
  from fin have eq2: "(\<Sum>v\<in>keys r \<union> keys s. lookup s v \<odot> v) = (\<Sum>v\<in>keys s. lookup s v \<odot> v)"
  proof (rule sum.mono_neutral_right)
    show "\<forall>v\<in>keys r \<union> keys s - keys s. lookup s v \<odot> v = 0" by simp
  qed simp
  have "rep (r + s) = (\<Sum>v\<in>keys (r + s). lookup (r + s) v \<odot> v)" by (simp only: rep_def)
  also from fin keys_add_subset have "... = (\<Sum>v\<in>keys r \<union> keys s. lookup (r + s) v \<odot> v)"
  proof (rule sum.mono_neutral_left)
    show "\<forall>i\<in>keys r \<union> keys s - keys (r + s). lookup (r + s) i \<odot> i = 0" by simp
  qed
  also have "... = (\<Sum>v\<in>keys r \<union> keys s. lookup r v \<odot> v) + (\<Sum>v\<in>keys r \<union> keys s. lookup s v \<odot> v)"
    by (simp add: lookup_add smult_distrib_right sum.distrib)
...
proof -
  ...
  qed simp
  have "rep (r - s) = (\<Sum>v\<in>keys (r - s). lookup (r - s) v \<odot> v)" by (simp only: rep_def)
  also from fin keys_minus have "... = (\<Sum>v\<in>keys r \<union> keys s. lookup (r - s) v \<odot> v)"
  proof (rule sum.mono_neutral_left)
    show "\<forall>i\<in>keys r \<union> keys s - keys (r - s). lookup (r - s) i \<odot> i = 0" by simp
  qed
  also have "... = (\<Sum>v\<in>keys r \<union> keys s. lookup r v \<odot> v) - (\<Sum>v\<in>keys r \<union> keys s. lookup s v \<odot> v)"
    by (simp add: lookup_minus smult_left_diff_distrib sum_subtractf)
  also have "... = rep r - rep s" by (simp only: eq1 eq2 rep_def)
  finally show ?thesis .
qed

lemma rep_smult: "rep (monomial c 0 * r) = c \<odot> rep r"
proof -
  ...
  have "rep (monomial c 0 * r) = (\<Sum>v\<in>keys (monomial c 0 * r). lookup (monomial c 0 * r) v \<odot> v)"
    by (simp only: rep_def)
  also from finite_keys sub have "... = (\<Sum>v\<in>keys r. lookup (monomial c 0 * r) v \<odot> v)"
  proof (rule sum.mono_neutral_left)
    show "\<forall>v\<in>keys r - keys (monomial c 0 * r). lookup (monomial c 0 * r) v \<odot> v = 0" by simp
  qed
  also have "... = c \<odot> (\<Sum>v\<in>keys r. lookup r v \<odot> v)"
    by (simp add: l smult_assoc sum_smult_distrib_left)
  also have "... = c \<odot> rep r" by (simp add: rep_def)
  finally show ?thesis .
qed

lemma rep_in_module: "rep r \<in> module (keys r)"
  unfolding rep_def by (fact sum_in_moduleI)

lemma in_moduleE_rep:
  assumes "x \<in> module B"
  obtains r where "keys r \<subseteq> B" and "x = rep r"
proof -
  from assms obtain A q where "finite A" and "A \<subseteq> B" and x: "x = (\<Sum>a\<in>A. q a \<odot> a)" by (rule in_moduleE)
...
definition represents :: "'m set \<Rightarrow> ('m \<Rightarrow>\<^sub>0 'r) \<Rightarrow> 'm \<Rightarrow> bool"
  where "represents B r x \<longleftrightarrow> (keys r \<subseteq> B \<and> rep r = x)"
...
proof -
  have sub: "keys (monomial (1::'r) x) \<subseteq> {x}" by simp
  moreover have "rep (monomial (1::'r) x) = x" by simp
...
lemma represents_smult:
  assumes "represents B r x"
  shows "represents B (monomial c 0 * r) (c \<odot> x)"
proof -
  from assms have r: "keys r \<subseteq> B" and x: "x = rep r" by (rule representsD1, rule representsD2)
  show ?thesis
  proof (rule representsI)
...
lemma represents_in_module:
  assumes "represents B r x"
  shows "x \<in> module B"
proof -
  from assms have r: "keys r \<subseteq> B" and x: "x = rep r" by (rule representsD1, rule representsD2)
  have "x \<in> module (keys r)" unfolding x by (fact rep_in_module)
  also from r have "... \<subseteq> module B" by (rule module_mono)
  finally show ?thesis .
qed

definition syzygy_module :: "'m set \<Rightarrow> ('m \<Rightarrow>\<^sub>0 'r) set"
  where "syzygy_module B = {s. represents B s 0}"
...
proof -
  ...
  hence "represents B (monomial c 0 * s) (c \<odot> 0)" by (rule represents_smult)
