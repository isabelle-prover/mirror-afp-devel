  ...
  proof (rule poly_mapping_eqI, simp only: lookup_minus)
    ...
    proof (cases "v = ?lv")
      ...
      from \<open>lt p \<in> keys p\<close> have "lookup (?a p) v = 1"
        by (simp add: v_1 lookup_monom_mult lc_def lookup_not_eq_zero_eq_in_keys term_simps)
      also from \<open>lt q \<in> keys q\<close> have "... = lookup (?b q) v"
        by (simp add: v_2 lookup_monom_mult lc_def lookup_not_eq_zero_eq_in_keys term_simps)
