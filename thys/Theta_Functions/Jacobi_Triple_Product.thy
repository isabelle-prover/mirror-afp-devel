...
corollary has_sum_euler_phi_complex:
  fixes q :: complex
  assumes q: "norm q < 1"
  shows "((\<lambda>k. (-1) powi k * q powi (k * (3*k-1) div 2)) has_sum euler_phi q) UNIV"
proof (cases "q = 0")
  case [simp]: True
...
proof -
  ...
  next
    ...
    have prod: "(\<lambda>n. 1 - q ^ Suc n) has_prod (q; q)\<^sub>\<infinity>"
      using has_prod_qpochhammer_inf[of q q] q by simp
    have sum: "((\<lambda>n. (-1) powi n * q powi (n*(3*n-1) div 2)) has_sum euler_phi q) UNIV"
