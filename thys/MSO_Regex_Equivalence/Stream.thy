...
theory Stream
imports "~~/src/HOL/Library/Nat_Bijection" BNF
begin
...
proof (rule stream.dtor_set_induct)
  ...
  assume "a \<in>  Basic_BNFs.fsts (stream_dtor s)"
  then have "a = shd s"
    by (cases "stream_dtor s")
      (auto simp: shd_def fsts_def stream.dtor_ctor SCons_def
        split: stream.splits)
  with Base show "P a s" by simp
next
  fix a :: 'a and s' :: "'a stream"  and s :: "'a stream"
  assume "s' \<in>  Basic_BNFs.snds (stream_dtor s)" and prems: "a \<in> sset s'" "P a s'"
  then have "s' = stl s"
    by (cases "stream_dtor s")
      (auto simp: stl_def snds_def stream.dtor_ctor SCons_def
        split: stream.splits)
