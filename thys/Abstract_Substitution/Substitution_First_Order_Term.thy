...
next
  fix t and \<gamma> :: "('f, 'v) subst"
  assume "is_ground (t \<cdot> \<gamma>)"

  then show "\<forall>x\<in>vars_term t. is_ground (\<gamma> x)"
    by (meson ground_substD ground_vars_term_empty)
qed auto

lemma term_is_renaming_iff: "term.is_renaming \<rho> \<longleftrightarrow> inj \<rho> \<and> (\<forall>x. \<exists>x'. \<rho> x = Var x')"
  unfolding term.is_renaming_def
  by (metis eval_subst_def ex_inverse_of_renaming injI is_Var_def subst_apply_eq_Var term.inject(1))
...
next
  ...
  show "is_ground t \<longleftrightarrow> (\<forall>\<sigma>. t \<cdot> \<sigma> = t)"
  proof(induction t)
    case Var
    then show ?case
      by auto
...
next
  ...
  assume "finite ts" "\<not>is_ground t"
...
next
  ...
  then show "{t :: ('f, 'v) term. is_ground t} = range term_of_gterm"
    by fastforce
next
  fix t\<^sub>G :: "'f gterm"
...
next
  ...
  then show "inj \<rho> \<and> (\<forall>x. \<exists>x'. \<rho> x = Var x')"
    unfolding term_is_renaming_iff .
  ...
  proof (induction t)
    ...
    have "\<rho> x = Var (term.rename \<rho> x)"
      using \<rho>
      unfolding term_is_renaming_iff term.rename_def[OF \<rho>] is_Var_def
      by (meson someI_ex)
...
lemmas term_id_subst_rename = term.id_subst_rename[OF exists_nonground_term]
...
lemma Term_is_renaming_if_is_renaming:
  ...
  using assms
  unfolding is_renaming_def term_is_renaming_iff
  by (metis inj_on_subset is_VarI top_greatest)
...
lemma Unifiers_unifiers_Times_iff_is_unifier: 
  "\<mu> \<in> Unifiers.unifiers (X \<times> X) \<longleftrightarrow> term.is_unifier \<mu> X "
  unfolding term.is_unifier_iff unifiers_def
  by simp

lemma Unifiers_unifiers_Union_Times_iff_is_unifier_set:
  "\<mu> \<in> Unifiers.unifiers (\<Union>X\<in>XX. X \<times> X) \<longleftrightarrow> term.is_unifier_set \<mu> XX "
  using Unifiers_unifiers_Times_iff_is_unifier
  unfolding term.is_unifier_set_def unifiers_def
  by fast

lemma Unifiers_is_mgu_Union_Times_iff_is_mgu:
  "Unifiers.is_mgu \<mu> (\<Union>X\<in>XX. X \<times> X) \<longleftrightarrow> term.is_mgu \<mu> XX"
  unfolding term.is_mgu_def is_mgu_def 
  using Unifiers_unifiers_Union_Times_iff_is_unifier_set
  by (metis (mono_tags, lifting))

lemma Unifiers_is_imgu_Union_Times_iff_is_imgu:
  "Unifiers.is_imgu \<mu> (\<Union>X\<in>XX. X \<times> X) \<longleftrightarrow> term.is_imgu \<mu> XX"
...
global_interpretation "term": range_vars_subset_if_is_imgu where
  comp_subst = "(\<circ>\<^sub>s)" and id_subst = Var and subst = "(\<cdot>)" and
  apply_subst = apply_subst and vars = vars_term and is_ground = is_ground
proof unfold_locales
  ...
  then have is_imgu: "Unifiers.is_imgu \<mu> (\<Union>X\<in>XX. X \<times> X)"
    using Unifiers_is_imgu_Union_Times_iff_is_imgu[of _ "XX"]
    by simp
...
lemmas infinite_terms [intro] = term.infinite_exprs[OF exists_nonground_term]

lemma is_renaming_iff_ex_inj_fun_on_vars: "term.is_renaming \<rho> \<longleftrightarrow> (\<exists>f. inj f \<and> \<rho> = Var \<circ> f)"
  unfolding term_is_renaming_iff
  by (metis (no_types, lifting) ext comp_def inj_compose inj_on_Var inj_on_imageI2)
