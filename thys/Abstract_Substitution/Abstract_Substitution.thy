...
locale abstract_substitution_monoid = monoid comp_subst id_subst \<^marker>\<open>contributor \<open>Balazs Toth\<close>\<close>
  for
...
locale abstract_substitution =
  abstract_substitution_monoid comp_subst id_subst +
  comp_subst: right_monoid_action comp_subst id_subst subst +
  abstract_substitution_ops subst id_subst comp_subst is_ground
  for
...
lemma ground_eq_ground_if_unifiable:
  ...
  using assms 
  by (simp add: card_Suc_eq is_unifier_def le_Suc_eq subst_set_def)

corollary ground_eq_ground_if_mgu:
  assumes "is_mgu \<mu> {{t\<^sub>1, t\<^sub>2}}" "is_ground t\<^sub>1" "is_ground t\<^sub>2" 
  shows "t\<^sub>1 = t\<^sub>2"
  using assms ground_eq_ground_if_unifiable
  unfolding is_mgu_def is_unifier_set_insert
  by blast

corollary ground_eq_ground_if_imgu:
  assumes "is_imgu \<mu> {{t\<^sub>1, t\<^sub>2}}" "is_ground t\<^sub>1" "is_ground t\<^sub>2" 
  shows "t\<^sub>1 = t\<^sub>2"
  using assms ground_eq_ground_if_unifiable
