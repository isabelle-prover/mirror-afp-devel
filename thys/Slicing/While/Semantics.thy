...
proof(induct arbitrary:c' rule:Labels.induct)
  ...
  next
    ...
    have "\<exists>l'. c\<^isub>1 \<turnstile> \<langle>c\<^isub>1,s,0\<rangle> \<leadsto> \<langle>c\<^isub>1',s',l'\<rangle> \<and> labels c\<^isub>1 l' c\<^isub>1'" .
...
next
  ...
  have "\<langle>Skip,s\<rangle> \<rightarrow> \<langle>c',s'\<rangle>" .
