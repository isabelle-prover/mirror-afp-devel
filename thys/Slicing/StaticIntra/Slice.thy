...
proof -
  ...
  next
    ...
    with False have "card(obs n (backward_slice S)) = 1"
      by simp
    hence "\<exists>m. obs n (backward_slice S) = {m}" by(fastsimp dest:card_eq_SucD)
    thus ?thesis by simp
  qed
qed
...
proof -
  ...
  with `m \<in> obs n (backward_slice S)` show ?thesis by fastsimp
qed
...
lemma slice_kind_Pred_empty_obs_SOME:
  "\<lbrakk>sourcenode a \<notin> backward_slice S; kind a = (Q)\<^isub>\<surd>; 
  ...
  \<Longrightarrow> slice_kind S a = (\<lambda>s. True)\<^isub>\<surd>"
by(simp add:slice_kind_def)
...
lemma slice_kind_Pred_empty_obs_not_SOME:
  "\<lbrakk>sourcenode a \<notin> backward_slice S; kind a = (Q)\<^isub>\<surd>; 
    ...
    targetnode a \<noteq> (SOME n'. \<exists>a'. sourcenode a = sourcenode a' \<and> valid_edge a' \<and>
                                  targetnode a' = n')\<rbrakk>
  \<Longrightarrow> slice_kind S a = (\<lambda>s. False)\<^isub>\<surd>"
by(simp add:slice_kind_def)


lemma slice_kind_Pred_obs_nearer_SOME:
  assumes "sourcenode a \<notin> backward_slice S" and "kind a = (Q)\<^isub>\<surd>" 
  ...
  shows "slice_kind S a = (\<lambda>s. True)\<^isub>\<surd>"
proof -
...
proof -
  ...
  with `sourcenode a \<notin> backward_slice S` `kind a = (Q)\<^isub>\<surd>` in_obs the show ?thesis
    by(fastsimp simp:slice_kind_def Let_def)
qed


lemma kind_Predicate_notin_slice_slice_kind_Predicate:
  assumes "kind a = (Q)\<^isub>\<surd>" and "sourcenode a \<notin> backward_slice S"
  obtains Q' where "slice_kind S a = (Q')\<^isub>\<surd>" and "Q' = (\<lambda>s. False) \<or> Q' = (\<lambda>s. True)"
proof(atomize_elim)
  show "\<exists>Q'. slice_kind S a = (Q')\<^isub>\<surd> \<and> (Q' = (\<lambda>s. False) \<or> Q' = (\<lambda>s. True))"
...
proof -
  ...
  proof(cases "sourcenode a \<in> backward_slice S")
    case True
    with `slice_kind S a = (\<lambda>s. True)\<^isub>\<surd>` `kind a = (Q)\<^isub>\<surd>` have "Q = (\<lambda>s. True)"
...
proof(atomize_elim)
  ...
  from `valid_edge a` have ex1:"\<exists>!a'. sourcenode a = sourcenode a' \<and> valid_edge a' \<and>
               targetnode a' = (SOME n'. \<exists>a'. sourcenode a = sourcenode a' \<and> 
                                              valid_edge a' \<and> targetnode a' = n')"
    by(rule only_one_SOME_edge)
  show "\<exists>Q Q'. slice_kind S a = (Q)\<^isub>\<surd> \<and> slice_kind S a' = (Q')\<^isub>\<surd> \<and> 
                (\<forall>s. (Q s \<longrightarrow> \<not> Q' s) \<and> (Q' s \<longrightarrow> \<not> Q s))"
  proof(cases "sourcenode a \<in> backward_slice S")
    case True
    with `kind a = (Q)\<^isub>\<surd>` have "slice_kind S a = (Q)\<^isub>\<surd>"
      by(simp add:slice_kind_def Let_def)
    from True `kind a' = (Q')\<^isub>\<surd>` `sourcenode a = sourcenode a'`
    have "slice_kind S a' = (Q')\<^isub>\<surd>"
      by(simp add:slice_kind_def Let_def)
    with `slice_kind S a = (Q)\<^isub>\<surd>` det show ?thesis by blast
  next
    case False
...
inductive silent_move :: 
  ...
  'node \<Rightarrow> 'state \<Rightarrow> bool" ("_,_ \<turnstile> '(_,_') -_\<rightarrow>\<^isub>\<tau> '(_,_')" [51,50,0,0,50,0,0] 51) 
 
  where silent_moveI:
  "\<lbrakk>pred (f a) s; transfer (f a) s = s'; sourcenode a \<notin> backward_slice S; 
    valid_edge a\<rbrakk>  
  \<Longrightarrow> S,f \<turnstile> (sourcenode a,s) -a\<rightarrow>\<^isub>\<tau> (targetnode a,s')"
...
inductive silent_moves :: 
  ...
  where silent_moves_Nil: "S,f \<turnstile> (n,s) =[]\<Rightarrow>\<^isub>\<tau> (n,s)"

  | silent_moves_Cons:
  "\<lbrakk>S,f \<turnstile> (n,s) -a\<rightarrow>\<^isub>\<tau> (n',s'); S,f \<turnstile> (n',s') =as\<Rightarrow>\<^isub>\<tau> (n'',s'')\<rbrakk> 
  \<Longrightarrow> S,f \<turnstile> (n,s) =a#as\<Rightarrow>\<^isub>\<tau> (n'',s'')"


lemma silent_moves_obs_slice:
  "\<lbrakk>S,f \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s'); nx \<in> obs n' (backward_slice S)\<rbrakk>
...
next
  ...
  from `S,f \<turnstile> (n,s) -a\<rightarrow>\<^isub>\<tau> (n',s')`
...
lemma silent_moves_preds_transfers_path:
  "\<lbrakk>S,f \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s'); valid_node n\<rbrakk> 
...
next
  ...
  from `S,f \<turnstile> (n,s) -a\<rightarrow>\<^isub>\<tau> (n',s')` have "pred (f a) s" and "transfer (f a) s = s'"
...
lemma obs_silent_moves:
  ...
  obtains as where "S,slice_kind S \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s)"
proof(atomize_elim)
  ...
  show "\<exists>as. S,slice_kind S \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s)"
  proof(induct x arbitrary:n s rule:nat.induct)
    ...
    hence "S,slice_kind S \<turnstile> (n,s) =[]\<Rightarrow>\<^isub>\<tau> (n',s)" by(fastsimp intro:silent_moves_Nil)
    thus "\<exists>as. S,slice_kind S \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s)" by blast
  next
    fix x n s 
    assume "distance n n' (Suc x)" and "n' \<in> obs n (backward_slice S)"
      and IH:"\<And>n s. \<lbrakk>distance n n' x; n' \<in> obs n (backward_slice S)\<rbrakk> 
              \<Longrightarrow> \<exists>as. S,slice_kind S \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s)"
    ...
    with disj have "n' \<in> obs (targetnode a) (backward_slice S)" by fastsimp
    from IH[OF `distance (targetnode a) n' x` this,of "transfer (slice_kind S a) s"]
    obtain asx' where 
    moves:"S,slice_kind S \<turnstile> (targetnode a,transfer (slice_kind S a) s) =asx'\<Rightarrow>\<^isub>\<tau> 
    ...
    have "S,slice_kind S \<turnstile> (sourcenode a,s) -a\<rightarrow>\<^isub>\<tau> 
    ...
    have "S,slice_kind S \<turnstile> (n,s) =a#asx'\<Rightarrow>\<^isub>\<tau> (n',s)"
...
inductive observable_moves :: 
  ...
  "\<lbrakk>S,f \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s'); S,f \<turnstile> (n',s') -a\<rightarrow> (n'',s'')\<rbrakk> 
...
lemma silent_move_observable_moves:
  "\<lbrakk>S,f \<turnstile> (n'',s'') =as\<Rightarrow> (n',s'); S,f \<turnstile> (n,s) -a\<rightarrow>\<^isub>\<tau> (n'',s'')\<rbrakk>
...
proof(induct rule:observable_moves.induct)
  ...
  from `S,f \<turnstile> (n,s) -a\<rightarrow>\<^isub>\<tau> (nx,sx)` `S,f \<turnstile> (nx,sx) =as\<Rightarrow>\<^isub>\<tau> (n',s')`
  have "S,f \<turnstile> (n,s) =a#as\<Rightarrow>\<^isub>\<tau> (n',s')" by(rule silent_moves_Cons)
...
proof(induct rule:observable_moves.induct)
  ...
  proof(cases as)
    case Nil
    with `S,f \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s')` have "n = n'" and "s = s'"
      by(auto elim:silent_moves.cases)
    with `S,f \<turnstile> (n',s') -a\<rightarrow> (n'',s'')` show ?thesis
      by(fastsimp elim:observable_move.cases)
  next
    case (Cons a' as')
    with `S,f \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s')` show ?thesis
      by(fastsimp elim:silent_moves.cases silent_move.cases)
  qed
  with `S,f \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s')`
...
proof -
  ...
  proof(cases "sourcenode a \<in> backward_slice S")
    ...
    have "slice_kind S ax = kind ax" by(fastsimp simp:slice_kind_in_slice)
  ...
  next
    case False
    with `kind a = (Q)\<^isub>\<surd>` `slice_kind S a = (P)\<^isub>\<surd>`
    have "P = (\<lambda>s. False) \<or> P = (\<lambda>s. True)"
      by(fastsimp elim:kind_Predicate_notin_slice_slice_kind_Predicate)
    with `slice_kind S a = (P)\<^isub>\<surd>` `preds (slice_kinds S (a#as)) s`
    have "P = (\<lambda>s. True)" by(fastsimp simp:slice_kinds_def)
    from `kind ax = (Q')\<^isub>\<surd>` `slice_kind S ax = (P')\<^isub>\<surd>` 
      `sourcenode a = n` `sourcenode ax = n` False
    have "P' = (\<lambda>s. False) \<or> P' = (\<lambda>s. True)"
      by(fastsimp elim:kind_Predicate_notin_slice_slice_kind_Predicate)
    with `slice_kind S ax = (P')\<^isub>\<surd>` `preds (slice_kinds S (ax#asx)) s'`
    have "P' = (\<lambda>s. True)" by(fastsimp simp:slice_kinds_def)
...
lemma WS_silent_move:
  assumes "((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S" and "S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) -a\<rightarrow>\<^isub>\<tau> (n\<^isub>1',s\<^isub>1')"
  and "obs n\<^isub>1' (backward_slice S) \<noteq> {}" shows "((n\<^isub>1',s\<^isub>1'),(n\<^isub>2,s\<^isub>2)) \<in> WS S"
proof -
  from `((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S` have "valid_node n\<^isub>1" and "valid_node n\<^isub>2"
    by(auto dest:WSD)
  from `S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) -a\<rightarrow>\<^isub>\<tau> (n\<^isub>1',s\<^isub>1')` have "sourcenode a = n\<^isub>1"
    and "targetnode a = n\<^isub>1'" and "transfer (kind a) s\<^isub>1 = s\<^isub>1'"
    and "n\<^isub>1 \<notin> backward_slice S" and "valid_edge a" and "pred (kind a) s\<^isub>1"
    by(auto elim:silent_move.cases)
  from `targetnode a = n\<^isub>1'` `valid_edge a` have "valid_node n\<^isub>1'"
    by(auto simp:valid_node_def)
  have "(\<exists>m. obs n\<^isub>1' (backward_slice S) = {m}) \<or> obs n\<^isub>1' (backward_slice S) = {}"
    by(rule obs_singleton_disj)
  with `obs n\<^isub>1' (backward_slice S) \<noteq> {}` obtain n 
    where "obs n\<^isub>1' (backward_slice S) = {n}" by fastsimp
  hence "n \<in> obs n\<^isub>1' (backward_slice S)" by auto
  ...
  moreover
  from `\<forall>nx \<in> set(sourcenodes as). nx \<notin> (backward_slice S)` `sourcenode a = n\<^isub>1`
    `n\<^isub>1 \<notin> backward_slice S` 
  have "\<forall>nx \<in> set(sourcenodes (a#as)). nx \<notin> (backward_slice S)"
    by(simp add:sourcenodes_def)
  ultimately have "n \<in> obs n\<^isub>1 (backward_slice S)" using `n \<in> (backward_slice S)` 
    by(rule obs_elem)
  hence "obs n\<^isub>1 (backward_slice S) = {n}" by(rule obs_singleton_element)
  with `obs n\<^isub>1' (backward_slice S) = {n}` 
  have "obs n\<^isub>1 (backward_slice S) = obs n\<^isub>1' (backward_slice S)"
    by simp
  with `valid_node n\<^isub>1` `valid_node n\<^isub>1'` have "rv S n\<^isub>1 = rv S n\<^isub>1'"
    by(rule closed_eq_obs_eq_rvs)
  from `n \<in> obs n\<^isub>1 (backward_slice S)` `((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S` 
  have "obs n\<^isub>1 (backward_slice S) = obs n\<^isub>2 (backward_slice S)"
    and "\<forall>V \<in> rv S n\<^isub>1. state_val s\<^isub>1 V = state_val s\<^isub>2 V"
    by(fastsimp dest:WSD)+
  from `obs n\<^isub>1 (backward_slice S) = obs n\<^isub>2 (backward_slice S)`
    `obs n\<^isub>1 (backward_slice S) = {n}` `obs n\<^isub>1' (backward_slice S) = {n}` 
  have "obs n\<^isub>1' (backward_slice S) = obs n\<^isub>2 (backward_slice S)" by simp
  have "\<forall>V \<in> rv S n\<^isub>1'. state_val s\<^isub>1' V = state_val s\<^isub>2 V"
  proof
    fix V assume "V \<in> rv S n\<^isub>1'"
    with `rv S n\<^isub>1 = rv S n\<^isub>1'` have "V \<in> rv S n\<^isub>1" by simp
    then obtain as n' where "n\<^isub>1 -as\<rightarrow>* n'" and "n' \<in> (backward_slice S)"
      and "V \<in> Use n'" and "\<forall>nx \<in> set(sourcenodes as). V \<notin> Def nx"
      by(erule rvE)
    with `n\<^isub>1 \<notin> backward_slice S` have "V \<notin> Def n\<^isub>1"
    ...
    from `V \<in> rv S n\<^isub>1` `\<forall>V \<in> rv S n\<^isub>1. state_val s\<^isub>1 V = state_val s\<^isub>2 V`
  ...
  qed
  with `obs n\<^isub>1' (backward_slice S) = obs n\<^isub>2 (backward_slice S)`
    `valid_node n\<^isub>1'` `valid_node n\<^isub>2` show ?thesis by(fastsimp intro:WSI)
qed


lemma WS_silent_moves:
  "\<lbrakk>S,f \<turnstile> (n\<^isub>1,s\<^isub>1) =as\<Rightarrow>\<^isub>\<tau> (n\<^isub>1',s\<^isub>1'); ((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S; f = kind;
    obs n\<^isub>1' (backward_slice S) \<noteq> {}\<rbrakk>
  \<Longrightarrow> ((n\<^isub>1',s\<^isub>1'),(n\<^isub>2,s\<^isub>2)) \<in> WS S"
proof(induct rule:silent_moves.induct)
  case silent_moves_Nil thus ?case by simp
next
  case (silent_moves_Cons S f n s a n' s' as n'' s'')
  note IH = `\<lbrakk>((n',s'),(n\<^isub>2,s\<^isub>2)) \<in> WS S; f = kind; obs n'' (backward_slice S) \<noteq> {}\<rbrakk>
             \<Longrightarrow> ((n'',s''),(n\<^isub>2,s\<^isub>2)) \<in> WS S`
  from `S,f \<turnstile> (n',s') =as\<Rightarrow>\<^isub>\<tau> (n'',s'')` `obs n'' (backward_slice S) \<noteq> {}`
  have "obs n' (backward_slice S) \<noteq> {}" by(fastsimp dest:silent_moves_obs_slice)
  with `((n,s),(n\<^isub>2,s\<^isub>2)) \<in> WS S` `S,f \<turnstile> (n,s) -a\<rightarrow>\<^isub>\<tau> (n',s')` `f = kind`
  have "((n',s'),(n\<^isub>2,s\<^isub>2)) \<in> WS S" by -(rule WS_silent_move,simp+)
...
lemma WS_observable_move:
  assumes "((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S" and "S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) -a\<rightarrow> (n\<^isub>1',s\<^isub>1')"
  obtains as where "((n\<^isub>1',s\<^isub>1'),(n\<^isub>1',transfer (slice_kind S a) s\<^isub>2)) \<in> WS S"
  and "S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as@[a]\<Rightarrow> (n\<^isub>1',transfer (slice_kind S a) s\<^isub>2)"
proof(atomize_elim)
  from `((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S` have "valid_node n\<^isub>1" by(auto dest:WSD)
  from `S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) -a\<rightarrow> (n\<^isub>1',s\<^isub>1')` have [simp]:"n\<^isub>1 = sourcenode a" 
    and [simp]:"n\<^isub>1' = targetnode a" and "pred (kind a) s\<^isub>1"
    and "transfer (kind a) s\<^isub>1 = s\<^isub>1'" and "n\<^isub>1 \<in> (backward_slice S)" 
    and "valid_edge a" and "pred (kind a) s\<^isub>1"
    by(auto elim:observable_move.cases)
  from  `valid_edge a` have "valid_node n\<^isub>1'" by(auto simp:valid_node_def)
  from `valid_node n\<^isub>1` `n\<^isub>1 \<in> (backward_slice S)` 
  have "obs n\<^isub>1 (backward_slice S) = {n\<^isub>1}" by(rule n_in_obs)
  with `((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S` have "obs n\<^isub>2 (backward_slice S) = {n\<^isub>1}" 
    and "\<forall>V \<in> rv S n\<^isub>1. state_val s\<^isub>1 V = state_val s\<^isub>2 V" by(auto dest:WSD)
  from `valid_node n\<^isub>1` have "n\<^isub>1 -[]\<rightarrow>* n\<^isub>1" by(rule empty_path)
  with `n\<^isub>1 \<in> (backward_slice S)` have "\<forall>V \<in> Use n\<^isub>1. V \<in> rv S n\<^isub>1"
    by(fastsimp intro:rvI simp:sourcenodes_def)
  ...
  with `n\<^isub>1 \<in> (backward_slice S)` have "pred (slice_kind S a) s\<^isub>2"
    by(simp add:slice_kind_in_slice)
  from `n\<^isub>1 \<in> (backward_slice S)` obtain s\<^isub>2' 
    where "transfer (slice_kind S a) s\<^isub>2 = s\<^isub>2'"
    by(simp add:slice_kind_in_slice)
  with `pred (slice_kind S a) s\<^isub>2` `n\<^isub>1 \<in> (backward_slice S)` `valid_edge a` 
  have "S,slice_kind S \<turnstile> (n\<^isub>1,s\<^isub>2) -a\<rightarrow> (n\<^isub>1',s\<^isub>2')"
    by(fastsimp intro:observable_moveI)
  from `obs n\<^isub>2 (backward_slice S) = {n\<^isub>1}`
  obtain as where "S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as\<Rightarrow>\<^isub>\<tau> (n\<^isub>1,s\<^isub>2)"
    by(erule obs_silent_moves)
  with `S,slice_kind S \<turnstile> (n\<^isub>1,s\<^isub>2) -a\<rightarrow> (n\<^isub>1',s\<^isub>2')` 
  have "S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as@[a]\<Rightarrow> (n\<^isub>1',s\<^isub>2')"
    by -(rule observable_moves_snoc)
  have "\<forall>V \<in> rv S n\<^isub>1'. state_val s\<^isub>1' V = state_val s\<^isub>2' V"
  proof
    fix V assume rv:"V \<in> rv S n\<^isub>1'"
    ...
    qed
  qed
  with `valid_node n\<^isub>1'` have "((n\<^isub>1',s\<^isub>1'),(n\<^isub>1',s\<^isub>2')) \<in> WS S" by(fastsimp intro:WSI)
  with `S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as@[a]\<Rightarrow> (n\<^isub>1',s\<^isub>2')`
    `transfer (slice_kind S a) s\<^isub>2 = s\<^isub>2'` 
  show "\<exists>as. ((n\<^isub>1',s\<^isub>1'),(n\<^isub>1',transfer (slice_kind S a) s\<^isub>2)) \<in> WS S \<and>
    S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as@[a]\<Rightarrow> (n\<^isub>1',transfer (slice_kind S a) s\<^isub>2)"
    by blast
qed
...
definition is_weak_sim :: 
  ...
  \<forall>n\<^isub>1 s\<^isub>1 n\<^isub>2 s\<^isub>2 n\<^isub>1' s\<^isub>1' as. ((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> R \<and> S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) =as\<Rightarrow> (n\<^isub>1',s\<^isub>1')
  \<longrightarrow> (\<exists>n\<^isub>2' s\<^isub>2' as'. ((n\<^isub>1',s\<^isub>1'),(n\<^isub>2',s\<^isub>2')) \<in> R \<and> 
                      S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as'\<Rightarrow> (n\<^isub>2',s\<^isub>2'))"


lemma WS_weak_sim:
  assumes "((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S" 
  and "S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) =as\<Rightarrow> (n\<^isub>1',s\<^isub>1')"
  shows "((n\<^isub>1',s\<^isub>1'),(n\<^isub>1',transfer (slice_kind S (last as)) s\<^isub>2)) \<in> WS S \<and>
  (\<exists>as'. S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as'@[last as]\<Rightarrow> 
                             (n\<^isub>1',transfer (slice_kind S (last as)) s\<^isub>2))"
proof -
  from `S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) =as\<Rightarrow> (n\<^isub>1',s\<^isub>1')` obtain a' as' n' s'
    where "S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) =as'\<Rightarrow>\<^isub>\<tau> (n',s')" 
    and "S,kind \<turnstile> (n',s') -a'\<rightarrow> (n\<^isub>1',s\<^isub>1')" and "as = as'@[a']"
    by(fastsimp elim:observable_moves.cases)
  from `S,kind \<turnstile> (n',s') -a'\<rightarrow> (n\<^isub>1',s\<^isub>1')` have "obs n' (backward_slice S) = {n'}"
    by(fastsimp elim:observable_move.cases intro!:n_in_obs)
  hence "obs n' (backward_slice S) \<noteq> {}" by fast
  with `S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) =as'\<Rightarrow>\<^isub>\<tau> (n',s')` `((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S` 
  have "((n',s'),(n\<^isub>2,s\<^isub>2)) \<in> WS S"
    by -(rule WS_silent_moves,simp+)
  with `S,kind \<turnstile> (n',s') -a'\<rightarrow> (n\<^isub>1',s\<^isub>1')` obtain asx 
    where "((n\<^isub>1',s\<^isub>1'),(n\<^isub>1',transfer (slice_kind S a') s\<^isub>2)) \<in> WS S"
    and "S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =asx@[a']\<Rightarrow> 
    (n\<^isub>1',transfer (slice_kind S a') s\<^isub>2)"
    by(fastsimp elim:WS_observable_move)
  with `as = as'@[a']` show
    "((n\<^isub>1',s\<^isub>1'),(n\<^isub>1',transfer (slice_kind S (last as)) s\<^isub>2)) \<in> WS S \<and>
    (\<exists>as'. S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as'@[last as]\<Rightarrow> 
           (n\<^isub>1',transfer (slice_kind S (last as)) s\<^isub>2))" by simp blast
qed
...
subsection {* @{term "n -as\<rightarrow>* n'"} and transitive closure of 
  @{term "S,f \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s')"} *}
...
lemma silent_moves_no_slice_edges:
  "S,f \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s') \<Longrightarrow> slice_edges S as = []"
...
next
  ...
  proof(cases as')
    ...
    have "n \<notin> backward_slice S" by(fastsimp simp:slice_edges_def)
    from `valid_edge a` `sourcenode a = n` `n = n'` `n' \<in> S`
    have "n \<in> backward_slice S" by(fastsimp intro:refl)
...
lemma path_trans_observable_moves:
  ...
  and "S,kind \<turnstile> (n'',s'') =as'\<Rightarrow>\<^isub>\<tau> (n',s')" 
...
proof(atomize_elim)
  ...
  show "\<exists>n'' s'' as' as''. 
    ...
    S,kind \<turnstile> (n'',s'') =as'\<Rightarrow>\<^isub>\<tau> (n',s') \<and> slice_edges S as = slice_edges S as'' \<and>
  ...
  proof(induct arbitrary:s rule:path.induct)
    ...
    have "S,kind \<turnstile> (n,s) =[]\<Rightarrow>\<^isub>\<tau> (n,s)" by(rule silent_moves_Nil)
  ...
  next
    ...
    note IH = `\<And>s. \<lbrakk>preds (kinds as) s; transfers (kinds as) s = s'\<rbrakk>
      \<Longrightarrow> \<exists>nx s'' as' as''. S,kind \<turnstile> (n'',s) =slice_edges S as\<Rightarrow>* (nx,s'') \<and>
            S,kind \<turnstile> (nx,s'') =as'\<Rightarrow>\<^isub>\<tau> (n',s') \<and> 
    ...
    from IH[OF this] obtain nx sx asx asx'
      ...
      and "S,kind \<turnstile> (nx,sx) =asx\<Rightarrow>\<^isub>\<tau> (n',s')"
...
lemma WS_weak_sim_trans:
  assumes "((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S"
  and "S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) =as\<Rightarrow>* (n\<^isub>1',s\<^isub>1')" and "as \<noteq> []"
  shows "((n\<^isub>1',s\<^isub>1'),(n\<^isub>1',transfers (slice_kinds S as) s\<^isub>2)) \<in> WS S \<and> 
         S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as\<Rightarrow>* (n\<^isub>1',transfers (slice_kinds S as) s\<^isub>2)"
proof -
  obtain f where "f = kind" by simp
  with `S,kind \<turnstile> (n\<^isub>1,s\<^isub>1) =as\<Rightarrow>* (n\<^isub>1',s\<^isub>1')` 
  have "S,f \<turnstile> (n\<^isub>1,s\<^isub>1) =as\<Rightarrow>* (n\<^isub>1',s\<^isub>1')" by simp
  from `S,f \<turnstile> (n\<^isub>1,s\<^isub>1) =as\<Rightarrow>* (n\<^isub>1',s\<^isub>1')` `((n\<^isub>1,s\<^isub>1),(n\<^isub>2,s\<^isub>2)) \<in> WS S` `as \<noteq> []` `f = kind`
  show "((n\<^isub>1',s\<^isub>1'),(n\<^isub>1',transfers (slice_kinds S as) s\<^isub>2)) \<in> WS S \<and>
    S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as\<Rightarrow>* (n\<^isub>1',transfers (slice_kinds S as) s\<^isub>2)"
  ...
  next
    ...
    note IH = `\<And>n\<^isub>2 s\<^isub>2. \<lbrakk>((n',s'),(n\<^isub>2,s\<^isub>2)) \<in> WS S; as' \<noteq> []; f = kind\<rbrakk>
      \<Longrightarrow> ((n'',s''),(n'',transfers (slice_kinds S as') s\<^isub>2)) \<in> WS S \<and>
      S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =as'\<Rightarrow>* (n'',transfers (slice_kinds S as') s\<^isub>2)`
    from `S,f \<turnstile> (n,s) =as\<Rightarrow> (n',s')`
    obtain asx ax nx sx where "S,f \<turnstile> (n,s) =asx\<Rightarrow>\<^isub>\<tau> (nx,sx)"
    ...
    with `S,f \<turnstile> (n,s) =asx\<Rightarrow>\<^isub>\<tau> (nx,sx)` `((n,s),(n\<^isub>2,s\<^isub>2)) \<in> WS S` `f = kind`
    have "((nx,sx),(n\<^isub>2,s\<^isub>2)) \<in> WS S" by(fastsimp intro:WS_silent_moves)
    with `S,f \<turnstile> (nx,sx) -ax\<rightarrow> (n',s')` `f = kind`
    obtain asx' where "((n',s'),(n',transfer (slice_kind S ax) s\<^isub>2)) \<in> WS S"
      and "S,slice_kind S \<turnstile> (n\<^isub>2,s\<^isub>2) =asx'@[ax]\<Rightarrow> 
...
proof(atomize_elim)
  from path preds obtain n'' s'' as' as''
    ...
    and "S,kind \<turnstile> (n'',s'') =as'\<Rightarrow>\<^isub>\<tau> (n',transfers (kinds as) s)"
  ...
  from path have "valid_node n" and "valid_node n'" 
    by(fastsimp dest:path_valid_node)+
  from `valid_node n` have "((n,s),(n,s)) \<in> WS S" by(fastsimp intro:WSI)
  ...
  proof(cases "slice_edges S as = []")
    ...
    with `S,kind \<turnstile> (n'',s'') =as'\<Rightarrow>\<^isub>\<tau> (n',transfers (kinds as) s)`
    have "S,kind \<turnstile> (n,s) =as'\<Rightarrow>\<^isub>\<tau> (n',transfers (kinds as) s)" by simp
    with `valid_node n` have "n -as'\<rightarrow>* n'"
      by(fastsimp dest:silent_moves_preds_transfers_path)
    from `S,kind \<turnstile> (n,s) =as'\<Rightarrow>\<^isub>\<tau> (n',transfers (kinds as) s)`
    have "slice_edges S as' = []" by(fastsimp dest:silent_moves_no_slice_edges)
  ...
  next
    ...
    from `obs n' (backward_slice S) = {n'}`
      `S,kind \<turnstile> (n'',s'') =as'\<Rightarrow>\<^isub>\<tau> (n',transfers (kinds as) s)`
