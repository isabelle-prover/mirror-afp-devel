...
lemma slice_deterministic:
  ...
  shows "\<exists>Q Q'. slice_kind n' a = (Q)\<^isub>\<surd> \<and> slice_kind n' a' = (Q')\<^isub>\<surd> \<and> 
...
proof -
  ...
  have "\<exists>as. n\<^isub>c,slice_kind n\<^isub>c \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s)"
  ...
  next
    ...
    proof(rule ccontr)
      assume "\<not> n \<notin> backward_slice n\<^isub>c"
      hence isin:"n \<in> backward_slice n\<^isub>c" by simp
...
proof -
  from rv that show ?thesis by(auto elim:relevant_vars.cases)
qed
...
proof -
  ...
  next
    ...
    proof(rule ccontr)
      assume "\<not> x \<notin> Def (sourcenode ax')"
      hence "x \<in> Def (sourcenode ax')" by simp
...
proof -
  have "\<exists>as. ((n\<^isub>1',s\<^isub>1'),(n\<^isub>1',transfer (slice_kind n\<^isub>c a) s\<^isub>2)) \<in> WS n\<^isub>c \<and>
  ...
  proof -
    ...
    with `n\<^isub>c,slice_kind n\<^isub>c \<turnstile> (n\<^isub>2,s\<^isub>2) =as@[a]\<Rightarrow> (n\<^isub>1',s\<^isub>2')`
      `transfer (slice_kind n\<^isub>c a) s\<^isub>2 = s\<^isub>2'` show ?thesis by blast
  qed
  with that show ?thesis by blast
qed
...
definition slice_nodes :: "'node \<Rightarrow> 'edge list \<Rightarrow> 'edge list"
  where "slice_nodes n\<^isub>c as \<equiv> [a \<leftarrow> as. sourcenode a \<in> backward_slice n\<^isub>c]"


lemma silent_moves_no_slice_nodes:
  "n\<^isub>c,f \<turnstile> (n,s) =as\<Rightarrow>\<^isub>\<tau> (n',s') \<Longrightarrow> slice_nodes n\<^isub>c as = []"
by(induct rule:silent_moves.induct,auto elim:silent_move.cases simp:slice_nodes_def)


lemma observable_moves_last_slice_nodes:
  "n\<^isub>c,f \<turnstile> (n,s) =as\<Rightarrow> (n',s') \<Longrightarrow> slice_nodes n\<^isub>c as = [last as]"
by(induct rule:observable_moves.induct,
   fastsimp dest:silent_moves_no_slice_nodes elim:observable_move.cases 
            simp:slice_nodes_def)


lemma path_trans_observable_moves:
  assumes "n -as\<rightarrow>* n'" and "preds (kinds as) s" and "transfers (kinds as) s = s'"
  obtains n'' s'' as' as'' where "n\<^isub>c,kind \<turnstile> (n,s) =slice_nodes n\<^isub>c as\<Rightarrow>* (n'',s'')"
  and "n\<^isub>c,kind \<turnstile> (n'',s'') =as'\<Rightarrow>\<^isub>\<tau> (n',s')" 
  and "slice_nodes n\<^isub>c as = slice_nodes n\<^isub>c as''" and "n -as''@as'\<rightarrow>* n'"
proof -
  from `n -as\<rightarrow>* n'` `preds (kinds as) s` `transfers (kinds as) s = s'`
  have "\<exists>n'' s'' as' as''. 
    n\<^isub>c,kind \<turnstile> (n,s) =slice_nodes n\<^isub>c as\<Rightarrow>* (n'',s'') \<and>
    n\<^isub>c,kind \<turnstile> (n'',s'') =as'\<Rightarrow>\<^isub>\<tau> (n',s') \<and> slice_nodes n\<^isub>c as = slice_nodes n\<^isub>c as'' \<and>
  ...
  proof(induct arbitrary:s rule:path.induct)
    ...
    show ?case
      ...
      apply(rule_tac x="[]" in exI)
      by(fastsimp intro:path.empty_path simp:slice_nodes_def)
  next
    case (Cons_path n'' as n' a n)
    note IH = `\<And>s. \<lbrakk>preds (kinds as) s; transfers (kinds as) s = s'\<rbrakk>
      \<Longrightarrow> \<exists>nx s'' as' as''. n\<^isub>c,kind \<turnstile> (n'',s) =slice_nodes n\<^isub>c as\<Rightarrow>* (nx,s'') \<and>
            n\<^isub>c,kind \<turnstile> (nx,s'') =as'\<Rightarrow>\<^isub>\<tau> (n',s') \<and> 
            slice_nodes n\<^isub>c as = slice_nodes n\<^isub>c as'' \<and> n'' -as''@as'\<rightarrow>* n'`
    ...
    from IH[OF this] obtain nx sx asx asx'
      where "n\<^isub>c,kind \<turnstile> (n'',transfer (kind a) s) =slice_nodes n\<^isub>c as\<Rightarrow>* (nx,sx)"
      and "n\<^isub>c,kind \<turnstile> (nx,sx) =asx\<Rightarrow>\<^isub>\<tau> (n',s')"
      and "slice_nodes n\<^isub>c as = slice_nodes n\<^isub>c asx'"
	...
	by(fastsimp intro:observable_moves_snoc silent_moves_Nil)
      with `n\<^isub>c,kind \<turnstile> (n'',transfer (kind a) s) =slice_nodes n\<^isub>c as\<Rightarrow>* (nx,sx)`
      have "n\<^isub>c,kind \<turnstile> (n,s) =a#slice_nodes n\<^isub>c as\<Rightarrow>* (nx,sx)"
	by(fastsimp dest:tom_Cons)
      with `n\<^isub>c,kind \<turnstile> (nx,sx) =asx\<Rightarrow>\<^isub>\<tau> (n',s')`
	`slice_nodes n\<^isub>c as = slice_nodes n\<^isub>c asx'` `n'' -asx'@asx\<rightarrow>* n'`
	...
	by(auto intro:path.Cons_path simp:slice_nodes_def)
    next
      case False
      with `valid_edge a` `sourcenode a = n` `targetnode a = n''` `pred (kind a) s`
      have "n\<^isub>c,kind \<turnstile> (n,s) -a\<rightarrow>\<^isub>\<tau> (n'',transfer (kind a) s)"
	by(fastsimp intro:silent_moveI)
      from `n\<^isub>c,kind \<turnstile> (n'',transfer (kind a) s) =slice_nodes n\<^isub>c as\<Rightarrow>* (nx,sx)`
      obtain f s'' asx'' where "n\<^isub>c,f \<turnstile> (n'',s'') =asx''\<Rightarrow>* (nx,sx)"
	and "f = kind" and "s'' = transfer (kind a) s" 
	and "asx'' = slice_nodes n\<^isub>c as" by simp
      from `n\<^isub>c,f \<turnstile> (n'',s'') =asx''\<Rightarrow>* (nx,sx)` `f = kind`
	`asx'' = slice_nodes n\<^isub>c as` `s'' = transfer (kind a) s`
	`n\<^isub>c,kind \<turnstile> (n,s) -a\<rightarrow>\<^isub>\<tau> (n'',transfer (kind a) s)` 
	`n\<^isub>c,kind \<turnstile> (nx,sx) =asx\<Rightarrow>\<^isub>\<tau> (n',s')` `slice_nodes n\<^isub>c as = slice_nodes n\<^isub>c asx'`
	...
	with `sourcenode a = n` `valid_edge a` `targetnode a = n''`
	  `[] = slice_nodes n\<^isub>c as` `n \<notin> backward_slice n\<^isub>c`
	...
	show ?case
	  ...
	  apply(rule_tac x="a#asx" in exI)
	  apply(rule_tac x="[]" in exI)
	  by(fastsimp simp:slice_nodes_def intro:trans_observable_moves.tom_Nil)
      next
	...
	with `f = kind` `last asi # asi' = slice_nodes n\<^isub>c as` `n \<notin> backward_slice n\<^isub>c`
	  `n\<^isub>c,kind \<turnstile> (n'',s'') =asx\<Rightarrow>\<^isub>\<tau> (n',s')`  `sourcenode a = n` `asi \<noteq> []`
	  `ni -asx'@asx\<rightarrow>* n'` `slice_nodes n\<^isub>c as = slice_nodes n\<^isub>c asx'`
...
lemma transfers_slice_kinds_slice_nodes:
  "transfers (slice_kinds n\<^isub>c (slice_nodes n\<^isub>c as)) s =
   transfers (slice_kinds n\<^isub>c as) s"
proof(induct as arbitrary:s)
  case Nil thus ?case by(simp add:slice_kinds_def slice_nodes_def)
next
  case (Cons a' as')
  note IH = `\<And>s. transfers (slice_kinds n\<^isub>c (slice_nodes n\<^isub>c as')) s =
                  transfers (slice_kinds n\<^isub>c as') s`
  show ?case
  proof(cases "sourcenode a' \<in> backward_slice n\<^isub>c")
    case True
    hence eq:"transfers (slice_kinds n\<^isub>c (slice_nodes n\<^isub>c (a'#as'))) s
            = transfers (slice_kinds n\<^isub>c (slice_nodes n\<^isub>c as')) 
                (transfer (slice_kind n\<^isub>c a') s)"
      by(simp add:slice_nodes_def slice_kinds_def)
...
lemma trans_observable_moves_preds:
  ...
  obtains as' where "preds (map f as') s" and "slice_nodes n\<^isub>c as' = as"
  and "n -as'\<rightarrow>* n'"
proof -
  from `n\<^isub>c,f \<turnstile> (n,s) =as\<Rightarrow>* (n',s')` `valid_node n`
  have "\<exists>as'. preds (map f as') s \<and> slice_nodes n\<^isub>c as' = as \<and> n -as'\<rightarrow>* n'"
  proof(induct rule:trans_observable_moves.induct)
    case tom_Nil thus ?case 
      by(rule_tac x="[]" in exI,fastsimp intro:empty_path simp:slice_nodes_def)
  next
    case (tom_Cons n\<^isub>c f n s as n' s' as' n'' s'')
    note IH = `valid_node n' 
      \<Longrightarrow> \<exists>asx. preds (map f asx) s' \<and> slice_nodes n\<^isub>c asx = as' \<and> n' -asx\<rightarrow>* n''`
    from `n\<^isub>c,f \<turnstile> (n,s) =as\<Rightarrow> (n',s')`
    have "preds (map f as) s" and "transfers (map f as) s = s'"
      and "n -as\<rightarrow>* n'"
    ...
    from `n\<^isub>c,f \<turnstile> (n,s) =as\<Rightarrow> (n',s')` have "slice_nodes n\<^isub>c as = [last as]"
      by(rule observable_moves_last_slice_nodes)
    from IH[OF `valid_node n'`]
    obtain asx where "preds (map f asx) s'" and "slice_nodes n\<^isub>c asx = as'"
    ...
    with `slice_nodes n\<^isub>c as = [last as]` `slice_nodes n\<^isub>c asx = as'` 
      `n -as@asx\<rightarrow>* n''` show ?case
      by(rule_tac x="as@asx" in exI,auto simp:slice_nodes_def)
  qed
  with that show ?thesis by blast
qed
...
lemma slice_nodes_no_nodes_in_slice:
  "slice_nodes n\<^isub>c as = [] 
  \<Longrightarrow> \<forall>nx \<in> set(sourcenodes as). nx \<notin> (backward_slice n\<^isub>c)"
proof(induct as)
  case Nil thus ?case by(simp add:slice_nodes_def sourcenodes_def)
next
  case (Cons a' as')
  note IH = `slice_nodes n\<^isub>c as' = [] \<Longrightarrow>
    \<forall>nx\<in>set (sourcenodes as'). nx \<notin> backward_slice n\<^isub>c`
  from `slice_nodes n\<^isub>c (a'#as') = []` have "slice_nodes n\<^isub>c as' = []"
    and "sourcenode a' \<notin> backward_slice n\<^isub>c"
    by(auto simp:slice_nodes_def split:split_if_asm)
  from IH[OF `slice_nodes n\<^isub>c as' = []`] `sourcenode a' \<notin> backward_slice n\<^isub>c`
  show ?case by(simp add:sourcenodes_def)
qed
...
lemma exists_sliced_path_preds:
  assumes "n -as\<rightarrow>* n'" and "slice_nodes n\<^isub>c as = []" and "n' \<in> backward_slice n\<^isub>c"
  obtains as' where "n -as'\<rightarrow>* n'" and "preds (slice_kinds n\<^isub>c as') s"
  and "slice_nodes n\<^isub>c as' = []"
proof -
  have "\<exists>as'. n -as'\<rightarrow>* n' \<and> preds (slice_kinds n\<^isub>c as') s \<and> slice_nodes n\<^isub>c as' = []"
  proof -
    from `slice_nodes n\<^isub>c as = []`
    have "\<forall>nx \<in> set(sourcenodes as). nx \<notin> (backward_slice n\<^isub>c)"
      by(rule slice_nodes_no_nodes_in_slice)
    ...
    from `n -as\<rightarrow>* n'` obtain x where "distance n n' x" and "x \<le> length as"
      by(erule every_path_distance)
    from `distance n n' x` `obs n (backward_slice n\<^isub>c) = {n'}` show ?thesis
	...
	by(rule_tac x="[]" in exI,
           auto intro:empty_path simp:slice_kinds_def slice_nodes_def)
    next
      case (Suc x)
	...
	by(auto elim:distance_successor_distance)
      ...
      proof(rule ccontr)
	assume "\<not> n \<notin> backward_slice n\<^isub>c"
	hence "n \<in> backward_slice n\<^isub>c" by simp
	...
	and "slice_nodes n\<^isub>c as = []" by blast
      from `targetnode a -as\<rightarrow>* n'` `valid_edge a` `n = sourcenode a`
      have "n -a#as\<rightarrow>* n'" by(fastsimp intro:Cons_path)
      from `slice_nodes n\<^isub>c as = []` `n \<notin> backward_slice n\<^isub>c` `n = sourcenode a`
      have "slice_nodes n\<^isub>c (a#as) = []" by(simp add:slice_nodes_def)
      show ?case
      proof(cases "kind a")
	case (Update f)
	with `n \<notin> backward_slice n\<^isub>c` `n = sourcenode a` have "slice_kind n\<^isub>c a = \<Up>id"
	...
	with `preds (slice_kinds n\<^isub>c as) s` have "preds (slice_kinds n\<^isub>c (a#as)) s"
	  by(simp add:slice_kinds_def)
	with `n -a#as\<rightarrow>* n'` `slice_nodes n\<^isub>c (a#as) = []` show ?thesis
	  by blast
      next
	case (Predicate Q)
...
theorem fundamental_property_of_static_slicing:
  ...
  and "slice_nodes n' as = slice_nodes n' as'" and "n -as'\<rightarrow>* n'"
proof -
  have "\<exists>as'. preds (slice_kinds n' as') s \<and>
    (\<forall>V \<in> Use n'. state_val (transfers (slice_kinds n' as') s) V = 
                  state_val (transfers (kinds as) s) V) \<and>
    slice_nodes n' as = slice_nodes n' as' \<and> n -as'\<rightarrow>* n'"
  proof -
    from path preds obtain n'' s'' as' as''
      where "n',kind \<turnstile> (n,s) =slice_nodes n' as\<Rightarrow>* (n'',s'')"
      and "n',kind \<turnstile> (n'',s'') =as'\<Rightarrow>\<^isub>\<tau> (n',transfers (kinds as) s)"
      and "slice_nodes n' as = slice_nodes n' as''"
    ...
    show ?thesis
    proof(cases "slice_nodes n' as = []")
      case True
      hence "preds (slice_kinds n' []) s" and "slice_nodes n' [] = slice_nodes n' as"
	by(simp_all add:slice_kinds_def slice_nodes_def)
      from `n',kind \<turnstile> (n,s) =slice_nodes n' as\<Rightarrow>* (n'',s'')` True
      have "n = n''" and "s = s''"
	...
	state_val (transfers (slice_kinds n' (slice_nodes n' as)) s) V"
	by(fastsimp dest:WSD simp:slice_nodes_def slice_kinds_def)
      with `\<forall>V \<in> Use n'. V \<in> rv n' n'`
      have "\<forall>V \<in> Use n'. state_val (transfers (kinds as) s) V = 
	state_val (transfers (slice_kinds n' (slice_nodes n' as)) s) V" by simp
      with `slice_nodes n' asx = []` `slice_nodes n' [] = slice_nodes n' as`
      have "\<forall>V \<in> Use n'. state_val (transfers (kinds as) s) V = 
	state_val (transfers (slice_kinds n' (slice_nodes n' asx)) s) V"
	by(simp add:slice_nodes_def)
      hence "\<forall>V \<in> Use n'. state_val (transfers (kinds as) s) V = 
	state_val (transfers (slice_kinds n' asx) s) V"
	by(simp add:transfers_slice_kinds_slice_nodes)
      with `n -asx\<rightarrow>* n'` `preds (slice_kinds n' asx) s`
	`slice_nodes n' asx = []` `slice_nodes n' [] = slice_nodes n' as`
      show ?thesis
	by(rule_tac x="asx" in exI,simp add:slice_nodes_def)
    next
      case False
      with `n',kind \<turnstile> (n,s) =slice_nodes n' as\<Rightarrow>* (n'',s'')` `((n,s),(n,s)) \<in> WS n'`
      have "((n'',s''),(n'',transfers (slice_kinds n' (slice_nodes n' as)) s))
	\<in> WS n'"
	"n',slice_kind n' \<turnstile> (n,s) =slice_nodes n' as\<Rightarrow>* 
                              (n'',transfers (slice_kinds n' (slice_nodes n' as)) s)"
	by(fastsimp dest:WS_weak_sim_trans)+
      from `n',slice_kind n' \<turnstile> (n,s) =slice_nodes n' as\<Rightarrow>* 
                               (n'',transfers (slice_kinds n' (slice_nodes n' as)) s)`
	`valid_node n`
      obtain asx where "preds (slice_kinds n' asx) s" 
	and "slice_nodes n' asx = slice_nodes n' as"
	and "n -asx\<rightarrow>* n''"
	...
	by(rule path_Append)
      from `slice_nodes n' asx = slice_nodes n' as` `slice_nodes n' asx' = []`
      have "slice_nodes n' as = slice_nodes n' (asx@asx')"
	by(auto simp:slice_nodes_def)
	...
	`((n'',s''),(n'',transfers (slice_kinds n' (slice_nodes n' as)) s)) \<in> WS n'`
      have "((n',transfers (kinds as) s),
	(n'',transfers (slice_kinds n' (slice_nodes n' as)) s)) \<in> WS n'"
	by(fastsimp intro:WS_silent_moves)
      hence "\<forall>V \<in> rv n' n'. state_val (transfers (kinds as) s) V = 
	state_val (transfers (slice_kinds n' (slice_nodes n' as)) s) V"
	by(fastsimp dest:WSD)
      with `\<forall>V \<in> Use n'. V \<in> rv n' n'` `slice_nodes n' asx = slice_nodes n' as`
      have "\<forall>V \<in> Use n'. state_val (transfers (kinds as) s) V = 
	state_val (transfers (slice_kinds n' (slice_nodes n' asx)) s) V"
	by fastsimp
      with `slice_nodes n' asx' = []`
      have "\<forall>V \<in> Use n'. state_val (transfers (kinds as) s) V = 
	state_val (transfers (slice_kinds n' (slice_nodes n' (asx@asx'))) s) V"
	by(auto simp:slice_nodes_def)
      hence "\<forall>V \<in> Use n'. state_val (transfers (kinds as) s) V = 
	state_val (transfers (slice_kinds n' (asx@asx')) s) V"
	by(simp add:transfers_slice_kinds_slice_nodes)
      with `preds (slice_kinds n' (asx@asx')) s` `n -asx@asx'\<rightarrow>* n'`
	`slice_nodes n' as = slice_nodes n' (asx@asx')`
...
theorem fundamental_property_of_path_slicing_semantically:
  assumes identifies:"n identifies c" and reds:"\<langle>c,s\<rangle> \<Rightarrow> \<langle>c',s'\<rangle>"
  shows "\<exists>n' as. n -as\<rightarrow>* n' \<and> preds (slice_kinds n' as) s \<and> n' identifies c' \<and>
  (\<forall>V \<in> Use n'. state_val (transfers (slice_kinds n' as) s) V = 
                  state_val s' V)"
proof -
  ...
  with preds' path' identifies' show ?thesis by blast
qed
