...
lemma lcosI: "[| h \<in> H; H \<subseteq> carrier G; x \<in> carrier G|] ==> x \<otimes> h \<in> x \<subset># H"
...
next
  ...
  with x g H show "x \<in> g \<subset># H" by (metis is_group subgroup.lcos_module_rev subgroup.mem_carrier)
qed
...
lemma cardeq_lcoset:
  ...
  shows "card (g \<subset># M) = card  M"
proof -
  have "bij_betw (\<lambda>m. g \<otimes> m) M (g \<subset># M)"
  ...
  next
    fix x
    assume "x \<in> M"
    thus "g \<otimes> x \<in> g \<subset># M" unfolding l_coset_def by auto
  next
    fix x
    assume x:"x \<in> g \<subset># M"
    then obtain m where "x = g \<otimes> m" "m \<in> M" unfolding l_coset_def by auto
    thus "x \<in> op \<otimes> g ` M" by simp
  qed
  thus "card (g \<subset># M) = card M" by (metis bij_betw_same_card)
qed
...
lemma conjugation_subgroup:
  ...
  shows "subgroup (g \<subset># (H #> inv g)) G"
proof
  ...
  with gG show "g \<subset># (H #> inv g) \<subseteq> carrier G" by (metis l_coset_subset_G)
next
  ...
  fix x y
  assume x:"x \<in> g \<subset># (H #> inv g)" and y:"y \<in> g \<subset># (H #> inv g)"
  ...
  with gG lcosSubset have "g \<otimes> (hx \<otimes> hy \<otimes> inv g) \<in> g \<subset># (H #> inv g)" by (metis lcosI)
  with xy show "x \<otimes> y \<in> g \<subset># (H #> inv g)" by simp
next
  from gG have invgG:"inv g \<in> carrier G" by (metis inv_closed)
  with HG have lcosSubset:"(H #> inv g) \<subseteq> carrier G" by (metis r_coset_subset_G subgroup_imp_subset)
  ...
  with gG lcosSubset have "g \<otimes> (\<one> \<otimes> inv g) \<in> g \<subset># (H #> inv g)" by (metis lcosI)
  with gG invgG show "\<one> \<in> g \<subset># (H #> inv g)" by simp
next
  from gG have invgG:"inv g \<in> carrier G" by (metis inv_closed)
  with HG have lcosSubset:"(H #> inv g) \<subseteq> carrier G" by (metis r_coset_subset_G subgroup_imp_subset)
  fix x
  assume "x \<in> g \<subset># (H #> inv g)"
  ...
  with gG lcosSubset have "g \<otimes> (inv hx \<otimes> inv g) \<in> g \<subset># (H #> inv g)" by (metis lcosI)
  with invx show "inv x \<in> g \<subset># (H #> inv g)" by simp
qed

definition conjugation_action::"nat \<Rightarrow> _"
  where "conjugation_action p = (\<lambda>g\<in>carrier G. \<lambda>P\<in>subgroups_of_size p. g \<subset># (P #> inv g))"
...
proof -
  ...
  have " g \<subset># (P #> inv g) \<in> subgroups_of_size p"
  proof(auto simp add:subgroups_of_size_def)
    show "subgroup (g \<subset># (P #> inv g)) G" by (metis g PG conjugation_subgroup)
  next
    from card PsubG fin invg have "card (P #> inv g) = p" by (metis cardeq_rcoset)
    with g PinvgsubG fin show "card (g \<subset># (P #> inv g)) = p" by (metis cardeq_lcoset)
  qed
...
proof -
  ...
  proof(auto simp add:bij_betw_def)
    ...
    proof(rule inj_onI)
      ...
      with g U V have "g \<subset># (U #> inv g) = g \<subset># (V #> inv g)" unfolding conjugation_action_def by simp
      hence "(inv g) \<subset># (g \<subset># (U #> inv g)) = (inv g) \<subset># (g \<subset># (V #> inv g))" by simp
      hence "(inv g \<otimes> g) \<subset># (U #> inv g) = (inv g \<otimes> g) \<subset># (V #> inv g)" by (metis g invg lcos_m_assoc r_coset_subset_G subsetG)
      hence "\<one> \<subset># (U #> inv g) = \<one> \<subset># (V #> inv g)" by (metis g l_inv)
  ...
  next
    ...
    with invg P have "(inv g) \<subset># (P #> (inv (inv g))) \<in> subgroups_of_size p" unfolding conjugation_action_def by simp
    hence 1:"(inv g) \<subset># (P #> g) \<in> subgroups_of_size p" by (metis g inv_inv)
    have "g \<subset># (((inv g) \<subset># (P #> g)) #> inv g) = (\<Union>p \<in> P. {g \<otimes> (inv g \<otimes> (p \<otimes> g) \<otimes> inv g)})" unfolding r_coset_def l_coset_def by (simp add:m_assoc)
    ...
    finally have "g \<subset># (((inv g) \<subset># (P #> g)) #> inv g) = P".
    with 1 have "P \<in> (\<lambda>P. g \<subset># (P #> inv g)) ` subgroups_of_size p" by auto
...
lemma lr_coset_assoc:
  ...
  shows "g \<subset># (P #> h) = (g \<subset># P) #> h"
proof(auto)
  fix x
  assume "x \<in> g \<subset># (P #> h)"
  ...
  with `p \<in> P` show "x \<in> (g \<subset># P) #> h" unfolding l_coset_def r_coset_def by auto
next
  fix x
  assume "x \<in> (g \<subset># P) #> h"
  then obtain p where "p \<in> P" and p:"x = (g \<otimes> p) \<otimes> h" unfolding l_coset_def r_coset_def by auto
...
next
  ...
  also have "... = (\<lambda>P\<in>subgroups_of_size p. x \<otimes> y \<subset># (P #> inv (x \<otimes> y)))"
  proof(rule restrict_ext)
    fix P
    assume P:"P \<in> subgroups_of_size p"
    hence PG:"P \<subseteq> carrier G" unfolding subgroups_of_size_def by (auto simp:subgroup_imp_subset)
    with y have yPG:"y \<subset># P \<subseteq> carrier G" by (metis l_coset_subset_G)
    ...
    with x y P have "conjx (conjy P) = x \<subset># ((y \<subset># (P #> inv y)) #> inv x)" unfolding conjy_def conjx_def conjugation_action_def by simp
    also from y invy PG have "... = x \<subset># (((y \<subset># P) #> inv y) #> inv x)" by (metis lr_coset_assoc)
    also from PG invx invy y have "... = x \<subset># ((y \<subset># P) #> (inv y \<otimes> inv x))" by (metis coset_mult_assoc yPG)
    also from x y have "... = x \<subset># ((y \<subset># P) #> inv (x \<otimes> y))" by (metis inv_mult_group)
    also from invxyG x yPG have "... = (x \<subset># (y \<subset># P)) #> inv (x \<otimes> y)" by (metis lr_coset_assoc)
    also from x y PG have "... = ((x \<otimes> y) \<subset># P) #> inv (x \<otimes> y)" by (metis lcos_m_assoc)
    also from xyG invxyG PG have "... = (x \<otimes> y) \<subset># (P #> inv (x \<otimes> y))" by (metis lr_coset_assoc)
    finally show "conjx (conjy P) = x \<otimes> y \<subset># (P #> inv (x \<otimes> y))".
  qed
  finally have "conjx \<otimes>\<^bsub>BijGroup (subgroups_of_size p)\<^esub> conjy = (\<lambda>P\<in>subgroups_of_size p. x \<otimes> y \<subset># (P #> inv (x \<otimes> y)))".
...
proof
  ...
  with P have "conjugation_action p x P = x \<subset># (P #> inv x)" unfolding conjugation_action_def by simp
  also from `inv x \<in> P` PG have "... = x \<subset># P" by (metis coset_join2 subgroup.mem_carrier)
...
lemma conj_wo_inv:
  ...
  assumes g:"g \<in> carrier G"
  assumes conj:"inv g \<subset># (Q #> g) = P"
  shows "Q #> g = g \<subset># P"
proof -
  from g have invg:"inv g \<in> carrier G" by (metis inv_closed)
  from conj have "g \<subset># (inv g \<subset># (Q #> g)) = g \<subset># P" by simp
  with QG g invg have "(g \<otimes> inv g) \<subset># (Q #> g) = g \<subset># P" by (metis lcos_m_assoc r_coset_subset_G subgroup_imp_subset)
  with g invg have "\<one> \<subset># (Q #> g) = g \<subset># P" by (metis r_inv)
  with QG g show "Q #> g = g \<subset># P" by (metis lcos_mult_one r_coset_subset_G subgroup_imp_subset)
qed
