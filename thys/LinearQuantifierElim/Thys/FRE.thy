...
definition FR\<^isub>1 :: "atom fm \<Rightarrow> atom fm" where
"FR\<^isub>1 \<phi> =
(let as = R.atoms\<^isub>0 \<phi>; lbs = lbounds as; ubs = ubounds as; ebs = ebounds as;
...
proof(induct f)
  ...
  proof (cases a)
    ...
    show ?thesis
    proof (cases cs)
      case Nil thus ?thesis using Atom Less by (simp add:depends\<^isub>R_def)
    next
      case (Cons c cs)
  ...
  next
    ...
    show ?thesis
    proof (cases cs)
      case Nil thus ?thesis using Atom Eq by (simp add:depends\<^isub>R_def)
    next
      case (Cons c cs)
...
proof
  ...
  { assume "\<exists>x \<in> EQ \<phi> xs. R.I \<phi> (x#xs)"
    hence ?EX using `?FR` by(auto simp add:FR\<^isub>1_def)
  } moreover
  { assume "\<not>R.I (inf\<^isub>- \<phi>) xs \<and> \<not>R.I (inf\<^isub>+ \<phi>) xs \<and>
...
next
  ...
  { assume "R.I (inf\<^isub>- \<phi>) xs \<or> R.I (inf\<^isub>+ \<phi>) xs"
    hence ?FR by(auto simp:FR\<^isub>1_def)
  } moreover
  { assume "x : EQ \<phi> xs"
    then obtain r cs
  ...
  { assume "\<not> R.I (inf\<^isub>- \<phi>) xs" and "\<not> R.I (inf\<^isub>+ \<phi>) xs" and "x \<notin> EQ \<phi> xs"
    ...
    then obtain r c cs s d ds
      where "Less r (c#cs) : set(R.atoms\<^isub>0 \<phi>) \<and> c>0 \<and>
             Less s (d#ds) : set(R.atoms\<^isub>0 \<phi>) \<and> d<0 \<and>
...
definition "FR = R.lift_nnf_qe FR\<^isub>1"


lemma qfree_FR\<^isub>1: "nqfree \<phi> \<Longrightarrow> qfree (FR\<^isub>1 \<phi>)"
apply(simp add:FR\<^isub>1_def)
...
by(simp add:I_FR\<^isub>1 FR_def R.I_lift_nnf_qe qfree_FR\<^isub>1)

theorem qfree_FR: "qfree (FR \<phi>)"
by(simp add:FR_def R.qfree_lift_nnf_qe qfree_FR\<^isub>1)
