...
definition
"qelim qe as =
 (let qf = qe [a\<leftarrow>as. depends\<^isub>0 a];
      indep = [Atom(decr a). a\<leftarrow>as, \<not> depends\<^isub>0 a]
  in and qf (list_conj indep))"

abbreviation is_dnf_qe :: "('a list \<Rightarrow> 'a fm) \<Rightarrow> 'a list \<Rightarrow> bool" where
"is_dnf_qe qe as \<equiv> \<forall>xs. I(qe as) xs = (\<exists>x.\<forall>a\<in>set as. I\<^isub>a a (x#xs))"
...
lemma I_qelim:
assumes qe: "\<And>as. (\<forall>a \<in> set as. depends\<^isub>0 a) \<Longrightarrow> is_dnf_qe qe as"
shows "is_dnf_qe (qelim qe) as" (is "\<forall>xs. ?P xs")
proof
  fix  xs
  let ?as0 = "filter depends\<^isub>0 as"
  let ?as1 = "filter (Not o depends\<^isub>0) as"
  have "I (qelim qe as) xs =
        (I (qe ?as0) xs \<and> (\<forall>a\<in>set(map decr ?as1). I\<^isub>a a xs))"
    (is "_ = (_ \<and> ?B)") by(force simp add:qelim_def)
  also have "\<dots> = ((\<exists>x. \<forall>a \<in> set ?as0. I\<^isub>a a (x#xs)) \<and> ?B)"
    by(simp add:qe not_dep_decr)
  also have "\<dots> = (\<exists>x. (\<forall>a \<in> set ?as0. I\<^isub>a a (x#xs)) \<and> ?B)" by blast
  also have "?B = (\<forall>a \<in> set ?as1. I\<^isub>a (decr a) xs)" by simp
  also have "(\<exists>x. (\<forall>a \<in> set ?as0. I\<^isub>a a (x#xs)) \<and> \<dots>) =
             (\<exists>x. (\<forall>a \<in> set ?as0. I\<^isub>a a (x#xs)) \<and>
                  (\<forall>a \<in> set ?as1. I\<^isub>a a (x#xs)))"
    by(simp add: not_dep_decr)
  also have "\<dots> = (\<exists>x. \<forall>a \<in> set(?as0 @ ?as1). I\<^isub>a a (x#xs))"
    by (simp add:ball_Un)
  also have "\<dots> = (\<exists>x. \<forall>a \<in> set(as). I\<^isub>a a (x#xs))"
    by simp blast
  finally show "?P xs" .
qed
...
"lift_dnf_qe qe (And \<phi>\<^isub>1 \<phi>\<^isub>2) = and (lift_dnf_qe qe \<phi>\<^isub>1) (lift_dnf_qe qe \<phi>\<^isub>2)" |
"lift_dnf_qe qe (Or \<phi>\<^isub>1 \<phi>\<^isub>2) = or (lift_dnf_qe qe \<phi>\<^isub>1) (lift_dnf_qe qe \<phi>\<^isub>2)" |
...
lemma qfree_lift_dnf_qe: "(\<And>as. (\<forall>a\<in>set as. depends\<^isub>0 a) \<Longrightarrow> qfree(qe as))
 \<Longrightarrow> qfree(lift_dnf_qe qe \<phi>)"
by (induct \<phi>) (simp_all add:qelim_def)

lemma qfree_lift_dnf_qe2: "qe : lists (Collect depends\<^isub>0) \<rightarrow> Collect qfree
 \<Longrightarrow> qfree(lift_dnf_qe qe \<phi>)"
...
lemma I_lift_dnf_qe:
assumes  "\<And>as. (\<forall>a \<in> set as. depends\<^isub>0 a) \<Longrightarrow> qfree(qe as)"
and "\<And>as. (\<forall>a \<in> set as. depends\<^isub>0 a) \<Longrightarrow> is_dnf_qe qe as"
...
assumes  "qe : lists (Collect depends\<^isub>0) \<rightarrow> Collect qfree"
and "\<forall>as \<in> lists (Collect depends\<^isub>0). is_dnf_qe qe as"
shows "I (lift_dnf_qe qe \<phi>) xs = I \<phi> xs"
...
assumes qe: "\<And>xs as. \<forall>a \<in> set as. depends\<^isub>0 a \<and> anormal a \<Longrightarrow> is_dnf_qe qe as"
and nm: "\<forall>a \<in> set as. anormal a"
shows "I (qelim qe as) xs = (\<exists>x. \<forall>a\<in>set as. I\<^isub>a a (x#xs))"
proof -
  let ?as0 = "filter depends\<^isub>0 as"
  let ?as1 = "filter (Not o depends\<^isub>0) as"
  have "I (qelim qe as) xs =
...
lemma anormal_atoms_qelim:
  "(\<And>as. \<forall>a \<in> set as. depends\<^isub>0 a \<and> anormal a \<Longrightarrow> normal(qe as)) \<Longrightarrow>
...
apply(auto simp add:anormal_decr dest!: atoms_list_conjE)
 apply(erule_tac x = "filter depends\<^isub>0 as" in meta_allE)
 apply(simp)
apply(erule_tac x = "filter depends\<^isub>0 as" in meta_allE)
apply(simp)
done
...
assumes "\<And>as. \<forall>a \<in> set as. depends\<^isub>0 a \<Longrightarrow> qfree(qe as)"
and "\<And>as. \<forall>a \<in> set as. depends\<^isub>0 a \<and> anormal a \<Longrightarrow> normal(qe as)"
...
assumes "\<And>as. \<forall>a \<in> set as. depends\<^isub>0 a \<Longrightarrow> qfree(qe as)"
and "\<And>as. \<forall>a \<in> set as. depends\<^isub>0 a \<and> anormal a \<Longrightarrow> normal(qe as)"
and "\<And>xs as. \<forall>a \<in> set as. depends\<^isub>0 a \<and> anormal a \<Longrightarrow> is_dnf_qe qe as"
...
assumes "qe : lists (Collect depends\<^isub>0) \<rightarrow> Collect qfree"
and "qe : lists(Collect depends\<^isub>0 \<inter> Collect anormal) \<rightarrow> Collect normal"
and "\<forall>as \<in> lists(Collect depends\<^isub>0 \<inter> Collect anormal). is_dnf_qe qe as"
shows "normal f \<Longrightarrow> I (lift_dnf_qe qe f) xs = I f xs"
using assms in_lists_conv_set[where ?'a = 'a]
...
"lift_nnf_qe qe (And \<phi>\<^isub>1 \<phi>\<^isub>2) = and (lift_nnf_qe qe \<phi>\<^isub>1) (lift_nnf_qe qe \<phi>\<^isub>2)" |
"lift_nnf_qe qe (Or \<phi>\<^isub>1 \<phi>\<^isub>2) = or (lift_nnf_qe qe \<phi>\<^isub>1) (lift_nnf_qe qe \<phi>\<^isub>2)" |
...
lemma qfree_lift_nnf_qe2:
  "qe : Collect nqfree \<rightarrow> Collect qfree \<Longrightarrow> qfree(lift_nnf_qe qe \<phi>)"
...
assumes  "qe : Collect nqfree \<rightarrow> Collect qfree"
and "ALL \<phi> : Collect nqfree. ALL xs. I (qe \<phi>) xs = (\<exists>x. I \<phi> (x#xs))"
shows "I (lift_nnf_qe qe \<phi>) xs = I \<phi> xs"
...
assumes  "qe : Collect nqfree \<rightarrow> Collect qfree"
and "qe : Collect nqfree \<inter> Collect normal \<rightarrow> Collect normal"
and "ALL \<phi> : Collect normal Int Collect nqfree. ALL xs. I (qe \<phi>) xs = (\<exists>x. I \<phi> (x#xs))"
shows "normal \<phi> \<Longrightarrow> I (lift_nnf_qe qe \<phi>) xs = I \<phi> xs"
...
locale ATOM_EQ = ATOM +
fixes solvable\<^isub>0 :: "'a \<Rightarrow> bool"
and trivial :: "'a \<Rightarrow> bool" 
and subst\<^isub>0 :: "'a \<Rightarrow> 'a \<Rightarrow> 'a"
assumes subst\<^isub>0:
...
"lift_eq_qe qe as =
 (let as = [a\<leftarrow>as. \<not> trivial a]
  in case [a\<leftarrow>as. solvable\<^isub>0 a] of
    [] \<Rightarrow> qe as
  | eq # eqs \<Rightarrow>
        (let ineqs = [a\<leftarrow>as. \<not> solvable\<^isub>0 a]
         in list_conj (map (Atom \<circ> (subst\<^isub>0 eq)) (eqs @ ineqs))))"

theorem I_lift_eq_qe:
assumes dep: "\<forall>a\<in>set as. depends\<^isub>0 a"
assumes qe: "\<And>as. (\<forall>a \<in> set as. depends\<^isub>0 a \<and> \<not> solvable\<^isub>0 a) \<Longrightarrow>
   I (qe as) xs = (\<exists>x. \<forall>a \<in> set as. I\<^isub>a a (x#xs))"
shows "I (lift_eq_qe qe as) xs = (\<exists>x. \<forall>a \<in> set as. I\<^isub>a a (x#xs))"
  (is "?L = ?R")
proof -
  let ?as = "[a\<leftarrow>as. \<not> trivial a]"
  show ?thesis
  proof (cases "[a\<leftarrow>?as. solvable\<^isub>0 a]")
    case Nil
    hence "\<forall>a\<in>set as. \<not> trivial a \<longrightarrow> \<not> solvable\<^isub>0 a"
      by(auto simp: filter_empty_conv)
    thus "?L = ?R"
      by(simp add:lift_eq_qe_def dep qe cong:conj_cong) (metis trivial)
  next
    case (Cons eq _)
    then have "eq : set as" "solvable\<^isub>0 eq" "\<not> trivial eq"
      by (auto simp: filter_eq_Cons_iff)
    then obtain e where "I\<^isub>a eq (e#xs)" by(metis solvable)
    have "\<forall>a \<in> set as. I\<^isub>a a (e # xs) = I\<^isub>a (subst\<^isub>0 eq a) xs"
      by(simp add: subst\<^isub>0[OF `solvable\<^isub>0 eq` `\<not> trivial eq` `I\<^isub>a eq (e#xs)`] dep)
...
lemma qfree_lift_eq_qe:
  "(\<And>as. \<forall>a\<in>set as. depends\<^isub>0 a \<Longrightarrow> qfree (qe as)) \<Longrightarrow>
   \<forall>a\<in>set as. depends\<^isub>0 a \<Longrightarrow> qfree(lift_eq_qe qe as)"
by(simp add:lift_eq_qe_def ball_Un split:list.split)

lemma qfree_lift_dnfeq_qe: "(\<And>as. (\<forall>a\<in>set as. depends\<^isub>0 a) \<Longrightarrow> qfree(qe as))
...
lemma I_lift_dnfeq_qe:
  "(\<And>as. (\<forall>a \<in> set as. depends\<^isub>0 a) \<Longrightarrow> qfree(qe as)) \<Longrightarrow>
   (\<And>as. (\<forall>a \<in> set as. depends\<^isub>0 a \<and> \<not> solvable\<^isub>0 a) \<Longrightarrow> is_dnf_qe qe as) \<Longrightarrow>
...
lemma I_lift_dnfeq_qe2:
  "qe : lists (Collect depends\<^isub>0) \<rightarrow> Collect qfree \<Longrightarrow>
   (\<forall>as \<in> lists(Collect depends\<^isub>0 \<inter> - Collect solvable\<^isub>0). is_dnf_qe qe as) \<Longrightarrow>
   I (lift_dnfeq_qe qe \<phi>) xs = I \<phi> xs"
using in_lists_conv_set[where ?'a = 'a]
