...
'a. The product is then defined in FuncSet.thy, as \verb+PiE I (\lambda i. X i)+, the set of
elements from 'i to 'a such that the $i$-th coordinate belongs to $X i$ for all $i \in I$.
...
text {* In the definition below of a generated topology, the \verb+Empty+ case is not necessary,
as it follows from \verb+UN+ taking for $K$ the empty set. However, it is convenient to have,
...
text {*Pulling back a topology by map gives again a topology. \verb+subtopology+ is
...
proof -
  have "generate_topology_on {(\<Pi>\<^sub>E i\<in>I. X i) |X. (\<forall>i. openin (T i) (X i)) \<and> finite {i. X i \<noteq> topspace (T i)}} U"
    apply (rule openin_topology_generated_by)
    using assms unfolding product_topology_def by auto
  then have "\<forall>x\<in>U. \<exists>X. x \<in> (\<Pi>\<^sub>E i\<in>I. X i) \<and> (\<forall>i. openin (T i) (X i)) \<and> finite {i. X i \<noteq> topspace (T i)} \<and> (\<Pi>\<^sub>E i\<in>I. X i) \<subseteq> U"
  proof (induct, auto)
    fix U V x assume "x \<in> U" "x \<in> V"
      "\<forall>x\<in>U. \<exists>XU. x \<in> Pi\<^sub>E I XU \<and> (\<forall>i. openin (T i) (XU i)) \<and> finite {i. XU i \<noteq> topspace (T i)} \<and> Pi\<^sub>E I XU \<subseteq> U"
      "\<forall>x\<in>V. \<exists>XV. x \<in> Pi\<^sub>E I XV \<and> (\<forall>i. openin (T i) (XV i)) \<and> finite {i. XV i \<noteq> topspace (T i)} \<and> Pi\<^sub>E I XV \<subseteq> V"
    ...
    moreover have "x \<in> Pi\<^sub>E I X"
      unfolding X_def using H by auto
    ultimately show "\<exists>X. x \<in> (Pi\<^sub>E I X) \<and> (\<forall>i. openin (T i) (X i)) \<and> finite {i. X i \<noteq> topspace (T i)} \<and> Pi\<^sub>E I X \<subseteq> U \<and> Pi\<^sub>E I X \<subseteq> V"
      by auto
  next
    fix K k x assume
      "\<And>k. k \<in> K \<Longrightarrow> \<forall>x\<in>k. \<exists>X. x \<in> Pi\<^sub>E I X \<and> (\<forall>i. openin (T i) (X i)) \<and> finite {i. X i \<noteq> topspace (T i)} \<and> Pi\<^sub>E I X \<subseteq> k"
      "k \<in> K"
      "x \<in> k"
    then have "\<exists>X. x \<in> Pi\<^sub>E I X \<and> (\<forall>i. openin (T i) (X i)) \<and> finite {i. X i \<noteq> topspace (T i)} \<and> Pi\<^sub>E I X \<subseteq> k"
      by simp
    ...
    then show "\<exists>X. x \<in> Pi\<^sub>E I X \<and> (\<forall>i. openin (T i) (X i)) \<and> finite {i. X i \<noteq> topspace (T i)} \<and> Pi\<^sub>E I X \<subseteq> \<Union>K"
      by auto
  qed
...
lemma continuous_on_product_then_coordinatewise [intro, continuous_intros]:
