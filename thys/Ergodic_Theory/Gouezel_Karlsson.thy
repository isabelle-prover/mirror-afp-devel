...
proof -
  ...
  have fin: "finite {N. Nf N \<le> n}" for n
    using pseudo_inverse_finite_set[OF filterlim_subseq[OF \<open>subseq Nf\<close>]] by auto
  ...
  have "LIM n sequentially. prev_N n :> at_top"
    ...
    using \<open>subseq Nf\<close> by (simp add: filterlim_subseq)
  ...
  proof -
    ...
    {
      fix N assume "N \<ge> B"
      have "Nf N \<ge> B" using seq_suble[OF \<open>subseq Nf\<close>, of N] \<open>N \<ge> B\<close> by simp
  ...
  proof -
    ...
    proof (cases)
      ...
      then have "{N. Nf N \<le> n} = {0}"
        apply auto using \<open>subseq Nf\<close> unfolding subseq_def
