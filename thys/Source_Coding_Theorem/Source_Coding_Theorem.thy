...
proof -
  ...
  next
      ...
      "KL_div (S \<inter> {j. 0 < a j}) a (\<lambda>i. d i / (\<Sum>i\<in>(S \<inter> {i. 0 < a i}). d i)) \<le> KL_div (S \<inter> {j. 0 < a j}) a d"
        using non_null setsum.inter_restrict[OF fin, of d, of "{i. 0 < a i}"]
...
shows "(\<Sum>i\<in>A. f i / (\<Sum>j\<in>A. f j)) = 1"
    by (metis (no_types) assms right_inverse_eq setsum_divide_distrib)
...
proof -
    ...
    also have "\<dots> = -1 * (\<Sum>i \<in> L. fi i * (log b (1/(b powr (cw_len i)))))"
      using setsum_right_distrib[of "-1" "(\<lambda>i. fi i * (- 1 * log b (1 / b powr (cw_len i))))" L]
      by simp
    finally have
    ...
    moreover from McMillan assms have "0 \<le> log b (1 / \<K>)"
      using \<K>_pos
      by (simp add: b_gt_1)
    ultimately show ?thesis by simp
qed
