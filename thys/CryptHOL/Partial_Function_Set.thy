...
interpretation set: partial_function_definitions "op \<subseteq>" Union
...
abbreviation "mono_set \<equiv> monotone (fun_ord op \<subseteq>) op \<subseteq>"
...
interpretation coset: partial_function_definitions "op \<supseteq>" Inter
...
lemma fun_ord_ge: "fun_ord op \<ge> = op \<ge>"
by(auto simp add: fun_ord_def fun_eq_iff le_fun_def)
...
abbreviation "mono_coset \<equiv> monotone (fun_ord op \<supseteq>) op \<supseteq>"

lemma gfp_eq_fixp:
  fixes f :: "'a :: complete_lattice \<Rightarrow> 'a"
  assumes f: "monotone op \<ge> op \<ge> f"
  shows "gfp f = ccpo.fixp Inf op \<ge> f"
proof (rule antisym)
  from f have f': "mono f" by(simp add: mono_def monotone_def)

  interpret ccpo Inf "op \<ge>" "mk_less op \<ge> :: 'a \<Rightarrow> _"
    by(rule ccpo)(rule complete_lattice.lattice_partial_function_definition[OF dual_complete_lattice])
  show "ccpo.fixp Inf op \<ge> f \<le> gfp f"
    by(rule gfp_upperbound)(subst fixp_unfold[OF f], rule order_refl)

  show "gfp f \<le> ccpo.fixp Inf op \<ge> f"
...
lemma fixp_coinduct_set:
  ...
  and eq: "f \<equiv> C (ccpo.fixp (fun_lub Inter) (fun_ord op \<ge>) (\<lambda>f. U (F (C f))))"
  and inverse2: "\<And>f. U (C f) = f"
...
proof(rule contrapos_np)
  have mono': "monotone (op \<ge>) (op \<ge>) (\<lambda>f. U (F (C f)))"
...
abbreviation "mono_set' \<equiv> monotone (fun_ord op \<supseteq>) op \<supseteq>"
