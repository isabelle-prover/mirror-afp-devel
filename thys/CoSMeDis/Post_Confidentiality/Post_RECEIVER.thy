...
can learn nothing about the updates to a post received from a remote node \<open>j\<close>
    ...
    have ?react proof
      ...
      proof-
        have ?match proof(cases "\<exists> p pst uid vs. a = COMact (comReceivePost AID p PID pst uid vs) \<and> ou = outOK")
          ...
          define a1 where "a1 \<equiv> COMact (comReceivePost AID p PID pst1 uid vs)" note a1 = this
