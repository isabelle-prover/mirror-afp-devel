...
subsection \<open>Confidentiality for the (binary) issuer-receiver composition\<close>
...
|"comSyncOA (O_sendPost (aid2, sp1, pid1, pst1, uid1, vis1)) (comReceivePost aid1 sp2 pid2 pst2 uid2 vis2) =
   (aid1 = AID1 \<and> aid2 = AID2 \<and> (pid1, pst1, uid1, vis1) = (pid2, pst2, uid2, vis2))"
...
assumes "validTrans trn1" and "reach (srcOf trn1)" and "\<phi>1 trn1"
shows "isCom1 trn1 \<longleftrightarrow> isComV1 (f1 trn1)"
using assms apply(cases trn1) by (auto simp: Iss.\<phi>_def2 split: prod.splits)
...
and "isCom1 trn1" and "isCom2 trn2" and "\<phi>1 trn1" and "\<phi>2 trn2"
and "sync trn1 trn2"
shows "syncV (f1 trn1) (f2 trn2)"
using assms apply(cases trn1, cases trn2)
by (auto simp: Iss.\<phi>_def2 Rcv.\<phi>_def2 split: prod.splits)
