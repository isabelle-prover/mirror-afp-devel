...
lemma \<phi>_def1:
"\<phi> trn \<longleftrightarrow>
 (\<exists>uid p pst. actOf trn = Uact (uPost uid p PID pst) \<and> outOf trn = outOK) \<or>
 (\<exists>uid p aid. actOf trn = COMact (comSendPost uid p aid PID) \<and> outOf trn \<noteq> outErr) \<or>
 ((\<forall>uid p pid pst. actOf trn \<noteq> Uact (uPost uid p pid pst)) \<and>
  (\<forall>uid p aid pid. actOf trn \<noteq> COMact (comSendPost uid p aid pid)) \<and>
   open (srcOf trn) \<noteq> open (tgtOf trn))"
by (cases trn rule: \<phi>.cases) auto
...
"\<phi> (Trans s a ou s') \<longleftrightarrow>
 ...
 (\<exists>uid p aid. a = COMact (comSendPost uid p aid PID) \<and> ou \<noteq> outErr) \<or>
  open s \<noteq> open s'"
using assms
by (cases "Trans s a ou s'" rule: \<phi>.cases) (auto simp: all_defs open_def)
