section \<open>Certificates for (Un)Reachability Properties\<close>
...
definition f_topo where
  "f_topo \<equiv> \<lambda>(l, s). Max {i. (s, i) \<in> M l}"

lemma
  assumes "l \<in> L" "(s, i) \<in> M l"
  shows f_in: "(s, f_topo (l, s)) \<in> M l" (is "?P1")
    and f_ge: "\<forall>j. (s, j) \<in> M l \<longrightarrow> j \<le> f_topo (l, s)" (is "?P2")
proof -
...
lemma f_topo:
  ...
  shows \<open>if F (l, s) then f_topo (l, s) < f_topo (l2, s2) else f_topo (l, s) \<le> f_topo (l2, s2)\<close>
proof -
  ...
  from f_in assms have "(s, f_topo (l, s)) \<in> M l"
    by auto
  with assms obtain s' i where "(s', i) \<in> M l2" "buechi_prop l l2 (f_topo (l, s)) i s s1 s'"
    unfolding check_buechi_spec'_def by fastforce
  then have "(s', i) \<in> M l2" "s1 \<preceq> s'" "?le l s (f_topo (l, s)) i"
    unfolding buechi_prop_def by auto
  from is_max \<open>(s', i) \<in> _\<close> \<open>s1 \<preceq> s'\<close> have "i \<le> j"
    unfolding is_arg_max_linorder by simp
  also from f_ge \<open>(s2, j) \<in> M l2\<close> have "j \<le> f_topo (l2, s2)"
    using assms by auto
  finally show ?thesis
