section \<open>Assembling the Checker and Generating Code\<close>

text \<open>\label{export-muntac}
...
\<^item> The (binary) certificate is read
\<^item> There is an additional ``renaming'' that maps identifiers between the input and the certificate
\<^item> The certficiate, the renaming, and the model are passed to the checker
\<^item> and the output is printed
...
text \<open>\label{self-check} We can actually let \munta\ produce reachability certificates
for \muntac. This is what the following code does.
A few test cases are run below right in the Isabelle/ML environment.\<close>
...
definition convert_renaming ::
  ...
  "convert_renaming ids_to_names process_names_to_index json \<equiv> do {
    ...
    locations \<leftarrow> list_of_json_object locations; \<comment>\<open>process name \<open>\<rightarrow>\<close> json\<close>
    locations \<leftarrow> combine_map (\<lambda>(name, renaming). do {
        ...
        renaming \<leftarrow> nat_renaming_of_json (STR ''process'' + show_str p_num) max_id renaming;
          \<comment>\<open>location id \<open>\<rightarrow>\<close> nat\<close>
        Result (p_num, renaming)
      }
      ) locations;
    \<comment>\<open>process id \<open>\<rightarrow>\<close> location id \<open>\<rightarrow>\<close> nat\<close>
...
text \<open>Delete unused code module.\<close>
...
text \<open>We eliminate this module because it uses constants that are only implemented by \<open>IntInf\<close>
and not \<open>Int\<close>. When compiling we will use a hack to change \<open>IntInf\<close> with \<open>Int\<close> (for efficiency)
and therefore this module would break the hack.\<close>
code_printing code_module Bit_Shifts \<rightharpoonup> (SML) \<open>\<close>

text \<open>Using the \<open>Eval\<close> code target instead of \<open>SML\<close> makes it easier to replace \<open>IntInf\<close> with \<open>Int\<close>.\<close>
