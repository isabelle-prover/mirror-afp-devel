...
sig
  ...
  val update_if: ('a -> 'a -> bool) -> key * 'a -> 'a table -> 'a table
  ...
  val tl_list: key -> 'a list table -> 'a list table
...
(* Own update if predicate holds *)
fun update_if p (key, x) tab = modify key (fn NONE => x | SOME x' => if p x x' then x else x') tab
...
fun tl_list key tab =
    map_entry key
	      (fn [] => raise UNDEF key | [x] => raise UNDEF key | (x::xs) => xs) tab
...
(* For compatibilty reasons we disable the pretty printing *)
(* ML pretty-printing *)
