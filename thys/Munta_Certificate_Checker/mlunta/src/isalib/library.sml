...
infix 4 ~~ (* upto downto needs to be commented out for Iter *)
infix orf andf
...
sig
  ...
  val trip_ord: ('a * 'b -> order) -> ('c * 'd -> order) -> ('e * 'f -> order) -> ('a * 'c * 'e) * ('b * 'd * 'f) -> order
end;
...
fun fast_string_ord (s1, s2) =
    (* We removed pointer_eq here because maybe we also want this to work with mlton *)
...
fun trip_ord a_ord b_ord c_ord ((x, y, z), (x', y', z')) =
    (case a_ord (x, x') of
	 EQUAL => (case b_ord (y, y') of
		     EQUAL => c_ord (z, z') |
		     ord => ord) |
	 ord => ord)
end;
