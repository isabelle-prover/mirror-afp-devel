...
lemma sip_in_kD:
  "paodv i \<TTurnstile> onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l). l \<in> ({PAodv-:7} \<union> {PAodv-:5} \<union> {PRrep-:0..PRrep-:4}
...
lemma dests_bigger_than_sqn:
  "paodv i \<TTurnstile> onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l). l \<in> {PAodv-:15..PAodv-:17}
                                 \<union> {PPkt-:7..PPkt-:9}
                                 \<union> {PRreq-:11..PRreq-:13}
                                 \<union> {PRreq-:20..PRreq-:22}
                                 \<union> {PRrep-:7..PRrep-:9}
                                 \<union> {PRerr-:1..PRerr-:4} \<union> {PRerr-:6}
...
lemma rreq_dip_in_vD_dip_eq_ip:
  "paodv i \<TTurnstile> onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l). (l \<in> {PRreq-:16..PRreq-:17} \<longrightarrow> dip \<xi> \<in> vD(rt \<xi>))
                            \<and> (l \<in> {PRreq-:6, PRreq-:7} \<longrightarrow> dip \<xi> = ip \<xi>)
                            \<and> (l \<in> {PRreq-:15..PRreq-:17} \<longrightarrow> dip \<xi> \<noteq> ip \<xi>))"
  by inv_cterms

lemma rrep_dip_in_vD:
  "paodv i \<TTurnstile> onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l). (l \<in> {PRrep-:4} \<longrightarrow> dip \<xi> \<in> vD(rt \<xi>)))"
  by inv_cterms
...
lemma anycast_msg_zhops:
  ...
  proof (inv_cterms inv add:
    ...
    assume "(\<xi>, pp) \<in> reachable (paodv i) TT"
       and "{PRreq-:16}unicast(\<lambda>\<xi>. the (nhop (rt \<xi>) (oip \<xi>)),
  ...
  next
    fix l \<xi> a pp p' pp'    
    assume "(\<xi>, pp) \<in> reachable (paodv i) TT"
      and "{PRrep-:4}unicast(\<lambda>\<xi>. the (nhop (rt \<xi>) (oip \<xi>)),
              \<lambda>\<xi>. Rrep (the (dhops (rt \<xi>) (dip \<xi>))) (dip \<xi>) (sqn (rt \<xi>) (dip \<xi>)) (oip \<xi>) (ip \<xi>)).
                     p' \<triangleright> pp' \<in> sterms \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V pp" 
      and "l = PRrep-:4"
      and "a = unicast (the (nhop (rt \<xi>) (oip \<xi>)))
...
lemma kD_unk_or_atleast_one:
  ...
  proof -
    { fix sip rt dsn1 dsn2 dsk1 dsk2 flag1 flag2 hops1 hops2 nhip1 nhip2
      ...
      hence "\<pi>\<^sub>3(the (update rt sip (dsn1, dsk1, flag1, hops1, nhip1) sip)) = unk
            \<or> Suc 0 \<le> sqn (update rt sip (dsn2, dsk2, flag2, hops2, nhip2)) sip"
    ...
    { fix dip sip rt dsn1 dsn2 dsk1 dsk2 flag1 flag2 hops1 hops2 nhip1 nhip2
      ...
      have "\<forall>dip\<in>kD(rt). \<pi>\<^sub>3(the (update rt sip (dsn1, dsk1, flag1, hops1, nhip1) dip)) = unk
            \<or> Suc 0 \<le> sqn (update rt sip (dsn2, dsk2, flag2, hops2, nhip2)) dip"
...
lemma rreq_rrep_sn_any_step_invariant:
  ...
  proof -
    (* due to lack of addpreRT_welldefined, sqnf_know needed some small adaption [adding dip \<xi> \<in> kD (rt \<xi>)] *)
    have sqnf_kno: "paodv i \<TTurnstile> onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l).
                                      (l \<in> {PRreq-:16} \<longrightarrow> dip \<xi> \<in> kD (rt \<xi>) \<and> sqnf (rt \<xi>) (dip \<xi>) = kno))"
      by (inv_cterms)

    have rrep_sqn_greater_dsn: "paodv i \<TTurnstile> (recvmsg rreq_rrep_sn \<rightarrow>) onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l).
                                      (l \<in> {PRrep-:1 .. PRrep-:4} \<longrightarrow> 1 \<le> sqn (rt \<xi>) (dip \<xi>)))"
      by (inv_cterms inv add: onl_invariant_sterms [OF aodv_wf received_msg_inv]
                              onl_invariant_sterms [OF aodv_wf dsn_rrep])
...
lemma rreq_rrep_fresh_any_step_invariant:
  ...
  proof -                                                      
    have rreq_oip: "paodv i \<TTurnstile> onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l).
                       (l \<in> {PRreq-:3..PRreq-:9} \<union> {PRreq-:15, PRreq-:24, PRreq-:26}
    ...
    have rrep_prrep: "paodv i \<TTurnstile> onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l).
          (l \<in> {PRrep-:4} \<longrightarrow> (dip \<xi> \<in> kD(rt \<xi>)
                                        \<and> the (flag (rt \<xi>) (dip \<xi>)) = val)))"
      by (inv_cterms inv add: onl_invariant_sterms [OF aodv_wf sip_in_kD]) 

    have rreq_oip_kD: "paodv i \<TTurnstile> onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l). (l \<in> {PRreq-:3..PRreq-:22} \<longrightarrow> oip \<xi> \<in> kD(rt \<xi>)))"
      by(inv_cterms) 

    have rreq_dip_kD_oip_sqn: "paodv i \<TTurnstile> onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l).
                       (l \<in> {PRreq-:16..PRreq-:17}
  ...
  proof -
    have dests_inv: "paodv i \<TTurnstile>
                      onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l). (l \<in> {PAodv-:15, PPkt-:7, PRreq-:11,
                         ...
                         \<and> (l \<in> {PAodv-:16..PAodv-:17}
                              \<union> {PPkt-:8..PPkt-:9}
                              \<union> {PRreq-:12..PRreq-:13}
                              \<union> {PRreq-:21..PRreq-:22}
                              \<union> {PRrep-:8..PRrep-:9}
...
lemma dests_vD_inc_sqn:
  "paodv i \<TTurnstile>
        onl \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V (\<lambda>(\<xi>, l). (l \<in> {PAodv-:15, PPkt-:7, PRreq-:11, PRreq-:20, PRrep-:7}
...
lemma route_tables_fresher:
  ...
  next
    fix \<xi> pp p'
    assume "(\<xi>, pp) \<in> reachable (paodv i) (recvmsg rreq_rrep_sn)"
       and "{PRrep-:0}\<lbrakk>\<lambda>\<xi>. \<xi>\<lparr>rt := update (rt \<xi>) (dip \<xi>) (dsn \<xi>, kno, val, Suc (hops \<xi>), sip \<xi>)\<rparr>\<rbrakk>
            p' \<in> sterms \<Gamma>\<^sub>A\<^sub>O\<^sub>D\<^sub>V pp"
       and "Suc 0 \<le> dsn \<xi>"
       and *: "\<forall>ip\<in>kD (rt \<xi>). Suc 0 \<le> the (dhops (rt \<xi>) ip)"
