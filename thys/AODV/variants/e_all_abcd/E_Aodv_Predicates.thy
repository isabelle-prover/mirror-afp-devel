...
where "msg_sender m \<equiv> case m of Rreq _ _ _ _ _ _ ipc _ \<Rightarrow> ipc
...
where "msg_zhops m \<equiv> case m of
                                 Rreq hopsc dipc _ _ oipc _ sipc _ \<Rightarrow> hopsc = 0 \<longrightarrow> oipc = sipc
...
where "rreq_rrep_sn m \<equiv> case m of Rreq _ _ _ _ _ osnc _ _ \<Rightarrow> osnc \<ge> 1
                                | Rrep _ _ dsnc _ _ \<Rightarrow> dsnc \<ge> 1
                                | _ \<Rightarrow> True"

lemma rreq_rrep_sn_simps [simp]:
  "\<And>hops dip dsn dsk oip osn sip handled.
           rreq_rrep_sn (Rreq hops dip dsn dsk oip osn sip handled) = (osn \<ge> 1)"
...
where "rreq_rrep_fresh crt m \<equiv> case m of Rreq hopsc _ _ _ oipc osnc ipcc _ \<Rightarrow> (ipcc \<noteq> oipc \<longrightarrow>
