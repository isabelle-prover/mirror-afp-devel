...
although we do not try to get something particularly good -- in particular,
the dependence on $\delta$ is quadratic (the proof gives in fact $\delta \ln \delta$,
...
lemma Gromov_hyperbolic_invariant_under_quasi_isometry_explicit:
  ...
  shows "Gromov_hyperbolic_subset (1956 * lambda^4 * (lambda + C + deltaG(TYPE('a))^2)) (UNIV::('b set))"
proof -
  have C: "lambda \<ge> 1" "C \<ge> 0"
    using quasi_isometry_onD[OF quasi_isometry_betweenD(1)[OF assms]] by auto
  obtain g::"'b \<Rightarrow> 'a" where g: "quasi_isometry_between lambda (3 * C * lambda) UNIV UNIV g"
    using quasi_isometry_inverse[OF assms] by auto
  then have g2: "quasi_isometry_on lambda (3 * C * lambda) UNIV g"
    by (auto simp add: quasi_isometry_betweenD(1))
  ...
  define K where "K = 81 * lambda^2 * ((3 * C * lambda) + lambda + deltaG(TYPE('a))^2)"
  ...
  define M where "M = 489 * lambda^4 * (lambda + C + deltaG(TYPE('a))^2)"
  have "4 * x \<le> 4 * lambda + 4 * x * x" if "x \<ge> 0" for x
  proof (cases "x \<ge> 1")
    case True
    then have "x \<le> x * x" by auto
    then show ?thesis using C(1) by auto
  next
    case False
    then have "4 * x \<le> 4 * lambda" using C(1) by simp
    then show ?thesis
      by (meson le_add_same_cancel1 mult_nonneg_nonneg order_trans that zero_le_numeral)
  qed
  then have "4 * deltaG(TYPE('a)) \<le> 4 * lambda + 4 * deltaG(TYPE('a)) * deltaG(TYPE('a))"
    using delta_nonneg by auto
  ...
  also have "... = lambda * (4 * lambda * 1 * 1 * 1 + 4 * deltaG(TYPE('a)) * deltaG(TYPE('a)) * 1 * 1 * 1
                              + 162 * lambda * lambda * ((3 * C * lambda) + lambda * 1 + deltaG(TYPE('a))^2 * 1))
  ...
  also have "... \<le> lambda * (4 * lambda * lambda * lambda * lambda
                              ...
                              + 162 * lambda * lambda * ((3 * C * lambda) + lambda * lambda + deltaG(TYPE('a))^2 * lambda))
                    + 3 * C * lambda * lambda * lambda * lambda"
    apply (intro mono_intros) using C by auto
  also have "... = lambda * lambda * lambda * lambda * (166 * lambda + 489 * C + 166 * deltaG(TYPE('a))^2)"
    by (auto simp add: algebra_simps power2_eq_square)
  also have "... \<le> lambda * lambda * lambda * lambda * (489 * lambda + 489 * C + 489 * deltaG(TYPE('a))^2)"
    apply (intro mono_intros) using C by auto
  ...
  have Thin: "infdist w (Gxz \<union> Gyz) \<le> M" if
    ...
    for w x y z::'b and Gxy Gyz Gxz
  proof -
    obtain w2 where w2: "w2 \<in> {g x--g y}" "infdist (g w) {g x--g y} = dist (g w) w2"
      using infdist_compact_attained[of "{g x--g y}" "g w"] by auto
    have "dist (g w) w2 = infdist (g w) {g x-- g y}"
      using w2 by simp
    also have "... \<le> hausdorff_distance (g`Gxy) {g x-- g y}"
      using geodesic_segment_topology(4)[OF geodesic_segmentI] H
      by (auto intro!: quasi_isometry_on_bounded[OF quasi_isometry_on_subset[OF g2]] infdist_le_hausdorff_distance)
    also have "... \<le> K" using HD[OF H(1)] by simp
    finally have *: "dist (g w) w2 \<le> K" by simp

    have "infdist w2 (g`Gxz \<union> g`Gyz) \<le> infdist w2 ({g x--g z} \<union> {g y--g z})
                + hausdorff_distance ({g x--g z} \<union> {g y--g z}) (g`Gxz \<union> g`Gyz)"
      apply (rule hausdorff_distance_infdist_triangle)
      using geodesic_segment_topology(4)[OF geodesic_segmentI] H
      by (auto intro!: quasi_isometry_on_bounded[OF quasi_isometry_on_subset[OF g2]])
    also have "... \<le> 4 * deltaG(TYPE('a)) + hausdorff_distance ({g x--g z} \<union> {g y--g z}) (g`Gxz \<union> g`Gyz)"
      apply (simp, rule thin_triangles[of "{g x--g z}" "g z" "g x" "{g y--g z}" "g y" "{g x--g y}" w2])
      using w2 apply auto
      using geodesic_segment_commute some_geodesic_is_geodesic_segment(1) by blast+
    also have "... \<le> 4 * deltaG(TYPE('a)) + max (hausdorff_distance {g x--g z} (g`Gxz)) (hausdorff_distance {g y--g z} (g`Gyz))"
      apply (auto intro!: hausdorff_distance_union) using H by auto
    also have "... \<le> 4 * deltaG(TYPE('a)) + K"
      using HD[OF H(2)] HD[OF H(3)] by (auto simp add: hausdorff_distance_sym)
    finally have **: "infdist w2 (g`Gxz \<union> g`Gyz) \<le> 4 * deltaG(TYPE('a)) + K" by simp

    have "infdist (g w) (g`Gxz \<union> g`Gyz) \<le> infdist w2 (g`Gxz \<union> g`Gyz) + dist (g w) w2"
      by (rule infdist_triangle)
    then have A: "infdist (g w) (g`(Gxz \<union> Gyz)) \<le> 4 * deltaG(TYPE('a)) + 2 * K"
    ...
    proof -
      ...
      have "\<exists>z \<in> g`(Gxz \<union> Gyz). dist (g w) z < 4 * deltaG(TYPE('a)) + 2 * K + epsilon/lambda"
        apply (rule infdist_almost_attained)
        using A * H(2) by auto
      then obtain z where z: "z \<in> Gxz \<union> Gyz" "dist (g w) (g z) < 4 * deltaG(TYPE('a)) + 2 * K + epsilon/lambda"
        by auto

      have "infdist w (Gxz \<union> Gyz) \<le> dist w z"
        by (auto intro!: infdist_le z(1))
      also have "... \<le> lambda * dist (g w) (g z) + 3 * C * lambda * lambda"
        using quasi_isometry_onD[OF g2] by (auto simp add: algebra_simps divide_simps)
      also have "... \<le> lambda * (4 * deltaG(TYPE('a)) + 2 * K + epsilon/lambda) + 3 * C * lambda * lambda"
...
theorem Gromov_hyperbolic_invariant_under_quasi_isometry:
  assumes "quasi_isometric (UNIV::('a::Gromov_hyperbolic_space_geodesic) set) (UNIV::('b::geodesic_space) set)"
  shows "\<exists>delta. Gromov_hyperbolic_subset delta (UNIV::'b set)"
proof -
  ...
  show ?thesis using Gromov_hyperbolic_invariant_under_quasi_isometry_explicit[OF f] by blast
qed
