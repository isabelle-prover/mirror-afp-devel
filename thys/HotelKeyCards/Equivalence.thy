...
by(simp add:Trace.safe_def ext_iff)
...
apply(erule reach_cong)
apply (fastsimp simp add: ext_iff)
done
...
apply (simp add: ext_iff [where 'a=room] ext_iff [where 'a=guest])
apply (clarsimp simp add: ext_iff [where 'a=room] ext_iff [where 'a=guest and 'b="(key \<times> key) set"])
apply(erule disjE)
apply clarsimp
