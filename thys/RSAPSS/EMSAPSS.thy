...
consts  BC:: "bv" (* 0xbc *)
        salt:: "bv"
        sLen:: "nat" (* length of salt *)
        generate_M':: "bv \<Rightarrow> bv \<Rightarrow> bv" (* generate_M' outputs "M'" a bit string of length "64 + length(sha1 _ ) + sLen" with 64 initial zero bits *)
        generate_PS:: "nat \<Rightarrow> nat \<Rightarrow> bv" (* generate_PS outputs "PS" a bit string consisting of "nat (roundup emBits 8) * 8 - sLen - length (sha1 _ ) - 16" zero bits *)
        generate_DB:: "bv \<Rightarrow> bv" (* generate_DB outputs the bit string DB="PS#(0x01)#salt" of length "nat (roundup emBits 8) * 8 - length (sha1 _ ) - 8" *)
        generate_H:: "bv \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> bv" (* generate_H returns a bit string of length (sha1 _) *)
        generate_maskedDB:: "bv \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> bv " (* outputs a bit string of length "nat (roundup emBits 8) * 8 - length (sha1 _) - 8" *)
        generate_salt:: "bv \<Rightarrow> bv" (* outputs a bit string of length sLen *)
        ...
        emsapss_encode_help2:: "bv \<Rightarrow> nat \<Rightarrow> bv"
        emsapss_encode_help3:: "bv \<Rightarrow> nat \<Rightarrow> bv"
        emsapss_encode_help4:: "bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bv"
        emsapss_encode_help5:: "bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bv"
        emsapss_encode_help6:: "bv \<Rightarrow> bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bv"
        emsapss_encode_help7:: "bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bv"
        emsapss_encode_help8:: "bv \<Rightarrow> bv \<Rightarrow> bv"
        emsapss_decode:: "bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bool" (* outputs "true" if EM is encoding of M *)
        emsapss_decode_help1:: "bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bool"
        emsapss_decode_help2:: "bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bool"
        emsapss_decode_help3:: "bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bool"
        emsapss_decode_help4:: "bv \<Rightarrow> bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bool"
        emsapss_decode_help5:: "bv \<Rightarrow> bv \<Rightarrow> bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bool"
        emsapss_decode_help6:: "bv \<Rightarrow> bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bool"
        emsapss_decode_help7:: "bv \<Rightarrow> bv \<Rightarrow> bv \<Rightarrow> nat \<Rightarrow> bool"
        emsapss_decode_help8:: "bv \<Rightarrow> bv \<Rightarrow> bv \<Rightarrow> bool"
        emsapss_decode_help9:: "bv \<Rightarrow> bv \<Rightarrow> bv \<Rightarrow> bool"
        emsapss_decode_help10:: "bv \<Rightarrow> bv \<Rightarrow> bool"
        emsapss_decode_help11:: "bv \<Rightarrow> bv \<Rightarrow> bool"

defs

  show_rightmost_bits:
  "show_rightmost_bits bvec n == rev(take n (rev bvec) )"
  ...
  generate_M':
  "generate_M' mHash salt_new  == (bv_prepend 64 \<zero> []) @ mHash @ salt_new"

  generate_PS:
  "generate_PS emBits hLen == bv_prepend ((roundup emBits 8)*8 - sLen - hLen - 16) \<zero> []"

  generate_DB:
  "generate_DB PS == PS @ [Zero, Zero, Zero, Zero, Zero, Zero, Zero, One] @ salt"

  maskedDB_zero:
  "maskedDB_zero maskedDB emBits == bv_prepend ((roundup emBits 8) * 8 - emBits)  \<zero> (drop ((roundup emBits 8)*8 - emBits) maskedDB)"

  generate_H:
  "generate_H EM emBits hLen == take hLen (drop ((roundup emBits 8)*8 - hLen - 8) EM)"

  generate_maskedDB:
  "generate_maskedDB EM emBits hLen == take ((roundup emBits 8)*8 - hLen - 8) EM"
...
defs
  ...
  emsapss_encode:
  "emsapss_encode M emBits == if (2^64 \<le> length M \<or> 2^32 * 160 < emBits)
                              then []
                              else emsapss_encode_help1 (sha1 M) emBits"

  emsapss_encode_help1:
  "emsapss_encode_help1 mHash emBits == if emBits  < length (mHash) + sLen + 16
                                        then []
                                        else emsapss_encode_help2 (generate_M' mHash salt) emBits"
  emsapss_encode_help2:
  "emsapss_encode_help2 M' emBits == emsapss_encode_help3 (sha1 M') emBits"

  emsapss_encode_help3:
  "emsapss_encode_help3 H emBits == emsapss_encode_help4 (generate_PS emBits (length H)) H emBits"

  emsapss_encode_help4:
  "emsapss_encode_help4 PS H emBits == emsapss_encode_help5 (generate_DB PS) H emBits"

  emsapss_encode_help5:
  "emsapss_encode_help5 DB H emBits == emsapss_encode_help6 DB (MGF H (length DB)) H emBits"

  emsapss_encode_help6:
  "emsapss_encode_help6 DB dbMask H emBits == if dbMask = []
                                              then []
                                              else emsapss_encode_help7 (bvxor DB dbMask) H emBits"

  emsapss_encode_help7:
  "emsapss_encode_help7 maskedDB H emBits == emsapss_encode_help8 (maskedDB_zero maskedDB emBits) H"

  emsapss_encode_help8:
  "emsapss_encode_help8 DBzero H == DBzero @ H @ BC"

  emsapss_decode:
  "emsapss_decode M EM emBits == if (2^64 \<le> length M \<or> 2^32*160<emBits)
                                 then False
                                 else emsapss_decode_help1 (sha1 M) EM emBits"

  emsapss_decode_help1:
  "emsapss_decode_help1 mHash EM emBits == if emBits < length (mHash) + sLen + 16
                                           then False 
                                           else emsapss_decode_help2 mHash EM emBits"

  emsapss_decode_help2:
  "emsapss_decode_help2 mHash EM emBits == if show_rightmost_bits EM 8 \<noteq> BC
                                           then False
                                           else emsapss_decode_help3 mHash EM emBits"

  emsapss_decode_help3:
  "emsapss_decode_help3 mHash EM emBits == emsapss_decode_help4 mHash (generate_maskedDB EM emBits (length mHash)) (generate_H EM emBits (length mHash)) emBits"

  emsapss_decode_help4:
  "emsapss_decode_help4 mHash maskedDB H emBits == if take ((roundup emBits 8)*8 - emBits) maskedDB \<noteq> bv_prepend ((roundup emBits 8)*8 - emBits) \<zero> []
                                                   then False
                                                   else emsapss_decode_help5 mHash maskedDB (MGF H ((roundup emBits 8)*8 - (length mHash) - 8)) H emBits"

  emsapss_decode_help5:
  "emsapss_decode_help5 mHash maskedDB dbMask H emBits == emsapss_decode_help6 mHash (bvxor maskedDB dbMask) H emBits"

  emsapss_decode_help6:
  "emsapss_decode_help6 mHash DB H emBits == emsapss_decode_help7 mHash (maskedDB_zero DB emBits) H emBits"

  emsapss_decode_help7:
  "emsapss_decode_help7 mHash DB_zero H emBits == if (take ( (roundup emBits 8)*8 - (length mHash) - sLen - 16) DB_zero \<noteq> bv_prepend ( (roundup emBits 8)*8 - (length mHash) - sLen - 16) \<zero> []) \<or> (take 8 ( drop ((roundup emBits 8)*8 - (length mHash) - sLen - 16 ) DB_zero ) \<noteq> [Zero, Zero, Zero, Zero, Zero, Zero, Zero, One])
                                                  then False
                                                  else emsapss_decode_help8 mHash DB_zero H"

  emsapss_decode_help8:
  "emsapss_decode_help8 mHash DB_zero H == emsapss_decode_help9 mHash (generate_salt DB_zero) H"

  emsapss_decode_help9:
  "emsapss_decode_help9 mHash salt_new H == emsapss_decode_help10 (generate_M' mHash salt_new) H"

  emsapss_decode_help10:
  "emsapss_decode_help10 M' H == emsapss_decode_help11 (sha1 M') H"

  emsapss_decode_help11:
...
proof -
  have "drop x (bvxor (bv_prepend x \<zero> (drop x (bvxor a b))) b) = drop x a"
    ...
    apply (subgoal_tac "length (replicate x \<zero> @ drop x (bv_mapzip op \<oplus>\<^sub>b a b)) = length b")
    apply (subgoal_tac "b = (take x b)@(drop x b)")
    apply (insert drop_bv_mapzip [of x "(replicate x \<zero> @ drop x (bv_mapzip op \<oplus>\<^sub>b a b))" b "op \<oplus>\<^sub>b"])
    apply (simp)
    apply (insert drop_bv_mapzip [of x a b "op \<oplus>\<^sub>b"])
    apply (simp)
    apply (fold bvxor)
    apply (simp_all)
    done
...
lemma bvxorxor_elim: "\<lbrakk> roundup emBits 8 * 8 - emBits \<le> length a; length a = length b\<rbrakk> \<Longrightarrow> (maskedDB_zero (bvxor (maskedDB_zero (bvxor a b) emBits)b) emBits) = bv_prepend (roundup emBits 8 * 8 - emBits) \<zero> (drop (roundup emBits 8 * 8 - emBits) a)"
  by (simp add: maskedDB_zero bvxorxor_elim_help)
...
lemma verify: "\<lbrakk>(emsapss_encode M emBits) \<noteq> []; EM=(emsapss_encode M emBits)\<rbrakk> \<Longrightarrow> emsapss_decode M EM emBits = True"
  apply (simp add: emsapss_decode emsapss_encode)
  apply (safe, simp+)
  apply (simp add: emsapss_decode_help1 emsapss_encode_help1)
  apply (safe, simp+)
  apply (simp add: emsapss_decode_help2 emsapss_encode_help2)
  apply (safe)
  ...
  apply (simp only: emsapss_encode_help8)
  apply (simp only: maskedDB_zero)
  apply (simp only: take_bv_prepend2 min_max.inf_absorb1)
  apply (simp)
  apply (simp add: emsapss_encode_help5 emsapss_encode_help6)
  apply (safe)
  apply (simp)+
  ...
  apply (simp add: sha1len emsapss_encode_help7 emsapss_encode_help8)
  ...
  apply (simp add: sha1len emsapss_decode_help5)
  apply (simp only: emsapss_decode_help6 emsapss_decode_help7)
