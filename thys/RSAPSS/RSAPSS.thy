...
proof -
  ...
  proof -
    ...
    proof -
      from p have p2: "1<p" by (simp add: is_prime_nat_iff)
      moreover from q have "1<q" by (simp add: is_prime_nat_iff)
...
proof -
  ...
  proof -
    from p have s: "1<p" by (simp add: is_prime_nat_iff)
    moreover from q have "1<q" by (simp add: is_prime_nat_iff)
    ultimately have "p<p*q" by simp 
  ...
  proof (cases "8 dvd ((length (nat_to_bv (p * q))) - Suc 0)")
    ...
    proof -
      ...
      proof -
        have "0<p*q"
        proof -
          have "0<p" using a by (simp add: is_prime_nat_iff)
          moreover have "0<q" using b by (simp add: is_prime_nat_iff)
