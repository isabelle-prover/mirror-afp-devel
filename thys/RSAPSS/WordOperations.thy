(*  Title:      RSAPSS/Wordoperations.thy
    ID:         $Id: WordOperations.thy,v 1.3 2008-04-22 06:34:09 fhaftmann Exp $
...
definition
  addmod32:
  "addmod32 a b = rev (select (rev (nat_to_bv ((bv_to_nat a) + (bv_to_nat b)))) 0 31)"
...
lemma selectlenhelp: "ALL l. length (selecthelp l i) = (i + 1)"
proof
  show "\<And> l. length (selecthelp l i) = i+1"
  proof (induct i)
    fix l
    show "length (selecthelp l 0) = 0 + 1"
    proof (cases l)
      case Nil
      hence "selecthelp l 0 = [Zero]" by (simp)
      thus ?thesis by (simp)
    next
      case (Cons a list)
      hence "selecthelp l 0 = [a]" by (simp)
      thus ?thesis by (simp)
    qed
  next
    fix l
    ...
    proof (cases l)
      case Nil
      hence "selecthelp l (Suc x) = Zero # selecthelp l x" by (simp) 
      thus "length (selecthelp l (Suc x)) = Suc x + 1" using Suc by (simp)
    next
      case (Cons a b)
      hence "selecthelp l (Suc x) = a # selecthelp b x" by simp
      hence "length (selecthelp l (Suc x)) = 1 + length (selecthelp b x)" by simp
      thus "length (selecthelp l (Suc x)) = Suc x + 1" using Suc by simp
    qed
  qed
qed
...
proof (auto)
  ...
  proof (induct i)
    ...
    thus "EX k. select l 0 j = select k 0 (j - (0::nat))" by auto
  next
    case (Suc x)
    have b: "select l (Suc x) j = select (tl l) x (j-(1::nat))"
    proof (cases l)
      case Nil
      hence "select l (Suc x) j = select l x (j-(1::nat))" by simp
      moreover have "tl l = l" using Nil by simp
      ultimately show ?thesis by (simp)
    next
      case (Cons head tail)
      hence "select l (Suc x) j = select tail x (j-(1::nat))" by simp
    ...
    thus "EX k. select l (Suc x) j = select k 0 (j - Suc x)" by simp
  qed
qed
...
proof
  ...
  proof (cases l)
    case Nil
    assume "l=[]"
    thus "select l 0 j = selecthelp l j" by simp
  next
    case (Cons a b)
    thus "select l 0 j = selecthelp l j" by simp
  qed
...
proof -
  ...
  hence "EX k. length (select l i j) = length (select k 0 (j-i))" by auto
  hence c: "EX k. length (select l i j) = length (selecthelp k (j-i))" using selectlenhelp3 by simp
  from c obtain k where d: "length (select l i j) = length (selecthelp k (j-i))" by auto
  have "0<=j-i" by arith
  hence "length (selecthelp k (j-i)) = j-i+1" using selectlenhelp by simp
  thus "length (select l i j) = j-i+1" using d by simp
qed
